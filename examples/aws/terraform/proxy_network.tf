...
resource "aws_security_group_rule" "proxy_ingress_allow_web_acm" {
  ...
  security_group_id = "${aws_security_group.proxy_acm.id}"
  count             = "${var.use_acm ? 1 : 0}"
}
...
resource "aws_lb" "proxy_acm" {
  ...
  idle_timeout        = 3600
  security_groups     = ["${aws_security_group.proxy_acm.id}"]
  count               = "${var.use_acm ? 1 : 0}"
  tags {
    TeleportCluster   = "${var.cluster_name}"
  }
...
resource "aws_lb_listener" "proxy_web_acm" {
  load_balancer_arn = "${aws_lb.proxy_acm.arn}"
  port              = "443"
  protocol          = "HTTPS"
  ...
  default_action {
    target_group_arn = "${aws_lb_target_group.proxy_web_acm.arn}"
    type             = "forward"
  }
}
...
resource "aws_lb_listener" "proxy_grafana_acm" {
  ...
  default_action {
    target_group_arn = "${aws_lb_target_group.proxy_grafana_acm.arn}"
    type             = "forward"
  }
}

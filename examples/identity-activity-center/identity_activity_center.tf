...
resource "aws_glue_catalog_table" "identity_activity_center_table" {
  ...
  storage_descriptor {
    location      = format("s3://%s", aws_s3_bucket.identity_activity_center_long_term_storage.bucket)
...
identity_activity_center:
  ...
  s3:  '${format("s3://%s/data", var.iac_long_term_bucket_name)}'
  s3_results: '${format("s3://%s/results", var.iac_transient_bucket_name)}'
  s3_large_files: '${format("s3://%s/large_files", var.iac_transient_bucket_name)}'

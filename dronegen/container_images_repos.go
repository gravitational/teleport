...
func (cr *ContainerRepo) tagAndPushStep(buildStepDetails *buildStepOutput, imageTags []*ImageTag) (step, map[*ImageTag]*Image) {
	...
	archImageKeys := getImageMapKeys(archImageMap)
...
// This is a helper function for backwards compatibility reasons. This version of Go does not support the maps library,
// or generics, so we implement a specific `maps.Keys` here.
func getImageMapKeys(archImageMap map[*ImageTag]*Image) []*ImageTag {
	archImageKeys := make([]*ImageTag, 0, len(archImageMap))
	for archImageKey := range archImageMap {
		archImageKeys = append(archImageKeys, archImageKey)
	}

	return archImageKeys
}

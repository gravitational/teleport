...
func (rv *ReleaseVersion) buildVersionPipeline(triggerSetupSteps []step, flags *TriggerFlags) pipeline {
	...
	// Corner case that should basically never be hit in practice, but can under very specific backport scenarios
	if len(pipeline.Steps) == len(setupSteps) {
		pipeline.Steps = make([]step, 0)
	}

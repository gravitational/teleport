...
func pushCheckoutCommandsDarwin(artifactConfig darwinArtifactConfig) []string {
	...
	commands = append(commands,
		...
		`cd $WORKSPACE_DIR/go/src/github.com/gravitational/webapps`,
		`GIT_SSH_COMMAND='ssh -i $WORKSPACE_DIR/.ssh/id_rsa -o UserKnownHostsFile=$WORKSPACE_DIR/.ssh/known_hosts -F /dev/null' git submodule update --init --recursive packages/webapps.e || true`,

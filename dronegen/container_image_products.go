...
// Note: Teleport Operator product does not exist in this version, so it has been removed from the pipeline builder.
...
func (p *Product) createBuildStep(arch string, version *ReleaseVersion, publicEcrPullRegistry *ContainerRepo) (step, *buildStepOutput) {
	...
	envVars := cloneEnvVars(publicEcrPullRegistry.EnvironmentVars)
...
// Helper function as this version of Go doesn't container "golang.org/x/exp/maps" for maps.Clone
func cloneEnvVars(envVars map[string]value) map[string]value {
	copiedMap := make(map[string]value, len(envVars))

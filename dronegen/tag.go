...
for file in $(find . -type f ! -iname '*.sha256'); do
  ...
  curl $CREDENTIALS --fail-with-body -o /dev/null -F description="TODO" -F os="%s" -F arch="%s" -F file=@$file -F sha256="$shasum" -F releaseId="$product@$VERSION" $RELEASES_HOST/assets;
done`,

...
for file in $(find . -type f ! -iname '*.sha256'); do
  ...
  status_code=$(curl -o $WORKSPACE_DIR/tmp/curl_out.txt -w "%%{http_code}" -F product=$product -F version=$VERSION -F notesMd="# Teleport $VERSION" -F status=draft $RELEASES_HOST/releases)
  [ $status_code = 200 ] || [ $status_code = 409 ] || (echo "curl HTTP status: $status_code"; cat $WORKSPACE_DIR/tmp/curl_out.txt)

...
of the list to owners who otherwsie do not have access list modification/creation permissions.

Access lists are a simple but powerful tool due to their ease of composition with other tooling. Access lists creation/management
is easy to automate, and they form the natural basis of more advanced access management flows like identity provider group sync.
...
near realtime in `tsh scopes ls` (though they will still need to relogin to acquire newly granted permissions).
...
- **Scope-Aware UI**: The initial MVP will only provide CLI-based scoping features. Follow-up work will add scope-awareness
  to the web UI, including a polished UX for scope pinning and scoped administration.
...
- **Scoped Machine Identites**: Bot/machine-id features will be updated to be scope aware. In particular, we will be looking
  ...
  operations limited to their scope.

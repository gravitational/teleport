...
The [release checklist](https://gravitational.slab.com/posts/teleport-release-checklist-cf0s6vmt) with just a few additional steps:
...
The API can start at `v0`, which implies instability. When we are ready to guarantee the API's backwards compatibility and stability, it can be upgraded to `v1`. I would suggest that we upgrade to `v1` once the old client is completely deprecated and the new client is used internally.

Once a breaking change is added to the `api` module, the major version will be incremented, and the import path will be updated with the major version suffix. Ideally, we would keep it in `v1` for as long as possible in order to avoid changing the import path on releases to include the major version suffix. API major versions will be a special case unlike current Teleport releases.
...
Pros:
 ...
 - Flexibility - The API will only be released when it is necessary to do so. This can be more or less frequently than Teleport, depending on what features are needed and when.
...
The router will use composite keys to match a request to a route. Composite keys were chosen rather than a more sophisticated `Matcher` interface solution due to how easy it is to compare composite keys in go. This strategy should be significantly simpler and more scalable.
...
Composite keys can be used to match several relevant values associated with a request, including `request.user.traits`, `request.user.labels`, `request.state`, etc. If more complex matching mechanisms are required in a future iteration, such as user definable `where` statements, they can be added alongside `Route.Keys` struct and checked separately.
...
Since a simple tree-like solution is not an option, the router will instead loop over the routes available looking for a match with the request's associated composite keys. The first one to match will be used to handle the request, so it is important for users to consider handler priority. 

This brute force solution of ranging over all available routes can be improved by adding a graph traversal solution.
...
In my opinion, we should implement all three options. The higher level options will simply be wrappers around the low level option.

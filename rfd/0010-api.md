...
However, it may be that releasing the API and other futures modules separately (Option 2) will give less headaches at scale, but this is hard for me to measure. For that reason, it would make more sense for the release managers to choose a strategy. So far the consensus is to go with Release Option 1.

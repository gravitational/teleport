...
creating a TUN device (and passing back its file descriptor or [GUID on
...
substitute it with an osascript wrapper that runs the same command as root.
...
"Connect and copy VNet address" opens the VNet panel. If this is the first time that the user starts
VNet, Connect adds a login item for the daemon and prompts the user to enable it in the system
settings, while continuously polling for the daemon status. While the prompt is shown, the VNet
panel is in the "starting" state, explaining what VNet is.

...
By default IPv4 addresses will be assigned from the carrier-grade NAT shared
address space `100.64.0.0/10` defined in [RFC 6598](https://datatracker.ietf.org/doc/html/rfc6598).

This range was chosen to avoid conflicts with public and private network ranges
that are already commonly used, to minimize the chance that VNet IPs and routes
will conflict and require custom configuration.

Another option would be to use a private IP range from [RFC 1918](https://datatracker.ietf.org/doc/html/rfc1918)
(the 10/8, 172.16/12, and 192.168/16 ranges).
However, this are already widely used in local and private networks by many
enterprises and would be more likely to conflict.

There is some prior art for using the CGNAT range for our intended purpose, it
is already in use by other VPN-replacement apps such as

...
The TUN interface will read and write IP packets from the host OS.
To implement TCP we will leverage the userspace networking stack from gVisor
"gvisor.dev/gvisor/pkg/tcpip/stack", which is a Go library available under the
Apache 2.0 license.

The VNET will also host a local DNS service providing local DNS resolution for
all Teleport-protected apps.
DNS will resolve each app's domain name (`public_addr`) to a local IP address
...
If a matching app is found, it will be assigned an IP address in the VNet and
the DNS query will resolve to that address.
...
and searching all clusters for apps with a matching `public_addr` matching,
this would introduce too much latency to unrelated DNS queries.
...
When custom address are desired the Teleport admin can configure a custom DNS
...
likewise for AAAA records and IPv6 addresses.
...
1. If the name matches `*.<custom-dns-zone>`:
    1. VNet will determine which cluster the matching custom DNS zone is in from
       its local state.
    1. VNet will query the cluster to see if any apps have a `spec.vnet_addr`

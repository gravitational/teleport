...
now so I'll leave this out of scope for this RFD.
...
When receiving a query for an A record an IPv4 address will be assigned, IPv6
addresses will be assigned and returned in response to queries for AAAA records.
...
just "Copy VNet address" when VNet is running. Copying the address copies `spec.vnet_addr` if set.
Otherwise, it prioritizes the first custom DNS zone, if none then it copies the `.internal` address.

For TCP apps, the main "Connect" button is the equivalent of "Connect and copy VNet address". From
the three dots menu, the user can select the sole option "Connect to local port". If VNet is active,
the address displayed on the card of a TCP app follows the same rules as the address that is copied
to the clipboard: `spec.vnet_addr` > an address in the first custom DNS zone > an `.internal`
address.
...
message CustomDNSZone {
  string suffix = 1;
  repeated string upstream_nameservers = 2;
}
...
The current `AppSpecV3` message will get a new field to hold the custom VNet
address for apps.

```
// AppSpecV3 is the AppV3 resource spec.
message AppSpecV3 {
  // VnetAddr is the VNet address the application is accessible at.
  string vnet_addr = 9;
}
```

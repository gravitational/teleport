...
For TCP apps (any app with a `spec.uri` starting with `tcp://`) VNet will proxy
all TCP connections directly from clients to the application unaltered.
If the destination app is a TLS server, the client may connect to it over TLS.
Any custom protocol over TCP is supported.

HTTP apps (any app with a `spec.uri` starting with `http://` or `https://`) will be
accessible through VNet over plain HTTP.
They will also continue to be accessible over HTTPS from the browser as they are
today - to support this VNet will detect TLS connections to HTTP app addresses
and forward TCP directly to the Teleport Proxy instead of opening a tunnel all
the way to the app, with authentication and routing handled by the proxy via
cookies sent by the web browser.
Teleport HTTP app access enforces that all proxied connections are actually HTTP
and supports custom header rewriting, this will be the same when connecting
through VNet.
...
process as root.
Alternatives are considered under the
[Alternatives to osascript](#alternatives-to-osascript) section.
The privileged child process will still be the `tsh` binary, just called with
specific arguments to do the host setup.

The admin process will be started in the background by `tsh vnet` when it first
starts, or by Connect when VNet is first turned on (usually when it first opens
after the first time VNet is enabled).
It will watch or poll the contents of the `TELEPORT_HOME` directory for
currently active Teleport profiles.

The process will create clients for all currently logged-in profiles and all of
...
It will then configure the host IP routes and split DNS accordingly.

When the user logs out of a profile and it's deleted from `TSH_HOME`, the
...
The admin process will clean up all configuration and exit when the parent
process exits.
Since the non-root parent cannot send a signal to a root process, the root
process will watch a PID file created by the parent process, and exit when it is
deleted or when the PID in the file is no longer running.
...
Because VNet will expose Teleport protected apps on the local machine over HTTP
without TLS or any per-connection authentication, we need to be careful that
they cannot be reached by untrusted code.
...
- The HTTP app `api.teleport.example.com` is listening on the VNet address `100.64.0.2`.
...
There are a number of possible mitigations for this DNS rebinding attack
scenario.

1. Apply `Host` header allowlists for HTTP apps.
2. Introduce TLS between clients and VNet for HTTP apps.
3. Move HTTP app access out of scope for VNet (only provide access to TCP apps).
4. Make VNet the global DNS nameserver on the host and reject any forwarded
   queries that resolve to a VNet address or CNAME for a VNet name.
5. Use a private IP range for VNet IPv4 addresses.
6. Drop IPv4 and only support IPv6.

`Host` header allowlists for HTTP apps are easy to implement, we will do it.
This means that all HTTP apps will only be allowed to be accessed with a `Host`
header exactly matching the registered app's `spec.public_addr`.
Javascript loaded from `hacker.example.com` would have a mismatched `Host`
header and would not be allowed to access any VNet HTTP apps.
This should not impact most workflows.
Unfortunately it will not protect any HTTP server actually registered as a TCP
app in Teleport because VNet won't be able to assume that all requests can be
parsed as HTTP.

Introducing TLS between clients and VNet for HTTP apps would be tricky and
wouldn't really give us any extra protection against DNS rebinding than `Host`
header allowlists.
The only reason HTTPS apps are generally safe from DNS rebinding attacks is that
the `Host` in the request does not match any valid names for the certificate.
The only way I think we could implement it would be to have VNet create a
self-signed CA and configure it to be trusted on the host, and then issue TLS
certificates for each HTTP app and use those to serve the app.
It would be slow and vulnerable and I (nklaassen) don't think it's worth implementing.

Moving HTTP app access out of scope would hurt VNet as a feature and would
encourage people to register HTTP apps as TCP apps instead, which can't be
protected on the VNet side by a `Host` header allowlist.
I (nklaassen) don't think we should do this.
...
DNS rebinding attacks in all clients.
However, it would require us to parse every recursively resolved DNS response,
which is otherwise unecessary.
...
protections.
This would prevent Javascript from the public IP address from making any
requests to a private IP address.
...
This would not give any additional protection to HTTP apps already covered by a
`Host` header allowlist, but could protect any registered TCP apps from
receiving illegitimate HTTP requests.
I (nklaassen) don't think we should switch to an RFC 1918 range by default because of:
- the limited benefit when combined with `Host` header allowlists
- CGNAT protection appears to be already on its way according to the draft PNA spec
- the likeliness that any RFC 1918 range will conflict with existing customer networks
...
With `Host` header allowlists in place, the only additional protection this
would give us is for HTTP apps registered as TCP apps.
...
"Connect and copy VNet address" opens the VNet panel and shows the root privileges prompt. While the
prompt is shown, the VNet panel is in the "starting" state, explaining what VNet is.

<img width="871" alt="VNet starting" src="assets/0163-vnet-starting.png">

Once VNet starts, the VNet address of the app is copied to clipboard along with an in-app
notification about the clipboard contents. Afterwards the button becomes just "Copy VNet address"
when VNet is running. Copying the address copies `spec.public_addr` but with HTTP as the protocol.
...
When VNet is running and the user logs in to another cluster, the privileged process detects the new
...
The privileged process could be started on system start, separately from Connect. Whenever it'd need
a UI, for example to refresh expired certs, it could open a deep link to bring up Connect.

However, VNet would have to be started from a process that can later communicate with the Electron
app. One way to work around would be to to start Connect in some kind of headless mode, then
boot up tsh daemon along with the privileged process. Otherwise the process that launched VNet on
system start (this does not need to be a privileged process) would need to offer some kind of
communication, perhaps through a gRPC service, so that the Electron app can talk to it after deep
link launch.

Of course it'd also help to solve the issue with asking for root privileges just once, so that the
user wouldn't need to provide credentials on each system start.
...
`tsh vnet` prompts for a password and sets up VNet for all logged-in (non-expired) profiles.

---
authors: Nic Klaassen (nic@goteleport.com), Maja Cieślak (maja.cieslak@goteleport.com)
state: implemented
---
...
1. If an IP address is already assigned to the full queried name, an
   ...
   These IP assignments will not expire for the lifetime of the VNet process.
...
Here is a sequence diagram for an example VNet session:
...
This range can be overridden with a cluster-wide configuration.
...
parse every recursively resolved DNS response, which is otherwise unnecessary.

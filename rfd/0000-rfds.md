...
Always start the RFD with a user experience section where you describe user stories. Every other part of your design - security, scale and privacy - will flow from the UX, not vice-versa.

When designing the UX, consider autonomous AI agents as first-class citizens:

**Start with user stories that describe how AI agents will discover, configure and use the feature without human at each step.**

See "AI Proofing" section below for more details on what to consider.

#### User stories

Explore UI, CLI and API user experience by going through scenarios that users and AI agents would go through while solving specific problems.

In each story, explain specific step-by-step UI, CLI and API requests/responses that the user or an AI agent would observe,
...
**Example 1. An AI agent integrates Okta via CLI and IaC**

An AI agent such as Claude Code, on behalf of its human operator or perhaps running inside a CI/CD pipeline, is tasked with setting up
Okta integration on the Teleport cluster:

_Set up Okta integration with SSO and user sync on my Teleport cluster. Follow Okta integration docs guide at https://goteleport.com/docs/,_
_generate and apply terraform and use appropriate tctl/tsh CLI commands when necessary. My Okta tenant URL is okta123.example.com._

The agent starts by reading Okta integration guide learning the required inputs (Okta tenant URL, API token with correct permission scopes)
and IaC reference for creating the integration.

Following documentation, the agent renders terraform resource definition for creating an Okta integration and applies it to the cluster
(this assumes that the agent has proper terraform env setup for terraform apply to work).

Since the Okta API token is not a part of IaC (to avoid it being saved to the state file), the agent explores available `tctl` CLI, finds
a way to set the API key once the integration has been created and instructs the user to set it.

Once the integration is created, the agent performs verification using `tctl` CLI that shows structured JSON information about the integration
status. The status command shows an error: "Okta API token is missing the okta.users.read scope" which agent relays to the user.

After the user fixes the permission issue, the agent verifies successful status and signals to the user that IaC is good to commit to the
repository.

**Example 2. Alice integrates Okta via UI**
...
Consider AI agents first-class users of Teleport features. Autonomous product evaluations and configurations will very likely
overtake manual human labor over time so every new feature we build must be designed with the agent usage in mind rather than
being retrofit for it later.

Building on the AI user stories, explain in your RFD what changes you will make to ensure the feature can be used by AI agents
and with tools like Claude Code or Codex. A few things to consider below.

#### CLI UX

CLI and API are primary interfaces for agents. Incidentally, they are also much cheaper to implement than elaborate UI
interfaces so prioritize clean CLI that is agent-friendly.

A (non-exhaustive) list of principles to follow when designing CLI for your feature:

- First and most obvious, the CLI should actually exist. Quite often, we would ship functionality that's just not exposed via CLI.
- The CLI should support structured output (`--format=json`). **Stdout is the agent's API.**
  - _Including commands that create/modify things._ Most of the time our existing "create" commands only output text.
- Ensure the command and its CLI flags descriptions are verbose and helpful so an agent can understand what they do.
- Avoid interactive prompts or have a way to explicitly suppress them / provide all necessary information via CLI flags.
- Ensure commands return appropriate error messages under failure conditions that give the agent feedback on what went wrong and how to fix it.
  - _Also ensure that stdout and stderr aren't mixed but that should be handled automatically with our CLI._
- In addition to providing actionable feedback in response to failures, ensure the feature provides some form of status/diagnostics/troubleshooting via CLI.
  - _This is more relevant for various integrations we're building._
- Agents are good at using modular CLIs and figuring out how to string multiple CLI calls together but it can quickly
  get slow and inefficient for moderately complex workflows:
  - _Design responses that return high-signal information._
  - _Looks for opportunities to make the CLI more efficient for agent consumption. E.g. an agent needs to call 3 tctl commands to get_
    _full picture, can I condense it to 1? It's not a big deal if it does once but adds up quickly if it needs to do it 100 times over_
    _in the same workflow._

Get familiar with https://clig.dev/ for general guidelines on writing CLI tools.

In your RFD include a section on the CLI UX and examples of how an agent (or a human) would use it.

#### Agent Skills

When appropriate, or when the workflow for your new feature is moderately complex, consider creating a reference Agent Skill
which would serve as a self-contained script that an AI agent can use to exercise this feature end-to-end:

- https://agentskills.io/
- https://platform.claude.com/docs/en/agents-and-tools/agent-skills/overview

Take a look at [RFD0037e: Skills](https://github.com/gravitational/teleport.e/blob/master/rfd/0037e-skills.md) for
more information on how and why we write agent skills.

In your RFD include a section that describes the skill.
...
Teleport is used at scale in business critical operations at many of the world's
top companies. This means we can no longer afford to avoid thinking about scale
when designing and building new features.

- Consider the breaking points of your new feature. Where are the bottlenecks?
- Make sure that any new APIs are using pagination and not assuming that all
  resources can be returned in a single call. Failing to paginate results in
  slow loading times at best, and broken gRPC APIs or unhealthy caches at worst.

You should also consider the UI.

- Will users have the ability to sort or filter? What indexes
  do you need to maintain in order to make this efficient? At what cost to
  memory?
- How many round trips does your UI need to make to get the data it needs to
  present on screen? Can it be reduced to one?

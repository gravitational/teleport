...
**Complex setup when using an App Service** which must be installed/managed in an EC2 instance with a proper IAM Profile.
...
**AWS Identity Center and AWS External Audit Storage** could use the Roles Anywhere integration (instead of OIDC), however that is currently not being considered.
...
AWS Admin must create another IAM Roles Anywhere Profile, and associate the AWS IAM Roles they whish to provide access to.
...
As with other flows for accessing AWS from Teleport, users must have both:
- App access using the App Label matcher(`app_labels`)
- AWS Role ARN using the `aws_role_arn` Role rule and user trait
...
#### Integration
A new integration resource must be created which supports the synchronization process, with `aws-ra` subkind.
It will include the Trust Anchor ARN which trusts Teleport CA and a synchronization section that contains:
- IAM Roles Anywhere Profile and IAM Role ARNs used to fetch existing AWS Roles Anywhere Profiles
- label matchers which are used to selectively sync profiles
...
spec:
  aws_ra:
    ...
    sync:
      ...
      role_arn: <role arn>
      labels:
        <tag matcher to filter out synced Profiles>
status:
  aws_ra:
    sync:
...
This will be imported as a go module and used to output the expected format.
...
A new gRPC API must be added so that `tsh` can request the AWS credentials.

```proto
// IntegrationService provides methods to manage Integrations with 3rd party APIs.
service IntegrationService {
  // GenerateAWSRACredentials generates AWS credentials for a given Application and IAM Role.
  rpc GenerateAWSRACredentials(GenerateAWSRACredentialsRequest) returns (GenerateAWSRACredentialsResponse);
}

// GenerateAWSRACredentialsRequest contains the fields that represent an AWS Roles Anywhere credentials request.
message GenerateAWSRACredentialsRequest {
  // ApplicationName is the name of the Application, which is derived from the AWS Roles Anywhere Profile name.
  string ApplicationName = 1;

  // IAMRoleARN is the Role ARN the client is requesting access to.
  string IAMRoleARN = 2;
}

// GenerateAWSRACredentialsResponse contains the credentials required to access AWS.
...
message AWSRASyncConfiguration {
  ...
  // Labels is a label matcher for filtering IAM Role Anywhere Profiles based on their tags.

...
1. A modal opens with title "Configure SSH client for VNet"
1. The modal provides instructions to includes VNet's ssh config file in
   `~/.ssh/config` with a button to make Connect handle the configuration
   automatically.

Alice clicks the "Configure Automatically" button in the modal.

The modal closes and a notification appears that reads:
> Connect with any SSH client to `devbox.example.com` (copied to clipboard)
...
```
VNet: access denied to root connecting to devbox
...
```
Include "/Users/nic/Library/Application Support/Teleport Connect/tsh/vnet_ssh_config"
```
...
or run `tsh vnet-ssh-autoconfig`.
...
connections.
This key will only be used for local connections between the third-party SSH
client and the VNet process, it will not be used for any connections to remote addresses.
...
1. VNet will apply any proxy templates found in the user's `TELEPORT_HOME` to
   the FQDN (note: proxy template support is not yet implemented (2025-06-27).
...
1. VNet will set `ssh.ServerConfig.PublicKeyCallback` to a function that:
   ...
   1. Fetches a Teleport user SSH certificate valid for the session, completing
      an MFA ceremony if necessary.
   1. Attempts an SSH connection to the target.
   1. If either of the previous 2 steps fail, a `BannerError` is returned to
      x/crypto/ssh that causes an Auth Banner to be sent to the client with
      information about the error. This is done in `PublicKeyCallback` so that
      authentication failures to the target are returned as authentication
      failures to the client rather than completing the SSH handshake from the
      client successfully and then closing it.

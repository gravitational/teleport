...
- It is impossible to change the schema of an existing message because older or
  newerpeers with different schema definitions will incorrectly determine message
...
cannot be changed due to TDP's inflexible binary format. The omition or addition
...
```                                                                    
                                                   ┌─────────┐ 
 ┌─────────┐      TDPB                             │ Desktop │ 
 │ Browser │  (Proxy Translation)                  │ Agent   │ 
 │ Client  │◄────────────────┐                     │ (Legacy)│ 
 └─────────┘                 ▼                     └─────────┘ 
                           ┌─────────┐  ALPN: (None)    ▲      
                           │         │◄─────────────────┘      
...
single message. The optimization brought on by the PNG 2 mesage is obsoleted
under protobufs.
...
existing `MFAAuthenticationChallenge` and `MFAAthenticateResponse` messages.

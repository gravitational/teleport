...
- Auth Server must run in the AWS management account[^1].
...
This eases the onboarding of new AWS Accounts and regions on those accounts, because it removes the only step besides creating the IAM Role on the AWS Account.
...
INSTALL_SCRIPT_URL="https://proxy.example.com:443/scripts/install.sh"
...
set +x
sudo /usr/local/bin/teleport install autodiscover-node --public-proxy-addr=proxy.example.com:443 --teleport-package=teleport-ent --repo-channel=stable/cloud --auto-upgrade=true --azure-client-id= my-token
`,
			},
		},
	})
...
**For EC2 instances**, it will queue up all the discovered instances.
...
**For RDS databases**, it will create the resources in the backend (TODO!) ...

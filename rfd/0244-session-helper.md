...
The build that embeds the session helper is 11MB larger than the release one (401.4 and 390.5), consistent with the 11MB size of the binary file produced during the build. The helper binary includes ~3MB of sqlite code that we could shave off by moving the manipulation of the wtmpdb database in the main process (which has to include sqlite anyway) or by opportunistically using libwtmpdb instead, but such changes in behavior were not considered for the proof-of-concept.

...
- [RFD 225](https://github.com/gravitational/teleport/pull/59287) - Teleport Extended Release Support
...
> [!IMPORTANT]
> This mechanism is intentionally scoped narrowly to UX gating. Backwards-compatibility and fail-closed behavior are already handled in RFD 228. This behavior should never be determined by feature flags.

## Why
Relying solely on version numbers to determine feature support is fragile, especially in a distributed system with multiple components that can be upgraded independently, and where certain feature may be backported to older versions. A small set of Boolean feature flags provides an explicit, composable signal that the UI or other consumers can intersect across components, avoiding guesswork and making UX conditionality predictable.

## Design
The payload is simple and forward-compatible by design. In v1, features are Boolean. Presence means "supported", absence means "not supported". Each feature is identified by an append-only enum value so IDs remain stable over time. The envelope is versioned, allowing us to introduce a v2 shape later without breaking v1 consumers.
...
Forward compatibility follows the same pattern. New enum values can be appended without affecting older readers and intersections remain safe. If we ever need non-Boolean data or a different shape, we can introduce a `ComponentFeaturesV2` inside the existing envelope while continuing to emit v1 for older readers.

When a feature becomes ubiquitous, components can stop sending it entirely; since absence is interpreted as "not supported", this does not risk accidental enablement.

## Caveats and guidance
Feature flags should represent user-visible or API-visible capabilities rather than internal implementation details. Keep the set small and focused on real UX gating needs. Only advertise a feature when it is both implemented and usable in the current runtime context. This approach is intended as a UX coordination tool, not a substitute for robust server-side compatibility checks and fail-closed behavior.

...
import useKubeTUISession from './useKubeTUISession';

type Props = {
  visible: boolean;
  doc: stores.DocumentKubeTUI;
};

export default function DocumentKubeTUI({ doc, visible }: Props) {
  const terminalRef = useRef<TerminalRef>(undefined);
  const { tty, status, closeDocument } = useKubeTUISession(doc);

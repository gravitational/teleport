...
describe('buttonLockedFeature', () => {
  ...
  test('it has upgrade-community href for community edition', () => {
    ...
    expect(screen.getByText('othertext').closest('a')).toHaveAttribute(
      'href',
      `https://goteleport.com/r/upgrade-community?${version}&utm_campaign=${CtaEvent[CtaEvent.CTA_ACCESS_REQUESTS]
      }`
    );
  });
  ...
  test('it has upgrade-igs href for Enterprise + IGS Plan', () => {
    ...
    expect(screen.getByText('othertext').closest('a')).toHaveAttribute(
      'href',
      `https://goteleport.com/r/upgrade-igs?e_${version}&utm_campaign=${CtaEvent[CtaEvent.CTA_ACCESS_REQUESTS]
      }`
    );
  });

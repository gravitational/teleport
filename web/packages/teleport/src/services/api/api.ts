...
import 'whatwg-fetch';
import auth, { WebAuthnChallengeScope } from 'teleport/services/auth/auth';
...
const api = {
  ...
  async fetchJsonWithMfaAuthnRetry(
    ...
    try {
      webauthnResponseForRetry = await auth.getWebauthnResponse(WebAuthnChallengeScope.ADMIN_ACTION);
    } catch (err) {
      throw new Error(

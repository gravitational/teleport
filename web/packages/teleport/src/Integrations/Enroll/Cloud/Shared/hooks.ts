...
function getIntegrationPrefix(kind: CloudIntegrationKind): 'aws' | 'azure' {
  switch (kind) {
    case IntegrationEnrollKind.AwsCloud:
      return 'aws';
    case IntegrationEnrollKind.AzureCloud:
      return 'azure';
  }
}

function generateIntegrationName(kind: CloudIntegrationKind): string {
  const provider = getIntegrationPrefix(kind);
  const randomHex = Array.from(crypto.getRandomValues(new Uint8Array(4)))
    .map(b => b.toString(16).padStart(2, '0'))
    .join('');
  return `${provider}-integration-${randomHex}`;
}

export function useCloudIntegration(integrationKind: CloudIntegrationKind) {
  const [eventId] = useState(() => crypto.randomUUID());
  const [integrationName, setIntegrationName] = useState(() =>
    generateIntegrationName(integrationKind)
  );
  ...
  const checkIntegration = () => {
    emitEvent(IntegrationEnrollEvent.Step, {
      ...
      status: { code: IntegrationEnrollStatusCode.Success },
    });
    refetch().then(result => {
      if (result.isSuccess) {

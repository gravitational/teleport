...
describe('buildTerraformConfig', () => {
  ...
  test('sets teleport_integration_use_name_prefix = false when name is provided', () => {
    const result = buildTerraformConfig(baseConfig);

    expect(result).toContain('teleport_integration_use_name_prefix = false');
  });

  test('omits teleport_integration_use_name_prefix when name is empty', () => {
    const result = buildTerraformConfig({ ...baseConfig, integrationName: '' });

    expect(result).not.toContain('teleport_integration_use_name_prefix');
  });
});

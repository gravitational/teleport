...
import { AzureScope, AzureTag, VmConfig, WildcardRegion } from './types';
...
export const buildTerraformConfig = ({
  ...
  const resourceGroupOrNull = configScope.resource_group.trim() || null;
  const managedIdentityRegionOrNull =
    configScope.managed_identity_region || null;
...
module "azure_discovery" {
  ...
  azure_resource_group_name = ${resourceGroupOrNull}

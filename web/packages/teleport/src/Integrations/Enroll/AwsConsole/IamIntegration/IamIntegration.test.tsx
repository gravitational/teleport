...
import { parseOutput } from 'teleport/Integrations/Enroll/AwsConsole/IamIntegration/IamIntegration';
...
test.each`
  ...
  ${'invalid empty'}            | ${empty}          | ${false} | ${'Output lines should start with arn:aws:, please double check the output'}
...
`(`parseOutput $name`, ({ input, valid, message }) => {
  ...
  expect(values.trustAnchor).toEqual(
    valid
      ? 'arn:aws:rolesanywhere:eu-west-2:123456789012:trust-anchor/00000000-0000-0000-0000-000000000000'
      : ''
  );
  ...
  expect(values.role).toEqual(
    valid ? 'arn:aws:iam::123456789012:role/MarcoRARoleFromCLI' : ''
  );
});

...
// todo mberg what if I want to edit integration name, should generate command just always be enabled?
// question posed in figma
enum Phase {
  One, // enable step one
...
export function IamIntegration() {
  ...
  const [formState, setFormState] = useState({
    integrationName: '', // required by user, no default
  ...
  function testCfg() {
    // todo mberg send to API when ready
    const inputs = parseOutput(sync);
    console.log(inputs);
    const response = Math.floor(Math.random() * 3);

    if (response == 0) {
      // response says profiles found
      setAlert('success');
      setPhase(Phase.Three);
    } else if (response === 1) {
      //  if response says no profiles were found
      setAlert('warning');
      setPhase(Phase.Three);
    } else {
      // response is error
      // todo use actual api error
      setAlert('danger');
    }
  }

  function testCfgAlert() {
    switch (alert) {
      case 'success':
        return (
      ...
      case 'danger':
        return (
          <Alert kind="danger" details="todo api response error message">
...
export function parseOutput(value: string): OutputParams {
  if (!value) {
    return;
  }
  ...
  return {
    trustAnchor: trustAnchor.replace(/"/g, '').trim(),
    profile: profile.replace(/"/g, '').trim(),

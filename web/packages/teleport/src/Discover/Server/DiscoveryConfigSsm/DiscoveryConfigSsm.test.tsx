...
beforeEach(() => {
  jest.spyOn(discoveryService, 'createDiscoveryConfig');
  jest
...
test('calls with wildcard tag', async () => {
  ...
  completeForm();
  await submitConfig();
  const [[, configArg]] = (discoveryService.createDiscoveryConfig as jest.Mock)
    .mock.calls;
  expect(configArg.aws[0].tags).toEqual({ '*': ['*'] });
});

test('calls with entered tags', async () => {
  const ctx = new TeleportContext();
  ...
  const [[, configArg]] = (discoveryService.createDiscoveryConfig as jest.Mock)
    .mock.calls;
  expect(configArg.aws[0].tags).toEqual({ asdf: ['fdsa'] });
});

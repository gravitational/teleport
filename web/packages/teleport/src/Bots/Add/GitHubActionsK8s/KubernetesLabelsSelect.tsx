...
import Flex, { FlexProps } from 'design/Flex/Flex';
...
type Label = { name: string; values: string[] };
      ...
      >
        <PickerContainer>
          ...
          <ColumnContainer>
            <ColumnHeading>
              Selected Labels (
              {selected.reduce((acc, cur) => acc + cur.values.length, 0)})
            </ColumnHeading>
...
function formatLabel(label: Label) {
  let values = '*';
  if (label.values.length === 1) {
    values = label.values[0];
  } else if (label.values.length) {
    values = `${label.values.join(' or ')}`;
  }
  ...
  return (
    <Component p={0}>
      ...
      >
        ...
        {onAction && (
          <ButtonIcon size={0} onClick={onAction} aria-label={actionAriaLabel}>
            {actionIcon}
          </ButtonIcon>
        )}
      </Flex>

...
describe('KubernetesLabelsSelect', () => {
  ...
  test('manual entry', async () => {
    ...
    const fastType = async (input: HTMLElement, text: string) => {
    ...
    await fastType(nameInput, 'foo');
    await fastType(valueInput, 'bar');
    await pressEnter();
    checkFieldValidationPasses();
    checkForAddedLabel('foo: bar');

    await fastType(nameInput, 'foo2:bar'); // colon in name
    await fastType(valueInput, 'baz');
    await pressEnter();
    checkFieldValidationPasses();
    checkForAddedLabel('foo2:bar: baz');

    await fastType(nameInput, 'foo3');
    await fastType(valueInput, 'bar:baz'); // colon in value
    await user.click(nameInput);
    await pressEnter();
    checkFieldValidationPasses();
    checkForAddedLabel('foo3: bar:baz');

    await fastType(nameInput, 'foo4');
    await fastType(valueInput, 'bar: baz'); // colon and space in value
    await pressEnter();
    checkFieldValidationPasses();
    checkForAddedLabel('foo4: bar: baz');

    await fastType(nameInput, 'env');
    await fastType(valueInput, '*');
    await pressEnter();
    checkFieldValidationPasses();
    checkForAddedLabel('env: *');

    await fastType(nameInput, 'region');
    await fastType(valueInput, '^eu-(west|east)-[0-9]+$');
    await user.click(

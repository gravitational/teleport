...
export default function ConnectDialog({ username, clusterId, dbName, onClose, authType, accessRequestId, dbProtocol, supportsInteractive }: Props) {
  ...
  const onConnect = () => {
    const url = cfg.getDBConnectUrl({ clusterId, name: dbName });
    openNewTab(url)
    onClose()
  }
        ...
        { supportsInteractive && (
          <Box borderTop={1} mb={4} mt={4}>
            <Flex mt={4} flex-direction="row" justifyContent="space-between">
              <Text mt={1} bold>
                Or connect to this database in Web UI
              </Text>

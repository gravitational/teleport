...
import { MfaChallengeScope, type MfaChallengeResponse } from '../mfa/types';
import { session } from '../websession/websession';
...
export const api = {
  get<T extends object>(
    ...
    return api.fetchJsonWithMfaAuthnRetry<T>(
      url,
      { signal: abortSignal },
      mfaResponse
    );
  ...
  deleteWithHeaders<T extends object>(
    url: string,
   ...
   */
  async fetchJsonWithMfaAuthnRetry<T extends object>(
    url: string,
    ...
    try {
      ...
      return await api.getJsonFromFetchResponse<T>(response);
    } catch (err) {
  ...
  async getJsonFromFetchResponse<T extends object>(
    response: Response
  ): Promise<T> {
    let json;
    try {
    ...
    const messages =
      'messages' in json ? (json.messages as string[]) : undefined;
  ...
  async fetch(
    ...
    if (mfaResponse) {
      (options.headers as Record<string, string>)[MFA_HEADER] = JSON.stringify({
        ...mfaResponse,
...
function isAdminActionRequiresMfaError(err: unknown) {
  if (err instanceof Error) {

...
export const auth = {
  ...
  login(userId: string, password: string, otpCode: string) {
    ...
    return api.post<LoginResponse>(cfg.api.webSessionPath, data);
  },
  ...
  loginWithWebauthn(creds?: UserCredentials) {
    return auth
      ...
      .then(res => {
        if (!res) {
          throw new Error('no webauthn response');
        }
        ...
        return api.post<LoginResponse>(cfg.api.mfaLoginFinish, request);
      });
  },
  ...
  async getMfaChallengeResponse(
    challenge: MfaAuthenticateChallenge | undefined,
    mfaType?: DeviceType,
    totpCode?: string
  ...
  async getWebAuthnChallengeResponse(
    webauthnPublicKey: PublicKeyCredentialRequestOptions | undefined
  ): Promise<MfaChallengeResponse> {

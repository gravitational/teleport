...
import {
  base64urlToBuffer,
  bufferToBase64url,
} from 'shared-new/utils/base64url-arraybuffer';
...
export function parseMfaRegistrationChallengeJson(
  ...
  return {
    qrCode: challenge.totp?.qrCode ?? null,
...
export function makeWebauthnCreationResponse(
  ...
  const publicKey = cred as PublicKeyCredential | null;
  ...
  const clientExtensions = publicKey.getClientExtensionResults();
  ...
  return {
    ...
    type: cred.type,
    extensions: {
      appid: Boolean(clientExtensions.appid),
      credProps: clientExtensions.credProps,
    },
  ...
  return {
    ...
    response: {
      ...
      userHandle: bufferToBase64url(
        assertionResponse.userHandle ?? new ArrayBuffer(0)
      ),
    },
  };
}

    ...
    <Wrapper fitContent={fitContent} size={size} appearance={appearance}>
      ...
      >
        {tabs.map((tabSpec, tabIndex) => {
          ...
          const tabName =
            typeof tabSpec === 'string' ? tabSpec : tabSpec.content;
          const controls =
            typeof tabSpec === 'string' ? undefined : tabSpec.controls;
          const tabId = typeof tabSpec === 'string' ? tabIndex : tabSpec.id;
...
/**
 * Definition of a tab. If it's a string, it denotes a name displayed on the

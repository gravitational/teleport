  ...
  return (
    <>
      <Box>
        ...
        <InfoParagraph>
          {getStatusDescription(resource, unhealthyOrUnknownServers.length)}
        </InfoParagraph>
        <InfoParagraph mb={4}>
          <ButtonBorder
            as="a"
          ...
          {attempt.status === 'success' && servers?.length > 0 && (
            <Box
              css={`
                ...
                // for the root container, we want the contents
                // to render flushed against the side of box.
...
function getStatusDescription(
  ...
  if (resource.kind === 'db') {
    ...
    const servicedWord = pluralize(numUnhealthyServers, 'service');

...
const AWSConsoleConstraintsList = <T extends PendingListItem>({
  ...
  <Flex flexDirection="column" gap={2} mt={1} width="100%">
    ...
    <Flex flexDirection="column" width="100%">
      {item.constraints.AWSConsole.RoleARNs.map((arn, idx) => (
        <Flex
          key={arn}
          flexDirection="row"
          ...
          gap={2}
          borderBottom={
            idx !== item.constraints.AWSConsole.RoleARNs.length - 1 ? 1 : 0
          }
...
export function RequestCheckout<T extends PendingListItem>({
  ...
  const renderAfter = (item: DisplayRow<T>) => {
    if (hasResourceConstraints(item, ConstraintDomain.AWS_CONSOLE)) {
      return (

...
>({
  ...
  if (
    !grouping?.groupedByAccessList?.[grouping?.recommendedAccessList]?.length
  ) {
    return (
      <Alert kind="danger" vertical>
...
>(
  ...
  const optimalGrouping =
    grouping.groupedByAccessList?.[grouping.recommendedAccessList];
  ...
  return pendingRequests.filter(
    item =>
      ...
      !optimalGrouping.some(i =>
        item.kind === 'node' ? item.id === i.name : item.name === i.name
      )
  );
};
...
>(
  ...
  const groupings = Object.values(grouping.groupedByAccessList || {}).flat();
...
>({
  ...
  return pendingAccessRequests.every(r => {
    ...
    const id = r.kind === 'node' ? r.id : r.name;
    const key = `${r.kind}:${id}`;

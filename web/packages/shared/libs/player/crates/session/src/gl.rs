//! WebGL2 painter for the framebuffer.
//!
...
//! textured fullscreen quad to the default framebuffer (the
//! OffscreenCanvas drawing buffer). The quad geometry comes from
//! `gl_VertexID` so we don't need any vertex buffer.
//!
...
use web_sys::{
    OffscreenCanvas, VideoFrame, WebGl2RenderingContext as Gl, WebGlProgram, WebGlShader,
...
impl GlPainter {
    pub fn new(canvas: &OffscreenCanvas, width: u32, height: u32) -> Result<Self> {
    ...
    pub fn resize(&mut self, width: u32, height: u32) -> Result<()> {
        ...
        let canvas = self
            .gl
            .canvas()
            .ok_or_else(|| anyhow!("WebGL2 context lost its canvas"))?
            .dyn_into::<OffscreenCanvas>()
            .map_err(|_| anyhow!("WebGL2 canvas is not OffscreenCanvas"))?;
    ...
    /// framebuffer into the (viewport-sized) texture. Does NOT draw — `render`
    /// uploads every dirty rect for the frame and then calls [`GlPainter::draw`]
    /// once, so a multi-rect dirty set issues a single fullscreen-quad pass
    /// instead of one draw per rect.
    ///
    ...
    #[expect(clippy::too_many_arguments, reason = "explicit src/dst rect avoids a wrapper struct")]
    pub fn upload_rect(
        &self,
        framebuffer: &[u8],
        fb_width: u32,
    ...
    /// Upload a decoded `VideoFrame` straight into the texture at `(dst_x,
    /// dst_y)` and draw — GPU→GPU, no CPU readback (and the browser does the
    /// YUV→RGB conversion). The frame's display size determines the uploaded
    /// region. This is the fast path for AVC/H.264 video: it replaces the
    /// `copyTo` (GPU→CPU readback) + CPU copies + `tex_sub_image` re-upload.
    pub fn upload_video_frame_and_draw(
        &self,
        frame: &VideoFrame,
        dst_x: i32,
        dst_y: i32,
        ...
        self.gl
            ...
            .map_err(|e| anyhow!("texSubImage2D(VideoFrame) failed: {e:?}"))?;
        self.gl.draw_arrays(Gl::TRIANGLES, 0, 6);
        self.presented.set(true);
        Ok(())
    }

...
!macro customInstall
  ${If} $installMode == "all"
    ...
    nsExec::ExecToStack '"$INSTDIR\resources\bin\tsh.exe" privileged-updater-install-service'
    Pop $0 # ExitCode
    Pop $1 # Output
    ${If} $0 != 0
        MessageBox MB_ICONSTOP \
            "tsh.exe windows-install-update-service failed with exit code $0. The installer is going to continue. Output: $1"
    ${Endif}
...
!macro customUnInstall
  ${If} $installMode == "all"
    ...
    nsExec::ExecToStack '"$INSTDIR\resources\bin\tsh.exe" privileged-updater-uninstall-service'
    Pop $0 # ExitCode
    Pop $1 # Output
    ${If} $0 != 0
        MessageBox MB_ICONSTOP \

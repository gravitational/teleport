...
describe('buildPtyOptions', () => {
  ...
  it("Teleport Connect env variables are prepended to the user's WSLENV for wsl.exe", async () => {
    ...
    expect(processOptions.env.WSLENV).toBe(
      'CUSTOM_VAR:KUBECONFIG/p:TERM_PROGRAM:TERM_PROGRAM_VERSION:COLORTERM:TELEPORT_CLUSTER:TELEPORT_PROXY:TELEPORT_HOME/p:TELEPORT_TOOLS_VERSION:TELEPORT_AUTH_SERVER'
    );
  });

...
export function useActiveDesktopSession(): DesktopSessionStatusBarState | null {
  const { activeDesktopSessionService } = useAppContext();
  return useSyncExternalStore(
    useCallback(
      listener => activeDesktopSessionService.subscribe(listener),
      [activeDesktopSessionService]
    ),
    () => activeDesktopSessionService.getState()
  );
}

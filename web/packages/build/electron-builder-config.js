 ...
 * @param {string} options.buildResourcesPath - Path to the folder with build resources, relative to the
 * final config file.
 * @returns { import('electron-builder').Configuration }
 */
function createDefaultConfig({ buildResourcesPath }) {
  ...
  const buildResources = resourcePath =>
    path.resolve(path.join(buildResourcesPath, resourcePath));
  ...
  return {
    ...
    mac: {
      ...
      icon: buildResources('icon-mac.png'),
    ...
    win: {
      ...
      icon: buildResources('icon-win.ico'),
      extraResources: [
        env.CONNECT_TSH_BIN_PATH && {
          from: env.CONNECT_TSH_BIN_PATH,
          to: './bin/tsh.exe',
    ...
    rpm: {
      ...
      afterInstall: buildResources('linux/after-install.tpl'),
      afterRemove: buildResources('linux/after-remove.tpl'),
    ...
    linux: {
      ...
      icon: buildResources('icon-linux'),
      extraResources: [
        env.CONNECT_TSH_BIN_PATH && {
          from: env.CONNECT_TSH_BIN_PATH,
          to: './bin/tsh',

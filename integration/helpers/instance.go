...
func (i *TeleInstance) GenerateConfig(t *testing.T, trustedSecrets []*InstanceSecrets, tconf *servicecfg.Config) (*servicecfg.Config, error) {
	...
	tconf.SSH.PublicAddrs = []netutils.Addr{
		{
			AddrNetwork: "tcp",
			Addr:        Loopback,
		},
	...
	tconf.Auth.PublicAddrs = []netutils.Addr{
		{
			AddrNetwork: "tcp",
			Addr:        i.Hostname,
		},
	}
	tconf.Proxy.PublicAddrs = []netutils.Addr{
		{
			AddrNetwork: "tcp",
			Addr:        i.Web,
		},
	...
	if i.IsSinglePortSetup {
		...
		tconf.Proxy.ReverseTunnelListenAddr = netutils.Addr{}
		tconf.Proxy.MySQLAddr = netutils.Addr{}
		tconf.Proxy.SSHAddr = netutils.Addr{}
	} else {
...
func (i *TeleInstance) StartApps(configs []*servicecfg.Config) ([]*service.TeleportProcess, error) {
	...
	for _, conf := range configs {
		go func(cfg *servicecfg.Config) {
			...
			cfg.DataDir = dataDir
			cfg.SetAuthServerAddress(netutils.Addr{
				AddrNetwork: "tcp",
				Addr:        i.Web,
			})
			cfg.SetToken("token")

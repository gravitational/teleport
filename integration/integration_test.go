...
func TestIntegrationsParallel(t *testing.T) {
	...
	t.Run("BPFInteractive", newSuite(t).bind(testBPFInteractive))
	t.Run("BPFSessionDifferentiation", newSuite(t).bind(testBPFSessionDifferentiation))
	...
	t.Run("DiscoveryNode", newSuite(t).bind(testDiscoveryNode))
	t.Run("DiscoveryRecovers", newSuite(t).bind(testDiscoveryRecovers))
	t.Run("EnvironmentVars", newSuite(t).bind(testEnvironmentVariables))
	t.Run("ExecEvents", newSuite(t).bind(testExecEvents))
	t.Run("ExternalClient", newSuite(t).bind(testExternalClient))
	t.Run("HA", newSuite(t).bind(testHA))
	t.Run("Interactive (Regular)", newSuite(t).bind(testInteractiveRegular))
	t.Run("Interactive (Reverse Tunnel)", newSuite(t).bind(testInteractiveReverseTunnel))
	t.Run("Interoperability", newSuite(t).bind(testInteroperability))
	t.Run("InvalidLogin", newSuite(t).bind(testInvalidLogins))
	t.Run("IP Propagation", newSuite(t).bind(testIPPropagation))
	t.Run("JumpTrustedClusters", newSuite(t).bind(testJumpTrustedClusters))
	t.Run("JumpTrustedClustersWithLabels", newSuite(t).bind(testJumpTrustedClustersWithLabels))
	t.Run("List", newSuite(t).bind(testList))
	t.Run("MapRoles", newSuite(t).bind(testMapRoles))
	t.Run("ModeratedSessions", newSuite(t).bind(testModeratedSessions))
	t.Run("MultiplexingTrustedClusters", newSuite(t).bind(testMultiplexingTrustedClusters))
	t.Run("PAM", newSuite(t).bind(testPAM))
	t.Run("PortForwarding", newSuite(t).bind(testPortForwarding))
	t.Run("ProxyHostKeyCheck", newSuite(t).bind(testProxyHostKeyCheck))
	t.Run("ReverseTunnelCollapse", newSuite(t).bind(testReverseTunnelCollapse))
	t.Run("RotateRollback", newSuite(t).bind(testRotateRollback))
	t.Run("RotateSuccess", newSuite(t).bind(testRotateSuccess))
	t.Run("RotateTrustedClusters", newSuite(t).bind(testRotateTrustedClusters))
	t.Run("SessionStartContainsAccessRequest", newSuite(t).bind(testSessionStartContainsAccessRequest))
	t.Run("SessionStreaming", newSuite(t).bind(testSessionStreaming))
	t.Run("SSHExitCode", newSuite(t).bind(testSSHExitCode))
	t.Run("Shutdown", newSuite(t).bind(testShutdown))
	t.Run("TrustedClusters", newSuite(t).bind(testTrustedClusters))
	t.Run("TrustedDisabledClusters", newSuite(t).bind(testDisabledTrustedClusters))
	t.Run("TrustedClustersRoleMapChanges", newSuite(t).bind(testTrustedClustersRoleMapChanges))
	...
	t.Run("ListResourcesAcrossClusters", newSuite(t).bind(testListResourcesAcrossClusters))
	t.Run("SessionRecordingModes", newSuite(t).bind(testSessionRecordingModes))
	t.Run("DifferentPinnedIP", newSuite(t).bind(testDifferentPinnedIP))
	t.Run("JoinOverReverseTunnelOnly", newSuite(t).bind(testJoinOverReverseTunnelOnly))
	t.Run("SFTP", newSuite(t).bind(testSFTP))
	t.Run("EscapeSequenceTriggers", newSuite(t).bind(testEscapeSequenceTriggers))
	t.Run("AuthLocalNodeControlStream", newSuite(t).bind(testAuthLocalNodeControlStream))
	t.Run("AgentlessConnection", newSuite(t).bind(testAgentlessConnection))
	t.Run("LeafAgentlessConnection", newSuite(t).bind(testTrustedClusterAgentless))
}

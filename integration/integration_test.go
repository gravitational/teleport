...
func testShutdown(t *testing.T, suite *integrationTestSuite) {
	...
	require.Eventually(t, func() bool {
		teleport.GetPortReverseTunnel()
		// TODO: check that we either get a connection that fully works or a connection refused error
		c, err := net.DialTimeout("tcp", net.JoinHostPort(teleport.Hostname, teleport.GetPortReverseTunnel()), 250*time.Millisecond)
		if err != nil {
			require.Contains(t, trace.Unwrap(err).Error(), "connection refused")
			return true
		}

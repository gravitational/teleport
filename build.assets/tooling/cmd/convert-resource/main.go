...
var resourceConfig = map[string]conversionRule{
	...
	"bot": {
		toTerraformResource: func(data []byte) (tfgen.Resource, error) {
			...
			// tfgen.Generate silently drops the scope.
			if r.Scope != "" {
				return nil, trace.Errorf("converting tctl resources to HCL does not yet support scoped bot resources")
			}
		...
		generateOpts: []tfgen.GenerateOpt{tfgen.WithFieldTransform("spec.traits", transform.BotTraits)},
	},
	"autoupdate_config": {
		toTerraformResource: func(data []byte) (tfgen.Resource, error) {
	...
	"workload_identity": {
		toTerraformResource: func(data []byte) (tfgen.Resource, error) {
			...
			// tfgen.Generate silently drops the scope.
			if r.Scope != "" {
				return nil, trace.Errorf("converting tctl resources to HCL does not yet support scoped workload_identity resources")
			}
	...
	"access_list": {
		toTerraformResource: func(data []byte) (tfgen.Resource, error) {
			...
			// tfgen.Generate silently drops the scope.
			if al.Scope != "" {
				return nil, trace.Errorf("converting tctl resources to HCL does not yet support scoped access_list resources")
			}

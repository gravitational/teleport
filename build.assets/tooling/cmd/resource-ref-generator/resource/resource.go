...
func ReferenceDataFromDeclaration(prefix string, decl DeclarationInfo, allDecls map[PackageInfo]DeclarationInfo) (map[PackageInfo]ReferenceEntry, error) {
	...
	for _, f := range fieldsToProcess {
		...
		for _, d := range c {
			...
			r, err := ReferenceDataFromDeclaration(prefix, gd, allDecls)

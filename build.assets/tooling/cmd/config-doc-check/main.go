	...
	for _, section := range config.ServiceSections {
		...
		if err != nil {
			configErrors = append(configErrors, fmt.Errorf(
				"warning: cannot read the configuration example %s: %w", section.ExamplePath, err))
			continue
		}

		var unmarshaledExample map[string]any
		if err := yaml.Unmarshal(preprocessExampleYAML(data), &unmarshaledExample); err != nil {
			configErrors = append(configErrors, fmt.Errorf("warning: cannot parse the configuration example %s: %w", section.ExamplePath, err))
			continue
		}
		...
		for _, pair := range section.KeyTypePairs {
			...
			if err != nil {
				configErrors = append(configErrors, fmt.Errorf("warning: cannot inspect config type %s: %w", pair.TypeName, err))
				failedToProcessSection = true
				continue
			}

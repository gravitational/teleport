...
// pkgs represent package references to testify.suite and testing
type pkgs struct {
	// testing "testing" package name
	testing string

	// testifySuite stretchr/testify/suite package name
	testifySuite string
}
...
func findTestPkgImports(f *ast.File) pkgs {
	p := pkgs{}
...
func getMethodReceiver(fset *token.FileSet, ret *ast.FuncDecl) string {
	// If a method has a receiver
	if ret.Recv == nil || len(ret.Recv.List) == 0 {
		return ""
	}

	var b bytes.Buffer
	printer.Fprint(&b, fset, ret.Recv.List[0].Type)

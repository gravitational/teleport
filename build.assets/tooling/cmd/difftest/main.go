...
var (
	...
	doubleDollar   = testCmd.Flag("double-dollar", "Output $ as $$").Short('d').Bool()
)
...
func diff(repoPath string, ref string, changedFiles []string, elapsed time.Duration) {
	...
	inspect(repoPath, ref, changedFiles, func(filename string, r CompareResult) {
...
func test(repoPath string, ref string, changedFiles []string) {
	...
	inspect(repoPath, ref, changedFiles, func(filename string, r CompareResult) {
		...
		for _, n := range r.New {
			methods = append(methods, "^"+n.RefName+dollar)
			dirs[relDirName(filename, *relative)] = struct{}{}
		}
...
func inspect(repoPath string, ref string, changedFiles []string, fn func(string, CompareResult)) {
...
func relDirName(filename string, relative string) string {
	f := filename

...
@test "[${TEST_SUITE?}] proxy_service.kube_public_addr is not set" {
    load ${TELEPORT_CONFD_DIR?}/conf
    echo "${PROXY_BLOCK?}"
    # this test inverts the regular behaviour of grep -q, so only succeeds if the line _isn't_ present
    echo "${PROXY_BLOCK?}" | { ! grep -qE "^  kube_public_addr: "; }
}

@test "[${TEST_SUITE?}] proxy_service.kube_listen_addr is not set" {
    load ${TELEPORT_CONFD_DIR?}/conf
    echo "${PROXY_BLOCK?}"
    # this test inverts the regular behaviour of grep -q, so only succeeds if the line _isn't_ present
    echo "${PROXY_BLOCK?}" | { ! grep -qE "^  kube_listen_addr: 0.0.0.0:3026"; }
}
...
@test "[${TEST_SUITE?}] proxy_service.proxy_protocol is set to on" {
    load ${TELEPORT_CONFD_DIR?}/conf
    echo "${PROXY_BLOCK?}"

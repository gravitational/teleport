...
: "${1:?teleport_version was not provided}"
    ...
    zypper "${ZYPPER_FLAGS[@]}" install --allow-downgrade --oldpackage "teleport-${teleport_version}.$(rpm --eval '%{_arch}').rpm"
    verify_teleport "${teleport_version}"
    echo "successfully installed teleport teleport-${teleport_version}.$(rpm --eval '%{_arch}').rpm"
}

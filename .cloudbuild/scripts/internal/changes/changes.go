...
// Package changes analyses the code changes in a PR by examining the changes
// between the tip of a PR and its target branch
package changes
...
func getChanges(repo *git.Repository, targetBranch, commit string) (object.Changes, error) {
	log.Printf("Getting filetree for target branch %s", targetBranch)
	...
	log.Printf("Comparing commit %q with target %q", commit, targetBranch)

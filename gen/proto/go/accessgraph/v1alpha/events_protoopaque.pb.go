//
// Teleport
// Copyright (C) 2024  Gravitational, Inc.
//
// This program is free software: you can redistribute it and/or modify
// it under the terms of the GNU Affero General Public License as published by
// the Free Software Foundation, either version 3 of the License, or
// (at your option) any later version.
//
// This program is distributed in the hope that it will be useful,
// but WITHOUT ANY WARRANTY; without even the implied warranty of
// MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
// GNU Affero General Public License for more details.
//
// You should have received a copy of the GNU Affero General Public License
// along with this program.  If not, see <http://www.gnu.org/licenses/>.

// Code generated by protoc-gen-go. DO NOT EDIT.
// versions:
// 	protoc-gen-go v1.36.11
// 	protoc        (unknown)
// source: accessgraph/v1alpha/events.proto

//go:build protoopaque

package accessgraphv1alpha

import (
	v1 "github.com/gravitational/teleport/api/gen/proto/go/teleport/header/v1"
	protoreflect "google.golang.org/protobuf/reflect/protoreflect"
	protoimpl "google.golang.org/protobuf/runtime/protoimpl"
	reflect "reflect"
	unsafe "unsafe"
)

const (
	// Verify that this generated code is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(20 - protoimpl.MinVersion)
	// Verify that runtime/protoimpl is sufficiently up-to-date.
	_ = protoimpl.EnforceVersion(protoimpl.MaxVersion - 20)
)

// AccessPathChanged is an event that is emitted when an access path is changed.
type AccessPathChanged struct {
	state                             protoimpl.MessageState `protogen:"opaque.v1"`
	xxx_hidden_Metadata               *v1.Metadata           `protobuf:"bytes,1,opt,name=metadata,proto3"`
	xxx_hidden_ChangeId               string                 `protobuf:"bytes,2,opt,name=change_id,json=changeId,proto3"`
	xxx_hidden_AffectedResourceName   string                 `protobuf:"bytes,3,opt,name=affected_resource_name,json=affectedResourceName,proto3"`
	xxx_hidden_AffectedResourceSource string                 `protobuf:"bytes,4,opt,name=affected_resource_source,json=affectedResourceSource,proto3"`
	xxx_hidden_AffectedResourceKind   string                 `protobuf:"bytes,5,opt,name=affected_resource_kind,json=affectedResourceKind,proto3"`
	unknownFields                     protoimpl.UnknownFields
	sizeCache                         protoimpl.SizeCache
}

func (x *AccessPathChanged) Reset() {
	*x = AccessPathChanged{}
	mi := &file_accessgraph_v1alpha_events_proto_msgTypes[0]
	ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
	ms.StoreMessageInfo(mi)
}

func (x *AccessPathChanged) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*AccessPathChanged) ProtoMessage() {}

func (x *AccessPathChanged) ProtoReflect() protoreflect.Message {
	mi := &file_accessgraph_v1alpha_events_proto_msgTypes[0]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
			ms.StoreMessageInfo(mi)
		}
		return ms
	}
	return mi.MessageOf(x)
}

func (x *AccessPathChanged) GetMetadata() *v1.Metadata {
	if x != nil {
		return x.xxx_hidden_Metadata
	}
	return nil
}

func (x *AccessPathChanged) GetChangeId() string {
	if x != nil {
		return x.xxx_hidden_ChangeId
	}
	return ""
}

func (x *AccessPathChanged) GetAffectedResourceName() string {
	if x != nil {
		return x.xxx_hidden_AffectedResourceName
	}
	return ""
}

func (x *AccessPathChanged) GetAffectedResourceSource() string {
	if x != nil {
		return x.xxx_hidden_AffectedResourceSource
	}
	return ""
}

func (x *AccessPathChanged) GetAffectedResourceKind() string {
	if x != nil {
		return x.xxx_hidden_AffectedResourceKind
	}
	return ""
}

func (x *AccessPathChanged) SetMetadata(v *v1.Metadata) {
	x.xxx_hidden_Metadata = v
}

func (x *AccessPathChanged) SetChangeId(v string) {
	x.xxx_hidden_ChangeId = v
}

func (x *AccessPathChanged) SetAffectedResourceName(v string) {
	x.xxx_hidden_AffectedResourceName = v
}

func (x *AccessPathChanged) SetAffectedResourceSource(v string) {
	x.xxx_hidden_AffectedResourceSource = v
}

func (x *AccessPathChanged) SetAffectedResourceKind(v string) {
	x.xxx_hidden_AffectedResourceKind = v
}

func (x *AccessPathChanged) HasMetadata() bool {
	if x == nil {
		return false
	}
	return x.xxx_hidden_Metadata != nil
}

func (x *AccessPathChanged) ClearMetadata() {
	x.xxx_hidden_Metadata = nil
}

type AccessPathChanged_builder struct {
	_ [0]func() // Prevents comparability and use of unkeyed literals for the builder.

	// metadata is a common event metadata
	Metadata *v1.Metadata
	// change_id is the id of the change.
	ChangeId string
	// affected_resource_name is the name of the affected resource.
	AffectedResourceName string
	// affected_resource_source is the source of the affected resource, ex: Teleport, AWS, GitLab, etc.
	AffectedResourceSource string
	// affected_resource_kind is the kind of the affected resource, ex: user, role, etc.
	AffectedResourceKind string
}

func (b0 AccessPathChanged_builder) Build() *AccessPathChanged {
	m0 := &AccessPathChanged{}
	b, x := &b0, m0
	_, _ = b, x
	x.xxx_hidden_Metadata = b.Metadata
	x.xxx_hidden_ChangeId = b.ChangeId
	x.xxx_hidden_AffectedResourceName = b.AffectedResourceName
	x.xxx_hidden_AffectedResourceSource = b.AffectedResourceSource
	x.xxx_hidden_AffectedResourceKind = b.AffectedResourceKind
	return m0
}

var File_accessgraph_v1alpha_events_proto protoreflect.FileDescriptor

const file_accessgraph_v1alpha_events_proto_rawDesc = "" +
	"\n" +
	" accessgraph/v1alpha/events.proto\x12\x13accessgraph.v1alpha\x1a!teleport/header/v1/metadata.proto\"\x90\x02\n" +
	"\x11AccessPathChanged\x128\n" +
	"\bmetadata\x18\x01 \x01(\v2\x1c.teleport.header.v1.MetadataR\bmetadata\x12\x1b\n" +
	"\tchange_id\x18\x02 \x01(\tR\bchangeId\x124\n" +
	"\x16affected_resource_name\x18\x03 \x01(\tR\x14affectedResourceName\x128\n" +
	"\x18affected_resource_source\x18\x04 \x01(\tR\x16affectedResourceSource\x124\n" +
	"\x16affected_resource_kind\x18\x05 \x01(\tR\x14affectedResourceKindBWZUgithub.com/gravitational/teleport/gen/proto/go/accessgraph/v1alpha;accessgraphv1alphab\x06proto3"

var file_accessgraph_v1alpha_events_proto_msgTypes = make([]protoimpl.MessageInfo, 1)
var file_accessgraph_v1alpha_events_proto_goTypes = []any{
	(*AccessPathChanged)(nil), // 0: accessgraph.v1alpha.AccessPathChanged
	(*v1.Metadata)(nil),       // 1: teleport.header.v1.Metadata
}
var file_accessgraph_v1alpha_events_proto_depIdxs = []int32{
	1, // 0: accessgraph.v1alpha.AccessPathChanged.metadata:type_name -> teleport.header.v1.Metadata
	1, // [1:1] is the sub-list for method output_type
	1, // [1:1] is the sub-list for method input_type
	1, // [1:1] is the sub-list for extension type_name
	1, // [1:1] is the sub-list for extension extendee
	0, // [0:1] is the sub-list for field type_name
}

func init() { file_accessgraph_v1alpha_events_proto_init() }
func file_accessgraph_v1alpha_events_proto_init() {
	if File_accessgraph_v1alpha_events_proto != nil {
		return
	}
	type x struct{}
	out := protoimpl.TypeBuilder{
		File: protoimpl.DescBuilder{
			GoPackagePath: reflect.TypeOf(x{}).PkgPath(),
			RawDescriptor: unsafe.Slice(unsafe.StringData(file_accessgraph_v1alpha_events_proto_rawDesc), len(file_accessgraph_v1alpha_events_proto_rawDesc)),
			NumEnums:      0,
			NumMessages:   1,
			NumExtensions: 0,
			NumServices:   0,
		},
		GoTypes:           file_accessgraph_v1alpha_events_proto_goTypes,
		DependencyIndexes: file_accessgraph_v1alpha_events_proto_depIdxs,
		MessageInfos:      file_accessgraph_v1alpha_events_proto_msgTypes,
	}.Build()
	File_accessgraph_v1alpha_events_proto = out.File
	file_accessgraph_v1alpha_events_proto_goTypes = nil
	file_accessgraph_v1alpha_events_proto_depIdxs = nil
}

...
type ClientApplicationServiceClient interface {
	...
	// ReissueDBCert issues a new database certificate. The database protocol is
	// looked up from the cluster and returned alongside the cert so the caller
	// can configure the correct ALPN protocol for the local proxy.
	ReissueDBCert(ctx context.Context, in *ReissueDBCertRequest, opts ...grpc.CallOption) (*ReissueDBCertResponse, error)
	// SignForDB issues a signature with the private key associated with a
	// certificate previously issued by ReissueDBCert.

...
type ResolveFQDNResponse struct {
	...
	//	*ResolveFQDNResponse_MatchedRejected
...
func (x *ResolveFQDNResponse) GetMatchedRejected() *MatchedRejected {
	if x != nil {
		if x, ok := x.Match.(*ResolveFQDNResponse_MatchedRejected); ok {
			return x.MatchedRejected
		}
	}
	return nil
}
...
type ResolveFQDNResponse_MatchedDatabase struct {
	// MatchedDatabase will be set when the query matched a database using the
	// <db-user>.<db-service-name>.db.<proxy-addr> pattern.
	MatchedDatabase *MatchedDatabase `protobuf:"bytes,4,opt,name=matched_database,json=matchedDatabase,proto3,oneof"`
}

type ResolveFQDNResponse_MatchedRejected struct {
	// MatchedRejected will be set when the query matched but the connection
	// should be rejected. VNet will assign a VNet IP (so DNS succeeds) but
	// TCP connections to that IP will receive a RST. Used in restricted mode
	// to block connections to domains not explicitly handled by VNet.
	MatchedRejected *MatchedRejected `protobuf:"bytes,5,opt,name=matched_rejected,json=matchedRejected,proto3,oneof"`
}
...
func (*ResolveFQDNResponse_MatchedRejected) isResolveFQDNResponse_Match() {}

// MatchedRejected signals that VNet should assign a VNet IP for the FQDN but
// reject any TCP connection to it with a RST.
type MatchedRejected struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Ipv4CidrRange is the CIDR range from which an IPv4 address should be
	// assigned so that the DNS reply points into VNet's address space.
	Ipv4CidrRange string `protobuf:"bytes,1,opt,name=ipv4_cidr_range,json=ipv4CidrRange,proto3" json:"ipv4_cidr_range,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}

func (x *MatchedRejected) Reset() {
	*x = MatchedRejected{}
...
func (x *MatchedRejected) String() string {
	return protoimpl.X.MessageStringOf(x)
}

func (*MatchedRejected) ProtoMessage() {}

func (x *MatchedRejected) ProtoReflect() protoreflect.Message {
	mi := &file_teleport_lib_vnet_v1_client_application_service_proto_msgTypes[9]
	if x != nil {
		ms := protoimpl.X.MessageStateOf(protoimpl.Pointer(x))
		if ms.LoadMessageInfo() == nil {
...
// Deprecated: Use MatchedRejected.ProtoReflect.Descriptor instead.
func (*MatchedRejected) Descriptor() ([]byte, []int) {
	return file_teleport_lib_vnet_v1_client_application_service_proto_rawDescGZIP(), []int{9}
}

func (x *MatchedRejected) GetIpv4CidrRange() string {
	if x != nil {
...
// MatchedDatabase holds info about a database that matched a VNet DNS query.
type MatchedDatabase struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// DbServiceName is the name of the database service parsed from the FQDN.
	DbServiceName string `protobuf:"bytes,1,opt,name=db_service_name,json=dbServiceName,proto3" json:"db_service_name,omitempty"`
	// DbUser is the database user parsed from the FQDN.
	DbUser string `protobuf:"bytes,2,opt,name=db_user,json=dbUser,proto3" json:"db_user,omitempty"`
	// Profile is the profile the matched cluster was found in.
	Profile string `protobuf:"bytes,3,opt,name=profile,proto3" json:"profile,omitempty"`
	// RootCluster is the name of the root cluster.
	RootCluster string `protobuf:"bytes,4,opt,name=root_cluster,json=rootCluster,proto3" json:"root_cluster,omitempty"`
	// LeafCluster is the leaf cluster, or empty if the database is in the root cluster.
	LeafCluster string `protobuf:"bytes,5,opt,name=leaf_cluster,json=leafCluster,proto3" json:"leaf_cluster,omitempty"`
	// Ipv4CidrRange is the CIDR range from which an IPv4 address should be
	// assigned for this database.
	Ipv4CidrRange string `protobuf:"bytes,6,opt,name=ipv4_cidr_range,json=ipv4CidrRange,proto3" json:"ipv4_cidr_range,omitempty"`
	// DialOptions holds options for dialing the root cluster proxy.
	DialOptions *DialOptions `protobuf:"bytes,7,opt,name=dial_options,json=dialOptions,proto3" json:"dial_options,omitempty"`
	// DbProtocol is the database protocol string (e.g. "postgres", "mysql").
	// Populated at DNS resolution time so cert issuers don't need to re-query
	// the cluster.
	DbProtocol    string `protobuf:"bytes,8,opt,name=db_protocol,json=dbProtocol,proto3" json:"db_protocol,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *MatchedDatabase) GetDbServiceName() string {
	if x != nil {
...
func (x *MatchedDatabase) GetDbUser() string {
	if x != nil {
		return x.DbUser
	}
	return ""
...
func (x *MatchedDatabase) GetProfile() string {
	if x != nil {
		return x.Profile
	}
	return ""
...
func (x *MatchedDatabase) GetRootCluster() string {
	if x != nil {
		return x.RootCluster
	}
	return ""
...
func (x *MatchedDatabase) GetLeafCluster() string {
	if x != nil {
		return x.LeafCluster
	}
	return ""
...
func (x *MatchedDatabase) GetIpv4CidrRange() string {
	if x != nil {
		return x.Ipv4CidrRange
	}
	return ""
...
func (x *MatchedDatabase) GetDialOptions() *DialOptions {
	if x != nil {
...
func (x *MatchedDatabase) GetDbProtocol() string {
	if x != nil {
		return x.DbProtocol
	}
	return ""
...
// DBKey uniquely identifies a database session for certificate caching.
type DBKey struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// Profile is the profile in which the database is found.
	Profile string `protobuf:"bytes,1,opt,name=profile,proto3" json:"profile,omitempty"`
	// LeafCluster is the leaf cluster, or empty for the root cluster.
	...
	DbServiceName string `protobuf:"bytes,3,opt,name=db_service_name,json=dbServiceName,proto3" json:"db_service_name,omitempty"`
	// DbUser is the database user.
	DbUser string `protobuf:"bytes,4,opt,name=db_user,json=dbUser,proto3" json:"db_user,omitempty"`
	// DbProtocol is the database protocol string (e.g. "postgres", "mysql").
	// When set, cert issuers can skip a separate cluster lookup for the protocol.
	DbProtocol    string `protobuf:"bytes,5,opt,name=db_protocol,json=dbProtocol,proto3" json:"db_protocol,omitempty"`
	unknownFields protoimpl.UnknownFields
	sizeCache     protoimpl.SizeCache
}
...
func (x *DBKey) GetProfile() string {
	if x != nil {
		return x.Profile
	}
	return ""
...
func (x *DBKey) GetLeafCluster() string {
	if x != nil {
		return x.LeafCluster
	}
	return ""
...
func (x *DBKey) GetDbServiceName() string {
	if x != nil {
		return x.DbServiceName
	}
	return ""
...
func (x *DBKey) GetDbProtocol() string {
	if x != nil {
		return x.DbProtocol
	}
	return ""
...
type ReissueDBCertRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// DbKey identifies the database session.
	DbKey *DBKey `protobuf:"bytes,1,opt,name=db_key,json=dbKey,proto3" json:"db_key,omitempty"`
	// DbName is the database name to connect to. May be empty to use the
	// server default.
...
func (x *ReissueDBCertRequest) GetDbKey() *DBKey {
	if x != nil {
		return x.DbKey
	}
	return nil
...
func (x *ReissueDBCertRequest) GetDbName() string {
	if x != nil {
		return x.DbName
	}
	return ""
...
type ReissueDBCertResponse struct {
	...
	Cert []byte `protobuf:"bytes,1,opt,name=cert,proto3" json:"cert,omitempty"`
	// DbProtocol is the database protocol (e.g. "postgres", "mysql") looked up
	// from the cluster. The caller uses this to select the correct ALPN protocol.
...
func (x *ReissueDBCertResponse) GetDbProtocol() string {
	if x != nil {
		return x.DbProtocol
	}
	return ""
...
// SignForDBRequest is a request for SignForDB.
type SignForDBRequest struct {
	state protoimpl.MessageState `protogen:"open.v1"`
	// DbKey identifies the database session whose cached signer should be used.
	DbKey *DBKey `protobuf:"bytes,1,opt,name=db_key,json=dbKey,proto3" json:"db_key,omitempty"`
	// Sign holds signature request details.
	Sign          *SignRequest `protobuf:"bytes,2,opt,name=sign,proto3" json:"sign,omitempty"`
	unknownFields protoimpl.UnknownFields
...
func (x *SignForDBRequest) GetDbKey() *DBKey {
	if x != nil {
		return x.DbKey
	}
	return nil
...
const file_teleport_lib_vnet_v1_client_application_service_proto_rawDesc = "" +
	...
	"\x04fqdn\x18\x01 \x01(\tR\x04fqdn\"\xb5\x03\n" +
	...
	"\x10matched_database\x18\x04 \x01(\v2%.teleport.lib.vnet.v1.MatchedDatabaseH\x00R\x0fmatchedDatabase\x12R\n" +
	"\x10matched_rejected\x18\x05 \x01(\v2%.teleport.lib.vnet.v1.MatchedRejectedH\x00R\x0fmatchedRejectedB\a\n" +
	"\x05match\"9\n" +
	"\x0fMatchedRejected\x12&\n" +
	"\x0fipv4_cidr_range\x18\x01 \x01(\tR\ripv4CidrRange\"I\n" +
	"\rMatchedTCPApp\x128\n" +
	...
	"\fleaf_cluster\x18\x05 \x01(\tR\vleafCluster\"\xc1\x02\n" +
	"\x0fMatchedDatabase\x12&\n" +
	"\x0fdb_service_name\x18\x01 \x01(\tR\rdbServiceName\x12\x17\n" +
	"\adb_user\x18\x02 \x01(\tR\x06dbUser\x12\x18\n" +
	"\aprofile\x18\x03 \x01(\tR\aprofile\x12!\n" +
	"\froot_cluster\x18\x04 \x01(\tR\vrootCluster\x12!\n" +
	"\fleaf_cluster\x18\x05 \x01(\tR\vleafCluster\x12&\n" +
	"\x0fipv4_cidr_range\x18\x06 \x01(\tR\ripv4CidrRange\x12D\n" +
	"\fdial_options\x18\a \x01(\v2!.teleport.lib.vnet.v1.DialOptionsR\vdialOptions\x12\x1f\n" +
	"\vdb_protocol\x18\b \x01(\tR\n" +
	"dbProtocol\"\xa6\x01\n" +
	"\x05DBKey\x12\x18\n" +
	"\aprofile\x18\x01 \x01(\tR\aprofile\x12!\n" +
	"\fleaf_cluster\x18\x02 \x01(\tR\vleafCluster\x12&\n" +
	"\x0fdb_service_name\x18\x03 \x01(\tR\rdbServiceName\x12\x17\n" +
	"\adb_user\x18\x04 \x01(\tR\x06dbUser\x12\x1f\n" +
	"\vdb_protocol\x18\x05 \x01(\tR\n" +
	"dbProtocol\"\xe8\x01\n" +
	"\aAppInfo\x125\n" +
	...
	"\x0fuser_public_key\x18\x01 \x01(\fR\ruserPublicKey\"c\n" +
	"\x14ReissueDBCertRequest\x122\n" +
	"\x06db_key\x18\x01 \x01(\v2\x1b.teleport.lib.vnet.v1.DBKeyR\x05dbKey\x12\x17\n" +
	"\adb_name\x18\x02 \x01(\tR\x06dbName\"L\n" +
	"\x15ReissueDBCertResponse\x12\x12\n" +
	"\x04cert\x18\x01 \x01(\fR\x04cert\x12\x1f\n" +
	"\vdb_protocol\x18\x02 \x01(\tR\n" +
	"dbProtocol\"}\n" +
	"\x10SignForDBRequest\x122\n" +
	"\x06db_key\x18\x01 \x01(\v2\x1b.teleport.lib.vnet.v1.DBKeyR\x05dbKey\x125\n" +
	...
	"\tSignForDB\x12&.teleport.lib.vnet.v1.SignForDBRequest\x1a'.teleport.lib.vnet.v1.SignForDBResponseBLZJgithub.com/gravitational/teleport/gen/proto/go/teleport/lib/vnet/v1;vnetv1b\x06proto3"
...
var file_teleport_lib_vnet_v1_client_application_service_proto_msgTypes = make([]protoimpl.MessageInfo, 44)
var file_teleport_lib_vnet_v1_client_application_service_proto_goTypes = []any{
	...
	(*MatchedRejected)(nil),                  // 10: teleport.lib.vnet.v1.MatchedRejected
	...
	(*MatchedDatabase)(nil),                  // 14: teleport.lib.vnet.v1.MatchedDatabase
	(*DBKey)(nil),                            // 15: teleport.lib.vnet.v1.DBKey
	...
	(*types.AppV3)(nil),                      // 45: types.AppV3
}
var file_teleport_lib_vnet_v1_client_application_service_proto_depIdxs = []int32{
	...
	14, // 4: teleport.lib.vnet.v1.ResolveFQDNResponse.matched_database:type_name -> teleport.lib.vnet.v1.MatchedDatabase
	10, // 5: teleport.lib.vnet.v1.ResolveFQDNResponse.matched_rejected:type_name -> teleport.lib.vnet.v1.MatchedRejected
	16, // 6: teleport.lib.vnet.v1.MatchedTCPApp.app_info:type_name -> teleport.lib.vnet.v1.AppInfo
	18, // 7: teleport.lib.vnet.v1.MatchedDatabase.dial_options:type_name -> teleport.lib.vnet.v1.DialOptions
	17, // 8: teleport.lib.vnet.v1.AppInfo.app_key:type_name -> teleport.lib.vnet.v1.AppKey
	45, // 9: teleport.lib.vnet.v1.AppInfo.app:type_name -> types.AppV3
	...
	22, // 20: teleport.lib.vnet.v1.SignForSSHSessionRequest.sign:type_name -> teleport.lib.vnet.v1.SignRequest
	15, // 21: teleport.lib.vnet.v1.ReissueDBCertRequest.db_key:type_name -> teleport.lib.vnet.v1.DBKey
	15, // 22: teleport.lib.vnet.v1.SignForDBRequest.db_key:type_name -> teleport.lib.vnet.v1.DBKey
	22, // 23: teleport.lib.vnet.v1.SignForDBRequest.sign:type_name -> teleport.lib.vnet.v1.SignRequest
	1,  // 24: teleport.lib.vnet.v1.ClientApplicationService.AuthenticateProcess:input_type -> teleport.lib.vnet.v1.AuthenticateProcessRequest
	3,  // 25: teleport.lib.vnet.v1.ClientApplicationService.ReportNetworkStackInfo:input_type -> teleport.lib.vnet.v1.ReportNetworkStackInfoRequest
	6,  // 26: teleport.lib.vnet.v1.ClientApplicationService.Ping:input_type -> teleport.lib.vnet.v1.PingRequest
	8,  // 27: teleport.lib.vnet.v1.ClientApplicationService.ResolveFQDN:input_type -> teleport.lib.vnet.v1.ResolveFQDNRequest
	19, // 28: teleport.lib.vnet.v1.ClientApplicationService.ReissueAppCert:input_type -> teleport.lib.vnet.v1.ReissueAppCertRequest
	21, // 29: teleport.lib.vnet.v1.ClientApplicationService.SignForApp:input_type -> teleport.lib.vnet.v1.SignForAppRequest
	24, // 30: teleport.lib.vnet.v1.ClientApplicationService.OnNewAppConnection:input_type -> teleport.lib.vnet.v1.OnNewAppConnectionRequest
	26, // 31: teleport.lib.vnet.v1.ClientApplicationService.OnInvalidLocalPort:input_type -> teleport.lib.vnet.v1.OnInvalidLocalPortRequest
	28, // 32: teleport.lib.vnet.v1.ClientApplicationService.GetTargetOSConfiguration:input_type -> teleport.lib.vnet.v1.GetTargetOSConfigurationRequest
	31, // 33: teleport.lib.vnet.v1.ClientApplicationService.UserTLSCert:input_type -> teleport.lib.vnet.v1.UserTLSCertRequest
	33, // 34: teleport.lib.vnet.v1.ClientApplicationService.SignForUserTLS:input_type -> teleport.lib.vnet.v1.SignForUserTLSRequest
	35, // 35: teleport.lib.vnet.v1.ClientApplicationService.SessionSSHConfig:input_type -> teleport.lib.vnet.v1.SessionSSHConfigRequest
	37, // 36: teleport.lib.vnet.v1.ClientApplicationService.SignForSSHSession:input_type -> teleport.lib.vnet.v1.SignForSSHSessionRequest
	39, // 37: teleport.lib.vnet.v1.ClientApplicationService.ExchangeSSHKeys:input_type -> teleport.lib.vnet.v1.ExchangeSSHKeysRequest
	41, // 38: teleport.lib.vnet.v1.ClientApplicationService.ReissueDBCert:input_type -> teleport.lib.vnet.v1.ReissueDBCertRequest
	43, // 39: teleport.lib.vnet.v1.ClientApplicationService.SignForDB:input_type -> teleport.lib.vnet.v1.SignForDBRequest
	2,  // 40: teleport.lib.vnet.v1.ClientApplicationService.AuthenticateProcess:output_type -> teleport.lib.vnet.v1.AuthenticateProcessResponse
	5,  // 41: teleport.lib.vnet.v1.ClientApplicationService.ReportNetworkStackInfo:output_type -> teleport.lib.vnet.v1.ReportNetworkStackInfoResponse
	7,  // 42: teleport.lib.vnet.v1.ClientApplicationService.Ping:output_type -> teleport.lib.vnet.v1.PingResponse
	9,  // 43: teleport.lib.vnet.v1.ClientApplicationService.ResolveFQDN:output_type -> teleport.lib.vnet.v1.ResolveFQDNResponse
	20, // 44: teleport.lib.vnet.v1.ClientApplicationService.ReissueAppCert:output_type -> teleport.lib.vnet.v1.ReissueAppCertResponse
	23, // 45: teleport.lib.vnet.v1.ClientApplicationService.SignForApp:output_type -> teleport.lib.vnet.v1.SignForAppResponse
	25, // 46: teleport.lib.vnet.v1.ClientApplicationService.OnNewAppConnection:output_type -> teleport.lib.vnet.v1.OnNewAppConnectionResponse
	27, // 47: teleport.lib.vnet.v1.ClientApplicationService.OnInvalidLocalPort:output_type -> teleport.lib.vnet.v1.OnInvalidLocalPortResponse
	29, // 48: teleport.lib.vnet.v1.ClientApplicationService.GetTargetOSConfiguration:output_type -> teleport.lib.vnet.v1.GetTargetOSConfigurationResponse
	32, // 49: teleport.lib.vnet.v1.ClientApplicationService.UserTLSCert:output_type -> teleport.lib.vnet.v1.UserTLSCertResponse
	34, // 50: teleport.lib.vnet.v1.ClientApplicationService.SignForUserTLS:output_type -> teleport.lib.vnet.v1.SignForUserTLSResponse
	36, // 51: teleport.lib.vnet.v1.ClientApplicationService.SessionSSHConfig:output_type -> teleport.lib.vnet.v1.SessionSSHConfigResponse
	38, // 52: teleport.lib.vnet.v1.ClientApplicationService.SignForSSHSession:output_type -> teleport.lib.vnet.v1.SignForSSHSessionResponse
	40, // 53: teleport.lib.vnet.v1.ClientApplicationService.ExchangeSSHKeys:output_type -> teleport.lib.vnet.v1.ExchangeSSHKeysResponse
	42, // 54: teleport.lib.vnet.v1.ClientApplicationService.ReissueDBCert:output_type -> teleport.lib.vnet.v1.ReissueDBCertResponse
	44, // 55: teleport.lib.vnet.v1.ClientApplicationService.SignForDB:output_type -> teleport.lib.vnet.v1.SignForDBResponse
...
func file_teleport_lib_vnet_v1_client_application_service_proto_init() {
	...
	file_teleport_lib_vnet_v1_client_application_service_proto_msgTypes[8].OneofWrappers = []any{
		...
		(*ResolveFQDNResponse_MatchedRejected)(nil),
	}
	file_teleport_lib_vnet_v1_client_application_service_proto_msgTypes[21].OneofWrappers = []any{}
	type x struct{}

...
| Access Controls | Teleport Features |
| - | - |
...
| Audit and Accountability| Teleport Features |
| - | - |
| [AU-03, AU-04, AU-12 Audit and Accountability]((=fedramp.control_url=)AU-3) – Content of Audit Records and [AU-12 Audit Generation]((=fedramp.control_url=)AU-12) | Teleport contains an [Audit Log](../../reference/audit.mdx) that records cluster-wide events such as: <br/>• Failed login attempts.<br/>• Commands that were executed (SSH “exec” commands).<br/> • Ports that were forwarded. <br/>• File transfers that were initiated. |
...
| Configuration Management | Teleport Features |
| - | - |
...
| Identification and Authentication | Teleport Features |
| - | - |
| [IA-02 Concurrent Session Control]((=fedramp.control_url=)IA-2) | Integrates with SSO providers such as GitHub, Okta, Google, etc. Acts as its own SSO provider. Enforces the use of multi-factor authentication (MFA), including requiring per-session MFA. Supports PIV-compatible hardware keys. |
| [IA-04 Identifier Management]((=fedramp.control_url=)IA-4) | Maintains several unique identifiers: local users are required to be unique (unique username), roles have unique names and tied to organization roles via SSO, identifiers for devices are unique randomly generated IDs (UUID). |
...
| System and Communications Protection | Teleport Features |
| - | - |

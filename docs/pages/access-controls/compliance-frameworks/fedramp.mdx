...
This standard is the US government approved standard for cryptographic modules. This document provides a high
...
After you download and install, all of the Teleport Enterrpise binaries are 
installed in the `/usr/local/bin` directory. You can verify you have FIPS-compliant
binaries installed by running the `version` command and verifying that the `X:boringcrypto`
library is listed. For example:

```code
$ teleport version
Teleport Enterprise v13.3.9 git:api/v13.3.9-0-gd1a97d9 go1.20.8 X:boringcrypto
$ tsh version
Teleport v13.3.9 git:api/v13.3.9-0-gd1a97d9 go1.20.8 X:boringcrypto
$ tctl version
Teleport Enterprise v13.3.9 git:api/v13.3.9-0-gd1a97d9 go1.20.8 X:boringcrypto
$ tbot version
Teleport v13.3.9 git:api/v13.3.9-0-gd1a97d9 go1.20.8 X:boringcrypto
...
If your Teleport cluster runs on AWS, the cluster must run in a GovCloud
...
## Configure the Teleport Node Service
...
Dashboardownload the `systemd` service unit file from the [examples directory](https://github.com/gravitational/teleport/tree/branch/v(=teleport.major_version=)/examples/systemd/fips)

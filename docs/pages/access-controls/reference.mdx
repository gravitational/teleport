...
| `pin_source_ip` (Preview) | Enable source IP pinning for SSH certificates. Note: IP pinning is currently in Preview mode | Logical "OR" i.e. evaluates to "yes" if at least one role requires session termination |

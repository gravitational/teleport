...
| `require_session_mfa` | Enforce per-session MFA or PIV-hardware key restrictions on user login sessions | Logical "OR" i.e. evaluates to "yes" if at least one role requires session MFA |

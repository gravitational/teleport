...
`allow` rule describing a `list` verb applied to the SSH `ssh_session` resource.  It means "allow
...
prohibits users from getting a list of active sessions. You can see
...
```yaml
  allow:
    ...
    - resources:
      - auth_connector
      verbs:  [list, create, read, update, delete]
    # SSH Sessions: Provides access to active sessions
    # e.g If ssh sessions list is denied, users can't see the list of active sessions
    # If the session read is denied, the active sesion list will show empty.
    - resources:
      - ssh_session
      verbs:  [list,read]    

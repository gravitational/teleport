...
| `create_host_user_mode` | Available in Teleport 13.2+ (ealier versions use the deprecated `create_host_user` boolean option, equavalent to `drop`) . Allow users to be automatically created on a host |  Logical "AND" i.e. if all roles matching a server specify host user creation (`off`, `drop`, `keep`), it will evaluate to the option specified by all of the roles. If some roles specify both `drop` or `keep` it will evaluate to `keep`|

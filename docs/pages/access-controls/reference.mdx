...
| `create_host_user_mode` | Allow users to be automatically created on a host |  Logical "AND" i.e. if all roles matching a Node specify host user creation (`off`, `drop`, `keep`), it will evaluate to the option specified by all of the roles, if some roles specify both `drop` or `keep` it will evaluate to `keep`|

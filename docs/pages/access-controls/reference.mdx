...
| `lock` | Locking mode (`strict` or `best_effot`) | `strict` wins in case of conflict |
...
| `desktop_clipboard` | Allow clipboard sharing for desktop sessions | Logical "AND" i.e. evaluates to "yes" if all roles enable clipbaord sharing |

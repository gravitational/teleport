...
| - | - |
| `{{internal.logins}}` | Substitutes with a value stored in Teleport's own local user database and logins from a root cluster.  For local users that will substitute in the "allowed logins" parameter used in `tctl users add [user] <allowed logins>` command. If the role is within a leaf cluster in [Trusted Clusters](../setup/admin/trustedclusters.mdx) the logins from the root cluster will substitute whether the user is a local user or from a SSO provider. As an example if the user has `ubuntu` user in the root cluster then `ubuntu` will be substituted in the leaf cluster with this variable. |

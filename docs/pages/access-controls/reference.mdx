...
| - | - |
| `{{internal.logins}}` | This applies users stored in Teleport's own local database and users from a root cluster.  For local users that will substitute in the "allowed logins" parameter used in `tctl users add [user] <allowed logins>` command.   If the role is within a leaf cluster in [Trusted Clusters](../setup/admin/trustedclusters.mdx) the logins from the root cluster will also apply. |

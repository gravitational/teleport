...
spec:
  ...
  allow:
    ...
    node_labels:
      # A user can only connect to a node marked with the 'test' label:
      'environment': 'test'
    ...
    kubernetes_labels:
      ...
      'env': 'prod'
      # A user can access any region in us-west, e.g us-west-1, us-west-2
      'region': 'us-west-*'
...
The following variables can be used with the `logins` field:

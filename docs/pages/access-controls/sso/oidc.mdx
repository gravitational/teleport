...
Teleport supports setting the `max_age` field to control the maximum age of users'
sessions before they will be forced to reauthenticate. By default `max_age` is empty,
meaning once a user authenticates using OIDC they will not have to reauthenticate
unless the configured OIDC provider forces them to. This can be set to a duration
of time to force users to reauthenticate more often. If `max_age` is set to zero
seconds, users will be forced to reauthenticate every time.

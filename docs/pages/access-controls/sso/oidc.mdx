...
Teleport supports setting the `max_age` field since version 13.3.7 to control the
...
`max_age` is set to zero seconds, users will be forced to reauthenticate every time.

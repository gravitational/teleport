...
manipulated via `tctl` [resource commands](../../reference/resources.mdx). To
create a new connector, create a connector resource file in YAML format, for
example `oidc-connector.yaml`.
...
Teleport now lets Teleport Admins configure this option. Since `prompt` is
optional, by setting the variable to `none`, Teleport will override the default
`select_account`.
...
The below example will prompt the end-user for reauthentication and will require

...
- A One Login account with admin access, and users assigned to at least two groups.
...
`https://teleport.example.com:3080/v1/webapi/saml/acs` where
`teleport.example.com` is the public name of the teleport web proxy service:

![Configure SAML](../../../img/sso/onelogin/onelogin-saml-2.png)

Teleport needs to assign groups to users. Configure the application with some
parameters exposed as SAML attribute statements:
...
Update the `acs` value with your public cluster URL and paste the SAML metadata
that you previously downloaded in `entity_descriptor`.
...
In the below role, Devs are only allowed to login to nodes labelled with

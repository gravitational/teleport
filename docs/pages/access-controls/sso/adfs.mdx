...
- Select *"Enable support for SAML 2.0 Web SSO protocol"* and set the URL to
  `https://teleport.example.com/webapi/saml/acs`, replacing the domain name
  with your Teleport proxy URL.
- Set the relying party trust identifier to
  `https://teleport.example.com/webapi/saml/acs` as well.
...
```code
$ tctl sso configure saml --acs https://teleport.example.com/webapi/saml/acs \
  --preset adfs \

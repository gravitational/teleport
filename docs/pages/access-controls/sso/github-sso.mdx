...
```
<Admonition type="tip">
  GitHub organizations and teams should be referred to by their slug value, not display name. 
  Slug values are available in the url or GitHub API.
  Example: `example-team` in the url `https://github.com/orgs/Org-Example/teams/example-team`
  matches to the slug value for team `Example Team`.
</Admonition>
...
If you are not seeing teams mapping to roles confirm you 
are using the slug value of the organizations and teams in the connector.
The GitHub Web application URIs and GitHub API can provide the slug values.

Example with team name `My Team`:
```yaml
  teams_to_roles:
    - organization: ORG-NAME
      roles:
...
Navigate to the team `My Team`. The url `https://github.com/orgs/ORG-NAME/teams/my-team`
shows the slug value is `my-team`. Update the teams to roles mapping.

...
```
<Admonition type="tip">
  GitHub will send the team names in lowercase. Configure the team mapping to roles
  with the lowercase version of the name e.g. `MyTeam` -> `myteam`.
</Admonition>
...
are using the lower case version of the team name in the connector.
GitHub transmits the lower case of team names.

Example with mixed case team name `MyTeam`:
```yaml
  teams_to_roles:
    - organization: ORG-NAME
      roles:
...
Convert to use the lower case version of the team name `my-team`:

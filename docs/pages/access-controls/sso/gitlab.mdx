...
- Only members of "DBA" group can access PostgreSQL databases.
...
- At least two groups in GitLab with users assigned. In our examples below,
  we'll assume a group named `company` with two subgoups, `admin` and `dev`.
...
1. Create a Application in one of your Groups that will allow using GitLab as a
   OAuth provider to Teleport.
...
3. Confirm the GitLab Issuer Address.
   ...
   For GitLab cloud the issue address is `https://gitlab.com`. This allows
...
Create a OIDC connector resource using `tctl`.

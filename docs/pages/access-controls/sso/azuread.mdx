...
8. Edit **Attributes & Claims**
   ...
   - Add a claim that transforms an Azure AD username in order to pass it to
...
9. In **SAML Certificates** section, copy the **App Federation
   Metadata URL** link and save it for the next step:
...
- `--entity-descriptor` is give the app federation metadata URL saved in the
  previous step.
- Each `--attributes-to-roles` is give the name of the schema definition for
  groups, the UUID of an AD group, and the Teleport role members of that group
  will be assigned.

The file `azure-connector.yaml` should now resemble:
...
Your browser should open and log you in to the Teleport cluster through your

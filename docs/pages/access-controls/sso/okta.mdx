...
Okta provides group membership as a SAML assertion in the user data it provides
to Teleport. We will configure Teleport to assign roles based on these groups.
...
In this section we will create an application in the Okta dashboard to define
and allow our Teleport cluster to access Okta as an IdP provider. We'll also
locate the address where Okta provides their IdP metadata to provide to Teleport.
...
and SSO URL to the same value. This is so Teleport can read and use Okta users'
email address to create their username in Teleport, instead of relying on additional
name fields.
</Admonition>

...
- User devices: macOS and Windows OS.
...
Inventory management can be manually operated ([Set Up Trusted Device](./device-trust/guide.mdx)),
or automatically synced with Mobile Device Management (MDM) solution such as Jamf
...
Server. Enrollment has to run on the actual device that you want to enroll in. For
...
The signed attestation ensures that the Teleport Auth Server knows both the state
...
- **Role-based enforcement** can be used to enforce devices at role level, using RBAC.
- **Cluster-wide enforcement** can be used to enforce Device Trust at cluster level.

## Guides
- [Getting Started with Device Trust](./device-trust/device-management.mdx)
- [Device Registration and Enrollment](./device-trust/device-management.mdx)
- [Device Trust Enforcement](./device-trust/enforcing-device-trust.mdx)
- [Jamf Pro Integration](./device-trust/jamf-integration.mdx)

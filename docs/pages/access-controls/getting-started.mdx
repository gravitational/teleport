...
Teleport provides several preset roles: `editor`, `auditor`, <ScopedBlock scope={["oss"]}>and `access`</ScopedBlock>
<ScopedBlock scope={["enterprise", "cloud"]}> `access`, `reviewer`, and `requester`</ScopedBlock>.
...
- The `reviewer` role authorizes users to review application and user group access requests.
- The `requester` role authorizes users to request applications and user groups.
</ScopedBlock>

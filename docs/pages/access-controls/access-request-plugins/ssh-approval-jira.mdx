...
This URL depends on where you have installed.
...
For the webhook URL, use the URL that you'll run the plugin on. The URL must be publicly 
...
updates. It's not critical if anything else gets sent. The plugin ignore everything else.

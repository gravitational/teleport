...
For the plugin to know which serviceno rotations to check for the
auto approval flow, the rota you wish users to be checked against
must be included in a role the requesting user has. e.g.
...
If the a ServiceNow rotation was specified in the requesters role annotations,
then the ServiceNow plugin will check that the requesters username matches the

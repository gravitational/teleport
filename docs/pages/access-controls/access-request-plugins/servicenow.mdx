...
### Create a demo-requester role

To create a user first navigate to Management -> Access -> Roles

Then select 'Create New Role' and create the demo-requester role.
...
Create a user called `myuser` who has the `demo-requester` role. Later in this
...
Then select 'Create New User' and create a user with the demo-requester role.
...
For the plugin to function the credentials given to the plugin to use
must allow for reading and writing from the incident table.

---
title: Access Requests with PagerDuty
...
When a user submits a new access request, the Teleport PagerDuty plugin creates 
an incident in a PagerDuty service. For the notification to be posted in the right 
service, the service name must be included in a request annotation for a role 
associated with the user requesting access.

For example, assume you have created a PagerDuty service called **Teleport Notifications** 
and you want this service to be notified about all new access requests from users with the 
`challenger` role. To do so, you should edit the `challenger` role to include a request 
annotation called `pagerduty_notify_service` that contains the service name 

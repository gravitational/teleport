...
```code
$ tctl users add myuser --roles=requester
```
...
Here is an example file which defines a role called `reviewer` that can
review requests for the `editor` role, plus a `requester` role that can
request this role.
    ...
    request:
      ...
      thresholds:
        - approve: 1
          deny: 1
      annotations:
        schedules: ["Teleport Access Request Notifications"]
```
...
![Integrations page](../../../img/enterprise/plugins/opsgenie/opsgenie-integrations-menu.png)

Then select the Opsgenie integration.
...
or to the schedules specified by `opsgenie_notify_services` annotation in the requester's role.

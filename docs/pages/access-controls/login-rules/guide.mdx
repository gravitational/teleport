...
## Step 1/5. Configure a Teleport role with access to Login Rules objects

First, ensure you are logged into Teleport as a user that has permissions
to read and modify `login_rule` objects. The default `editor` role
...
(!docs/pages/includes/access-controls/loginrule-manager-role.mdx!)
...
jq -r '.[].spec.roles | join(",")'), loginrule-manager <Var name="teleport-admin"/>
User teleport-admin has been updated:
	New roles: auditor,editor,access,loginrule-manager
```

---
title: Deploy Login Rules to Kubernetes (Preview)
...
## Step 1/3. Install the Teleport Operator

Follow Step 1 of the
[Teleport operator guide](../../management/guides/teleport-operator.mdx)
...
## Step 2/3. Create a Login Rule using `kubectl`
...
## Step 3/3. Edit the Login Rules with `kubectl`

Edit the `example-traits-map-rule` to add an extra login (`example`) by writing
the following to `login-rule-edit.yaml`:

```
# login-rule-edit.yaml
apiVersion: resources.teleport.dev/v1
kind: TeleportLoginRule
metadata:
  name: example-traits-map-rule
...
```code
$ kubectl apply -f login-rule-edit.yaml
```

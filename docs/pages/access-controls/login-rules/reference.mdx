...
Users must have right to read and maintain `login_rule` resources.
The preset `editor` role includes these permissions by default.
Here is an example role with those permissions:

...
Users with the `editor` role can register and enroll their device in a single step with
...
For clusters created using Teleport v12 or newer, the preset `editor` role has the necessary permissions to manage devices.
If you Teleport cluster is older than v12, you will need to create and grant [`device` role](#dedicated-device-admin-role) to manage device inventory.
</Admonition>

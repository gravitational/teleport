...
description: Learn how to manage Trusted Devices in Teleport
---
...
<Admonition type="tip" title="Self enrollment: v13.3.5+">
  Users with the preset `editor` or `device-enroll` role (available from v13.3.6 and above)
...
<Admonition type="warning" title="Device Role">
  ...
  the necessary permissions to manage devices. If your Teleport cluster is older than v12, you will
  need to create and grant [`device` role](#dedicated-device-admin-role) to manage device inventory.
</Admonition>
...
In a large organization, distributing enrollment tokens to each user can be challenging.
...
<Admonition type="tip" title="v13.3.6+">
  Teleport version 13.3.6 and above has auto-enrollment enabled by default.
</Admonition>
...
<Admonition type="tip" title="v13.3.6+">
  Teleport version 13.3.6 and above supports preset `device-admin` role, which
  can be used as an alternative to creating a new role as explained below.
</Admonition>

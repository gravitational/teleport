...
Retreive device serial number with `tsh` (must be run on the device you want to register):
```code
...
<Details title="Manually retrieving device serial" closed>
  <Tabs>
  <TabItem label="macOS">
    The serial number is visible under Apple manu -> "About This Mac" -> "Serial number".
  </TabItem>
...
A device must be registered for auto-enrollment to work. Registration may be

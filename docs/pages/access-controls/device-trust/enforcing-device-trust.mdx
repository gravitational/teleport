...
<Admonition type="tip" title="v13.3.6+">
  Teleport version 13.3.6 and above supports preset `require-trusted-device` role to enforce Device Trust.
  You have an option to either update existing role as referenced below, or use the preset role.
  Ensure to update rules defined in preset `require-trusted-device` role as required.
</Admonition>

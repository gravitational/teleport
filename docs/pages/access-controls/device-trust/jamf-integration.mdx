...
well as the set of devices synced from Jamf by applying RSQL filters provided by
the [Jamf Pro API](
...
Unpacking the configuration, we have the following:
...
The on\_missing key determines what to do with devices deleted from Jamf, but
...
The filter\_rsql key applies a Jamf Pro API filter when querying for devices.
Refer to

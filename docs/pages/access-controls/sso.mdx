...
To set the default authentication type as `saml`, `oidc` or `github`, either modify your
...
To set the default authentication type as `github`, either modify your
Auth Service configuration file or
create a `cluster_auth_preference` resource.
...
<Tabs>
 ...
 Update `/etc/teleport.yaml` in the `auth_service` section and restart the `teleport` daemon.
  ...
  ```code
  # Log in to your cluster with tsh so you can run tctl commands.
  $ tsh login --proxy=<Var name="teleport.example.com" /> --user=myuser
  $ tctl create -f cap.yaml
  ```
  </TabItem>
</Tabs>

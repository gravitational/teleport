...
## Step 1/8. Create the access-requester role
...
```code
$ tctl create access-requester.yaml
```

## Step 2/8. Create the access-reviewer role

This role allows the reviewer to approve all requests for the `access` role.

```yaml
# access-reviewer.yaml
kind: role
version: v5
metadata:
...
```code
$ tctl create access-reviewer.yaml
```

## Step 3/8. Grant the roles to users

Grant the `access-requester` and `access-reviewer` roles to existing users, or create new
users to test this feature.
Make sure the requester has a valid `login` so that they can view and access SSH
nodes.

```code
$ tctl users add alice --roles access-requester --logins alice
$ tctl users add bob --roles access-reviewer
```

For the rest of the guide we will assume that the `access-requester` role has been
granted to a user named `alice` and the `access-reviewer` role has been granted to a
user named `bob`.

...
## Step 1/8. Create the my-requester role
...
```code
$ tctl create my-requester.yaml
```

## Step 2/8. Create the my-reviewer role
...
```code
$ tctl create my-reviewer.yaml
```

## Step 3/8. Grant the roles to users

Grant the `my-requester` and `my-reviewer` roles to existing users, or create new
users to test this feature.
Make sure the requester has a valid `login` so that they can view and access SSH
nodes.

```code
$ tctl users add alice --roles my-requester --logins alice
$ tctl users add bob --roles my-reviewer
```

For the rest of the guide we will assume that the `my-requester` role has been
granted to a user named `alice` and the `my-reviewer` role has been granted to a
user named `bob`.

...
Using Teleport Policy’s Access Graph feature with a self-hosted Teleport cluster requires
setting up the Access Graph, a dedicated service which uses 
PostgreSQL as its backing storage and communicates with Auth Service and Proxy Service
to collect information about resources and access.

This guide will help you set up the service and enable Access Graph (AG) in your Teleport cluster.
...
- A TLS certificate for the Access Graph service
  - The TLS certificate must be issued for "server authentication" key usage,
    and must list the IP or DNS name of the AG service in an X.509 v3 `subjectAltName` extension.
...
Then, on the same machine, create a configuration file for the AG service, similar to this:
...
# IP address (optional) and port for the AG service to listen to.
# This is the default value. This key can be omitted to listen on port 50051 on all interfaces.
address: ":50051"

tls:
  # File paths of PEM-encoded TLS certificate and private key for the AG server.
  cert: /etc/access_graph/tls.crt
  key: /etc/access_graph/tls.key

# This lists the file paths for host CAs of Teleport clusters that are allowed to register with this AG service.
# Several paths can be included to allow several Teleport clusters to connect to the AG service.
registration_cas:
  - /etc/access_graph/teleport_host_ca.pem # A full path to the file containing the Teleport cluster's host CA certificate.
```

Finally, start the AG service using Docker as follows:
...
access_graph:
  enabled: true
  # host:port where the AG service is listening
  endpoint: access-graph.example.com:50051
  # Specify a trusted CA we expect the AG server certificate to be signed by.

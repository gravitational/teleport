...
- A running Teleport Enterprise cluster v14.3.4 or later.
  - For the purposes of this guide, we assume that the Teleport cluster is set up
    [using a Helm chart](../../deploy-a-cluster/helm-deployments.mdx)
...
```code
$ helm install -n teleport-access-graph -f tag-values.yaml teleport-access-graph teleport/teleport-access-graph
...
Unless you are using a public Certificate Authority to issue the server certificate for TAG,
the Teleport Auth Service will need a copy of the certificate of the CA that issued the server certificate.
...
Then, update the values for the deployment of the [teleport-cluster](../reference/helm-reference/teleport-cluster/) Helm chart as follows:
...
```code
$ helm upgrade -n teleport -f values-teleport.yaml --version <version> teleport teleport/teleport-cluster 
$ kubectl -n teleport rollout status deployment/teleport-auth   # Wait for the deployment to succeed
$ kubectl -n teleport rollout restart deployment/teleport-proxy 
$ kubectl -n teleport rollout status deployment/teleport-proxy  # Wait for the deployment to succeed
```

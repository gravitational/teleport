...
response under the requested attribute name.
...
| Roles      | User roles.                   | `eduPersonAffiliation` or `user.spec.roles`.                 |
| Traits     | All values under user traits. | `user.spec.traits.firstname`, `user.spec.traits.groups` etc. |
...
Expression syntax are similar to how expressions are used in [Login Rules](../login-rules/reference.mdx). But not all of the 
functions and methods are available for attribute mapping. The supported functions and methods are listed below, along with 
...
### `test_attribute_mapping` command

Attribute mapping can be tested using `tctl idp saml test_attribute_mapping` command.
`test_attribute_mapping` command accepts three arguments.

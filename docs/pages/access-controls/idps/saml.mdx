...
- Teleport role with access to maintaining `saml_idp_service_provider` resources.
  This is available in the default `editor` role.
...
be functioning here as a **SAML service provider**, and Teleport's SAML identity
provider. This process can vary greatly between applications, so you will likely
...
format by navigating to `https://<proxy-address>/enterprise/saml-idp/metadata`.

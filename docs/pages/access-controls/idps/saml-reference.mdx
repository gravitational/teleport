...
The SAML identity provider allows users to use Teleport to authenticate and
authorize to external applications outside of Teleport that support SAML
Single Sign-On.
...
fact about a user, e.g. "user *example* belongs to roles *editor*, *access*."
...
The service provider initiates the login process to an external application.

### Identity provider initiated SSO

The identity provider initiates the login process to an external application.

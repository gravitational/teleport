...
This guide will cover an example on how to use Teleport as an identity provider
so that Teleport can be used to authenticate to external services.
...
And then add the role to your user. Replace `teleport-admin` with your user name:
...
Navigate to samltest.id, and click on "Upload Metadata." Next, choose to upload
...
Download the service provider metadata from samltest.id. Run the following
command which will save the samltest.id service provider metadata to a file
called `saml-sp.xml`:
...
Next, create a file called `saml-sp.yaml` that looks like this, where the
metadata you just downloaded is assigned to the `entity_descriptor` field
in the `saml_idp_service_provider` object:

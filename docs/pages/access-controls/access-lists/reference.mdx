...
to Access Lists as members and meet membership requirements will be granted these roles
and traits upon login in Teleport. By combining this functionality with an auditing
functionality, Teleport administrators will have an audit trail for long lived access
granted to users of Teleport.
...
must be added by a Teleport administrator. Owners must meet requirements in order for
their ownership to be effective.
...
Access List must be audited. If the Access List is not audited on time, then the Access
List will be rendered ineffective. All members will not be granted the roles or traits
specified by the Access List. However, owners are still able to audit the lists in order
to re-establish the validity of the Access List.
...
spec:
  ...
  # with the next audit date. If an Access List is not audited by the next audit date, the
  # roles and traits granted by this Access List will be revoked from the users until the
  # audit occurs.
  audit:
  ...
  description: "A plaintext description of the Access List and its purpose"

...
The following are options for `require_session_join`:
...
The `on_leave` string option in require policies is used to define what happens when a user leaves a session, causing a policy to no longer be satisfied.

There are two possible actions to take in this scenario:
1. Terminate the session and disconnect all participants.
2. Pause the session and stop any input/output streaming until the policy is satisfied again.
...
but the session will remain open. This discards all input from session participants and buffers output until the session can resume.

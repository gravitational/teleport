...
The filter expressions follow a restricted subset of Go syntax and have the environment supports
the following functions and operators:
- `contains(set, item)`: Returns true if the item is in the set, otherwise false. The set can be a list or an array
...
`equals(user.name, "adam") || contains(user.roles, "cs-observe")` is a simple example of a filter expression

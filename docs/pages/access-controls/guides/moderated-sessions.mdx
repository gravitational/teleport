...
have Teleport Enterprise or Teleport Enterprise Cloud to join a session as a **moderator** 
with permission to terminate sessions.
...
assigned the `prod-access` role must have a minimum of one user with the role `auditor` 
...
Because this sample policy requires that at least one user with the `auditor` role is present
...
must have configured a device for multifactor authentation.

Every 30 seconds, Teleport moderating users who have joined the terminal session to 
authenticate in the next 15 seconds. This behavior continues throughout the session 
to ensure that moderators are always present and watching a given session.

...
#### Example Session Moderation Required Role
...
Servers and Kubernetes resources labeled `env: prod`. The session will pend starting
...
Joining from the UI is available for Server for SSH sessions.  Kubernetes are only joinable by CLI currently.
</Admonition>
...
![Join Server Session from UI](../../../img/access-controls/joinsession.png)

<Admonition title="Note" type="tip">

Joining as a moderator and observer is available for Servers in the UI Console.

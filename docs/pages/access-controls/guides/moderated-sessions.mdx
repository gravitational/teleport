...
the policy is fulfilled. The role matches to `env: prod` labeled `ssh` and `k8s`
resources. 
...
Moderator or observers can only join via CLI currently with `tsh join --mode=observer|moderator <session id>`.  Web joining is 
...
Here is an example of Jeff with role `prod-access` joining to a 

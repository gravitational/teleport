...
The following are optional options for `require_session_join`:
...
`on_leave` in require policies are used to define what happens when a user leaves a session, causing a policy to no longer be satisfied.

...
<Details title="Details title" min="8.3" opened>
  This feature is only compatible with Teleport 8.3.0 and newer.
  The role examples in this documentation `version: v5` for role resources. This version
  is only supported on Teleport 8.3.0 and newer. Older versions of Teleport should specify `version: v4` for roles.
</Details>

...
The authorizer applies all require policies within a role together with an OR operator and the sets from each role with an AND operator. In practice, this means that every role with at least one require policiey requires one of it's policies to be met before a session can be started.

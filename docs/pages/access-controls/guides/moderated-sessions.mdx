...
|`filter`|[Filter](./moderated-sessions.mdx#filters)|An expression that, if it evaluates to true for a given user, enables the user to be present in a Moderated Session|
|`kinds`|`[]`[Session kind](./moderated-sessions.mdx/#session-kinds)|The kind of session that the policy applies to|
|`modes`|`[]`[Participant mode](./moderated-sessions.mdx#participant-modes)|The participant mode that applies to the user joining the Moderated Session under this policy|
|`count`|[Participant mode](./moderated-sessions.mdx#require-policy-count)|The amount of users that need to match the filter expression to satisfy the policy|
...
|`kinds`|`[]`[Session kind](./moderated-sessions.mdx#session-kinds)|The kind of session that the policy applies to|
|`modes`|`[]`[Participant mode](./moderated-sessions.mdx#participant-modes)|The participant mode that paplies to the user joining the Moderated Session under this policy|
...
Filter expressions  allow for more detailed control.
`user.name == "adam" || contains(user.roles, "cs-observe")` is a simple example of a filter expression
that evaluates to true if the user is Adam or if the user has the trait `cs-observe`.
...
which the set fields `roles` and `name`.

### Session kinds

Require and allow policies have to specify which sessions they apply to. Valid options are `ssh` and `k8s`.

- `ssh` policies apply to all SSH sessions towards a node running the Teleport SSH server.
- `k8s` policies apply to all kubernetes sessions to clusters connected to teleport.
...
Moderatated Sessions tracks sessions via `SessionTracker` resource. When starting
an interactive SSH or Kubernetes session using `tsh ssh` or `tsh kube exec` respectively,
one may supply a `--reason <reason>` and/or a `--invited <users>` flag where `<reason>`

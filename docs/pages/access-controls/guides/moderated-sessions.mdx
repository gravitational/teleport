...
You can use `require_session_join` in a role to specify the conditions that must be a met 
for a session to start or run. For example, the following policy specifies that users 
assigned the `prod-access` role must have a minimum of two users with the role `auditor` 
and the mode `moderator` present in to start SSH or Kubernetes sessions:
...
|`kinds`|List|The kind of session—SSH, Kubernetes, or both—that the policy applies to.|
...
OR operator and the policies from each role are evluated using an AND operator. In practice, this 
...
|`kinds`|List|The kind of sessions—SSH, Kubernetes, or both—that the allow policy applies to.|
...
must have configured a Universal 2nd Factor (U2F) or WebAuthn MFA token.

Every 30 seconds, Teleport will prompt users in the terminal session to authenticate in 
the next 15 seconds. This behavior continues throughout the session to ensure that 

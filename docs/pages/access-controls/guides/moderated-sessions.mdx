...
You can also [define rules](#join_sessions) that allow users to join each other's
sessions from `tsh` and the Teleport Web UI.

The most common use cases for moderated sessions involve the following scenarios:

- If you have stringent security or compliance requirements and need to have people watching
  over user-initiated sessions on a set of servers.
- If you want to share a terminal with someone else to be able to instruct or collaborate.

You must have Teleport Enterprise or Teleport Enterprise Cloud to enable moderated Sessions
in a Teleport cluster.

## Policies

Moderated sessions make use of role-based access control policies to allow for 
fine grained control over who can join a session and who is required to be present 
to start one.
...
You can use `require_session_join` in a role to set of conditions that must be a met for 
a session to start or run. For example, the following policy specifies that users assigned
the `prod-access` role must have a minimum of two users with the role `auditor` and the 
mode `moderator` present in to start SSH or Kubernetes sessions:
...
This sample policy requires that at least one user with the `auditor` role is present
as a moderator for SSH or Kubernetes sessions to start on servers and Kubernetes resources 
with the `env: prod` label. A user assigned this `prod-access` role won't be able to start 
any sessions on matching resources until the policy requirements are fulfilled.
...
#### Require filter expressions

Filter expressions allow for more detailed control over the scope of a policy.
For example, rcequire policies can specify which users they consider as valid with a filter
expression. The filter context has a `user` object defined with the set fields
`roles` and `name`.

In the following example, the filter expression evaluates to true if the user is
...
The `on_leave` string option in require policies is used to define what happens 
when a moderator leaves a session, causing a policy to no longer be satisfied.
...
- Pause the session and stop any input/output streaming until the policy is satisfied again, 
  corresponding to the `"pause"` value.
...
discards all input from session participants and buffers the most recent output but remains open
until the session can resume.

### Combining require policies and roles

The authorizer applies require policies within a role together with an OR operator and the policies 
from each role with an AND operator. In practice, this means that for every role with at least one 
require policy, one of its policies must be met before a session can be started.
...
they can join a session. For example, the following policy is attached to the role `auditor` and allows 
a user assigned to the auditor role to join SSH and Kubernetes sessions started by a user with the 
role `prod-access` as a moderator or observer:
...
input variables. Filter expressions follow a restricted subset of Go syntax
and support the following functions and operators:

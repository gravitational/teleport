...
Both the session originiator and the moderator(s) must be present in the Web UI during the file transfer initiation to receive
...
If a Deny is sent, the file transfer request is immediately removed and all party members are notified.

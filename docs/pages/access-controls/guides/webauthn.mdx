...
<Tabs dropDownCaption="Teleport Deployment">
  ...
  spec:
    ...
    webauthn:
      ...
      # Optional, attestation_allowed_cas is an optional deny list
      # of certificate authorities.
...
spec:
  ...
  webauthn:
    # Required, replace with proxy web address (example.com, mytenant.teleport.sh),
    #  see `webauthn` Fields Definitions for details.
    rp_id: example.com
    # Optional, see `webauthn` Fields Definitions for details.
    attestation_allowed_cas:
    - |
      -----BEGIN CERTIFICATE-----
      ...

...
- A running Teleport cluster version 16.0.0-dev or above. If you want to get started with Teleport, 
...
<TabItem label="Dynamic Resources" options="Self-Hosted,Teleport Enterprise" >
...
Update the `cluster_auth_preference` definition to include the following content:
  ...
  <TabItem label="Static Config" scope={["oss", "enterprise"]}>
  Edit the Auth Service instance's `teleport.yaml` file and restart the instances:

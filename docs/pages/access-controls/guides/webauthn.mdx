...
- A running Teleport cluster version 16 or above. If you want to get started with Teleport, 
...
<Admonition type="note" title="Machine ID and Device Trust">
...
Update the `cluster_auth_preference` definition to include the following content:
  ...
  The comand `tctl` will update the remote definition:

  ```text
  cluster auth preference has been updated
  ```

For more information about cluster hardening, see:

- [Per-Session MFA](./per-session-mfa.mdx): Per-session multi-factor authentication.
- [MFA for Administrative Actions](./mfa-for-admin-actions.mdx): Multi-factor authentication for admin actions.

...
Starting on Teleport v10, WebAuthn replaces U2F. If you didn't configure U2F
...
from U2F. You may write the WebAuthn configuration yourself, but keep the U2F
"app_id" field - doing so ensures any already-registered U2F devices won't need
to be re-registered.

For example, consider the U2F configuration below:

```yaml
# snippet from /etc/teleport.yaml showing an U2F configuration:
auth_service:
  authentication:
    type: local
    second_factor: u2f

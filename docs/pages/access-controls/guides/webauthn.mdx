...
[Enable WebAuthn support](#enable-webauthn).

<h2 id="migrating-from-u2f">Migrating from U2F</h2>
...
If you have a pre-existing Teleport cluster configured with either
...
It is recommended to capture the inferred `rp_id` in your `/etc/teleport.yaml`
file. You can find the inferred `rp_id` in the Teleport Auth Server logs:
...
<h2 id="enable-webauthn">Enable WebAuthn support</h2>

WebAuthn is disabled by default. To enable WebAuthn support, edit the Teleport
...
## Register WebAuthn devices as a user

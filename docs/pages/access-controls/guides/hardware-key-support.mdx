...
<Admonition type="warning" title="Compatibility Issues">
  ...
  Hardware-backed Login Sessions provides the best security available, but it is a new feature with some compatiblity trade-offs. We hope to improve compatibility in the future.
...
You can also enabled Hardware Key support cluster-wide by updating your Teleport configuration as below:
...
Affected users with existing sessions will be prompted to re-login on their next request:

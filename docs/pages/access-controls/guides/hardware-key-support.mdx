---
title: Hardware Key Support (YubiKey)
...
<ScopedBlock scope="oss">
  ...
  Hardware-backed Login Sessions require Teleport Enterprise or Teleport Cloud.

</ScopedBlock>

<Admonition type="note" title="PIV support">

  Hardware-backed Login Sessions require a PIV-compatible hardware key. Currently, this feature is only guarenteed to support YubiKey series 5+.

</Admonition>

<Admonition type="warning" title="Compatibility Issues">

  Hardware-backed Login Sessions provides the best security available, but it is a new feature with some compatiblity trade-offs. We hope to remove these incompatibilities as development progresses.
  
  Currently, Hardware-backed Login Sessions support `tsh`, `tctl`, and Teleport Connect for:
    
    - Standard Teleport API requests (`tsh ls`, `tctl create`, etc.)
    - Server Access
    - Database Access (using `tsh proxy db` instead of `tsh db connect`)
    - Application Access
    - Desktop Access
  
  Hardware-backed Login sessions do not yet support:
  ...
  To navigate these incompatiblities, we recommend enabled Hardware Key support only when necessary, such as for roles with access to critical infrastructure. These roles can be accessed as needed with Access Requests so that users can avoid these issues for their normal login sessions.
...
## Step 1/2. Enable Hardware Key support

Hardware Key support is disbled by default. You can enable Hardware Key support for specific roles like below:
...
You can also enabled Hardware Key support cluster-wide by updating yout Teleport configuration as below:
...
Once enabled on the Auth server, affected users will be prompted to connect and touch their YubiKey on login:
...
Affected user's with existing sessions will be prompted to re-login on their next request:

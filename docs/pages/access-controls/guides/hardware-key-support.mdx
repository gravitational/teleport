...
There are three primary options for hardware key support, in order of security benefit:
  ...
  - `hardware_key_touch`: User keys are stored on their hardware key with touch protection, meaning on each request the user will
  be required to touch their hardware key.
  - `hardware_key_touch_and_pin`: User keys will be stored on their hardware key with touch and pin protection, meaning on each
  request the user will be required to touch their hardware key and enter their PIV pin.
    - If the user has not yet set a PIV pin, they will be prompted to set it during login.
...
If you are using any other PIV applications, you may need to specify a different slot to be used.
For example, `yubikey-agent` uses slot `9a`, so a user with the `hardware_key` requirement would need
to specify a different slot to avoid overwriting their `yubikey-agent` key and cert.

During login, users can specify a piv slot themselves with the `tsh` flag or env var:
  - `tsh login --piv-slot=9c`
  - `TELEPORT_PIV_SLOT=9c tsh login`

The PIV slot can also be set cluster wide with a config option:
...
Teleport Clients generate keys in the slots specified above using the default management key.

If your PIV key has a differement management key set, you will need to generate the key yourself.
This can be done with the [yubikey manager tool command](https://developers.yubico.com/yubikey-manager/):

`ykman piv keys generate -a ECCP256 [slot] --touch-policy=[never|cached|always] --pin-policy=[never|once|always] -`

You'll be prompted to enter your management key to complete the request. Make sure that the touch and pin policy satisfy
the hardware key requirement for your cluster and roles.
...
If you want to use a different management key, you can follow the [custom PIV setup instructions](#custom-piv-setup).

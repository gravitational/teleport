...
There are three primary options for hardware key support:
  ...
  - `hardware_key_touch_and_pin`: User keys will be stored on their hardware key with touch and PIN protection. On each request, users musr touch their hardware key and enter their PIV pin.

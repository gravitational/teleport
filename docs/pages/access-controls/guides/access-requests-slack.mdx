...
Navigate back to the certificates you generated with `tctl auth sign` before. 
...
cd ../../../
# Copy PEM files created by 'tctl auth sign'
sudo cp -f auth.cas /var/lib/teleport/plugins/slack/
sudo cp -f auth.crt /var/lib/teleport/plugins/slack/
sudo cp -f auth.key /var/lib/teleport/plugins/slack/
```
...
```bash
sudo nano teleport-plugins/access/slack/teleport-slack.toml
```
...
3. Copy `teleport-slack.toml` to the location used by the Slack Plugin:

...
## Step 3/4. Export access-plugin certificate
...
The above sequence will generate a single certificate file: `auth`. We'll use `auth` below.
...
<Admonition title="Tip" type="tip">
  It's generally a good idea to create roles that are specific to a task or scenario. Above, we create a role specifically for testing Slack integration.
</Admonition>

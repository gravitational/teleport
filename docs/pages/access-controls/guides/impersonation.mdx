...
<Admonition
  ...
  scope="oss"
>
We also assign the preset `access` role that allows users to access clusters for Alice's
convenience.
</Admonition>

<Admonition
  type="note"
  title="Access and Requester Role"
  scope="enterprise"
>
We also assign the preset `access` role that allows users to access clusters and the
preset `requester` role that allows for requesting of cluster resources for Alice's
convenience.
</Admonition>
...
```code
$ tctl users add alice  --roles=impersonator,access,requester
```
</ScopedBlock>

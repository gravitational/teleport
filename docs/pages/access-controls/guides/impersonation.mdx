...
## Step 1: Create a CI/CD user and corresponding role
...
## Step 2: Create an impersonator role
...
that `alice` can impersonate `jenkins`.who can Note: we also assign the preset
`access` role that allows users to access clusters for Alice's convenience.

```code
$ tctl users add alice  --roles=impersonator,access
```

## Step 3: Use impersonation to issue a certificate
...
- Even though Alice's `max_session_ttl` is 10 hours, she can issue a cert with a
  longer TTL of 240 hours, because `jenkins` role allows it. A certificate's TTL
  issued using impersonation is limited to the `max_session_ttl` of the roles
  being impersonated.
...
While user traits typically from from an external identity provider, we can test
...
Note that Alice will need to log in again to receive the newly updated traits.

...
because `jenkins` role allows it. A certificate's TTL issued using impersonation is limited the `max_session_ttl` of the roles being impersonated.
...
```bash
# Once Alice logins again, she will receive a new certificate with updated roles.

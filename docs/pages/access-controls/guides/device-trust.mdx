...
Ensure only trusted and authenticated device can access protected resources.

Device trust has the following three modes of operation:
...
Cluster-wide configuration will enforce authenticated device access to all the configured roles and resources.
...
accesses. To enable device mode `required` update your configuration as follows:
...
Once this configuration is picked up by the Auth Server, SSH, Database and Kubernetes access without a trusted device will be forbidden.

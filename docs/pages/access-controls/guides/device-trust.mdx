...
<Admonition type="warning">
  Device Trust is currently in Preview mode. Device Trust requires Teleport
  Enterprise.
</Admonition>
...
<Admonition type="tip" title="Editor Role">
  ...
  has the necessary permissions to manage devices. For production setups a
  separate role is still recommended.
</Admonition>
...
## Optional: Cluster-wide device enforcement

Cluster-wide configuration enforces trusted device access at cluster level. It can be configured with the following three modes of operation, represented by the
...
<Admonition type="tip" title="Trusted Clusters">
  ...
  cluster. Likewise, a root cluster will not enforce device trust on resources
  in leaf clusters.
</Admonition>

## Optional: Role-based device enforcement

Role-based configuration enforces trusted device access at role level. It can be configured with `device_trust_mode` (`required`, `optional`, `off`) option and applies to the resources in its `allow` rules. It works similar to [`require_session_mfa`](per-session-mfa.mdx).

To enforce authenticated device checks for a specific role, update the role as the following:
...
Per-role configuration let's progressively block unauthorized device access and is useful for getting started with Teleport Device Trust.

Once all the devices used by engineers are enrolled to Teleport, cluster-wide configuration will ensure every resources can only ever be accessed with a trusted device.

</Admonition>

## Optional: Locking a trusted device

Similar to [session and identity locking](./locking.mdx), a device can be locked using `tctl lock`.

Locking blocks certificate issuance and ongoing or future accesses originating from a locked device.
Locking device only works if device trust is enabled and if the device is enrolled to Teleport.

Find device ID to lock:

```code
$ tctl devices ls
Asset Tag     OS    Enroll Status   Device ID
------------ -----  ------------- ------------------------------------
C0XXXXXXXXXX  macOS  enrolled      9cdfc0ad-64b7-4d9c-9342-50e97f418ba0
```

Lock device:

```code
$ tctl lock --device 9cdfc0ad-64b7-4d9c-9342-50e97f418ba0 --ttl=12h
Created a lock with name "5444970a-39a0-4814-968d-e58b4a8fa686".
```

Now if when user tries to access, say an SSH server, Teleport will deny access.

```code
$ tsh ssh server1
ERROR: ssh: rejected: administratively prohibited (lock targeting Device:"9cdfc0ad-64b7-4d9c-9342-50e97f418ba0" is in force)
```

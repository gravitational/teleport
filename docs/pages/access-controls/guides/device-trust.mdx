...
The device trust preview works only with macOS and Windows devices and supports
the following Teleport features:
...
Device Trust leverages specialist hardware in devices to store sensitive
...
On Windows devices, a TPM (Trusted Platform Module) is used to perform an
attestation as to the state of the device. This attestation is signed by a
private key that is protected by the TPM. This means that we know both
information about the state of the device, and that the request has come from
the device.
...
Before we can enroll the device, we need to register it. In order to register
a device, we need to first determine its serial number.
...
Replace the `$SERIAL` below with the serial number obtained from the device you
wish to enroll and `$OS` with `macos` or `windows`, and execute the command:
...
Enrollments are tied to the current user in the device. If the same device is
handed to a new user, they will need to perform enrollment again.
...
On Windows, the `tsh device enroll` command will attempt to elevate to
administrator privileges on the device. This is required in order to perform a
TPM Credential Activation. This is only required on enrollment and will not be
required on future device authentications.
</Admonition>
...
manufacturer's CA (Certificate Authority). This certificate allows a third party
...
Once you have obtained the CA certificate in PEM format, there are two ways of

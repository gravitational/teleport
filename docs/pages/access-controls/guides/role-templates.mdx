...
traits `logins` and `kubernetes_groups`.  The `internal` notion is limited to a few specific
predefined traits.  The `external.<trait-name>` notation can be used if using custome traits
for Teleport local users.

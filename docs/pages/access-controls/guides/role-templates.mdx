...
## Pattern matching in Access & Review Requests
...
spec:
  allow:
    request:
      # `roles` is a static list of roles a user with the `peodyct-admin` role may 
      ...
      claims_to_roles:
        - claim: 'projects'         
          ...
          roles: ['$1-admin']     # gerenerates a role name from the value capture
```

If we grant Alice the `product-admin` role and add some entried to the `projects` trait:
...
then Alice would be allowed to request access to the RBAC roles `access` (from the static 

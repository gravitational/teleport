...
1. Create an `inference-model.yaml` file with the following content:
   ...
   `bedrock_model_id` is an ARN of the Amazon Bedrock foundation model or an
   inference profile. A Bedrock model ID can also be used.
...
the following command to fetch details of the inference profile:

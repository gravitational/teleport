...
which is granted by default through the built-in `editor` role.
...
also has access to its summary through a `session` resource kind. The built-in
`access` role allows users to only view their sessions; in order to allow a
user to view all recordings and summaries, it's easiest to use the built-in
`auditor` role.
...
appear up to a minute after the session is uploaded to Auth service.
</Admonition>
...
| resource.spec.hostname | Server hostname (SSH sessions only) | `resource.spec.hostname == "host1"` |
| resource.spec.protocol | Database protocol (DB sessions only) | `resource.spec.protocol == "postgres" |
| resource.spec.uri | Database URI (DB sessions only) | `resource.spec.uri == "db-host:5432" |

...
| Using OpenAI-compatible LLM gateways such as Lite LMM  | ✅ | ✅ |
...
This allows, for example, using the same OpenAI or Bedrock model with different
...
inference profile in Bedrock (not to be confused with Teleport inference
...
creating Bedrock inference profiles and can be used as is. Use the following
command to list models that support on-demand invocation:
...
### Find or create a Bedrock inference profile
...
Next, find an inference profile for that model:
...
The following policy needs to be applied to the IAM role that is used by
  ...
  "Statement": [
    {
      ...
      "Resource": "<Var name="bedrock-model-or-inference-profile-arn"/>"
    }
  ]
}
```

`Resource` is an ARN of the foundation model or an inference profile. Use a
wildcard (`*`) to allow access to all models.
...
1. Create an `inference-model.yaml` file with the following content:
   ...
   `bedrock_model_id` is an ARN of the Bedrock foundation model or an inference
   profile. A Bedrock model ID can also be used.

   If a foundation model is directly invoked (i.e. `bedrock_model_id` points to
   a model, not an inference profile), `region` should be one of the supported
   regions for a given model. Consult the [table of model support by AWS
   region](https://docs.aws.amazon.com/bedrock/latest/userguide/models-regions.html)
   from the official AWS documentation.
   
   If `bedrock_model_id` refers to an inference profile, `region` needs to
   point to a region specified in the inference profile ARN.
...
1. Create the configuration policy in the backend:
...
| Problem | Solution |
| --- | --- |
| Summarization fails with "ValidationException: Invocation of model ID *[model-id]* with on-demand throughput isn’t supported. Retry your request with the ID or ARN of an inference profile that contains this model." | Use an Amazon Bedrock inference profile instead of a model ID in the `inference_model` resource. |
| Summarization fails with "ValidationException: The provided model identifier is invalid." | One possible reason is a mismatch between the `region` declared in the `inference_model` resource and available regions for the given model or inference profile. Make sure that the region is supported. |
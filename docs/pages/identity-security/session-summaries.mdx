...
| Using an Amazon Bedrock model provided by Teleport | ❌ | ✅ (v18.7.1+) |
...
There are three fundamental concepts that underpin session summarization:
...
<Admonition type="warning" scope={["cloud", "enterprise"]} scopeOnly>
This feature is powered by AI leveraging a Large Language Model, and as such,
its accuracy may vary. Teleport doesn't currently support any spend controls;
controlling the LLM token budget is your responsibility.

This feature is powered by AI leveraging a Large Language Model, and as such,
its accuracy may vary. For Teleport Enterprise Cloud customers using the
Teleport-managed model, session recording summary costs are included in the
price of Identity Security. If you bring your own LLM provider, controlling the
...
- A Teleport Enterprise Cloud tenant running v18.7.1 or later with Identity
  Security enabled.
...
- A running Teleport Enterprise cluster v18.2.0 or later with Identity
  Security enabled.
- Access to an LLM API. This can be:
  - OpenAI or OpenAI-compatible inference API, either public or self-hosted.
  - Amazon Bedrock (see the availability table above).
</TabItem>
</Tabs>

<Admonition type="tip" title="Teleport Enterprise Cloud Quick Start" scope={["cloud"]} scopeOnly>
Teleport Enterprise Cloud tenants include a pre-provisioned inference model
(`teleport-cloud-default`) and the `editor` role already has the required
permissions. You can skip directly to
[Step 3: Set up an inference policy](#step-3-set-up-an-inference-policy) to
start summarizing sessions.
</Admonition>

## Step 1: Configure access to inference configuration
...
## Step 2: Configure the inference model
...
<Tabs>
<TabItem label="Teleport-Managed Model" scope={["cloud"]}>
...
underlying model, Claude Sonnet 4.6, and infrastructure. 

No additional configuration is needed for this step. Skip to
[Step 3](#step-3-set-up-an-inference-policy) and reference
...
Bedrock** tabs, or through the Teleport Web UI.

![Create Inference Model dialog in the Teleport Web UI](https://website.goteleport.com/_uploads/internal_ben_session_teleport_sh_web_cluster_internal_ben_session_teleport_sh_recordings_summaries_b778b06360.png)

<Admonition type="note">
The Teleport-managed model is available starting with Teleport v18.7.1. There
are currently no usage limits, but quotas may be introduced in the future.
</Admonition>

</TabItem>
<TabItem label="Web UI">

You can also create an inference model through the Teleport Web UI. Navigate to
**Identity Security -> Session Recording Summaries** and click **Add Inference Model**.
Choose **OpenAI-Compatible API** or **Amazon Bedrock** as the provider, fill in the
required fields, and click **Create**. You can use **Test Connection** to verify
...
## Step 3: Set up an inference policy
...
You can create an inference policy through the Teleport Web UI or using `tctl`.

<Tabs>
<TabItem label="Web UI">

Navigate to **Identity Security -> Session Recording Summaries** and click **Add Inference Policy**.
Select **Teleport Cloud** to use the managed model, or **Use your own model**
to use a model you configured in Step 2. Choose which session types to
summarize (Kubernetes, Database, SSH), give the policy a name, and click
**Create**.

![Create Inference Policy dialog in the Teleport Web UI](https://goteleport.com/_uploads/sh_recordings_summaries_f317d70e86.png)
...
1. Create an `inference-policy.yaml` file with the following content:
   ...
   <Tabs>
   <TabItem scope={["cloud"]} label="Teleport-Managed Model">
...
## Step 4: Configure access to session summaries
...
## Step 5: Conduct a session and view its summary
...
### On-demand summarization is not supported

Summaries are generated automatically after a session ends based on matching
inference policies. It is not possible to manually trigger summarization for a
specific session or re-summarize a session that has already been processed.

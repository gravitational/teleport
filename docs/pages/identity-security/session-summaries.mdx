...
terminal. Start an interactive session and execute any command, quit the session, and go to **Audit -> Session
...
costs, so it's possible to be specific about which sessions will be

...
With Crown Jewels, you can prevent changes in your organization's access
controls from unintentionally allowing access to a resource.
...
Use `affected_resources` to inspect all resources involved in the change. The
older `affected_resource_name`, `affected_resource_source`, and
`affected_resource_type` fields are deprecated and may be omitted.

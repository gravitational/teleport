...
description: Describes how to enable SSH Key Scanning using Identity Security to identify access patterns that use long-lived SSH keys.
tags:
 - how-to
 - identity-security
 - access-risks
...
- `derivation_mode`: The method used to derive the public key from the private key. It can be either `private_key` if the
...
for details on how to enroll devices in Teleport.
...
device. [Download the macOS tsh installer](../../installation/single-machine/single-machine.mdx) to fix
the problem.

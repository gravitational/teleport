...
The `tsh` tool authenticates with the Teleport cluster through the [Device Trust](../../admin-guides/access-controls/device-trust/device-trust.mdx) feature,
...
- Devices enrolled in the [Teleport Device Trust feature](../../admin-guides/access-controls/device-trust/device-trust.mdx).
...
On devices enrolled in the Teleport, you can use the `tsh` CLI tool to scan for SSH private keys. Check [Device Trust](../../admin-guides/access-controls/device-trust/device-trust.mdx)
...
process and periodically scan for SSH private keys. Check the [Jamf Pro Integration](../../admin-guides/access-controls/device-trust/jamf-integration.mdx)
...
in the Teleport Device Trust feature. Check the [Device Trust](../../admin-guides/access-controls/device-trust/device-trust.mdx) page for details on how to enroll devices
in Teleport.

### `"binary missing signature or entitlements"` error

A signed and notarized `tsh` binary is necessary to enroll and use a trusted
device. [Download the macOS tsh installer](../../installation.mdx) to fix
the problem.

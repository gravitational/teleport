...
setup but has a limitation of not being possible to enable the [Identity Security](../teleport-security.mdx) integration.
...
If your Teleport license does not include [Identity Security](../teleport-security.mdx), include the `--no-access-graph` flag.

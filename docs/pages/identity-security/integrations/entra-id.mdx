...
setup but has a limitation of not being possible to enable the [Identity Security](../identity-security.mdx) integration.
...
If your Teleport license does not include [Identity Security](../identity-security.mdx), include the `--no-access-graph` flag.
...
To display Entra ID applications in Graph Explorer, Identity Security needs a file containing the Federated SSO
configuration for every Entra ID application that supports SSO. This file includes each application's details,
such as the Attributes & Claims sent during the SSO process to target applications. These attributes are used to
determine, for example, which IAM roles a user receives when signing into Amazon AWS through an Entra ID–backed
SSO integration.

To set up the Federated SSO cache for your Entra ID tenant, run the following script in the same Azure Cloud Shell session
used above. When the script finishes, download the generated `cache.json` file.

<details>
<summary>Create Federated SSO cache for Entra ID Applications</summary>

```powershell

# PowerShell script for retrieving and caching Azure SSO settings
# Equivalent to teleport/lib/integrations/azureoidc/accessgraph.go
...
# Function to make private API calls to Azure
function Invoke-PrivateApiGet {
    param (
        [System.String]$Token,
        [string]$Endpoint
    )
    ...
    # Generate a correlation ID (GUID)
    ...
    try {
        ...
        Write-Error "Error calling private API: $_"
        throw
    }
}

# Function to get Azure AD tenant ID
...
# Get refresh tokens from the Azure CLI token cache
function Get-RefreshTokens {
    ...
    $tokenCachePath = Join-Path -Path $homeDir -ChildPath ".azure/msal_token_cache.json"
    
    if (-not (Test-Path $tokenCachePath)) {
        Write-Error "MSAL token cache not found at $tokenCachePath"
        return $null
    }
    
    $tokenCacheContent = Get-Content -Path $tokenCachePath -Raw
    $tokenCache = $tokenCacheContent | ConvertFrom-Json
    
    $tokens = @()
    if ($tokenCache.RefreshToken) {
        foreach ($key in $tokenCache.RefreshToken.PSObject.Properties.Name) {
            $token = $tokenCache.RefreshToken.$key
            $tokens += @{
    ...
    if ($tokens.Count -eq 0) {
        throw "No refresh tokens found in MSAL token cache"
    }
    
    return $tokens
}

# Exchange a refresh token for an access token for the private API
function Exchange-Token {
    ...
    try {
        ...
        if ([string]::IsNullOrEmpty($response.access_token)) {
            throw "Expected non-empty AccessToken in the response"
        }
        
        return $response.access_token
    }
    catch {
        Write-Warning "Failed to exchange token: $_"
        return $null
    }
}

# Get a token for the private Azure API using the Azure CLI token cache
function Get-PrivateApiToken {
    param (
        [string]$TenantId
    )
    
    $tokens = Get-RefreshTokens
    if ($null -eq $tokens) {
        throw "Failed to get refresh tokens from MSAL cache"
    }
    
    $lastError = $null
    foreach ($token in $tokens) {
        Write-Debug "Trying token with client ID: $($token.ClientId)"
        
        $accessToken = Exchange-Token -TenantId $TenantId -Token $token
        if ($accessToken) {
            return $accessToken
        }
        
        $lastError = "Failed to exchange token with client ID $($token.ClientId)"
    }
    
    throw "No viable token found: $lastError"
}


# Function to get single sign-on information
function Get-SingleSignOn {
    param (
        [System.String]$Token,
        [string]$ServicePrincipalId
    )
    
    $endpoint = "ApplicationSso/$ServicePrincipalId/SingleSignOn"
    $result = Invoke-PrivateApiGet -Token $Token -Endpoint $endpoint
    
    return $result
}

# Function to get federated SSO V2 information
function Get-FederatedSSOV2Compressed {
    param (
        [System.String]$Token,
        [string]$AppId
    )
    
    # Get the service principal ID from the app ID
    $sp = Get-AzADServicePrincipal -ApplicationId $AppId
    if ($null -eq $sp) {
        throw "Could not retrieve service principal for app ID $AppId"
    }
    
    $spId = $sp.Id
    
    # Get SSO information
    $sso = Get-SingleSignOn -Token $Token -ServicePrincipalId $spId
    
    if ($sso.currentSingleSignOnMode -ne "federated") {
        Write-Debug "App $AppId does not have federated SSO set up"
        return $null
    }
    
    # Get federated SSO V2 information
    $endpoint = "ApplicationSso/$spId/FederatedSsoV2"
    $federatedSSOV2 = Invoke-PrivateApiGet -Token $Token -Endpoint $endpoint
    
    # Convert to JSON and then to bytes for compression
    $federatedSSOV2Bytes = [System.Text.Encoding]::UTF8.GetBytes(($federatedSSOV2 | ConvertTo-Json -Depth 20 -Compress))
    
    # Compress the bytes
    $compressedBytes = Compress-GzipBytes -InputBytes $federatedSSOV2Bytes
    
    return $compressedBytes
}

# Main function to create the TAG cache file
function New-TAGCacheFile {
    # Get tenant ID and token
    $tenantId = Get-TenantId
    $token = Get-PrivateApiToken -TenantId $tenantId
    
    # Initialize cache object
    $cache = @{
        app_sso_settings_cache = @()
    }
    ...
    foreach ($app in $applications) {
        ...
        Write-Host "Processing application: $appId"
        
        try {
            $federatedSSOV2Compressed = Get-FederatedSSOV2Compressed -Token $token -AppId $appId
            
            if ($null -ne $federatedSSOV2Compressed) {
                # Convert byte array to base64 for JSON storage
                $base64Compressed = [Convert]::ToBase64String($federatedSSOV2Compressed)
                
                $cacheEntry = @{
                    app_id = $appId
                    federated_sso_v2 = $base64Compressed
                }
                
                $cache.app_sso_settings_cache += $cacheEntry
                Write-Host "Added SSO info for $appId to cache"
            }
        }
        catch {
            Write-Debug "Failed to retrieve SSO info for app $appId : $_"
        }
    }
    
    # Write cache to file
    $cacheJson = $cache | ConvertTo-Json -Depth 20
    $cacheJson | Out-File -FilePath "cache.json" -Encoding utf8
    
    Write-Host "TAG cache file created successfully at $(Get-Location)/cache.json"
}
...
try {
    ...
    Write-Error "Failed to create TAG cache file: $_"
}
...
<Admonition type="note">
If you would like to disable the Identity Security integration, append the --no-access-graph flag to the command shown below.
</Admonition>

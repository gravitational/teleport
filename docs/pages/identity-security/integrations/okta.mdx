...
```sql
SELECT * FROM access_path WHERE resource_labels @> '{"teleport.dev/origin": "okta"}';
```

#### Show all Okta resources for a specific org

```sql
SELECT * FROM access_path WHERE resource_labels @> '{"okta/org": "https://<Var name="your-org" />.okta.com"}';
```
...
Navigate to **Identity Security** > **Investigate** in the Teleport Web UI to search

---
title: Discover AWS Access Patterns and AWS CloudTrail/EKS events with Teleport Identity Security
sidebar_label: AWS
...
In your `discovery_config` resource, under the `access_graph.aws` section, add an `eks_audit_logs`
section with tags to match AWS EKS cluster tags. Those clusters that match the tags will have
the API server audit logs fetched from CloudWatch Logs and imported into Identity Activity Center.

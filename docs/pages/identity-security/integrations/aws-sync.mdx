...
### Importing AWS EKS API Server audit logs (optional)
...
## Step 3/4. Integrate EKS Audit Logs for Identity Activity Center (optional)
...
- <Var name="discovery-group" />: The `discovery_group` name matching the Teleport configuration.
- <Var name="regions" />: A list of AWS regions in which to discovery EKS clusters.
- <Var name="tag-name" />: The name of a tag that EKS clusters must to match, or `"*"` for any tag.
- <Var name="tag-value" />: The value or list of values that the named tag must have to match, or `"*"` for any value. If a list, only one of the values need to match.

...
Teleport Identity Security help teams expose and eliminate hidden risk in their infrastructure. 

There are some use cases of Teleport Identity Security  

- Find users with the highest standing privilege with the Standing Privilege Dashoard.
- Visually understand a users access pathways, or resources pathways with Graph Explorer.
- Quickly query key access pathways with SQL Editor.
- Know when access pathways change for a critial asset wtih Crown Jewel Alerting.
- Investiage Identity Breaches by corelating audit logs with Identity Actvitiy Center. 
...
## Dashboard: Identity Actvity Center - Investigate

The Identity Security with Identity Actvity Center dashboard provides a high-level overview of recent identity activity
across your infrastructure. Injesting and corelating data between IdPs [(]Okta]()), Code (GitHub), Cloud Providers (AWS) and 
Teleport Audit Log. 

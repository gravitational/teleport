...
(**) For Teleport Cloud tenants running v18.7.1 or later.
...
Session Recording Summary Player and Timeline
![Session Detail with High Alert](https://website.goteleport.com/_uploads/session_timeline_high_0755c66e78.png)
...
<Tabs>
<TabItem label="Teleport-Managed Model">
...
![Create Inference Model dialog in the Teleport Web UI](https://website.goteleport.com/_uploads/inference_model_9e1bc592d2.png)
...
![Session recording list screen with a summary visible](https://website.goteleport.com/_uploads/session_summary_list_3fe0cae7ac.png)
...
The `desktop` kind matches both Windows and Linux desktop sessions. To target
one operating system, combine the kind with a filter on `resource.kind`, which
is `windows_desktop` for Windows desktops and `linux_desktop` for Linux
desktops.
...
Because the `desktop` kind covers both Windows and Linux desktops, narrowing a
policy to a single operating system requires a filter on `resource.kind`. The
following example only matches Linux desktop sessions:

```yaml
kind: inference_policy
version: v1
metadata:
  name: linux-desktop-sessions
  description: Summarize Linux desktop sessions
spec:
  kinds:
  - desktop
  filter: 'equals(resource.kind, "linux_desktop")'
  model: shell-summary-model
```
...
| session.cluster_name             | Name of the Teleport cluster                                                                                                                                    | `session.cluster_name == "teleport.example.com"`          |

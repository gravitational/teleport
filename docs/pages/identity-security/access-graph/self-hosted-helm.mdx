...
Access Graph is a feature of the [Identity Security](https://goteleport.com/platform/identity-security/) product that is only available
...
- A running Teleport Enterprise cluster v18.0.0 or later.
  - For the purposes of this guide, we assume that the Teleport cluster is set up
    [using the `teleport-cluster` Helm chart](../../installation/self-hosted/helm-deployments/helm-deployments.mdx)
...
To access the interface, your user must have a role that allows `list` and
`read` verbs on the `access_graph` resource. The preset `editor` role has the
required permissions by default. To create a role with these permissions, open a
file called `access-graph-reader.yaml` with the following content:

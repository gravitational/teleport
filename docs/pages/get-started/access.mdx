---
title: "Get Started with Teleport Access Controls"
...
You were able to connect to your Ubuntu server in [Get Started Connecting
Infrastructure to Teleport](connect.mdx) because of the preset roles assigned to
your user account when you created your cluster.
...
<EnterpriseFeatureWidget title="Manage role assignments at scale with Access Lists">
   As your organization grows, assigning roles user-by-user becomes hard to audit.
   **Access Lists** let you grant roles to a group of users with required-role
   gates, periodic reviews, and a clear audit trail. See [Getting Started with
   Access Lists](../identity-governance/access-lists/guide.mdx).
</EnterpriseFeatureWidget>

## Next steps

In the next step, we'll connect your identity provider so your team can log in to Teleport through SSO.

<div style={{ marginTop: 'var(--m-4)' }}>
  <Button style={{ padding: '0 var(--m-2)' }} as="link" href="../sso/" variant="primary" shape="lg">Get Started with Single Sign-On <Icon name="arrowRight" inline size="sm"/> </Button>
</div>

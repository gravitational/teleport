...
| Teleport Database Service    | Database-native wire protocols such as Redis, MySQL, plus HTTP-based database protocols such as DynamoDB.|
...
| Teleport Desktop Service     | Remote Desktop Protocol traffic to remote servers. Converts incoming RDP traffic to Teleport Desktop Protocol traffic before returning itto the Teleport Web UI via the browser.|
...
|---|---|
|Databases|The user retrieves a certificate for a database with `tsh db login`. The Teleport Database Service verifies the certificate when a user connects to a database.|
...
|Web applications|The user authenticates to the Teleport Web UI and visits a Teleport-protected application. The Teleport Proxy Service forwards traffic to the application along with Teleport-signed JSON web tokens that the application can verify. |
...
mode supports the Teleport SSH Service, Teleport Desktop Service, and Teleport
Kubernetes Service.
...
|---|---|
|`tsh proxy app`|[TCP applications](../application-access/guides/tcp.mdx)|

...
As explained in the [architecture document](overview.mdx#design-principles),
...
happens for *the first time*, clusters know nothing about each other, thus a shared secret needs to exist for "root" to accept the connection from
"leaf".

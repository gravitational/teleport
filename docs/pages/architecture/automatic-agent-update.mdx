...
Starting with version 13, Teleport supports automatic agent updates for Linux
distributions using `apt` or `yum` package managers, and Kubernetes clusters.
...
validates the OCI image signature (using cosign's signature format).

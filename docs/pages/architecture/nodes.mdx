...
<Notice>
  As of Teleport `v13` proxy recording mode is not required for capturing
  OpenSSH recordings. Using the default `node` or `node-sync` recording mode is
  recommended.  
</Notice>
...
helpful when gradually transitioning large server fleets to Teleport. Now all
recording modes as of `v13` can record OpenSSH interactions that go through the proxy.

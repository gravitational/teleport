...
ping messages. They report their IP addresses and the values of their assigned
...
through a cluster's proxy. When the proxy server receives a connection request
...
on the Teleport proxy server. This is not the case in the default configuration

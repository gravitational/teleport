...
- **Cluster-Oriented Design**: Teleport is built for managing clusters, not individual servers. In practice this means that hosts and users
...
- [Teleport CLI client: `tsh ssh`](../setup/reference/cli.mdx#tsh-ssh) (works on Linux, macOS and Windows)
...
>
  ...
  occur on the proxy, see [Audit Reference](../setup/reference/audit.mdx) for more
  information.
</Admonition>
...
When running `tctl`, a user must authenticate to a Teleport cluster. `tctl` can
be run either on the same host as the Teleport Auth Service or from a remote
host.

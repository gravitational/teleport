...
1. A [`tsh` client](../cli-docs.mdx#tsh) generates an OpenSSH key pair. It forwards the generated public key, username, password, and 2nd-factor token to the proxy.
...
3. If Auth Service accepts credentials, it generates a new certificate signed by its user CA and sends it back to the Proxy Server. The certificate has a TTL
   that defaults to 12 hours but can be configured in
   [`tctl`](../cli-docs.mdx#tctl).
4. The Proxy Server returns the user certificate to the client and the client stores it in `~/.tsh/keys/example.com`. The certificate is also added to the local SSH agent if
   one is running.
...
<Admonition
  ...
  title="NOTE"
>
  Teleport's proxy command makes it compatible with [SSH jump hosts](https://wiki.gentoo.org/wiki/SSH_jump_host) implemented using OpenSSH's `ProxyCommand`. It also supports OpenSSH's ProxyJump/ssh -J implementation as of Teleport 4.1. See [User Manual](../user-manual.mdx#ssh-proxy-configuration)
</Admonition>
...
its SSH connection to the final destination server, effectively becoming an
...
2. Recording proxy mode requires SSH agent forwarding. Agent forwarding is

...
<Admonition
  ...
  title="NOTE"
>
  Teleport's proxy command makes it compatible with [SSH jump hosts](https://wiki.gentoo.org/wiki/SSH_jump_host) implemented using OpenSSH's `ProxyCommand`. It also supports OpenSSH's ProxyJump/ssh -J implementation.
</Admonition>
...
See the [reference](../setup/reference/audit.mdx#recorded-sessions) to learn how to turn

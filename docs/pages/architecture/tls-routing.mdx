...
<Details
  ...
  min="13.0"
>
  TLS routing behind layer 7 (HTTP/HTTPS) load balancers and reverse proxies
  support is available in Preview starting from Teleport `13.0`.
</Details>
...
connection is tunneled through. The local proxy uses ALPN values like
...
Teleport proxy to serve a single port behind a layer 7 load balancer or reverse
proxy.
...
Teleport clients automatically detect whether Teleport proxy behind a layer 7
load balancer or a reverse proxy. In such cases, the client initiates a
connection upgrade similar to "WebSocket" and then sends the TLS routing
request through the upgraded connection.
...
When transporting SSH protocol over TLS routing, `tsh` make connection upgrades
seamlessly. This is applicable to `tsh ssh/scp` commands, as well as `tsh proxy
ssh` when connected via `ProxyCommand` using OpenSSH clients.

### Kubernetes

The `tsh proxy kube` command creates a local proxy and an ephemeral kubeconfig
for Kubernetes clients like `kubectl` to connect to. The local proxy generates
a self-signed certificate to secure local communications with the Kubernetes
clients.
...
The Teleport web UI, along with all its features such as Desktops and AWS
Console, is fully functional with standard browsers, without any special
ALPN/SNI values or special connection upgrades.
...
`tsh` Cloud CLI commands like `tsh aws` transparently starts a local proxy that
performs connection upgrades for TLS routing. To start local proxies for native
applications, one may use commands such as "tsh proxy aws" and similar ones.

...
connection upgrade similar to "WebSocket" to bypass the load balancer and then
tunnels the TLS routing traffic within.

![Connection upgrade](../../img/architecture/tls-routing-connection-upgrade.svg)

Non-Teleport clients require local proxies that can perform the connection
upgrade. For example, the `tsh proxy kube` command creates a local proxy and an
ephemeral kubeconfig for Kubernetes clients like `kubectl` to connect to.

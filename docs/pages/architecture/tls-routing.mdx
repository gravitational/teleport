...
<Admonition
  type="warning"
  title="TLS routing is incompatible with load balancers and reverse proxies"
>
...
- AWS ALB (Application Load Balancers), especially when using TLS via ACM (Amazon Certificate Manager)
- Commonly used reverse HTTP proxies like nginx, Apache, Caddy, Traefik, HAProxy and others

Deploying Teleport in TLS routing mode behind these proxies will result in a Teleport web UI experience that seems
to work perfectly, but the use of `tsh`, `tctl` and attempting to join remote Teleport services to the cluster will fail
with errors like `ssh: overflow reading version string` and `EOF`. A working Teleport web UI is not an indication of a
correctly configured Teleport cluster.

If in doubt, remove all load balancers/proxies from the equation and connect to Teleport directly to isolate the issue.

To use Teleport behind a reverse proxy, either:
- use a layer 4 (TCP) proxy only which forwards TCP streams directly to Teleport (which will then handle TLS termination itself)
- deploy Teleport in its [../reference/networking.mdx](separate ports) mode by added `version: v1` to `/etc/teleport.yaml` and removing `proxy_listener_mode: multiplex`
</Admonition>

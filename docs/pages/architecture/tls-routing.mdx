...
- deploy Teleport in its [../reference/networking.mdx](separate ports) mode by adding `version: v1` to `/etc/teleport.yaml` and removing `proxy_listener_mode: multiplex`
</Admonition>

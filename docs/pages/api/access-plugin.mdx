...
First, we make some globally scoped declarations:

```go
(!examples/access-plugin-minimal/types.go!)
```
...
Later in the program, we will need a way to map these states to strings that we
...
The plugin will call a handler function when it receives an event. To set this
up, we use the `run` method below with a `handleEvent` function that determines
whether to call `updateSpreadsheet` or `createRow` depending on whether an
incoming Access Request event is a new Access Request or an update to an
existing one:
...
To pass events to `handleEvent`, we define a `run` method, which contains the
main loop for our demo plugin.
...
The result of `NewWatcher`, a `types.Watcher`, enables `run`  to respond to
new audit events by calling the `Events` method. This returns a Go
**channel**, a runtime abstraction that allows concurrent routines to
communicate. Another channel, returned by `Done`, indicates when the watcher
has finished.
...
`Events` channel, it calls the `handleEvents` method to process the event.

...
After the `import` block, you can find some globally scoped declarations:
...
way to look up the row in the spreadsheet that corresponds to the Access Reqeust
...
Our plugin determines whether to call `updateSpreadsheet` or `createRow`
...
The plugin will call our handler function when it receives an event. To set this
up, we use the `run` method below: 
...
Sheets. Execute the following commands from within

...
description: Manage Access Requests using custom workflows by using the Teleport API
---
...
- Go installed on your workstation. See the [Go download
  page](https://go.dev/dl/). You will not need to be familiar with Go to
...
`proxyAddr` indicates the address and port of your Teleport Proxy Service or
Teleport Enterprise Cloud tenant. `initTimeout` is the maximum time we will wait
to connect to the Auth Service, in this case, 30 seconds.
...
func (g *googleSheetsPlugin) makeRowData(ar types.AccessRequest) *sheets.RowData {
	...
	u := ar.GetUser()
	s := strings.Join(ar.GetRoles(), ",")
	i := ar.GetName()
	a, ok := requestStates[ar.GetState()]
	l := fmt.Sprintf(
		"=HYPERLINK(\"%v\", \"%v\")",
	...
	if !ok {
		a = requestStates[types.RequestState_NONE]
	}
...
func (g *googleSheetsPlugin) updateSpreadsheet(ar types.AccessRequest) error {
	...
	if len(s.Sheets) != 1 {
		return errors.New("the spreadsheet must have a single sheet")
	}
...
func (g *googleSheetsPlugin) handleEvent(ctx context.Context, event types.Event) error {
	...
	// Ignore Access Requests that are not Role Requests
	if len(r.GetRoles()) == 0 {
		return nil
	}
...
Since this plugin only processes Role Access Requests, we return early if the
request has no associated roles. We then check whether the request is in a
pending state, i.e., whether the request is new. If so, we call `createRow`. If
not, we call `updateSpreadsheet`.
...
Sign into the Teleport Web UI as your original user. In your spreadsheet,  click
"View Access Request" next to your new request. When you submit your review,
...
While this plugin is rather simple, you can use the Teleport API to set up more
sophisticated workflows that take full advantage of your communication and
project management tools. For example, the
...
application like Discord or Slack.

See the
[`gravitational/teleport-plugins`](https://github.com/gravitational/teleport-plugins)
repository on GitHub for examples of plugins developed at Teleport.

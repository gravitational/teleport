...
clusters](../kubernetes-access/discovery.mdx) in AWS, Azure, and Google Cloud,
as well as [servers](../server-access/guides/ec2-discovery.mdx) on Amazon EC2.
...
- For any resources in your infrastructure that are not registered with
  ...
  the resource with an exisitng Teleport service.
...
```code
$ mkdir register-app-service
$ cd register-app-service
$ go mod init register-app-service
go: creating new go.mod: module register-app-service
```
...
When writing a Teleport API client application, you must define a Teleport role
that gives the application adequate permissions for the API operations it needs
to perform.

This program will authenticate to Teleport as a user with permissions to create
join tokens, list applications, and delete records of Application Service
instances registered with the Auth Service. The user also has permissions to
access applications with any label (`app_labels`), which is necessary for
listing applications.

Define a user and role called `register-apps` by adding the following content to
a file called `register-apps.yaml`:
...
To grant your user impersonation privileges for `register-apps`, define a role
called `register-apps-impersonator` by pasting the following YAML document into
a file called `register-apps-impersonator.yaml`:
...
Our demo application will watch for containers running RabbitMQ, a popular open
...
- If a RabbitMQ container does not conrrespond to a management API endpoint,
...
While the client application we will write in this guide launches a new
Application Service instance to proxy every instance of the target application,
you can also proxy multiple applications through the same Application Service
instance.
...
|`encoding/hex`|The number generator we use from `crypto/rand` returns data in bytes, so we will encode these as hexidecimal strings using this package.|
|`fmt`|Formatting data for printing, strings, or errors.|
|`net/url`|Parsing URLs.|
...
### Make program-level declarations
...
|`tokenTTL`|How long of a TTL to set for join tokens that Application Service instances will use to establish trust with the Teleport cluster. This client application will use join tokens immediately after creating them, so we can set this TTL to a small value (5 minutes in this case) to prevent this credential from leaking.|
...
Below the `const` declaration, add the following: 
...
In our application, we will list Application Service instances and examine the
resources they are proxying. This because we are spinning up one Application
Service instance per application. Other client applications may need to look up
dynamically registered resources instead.

In `main.go`, add the following below the `tokenDemoApp` declaration:
...
We will generate the second map in the next section.
...
Next, we will write a function that fetches the URLs of our RabbitMQ containers
.

Add the following function below `listRegisteredAppURLs`:
...
Add the following two functions below `listAppContainerURLs`:
...
string in hexidecimal format, which we will use for the join token. You can use
...
Add the following below  `createAppToken`:
...
additonal flags that launch the container as an Application Service instance and
...
Add the following function below `startApplicationServiceContainer`:
...
To deregister the Application Service instance manually, we use the
`*Client.DeleteApplicationServer` method of Teleport's API library, using the
`p` parameter of our function to get the namespace, host ID, and name of the
Application Service instance to delete.
...
Add the following below `pruneAppServiceInstance`:
...
Add the following below `*tokenDemoApp.reconcileApps`:
...
We are now ready to tie our application together with the entrypoint function,
`main`. Add the following below the `newTokenDemoApp` function:
...
Now that you have written your demo application, run it to see how Teleport can
register and deregister resources in your infrastructure to synchronize with
your service discovery solution.
...
Run three new RabbitMQ containers:

...
## Step 1/6. Set up your Kubernetes cluster
...
Define a role and role binding that allows users in the `app-developer` group to
read and list pods in the `app` namespace. Add the following to a file called
`pod-reader.yaml`:
...
Ensure that you are connected to the right Kubernetes cluster:
...
## Step 3/6. Write the client application
...
$ mkdir sync-roles
$ cd sync-roles
$ go mod init sync-roles
go: creating new go.mod: module sync-roles
```

This creates a new file in your project directory, `go.mod`, which Go's package
management system uses to fetch your client application's dependencies.
...
Here are the packages our client application will import from Go's standard
library:
...
|`io/ioutil`|Helper functions for dealing with I/O operations, e.g., reading files or network sockets.|
...
We will define constants for the program in a visible location so, later on,
it's easier to make them configurable outside the program.

Add the following to `main.go` after the package imports:
...
### Initialize a Kubernetes RBAC client
...
Add the following below the `const` block:
...
`getRBBACClient` opens and reads the Kubernetes credentials file at
...
### Create a Teleport role from a Kubernetes cluster role binding

Add the following function below `getRBACClient`. This creates a Teleport role
...
To avoid unexpected behavior, we will ignore Kubernetes-managed roles and
roles for internal systems like the Teleport Kubernetes Service. This function
checks the cluster role binding's metatada for an annotation with a specific
...
### Create a Teleport role from a Kubernetes role binding
...
### Create Teleport roles based on Kubernetes resources
...
### Initialize clients and start the application

The functions we declared earlier require a Teleport API client and a
Teleport-registered Kubernetes cluster, and we will initialize these in the
...
If there is, the code calls the `createTeleportRolesForKubeClusters` function we
defined earlier. If not, the program prints an error message and a stack trace
by calling Go's built-in `panic` function.

## Step 5/5. Test your client application

Now that you have written your client application, retreive its dependencies and
start it up from within your Go project directory:
...
The remaining fields use the default values.
...
- [Kubernetes clusters](../manage-access/rbac.mdx)

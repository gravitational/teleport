...
- User assigned a Teleport role allowed to maintain users such as the built-in `editor` role.
...
Create a user `api-user` with the built-in role `access`:

```code
# Add a user and login via the Proxy Service
$ tctl users add api-user --roles=access
```

## Step 2/3. Generate client credentials

Log in as the newly created user with `tsh`. Set the proxy host (e.g. `teleport.example.com` or in the form `mytenant.teleport.sh`
for Teleport Enterprise Cloud tenants).

```code
# generate tsh profile
$ tsh login --user=api-user --proxy=<Var name="teleport.example.com" />
```
...
```code
$ go list -f '{{.Version}}' -m "github.com/gravitational/teleport/api@$(git rev-parse v(=teleport.version=))"
...
func main() {
  if len(os.Args) != 2 {
    log.Fatalf("Require proxy host (teleport.example.com or mytenant.tenant.sh) argument")
    return
  ...
  clt, err := client.New(ctx, client.Config{
    Addrs: []string{
      //Alter if using different ports
      fmt.Sprintf("%s:443", proxyHost),
      fmt.Sprintf("%s:3025", proxyHost),
      fmt.Sprintf("%s:3024", proxyHost),
      fmt.Sprintf("%s:3080", proxyHost),
    },
...
```code
$ go run main.go <Var name="teleport.example.com" />
```

...
$ go get github.com/gravitational/teleport/api/v(=teleport.major_version=)@v(=teleport.version=)
```
...
import (
	...
	"github.com/gravitational/teleport/api/v(=teleport.major_version=)"
)

func main() {
	ctx := context.Background()

	clt, err := api.New(ctx, api.Config{
		Addrs: []string{

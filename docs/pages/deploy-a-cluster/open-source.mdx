...
- A Linux machine with only port `443` open to ingress traffic. You must be able
  ...
  - [Local Kubernetes Cluster](../try-out-teleport/local-kubernetes.mdx)
  - [Docker Compose](../try-out-teleport/docker-compose.mdx)
...
<Tabs>
  ...
  Create a systemd unit file using the `teleport install systemd` command on Teleport `v10.1.4` and later. Enter the unit file path you'd
  like to use for your configuration as part of the command. We recommend the path `/etc/systemd/system/teleport.service` for most use cases:

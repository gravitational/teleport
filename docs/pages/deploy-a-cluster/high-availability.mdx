...
- An **automated TLS credential provisioner** that obtains TLS credentials from
  a certificate authority like Let's Encrypt (or an internal public key
  infrastructure), renews the credentials, and provisions `teleport` instances
  with them.
...
Routing](../management/guides/tls-routing.mdx) in your Teleport cluster.
...
listens on a separate port for each protocol.
...
The approach we describe in this guide uses a Layer 4 load balancer for
simplicity, but users that require a separate load balancer for HTTPS traffic
should disable TLS Routing.
...
depending on whether you will enable TLS Routing:
...
configures the key-value store and object store for the Teleport Auth Service.
...
In the `storage` section, the `audit_sessions_uri` field names the bucket within
Google Cloud Storage or an S3-compatible service to use for session recordings,
and the remaining fields in `storage` are specific to each backend. 

Consult our [Backends Reference](../reference/backends.mdx) for the values you
should use.  

...
- **Auth Service load balancer:** A load balancer to forward traffic from a
  ...
  traffic to the Auth Service's gRPC endpoint. It can also handle traffic to
  Teleport Kubernetes Service instances running in the Auth Service pool in
...
balancer to port `3026` on an Auth Service instance, then configure each Auth
Service instance to run the Teleport Kubernetes Service as well to let operators
securely access your Kubernetes cluster via Teleport.
...
The `proxy_service` section configures the Proxy Service, which we will run
together on each Teleport instance. The configuration will depend on whether you
are enabling TLS Routing in your cluster:
...
proxy_service:
  enabled: true
  public_addr: "mycluster.example.com:3080"
  https_keypairs:
...
| `3026` | Optional port to open for traffic to the Teleport Kubernetes Service (if deploying on Kubernetes).|
| `3022` | Optional port to open for traffic to the Teleport SSH Service (if deploying on a VM).|
...
#### `ssh_service` or `kubernetes_service`

The SSH Service is enabled by default.  If you are deploying Teleport on a
cluster of virtual machines, run the SSH Service and enable secure access to the
host.

If you are running Teleport on Kubernetes, you can run the Kubernetes Service on
your Auth Service instances to enable secure access to your Kubernetes cluster.
The Kubernetes Service detects that it is running on Kubernetes automatically.

In this configuration, the Kubernetes Service listens on port `3026` of each
Auth Service host. Its public address is the fully qualified domain name of your
Teleport cluster with port `3026`:
...
kubernetes_service:
  ...
  public_addr: "mycluster.example.com:3026"
```

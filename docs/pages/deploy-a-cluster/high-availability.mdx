...
- A **key-value store** for Auth Service state and audit events that all
  `teleport` instances can access. This requires permissions for `teleport`
...
| Port | Description |
| - | - |
| `3021` | Port used by Teleport Proxy Service instances to dial agents in [Proxy Peering mode](../management/operations/proxy-peering.mdx). |
...
| `3025` | TLS port used by the Auth Service to serve its gRPC API to Teleport services  in a cluster. |
...
Teleport instances need permissions to read from and write to your chosen
...
High-availability Teleport deployments require an automated system to fetch
certificates from Let's Encrypt and provision Teleport instances with them. 
...
behind a load balancer, you will need to use the [ACME
...
2esources by launching Teleport services. You can run these services in a

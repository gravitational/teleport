...
Teleport generates User, Host, JWT, and Database Certificate Authorities (CAs) during
the first Auth Server's initial startup.
...
```code
$ gcloud kms keyrings create "teleport-keyring" --location global
```
...
```code
$ export KEYRING="teleport-keyring"
...
Read on to [choosing a protection level](#choosing-a-protection-level) to decide if software or HSM keys are
right for your cluster.

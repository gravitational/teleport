...
Teleport generates User, Host, JWT, and Database Certificate Authorities (CAs)
during the first Auth Server's initial startup.
These CAs are used to sign all certificates issued to clients and hosts in the
Teleport cluster.
When configured to use Google KMS, all private key material for these CAs will
be generated, stored, and used for signing inside of Google KMS.
In short, private key material will never leave Google KMS.
...
startup with no specific interventions required after configuration. If
configuring an existing Teleport cluster to switch to KMS keys after it has
already started with software keys a CA rotation must be performed, read on to
[migrating an existing cluster](#migrating-an-existing-cluster) to learn more.
...
It is recommended to create a unique key ring for use by Teleport to logically
...
```code
$ gcloud kms keyrings create "<Var name="teleport-keyring" description="The name for your teleport-specific keyring"/>" --location <Var name="global" description="The location for your Teleport keyring"/>
```

The rest of the guide will assume you have a key ring in the `global` location
called `teleport-keyring`, replace this with your desired location and keyring
name as necessary. The example will use the `global` location but you can use
...
```code
$ export KEYRING="<Var name="teleport-keyring"/>"
$ gcloud kms keys list --location global --keyring "${KEYRING}"
$ gcloud kms keys create --keyring "${KEYRING}" --location global "test-key" \
    --purpose asymmetric-signing \
    --default-algorithm rsa-sign-pkcs1-4096-sha512
$ echo hello > /tmp/hello.txt
$ gcloud kms asymmetric-sign --keyring "${KEYRING}" --location global \
    --key "test-key" --version 1 \
    --input-file /tmp/hello.txt --signature-file /tmp/hello.sig
$ gcloud kms keys versions destroy --keyring "${KEYRING}" --location global --key "test-key" 1
```
...
```code
$ gcloud kms keyrings list --location <Var name="global"/>
```
...
auth_service:
  ...
  ca_key_params:
    gcp_kms:
      keyring: "projects/<your-gcp-project>/locations/<location>/keyRing/teleport-keyring"
...
```code
$ gcloud kms keys list --keyring "<Var name="teleport-keyring"/>" --location <Var name="global"/>
```

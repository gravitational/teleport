...
For the teams with limited bandwidth who need multi-region clusters, Teleport
offers hosting cross-region clusters so you can benefit from the
setup described in this document without the operational costs and complexity.
See [the Teleport Enterprise Cloud page](../choose-an-edition/teleport-cloud/introduction.mdx)
for more details.
...
[Proxy Peering](../architecture/proxy-peering.mdx) to connect users to resources
hosted a different region.

The recommended multi-region backend
is [the CockroachDB backend](../reference/backends.mdx#cockroachdb)
available in [Teleport Enterprise](../choose-an-edition/teleport-enterprise/introduction.mdx).
...
- A **multi-region CockroachDB cluster**. You can use the CockroachDB core
  features but optimal performance and fault-tolerance is achieved by using
  [the multi-region features from CockroachDB Enterprise](https://www.cockroachlabs.com/docs/stable/enterprise-licensing).
...
This section describes this setup with more details.
...
- Ensure pod mesh connectivity. Proxy pods must be able to dial each other
  [on port 3021](../reference/networking.mdx#ports). The setup will vary
...
<Admonition type="note" title="Impact of the CockroachDB regions">
    ...
    on different continents) this will make CockraochDB write operations
    slower.
</Admonition>
...
<Admonition type="note">
    CockraochDB core does not offer the regional features CockraochDB enterprise
...
The CockraochDB performance depends on the storage type. Make sure to store
...
user activity, ...), so you will need to test and adapt the sizing based on your
specific needs.
...
<Admonition type="important">
    ...
    we don't recommend setting CPU limits to Teleport nor CockroachDB pods.
    ...
    As rule of thumb, most critical pod `resources` section should look like this:
...
<Admonition type="note">
    If you want Teleport to automatically cleanup session recordings, you must
    to enable "Delete marker replication" in the replication rules.
</Admonition>

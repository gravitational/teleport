...
With DynamoDB, the kube operator supports <=2 pod replicas per cluster. So, It is not recommended to enable `kube operato` on 
secondary region, as it will create instability to the cluster. We will enable `kube operator` in the event of Primary Region failure.

...
### Key Components
- Two ReplicaSets of Teleport Auth and Proxy components are deployed in separate regions. In this example, the us-west-1 (Primary)
  region Teleport cluster is the Active cluster, and the us-east-1 (Secondary) region Teleport cluster is the Passive cluster.
- All teleport cluster components are deployed as ReplicaSets in both regions.
...
## Step 3/7. Set up Teleport Cluster in Primary Region (In this example us-west-1 is the Primary Region):
...
## Step 4/7. Setup DynamoDB & S3 Global replication to Secondary Region (In this example us-east-1 is the Secondary Region):
...
- In the teleportConfig section, you can customize the Teleport configuration for proxy pods. To set up proxy peering, you would 
  ...
  ```
  - In the above snippet, the `type` is set to `proxy_peering` and `agent_connection_count` is set to `1`. The `agent_connection_count` 
    ...
    higher availability
    Remember ti aookt these changes to your Helm chart accordingly.

## Step 6/7. Set up Teleport cluster in Secondary Region same as Step 3/7:
...
## Step 7/7. Configure Proxy Peering:
...
The Teleport proxy pod replica count should always be 2 per cluster all times in the active region. The default configuration of two proxy replicas in each active region of Teleport is, in part, a recommendation for high availability ensuring uninterrupted access. 
Having more than one proxy also allows for load to be distributed across multiple proxies, reducing the risk of a single point of failure and increasing the system's resilience to outages.

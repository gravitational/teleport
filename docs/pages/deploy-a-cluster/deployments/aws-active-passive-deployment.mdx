...
but same concepts applies with Virtual Machines/EC2 instances.
...
So, It is not possible to enable `kube operator` on the secondary region, as it will create instability to the cluster. 

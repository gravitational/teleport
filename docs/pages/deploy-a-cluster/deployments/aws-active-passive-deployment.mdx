...
- We will use  same (Primary Region cluster) `aws-values.yaml` & `aws-issuer.yaml` config files to install Teleport name space in Secondary Region (us-east-1) by changing the clusterName.
...
With DynamoDB, the kube operator supports <=2 pod replicas per cluster. So, It is not recommended to enable `kube operator` on 
the secondary region, as it will create instability to the cluster. We will enable `kube operator` in the event of Primary Region failure only.

...
Our reference deployment will provision the following instances for your cluster by default with AWS Auto Scaling Groups (ASG):
...
- 1 x m4.large Teleport **node** instance in an ASG. Server instances in this ASG will join with Server SSH Access enabled and connected to the **auth**

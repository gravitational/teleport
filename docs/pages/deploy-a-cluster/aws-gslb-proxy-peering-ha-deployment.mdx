...
The Auth Service instances needs [permissions to read from and write to DynamoDB](https://goteleport.com/docs/reference/backends/#dynamodb), 
as well as to create tables.
...
Depeding on how you choose to deploy the Teleport cluster, the Teleport Auth Service can create the required S3 bucket if it doens't already exist.

<Admonition title="Required permissions">

The Auth Service instances [need permissions to get S3 buckets](https://goteleport.com/docs/reference/backends/#s3) as well as to create, get, list, 
and update objects.
...
<Admonition
  ...
  title="Note"
>
For both Option 1 and 2, agent_connection_count should be set to a value >=2 to decrease

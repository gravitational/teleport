...
- Install the Teleport Enterprise License.
...
If you are experiencing errors connecting with the Teleport Cluster check the proxy
and auth pods status. A successful state should be both pods running as below:
...
```
If a Pod is Pending or not running completely use the `kubectl logs` and `kubectl describe` 
for that pod to check the status. The auth pod relies on being able to allocate a Persistent Volume Claim so that may
put it in a Pending state if no Persistent Volume is available.

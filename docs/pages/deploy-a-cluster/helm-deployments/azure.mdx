...
Now we'll configure the `teleport-cluster` Helm chart to use the `azure` mode. Create a
file called `azure-values.yaml` containing the values you've selected above:
...
azure:
  ...
  clientID: $(az identity show --resource-group <Var name="teleport-rg" /> --name teleport-id --query clientId -o tsv)
highAvailability:
...
We'll now set up DNS `A` records for `teleport.example.com` and `*.teleport.example.com`.

```code
# Change these parameters if you altered them above
$ NAMESPACE=<Var name="teleport-ns" /> 
$ RELEASE_NAME=<Var name="teleport-release" />
$ PREFIX=teleport
$ ZONE=example.com
$ ZONE_RG=dns-rg
$ external_ip="$(kubectl --namespace ${NAMESPACE} get service/${RELEASE_NAME} -o jsonpath='{.status.loadBalancer.ingress[*].ip}')"

...
| Concept | Description |
| - | - |
| Node | Synonym to "server" or "computer", something one can "SSH to". A node must be running the [`teleport`](./setup/reference/cli.mdx#teleport) daemon with "node" role/service turned on. |
...
You should use a [configuration file](#configuration-file) to configure the [`teleport`](setup/reference/cli.mdx#teleport) daemon. For simple experimentation, you can
use command line flags with the [`teleport start`](./setup/reference/cli.mdx#teleport-start) command. Read about all the allowed flags in the [CLI Docs](./setup/reference/cli.mdx#teleport-start) or run `teleport start --help`
...
For a complete reference, see our [Configuration Reference - teleport.yaml](./setup/reference/config.mdx#teleportyaml)
...
when they execute [`tsh login`](./setup/reference/cli.mdx#tsh-login) command. There are three
types of authentication connectors:

### Local connector

Local authentication is used to authenticate against a local Teleport user
database. This database is managed by [`tctl users`](./setup/reference/cli.mdx#tctl-users-add)
...
To add a new user to Teleport, you have to use the [`tctl`](./setup/reference/cli.mdx#tctl) tool on the same node where the auth server is running, i.e.
[`teleport`](./setup/reference/cli.mdx#teleport) was started with `--roles=auth` .
...
Users entries can be manipulated using the generic [resource commands](./setup/reference/resources.mdx) via [`tctl`](./setup/reference/cli.mdx#tctl). For example, to see the full list of user records, an administrator can execute:
...
Use the [`tctl`](./setup/reference/cli.mdx#tctl) tool to register a new invitation token (or
...
If `--token` is not provided, [`tctl`](./setup/reference/cli.mdx#tctl) will generate one:
...
# ---------------                  -----------     ---------------
# abcdefgh-do-not-use-this-token-123 Node            25 Sep 18 00:21 UTC

# ... or revoke an invitation before it's used:
$ tctl tokens rm abcdefgh-do-not-use-this-token-123
```
...
$ sudo teleport start \
   --roles=node \
   --token=abcdefg-insecure-do-not-use-this-token-123 \
   --ca-pin=sha256:7e12c17c20d9cb504bbcb3f0236be3f446861f1396dcbb44425fe28ec1c108f1 \
...
>
  ...
  [`tctl auth rotate`](./setup/reference/cli.mdx#tctl-auth-rotate) .
</Admonition>
...
del`](./setup/reference/cli.mdx#tctl-tokens-rm) command:
...
1. `static labels` do not change over time, while [`teleport`](./setup/reference/cli.mdx#teleport) the process is running.  Examples of static labels are the physical location of nodes, the name of the environment (staging vs production), etc.
...
1. `.bytes` file or `.chunks.gz` compressed format represents the raw session bytes and is somewhat
   human-readable, although you are better off using [`tsh play`](./setup/reference/cli.mdx#tsh-play)
...
Finally, create the connector using [`tctl`](./setup/reference/cli.mdx#tctl)
[resource](./setup/reference/resources.mdx) management command:
...
`/readyz` [diagnostics endpoint](./setup/reference/metrics.mdx) on machines running Teleport. This endpoint
...
- Teleport clients [`tsh`](./setup/reference/cli.mdx#tsh) for users and [`tctl`](./setup/reference/cli.mdx#tctl) for admins
...
To diagnose problems you can configure [`teleport`](./setup/reference/cli.mdx#teleport) to
...
Sometimes you may want to reset [`teleport`](./setup/reference/cli.mdx#teleport) to a clean
...
- `http://127.0.0.1:3000/metrics` is the list of internal metrics Teleport is tracking. It is compatible with [Prometheus](https://prometheus.io/)
  collectors. For a full list of metrics review our [metrics reference](./setup/reference/metrics.mdx).

...
The Teleport daemon is called [`teleport`](reference/cli-docs.mdx#teleport) and it supports
...
When experimenting, you can quickly start [`teleport`](reference/cli-docs.mdx#teleport) with verbose logging by typing [`teleport start -d`](reference/cli-docs.mdx#teleport-start).
...
You should use a [configuration file](#configuration-file) to configure the [`teleport`](reference/cli-docs.mdx#teleport) daemon. For simple experimentation, you can
use command line flags with the [`teleport start`](reference/cli-docs.mdx#teleport-start) command. Read about all the allowed flags in the [CLI Docs](reference/cli-docs.mdx#teleport-start) or run `teleport start --help`
...
For a complete reference, see our [Configuration Reference - teleport.yaml](reference/config-reference.mdx#teleportyaml)
...
when they execute [`tsh login`](reference/cli-docs.mdx#tsh-login) command. There are three
types of authentication connectors:

### Local connector

Local authentication is used to authenticate against a local Teleport user
database. This database is managed by [`tctl users`](reference/cli-docs.mdx#tctl-users-add)
...
To add a new user to Teleport, you have to use the [`tctl`](reference/cli-docs.mdx#tctl) tool on the same node where the auth server is running, i.e.
[`teleport`](reference/cli-docs.mdx#teleport) was started with `--roles=auth` .
...
Users entries can be manipulated using the generic [resource commands](#resources) via [`tctl`](reference/cli-docs.mdx#tctl). For example, to see the full list of user records, an administrator can execute:
...
Use the [`tctl`](reference/cli-docs.mdx#tctl) tool to register a new invitation token (or
...
If `--token` is not provided, [`tctl`](reference/cli-docs.mdx#tctl) will generate one:
...
>
  ...
  [`tctl auth rotate`](reference/cli-docs.mdx#tctl-auth-rotate) .
</Admonition>
...
del`](reference/cli-docs.mdx#tctl-tokens-rm) command:
...
setting up a [Trusted Cluster](architecture/trustedclusters.mdx) if you have workloads split
...
1. `static labels` do not change over time, while [`teleport`](reference/cli-docs.mdx#teleport) the process is running.  Examples of static labels are the physical location of nodes, the name of the environment (staging vs production), etc.
...
1. `.bytes` file or `.chunks.gz` compressed format represents the raw session bytes and is somewhat
   human-readable, although you are better off using [`tsh play`](reference/cli-docs.mdx#tsh-play)
...
- The [configuration file](#configuration) is used for static configuration like
  the cluster name.
- The [`tctl`](reference/cli-docs.mdx#tctl) admin tool is used for manipulating dynamic
  records like Teleport users.

[`tctl`](reference/cli-docs.mdx#tctl) has convenient subcommands for dynamic
...
trust, [`tctl`](reference/cli-docs.mdx#tctl) offers the more powerful, although
...
| - | - | - |
| [`tctl get`](reference/cli-docs.mdx#tctl-get) | Get one or multiple resources | `tctl get users` or `tctl get user/joe` |
| [`tctl rm`](reference/cli-docs.mdx#tctl-rm) | Delete a resource by type/name | `tctl rm user/joe` |
| [`tctl create`](reference/cli-docs.mdx#tctl-create) | Create a new resource from a YAML file. Use `-f` to override / update | `tctl create -f joe.yaml` |
...
user/joe`](reference/cli-docs.mdx#tctl-get)
...
Here's the list of resources currently exposed via [`tctl`](reference/cli-docs.mdx#tctl) :
...
To learn more about Trusted Clusters please visit our [Trusted Cluster Guide](architecture/trustedclusters.mdx).
...
Finally, create the connector using [`tctl`](reference/cli-docs.mdx#tctl)
...
login`](reference/cli-docs.mdx#tsh) and  [`tsh kube login`](reference/cli-docs.mdx#tsh-kube-login) command to retrieve credentials for both SSH and
Kubernetes API.
...
`/readyz` [diagnostics endpoint](reference/metrics-logs-reference.mdx) on machines running Teleport. This endpoint
...
- Teleport clients [`tsh`](reference/cli-docs.mdx#tsh) for users and [`tctl`](reference/cli-docs.mdx#tctl) for admins
...
The `--bootstrap` flag does not affect, except during backend initialization (performed by auth server on the first start), so it is safe for use in supervised/High Availability contexts.
...
1. Replace the Teleport binaries, usually [`teleport`](reference/cli-docs.mdx#teleport)
   and [`tctl`](reference/cli-docs.mdx#tctl)
...
To diagnose problems you can configure [`teleport`](reference/cli-docs.mdx#teleport) to
...
Sometimes you may want to reset [`teleport`](reference/cli-docs.mdx#teleport) to a clean
...
- `http://127.0.0.1:3000/metrics` is the list of internal metrics Teleport is tracking. It is compatible with [Prometheus](https://prometheus.io/)
  collectors. For a full list of metrics review our [metrics reference](reference/metrics-logs-reference.mdx).

...
Take a look at the [Certificates chapter](architecture.mdx#certificates) in the
...
`kubernetes_groups` are assigned to Teleport Roles as shown in the Teleport Enterprise [RBAC](ssh-rbac.mdx#roles)
section.

You may also find it useful to read our [Kubernetes guide](kubernetes-ssh.mdx) which contains some more specific examples
and instructions.

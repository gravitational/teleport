...
the [Teleport Architecture](architecture/teleport-architecture-overview.mdx) and review the key concepts this
...
<Admonition
  ...
  title="Version Warning"
>
  External user identities are only supported in [Teleport Enterprise](enterprise/index.mdx).
...
the command: [`tctl nodes ls`](cli-docs.mdx#tctl-nodes-ls).

#### Remove a Node Immediately

If you need to remove a node immediately, stop the node process and run [`tctl rm node/<UUID>`](cli-docs.mdx#tctl-rm):
...
Architecture](architecture/teleport-auth.mdx#audit-log) to learn more about how the audit log and
...
2. `.log` file or `.events.gz` compressed file contains the copies of the event
   log entries that are related to this session.
...
See [Audit Log Architecture](architecture/teleport-auth.mdx#audit-log) to understand how the
...
["recording proxy" mode](architecture/teleport-proxy.mdx#recording-proxy-mode).
...
from the start of [`tctl`](cli-docs.mdx#tctl) output and copy it to
...
Now you can use [`tsh ssh user@host.example.com`](cli-docs.mdx#tsh) to login
...
| connector | Authentication connectors for [single sign-on](enterprise/ssh-sso.mdx) (SSO) for SAML, OIDC and Github. |
...
To avoid using `--cluster` switch with [`tsh`](cli-docs.mdx#tsh) commands, you
...
>
  ...
  multiple values. This works by "east" establishing a tunnel to *every* proxy
...
Then, use [`tctl create`](cli-docs.mdx#tctl-create) to add the file:
...
tsh ssh --port=22 [root@database.work.example.com](mailto:root@database.work.example.com)
...
Take a look at the [Certificates chapter](architecture/teleport-auth.mdx#authentication-in-teleport) in the
...
>
  ...
  used, make sure *all* connected clusters are running version 2.6+. If one of
...
login`](cli-docs.mdx#tsh) command to retrieve credentials for both SSH and
Kubernetes API.
...
with SSH](kubernetes-ssh.mdx) guide.
...
Roles as shown in the Teleport Enterprise [RBAC](enterprise/ssh-rbac.mdx#roles) section.

You may also find it useful to read our [Kubernetes guide](kubernetes-ssh.mdx)
...
Teleport to access multiple Kubernetes clusters from a root Teleport cluster.
...
>
  ...
  [Cluster State section](architecture/teleport-nodes.mdx#cluster-state) in the Teleport
...
>
  Before continuing, please make sure to take a look at the
  [cluster state section](architecture/teleport-nodes.mdx#cluster-state) in Teleport
  Architecture documentation.
</Admonition>
...
>
  Before continuing, please make sure to take a look at the
  [cluster state section](architecture/teleport-nodes.mdx#cluster-state) in Teleport Architecture documentation.
</Admonition>

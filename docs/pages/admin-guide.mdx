...
the [Teleport Architecture](architecture/overview.mdx) and review the key concepts this
...
<Admonition type="note">
  SSO users can also register 2FA devices, but Teleport will not prompt them
  for 2FA during login. Login 2FA for SSO users should be handled by the SSO
  provider.
</Admonition>
...
A user can register multiple U2F devices [on the
CLI](cli-docs.mdx#tsh-mfa-add).

#### Enabling U2F support

By default U2F is disabled. To enable U2F support:

- Enable U2F in Teleport configuration `/etc/teleport.yaml` (see example
  below).
...
- `app_id` - public address of the Teleport proxy, _including_ the `https://`
...
#### U2F for Teleport logins

U2F for logging into Teleport is only required for [local
users](#local-connector). SSO users should configure multi-factor
authentication in their SSO provider.

Example local user login with U2F:
...
#### U2F for SSH and Kubernetes sessions

Teleport supports U2F authentication on every SSH and Kubernetes "connection"
(a single `tsh ssh` or `kubectl` call, not necessarily every TCP/SSH
connection). This can be enforced cluster-wide or only for some specific roles.
...
clusters matching that role's `allow` section. For example: a user has two
roles, `dev` and `prod`. Role `dev` grants access to SSH nodes in the dev
environment and role `prod` grants access to SSH nodes in production. Only role
`prod` has `require_session_mfa: true` set. This user will be prompted for U2F
authentication when accessing production nodes, but not when accessing dev
nodes.
...
>
  ...
  the node establishing a tunnel to *every* proxy. This requires that an LB
...
Follow our [Kubernetes guide](kubernetes-access.mdx) which contains some more specific

...
See [Github OAuth 2.0](./setup/admin/github-sso.mdx) for details on how to configure it.
...
>
  If you are using [CA Pinning](./setup/admin/adding-nodes.mdx#untrusted-auth-servers) when adding new nodes, the CA pin will change after the rotation. Make sure you use the
...
to a cluster via [node tunneling mode](./setup/admin/adding-nodes.mdx#adding-a-node-located-behind-nat), the following settings

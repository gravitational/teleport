...
| Concept | Description |
| - | - |
| Node | Synonym to "server" or "computer", something one can "SSH to". A node must be running the [`teleport`](reference/cli/teleport.mdx) daemon with "node" role/service turned on. |
...
The Teleport daemon is called [`teleport`](reference/cli/teleport.mdx) and it supports
...
When experimenting, you can quickly start [`teleport`](reference/cli/teleport.mdx) with verbose logging by typing [`teleport start -d`](reference/cli/teleport.mdx#teleport-start).
...
You should use a [configuration file](#configuration-file) to configure the [`teleport`](reference/cli/teleport.mdx) daemon. For simple experimentation, you can
use command line flags with the [`teleport start`](reference/cli/teleport.mdx#teleport-start) command. Read about all the allowed flags in the [CLI Docs](reference/cli/teleport.mdx#teleport-start) or run `teleport start --help`
...
For a complete reference, see our [Configuration Reference - teleport.yaml](reference/product-config/teleport.mdx)
...
when they execute [`tsh login`](reference/cli/tsh.mdx#tsh-login) command. There are three
types of authentication connectors:

### Local connector

Local authentication is used to authenticate against a local Teleport user
database. This database is managed by [`tctl users`](reference/cli/tctl.mdx#tctl-users-add)
...
To add a new user to Teleport, you have to use the [`tctl`](reference/cli/tctl.mdx) tool on the same node where the auth server is running, i.e.
[`teleport`](reference/cli/teleport.mdx) was started with `--roles=auth` .
...
Users entries can be manipulated using the generic [resource commands](#resources) via [`tctl`](reference/cli/tctl.mdx). For example, to see the full list of user records, an administrator can execute:
...
Use the [`tctl`](reference/cli/tctl.mdx) tool to register a new invitation token (or
...
If `--token` is not provided, [`tctl`](reference/cli/tctl.mdx) will generate one:
...
>
  ...
  [`tctl auth rotate`](reference/cli/tctl.mdx#tctl-auth-rotate) .
</Admonition>
...
del`](reference/cli/tctl.mdx#tctl-tokens-rm) command:
...
1. `static labels` do not change over time, while [`teleport`](reference/cli/teleport.mdx) the process is running.  Examples of static labels are the physical location of nodes, the name of the environment (staging vs production), etc.
...
1. `.bytes` file or `.chunks.gz` compressed format represents the raw session bytes and is somewhat
   human-readable, although you are better off using [`tsh play`](reference/cli/tsh.mdx#tsh-play)
...
- The [configuration file](#configuration) is used for static configuration like
  the cluster name.
- The [`tctl`](reference/cli/tctl.mdx) admin tool is used for manipulating dynamic
  records like Teleport users.

[`tctl`](reference/cli/tctl.mdx) has convenient subcommands for dynamic
...
trust, [`tctl`](reference/cli/tctl.mdx) offers the more powerful, although
...
| - | - | - |
| [`tctl get`](reference/cli/tctl.mdx#tctl-get) | Get one or multiple resources | `tctl get users` or `tctl get user/joe` |
| [`tctl rm`](reference/cli/tctl.mdx#tctl-rm) | Delete a resource by type/name | `tctl rm user/joe` |
| [`tctl create`](reference/cli/tctl.mdx#tctl-create) | Create a new resource from a YAML file. Use `-f` to override / update | `tctl create -f joe.yaml` |
...
user/joe`](reference/cli/tctl.mdx#tctl-get)
...
Here's the list of resources currently exposed via [`tctl`](reference/cli/tctl.mdx) :
...
Finally, create the connector using [`tctl`](reference/cli/tctl.mdx)
...
login`](reference/cli/tsh.mdx) and  [`tsh kube login`](reference/cli/tsh.mdx#tsh-kube-login) command to retrieve credentials for both SSH and
Kubernetes API.
...
- Teleport clients [`tsh`](reference/cli/tsh.mdx) for users and [`tctl`](reference/cli/tctl.mdx) for admins
...
1. Replace the Teleport binaries, usually [`teleport`](reference/cli/teleport.mdx)
   and [`tctl`](reference/cli/tctl.mdx)
...
To diagnose problems you can configure [`teleport`](reference/cli/teleport.mdx) to
...
Sometimes you may want to reset [`teleport`](reference/cli/teleport.mdx) to a clean

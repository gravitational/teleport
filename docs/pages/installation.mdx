...
<Tabs>
<TabItem label="Unified installer (17.3+)">

1. Assign <Var name="example.teleport.sh:443"/> to your Teleport cluster hostname.
   This should contain you cluster hostname and port, but not the scheme (https://).

1. Run your cluster's install script:

   ```code
   $ bash -c "$(curl "https://<Var name="example.teleport.sh:443"/>/scripts/install.sh")"
   ```

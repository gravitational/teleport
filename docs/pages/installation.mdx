...
\[2] *`tsh` is a Command Line Client (CLI) and Teleport Connect is an Graphical User Interface (GUI) desktop client. See
...
\[4] *Teleport server does not run on Windows yet, but `tsh` (the Teleport client)

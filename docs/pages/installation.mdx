...
- Using the example Terraform module for deploying agents in [Deploy Agents with
  Terraform](enroll-resources/agents/deploy-agents-terraform.mdx).
...
by querying your Teleport Enterprise account. This way, the Teleport installation has
...
1. Assign the following environment variables in the terminal where you will run
   ...
   Teleport Enterprise installations must include the automatic agent updater. The
   following commands show you how to determine the Teleport version to install
   by querying your Teleport Enterprise account. This way, the Teleport installation
...
Teleport Enterprise customers must use the [one-line installation
...
| - | - | - | - | - |
| `teleport-oss-(=teleport.version=)-x86_64-$TIMESTAMP` | Teleport Community Edition | amd64 | No | `TeleportVersion: (=teleport.version=)`, `TeleportEdition: oss`, `TeleportFipsEnabled: false` |
| `teleport-oss-(=teleport.version=)-arm64-$TIMESTAMP` | Teleport Community Edition | arm64 | No | `TeleportVersion: (=teleport.version=)`, `TeleportEdition: oss`, `TeleportFipsEnabled: false` |

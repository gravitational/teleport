...
Teleport enhances your session auditing and logging with powerful features including:

1. Video session recording and playback
2. Enhanced audit log session recording 

This document describes the latter feature: Teleport [SSH and Kubernetes enhanced session recording](../architecture/nodes.mdx#session-recording) captures what is `echoed` in a terminal into your text-based audit logging.

## Advantages

Enhanced session recording has many advantages. 

For example, because no input is captured, Teleport session recordings typically do not contain passwords that were entered into a terminal. Logs created through enhanced session recording can typically be shared without fear of revealing passwords or user inputs.

By contrast, Teleport video session recording will record and replay each keystroke for greater visibility. 

The choice of these two session logging features provides flexibility in terms of what logs are shared and the appropriate level of visibility that's need when analyzing logging sources.

## Known limitations

One known limitation is that enhanced session recordings can be bypassed using several techniques:
...
<Admonition type="tip" title="Tip">
  Carefully restricting who has access to what logs helps to mitigate the first concern.
</Admonition>

Another known limitation: due to their unstructured nature, session recordings are sometimes difficult to ingest and perform monitoring/alerting on.

<Admonition type="tip" title="Tip">
  This concern can be mitigated by routing, transforming, or extracting logging data in a uniform and desirable format. For example, Teleport [Events](../admin-guide.mdx#events) can be defined to route and log specific kinds of logging messages.
</Admonition>

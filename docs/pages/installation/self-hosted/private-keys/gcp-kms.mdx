...
To discover available KMS locations, run:
...
<Checkpoint>
Verify the key ring was created:

```code
$ gcloud kms keyrings describe "<Var name="teleport-keyring"/>" --location <Var name="location"/> --format="value(name)"
```

You should see the fully qualified key ring name in the output (e.g., `projects/my-project/locations/us-east1/keyRings/teleport-keyring`).
</Checkpoint>
...
<Checkpoint>
Verify the IAM policy binding was applied:

```code
$ gcloud kms keyrings get-iam-policy <Var name="teleport-keyring"/> \
    --location <Var name="location"/> \
    --format="table(bindings.role, bindings.members)"
```

You should see your custom role bound to the `teleport-auth-server` service account.
</Checkpoint>
...
<Admonition type="tip" title="Using a GCE VM">
If your Teleport Auth Service runs on a GCE VM, you can attach the service account directly to the VM instance instead of using a JSON key file. Replace `INSTANCE_NAME` and `ZONE` with your VM's instance name and zone:
...
GCP requires an instance to be stopped before its service account or scopes
can change — this command fails on a running VM. If you have multiple Auth
Service instances for high availability, apply this one instance at a time
rather than stopping them all at once, so the cluster keeps serving requests
throughout.

This is the recommended approach as it avoids managing key files.
</Admonition>
...
<Checkpoint>
Before proceeding, verify that the service account can access the key ring from the Auth Service host:

```code
$ gcloud kms keys list --location <Var name="location"/> --keyring "<Var name="teleport-keyring"/>"
```

If this command fails with a permission error, review the IAM policy binding in Step 2.
</Checkpoint>

<Admonition type="note">
Use `gcloud kms keys list` rather than `gcloud kms keyrings describe` for this
check. `keyrings describe` requires the `cloudkms.keyRings.get` permission,
which `teleport_kms_role` doesn't grant — the role only covers `cryptoKeys`
and `cryptoKeyVersions` operations, so a `keyrings describe` here would fail
even when the service account has exactly the access it needs.
</Admonition>
...
<Admonition type="note" title="Choosing a protection level">
Supported KMS protection levels are `SOFTWARE` and `HSM`.

- **SOFTWARE**: GCP KMS performs all cryptographic operations in software. This offers a higher quota (60k operations/minute project-wide at time of writing) and is suitable for most deployments.
- **HSM**: GCP KMS performs all cryptographic operations in a Hardware Security Module. This has a lower quota (3k operations/minute for asymmetric keys) but provides hardware-backed key protection.

Both protection levels are considered secure by Google and Teleport. Evaluate your organization's requirements and security policies when making your decision. If your cluster has many thousands of hosts or active users, `SOFTWARE` keys may help avoid throttling, especially during CA rotations.

See the [KMS quotas documentation](https://cloud.google.com/kms/quotas) for current limits.
</Admonition>
...
After starting up your Auth Service with the `gcp_kms` configuration, verify
that Teleport has generated keys in your keyring.
...
You should see multiple keys created by Teleport (one per CA type).

<Checkpoint>
Verify Teleport is using GCP KMS for its CA keys:

```code
$ tctl status
```

The output should show `GCP KMS` as the storage method for all Certificate Authority keys. Also verify you can log in:

```code
$ tsh login --proxy=teleport.example.com
```

If login succeeds, certificates are being signed correctly using GCP KMS keys.
</Checkpoint>

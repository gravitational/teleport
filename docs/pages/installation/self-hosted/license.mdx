...
Self-hosted Teleport Enterprise subscriptions require a valid license. This page
describes the common operations for anaging your Teleport Enterprise license
file.
...
By default, self-hosted Teleport Enterprise clusters will periodically check
whether there have been any changes to their subscription. If a change is
detected, then the cluster will automatically fetch an updated license without
user interaction.

In order for this functionality to work:

- the cluster must be running Teleport 16.4.11 or later
- the cluster must have outbound connectivity to https://api.teleport.sh
- the customer must have license updates enabled on their account

It can take up to 15 minutes for a change on the backend to be detected by the
cluster. There is no way to disable this functionality, but the cluster will
continue to function even if network access to api.teleport.sh is not available
(so long as the license remains valid).

...
[GCP Quickstart](#2-install--configure-teleport):
...
shown in [GCP Quickstart](#2-install--configure-teleport). Since the storage
...
# Proxy instances. These are Google's documented health-checker ranges.
...
$ gcloud compute firewall-rules create teleport-allow-proxy-clients \
    --direction=INGRESS \
    --action=ALLOW \
    --rules=tcp:443,tcp:3023,tcp:3024,tcp:3025,tcp:3026,tcp:3036 \
...
You should see both rules listed with the expected source ranges and ports.

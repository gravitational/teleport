...
<Admonition type="note" title="Why this health check is regional">
The internal load balancer for the Auth Service (below) uses a regional
`INTERNAL`-scheme backend service, which needs a health check in the same
region — a global health check would require an extra
`--global-health-checks` flag on the backend service instead, so creating it
regional here is simpler and matches the region used throughout this guide.
</Admonition>
...
```code
$ gcloud compute health-checks list --regions=us-central1 --format="value(name, type)"
...
Save the following as `teleport-auth-startup.sh`. Note `diag_addr` — this is
...
<Admonition type="warning" title="Don't embed static join tokens in instance metadata">
An earlier version of this script set static `proxy:`/`node:` tokens under
`auth_service.tokens`. Since `--metadata-from-file=startup-script=...`
uploads this script verbatim into the instance template, anyone with read
access to the template or a VM's metadata could recover those non-expiring
tokens and enroll an unauthorized Teleport service — so this version omits
them entirely.

Instead, since the Proxy and Node instances already have dedicated GCP
service accounts, use the **GCP join method** — the same approach covered in
[Join Services with GCP](../agents/gcp.mdx), scoped by service account rather
than a shared secret. We will create a dynamic join token for this on an Auth VM
once the instances are provisioned.

Reference `teleport-proxy-token` (not a static secret) in
`teleport-proxy-startup.sh` below.
</Admonition>

<Admonition type="warning" title="Don't rely on built-in ACME with multiple Proxy replicas">
The Proxy `teleport.yaml` sample in [GCP Quickstart](#2-install--configure-teleport)
enables Teleport's built-in ACME support (`acme.enabled: true`), which is
[only supported for a single Proxy replica](../../../reference/helm-reference/teleport-cluster.mdx).
Behind the passthrough load balancer set up in this guide, the ACME HTTP-01
challenge can land on a different Proxy than the one that requested it, and
certificates aren't shared between replicas — so issuance and renewal become
unreliable with 2+ replicas. Use a single certificate obtained once (for
example via a one-off `certbot` DNS-01 run, or a purchased certificate) for
`teleport.example.com`, store it in Secret Manager, and have every Proxy VM
fetch it at boot instead of requesting its own:

```code
# Run once, from your workstation, after obtaining fullchain.pem and privkey.pem
$ gcloud secrets create teleport-proxy-cert --data-file=fullchain.pem
$ gcloud secrets create teleport-proxy-key --data-file=privkey.pem

$ gcloud secrets add-iam-policy-binding teleport-proxy-cert \
    --member="serviceAccount:$PROXY_SERVICE_ACCOUNT" \
    --role="roles/secretmanager.secretAccessor"
$ gcloud secrets add-iam-policy-binding teleport-proxy-key \
    --member="serviceAccount:$PROXY_SERVICE_ACCOUNT" \
    --role="roles/secretmanager.secretAccessor"
```

In `teleport-proxy-startup.sh`, fetch the secrets before starting Teleport,
and replace `acme` with `https_keypairs` in the Proxy config:

```bash
# Install dependencies and Google Cloud CLI
apt-get update
...
# Retrieve certificates from Secret Manager
mkdir -p /var/lib/teleport
gcloud secrets versions access latest --secret=teleport-proxy-cert > /var/lib/teleport/fullchain.pem
gcloud secrets versions access latest --secret=teleport-proxy-key > /var/lib/teleport/privkey.pem
chmod 600 /var/lib/teleport/privkey.pem
```
...
Renew the certificate (out-of-band, before it expires) by updating the Secret
Manager versions — new Proxy VMs pick up the latest version automatically on
boot, but running instances need a rolling restart to reload it.
</Admonition>
...
`diag_addr: 0.0.0.0:3000` line added, `acme` replaced with `https_keypairs` as
shown above, and `auth_server` set to
...
never creates a DNS record or public endpoint for it). Replace the sample's
`auth_token: (= presets.tokens.first =)` line with:

```yaml
join_params:
  token_name: teleport-proxy-token
  method: gcp
```
...
<Admonition type="note" title="Bootstrap the Proxy join token">
Now that the Auth instances are created, create the dynamic join token directly on one of the new Auth VMs:

```code
$ gcloud compute ssh $(gcloud compute instances list --filter="tags.items=teleport-auth" --format="value(name)" | head -n1) \
    --zone=us-central1-a \
    --command="cat << 'EOF' > /tmp/proxy-token.yaml
kind: token
version: v2
metadata:
  name: teleport-proxy-token
...
spec:
  ...
  gcp:
    allow:
      - project_ids: ['Example_GCP_PROJECT']
        service_accounts: ['teleport-proxy-server@Example_GCP_PROJECT.iam.gserviceaccount.com']
EOF
sudo tctl create -f /tmp/proxy-token.yaml && rm /tmp/proxy-token.yaml"
```
</Admonition>

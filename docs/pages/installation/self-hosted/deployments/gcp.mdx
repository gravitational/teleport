...
than a shared secret. Once at least one Auth instance is up and reachable
(via the internal load balancer set up below), create a dynamic join token
from your workstation:

```code
$ cat << EOF > proxy-token.yaml
kind: token
version: v2
metadata:

...
In addition, you will need `azure-cli` 2.71.0 or later to follow along these
instructions, since it uses the `microsoft-entra-admin` command group and the
`--microsoft-entra-auth` flag introduced in that version. Reference the Azure
docs on [how to install the Azure
...
$ az postgres flexible-server parameter set --resource-group <Var name="teleport-rg" /> --server-name <Var name="teleport-pg" /> \
...
$ az postgres flexible-server microsoft-entra-admin create --resource-group <Var name="teleport-rg" /> --server-name <Var name="teleport-pg" /> \
...
```code
$ az storage account create --resource-group <Var name="teleport-rg" /> --name <Var name="teleportblob" /> \
...
Access](../../../zero-trust-access/authentication/authentication.mdx) section to get started enrolling

...
```code
$ gcloud iam roles create storage_bucket_creator_role \
    --project=$(gcloud config get-value project) \
    --title="Storage Bucket Creator" \
    --description="Allows creation of storage buckets" \
...
```code
$ gcloud iam roles create dns_updater_role \
    --project=$(gcloud config get-value project) \
    --title="DNS Updater" \
    --description="Allows updates to Cloud DNS records" \
    --permissions=dns.resourceRecordSets.create,dns.resourceRecordSets.delete,dns.resourceRecordSets.list,dns.resourceRecordSets.update,dns.changes.create,dns.changes.get,dns.changes.list,dns.managedZones.list \
    --stage=GA
```
...
$ gcloud iam service-accounts create teleport-helm \
    --description="Service account for Teleport Helm chart" \
    --display-name="Teleport Helm"

$ export SA_EMAIL=teleport-helm@${GCP_PROJECT}.iam.gserviceaccount.com

# Bind the required roles to the service account
$ gcloud projects add-iam-policy-binding $GCP_PROJECT \
    --member="serviceAccount:${SA_EMAIL}" \
    --role="projects/${GCP_PROJECT}/roles/storage_bucket_creator_role"

$ gcloud projects add-iam-policy-binding $GCP_PROJECT \
    --member="serviceAccount:${SA_EMAIL}" \
    --role="projects/${GCP_PROJECT}/roles/dns_updater_role"
...
The JSON private key will be saved to `teleport-gcp-credentials.json` in your current directory.
...
```code
$ kubectl --namespace teleport get secret teleport-gcp-credentials -o jsonpath='{.metadata.name}'
```

You should see `teleport-gcp-credentials` in the output.
</Checkpoint>

<Admonition type="warning" title="Delete the local key file once the Secret exists">
The JSON key now lives inside your cluster as a Secret, and Kubernetes is
where Teleport reads it from — the copy on your workstation (or wherever it
was downloaded) is no longer needed. This key belongs to a service account
holding `storage-bucket-creator-role`, `dns-updater-role`,
`roles/datastore.owner`, and `roles/storage.objectAdmin`, so a leftover copy
sitting in a home directory, backup, or accidentally committed to a repo
gives an attacker write/delete access to your cluster state, session
recordings, and DNS. Once the Checkpoint above confirms the Secret exists,
delete the local file:

```code
$ rm teleport-gcp-credentials.json
```

(or the equivalent path if you used the Console download instead).
</Admonition>
...
All cert-manager pods should show `Running` status with `1/1` ready.
</Checkpoint>
...
<Admonition type="note">
  ...
  You can enable the API and create the database from the CLI:

  ```code
  $ gcloud services enable firestore.googleapis.com --project=$(gcloud config get-value project)
  $ gcloud firestore databases create --location=us-central1 --type=firestore-native --project=$(gcloud config get-value project)
  ```
</Admonition>
...
<Checkpoint>
Verify the Helm installation succeeded and pods are running:

```code
$ kubectl --namespace teleport get pods
```

You should see auth and proxy pods in `Running` state. If pods are in `CrashLoopBackOff` or `Error` state, check logs:

```code
$ kubectl --namespace teleport logs deploy/teleport-auth --tail=50
```
</Checkpoint>
...
<Tabs>
<TabItem label="gcloud CLI (transaction)">
...
<TabItem label="gcloud CLI (direct)">
...
# Create the A record for the cluster domain
$ gcloud dns record-sets create "${MYDNS}." \
    --zone="${MYZONE}" \
    --type="A" \
    --ttl="300" \
    --rrdatas="${MYIP}"

# Create the wildcard A record for application access
$ gcloud dns record-sets create "*.${MYDNS}." \
    --zone="${MYZONE}" \
    --type="A" \
    --ttl="300" \
...
<Checkpoint>
Verify DNS records were created and resolve correctly:

```code
$ gcloud dns record-sets list --zone="${MYZONE}" --filter="name=${MYDNS}."
```

You should see an A record pointing to your load balancer IP. You can also verify DNS resolution:

```code
$ dig +short ${MYDNS}
```

This should return the external IP of your load balancer.
</Checkpoint>
...
```code
$ kubectl --namespace teleport exec deploy/teleport-auth -- tctl status
```

You should see the cluster name, version, and CA pin displayed without errors.
</Checkpoint>

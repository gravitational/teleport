...
<Checkpoint
  title="Verify image pull"
  description="Confirm that you can pull the Teleport Docker image."
>

Run the following command to pull the image:
```code
$ docker pull <Var name="(=teleport.latest_oss_docker_image=)" />
```

You should see output indicating the image layers are being downloaded, ending with a success message.
...
<Checkpoint
  title="Verify Kubernetes pod status"
  description="Confirm that Teleport pods are running correctly in your Kubernetes cluster."
>
...
<Checkpoint
  title="Verify Teleport is responding in Kubernetes"
  description="Confirm that the Teleport service is healthy and accepting connections."
>
...
Here are some troubleshooting tips:
- If `tctl status` fails with connection errors, verify the Auth Service is reachable and the pod has the correct configuration mounted.
...
- Verify the mounted configuration with `kubectl exec -i <pod-name> -- cat /etc/teleport/teleport.yaml`.

</Checkpoint>

<Checkpoint
  title="Verify Kubernetes deployment and upgrades"
  description="Confirm that Teleport deployments and upgrades complete successfully."
>
...
<Checkpoint
  ...
  description="Confirm that the Teleport configuration file was generated successfully."
>
...
<Checkpoint
  ...
  description="Confirm that the Teleport container started successfully."
>
...
<Checkpoint
  title="Verify Teleport is responding"
  description="Confirm that the Teleport Proxy Service is accepting connections and the cluster is healthy."
>

Run the following command to verify the web API is responding:
```code
$ curl --insecure https://localhost:3080/webapi/ping
```
...
<Checkpoint
  ...
  description="Confirm that Teleport was upgraded successfully and the cluster is healthy."
>

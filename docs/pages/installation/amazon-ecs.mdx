...
## How it works
Using Terraform, a Teleport Agent is deployed and kept up to date in Amazon ECS.
You have to create or re-use an IAM Join Token so the Agent is able to join the cluster.

You will need to configure which Teleport services you want to run (e.g., discovery, database and kube services), and then pick the subnets and security groups which allow access to the resources you want to access.

After deploying, the Agent is part of your cluster and starts discovering or proxying requests.

You can customize the Agent configuration and re-deploying it as needed.

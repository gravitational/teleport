...
We provide a slimmed-down distroless image that contains only the `tbot` binary for use with Teleport Machine & Workload Identity.
...
If the upgrade fails or the container doesn't start, see [Container or pod fails to start after upgrade](#container-or-pod-fails-to-start-after-upgrade) below.

## Troubleshooting

The sections below cover common issues when running Teleport on Docker or Kubernetes. Each section names a specific symptom so you can scan for what you are seeing.
...
If the container exits immediately with `failed to read configuration` or `no such file or directory` for `/etc/teleport.yaml`, the configuration file is not mounted into the container at the expected path. Verify that the configuration file exists on the host at the path you passed to `-v`, and that the mount target inside the container is `/etc/teleport` (the directory containing `teleport.yaml`), not `/etc/teleport.yaml` (the file itself). Mounting a single file rather than its parent directory can cause issues when the file is replaced or symlinked.

### Permission denied writing to the data directory

If the container fails to start with `permission denied` errors against `/var/lib/teleport`, the container's user does not have write access to the mounted data directory. Distroless images run as a non-root user, so host directory permissions need to allow that user to write.

For Docker, adjust permissions on the host directory with `chmod` or `chown` so the container's user can write to it. For Kubernetes, set `runAsUser` and `fsGroup` in the pod's `securityContext` so the volume is writable by the Teleport process.
...
### Container or pod fails to start after upgrade

If the container or pod fails to start after upgrading to a newer Teleport version, check the logs (`docker logs teleport` or `kubectl logs <Var name="pod-name"/>`) for migration or compatibility errors. Confirm that you did not skip a major version: Teleport supports upgrades from one major version to the next, and skipping versions is not supported.

To roll back, stop the new container or revert the deployment, then start a container using the previous image version with the same data directory.

### Pod in CrashLoopBackOff

If the Teleport pod enters `CrashLoopBackOff`, the container is starting and exiting repeatedly. View logs from the previous crash with `kubectl logs <Var name="pod-name"/> --previous`, and check `kubectl describe pod <Var name="pod-name"/>` for events and the `lastState` reason. Common causes include configuration errors, missing license files for Enterprise deployments, and permission issues on the data directory volume.

### Pod in ImagePullBackOff

If the pod enters `ImagePullBackOff`, the cluster cannot pull the Teleport image. Verify the image name and tag are correct, that your cluster has network access to `public.ecr.aws`, and that any required pull secrets are configured on the namespace or service account.

...
- If you see `Error: No such container`, verify the container is running with `docker ps` and that the container name or ID is correct.
...
- If the pod is stuck in `Pending`, check for resource constraints or scheduling issues in the Events section of `kubectl describe pod <Var name="pod-name"/>`.
- If the pod is in `CrashLoopBackOff`, view logs from the previous crash with `kubectl logs <Var name="pod-name"/> --previous`.
...
- If the pod shows a high restart count, check for OOM kills or liveness probe failures with `kubectl describe pod <Var name="pod-name"/>` and review the `lastState` section.
...
- View pod logs for startup errors with `kubectl logs -f <Var name="pod-name"/>` and recent events with `kubectl get events --field-selector involvedObject.name=<Var name="pod-name"/>`.
...
- If pods fail after an upgrade, check for migration errors in logs with `kubectl logs <Var name="pod-name"/>`. Teleport supports upgrades from one major version to the next; do not skip major versions.
...
$ kubectl describe pod <Var name="pod-name"/>
```
...
```code
$ kubectl port-forward <Var name="pod-name"/> 3080:3080
...
```code
$ kubectl get pod <Var name="pod-name"/> -o jsonpath='{.spec.containers[0].image}'
```

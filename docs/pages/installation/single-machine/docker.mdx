...
This guide shows you how to install and run Teleport using the official Docker images, either directly with Docker or in Kubernetes pods. It covers selecting an image, configuring the container, running Teleport services, and upgrading.
...
The sections below cover common issues specific to running Teleport in a Kubernetes pod.

### Pod in CrashLoopBackOff

If the Teleport pod enters `CrashLoopBackOff`, the container is starting and exiting repeatedly. View logs from the previous crash with `kubectl logs <Var name="pod-name"/> --previous`, and check `kubectl describe pod <Var name="pod-name"/>` for events and the `lastState` reason.

Common causes include configuration errors, missing license files for Enterprise deployments, permission issues on the data directory volume (set `runAsUser` and `fsGroup` in the pod's `securityContext` so the volume is writable by the Teleport process), and migration errors after skipping a major version. Teleport supports upgrades from one major version to the next; skipping versions is not supported.

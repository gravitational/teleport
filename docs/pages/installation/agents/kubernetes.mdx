...
In this guide, you will use the **Kubernetes join method** to configure Teleport
services to join your Teleport cluster without sharing any secrets when running
in the same Kubernetes cluster as the Auth Service.
...
- A running Teleport cluster in Kubernetes. For details on how to set this up,
  see [Guides for running Teleport using Helm](../self-hosted/helm-deployments/helm-deployments.mdx).
...
## Step 1/4. Create a Kubernetes join token
...
## Step 2/4. Deploy a demonstration HTTP app
...
## Step 3/4. Configure and start the Application Service
...
token mounted. The `teleport-kube-agent` chart that you will install in this
section will take care of this by default.
...
1. Deploy the Teleport Application Service by running the following command:

   ```code
   $ helm install teleport-app-service teleport/teleport-kube-agent -n teleport-agent --create-namespace -f values.yaml
   ```
   
1. Validate the pod is running after a couple of seconds:
...
1. Validate you can see the application in the Teleport Web UI, or using the
   command line:

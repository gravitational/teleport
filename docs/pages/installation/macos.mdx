...
- If `teleport` is not found, ensure `/usr/local/bin` is in your `PATH` environment variable.
- If you see a permissions error during installation, verify you're using `sudo` with the installer command.
- If macOS blocks the installer, go to **System Settings** > **Privacy & Security** and click **Open Anyway** for the Teleport package.
- For Apple Silicon Macs, ensure you downloaded the correct architecture-specific installer if one is provided.

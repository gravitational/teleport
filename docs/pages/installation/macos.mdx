---
title: Installing Teleport on macOS
description: How to install Teleport on macOS using our official PKG installers for both Intel and Apple Silicon.
tags:
- reference
- platform-wide
---

This guide explains how to install Teleport on macOS.

For an overview of Teleport installation methods and supported platforms, see
[Installation](./installation.mdx).

## Operating system support

| Operating System | `teleport` Daemon | `tctl` Admin Tool | `tsh` and Teleport Connect User Clients [1] | Web UI (via the browser) | `tbot` Daemon |
| - | - | - | - | - | - |
| macOS 12+ (Monterey) | yes | yes | yes | yes | yes |

\[1] `tsh` is a Command Line Client (CLI) and Teleport Connect is a Graphical User Interface (GUI) desktop client. See
[Using Teleport Connect](../connect-your-client/teleport-clients/teleport-connect.mdx) for usage and installation.

(!docs/pages/includes/client-cluster-compatibility.mdx!)

## Installing Teleport

<Tabs>
<TabItem label="Teleport Enterprise (Managed)">

(!docs/pages/includes/cloud/install-macos.mdx!)

</TabItem>
<TabItem label="Teleport Enterprise (Self-Hosted)">

(!docs/pages/includes/enterprise/install-macos.mdx!)

</TabItem>
<TabItem label="Teleport Community Edition">

Download the Teleport installer for macOS:

[`teleport-(=teleport.version=).pkg`](https://cdn.teleport.dev/teleport-(=teleport.version=).pkg)

This universal installer works on both Intel and Apple Silicon Macs and includes the following binaries: `teleport`, `tctl`, `tsh`, `tbot`, and `fdpass-teleport`.

You can also fetch and run the installer via the command line:
```code
$ curl -O https://cdn.teleport.dev/teleport-(=teleport.version=).pkg
# Installs on Macintosh HD
$ sudo installer -pkg teleport-(=teleport.version=).pkg -target /
# Password:
# installer: Package name is teleport-(=teleport.version=)
# installer: Upgrading at base path /
# installer: The upgrade was successful.
$ which teleport
# /usr/local/bin/teleport
```

<Admonition type="danger">

We do not recommend using Homebrew to install Teleport. The Teleport package in
Homebrew is not maintained by Teleport and we can't guarantee its reliability or
security.

</Admonition>

</TabItem>
</Tabs>

<Checkpoint
  title="Verify Teleport installation"
  description="Confirm that Teleport binaries are installed and accessible from your terminal."
>

Run the following command to verify your installation:
```code
$ teleport version
```

You should see output displaying the Teleport version number.

Here are some troubleshooting tips:
- If `teleport` is not found, ensure `/Applications/Teleport.app/Contents/MacOS/` is in your `PATH` environment variable, or run the binary directly from that location.
- If you see `installer: must be run as root` when running the installer command, add `sudo` before the `installer` command.
- If macOS blocks the installer, go to **System Settings** > **Privacy & Security** and click **Open Anyway** for the Teleport package.

</Checkpoint>

...
Here are some troubleshooting tips:
- If `tsh` is not found, ensure `/Applications/Teleport.app/Contents/MacOS/` is in your `PATH`, or run the binary directly from that location.
...
For more information on using `tsh`, see the [tsh reference](../connect-your-client/tsh.mdx).
...
If you need to run Teleport services on macOS (such as for local development or running an agent), the PKG installer also includes the `teleport` daemon and `tbot`.
...
Running Teleport services in production is typically done on Linux servers. macOS is primarily used for client access and local development. For production deployments, see [Installing Teleport on Linux](./linux.mdx).
...
- [Machine ID Getting Started](../enroll-resources/machine-id/getting-started.mdx) for workload identity

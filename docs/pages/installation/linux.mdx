...
1. Follow the instructions for your package manager:
   ...
   # Fedora 41 or later
   $ sudo dnf config-manager addrepo --overwrite --from-repofile="$(rpm --eval "https://yum.releases.teleport.dev/$ID/$VERSION_ID/Teleport/%{_arch}/stable/v(=teleport.major_version=)/teleport.repo")"

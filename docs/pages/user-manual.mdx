...
<Admonition
  ...
  title="Tip"
>
  It is recommended to always use [`tsh login`](reference/cli/tsh.mdx#tsh-login) before using any other `tsh` commands. This allows users to omit `--proxy` flag in subsequent tsh commands. For example
...
He executes the [`tsh login`](reference/cli/tsh.mdx#tsh-login) command:

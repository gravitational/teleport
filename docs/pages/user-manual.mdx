...
[the admin manual](admin-guide.mdx#trusted-clusters) to learn how a trusted cluster
can be configured.
...
- `tsh ssh` operates *two* usernames: one for the cluster and another for the
...
He executes [`tsh login`](cli-docs.mdx#tsh-login) command:
...
Teleport](enterprise/index.mdx), please reach out to us: `sales@gravitational.com`.

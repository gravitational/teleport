...
dynamic `db` resources. All replicas will detect `db` resources in your cluster
and proxy connections to the database instances configured via these resources.

To configure the Database Service to watch for dynamic resources, add a `labels`
field to its `resources` configuration, for example:

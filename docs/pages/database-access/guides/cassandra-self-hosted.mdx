...
When connecting to  Cassandra, sign the certificate for the hostname over
which Teleport will be connecting to it.

<Tabs>
  <TabItem label="Cassandra">
    ```code
    $ auth sign --format=cassandra --host=cassandra.example.com --out=server --ttl=2190h
    ```
...
For example, if your Cassandra server is accessible at `cassandra.example.com`,
run:
...
<Tabs>
  <TabItem label="Cassandra">
    ...
    ```
    Where the `password`is the value generated in previous 4/5. step by `tctl auth sign` command.

  </TabItem>
  <TabItem label="Scylla">
    To configure Cassandra to accept TLS connections, add the following to your
    Cassandra configuration file, `scylla.yaml`:
    ```conf
    client_encryption_options:
      enabled: true

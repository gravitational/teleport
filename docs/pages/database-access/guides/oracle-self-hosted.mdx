...
## Step 5/6. Configure Teleport Oracle Audit Logs Puller (Optional).
...
Create a internal Oracle `teleport` user that will be used as db user that will be used to fetch
...
Enable table Oracle Audit Trail:
```
ALTER system SET audit_trail=db,extended scope=spfile;
```

<Admonition type="info" title="audit_trait">
In order to propagate audit_trail changes, Oracle instance needs to be restarted.
</Admonition>

Enable Audit Oracle activity for the `alice` user:
```sql
AUDIT ALL BY alice BY ACCESS;
AUDIT SELECT TABLE, UPDATE TABLE, INSERT TABLE, DELETE TABLE BY alice BY ACCESS;
```

<Admonition type="info" title="Audit Oracle user activity ">
Enabling Audit needs to be done for each user Teleport user that will be used to connect to Oracle.
Additionally you create different audit policy for each user.
</Admonition>

Configure Teleport DB Service to pull audit logs from Oracle Audit Trail:
...
<Admonition type="warning" title="Audit Trail Cleanup">
Teleport doesn't clean up audit trail events from Oracle Audit Trail.
Make sure to configure Oracle Audit Trail cleanup policy to avoid running out of disk space.
</Admonition>

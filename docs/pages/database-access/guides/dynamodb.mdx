...
<Admonition type="tip" title="non-standard AWS partitions">
Replace `https://console.aws.amazon.com` with
`https://console.amazonaws-us-gov.com` for AWS GovCloud (US) or replace with
`https://console.amazonaws.cn` for AWS China.
</Admonition>
...
<Admonition type="warning" title="non-standard AWS partitions">
For non-standard AWS partitions such as AWS GovCloud (US) and AWS China, please
set the corresponding region in the `AWS_REGION` environment variable or in the
AWS credentials file so that the Application Service can use the correct STS
endpoint.
</Admonition>

...
Access to AWS DynamoDB is provided by [**Teleport Application
Access**](../../application-access/introduction.mdx) for AWS Console and API.
...
- `aws` (AWS Command Line Interface) installed and added to your system's
...
The first step is to create an IAM role with desired IAM permissions that you
wish to grant to a Teleport user. The Teleport Application Service will assume
this role on the Teleport user's behave to access AWS resources. It is
recommended to limit the IAM permissions to the minimum of what the Teleport
user needs.

For access to DynamoDB, AWS provides IAM polices like
`AmazonDynamoDBReadOnlyAccess` and `AmazonDynamoDBFullAccess` that can be
incorporated in to roles. This example will create an IAM role with the
`AmazonDynamoDBFullAccess` policy. Nonetheless, you can also use other
AWS-managed polices or your own custom polices depending on your circumstances.

**Skip this step** if you already have the AWS roles you want to provide access
to.

Now visit the [Roles page](https://console.aws.amazon.com/iamv2/home#/roles) of
...
listing the IAM role ARN created in the previous step:
...
Now assign this role to the Teleport users you wish to grant access to
DynamoDB.

...
- Set up Teleport to access the AWS Console and API.
- Connect to your DynamoDB databases through Teleport.
...
wish to grant to a Teleport user. AWS provides IAM polices like
`AmazonDynamoDBReadOnlyAccess` and `AmazonDynamoDBFullAccess` that can be
incorporated in to roles. In this example, you will create an IAM role with
`AmazonDynamoDBFullAccess`. **Skip this step** if you already have the roles
you want to provide access to.

<Admonition type="note">
These policies may provide too much or not enough access for your intentions.
Validate these meet your expectations if you plan on using them.
</Admonition>
...
<Tabs>
  ...
  to environment variables for AWS SDK for Python:
  ```code

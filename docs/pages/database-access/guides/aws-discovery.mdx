...
Discovery Service can run independently on a different machine from Database
Services.

For example, you can run an instance of the Discovery Service for discovering
databases across all your AWS accounts, and an instance of the Database Service
in each VPC for accessing database servers in private subnets.

The same Discovery Service can also be used for Azure database discovery,
server instance discovery and Kubernetes discovery.
...
The Discovery Service require a valid auth token to connect to the cluster.

Generate an auth token by running the following command against your
Teleport Auth Service and save it in `/tmp/token` on the host that will
run the Discovery Service.

```code
$ tctl tokens add --type=discovery,db
```
...
Teleport can boostrap IAM permissions for the Discovery and Database Service
based on the `discovery_service` configuration using the `teleport discovery
bootstrap` command. You can use this command in automatic or manual mode:
...
## Next
- Learn [Dynamic Registration](./dynamic-registration.mdx) by Database Service.

...
configuration and register them in the cluster. It will also attempt to enable
IAM auth and configure IAM access policies for the discovered databases. Keep in
mind that AWS IAM changes may not propagate immediately and can take a few
minutes to come into effect.
...
<Tabs>
  ...
  MySQL/MariaDB users must have RDS authentication plugin enabled:

...
name="mytenant.teleport.sh:443" /> to the host **and port** of your Teleport
Proxy Service or Enterprise Cloud/Team tenant. Assign <Var name="us-west-1" />
to the region associated with the RDS databases you would like Teleport to
discover.
...
Retrieve your OIDCS issuer ID, assigning <Var name="cluster-name" /> to the name
...
```code
$ tsh login --proxy=<Var name="mytenant.teleport.sh:443" /> --user=alice
$ tsh db ls

...
If you plan to run the Teleport Database Service on Kubenetes, you will need the
following:
 ...
 - The [`jq` CLI](https://jqlang.github.io/jq/), which we use to process JSON
   data in this guide.
...
name="teleport.example.com:443" /> to the host and port of your Teleport Proxy
Service or Enterprise Cloud/Team tenant. Assign <Var name="us-west-1" /> to the
region associated with the RDS databases you would like Teleport to discover.
...
number, and <Var name="resource-id" /> to the resource ID of your RDS or Aurora
database:
...
Retrieve your OIDCS issuer, assigning <Var name="cluster-name" /> to the name of
your EKS cluster and <Var name="eks-region" /> to the AWS region where your EKS
cluster is running:

```code
$ aws eks describe-cluster --name <Var name="cluster-name" /> --region <Var name="eks-region" /> | jq .cluster.identity.oidc.issuer
...
Create an IAM pole with your trust policy. On success, the command will show the
...
name="mytenant.teleport.sh:443" /> to the host and port of your Teleport Proxy
Service, and <Var name="rds-uri" /> to the host and port of your RDS database
(e.g., `myrds.us-east-1.rds.amazonaws.com:5432`):

```code
cat > values.yaml << EOF
authToken: <Var name="token" />
proxyAddr: <Var name="mytenant.teleport.sh:443" />
roles: db
databases:
- name: example
...
annotations:
  serviceAccount:
    ...
    arn:aws:iam::<Var name="aws-account" />:role/teleport-rds-role
EOF
```

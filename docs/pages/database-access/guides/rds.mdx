...
name="RDS_CLUSTER_ID" /> to the ID of your cluster, <Var name="RDS_URI" /> to
your cluster's domain name and port, and <Var name="protocol" /> to either
`postgres` or `mysql`, depending on the database you want to proxy:
...
$ sudo teleport db configure create \
   ...
   --token=/tmp/token \
   --aws-rds-cluster-id=<Var name="RDS_CLUSTER_ID" />
```

Run the following command to proxy an AWS RDS instance, assigning <Var
name="RDS_CLUSTER_ID" /> to the ID of your instance, <Var name="RDS_URI" /> to
your instance's domain name and port, and <Var name="protocol" /> to either
`postgres` or `mysql`, depending on the database you want to proxy:
...
$ sudo teleport db configure create \
   ...
   --uri=<Var name="RDS_URI" /> \
   --token=/tmp/token \
   --aws-rds-instance-id=<Var name="RDS_INSTANCE_ID" />
```

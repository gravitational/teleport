...
- Set up Teleport to access your RDS instances, Aurora clusters, and RDS proxies.
...
<Tabs>
  ...
  database agent runs as IAM user (for example, it uses an AWS credentials
  file).
  ```json
  {
      "Version": "2012-10-17",
  ...
  <TabItem label="RDS proxy / IAM user">
  Use this policy if you're connecting to RDS proxies and your Teleport
  database agent runs as IAM user (for example, it uses an AWS credentials
  file).
  ```json
  {
  ...
  <TabItem label="RDS proxy / IAM role">
  Use this policy if you're connecting to RDS proxies and your Teleport
  database agent runs as IAM role (for example, on an EC2 instance with
  attached IAM role).
  ```json
...
Since Teleport will be managing its own IAM policies for access to RDS
databases and proxies, you need to create a permission boundary to limit its
...
<Tabs>
  ...
  your Teleport database agent runs as IAM user (for example, it uses an AWS
  credentials file).
  ```json
  {
      "Version": "2012-10-17",
  ...
  and your Teleport database agent runs as IAM user (for example, it uses an
  ...
  <TabItem label="RDS proxy / IAM user">
  Use this permission boundary policy if you're connecting to RDS proxies and
  your Teleport database agent runs as IAM user (for example, it uses an AWS
  credentials file).
  ```json
  {
  ...
  <TabItem label="RDS proxy / IAM role">
  Use this permission boundary policy if you're connecting to RDS proxies and
  your Teleport database agent runs as IAM role (for example, on an EC2
  instance with attached IAM role).
  ```json
  {
      "Version": "2012-10-17",
...
db_service:
  enabled: "yes"
  aws:
  - types: ["rds", "rds-proxy"] # Database types
...
When `db_service.aws[*].types` includes `rds`, the agent will discover all RDS
instances and Aurora clusters according to the configuration and register them
in the cluster. In addition to the primary endpoints of the discovered Aurora
clusters, the clusters' reader and custom endpoints will also be registered.

When `db_service.aws[*].types` includes `rds-proxy`, the agent will discover
and register all RDS proxies and proxy endpoints
...
<Admonition type="note" title="RDS proxies">
    Database IAM users are not required for RDS proxies as RDS proxies
    authenticate with regular user names and passwords. See [Connecting to a
    proxy using IAM
    authentication](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/rds-proxy-setup.html#rds-proxy-connecting-iam)
    for more information.
</Admonition>
...
# rds-proxy-read-endpoint        RDS proxy endpoint in us-west-1               ...
```

Each registered database represents one endpoint of the RDS service. The
database names have the following formats:

| Service Type | Endpoint Type | Name Format |
| ------------ | ------------- | ----------- |
| RDS instances | per instance endpoint | `<instance-id>` |
| Aurora clusters | primary endpoint | `<cluster-id>` |
| Aurora clusters | reader endpoint | `<cluster-id>-reader` |
| Aurora clusters | custom endpoint | `<cluster-id>-custom-<custom-endpoint-name>` |
| RDS proxies | default endpoint | `<proxy-name>` |
| RDS proxies | custom endpoints | `<proxy-name>-<proxy-endpoint-name>` |

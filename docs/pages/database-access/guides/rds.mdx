...
configuration and register them in the cluster, when database type `rds` is
configured. In addition to the primary endpoints of the discovered Aurora
clusters, their reader and custom endpoints will also be registered.

The agent will discover and register all RDS proxies and proxy endpoints when
database type `rds-proxy` is configured.
...
<Admonition type="note" title="RDS proxies">
    Database IAM users are not required for RDS proxies as they authenticate
    with regular user names and passwords. See [Connecting to a proxy using IAM
...
<Admonition type="note" title="Note">
  ...
  RDS proxies and proxy endpoints have names of `<proxy-name>` and
  `<proxy-name>-<proxy-endpoint-name>` respectively.
</Admonition>

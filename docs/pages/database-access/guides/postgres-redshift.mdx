...
Assign `CLUSTER_ID` to the ID of your Amazon Redshift cluster and `CLUSTER_URI`
to the domain name and port of the cluster:
...
$ sudo teleport db configure create \
   ...
   --protocol=postgres \
   --token=/tmp/token \
   --aws-redshift-cluster-id=${CLUSTER_ID?} \
   --uri=${CLUSTER_URI?}
```

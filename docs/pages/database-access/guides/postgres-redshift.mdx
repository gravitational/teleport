...
There are two methods of IAM authentication supported by Redshift. Teleport
requires different IAM permissions depending on which IAM authentication method
will be used:

<Tabs>
<TabItem label="IAM authentication as database users (recommended)">
Teleport Database Service can authenticate with Redshift by generating IAM
...
Teleport Database Service can authenticate with Redshift by generating IAM
authentication tokens as IAM roles, and Redshift maps these IAM roles to
database users automatically.

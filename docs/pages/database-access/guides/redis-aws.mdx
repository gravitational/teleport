...
managed by Teleport. You can configure some ElastiCache users for Teleport
access while reserving other users for other purposes like programming access.
</Admonition>
...
<Tabs>
  ...
  <TabItem label="Non-Teleport-managed user">
    The connection normally starts as the `default` user. Now you can
    authenticate the database user with its password:

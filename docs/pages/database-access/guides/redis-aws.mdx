...
<Tabs>
  ...
  Assign `ELASTICACHE_ID` to the the identifier of your ElastiCache replication
  group, and change `example.teleport.sh:443` to the host and port of your
  Teleport Proxy Service. Set `ELASTICACHE_URI` to the domain name and port of
  your ElastiCache database:
  ...
  $ sudo teleport db configure create \
     ...
     --uri=${ELASTICACHE_URI?} \
     --token=/tmp/token \
     --aws-elasticache-group-id=${ELASTICACHE_ID?}
  ```
  </TabItem>
  <TabItem label="MemoryDB">

  Assign `MEMORYDB_CLUSTER` to the name of your MemoryDB cluster, and change
  `example.teleport.sh:443` to the host and port of your Teleport Proxy Service.
  Set `MEMORYDB_URI` to the domain name and port of your ElastiCache
  database:
  ...
  $ sudo teleport db configure create \
     ...
     --uri=${MEMORYDB_URI} \
     --token=/tmp/token \
     --aws-memorydb-cluster-name=${MEMORYDB_CLUSTER}
  ```
  </TabItem>
</Tabs>

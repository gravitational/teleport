---
title: Database Access with AWS ElatiCache for Redis
description: How to configure Teleport Database Access with AWS ElatiCache for Redis.
---

This guide will help you to:

- Install Teleport `(=teleport.version=)`.
- Set up Teleport to access your ElastiCache clusters for Redis.
...
- AWS account with ElastiCache Redis clusters **with in-transit encryption
  (TLS) enabled**.
...
`/etc/teleport.yaml` location. See [Configuration
...
- Discover and register ElastiCache Redis clusters.
- Modify ElastiCache user passwords for Teleport managed users and
...
The Database Service will discover and register all ElastiCache Redis clusters
...
<Admonition type="tip">
It is not required for all users in the same ElastiCache user group to be
managed by Teleport. Just the tag users that are desired to be used for
Teleport database access. You can still have other ElastiCache users for other
usages like programming access.
</Admonition>

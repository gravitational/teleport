...
If you want to configure Snowflake Cluster, please read [Database Access with Snowflake Cluster](Snowflake-cluster.mdx).

This guide will help you to:

- Install and configure Teleport.
- Add Teleport's public key to a Snowflake's user.
- Connect to Snowflake through Teleport.

<ScopedBlock scope={["oss", "enterprise"]}>
  <h1>TODO Add Snowflake architecture diagram.</h1>
</ScopedBlock>
...
## Step 3/5. Export public key

Use the `tctl auth sign` command below to export the public key.

```code
$ tctl auth sign --format=snowflake --out=server
```

The command will create `server.pub` file with Teleport's public key.

(!docs/pages/includes/database-access/rotation-note.mdx!)

## Step 4/5. Add generated key to Snowflake's user

Use the generated public key to enable Key Pair Authentication.

Login to your Snowflake instance and execute the below SQL statement:
...
where `alice` is the name of the Snowflake user and the `rsa_public_key` is the key generated earlier without
the PEM type (first and the last line).

You can use `describe user` command to verify the user's public key.

```sql
desc user alice;
```

See [Snowflake documentation](https://docs.snowflake.com/en/user-guide/key-pair-auth.html#step-4-assign-the-public-key-to-a-snowflake-user)
for more details.

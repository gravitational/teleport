...
- Assign Teleport's public key to a Snowflake's user.
...
The command will create `server.pub` file with Teleport's public key. Teleport will use the corresponding private key to
generate JWT (JSON Web Token) which will be used to authenticate in Snowflake.

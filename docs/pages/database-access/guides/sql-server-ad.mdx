...
<Admonition type="note">
  ...
  `mssql-cli` is not required for SQL Server connections.  Use `tsh proxy db --db-user=teleport --tunnel sqlserver`
   to connect from other DB Clients such as Microsoft SQL Server Management Studio. See the [Database Access GUI Clients
   for SQL Server](../../connect-your-client/gui-clients.mdx#sql-server-azure-data-studio) for more information.
</Admonition>

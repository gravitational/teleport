---
title: Integration with self-hosted ClickHouse Database
...
<Admonition type="warning" title="Preview">
  ...
  Teleport Database audit logs are only supported for ClickHouse HTTP protocol
  and Teleport Database Access with ClickHouse Native protocol does not include audit logs of database query activity.
</Admonition>
...
- For ClickHouse HTTP Protocol integration self-hosted ClickHouse server v22.3 or later.
- For ClickHouse Native Protocol integration self-hosted ClickHouse Server v23.3 or later.
...
## Step 3/5. Configure ClickHouse Server
...
(!docs/pages/includes/database-access/db-configure-start.mdx dbProtocol="clickhouse" dbName="clickhouse-http" databaseAddress="clickhouse.example.com:8443" !)
  </TabItem>
  <TabItem label="ClickHouse Native protocol">

Configure and start the Teleport Service:

(!docs/pages/includes/database-access/db-configure-start.mdx dbProtocol="clickhouse" dbName="clickhouse" databaseAddress="clickhouse.example.com:9440" !)
  </TabItem>
</Tabs>
...
(!docs/pages/includes/database-access/db-helm-install.mdx dbName="clickhouse-http" dbProtocol="clickhouse-http" databaseAddress="clickhouse.example.com:8443" dbName="clickhouse-http" !)

  </TabItem>
  <TabItem label="clickHouse Native Protocol">

(!docs/pages/includes/database-access/db-helm-install.mdx dbName="clickhouse" dbProtocol="clickhouse" databaseAddress="clickhouse.example.com:9440" dbName="clickhouse" !)
...
# ------------------------------- ----------- ------------- ------- ------------------------------
# clickhouse-http (user: alice)             [*]           env=dev tsh db connect clickhouse-http
```
...
Start Teleport Local Proxy:

```code
$ tsh proxy db --tunnel clickhouse-http
...
To test connection you can run the following command:
...
```code
$ tsh db connect --db-user=alice clickhouse

...
Teleport Clickhouse integration leverages  ClickHouse x509 authentication that is only support
with  HTTPS or Native interfaces only.
The HTTP interface lets you use ClickHouse on any platform from any programming language in a form of REST API.
The HTTP interface is more limited than the Native interface, but it has better language support.
Teleport ClickHouse integration allow you to select between Native or HTTP interface
...
Create a authenticated proxy tunnel for allowing to connect to ClickHouse Database GUI database client send request via `curl`:

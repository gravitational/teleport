...
Database Service can communicate in both the ClickHouse TCP and HTTP protocols,
...
<Admonition type="warning" title="Preview">
 ...
 Teleport support for ClickHouse is currently in Preview.
...
On the host where you will run the Teleport Database Service, create a systemd
service configuration for Teleport, enable the Teleport service, and start
Teleport:
...
database client send a request via `curl`:

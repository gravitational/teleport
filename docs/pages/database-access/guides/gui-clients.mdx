...
<Admonition type="note">
  Remember to login to your Redis instance with Redis user first by using:
...
Next tick `Use TLS` and `Verify TLS Certificates` boxes and copy the CA certificate returned by `tsh proxy db`.
...
Congratulation. You have just connected to your Redis instance.

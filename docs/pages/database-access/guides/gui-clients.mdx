...
On the Connection tab, enter the default database name (Maintenance database) and the connection service name, same as when you specify connecting using
...
On the SSL tab, set SSL mode to "Require and Verify Identity" and enter the paths to the Certificate Authority, certificate, and private key files from `tsh db config` command:

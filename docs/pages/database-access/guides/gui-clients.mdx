...
<Admonition type="note">
  In order to use a service profile on a Windows client you will first have to export a PG Service File environmental variable in the terminal with `setx PGSERVICEFILE <path_to_.pg_service.conf_file>` and then restart the pgAdmin client. The `.pg_service.conf` file will be created in the user's home directory after a user logs into a database via `tsh db login <dbname>`.
</Admonition>
...
[DBeaver](https://dbeaver.io/) is a SQL client software application and database administration tool.

### MySQL 

<Admonition type="note">
  Teleport's DBeaver MySQL integration only supports MySQL server 8.0.2 or older due to changes made in required handshake parameters starting with MySQL 8.0.3 and above.
</Admonition>

To configure a new connection to a MySQL database, right-click in the "Database Navigator" menu in the main view and select Create > Connection:
...
Navigate to the "SSL" tab, check the "Use SSL" box, uncheck the "Verify Server Certificates" box, and copy the `CA Cert`, `Client Cert`, and `Client Private Key` paths from the `tsh db config` output:

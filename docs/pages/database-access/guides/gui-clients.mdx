...
In "Database Explorer" click and the add button and then pick "Data Source" and pick "Snowflake":
...
In the "Authentication" section set the "Username" to match the one that you are connecting to using Teleport
and enter any value in the "Password" field:

![DBeaver Main](../../../img/database-access/guides/snowflake/dbeaver-main.png)

Next click on "Driver properties" tab and set `account` to any value and in "User properties" set `ssl` to `off`:

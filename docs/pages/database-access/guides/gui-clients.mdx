...
Within the Connection tab, enter the default database name ("Maintenance database")
and the connection service name. This is the same name that you previously specified using
...
the database configuration parameters are shown by `tsh db config` command:

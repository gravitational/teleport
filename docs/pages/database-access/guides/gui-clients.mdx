...
Set the "Username" to match the one that you are assuming when you connect to Snowflake via Teleport and enter any value (e.g., "teleport") in the "Password" field:

![JetBrains General](../../../img/database-access/guides/snowflake/jetbrains-general.png)

Switch to the "Advanced" tab and set any value (e.g., "teleport") as `account` and add a new record named `ssl` with value `off`:

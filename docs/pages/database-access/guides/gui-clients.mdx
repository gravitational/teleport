...
<Admonition type="note">
  Snowflake integration works only in the authenticated proxy mode. Remember to add `--tunnel` flag when starting the proxy, example:
  ```
  tsh proxy db --tunnel --port 2000 snowflake
  ```
</Admonition>

In _Database Explorer_ click and the add button and then pick _Data Source_ and pick _Snowflake_:

![JetBrains Add Database](../../../img/database-access/guides/snowflake/jetbrains-add-database.png)

Next set `Host` to `localhost` and `port` as the one returned by the proxy command (`2000` in the example above).
Set the `Username` to match the one that you are connecting to using Teleport and enter any value in the `Password` field:

![JetBrains General](../../../img/database-access/guides/snowflake/jetbrains-general.png)

Switch to `Advanced` tab and set any value as `account` and add new record named `ssl` with value `off`:

![JetBrains Advanced](../../../img/database-access/guides/snowflake/jetbrains-advanced.png)

<Admonition type="note">
  Password and account name are being ignored as Teleport uses the values provided in the Database Agent configuration.
  SSL set to `off` disables only encryption on local machine. Connection to Snowflake is encrypted by Teleport.
</Admonition>

Now you can click _Test Connection_ to check your configuration.
...
Add new database by clicking the add icon in the left top corner:
...
Set `Host` to `localhost` and `port` as the one returned by the proxy command (`2000` in the example above).
In the _Authentication_ section set the `Username` to match the one that you are connecting to using Teleport
and enter any value in the `Password` field:

![DBeaver Main](../../../img/database-access/guides/snowflake/dbeaver-main.png)

Next click on `Driver properties` tab and set `account` to any value and in _User properties_ set `ssl` to `off`:
...
Now you can click on `Test Connection...` in the left bottom corner:

...
Snowflake integration works only in the authenticated proxy mode. Run `tsh` proxy by using the command below:
```
...
Set the "Username" to match the one that you are connecting to using Teleport and enter any value (e.g., "teleport") in the "Password" field:

![JetBrains General](../../../img/database-access/guides/snowflake/jetbrains-general.png)

Switch to "Advanced" tab and set any value (e.g., "teleport") as `account` and add new record named `ssl` with value `off`:
...
and enter any value (e.g., "teleport") in the "Password" field:

![DBeaver Main](../../../img/database-access/guides/snowflake/dbeaver-main.png)

Next click on "Driver properties" tab and set `account` to any value (e.g., "teleport") and in "User properties" set `ssl` to `off`:

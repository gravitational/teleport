...
Teleport currently supports RDS Proxies with engine family **PostgreSQL** or **MariaDB and MySQL**.
</Admonition>
...
- AWS account with RDS Proxies and permissions to create and attach IAM policies.
- Any RDS Proxies intended for connection through Teleport must have TLS enabled.
...
The command will generate a Database Service configuration with RDS Proxies
auto-discovery enabled on the <Var name="us-west-1" /> region and place it at the
...
The Database Service will discover all RDS Proxies according to the
configuration and register them in the cluster. In addition to the primary
endpoints of the RDS Proxies, their custom endpoints will also be registered.

## Step 4/5. Configure database user credentials

The Database Service connects to a RDS Proxy instance using IAM authentication.
In addition, the RDS Proxy instance must also be able to connect to the RDS
DB instance or Aurora DB cluster using pre-configured database user credentials.

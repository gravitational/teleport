...
- [Enabled AWS OpenSearch Service fine-grained access control](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/fgac.html#fgac-enabling)
- IAM permissions to create IAM roles.
- [sql-cli](https://github.com/opensearch-project/sql-cli)  Command Line Interface (CLI) tool installed in `$PATH`.
...
## Step 1/4. Create IAM roles for OpenSearch Mange Cluster access
...
Teleport AWS OpenSearch service integration leverages the [OpenSearch Fine-grained access control](https://docs.aws.amazon.com/opensearch-service/latest/developerguide/fgac.html) where the IAM role or user is mapped to the OpenSearch role.

In order to configure Role Mapping log into OpenSearch Domain Dashboard an go to the `Security` settings:

![Select Get Started](../../../img/database-access/guides/aws-opensearch/01-opensearch_get_started.png)

Then create new role with least privilege permission or select existing one. For the purpose of this example the `readall` OpenSearch will be used. Select the OpenSearch role and go to the `Mapped users` tab:

![Mapped User](../../../img/database-access/guides/aws-opensearch/02-opensearch_mapped_users.png)

Configure IAM role mapping and add OpenSearch Mange Cluster access ARN as a users:
...
Then you can interact with AWS OpenSearch API via local tunnel created by `tsh proxy db` command:
...
Interactive session can be started using the `tsh db connect` command that will under the hood invoke the `opensearchsql` binary with interactive mode:

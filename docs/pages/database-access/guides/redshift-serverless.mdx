...
Teleport needs AWS IAM permissions to be able to manage IAM user or IAM role
policies.
...
to bootstrap access to Amazon Redshift Serverless:
...
On the same host, create a file at `/etc/teleport.yaml` with the following
content.
...
Service. Replace `WORKGROUP_NAME` and `ENDPOINT_NAME` with the names of your
Redshift Serverless workgroup and endpoint, and replace
`REDSHIFT_SERVERLESS_URI` with the domain name and port of your Redshift
Serverless database.

...
(!docs/pages/includes/database-access/pg-cancel-request-limitation.mdx pidquery="SELECT session_id AS pid, database_name,start_time,trim(query_text) AS querytext FROM SYS_QUERY_HISTORY WHERE status = 'running';"!)

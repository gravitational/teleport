...
- Set up Teleport to access your AWS Redshift Serverless instances.
...
`teleport-redshift-node`.

Under **Trusted entity type** choose "AWS service", and under **Use case** choose "EC2".

Create and attach a policy to the `teleport-redshift-node` role to allow Teleport
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:iam::(=aws.aws_access_key=):role/teleport-redshift-node"
            ],
            "Effect": "Allow"
        },
        {
...
Apply this role to the EC2 instance the Teleport Database Service will be
installed in.
...
to allow the IAM role generated in the previous step to assume this role:
    ...
    "Statement": [
        {
            ...
            "Effect": "Allow",
            "Principal": {
                "AWS": "arn:aws:iam::(=aws.aws_access_key=):role/teleport-redshift-node",
...
Add this role to your Teleport user. You can do this through the web UI, or by
downloading your user definition:

```code
$ tctl get users/yourUser > yourUser.yaml
```
...
```code
$ tctl create -f yourUser.yaml
user "yourUser" has been updated
```
...
Use `--aws-tags <key>=<value>,<key>=<value>` to only match those with specified
AWS resource tags.

For example, if a workgroup has multiple VPC endpoints but
only one of the VPC endpoints will be accessed through Teleport, add the resource
tag `teleport.dev/discovery` with value true to the VPC endpoint, and then specify
...
- In automatic mode, Teleport will attempt to create appropriate IAM policies
  ...
  IAM permissions we applied to the `teleport-redshift-node` role earlier.

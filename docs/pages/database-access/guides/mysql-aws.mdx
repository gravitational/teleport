...
description: How to configure Teleport Database Access for AWS RDS/Aurora MySQL databases.
---

# AWS RDS/Aurora MySQL

This Guide describes how to configure Teleport Database Access for AWS RDS/Aurora MySQL databases.
...
database instance with IAM authentication enabled, or modify an existing one to turn it on. Make sure to use the "MySQL" database type.
...
- `db-cluster-resource-id`: the identifier for the database cluster, can be found under the "Configuration "section in the RDS control panel.
- `db-user-name`: name of the database account to associate with IAM authentication. Can be a wildcard.

See [Creating and using an IAM policy for IAM database access](https://docs.aws.amazon.com/AmazonRDS/latest/AuroraUserGuide/UsingWithRDS.IAMDBAuth.IAMPolicy.html) for more information.

## Create a database user

Database accounts must have IAM authentication enabled to be allowed access. For MySQL:

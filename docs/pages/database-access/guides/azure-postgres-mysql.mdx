...
<Details
  ...
  scopeOnly={true}
>
Static configuration of database access for Azure PostgreSQL/MySQL is available
starting from Teleport `8.1` and Azure database auto-discovery is available
starting from Teleport `10.2`.

Database access for Azure PostgreSQL/MySQL flexible servers is available starting
from Teleport `12.0`.

</Details>

This guide will help you to set up secure access and connect through Teleport
to Azure Database for PostgreSQL or MySQL servers, including Flexible servers.
...
role to provide access to your Azure database. Our example file is
`azure-databse-role.yaml`:
...
- The value(s) of `db_names` corresponds to your MySQL database name(s) in
  Azure, or `postgres` for any PostgreSQL servers.
- The value of `db_users` corresponds to the database user or role we will create
  later on in the guide.
- Edit `db_labels` to further restrict access for this role.

Save this file and apply it to your Teleport cluster:

```code
$ tctl create -f azure-databse-role.yaml
role 'azure-databse-role.yaml' has been created
```
...
$ psql "host=example.postgres.database.azure.com user=ad@example.com sslmode=require dbname=postgres"
```

...
- Set up access to your Azure database for [PostgreSQL](https://azure.microsoft.com/en-us/services/postgresql/)
  or [MySQL](https://azure.microsoft.com/en-us/services/mysql/) server.
...
- Deployed Azure Database for [PostgreSQL](https://portal.azure.com/#blade/HubsExtension/BrowseResource/resourceType/Microsoft.DBforPostgreSQL%2Fservers)
  or [MySQL](https://portal.azure.com/#blade/HubsExtension/BrowseResource/resourceType/Microsoft.DBforMySQL%2Fservers) server.
...
- Database Service can be registered as Azure AD application (via AD's "App
  registrations") and configured with its credentials. This is similar to
  using AWS credentials file and is only recommended for development and testing
  purposes.
- Database Service can run on an Azure VM with attached managed identity. This
  is similar to AWS IAM roles and is the recommended way of deploying the agent
...
<TabItem label="Using managed identity">
  ...
  Next, go to the Azure VM which will run your Database Service agent and add
  the identity you've just created to it:

  ![VM identity](../../../img/database-access/guides/azure/vm-identity@2x.png)

  Attach this identity to all Azure VMs that will be running Database Service
  agents.
</TabItem>
<TabItem label="Using app registrations">
  <Admonition type="note">
    Registering Database Service as Azure AD application is suitable for test
    and development scenarios, or if your Database Service does not run on an
    Azure VM. For production scenarios prefer to use the managed identity
    approach.
  </Admonition>
  ...
  *Add a certificate or secret*:
  ...
  Create a new client secret that Database Service agent will use to
  authenticate with Azure API:

  ![Registered app secrets](../../../img/database-access/guides/azure/registered-app-secrets@2x.png)

  Teleport Database Service uses Azure SDK default credential provider chain to
  ...
  environment-based authentication with client secret, Database Service should
...
Only Azure AD administrator for the database can connect to it and create
subsequent Azure AD users. Go to your database _Active Directory admin_ page
and set the AD admin using _Set admin_ button:
...
### Connect to database as AD admin

Next, you need to connect to your database as the AD admin user.

Use Azure `az` CLI utility to log in as the user that you set as the AD admin,
...
service principal Teleport Database Service will be using. Use `Client ID`

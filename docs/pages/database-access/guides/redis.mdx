...
- Configure mutual TLS authentication between Teleport and your Redis.
...
, run:
    ...
    * `server.key` with generated private key
    * `server.crt` with generated user certificate.
...
### Supported Redis commands
  ...
  <Tabs>
    <TabItem label="Standalone">
      ...
      returns `ERR Teleport: not supported by Teleport` error.
    ...
    <TabItem label="Cluster">
      ...
      returns `ERR Teleport: command not supported` error.
      ...
      - `MGET` - translates the commands to multiple `GET`s and sends them to multiple nodes. Result is merged in Teleport

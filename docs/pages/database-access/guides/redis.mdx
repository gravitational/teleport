...
Each Redis user must be protected by a strong password. We recommend using the password generator built into Redis.
...
, and sets of ACL rules. Redis allows you to provide passwords in plaintext or an SHA256 checksum.
We strongly recommend using SHA256 checksums instead of plaintext passwords.
...
<Admonition type="note" title="Redis Cluster Only">
  ...
  To enable Redis cluster mode in Teleport, add the `mode=cluster` parameter to the connection URI in your Teleport config file.
...
### Supported Redis commands
  ...
  <Tabs>
    <TabItem label="Standalone">
      ...
      Redis in standalone mode doesn't support the commands below.

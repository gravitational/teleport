...
<Tabs>
  ...
  You will need to provide a certificate/key pair for each MongoDB server using 
  the hostnames they are accessible at, and then add the Teleport Database 
  Client CA as an additional trusted CA.
  
  Creating certificate/key pairs using your own CA is out of this guide's scope.
  ...
  To add the Teleport Database Client CA as an additional trusted CA, export it 
  from Teleport and concatenate it with your CA's certificate:
  ...
  This will trust the Teleport Database Client CA and allow access via Teleport,
  while still allowing MongoDB replication over TLS using your CA's certs for
  peer verification.
  </TabItem>
</Tabs>

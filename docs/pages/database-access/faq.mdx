...
Teleport Database Access currently supports the PostgreSQL, MySQL, MariaDB and
MongoDB protocols.

For PostgreSQL and MySQL, both self-hosted and cloud-hosted versions such as AWS
RDS, Aurora (except for the serverless version which doesn't support IAM auth),
Redshift, and GCP Cloud SQL are supported. See the available
...
In Teleport Cloud, the Proxy Service uses the following public addresses for

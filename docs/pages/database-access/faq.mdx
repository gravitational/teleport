...
slated for July 15th, 2021 release.

For PostgreSQL and MySQL, both self-hosted and cloud-hosted versions (such as
AWS RDS, Aurora, Redshift and GCP Cloud SQL) are supported. See available
[guides](./guides.mdx) for all supported configurations.
...
Set `postgres_public_addr` and `mysql_public_addr` proxy configuration fields to
public addresses over which respective database clients should connect. See
...
[Teleport 7.0](../preview/upcoming-releases.mdx#teleport-70-stockholm) slated
for July 15th, 2021 release, but also planning to support more databases in
future based on the customer demand.

...
the user does not have role(s) which **deny** the corresponding `db_users`, `db_names`, or `db_labels`.
...
For example, the following Teleport user "alice@example.com"
is permitted to access any database name or database user

...
RBAC allows Teleport administrators to grant granular access permissions to users following 
the security best practice that: *users should be given access only what their role requires*.
...
3. *engineers* can gain temporary access to the production database in case of
emergency.
...
Teleport's *Role* definition provides the following configuration settings for restricting
database access:
...
It's also possible to use wildcards to match any database names/users.
...
>
  ...
  attempting to use the `postgres` database account (regardless of the database instance
  or database name) or the `postgres database name (regardless of the database instance
...
contain multiple schemas. To change to a different database, a user

...
<Admonition
  type="note"
  title="Database Public Addresses apply only to Separate Proxy Mode"
>
   Public addresses for specific port connection types only apply when using the separate `proxy_listener_mode: separate` in the `auth_service` configuration
  and for Teleport versions prior to 8.0.0.  The [TLS Routing](../../setup/operations/tls-routing.mdx) mode will only use the `public_addr` for all
  protocols. 
</Admonition>
```yaml
proxy_service:

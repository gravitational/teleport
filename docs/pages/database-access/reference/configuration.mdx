...
spec:
  ...
  # MySQL only options.
  mysql:
    # The default MySQL server version reported by the Teleport Proxy Service.
    # When this option is set, the Database Service does not try to check the MySQL server version.

...
configuring Database Access with self-hosted database instances. Note that for database
formats, tctl must be run on the auth server or the remote
user must be be able to impersonate the database service.

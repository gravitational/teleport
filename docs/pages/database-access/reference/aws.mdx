...
configures IAM policies for Amazon RDS, Redshift, ElastiCache and MemoryDB.
...
In addition to discovery, Teleport requires permissions to AWS Secrets Manager
for saving the temporary passwords for Teleport managed ElastiCache and
MemoryDB users. Teleport may also require permissions to KMS keys if custom KMS
...
If any custom key prefix or KMS key ID is used in the
[configuration](./configuration.mdx), you can add the following to the IAM
policy.
```
{
    "Version": "2012-10-17",

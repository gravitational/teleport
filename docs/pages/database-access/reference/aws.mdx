...
Teleport Database Service requires IAM permissions for various tasks depending
on the database type and setup, such as discovering endpoints and metadata of
the database servers, generating IAM authentication tokens, and assuming IAM
roles.

You can generate the IAM permissions with the [`teleport db configure aws
print-iam`](../../database-access/reference/cli.mdx#teleport-db-configure--awsprint-iam)
...
To learn more about a specific database, please refer to the related section
below.
...
Teleport Database Service can automatically manage IAM policies of the attached
IAM identity for RDS access. To use this feature, Teleport requires
`iam:PutRolePolicy` or `iam:PutUserPolicy` permissions to grant itself
necessary IAM permissions for each registered database.

If you prefer to manage IAM identities manually, please refer to the [manage
IAM identities yourself](#manage-iam-identities-yourself-for-rds-access)
...
<Tabs>
  ...
  (!docs/pages/includes/database-access/reference/rds-action-notes-with-modify.mdx!)
...
automatically manage IAM policies of the attached IAM identity for Redshift
access. To use this feature, Teleport requires `iam:PutRolePolicy` or
`iam:PutUserPolicy` permissions to grant itself neccessary IAM permissions for
each registered database. If you prefer to manage IAM identities manually,
please refer to the [manage IAM identities
...
of the attached IAM identity for ElastiCache and MemoryDB that use IAM
...
rotation](#auto-password-rotation-for-elasticachememorydb-access) section.

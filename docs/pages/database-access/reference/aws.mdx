...
authentication tokens, assuming IAM roles, and more.
...
can omit the other permissions if you only need some of them.
...
section](#manage-iam-identities-yourself-for-rds-access) which outlines the
...
automatically manage IAM policies for Redshift access. To use this feature,
Teleport requires `iam:PutRolePolicy` or `iam:PutUserPolicy` permissions to
grant itself neccessary IAM permissions for each registered database. If you
prefer to manage IAM identities on your own, please refer to [this sub
section](#manage-iam-identities-yourself-for-redshift-access) which outlines
the required IAM permissions.

When authenticating as an IAM role, please use the IAM permissions outlined in
[this sub section](#iam-authentication-as-iam-role-for-redshift-access).
...
Redshift Serverless maps IAM roles to database users. Teleport Database Service
must be able to assume these "access" IAM roles which are granted IAM
...
grant itself neccessary IAM permissions for each registered database. If you
prefer to manage IAM identities on your own, please refer to [this sub
section](#manage-iam-identities-yourself-for-elasticachememorydb-access) which
...
section](#auto-password-rotation-for-elasticachememorydb-access).
...
Auto-discovery is not available for DynamoDB.

DynamoDB access is provided by IAM roles with corresponding DynamoDB-related
IAM permissions. Teleport Database Service must be able to assume these
"access" IAM roles.
...
Auto-discovery is not available for Keyspaces.

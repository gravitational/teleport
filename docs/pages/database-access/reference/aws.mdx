...
For Amazon ElastiCache, Teleport requires permission to automatically discover
the Redis clusters. Teleport also requires permission to automatically discover
and modify any Teleport-managed ElastiCache users and permission to manage the
...
passwords, and save passwords in AWS Secrets Manager, if any ElastiCache users
are tagged to be managed by Teleport.
...
## Manual registration
If you prefer to register RDS, Redshift or ElastiCache databases manually using
a [static configuration](./configuration.mdx) or
...
If any ElastiCache users are tagged to be managed by Teleport, below are the
IAM permissions required for managing the ElastiCache users. Otherwise, no

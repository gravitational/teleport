...
To allow Teleport connections to an Aurora instance, it needs to support
IAM authentication.
...
account. Teleport Database Service will need to use the credentials of this AWS user
or service account to use this policy.
...
<Admonition
  ...
  title="Resource ID"
>
  The database resource ID is shown on the Configuration tab of a particular database instance in RDS control panel, under "Resource id". For regular
  RDS database it starts with `db-` prefix. For Aurora, use the database cluster resource ID (`cluster-`), not the individual instance ID.
</Admonition>
...
Teleport Database Access is available starting from the `6.0.0` release.
...
- Review [frequently asked questions](./faq.mdx).

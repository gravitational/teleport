...
Start Teleport and point it to your Aurora database instance. Make sure to
...
version `6.0.0` or newer that includes Database Access support.

```shell
tsh login --proxy=localhost:3080 --user=alice
```

<Admonition type="warning" title="Teleport Certificate">
For testing purposes you might need to provide `--insecure` flag to trust Teleport's

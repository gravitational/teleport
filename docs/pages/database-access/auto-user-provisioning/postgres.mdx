...
to the auto-provisioned user do not have the permission to connect to the target
database:
...
Database auto-user provisioning is curerntly not compatible with RDS Aurora
reader endpoints. Please use auto-user provisioning on the primary endpoints.

### No schema has been selected error

You may encounter the following error when connecting when the admin user does not have `USAGE`
...
This happens when the admin user does not have enough permission to grant the
role to the auto-provisioned the user. To fix this, grant the `ADMIN` option to
the admin user:
```sql
GRANT rds_iam TO "teleport-admin" WITH ADMIN TRUE;
```

...
- Teleport cluster v14.3 or above with a configured [self-hosted
  MongoDB](../guides/mongodb-self-hosted.mdx).
- Ability to connect to and create user accounts in the target database.

<Admonition type="note" title="Supported services">
Automatic user provisioning is not compatible with MongoDB Altas.
</Admonition>
...
<Details title="Limit grantRole action to specific databases">
...
(!docs/pages/includes/database-access/auto-user-provisioning/teleport-role-mongodb.mdx!)

Users created within the database will:

- Have the same name as Teleport username.
- Have `teleport-auto-user` set to `true` in user's `customData`.

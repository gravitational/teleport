...
apply labels to database objects when it encounters these objects in a user
session. When a user connects to a database, the Database Service allows or
denies the session by checking database object labels against the user's
Teleport roles.

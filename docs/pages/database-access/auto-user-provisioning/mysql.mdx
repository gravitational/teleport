...
<Admonition type="note" title="Database permissions">
By default, the newly created won't have permissions to access any particular database. These permissions must be
granted either through the common role `teleport-auto-user` or through database-specific roles, such as `reader`.

Otherwise you may see errors like follows:

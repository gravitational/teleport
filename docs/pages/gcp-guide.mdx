...
As a [BeyondCorp](https://cloud.google.com/beyondcorp/) leader, GCP already provides
some great tools out of the box such as [Cloud Identity-Aware Proxy](https://cloud.google.com/iap/).
This is an excellent tool to quickly get set up securely with GCP but it can become
...
While you can use Teleport for all the services that you would SSH into, this guide is focused on Google Compute Engine. 

In the future, we'll update how to use Teleport with Google Kubernetes Engine (GKE).
...
We recommend setting up Teleport in High Availability (HA) mode. In HA mode Firestore
...
Follow [adding users](enterprise/quickstart-enterprise.mdx#adding-users) or integrate with [G Suite](enterprise/sso/ssh-gsuite.mdx) to provide SSO access.

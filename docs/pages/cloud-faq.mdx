...
The `tctl` admin tool supports downloading session recordings for Teleport version >= `18.7.0` using the
[`tctl recordings download`](reference/cli/tctl.mdx#tctl-recordings-download) command:
...
### Are update times configurable for Teleport Enterprise Cloud?

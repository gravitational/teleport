...
built-in Machine & Workload Identity support to dynamically authenticate to your
Teleport cluster without any shared secrets. When run in this configuration, the
Terraform provider proves its identity to the Teleport Auth Service using
Terraform Cloud's [Workload Identity][tf-workload-id] tokens.

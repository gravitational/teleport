...
description: How to manage dynamic resources using the Teleport Terraform provider from a dedicated deployment server with Machine & Workload Identity .
tags:
...
This guide will setup Teleport Machine & Workload Identity on the server.
Machine & Workload Identity provides identities to machines and services, rather
than human users.

## How it works

This setup relies on a Machine & Workload Identity daemon (`tbot`) to join the Teleport cluster, obtain and refresh credentials for the
...
<Admonition type="tip" title="Failed bot joins">
If `tbot start` fails after consuming the one-time join token (e.g., due to
network issues), the token cannot be reused. Recreate the bot with `tctl bots instances add <name>` to generate credentials for another instance and recreate it to get a new token.
</Admonition>
...
output by Machine & Workload Identity, we use the `identity_file_path` option.

...
<Admonition type="tip" title="Running tctl on the Auth Service host">
If you are running `tctl terraform env` directly on the Auth Service host (e.g.,
via SSH), the output `TF_TELEPORT_ADDR` will be `127.0.0.1:3025`, which is not
reachable from a remote machine. Override it with the Teleport Proxy Service's public address.
{/* lint ignore page-structure remark-lint */}

```code
$ export TF_TELEPORT_ADDR="<Var name="example.teleport.sh:443" />"
```
</Admonition>

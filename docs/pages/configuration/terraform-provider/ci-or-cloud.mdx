...
To allow the Terraform Provider to join from Azure DevOps pipelines, create the following `terraform-bot-token.yaml`:
...
spec:
  azure_devops:
    allow:
    - project_name: "<Var name="ado-project" />"
    organization_id: <Var name="organization-id" />
  bot_name: terraform
...
<TabItem label="Azure DevOps">
In the repository containing your Terraform code add the following Azure DevOps properties in your provider configuration block:
```hcl
provider "teleport" {
  addr        = "<Var name="teleport.example.com:443" />"
  join_method = "azure_devops"
...
In your pipeline configuration, add `$(System.AccessToken)` as an environment variable to the step.
...
- job: Terraform
    ...
    - task: TerraformTask@5
      ...
      inputs:
        provider: 'azurerm'
        command: 'plan'
        environmentServiceNameAzureRM: '<Var name="service-connection" />'
      env:

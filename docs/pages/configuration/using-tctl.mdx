...
`tctl` is included in every Teleport binary release. See [Installing Teleport on
a Single Machine](../installation/single-machine/single-machine.mdx) for
platform-specific instructions, or visit [Download Client
Tools](https://goteleport.com/download/client-tools/) to get the latest version
for your platform.

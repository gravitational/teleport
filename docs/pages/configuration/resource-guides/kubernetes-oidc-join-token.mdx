...
<Admonition type="warning" title="Self-managed Kubernetes clusters">
Cloud-managed clusters (EKS, GKE) have publicly trusted TLS certificates and
public OIDC endpoints by default. Self-managed clusters (k3s, kubeadm, k0s)
require two additional steps:

1. **Make the OIDC endpoint public.** Enable anonymous authentication on the
   API server (e.g., `--anonymous-auth=true` for k3s) and create a
   ClusterRoleBinding granting `system:unauthenticated` the
   `system:service-account-issuer-discovery` ClusterRole.

2. **Trust the API server's CA.** Teleport's Auth Service must trust the TLS
   certificate of the OIDC endpoint.

   - **Self-hosted Teleport:** Copy the Kubernetes API server CA certificate to
     the system trust store (e.g., `/usr/local/share/ca-certificates/`) and run
     `update-ca-certificates`, then restart Teleport.
   - **Teleport Cloud:** You cannot modify the managed Auth Service's trust
     store. Instead, configure your self-managed Kubernetes cluster's OIDC
     endpoint to use a publicly trusted TLS certificate (e.g., via Let's
     Encrypt or a commercial CA).
</Admonition>

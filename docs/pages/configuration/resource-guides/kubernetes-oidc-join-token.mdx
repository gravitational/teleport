...
1. **Trust the API server's CA.** Teleport's Auth Service must trust the TLS
   certificate of the OIDC endpoint. Copy the Kubernetes API server CA
   certificate to the system trust store (e.g.,
   `/usr/local/share/ca-certificates/`) and run `update-ca-certificates`, then
   restart Teleport.
</Admonition>

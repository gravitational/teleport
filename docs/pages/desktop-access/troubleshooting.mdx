...
First, make sure that you [enable the Smart Card service](./manual-setup.mdx#enable-the-smart-card-service)
in Group Policy.
...
You connect to a Windows host and get the error message: **"Signing in with a smart card isn't supported for your account."**
or similar.
...
at `Event Viewer->Application and Services Logs->Microsoft->Windows->Security-Kerberos->Operational` in the Windows Event Log. By default this
...
[Desktop Access Manual Setup](./manual-setup.mdx#allow-remote-rdp-connections).

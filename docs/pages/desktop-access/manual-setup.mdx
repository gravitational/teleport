...
Starting with Teleport 11, you can install Active Directory and configure Teleport Desktop Access through the web UI. See [Getting Started](getting-started.mdx) for more information.
...
teleport:
  auth_token: /path/to/token
  auth_server: mytenant.teleport.sh # replace with your cloud tenant
windows_desktop_service:
  enabled: yes
  ldap:
    addr: "$LDAP_SERVER_ADDRESS"

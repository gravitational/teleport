...
1. Log on to a Windows compiter that is joined to the Active Directory domain with 
...
1. Force the retrieval of the CA from LDAP by running the following cmmand
...
### Enable smart card authenitcation
...
1. Right-click **Always prompt for password upon connection**, select **Edit**, 
   ...
   PIN for each desktop session and provides the PIN to the desktop when establishin the RDP 
   connection.
...
recommends thatyou only grant this right to administrators or other privileged groups.

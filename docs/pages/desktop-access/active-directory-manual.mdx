...
description: Explains hot to manually connect Teleport to an Active Directory domain.
videoBanner: YvMqgcq0MTQ
---

In most cases, you can configure access for Active Directory users through the 
Teleport Web UI and the discovery wizard as described in [Configure access for 
Active Directory automatically](./active-directory.mdx). However, if automation
won't work for your environment—for example, because you use an older version of
Teleport or have custom configuration requirements—you can manually configure 
access for Active Directory users using the Windows Desktop Service.

This guide demonstrates how to connect an Active Directory domain to Teleport manually 
using the Windows Desktop Service and how to log into a Windows desktop from 
...
- An Active Directory domain that is configured for LDAPS. Because Teleport requires an
  encrypted LDAP connection, you should verify that your domain uses
  [Active Direectory Certificate Services](https://learn.microsoft.com/en-us/windows-server/identity/ad-cs/)
  or a non-Microsoft certification authority (CA) to issue LDAPS certificates.
...
1. Create a service account with a randomly-generated password by copying and pasting in 
   ...
   authentication. You can reset the password for service account if you need to 
...
1. Get the security identifier for the new service account by running the following command:
   
   ```powershell
   Get-AdUser -Identity $SamAccountName | Select SID
   ```
   
   Copy the security identifier—beginning with "S-"—returned. You'll use this value
   for the `sid` field when you configure the `ldap` settings in a later step.

## Step 2/7. Prevent the service account from performing interactive logins

The next steps modify group policy objects (GPOs) and changes to group Policies
can take time to propagate to all hosts. You can force changes to take effect 
immediately on your current host at any time by opening PowerShell and running 
`gpupdate.exe /force`. The change might still take time to propagate to other 
hosts in the domain.

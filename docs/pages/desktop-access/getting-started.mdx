...
<Admonition type="note" title="gpupdate.exe">
  Throughout this step and the next one, you will be modifying GPO's, and
  sometimes GPO modifications can take some time to take effect. You can force
  your GPO changes to take effect immediately at any time by opening a
  PowerShell prompt and running `gpupdate.exe /force`.
</Admonition>

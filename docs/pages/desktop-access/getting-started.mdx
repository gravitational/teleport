...
## Step 1/6. Create a restrictive service account `User`.
...
Next, in the same PowerShell prompt, enter the following commands to limit your service account's TODO permissions to the bare minimum necessary for
Teleport to operate.
...
4. Give your GPO a name like `Block Interactive Login` and ensure `(none)` is selected for `Source Starter GPO`, then hit `OK`.
...
1. From the `Group Policy Management` window, right-click on the GPO you just created (`Block Interactive Login`) and select `Edit...` from the context menu.
2. Select:
...
Next, we need to configure a GPO to tell your computers to trust Teleport's CA, allow the certificate-based mechanism that Teleport uses under the hood
(smart cards), and allow Teleport to make RDP connections.
...
1. From the machine running your existing Teleport cluster, get the Teleport user CA certificate by running:
...
<Admonition type="note" title="Domain Wide Policy">
  For the purposes of this example, we apply the GPO we are about to create to
...
1. Back on your Domain Controller, create a new GPO like you did in step 2 above, this time giving it a name like `Teleport Access Policy`.

...
1. From the start menu of your Domain Controller, search for `Active Directory Users and Computers` and open the corresponding program.
2. Find your domain in the side menu, and right click the container you want the service account to live in (typically `Users`).
3. From the context menu, select `New > User`.
4. Fill out the fields for your Teleport LDAP Service Account. For the `User logon name` field, you can use `svc-teleport`.
5. On the password screen, **unselect**  “User must change password at next login” and **select** “Password never expires”. Make a note of what values you put
...
1. Right click the same container you did before, and select `New > Group`.
2. Enter a Group name such as `svc-teleport-group`, **noting down what you choose** as it will be used later for the PowerShell variable `$TeleportLDAPGroupName`.
3. Ensure that `Global` is selected for Group scope, and `Security` for Group type, then hit `OK`.
4. Finally, add the User to the Group you just made by right clicking the User and selecting `Add to group`. Type the name of the Group, and confirm correctness
...
1. To create the Group Policy Object (GPO), first open the "Start" menu and run "Group Policy Management".
2. On the left pane, navigate to `$FOREST > Domains > $DOMAIN`, selecting your forest and domain names respectively.
3. Right click `$DOMAIN` and select `Create a GPO in this domain, and Link it here...` from the context menu.
4. Give your GPO a name like `svc-teleport-gpo` and ensure `(none)` is selected for Source Starter GPO, then hit `OK`.

<Figure align="left" bordered caption="Create and Link GPO">
  ![Create and Link GPO](../../img/desktop-access/create-and-link-gpo.png)
</Figure>

5. Now, in the left pane navigate to `$FOREST > Domains > $DOMAIN > Group Policy Objects` and click on the GPO you just created (`svc-teleport-gpo`).
6. In the `Scope` tab, under `Security Filtering`, select `Authenticated Users` and then click the `Remove` button and hit `OK`.
...
7. In the same `Security Filtering` pane select `Add...`.
8. Enter the name of the Group (`svc-teleport-group`) in the resultant popup, clicking `Check Names` to confirm the spelling before pressing `OK`.

After this step, only the `svc-teleport-group` should be displayed in the "Security Filtering" pane:
...
Next, we need to configure our GPO (`svc-teleport-gpo`) to trust Teleport for user
...
2. Transfer the `user-ca.cer` file to your Domain Controller.
3. Then, on your Domain Controller, again open the "Group Policy Management" window.
4. On the left pane, navigate to `$FOREST > Domains > $DOMAIN > Group Policy Objects`
5. Right click on the Group Policy Object you made previously (`svc-teleport-gpo`), and select `Edit...`.
...
2. Right click on `Trusted Root Certification Authorities` and select `Import`.
3. Click through the wizard, selecting your CA file.
...
1. To enable the smart card service, select:
...
2. Double click `Deny log on locally` and in the popup, check `Define these policy settings`.
3. Then click `Add User or Group...`, `Browse ...`, enter the name of the group you created above (`svc-teleport-group`) and hit `Check Names` select your Group, and then hit `OK` on all the windows.
4. Repeat 1 through 3 above for `Deny log on through Remote Desktop Services`.
...
Now, complete the service account's configuration by giving its corresponding Group the minimum necessary permissions for Teleport to work.

...
- A server or virtual machine running a Windows Server operating system with
Remote Desktop Services (RDP) enabled and the RDP port available to the Linux server.
...
```code
# Installs Teleport Certificate and reboots
...
to your Teleport proxy service and put the Windows machine address under `non_ad_hosts`:

```yml
version: v3
teleport:
  nodename: examplewindowsaccess.teleport.com

...
## Step 1/5: Create a Service Account User, Group, and Group Policy Object
...
menu, select `New > User` and fill out the fields for your Teleport LDAP Service Account. Make a note of what value you put for user logon name.
On the password screen, unselect “User must change password at next login” and select “Password never expires”, make note of your password, and
...
>
  By default, domain users have Read and List Contents throughout Active Directory. If you have configured
...
be used later on for the PowerShell variable `$TeleportLDAPGroupName`.
...
Next, open "Start" menu and run "Group Policy Management". On the left pane, navigate to `$FOREST > Domains > $DOMAIN > Group Policy Objects`,

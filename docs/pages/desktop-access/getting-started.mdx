...
description: Getting started with Teleport Desktop Access
---
...
- An existing Teleport cluster and user, version 8.0 or newer
  - see [Teleport Getting Started](../getting-started.mdx) if you're new
...
## Step 2. Configure Group Policy to allow Teleport connections

{/* TODO: add screenshots */}
{/* TODO: script this using PowerShell */}

Next, we need to configure Active Directory to trust Teleport for user
authentication and allow the mechanisms that Teleport uses under the hood
(smart cards).

Get the Teleport CA certificate using `tctl auth export --type=windows >user-ca.cer`.
...
### Step 2b. Allow remote RDP connections
...
{/*
TODO: explain why we need to disable NLA.

awly: I don't understand this very well. NLA handles authentication at the
early stages of RDP connection (if not before it), before any bitmaps are even
sent from the server (i.e. no login screen). NLA appears to only support
passwords or Kerberos-based authn for members of the same domain as the target
host.
Smartcards do their magic after RDP connection was established and the login
screen was rendered, in the background. I think they are just incompatible with
NLA when you're not connecting from a Windows client.
*/}
...
### Step 2c. Enable the Smart Card service
...
### Step 2d. Open firewall rules for RDP
...
A new tab will open and, after a few seconds, you should be logged into your
target Windows host.

## Troubleshooting

If you hit any issues, check out the [Troubleshooting](./troubleshooting.mdx)

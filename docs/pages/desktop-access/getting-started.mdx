...
following for the Windows Desktop Service: 
   ...
   - List the Windows IP addresses or host names under `static_hosts`.
...
1. (Optional) Add labels for the Windows Desktop Service to the configuration file.
   ...
   You can add labels to Windows computers using `label` section in host config. For example, the following
   adds the `important: true` label:

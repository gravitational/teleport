...
- A browser on your local machine that supports the File System Access API,
  ...
  Chromium-based browsers like Google Chrome, Brave, and Microsoft Edge. You can
  see a full compatibility table in the [Mozilla Developer Network
...
Retrieve the role for the Teleport user you want to modify and save their role
definition in `user.yaml`. Here we assume the user is called `myser`:

```code
$ tctl get user/myuser > user.yaml
```

Edit `user.yaml` to add the new role:
...
The Teleport Desktop Service converts traffic from the Teleport Web UI into RDP
messages to send to the remote desktop. The Desktop Service also converts RDP
traffic from the the remote desktop into messages to send to the Teleport Web
UI. Based on these messages, the Teleport Web UI advertises information about—or
performs modifications on—the shared directory.

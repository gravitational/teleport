...
Direct mode is only available for self-hosted Teleport clusters. Teleport
Enterprise Cloud and Teleport Team fully manage and host the Teleport Proxy and 
Auth services. Teleport Auth Servers in that hosting are not available 
for direct connections from external networks, just Teleport Proxies.
</Admonition>

Direct mode requires network connectivity from the Teleport Proxy

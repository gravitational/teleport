...
Be aware, Desktop Access session recording comprises of raw PNG-based files,
and are stored on the host running the Desktop Access service. Session recordings
are stored permanently, but only accessible through the Web UI for sessions
still available in the audit log. Refer to the `storage` section of the [Teleport Configuration Reference](../../reference/config.mdx)
for more information on session logging and recoring management.

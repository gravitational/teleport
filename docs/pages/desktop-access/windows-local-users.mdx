---
title: Configure access for local users
description: Configure passwordless access for local Windows users on Windows computers that aren't joined to an Active Directory domain.
videoBanner: 9DyKQbg4ORc
---

This guide demonstrates how to configure Teleport to provide secure, passwordless access
to Microsoft Windows desktops that aren't joined to an Active Directory domain.

The following diagram provides a simplified view of how you configure Teleport
Desktop Access for local Windows users:

![Configure access for local Windows users](../../img/desktop-access/windows-local-users.png)
...
To prepare Microsoft Windows desktop:

1. Open a Command Prompt (`cmd.exe`) window on the Windows computer you want to enroll.

1. Export the Teleport user certificate authority by running the following command:
   
   ```code
   $ curl -o teleport.cer https://teleport-proxy.example.com/webapi/auth/export?type=windows
   ```

1. Download the [Teleport Windows Auth
Setup](https://cdn.teleport.dev/teleport-windows-auth-setup-v(=teleport.version=)-amd64.exe) program.

1. Run the Teleport Windows Auth Setup program and select the Teleport certificate that you 
exported when prompted.
   
   The setup program:

   - Installs the selected Teleport certificate.
   - Loads the required dynamic link library (DLL) for Teleport to use.
   - Disables Network Level Authentication (NLA) for remote desktop services. 

1. Restart the computer.

You can also run the Teleport Windows Auth Setup program as an administrator
in a Command Prompt window or PowerShell terminal to automate the process.
After you have downloaded the setup program, run following command
to install the Teleport certificate, load the DLL, disable NLA, and restart the 
...
following for the Windows Desktop Service: 
   ...
   - List the Windows IP addresses or DNS names under `non_ad_hosts`.
   
   Teleport can't automatically discover Windows desktops without Active Directory. 
   Therefore, you must specify the Windows IP addresses or DNS names that you want to 
   enroll using the Teleport configuration file or use the Teleport [API](../api/introduction.mdx) 
   to build your own integration. You can find an example of API integration on 
...
1. (Optional) Add labels for the Windows Desktop Service to the configuration file.
   ...
   EC2 private IP DNS names matching `.us-east-2.compute.internal`:
...
1. Under Resources, click **Desktops**.
...
Windows login to use for the connection.
   ...
   Teleport opens a remote desktop connection and start recording the desktop session. When you're
...
For more information about how to create, assign, and update roles, see [Access Controls 

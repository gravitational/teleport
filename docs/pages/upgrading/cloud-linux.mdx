---
title: Upgrade Teleport Cloud Clusters on Linux
description: Provides instructions for upgrading Teleport Cloud clusters that run on Linux servers. 
---
...
run the following `tctl` command to get the agent's host name and access the
host via `tsh`:

```code
$ HOST=00000000-0000-0000-0000-000000000000
$ USER=root
$ tsh ssh ${USER?}@$(tctl get node/${HOST?} --format json | jq -r '.[0].spec.hostname')
```

...
This guide explains how to enable automatic updates for Teleport agents.

## How it works

When automatic updates is enabled, a Teleport updater is installed alongside
...
critical update is availble, the Teleport agent will be updated outside the
...
1. Ensure that the Teleport repository is properly configured to use the
   ...
   ```code
   $ curl https://goteleport.com/static/install.sh | bash -s <Var name="version" /> cloud
   ```

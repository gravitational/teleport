...
included in the `teleport` package and uses the `autoupdate_version` and
...
## Quick Setup for Linux Servers

Users of cloud-hosted Teleport Enterprise can enable managed updates
on Linux servers with a single command:

```code
$ sudo teleport-update enable --proxy=[cluster-name].teleport.sh
```
...
the cluster version is updated. Keep reading to learn how to customize this.
...
If you are running a cloud-hosted Teleport Enterprise cluster, the
`autoupdate_version` is managed for you and cannot be edited.

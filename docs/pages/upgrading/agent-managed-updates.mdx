...
all Teleport packages, alongside the `teleport` binary. Admins configure updates by managing the
`autoupdate_version` and `autoupdate_config` dynamic resources.

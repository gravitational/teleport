...
In Teleport Enterprise (cloud hosted), we manage the Auth Service and Proxy Service for you. You
can determine the current version of these services by running the following
command, where `mytenant` is the name of your Teleport Enterprise tenant:

```code
$ curl -s example.teleport.sh/webapi/ping | jq '.server_version'
```

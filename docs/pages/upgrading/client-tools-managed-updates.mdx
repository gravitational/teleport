...
2. Store it in `~/.tsh/bin` (`$TELEPORT_HOME/bin`).
...
Allowing to mirror the CDN in a private network or to use a custom build.
...
### Managed updates initiation

Client tools make a request to the proxy discovery endpoint `/v1/webapi/find` during every login
in order to retrieve the required client tools version and check the update mode for the target cluster.

To enable version checks for `tsh ssh` or `tsh proxy ssh` commands in case when login is not required,

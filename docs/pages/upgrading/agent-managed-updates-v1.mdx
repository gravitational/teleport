---
title: Managed Updates for Teleport Agents (legacy)
...
Please consider using [Managed Updates for Agents v2](./agent-managed-updates/agent-managed-updates.mdx),
...
1. Determine the Teleport version to install by querying the Teleport Proxy
   ...
   Replace <Var name="stable/cloud" /> with the name of your update
...
1. Ensure that the Teleport repository is properly configured to use the 
   ...
   1. Follow the instructions in the Teleport [installation
      guide](../installation/single-machine/linux.mdx#package-repositories) to install the `teleport`
...
## Working with the teleport-upgrade tool

This section explains advanced usage of the v1 `teleport-upgrade` script for Linux.
Note that this tool is no longer actively developed, and the Managed Updates v2
`teleport-update` binary should be used instead.
For Managed Updates v2 instructions, see [Managed Updates for
Agents (v2)](./agent-managed-updates/agent-managed-updates.mdx).

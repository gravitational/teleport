...
<Admonition type="note">
   An earlier version of this guide required you to setup a public web server to
   host the Teleport version, this is not required anymore as the Teleport proxy
   service can now act as a version server.
</Admonition>

- (!docs/pages/includes/tctl.mdx!)

## Step 1/4. Check the version served by the proxy

The Teleport Proxy serviceexposes by default a version channel called `default`
and serving the same version the proxy is running.

If your proxy public address is <Var value="teleport.example.com:443" />, you can
query the version server with the following command:\

```code
$ curl "https://<Var value="teleport.example.com:443" />/v1/webapi/automaticupgrades/channel/default/version"
(=teleport.version=)
```

<Details title="Changing which version is served by the version server">

By default, the version server has a single `default` channel, serving the proxy
version. If you want to override the default version or add other channels you
can pass custom proxy configuration:
...
configuration. If proxies are configured differently, you will experience agents
...
1. Configure the upgrader to connect to your custom version server and subscribe
   ...
   ```code
   $ echo "<Var value="teleport.example.com:443" />/v1/webapi/automaticupgrades/channel/default" | sudo tee /etc/teleport-upgrade.d/endpoint
   ```

...
This policy might be created by AWS web console or via [AWS CLI](#via-aws-cli).
...
By creating `autoupdate_version.yaml` we control the version of the Teleport that must be installed
during EC2 instance bootstrap and enabled the managed updates.
...
teleport-update[160893]: INFO [UPDATER]   Teleport is up-to-date. Update window is active, but no action is needed. active_version:18.2.5 agent/updater.go:877
...
# Confirm that the node is joined (it might take up to 15 min to by synced).
$ tctl inventory ls

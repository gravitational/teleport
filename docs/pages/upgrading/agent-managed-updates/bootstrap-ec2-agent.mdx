       ...
       "Statement": [
         {
           ...
           "Resource": "arn:aws:secretsmanager:<Var name="us-west-2" />:<Var name="aws-account-id"></Var>:secret:teleport/join-token-*"
         }
       ]
     }'
   ```
...
1. Launch the EC2 instance with the instance profile:
   ...
   ```code
   # Specify the AMI image for the instance and a security group that allows at least outbound traffic.

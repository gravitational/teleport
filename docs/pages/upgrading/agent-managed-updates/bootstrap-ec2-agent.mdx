...
Delegated joins avoid shipping any secret token in user data (as is required with a join token)
You create a named token resource that encodes context rules (your AWS account, role ARNs, regions, etc.),
then the agent proves its identity to the Auth Service using cloud-issued credentials.
...
User data example with join token:

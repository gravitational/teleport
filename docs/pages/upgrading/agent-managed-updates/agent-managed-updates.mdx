---
title: Managed Updates (v2) for Teleport Agents & Bots
description: Describes how to set up Managed Updates (v2) for Teleport Agents & Bots
tags:
  - conceptual
  - platform-wide
---
...
This document describes Managed Updates for Agents & Bots (v2), which replaces Managed Updates v1.
...
- Teleport Enterprise, Teleport Community Edition, and OSS Teleport
...
tbot installations, while `teleport-upgrade` is only capable of managing Teleport Agents.
...
Managed Updates for Agents & Bots are designed to manage long-running, unattended Teleport
...
with the rollout for both Teleport Agents and long-running (agent-like) tbots.
...
## Quick setup for existing Linux Agent & Bot installations
...
## Quick setup for new Linux Agents & Bot installations
...
Note that the Install Script will install tbot by default, but it does not
automatically configure it like it does for Teleport. See below for bot configuration
instructions.
...
Similarly, you can create an `/etc/tbot.yaml` file, either manually or using
`tbot configure`. After, tbot can be enabled and started via the `systemctl`
command:
...
## Configuring managed agent & tbot updates

Managed agent & bot updates are configured via two Teleport resources:
...
This schedule would update agents & bots in the `staging` group at 4 UTC, and then update

...
## Quick setup for existing Linux Agents
...
Note that unmanaged installations of tbot are not automatically converted to managed
by `teleport-update enable`.

## Quick setup for new Linux Agents
...
the [Agent Installer & Updater section](https://goteleport.com/download/all-downloads/?kind=agentInstaller&os=linux) of the downloads page.
Next, invoke `teleport-update` to install the correct version for your cluster.

```code
$ tar xf teleport-update-[version].tgz
$ cd teleport-updates-[version]

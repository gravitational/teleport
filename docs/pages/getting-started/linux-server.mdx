...
<Tabs>
  ...
  1. Create a systemd unit file using the `teleport install systemd` command and the [unit file path](https://www.freedesktop.org/software/systemd/man/systemd.unit.html) you'd
  like to use for your configuration. The `/etc/systemd/system/teleport.service` path is recommended in most use case:

    ```code
    $ sudo teleport install systemd -o /etc/systemd/system/teleport.service
    ```
  2. Enable and start the new teleport service daemon:
...
<Admonition title="The Teleport daemon" type="tip">
...
teleport service daemon is configured, see our guide on how to [Run Teleport as a Daemon](https://goteleport.com/docs/setup/admin/daemon/).
</Admonition>

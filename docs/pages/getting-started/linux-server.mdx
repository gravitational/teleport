...
<Tabs>
  <TabItem label="Public internet eployment with Let's Encrypt">
  ...
  ```code
  $ sudo teleport configure --cluster-name=tele.example.com -o file
  $ sed -i '/^\s*https_keypairs/d' /etc/teleport.yaml
  # Replace `tele.example.com` with your domain name.
  $ export TELEPORT_PUBLIC_DNS_NAME="tele.example.com"
  $ cat >> /etc/teleport.yaml <<EOF
    public_addr: $TELEPORT_PUBLIC_DNS_NAME:443

...
Teleport Workload Identity will support SPIFFE Federation, allowing trust
...
Teleport will support web application access where one application depends on
...
Cluster admins will be able to get a detailed overview of the Okta integration
...
Kubernetes Operator will be able to lookup values from Kubernetes secrets for
`GithubConnector.ClientSecret` and `OIDCConnector.ClientSecret`.

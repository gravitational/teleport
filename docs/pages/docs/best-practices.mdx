...
1. Select a branch from [github.com/teleport](https://github.com/gravitational/teleport/) corresponding to the version of the documentation/product you'd like to work on.
...
4. Validate your changes locally using the super helpful linters provided [therein](https://github.com/gravitational/next#development-related-commands).
5. Commit your changes and create a PR.
6. Get approved and merge your changes in.

Typically if you make changes to some version `A.b`, you'll also want to port those changes to `master/main` and any version following `A.b.`

In otherwords, making changes to `6.1` usually entails making changes to `6.2` and `7.0 (master/main)`. This isn't universally the case (if some document or change is specific to a single version) but generally applies. Eventually most of this will be automated since it can be a bit cumbersome.
...
Our documentation is primarily built with [Next](https://github.com/gravitational/next) which has the following awesome features:
...
1. `tsh`, `tctl`, etc. should be placed in tic marks.

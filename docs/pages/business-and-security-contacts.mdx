...
This section explains how to view the configured Business and Security contacts for a cluster.

Currently, contacts can only be viewed and managed through the **Teleport Web UI**.  
Other tools like `tctl` and the API are not supported for contact operations.

Users will need permission to `list` the `contact` resource. On dashboards, all users have this permission.

### Teleport Cloud

To view or manage contacts for a cluster in Teleport Cloud, follow these steps:
...
Self-hosted users must go to their **dashboard cluster** in order to view and edit contacts.
...
This section explains how to add new Business or Security contacts to your cluster.

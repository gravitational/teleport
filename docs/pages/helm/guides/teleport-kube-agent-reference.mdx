...
`roles` is a comma-separated list of services (Teleport refers to these internally as `Roles`, hence the naming) which should be enabled
when running for the `teleport-kube-agent` chart.
...
<Admonition type="warning">
  Each static token you configure must be unique, as the token itself is used to define which services will be supported.

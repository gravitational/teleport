---
title: Running a Teleport cluster with a custom Teleport configuration in Kubernetes using Helm
description: How to install and configure a Gravitational Teleport cluster with a custom config in Kubernetes using Helm
---
...
We also have a specific [migration guide from older versions of the Helm charts](./migration.mdx) which may also be useful.
...
For this example, we'll be using this `teleport.yaml` configuration file:
...
auth_service:
  ...
  tokens:
  - "proxy,node:cars-are-nicer-than-trains"
  - "trustedcluster:cats-are-way-better-than-dogs"
...
<Admonition type="note" title="External proxy port">
  Note that although the `proxy_service` listens on port 3080 inside the pod, the `LoadBalancer` service configured by the chart will
  always listen externally on port 443 (which is redirected internally to port 3080).
...
After editing the `ConfigMap`, you must inititate a rolling restart of your Teleport deployment to pick up the changed `ConfigMap`:

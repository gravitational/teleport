---
title: Running a simple one-pod Teleport cluster in Kubernetes using Helm
description: How to install and configure a one-pod Gravitational Teleport cluster in Kubernetes using Helm
---

In this guide, we'll go through how to set up a simple one-pod Teleport cluster in Kubernetes using Teleport Helm charts.
...
In standalone mode, the `teleport-cluster` Helm chart runs a simple Teleport cluster deployment using one pod. You can deploy it into a

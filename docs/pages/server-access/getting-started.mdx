...
4. Create a simple Go client to interact with the Teleport API.
...
- A registered Doman Name with an available subdomain like `tele.example.com`.
- A Two-Factor Authentication app (such as Authy or Google Authenticator).
- Teleport (=teleport.version=) installed locally.

## Overview

The first four steps are visualized below since they involve using multiple terminals and instances:
...
>
  ![Getting Started overview](../../img/server-access/getting-started-overview.svg)
</Figure>
...
1. Create two new instances of your desired Linux distribution (such as Ubuntu 20.05, CentOS 8.0-1905, or Debian 10).
   ...
   >
     ![Subdomain and IP mapping](../../img/server-access/subdomains.svg)
   </Figure>
...
3. Configure Teleport on the *Bastion Host*.
   ...
   Teleport will now automatically acquire an ACME-compliant TLS certificate. 
...
1. Create a *join token* to add the second Node to the `tele.example.com` cluster. In a second, *Bastion Host*, terminal run the following command:

   <Tabs>
     <TabItem label="Create a token">
      ```bash
      # Let's save the token to a file
      sudo tctl tokens add --type=node > token.file
      ```
     </TabItem>

     <TabItem label="token.file">
     ```bash
     The invite token: 05e3346ea255c609d4747655439e2a5e
     ...
     > teleport start \
     --roles=node \
     --token=05e3346ea255c609d4747655439e2a5e \
     --ca-pin=sha256:14b144e13d98d2f53acf5730046127752f4aa21ce6071055b540a4d2145c306f \
     --auth-server=172.31.41.144:3025

     Please note:

     - This invitation token will expire in 60 minutes
     - 172.31.41.144:3025 must be reachable from the new node
     ```
     </TabItem>
   </Tabs>
...
4. You should now be able to view both Nodes in the Teleport Web interface after logging in as `tele-admin`:
   ...
   >
     ![Both Nodes in the Web UI](../../img/server-access/teleport_ui.svg)
   </Figure>
...
2. Execute the following command within your *Bastion Host* console:
   ...
   It displays the universal resource catalog with all queried resources in one view: 
...
<Figure
  align="center"
  bordered
  caption="Teleport API"
>
  ![Teleport API](../../img/server-access/api.svg)
</Figure>
...
2. Create a user `api-admin` with the built-in role `admin`: 

   ```bash
   # Create a new user for the API client
   sudo tctl users add api-user --roles=admin
   ```

3. Within the same terminal login as `api-admin`: 
   
   ```bash
   # Login in as the API user
   tsh login --user=api-user --proxy=tele.example.com:44
   ```
   
   This will save and load `api-admin`'s `tsh` profile automatically within the Go client.
...
6. Execute: `go run main.go`. On success, it will display the following:
...
To further recap, this Getting Started Guide walked through:

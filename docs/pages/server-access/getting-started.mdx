...
<Admonition type="tip" title="Tip">
  This guide also demonstrates how to configure Teleport SSH Service using the **bastion pattern** so that only a single Node can be accessed publicly.
</Admonition>
...
- One host running a Linux environment (such as Ubuntu 20.04, CentOS
  8.0, or Debian 10). This will serve as a Teleport SSH Service machine.
...
2. On the host where you will run your Teleport SSH Service, follow the instructions for your environment to install Teleport.

   (!docs/pages/includes/install-linux.mdx!)

   Next, we'll create a **join token** so you can start the Teleport SSH Service and
   add it to your cluster.

## Step 2/4. Add a SSH Service to the cluster

### Create a join token

Next, create a join token so you can add the SSH Service to your Teleport cluster.

```code
# Let's save the token to a file
$ tctl tokens add --type=node --format json | jq -r '.token'  > token.file
```

`--type=node` specifies that the Teleport Agent will act and join as an SSH
server.
...
### Join your SSH Service to the cluster

On your SSH Service machine, save `token.file` to an appropriate, secure, directory you have
the rights and access to read.

<ScopedBlock scope={["oss", "enterprise"]}>

Configure and start the SSH Service instance. Change `tele.example.com` to the address of your Teleport Proxy
Service. Assign the `--token` flag to the path where you saved
`token.file`.

```code
# Generate the Teleport configuration file, which is written to /etc/teleport.yaml
...
Configure and start the SSH Service instance. Change `mytenant.teleport.sh` to your Teleport Cloud tenant
...
Now that you have the SSH Service configured you can update the configuration
as needed, stop and restart with `sudo teleport start`. See
[Run Teleport as a Daemon](../management/admin/daemon.mdx) for running as a daemon using systemd.
...
You should now be able to view your Teleport SSH Service in the Teleport Web UI after
...
`tele-admin` can now execute the following to find the cluster's SSH Service names,
...
<ScopedBlock scope={["oss", "enterprise"]}>
   ...
   ---------------- -------------- --------------------------------------
   ip-172-31-35-170 ⟵ Tunnel       env=example, hostname=ip-172-31-35-170
...
<ScopedBlock scope={["cloud"]}>
   ...
   Node Name        Address        Labels
   ---------------- -------------- --------------------------------------
   ip-172-31-35-170 ⟵ Tunnel       env=example, hostname=ip-172-31-41-144
   ```
</ScopedBlock>
...
<ScopedBlock scope={["cloud"]}>
`tele-admin` can SSH into the Node by running the following command locally:
...
- Connect to other Nodes in the cluster by using the Node name in the `tsh ssh` command.
...
---------------- ------------------------------------ -------------- -------------------------------------
ip-172-31-35-170 4980899c-d260-414f-9aea-874feef71747                env=example,hostname=ip-172-31-41-170
ip-172-31-41-144 f3d2a65f-3fa7-451d-b516-68d189ff9ae5 127.0.0.1:3022 env=example,hostname=ip-172-31-41-144
```
</ScopedBlock>

<ScopedBlock scope={["cloud"]}>
Execute the following command on your desktop:
...
Nodename         UUID                                 Address        Labels
---------------- ------------------------------------ -------------- -------------------------------------
ip-172-31-35-170 4980899c-d260-414f-9aea-874feef71747                env=example,hostname=ip-172-31-35-170
```
</ScopedBlock>
...
Customized labels can be defined in your `teleport.yaml` configuration file or during SSH Service creation.
...
```code
# Run the ls command on all SSH Services with a label

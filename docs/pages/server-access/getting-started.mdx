...
4. Configure Teleport to use ACME and to automatically acquire a TLS certificate:

   <Admonition type="tip" title="Tip">
    Teleport will now automatically acquire an ACME-compliant TLS certificate by default. While you can use a third-party ACME client (like Let's Encrypt certbot), the configuration commands below will do the same.
   </Admonition>
...
5. Launch Teleport on `auth.example.com`: `sudo teleport start`. 
...
## Step 3c. Connect to the Teleport API

Let's connect to the Teleport API using a simple role-based authentication method:

1. Open a terminal to `auth.go-teleport.com`.

2. Create a user `api-admin` with the built-in role `admin`: `tctl users add api-user --roles=admin`.

3. Within the same terminal (or another), log in as `api-admin`: `tsh login --user=api-user --proxy=api.go-teleport.com:443`.
...
5. Create a file `main.go` via `sudo touch main.go` followed by `sudo nano main.go`. Copy the following Go script into `main.go` modifying the `Addrs` strings as needed:
   ...
   func main() {
       ...
       clt, err := client.New(ctx, client.Config{
           Addrs: []string{
               "auth.go-teleport.com:443",
               "auth.go-teleport.com:3025",
               "auth.go-teleport.com:3024",
               "auth.go-telport.com:3080",
           },
...
6. Execute: `go run main.go`. On success:
   ...
   ```bash
   2021/05/04 17:43:24 Example success!
   2021/05/04 17:43:24 Server response: {api.go-teleport.com 6.1.3 Kubernetes:true App:true DB:true  api.go-teleport.com:443 {}  %!s(int32=0)}
   2021/05/04 17:43:24 Server version: 6.1.3
   ```
...
5. How to connect to Teleport API programmatically.

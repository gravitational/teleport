...
$ tctl tokens add --type=node --format json | jq -r '.token'  > token
```

`--type=node` specifies that the Teleport Agent will act and join as an SSH
server.

`> token` indicates that you'd like to save the output to a file name `token`.
...
On your SSH Service machine, save `token` to an appropriate, secure, directory you have
the rights and access to read.

<ScopedBlock scope={["oss", "enterprise"]}>

Configure the SSH Service instance. Change `tele.example.com` to the address of your Teleport Proxy
...
Configure the SSH Service instance. Change `mytenant.teleport.sh` to your Teleport Cloud tenant
address. Assign the `--token` flag to the path where you saved `token`.
...
On the host where you will run the Teleport SSH Service, execute the

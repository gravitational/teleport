...
```code
$ sudo teleport join openssh --address <Var name="node" /> --proxy-server <Var name="proxy" /> \
  --join-method token --token <TOKEN> \
  --labels env=dev
```

Note that both the Teleport Proxy Service and node addresses must contain a host and a port.
Check that your new node was listed with `tsh ls` or in the Web UI.  You can edit the

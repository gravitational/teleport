...
sudo tsh ssh --port=22 user@host.example.com
...
```bsh
# Login as Joe
sudo tsh login --proxy=proxy.example.com --user=joe
...
```bash
sudo eval `ssh-agent`
sudo tsh --proxy=root.example.com login
```
...
```bash
sudo tsh ssh --port=22 dev@database.root.example.com
```

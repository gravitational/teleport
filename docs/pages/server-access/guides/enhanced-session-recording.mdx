...
Teleport with Enhanced Session Recording requires Linux kernel 4.18+ as well as kernel headers.
...
<Tabs>
  ...
  <TabItem label="CentOS/RHEL 8+">
    ...
    **Example Script to install relevant bcc packages for Amazon 2 Linux**
    ...
    Make sure the machine is at Kernel 4.19+/5+ (`uname -r`). Run the following script, sourced from [BCC github](https://github.com/iovisor/bcc/blob/master/INSTALL.md#amazon-linux-2---binary), to enable BCC in Amazon Linux Extras, install the required `kernel-devel` package for the Kernel version and install the BCC tools.

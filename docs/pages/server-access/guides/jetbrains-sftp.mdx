...
JetBrain's IDEs, like PyCharm, Goland, Intellij, allow browsing, copying, and editing files on a remote server
using SFTP protocol. The built-in integration will simplify the process if you are used to copying files back and forth
using `scp` or other 3rd party clients. You can do the same things without leaving your IDE anymore.
...
- `Port` - is the Teleport Node's SSH port (3022 by default).

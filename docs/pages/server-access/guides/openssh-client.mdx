...
1. `tsh` always requires the `--proxy` flag because `tsh` needs to know which cluster you are connecting to. However, once you execute `tsh --proxy=xxx login`, the current proxy address will be saved in your `~/.tsh` profile and won't be needed for subsequent `tsh` commands.
2. `tsh ssh` operates *two* usernames: one for the cluster and another for the node you are trying to log into. (Read [User Identities](../../user-manual.mdx#user-identities) for more information.) For convenience, `tsh` assumes `$USER` for both by default. However, if you use `tsh login` before the `tsh ssh` command, your Teleport username will be stored in `~/.tsh` and will override this default.

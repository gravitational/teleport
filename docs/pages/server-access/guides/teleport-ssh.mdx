...
2. You'll be prompted to for the password and One Time Passcode we set up previously.
...
Teleport admin's can further define [Events](../../admin-guide.mdx#events) to organize and trigger sending session recordings to customizeable storage back-ends:

1. All `session.start` events could be stored on AWS S3.
2. All `exec` events could be stored in subdirectory on AWS S3 specific to remote commands.

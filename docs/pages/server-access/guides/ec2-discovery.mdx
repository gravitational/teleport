...
description: How to configure Teleport to install itself on EC2 instances.
---

Teleport can be configured to run a discovery service that will
connect to EC2 and automatically discover instances. It will then
execute an install script on these discovered instances using AWS
Systems Manger, that will install Teleport, start it and join the
cluster.

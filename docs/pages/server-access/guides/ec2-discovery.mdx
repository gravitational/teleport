...
Teleport can be configured to run a discovery service that will
connect to EC2 and automatically discover and enroll EC2 instances
matching configured labels. It will then execute an install script on
these discovered instances using AWS Systems Manager, that will install
...
- EC2 instances running Ubuntu/Debian/RHEL/Amazon Linux 2 if making use of the default Teleport install script.
- A running Teleport Cluster.

## Step 1/5. Create an EC2 invite token

When discovering EC2 nodes, Teleport makes use of IAM invite tokens for
...
More documentation on IAM invite tokens can be found [here](../../management/guides/joining-nodes-aws-iam.mdx)
...
In order for Teleport to send commands to EC2 instances, the node
...
The Discovery Service requires a valid auth token to connect to the cluster. Generate one by running the following command against your Teleport Auth Service and save it in `/tmp/token` on the node that will run the Discovery Service:
...
- `{{ .PublicProxyAddr }}`: the public address of the Teleport Proxy to connect to.
...
The default installer can be found [here](todo(amk):wait-for-pr-with-change-to-merge).

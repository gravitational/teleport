...
Teleport can run as a Discovery Node by configuring the discovery
service. This will connect to EC2 and automatically discover and
enroll EC2 instances matching configured labels. It will then execute
an install script on these discovered instances using AWS Systems
...
in order to receive commands from the Discovery Node. 
...
To customize the default installer script, execute the following command:
```code
...
version: v1
```
Which, when retreived by the Systems Manager document will evaluate to a script with the following contents:
```sh

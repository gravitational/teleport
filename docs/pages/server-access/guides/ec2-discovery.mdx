...
policies as well as permission to create Systems Manager documents.
- EC2 instances running Ubuntu/Debian/RHEL/Amazon Linux 2 if making use of the
default Teleport install script. (For other Linux distributions, you can
install Teleport manually.)

(!docs/pages/includes/tctl.mdx!)

## Step 1/5. Define an IAM policy
...
## Step 2/5. Install Teleport on the Discovery Node
...
## Step 3/5. Configure Teleport to discover EC2 instances

Assuming a discrete Node for the Discovery Service, the service requires a
valid auth token to connect to the cluster. Generate one by running the
...
## Step 4/5. Bootstrap the Discovery Service

On the same node as above, run `teleport discovery bootstrap`. This command
will generate and display the additional IAM policies and SSM documents
...
by Systems Manager can be found in the [AWS Cloud Operations & Migrations Blog

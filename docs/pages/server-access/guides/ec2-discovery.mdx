...
Here, `teleport.example.com` should be substituted with the public proxy URL of
the Teleport cluster, e.g. `mytenant.teleport.sh` for a Teleport Cloud tenant.
...
receive commands from the Discovery Service.
...
teleport cluster automatically.
...
Documentation on IAM invite tokens can be found  in our

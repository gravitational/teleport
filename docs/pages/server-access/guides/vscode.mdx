...
for the Remote - SSH extension. The setting `remote.SSH.useLocalServer` should be set to `false`.
...
Prior to connecting with a host set the `Remote.SSH: Use Local Server` setting to false in
the extension setting.

<Figure align="left" bordered caption="VSCode Setting">
![Window Indicator in bottom left corner of VS Code](../../../img/vscode/settings.png)
</Figure>
...
2. Clicking on the Window Indicator again and selecting either "Connect to...". 

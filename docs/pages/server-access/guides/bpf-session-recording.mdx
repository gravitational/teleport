...
session). Also, commands executed via daemons (systemd, crond, atd, etc.) could
be outside of the recorded session scope. Proper network-based restrictions for
ingress traffic must also be implemented to prevent possible unauthorized data
transfer. Additionally, certain forensic information such as full binary paths
(accounting for any potential symbolic links) and environment variables may not
be captured in session recordings.

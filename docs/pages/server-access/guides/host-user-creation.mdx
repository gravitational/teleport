...
automatic user creation will be disabled. Roles that do not match the Node will
not be checked.

If multiple roles match where one might specify `keep` and another `drop`,
teleport will default to `keep`, retaining the user on the node after the session
ends.

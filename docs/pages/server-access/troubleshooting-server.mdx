...
By default, when Teleport connects to a server, the service runs as the user login that starts 
the SSH session. If starting the session fails, it might be because the file system permissions
for the Teleport binary don't allow other user logins to connect to the remote server.
...
inormation, including an expiration time. For example:
...
approximately eight to ten minutes from the current time. If the time to expire is
...
a session in real time. You can define rules and configure role-based policies to control which users can join 
other users' sessions from `tsh` and the Teleport Web UI. If you are unable to join a shared session, you should

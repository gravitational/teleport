...
are considered of an "app" type, but it can be changed for a service by Kubernetes annotation [`teleport.dev/discovery-type`](reference.mdx).
...
[`teleport.dev/name`](reference.mdx).
...
Otherwise, this port is ignored. But if annotation [`teleport.dev/protocol`](reference.mdx) is used on the service and its value is

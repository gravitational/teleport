...
Okta-managed AWS Identity Center users & groups, to having them managed by Teleport
while preserving Okta as your AWS SSO provider.

This approach lets you get set up with a minimal Teleport AWS Identity Center
...
You can also use this setup if you don't want to migrate fully to using Teleport
as an Identity Provider for AWS Identity Center. Following the steps in this
guide will allow you to have Teleport control user and group AWS account
Assignments, while preserving Okta as AWS Identity Center's IdP.

## How it works

By default, the Teleport AWS Identity Center integration:
...
<Admonition type="note">
 For the integration to function properly, both AWS Identity Center and Teleport must view the
...
## Step 4/7. Install the Teleport Identity Center Integration.
...
This will install the Teleport AWS Identity Center integration with a
Teleport configuration that:
- controls the AWS Identity Center instance indicated by `--instance-arn`.
...
- provisions all users imported from Okta into AWS Identity Center (from the `--user-origin okta` flag).
...
## Step 5/7. Migrate Account Assignments
...
Once you are satisfied that an AWS Identity Center group has been migrated to 
...
Deleting the integration will automatically remove all of the Teleport resources 
used by the integration to manage its own state, as well as all of the Teleport
roles created by the integration used to grant AWS Identity Center account
assignments and Access Lists imported from AWS Identity Center Groups.

Any user-created Access Lists or account-assignment-granting roles will not be
deleted.

Any user-created Access List that grants an integration-created role, or any user
that has an integration-created role a=directly assigned to them will need
manual cleanup to remove any references to the deleted roles.

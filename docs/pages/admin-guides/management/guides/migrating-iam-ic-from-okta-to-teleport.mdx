...
By default, the Teleport AWS IAM Identity Center integration:
- Controls user and group provisioning into Identity Center,
- Controls AWS account permission assignment provisioning into Identity Center, and
- Authenticates user access to AWS console via Teleport SAML IdP.

This is fine for most deployments, but can cause difficulties when integrating Teleport with
an existing Okta-managed AWS IAM Identity Center configuration.

This guide introduces a hybrid setup and a migration path to integrate Teleport in
such an existing Okta-managed AWS IAM Identity Center configuration where Okta will
continue working as an SSO provider to AWS console while Teleport manages user, group
and AWS account permission assignment provisioning.
...
## Prerequisites
 ...
 - An AWS role configured as per the [Identity Center guide](./aws-iam-identity-center.mdx#step-16-configure-aws-integration)
...
## Step 5/7. Migrate AWS account permission assignments

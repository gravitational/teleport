...
- **Partial hand-off (Hybrid setup)**: Keep Okta as the primary SSO provider for AWS IAM Identity Center 
...
<Admonition type="note">
Teleport version 17.5.1 and above supports group re-import based on changes to `group_sync_filters` value.
In an older Teleport version 17.5.0 and below, you will need to uninstall and re-install the plugin 
in order to apply the updated `group_sync_filters` value. 
</Admonition>
...
In the previous steps, we ensured that the users and grops that were originally 
provisioned to AWS IAM Identity Center instance by Okta have now been synced to Teleport.
And Teleport is now responsible for provisioning users and groups to AWS IAM Identity 
Center instance. We also retired group provisioning in Okta.

At this point, it is safe to fully transfer the ownership of AWS IAM Identity Center instance
from Okta to Teleport. If you wish to proceed with full transfer, head over to [Getting Started with AWS IAM Identity Center integration](./guide.mdx)
that guides you through configuring Teleport as an identity source in AWS IAM Identity Center.

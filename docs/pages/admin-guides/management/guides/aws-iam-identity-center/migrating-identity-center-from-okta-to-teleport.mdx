...
- Controls AWS account assignment provisioning into Identity Center, and
...
and AWS account assignment provisioning.
...
- Uses the system AWS credentials to authenticate with AWS (from `--use-system-credentials`)
  and assumes IAM role indicated by `--assume-role-arn`.
...
A second alternative is to [uninstall](#deleting-the-aws-iam-identity-center-integration)

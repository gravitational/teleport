...
servers, desktops, or MFA devices. For Machine ID bots, this additionally
includes the join token name (for delegated join methods) and the bot instance
UUID. After you create a lock, all existing sessions where the lock applies are
terminated and new sessions are rejected while the lock remains in force.

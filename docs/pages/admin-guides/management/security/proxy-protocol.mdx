...
Users of Teleport Enterprise don't need to manage PROXY protocol setting manually. It's set to `on` automatically 
and can't be changed, since cloud Proxy/Auth Service instances managed by Teleport run behind a L4 load balancer with PROXY protocol configured. 
</Admonition>
...
setting. If there's a PROXY-enabled L4 load balancer between your Proxy and Auth Service instances, you should set `proxy_protocol: on` on the Auth,

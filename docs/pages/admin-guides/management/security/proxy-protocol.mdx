...
1. If you need to connect from an IPv6-only network to an IPv4-only network, you
   can downgrade the source address by enabling `proxy_protocol_allow_downgrade`.
   IP pinning is not allowed for downgraded connections due to the possibility of
   colliding source addresses.

<Admonition type="warning">

If you are upgrading Teleport in order to enable `proxy_protocol_allow_downgrade`,
you will need to upgrade all instances of Teleport Proxy Service before enabling it.
Versions that do not support this configuration will be unable to verify the PROXY
header signature and fail to accept downgraded connections.
...
The `PROXY` 
If you need to facilitate connections from IPv6 networks into IPv4 networks without dualstack networking, you can enable `proxy_protocol_allow_downgrade`.

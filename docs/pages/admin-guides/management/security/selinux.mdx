...
The Teleport SSH SELinux module allows SELinux to confine the Teleport SSH Service. It ensures that Teleport SSH processes can only perform explicitly allowed operations, reducing the attack surface and preventing unauthorized actions even if the Teleport binary were compromised.

...
The Teleport SSH SELinux module builds on SELinux by confining the Teleport SSH Service. It ensures that Teleport SSH processes can only perform explicitly allowed operations, reducing the attack surface and preventing unauthorized actions even if the Teleport binary were compromised.
...
## Enforcing Teleport SSH with SELinux
...
If you copy or move the binary the label will need to be updated. You simply re-run the install script to update the label, or you can update the label manually:

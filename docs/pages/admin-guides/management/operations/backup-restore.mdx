...
Teleport Enterprise manages all Auth Service and Proxy Service backups.
...
When migrating backends, you should back up your Auth Service instances
...
also apply to a new cluster being bootstrapped from the state of an old cluster:
  ...
  - Some user authentication mechanisms (e.g. WebAuthn) require that the public
    ...
    so be careful!
  - Any Node whose invite token is defined in the Auth Service instances configuration
...
In Teleport Enterprise, backend data is managed for you automatically. 

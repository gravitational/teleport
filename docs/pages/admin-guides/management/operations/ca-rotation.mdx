...
Various components of a Teleport cluster authenticate to one another using
either X.509 or SSH certificates. To verify certificates, Teleport maintains
several certificate authorities. You can rotate Teleport CAs to prevent
malicious actors from impersonating part of your Teleport cluster. This guide
explains the CAs that Teleport maintains and how to rotate them.
...
infrastructure and roll back the migration if necessary.
...
1. `update_servers`: Teleport servers (agents, Auth Service, and Proxy Service
   instances) reload and start serving TLS and SSH certificates signed by the
   new certificate authority, but still accept certificates issued by the
   original certificate authority. This only applies to the Teleport [`host`
   CA](#host).
...
CA has not lost connectivity. You may also need to export the new CA to that
...
|CA type|Traffic it authenticates|
|---|---|
|[`host`](#host)|Traffic between Teleport agents and Proxy Service instances and the Teleport Auth Service.|
|[`user`](#user)|Traffic from authenticated Teleport users. Client traffic from the Teleport SSH Service and OpenSSH clients to Teleport-protected servers. Client traffic from the Windows Desktop Service to Teleport-protected desktops.|
|[`db`](#db-and-db_client)|Server traffic from self-hosted databases protected by Teleport.|
|[`db_client`](#db-and-db_client)|Client traffic from the Teleport Database Service to self-hosted databases protected by Teleport.|
|[`openssh`](#openssh)|Client traffic to OpenSSH servers enrolled in your Teleport cluster.|
|[`jwt`](#jwt)|Traffic from the Teleport Application Service to applications that accept Teleport-issued JSON web tokens.|
|[`saml_idp`](#saml_idp)|Traffic from the Teleport SAML IdP to service providers.|
|[`oidc_idp`](#oidc_idp)|Traffic from the Teleport OIDC IdP integration, which authenticates to AWS for features like EICE EC2 Access, External Audit Storage, Auto-Discovery, and AWS Sync for Access Graph.|

### `host`

The `host` CA authenticates traffic between Teleport agents and Proxy Service
instances and the Teleport Auth Service.
...
Teleport issues certificates from the `user` CA when a user authenticates to
Teleport. Once you have completed the rotation and reached the final `standby`
phase, users who have signed into Teleport must reauthenticate to receive a user
...
authority. If you have registered Windows desktops with Teleport, [follow the
...
The `db` CA authenticates server traffic from self-hosted databases protected by
Teleport, while the `db_client` CA authenticates client traffic from the
Teleport Database Service to self-hosted databases.
...
The `openssh` CA authenticates client traffic to OpenSSH servers enrolled in
...
The `jwt` CA authenticates traffic from the Teleport Application Service to
applications that accept Teleport-issued JSON web tokens.
...
The `saml_idp` CA authenticates traffic from the Teleport SAML IdP to service
providers.
...
The `oidc_idp` CA authenticates traffic from the Teleport OIDC IdP integration,
which Teleport uses for features like EICE EC2 Access, External Audit Storage,
Auto-Discovery, and AWS Sync for Access Graph. There are two ways to serve the
public key for the Teleport `oidc_idp` certificate authority:
...
Initiate the `update_servers` phase. In this phase, Teleport servers (agents,
Auth Service, and Proxy Service instances) reload and start serving TLS and SSH
certificates signed by the new certificate authority, but still accept
...
<<<<<<< HEAD:docs/pages/admin-guides/management/operations/ca-rotation.mdx
...
work.
>>>>>>> ccf3be7c90 (Include all Teleport CAs in the rotation guide):docs/pages/management/operations/ca-rotation.mdx

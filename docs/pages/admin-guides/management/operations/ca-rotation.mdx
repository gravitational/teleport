...
guide](../../desktop-access/active-directory-manual.mdx) to export the Teleport
user CA so the Windows Desktop Service can authenticate to RDP hosts. Verify
that you can connect to registered desktops throughout the rotation.
...
The Teleport Proxy Service serves the public key for the Teleport `oidc_idp`
certificate authority from the `/.well-known/jwks-oidc` path of the Web API.

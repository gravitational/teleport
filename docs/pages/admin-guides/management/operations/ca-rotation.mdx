...
|CA type|Certificate subjects|Verified by|
|---|---|
|[`host`](#host)|Teleport Agents. Auth Service and Proxy Service instances.|Clients communicating with Teleport components.|
|[`user`](#user)|Teleport users. Teleport SSH Service and Windows Desktop Service instances.|Teleport-protected servers and Windows desktops.|
|[`db`](#db-and-db_client)|Self-hosted databases protected by Teleport (users must distribute certificates to databases).|The Teleport Database Service.|
|[`db_client`](#db-and-db_client)|The Teleport Database Service.|Self-hosted databases protected by Teleport.|
|[`openssh`](#openssh)|OpenSSH servers enrolled in your Teleport cluster.|Clients connecting to Teleport-protected OpenSSH servers.|
|[`jwt`](#jwt)|Teleport users|Applications that accept Teleport-issued JSON web tokens sent by the Teleport Application Service.|
|[`saml_idp`](#saml_idp)|The Teleport SAML IdP|Applications configured to use the Teleport SAML IdP for authentication.|
|[`oidc_idp`](#oidc_idp)|The Teleport OIDC IdP integration|AWS during authentication for features like EICE EC2 Access, External Audit Storage, Auto-Discovery, and AWS Sync for Access Graph.|
...
trust certificates issued by the CA. Self-hosted databases present a certificate
signed by the `db` CA, which the Database Service uses to verify that a database
server is a genuine Teleport-protected resource.

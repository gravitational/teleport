...
```code
# Disable Graph Explorer integration if your license supports Identity Security with --no-access-graph flag.
...
```code
# enable Graph Explorer integration if your license supports Identity Security.
$ tctl plugins install entraid \
    --default-owner=<Var name="Access List Owner"/> \
    --default-owner=someOtherOwner@teleport.sh  \

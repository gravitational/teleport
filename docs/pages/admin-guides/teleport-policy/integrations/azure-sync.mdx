...
## Step 1/2. Install and Configure the Discovery Service (Self-hosted only)
<Notice type="warning">
...
Discovery service. Two options for authorization exist:

1. [Managed identity attached to a VM](https://goteleport.com/docs/enroll-resources/auto-discovery/servers/azure-discovery/#attach-an-identity-to-your-azure-vm) - Use the managed identity ID as the principal ID in step 2 of integration command
2. [OIDC credentials via Teleport Integration](https://goteleport.com/docs/admin-guides/teleport-policy/integrations/entra-id/#step-23-configure-the-entra-id-integration) - Use the application object ID as the princial ID in step 2 of the integration command

When using OIDC credentials via Teleport Integration, add the name of the <Var name="integration" /> to the configuration, e.g. `entra-id`:

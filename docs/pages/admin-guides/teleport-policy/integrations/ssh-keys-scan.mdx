---
title: Discover insecure Access Patterns with Teleport Policy
description: Describes how to enable SSH Keys Scanning using Teleport Policy and Access Graph.
---

Organizations often struggle to maintain a secure environment due to the lack of visibility into access patterns.
Long-lived SSH keys, for example, can pose a significant security risk as they can be forgotten or mismanaged while
still providing non-auditable access to critical systems.
...
### Importing Node's SSH Authorized Keys

Teleport Cluster Nodes running the Teleport SSH Service continuously scan for SSH Authorized Keys present on the server.
The public fingerprint of these keys is sent to the Teleport Auth Service, which then pushes them to the Access Graph.
Together with the key fingerprint, the node also sends the following metadata:

- `public_fingerprint`: The SHA256 fingerprint of the public key.
- `host_id`: The unique identifier of the node.
...
by examining files with certain prefaces to identify SSH private keys.

The `tsh` tool authenticates with the Teleport Cluster through the [Device Trust](../device-trust.mdx) feature,
...
### Creating insecure access paths

Teleport Policy’s Access Graph feature delves into the Authorized Keys, Private Keys imported and Device Trust inventory,
crafting a graphical representation thereof.

Users will have new nodes in the Access Graph representing the devices they own. These nodes will be connected to
the private keys they have on their devices. The private keys will be connected to the authorized keys on the servers
they can access. This way, you can identify insecure access paths and take corrective actions.

## Prerequisites

- A running Teleport Enterprise cluster v15.3.X/v16.2.0 or later.
...
- A Teleport Node running the Teleport SSH Service.
- Devices enrolled in the [Teleport Device Trust feature](../device-trust.mdx).
...
Save the changes and exit the editor. The Teleport cluster will now start informing the Teleport SSH nodes to scan for
...
On devices enrolled in the Teleport, you can use the `tsh` CLI tool to scan for SSH Private Keys. Check [Device Trust](../device-trust.mdx)
...
- A host user connected to the Teleport node where it is registered.
...
process and periodically scan for SSH Private Keys. Check the [Jamf Pro Integration](../device-trust/jamf-pro-integration.mdx)
...
![Example of a recurring Jamf Pro](../../../../img/access-graph/jamfpro.png)
...
in the Teleport Device Trust feature. Check the [Device Trust](../device-trust.mdx) page for details on how to enroll devices
in Teleport.

...
description: Describes how to enable SSH Key Scanning using Teleport Policy and Graph Explorer.
---

With Teleport Policy's Graph Explorer, you can gain insights on how SSH keys are used within your environment. By scanning
...
Teleport and Teleport Policy's Graph Explorer synchronize various resources, including SSH authorized keys and private keys.
...
The public fingerprint of these keys is sent to the Teleport Auth Service, which then pushes them to the Graph Explorer.
...
- For self-hosted clusters:
  ...
  - A running Graph Explorer node v1.22.0 or later.
...
To enable the SSH Key Scanning feature, edit the Graph Explorer configuration file:
...
After a few minutes, you can navigate to the Graph Explorer page in the Teleport UI to view the imported SSH authorized keys
and local users.
...
fingerprints to the Teleport cluster. The Teleport cluster will then import the keys and display them in the Graph Explorer.
...
## Step 3/3. View Graph Explorer

Once the SSH authorized keys and private keys have been imported, you can view them on the Graph Explorer page.
...
### Graph Explorer: Dedicated `ssh_keys` SQL View

Starting in version **v1.25.0**, Graph Explorer introduces a dedicated `ssh_keys` SQL view for managing SSH access paths.
...
Cluster. The keys will be imported and displayed in the Graph Explorer.

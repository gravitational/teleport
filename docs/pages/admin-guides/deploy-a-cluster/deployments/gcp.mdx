...
Teleport's Proxy Servers and Auth Service instances using instance groups for each.
...
the teleport section of your Auth Service instances config file (by default it's `/etc/teleport.yaml`):
...
teleport:
  ...
  # We recommend using a TCP load balancer pointed to the Auth Service instances when
...
# enable the SSH Service and disable the Auth Service and Proxy Service Services
ssh_service:
  enabled: true
auth_service:
  enabled: false

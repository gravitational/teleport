...
Teleport's Proxy Servers and Auth Services using instance groups for each.
...
updates to keep individual Auth Services in sync, and requires Firestore configured
in native mode.
...
Save your license file on the Auth Services at the path,
...
teleport:
  ...
  # We recommend using a TCP load balancer pointed to the Auth Services when

...
We will use an AWS NLB for this highly available deployment architecture example.
The NLB forwards traffic from users and services to an available Teleport Proxy Service instance.
An AWS Application Load Balancer (ALB) can also be used in this architecture. Please reference our [guide for using an AWS ALB](https://goteleport.com/docs/reference/architecture/tls-routing/#working-with-layer-7-load-balancers-or-reverse-proxies) for this step if you wish to use an ALB.

...
{/*TODO:
This also will not work with trusted clusters.
*/}

## Multiple public addresses

{/*TODO: introduce the architecture of this approach*/}
...
1. Configure the Teleport Proxy Service to use multiple public address values:
   ...
   In this configuration, the external address appears first. {/*TODO: why?*/}
...
1. On Teleport Agent hosts, start the `teleport` process with the
   ...
   Proxy Service load balancer address.  {/*TODO: add instructions for Linux
   servers (systemd environment file) and the `teleport-kube-agent` chart*/}

   Setting `TELEPORT_TUNNEL_PUBLIC_ADDR` overrides the default behavior, in
   which an agent joins using the first public address listed, to use the value
   of the environment variable instead.

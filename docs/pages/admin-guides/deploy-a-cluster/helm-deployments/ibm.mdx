...
Postgres offers better scalability and is easier to operate, and supports both Teleport
...
In this step, we'll create the PostgreSQL database Telpeort will use as a backend.
...
- a `teleport` user that will own the `teleport` database and schema
  ...
  teleportConfig:
    teleport:
      storage:
        ...
        # the result should look like: "postgres://teleport@60d25e94-edf5-4494-a6cc-5fcae28aa7c2.c0v4phir0ah9ul9trho0.databases.appdomain.cloud:31766/teleport_backend?sslmode=verify-full"
        conn_string: postgres://teleport@<DATABASE ADDRESS>:<DATABASE PORT>/teleport_backend?sslmode=verify-full
        # replace the database address and port with the values you saved during the PostgreSQL creation
        change_feed_conn_string: postgresql://repl@<DATABASE ADDRESS>:<DATABASE PORT>/teleport_backend?sslmode=verify-full
        audit_events_uri:
        # replace the database address and port with the values you saved during the PostgreSQL creation
          - "postgres://teleport@<DATABASE ADDRESS>:<DATABASE PORT>/teleport_audit?sslmode=verify-full#disable_cleanup=false&retention_period=8766h"
    ...
    - name: AWS_SECRET_ACCESS_KEY
      valueFrom:
        secretKeyRef:
          ...
          optional: false
    - name:  PGSSLROOTCERT
      value: /var/lib/postgres-ca/cacert.pem
    - name: PGPASSWORD
      valueFrom:
        secretKeyRef:
          name: postgres-pass
...
teleport            LoadBalancer   172.21.130.11   45a6ca70-ca-tor.lb.appdomain.cloud   443:31421/TCP,3023:31979/TCP,3026:32071/TCP,3024:32700/TCP,3036:30541/TCP   2m50s
teleport-auth       ClusterIP      172.21.109.96   <none>                               3025/TCP,3026/TCP                                                           2m50s
teleport-auth-v16   ClusterIP      None            <none>                               <none>                                                                      2m50s
teleport-auth-v17   ClusterIP      None            <none>                               <none>                                                                      2m50s
```
...
- create a new Teleport user:
  ```code
  $ kubectl exec -it -n teleport-cluster deployment/teleport-auth -- tctl users add myusername --roles=editor,auditor,access

  User "myusername" has been created but requires a password. Share this URL with the user to complete user setup, link is valid for 1h:
...
- enroll [servers](../../enroll-resources/server-access/introduction.mdx),
  ...
  [databases](../../enroll-resources/database-access/database-access/database-access.mdx),

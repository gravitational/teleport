...
The Identity Activity Center in Teleport Identity Security requires deploying the Access Graph
service and provisioning AWS infrastructure, including:
...
This guide walks you through setting up the Access Graph service and the required infrastructure.


Identity Activity Center is a feature of the [Identity Security](https://goteleport.com/platform/policy/) product that is only available
...
## Step 1/5. Install Identity Security

<Tabs>
<TabItem label="Self-hosted cluster">

To install the base Identity Security components for self-hosted clusters, follow the [Install Identity Security](./self-hosted.mdx) guide.

</TabItem>

<TabItem label="Self-hosted cluster using Helm Chart">

To install using Helm, follow the [Install Identity Security using Helm](./self-hosted-helm.mdx) guide.
...
Teleport Identity Security uses an SQS queue to publish and consume audit logs.
...
- <Var name="example_sqs_queue"/> is the name of the AWS SQS queue.
- <Var name="example_sqs_dlq"/> is the name of the AWS SQS dead-letter queue.
- <Var name="example_kms_key"/> is the name of the AWS KMS encryption key used to encrypt S3 bucket files and SQS messages.
...
sqs_queue_name        = <Var name="example_sqs_queue"/>
sqs_dlq_name          = <Var name="example_sqs_dlq"/>
kms_key_alias         = <Var name="example_kms_key"/>
long_term_bucket_name = <Var name="example-long-term-bucket"/>
transient_bucket_name = <Var name="example-transient-bucket"/>
database_name         = <Var name="example-db"/>
table_name            = <Var name="example-table"/>
...
##  Step 3/5. Configure MaxMind GeoIP City database (optional)
...
To enhance the audit events with GeoIP information such as country, city, region, latitude,
and longitude based on IP address, we highly recommend the usage of the [MaxMind GeoIP City database](https://www.maxmind.com/en/geoip-databases).

<Tabs>
<TabItem label="Self-hosted cluster" >

To configure enhanced location details for self-hosted clusters, upload the database file to the machine where
Identity Security runs.
...
## Step 4/5. Modify Identity Security configuration
...
identity_activity_center:
  ...
  sqs_queue_url: https://.../tiago-sqs-test2
...
## Step 5/5. Restart Identity Security

The last step is to restart the Identity Security docker process or restart the process

...
(CAs) during the first Auth Service's initial startup.
...
Each Teleport Auth Service will need to be configured to use a GCP key ring
which will hold all keys generated and used by that Auth Service.
If running a High-Availability Teleport cluster with two or more Auth Services,
every Auth Service can be configured to use the same key ring, or if desired each
...
for the key ring which is geographically near to your Teleport Auth Services.
...
## Step 3/5. Provide the service account credentials to your Auth Service
...
## Step 4/5. Configure your Auth Service to use KMS keys

CA key parameters are statically configured in the `teleport.yaml` configuration
file of the Teleport Auth Service(s) in your cluster.

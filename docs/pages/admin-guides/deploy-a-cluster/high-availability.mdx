...
- A **private Layer 4 load balancer** to direct traffic from the Proxy Service
  to the Auth Service instances gRPC API, which is how Teleport manages the Auth
...
- **Auth Service load balancer:** A load balancer to forward traffic from a
  ...
  traffic to the Auth Service instances gRPC endpoint.
...
The Auth Service load balancer must forward traffic to the Auth Service instances gRPC

...
1. In the `agent_installation_dev` and `agent_installation_prod` modules, leave
   `agent_count` to `1`, or assign to `2` for high availability. Each of these
   modules deploys a number of Linux servers equal to `agent_count`, labels them
   with the key/value pairs specified in `agent_labels`, and deploys them by
   passing userdata sdcripts to the `agent_deployment` module. 

   As you scale your Teleport usage, you can increase this count to ease the
   load on each Agent. 

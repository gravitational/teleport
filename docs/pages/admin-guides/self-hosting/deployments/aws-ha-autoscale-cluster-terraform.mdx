...
`examples/aws/terraform/ha-autoscale-cluster` section of the Teleport repo](https://github.com/gravitational/teleport/blob/branch/v(=teleport.major_version=)/examples/aws/terraform/ha-autoscale-cluster/README.md)
...
This license will be uploaded to AWS SSM and automatically downloaded to Teleport Auth Service instances in order to enable
...
See the [Teleport authentication reference](../../../../reference/operations/authentication.mdx) for more information.
...
More information about how Teleport works with DynamoDB can be found in our [Storage Backends guide](../../../../reference/operations/backends.mdx).
...
by the [`route53_domain`](#route53\_domain) variable. In our example this would be `<Var name="teleport.example.com" />`
...
<Tabs>
  ...
  - The SSH interface of the Teleport Proxy Service will be available via a network load balancer with an AWS-controlled hostname on port 3023.
  This is the default port used when connecting with the `tsh` client and will not require any additional configuration.
  - The reverse tunnel listener of the Teleport Proxy Service will be available via the same network load balancer with
  an AWS-controlled hostname on port 3024. This allows trusted clusters and nodes
  connected via reverse tunnel to access the cluster.
  - The Kubernetes listener of the Teleport Proxy Service will be available via the same network load balancer with
  an AWS-controlled hostname on port 3026. This allows Kubernetes clients to access Kubernetes clusters via the Teleport cluster.
  - If the [MongoDB listener port is enabled](#enable_mongodb_listener), it will be available via the same network load balancer with
  an AWS-controlled hostname on port 27017. This allows clients to connect to MongoDB databases registered with the Teleport cluster.
  - If the [MySQL listener port is enabled](#enable_mysql_listener), it will be available via the same network load balancer with
  an AWS-controlled hostname on port 3036. This allows clients to connect to MySQL databases registered with the Teleport cluster.
  - If the [Postgres listener port is enabled](#enable_postgres_listener), it will be available via the same network load balancer with
  an AWS-controlled hostname on port 5432. This allows clients to connect to Postgres databases registered with the Teleport cluster.

  After deploying, you can get the hostname of the Teleport Proxy Service network load balancer if needed with this command:
...
Teleport's Auth Service listener will be available via an internal load balancer with an AWS-controlled hostname on port 3025.

After deploying, you can get the hostname of the internal Auth Service load balancer if needed with this command:
...
To add users to the Teleport cluster, you will need to connect to a Teleport Auth Service instance via SSH and run the `tctl` command.

1. Get the public IP of the bastion server using `terraform output -raw bastion_ip_public`:
...
1. Get the IP of an Auth Service instance by using `terraform output -json auth_instance_private_ips` and piping to `awk` to extract the first IP.
...
1. Use both these values to SSH into the Auth Service instance. Make sure that the AWS keypair that you specified in the
...
Remember that there is more than one Teleport Auth Service instance in an High Availability deployment. You should use this command to get the IP addresses
of each Auth Service instance that you'll need to connect to:
...
You can run `tctl` commands on **any** of the Teleport Auth Service instances connected to your cluster, however.

### Teleport Proxy Service

The `systemd` service name for the Teleport Proxy Service is different between Let's Encrypt (`teleport-proxy.service`) and ACM (`teleport-proxy-acm.service`).
...
Remember that there is more than one Teleport Proxy Service instance in an High Availability deployment. You can use this command to get the IP addresses
of each proxy instance that you'd need to connect to for checking logs:

```code
$ terraform output -json proxy_instance_private_ips
["172.31.2.221","172.31.3.12"]
```
...
You can get detailed logs for the Teleport SSH Service using the `journalctl` command:
...
Customers run many workloads within AWS and depending on how you work, there are many
ways to integrate Teleport onto your servers. We recommend looking at our [Installation guide](../../../../get-started/download-install/installation.mdx).

To add new nodes/EC2 servers that you can "SSH into" you'll need to:

- [Install the Teleport binary on the Server](../../../../get-started/download-install/installation.mdx)
- [Run Teleport - we recommend using systemd](../../../common-operations/daemon.mdx)
- [Set the correct settings in /etc/teleport.yaml](../../../../reference/config-references/config.mdx)
- [Add Nodes to the Teleport
  cluster](../../../enroll-resources/agents/join-services-to-your-cluster/overview.mdx)
...
$ aws ssm get-parameter --region <Var name="us-west-2" /> --name "/teleport/${CLUSTER_NAME}/tokens/node" --query "Parameter.Value" --with-decryption --output text --no-cli-pager
...
```yaml
spec:
  web_proxy_addr: <Var name="teleport.example.com" />:443
```

You can generate a token for adding the trusted cluster using `tctl tokens add --type=trusted_cluster` after connecting
to a Teleport Auth Service instance. Follow the instructions in our [Trusted Clusters guide](../../../access-controls/trustedclusters.mdx).

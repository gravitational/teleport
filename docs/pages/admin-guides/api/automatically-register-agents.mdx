...
Auth Service instances gRPC endpoint.
...
|`github.com/gravitational/teleport/api/client`|A library for authenticating to the Auth Service instances gRPC API and making requests, aliased as `teleport`.|
...
|---|---|
|`proxyAddr`|The host and port of the Teleport Proxy Service, e.g., `example.teleport.sh:443`, which we will use to connect the client to your cluster. **Assign this to your own Proxy Service's host and port.**|

...
Request events from the Teleport Auth Service instances gRPC API, and use the data to
...
connecting to the Teleport Auth Service instances gRPC endpoint over TLS.
...
|Package|Description|
|---|---|
|`github.com/gravitational/teleport/api/client`|A library for authenticating to the Auth Service instances gRPC API and making requests.|
...
Teleport Enterprise tenant. Assign it to the address of your own Proxy
Service, e.g., `example.teleport.sh:443`.

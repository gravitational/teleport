...
Edit the `teleport.yaml` files for all Auth Service instances:
...
- Statically using the Teleport configuration file. This is the simplest
  option, but is not possible for Teleport Enterprise (Cloud) clusters and not recommended
...
<TabItem label="Static Config" options="Self-Hosted">
Edit the Auth Service instance's `teleport.yaml` file and restart:

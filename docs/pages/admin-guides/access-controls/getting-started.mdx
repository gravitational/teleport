...
Interactive and non-interactive users (bots) can assume one or many roles. Roles govern access to databases, SSH servers, Kubernetes clusters, web services and applications and Windows Desktops.
...
Internal users can authenticate using credentials managed within Teleport, such as:
    ...
    - Multi-factor authentication (2FA), such as TOTP (time-based one-time passwords) or U2F (universal second factor).

  Internal users are assigned roles that define what resources they can access and what actions they can perform within the Teleport cluster.
  They are typically used for small to mid-size deployments where user management is handled directly in Teleport without relying on external identity providers.
...
External users authenticate using external Identity Providers (IdP) like:
    - Okta, Google, GitHub, Microsoft Entra ID, or other SAML/OIDC providers.

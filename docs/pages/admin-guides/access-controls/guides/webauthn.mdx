...
be registered"/> with the serial number of the device you wish to enroll and
<Var name="macos" /> with the device's operating system (`linux`, `macos`, or
`windows`). Run the `tctl devices add` command:

...
- For the `tsh db exec` command, only WebAuthn devices are supported. A single
  MFA check can be reused for multiple connections within a `tsh db exec`
  command, but only up to 5 minutes. After the 5-minutes window, a new MFA check
  will be requested for new connections.

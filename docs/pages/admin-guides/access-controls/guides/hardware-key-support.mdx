...
Teleport Connect provides the best UX for hardware key support with its UI elements for
touch and PIN prompts. As a result, we have added the capability for Teleport Connect to
act as a Hardware Key Agent, making hardware key requests on behalf of Teleport clients.

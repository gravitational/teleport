...
- a Teleport [RBAC](../../reference/rbac/teleport-roles.mdx) role by the role's name
- a Teleport [trusted device](
  ../authentication/device-trust/enforcing-device-trust.mdx#locking-a-device) by the device ID
...
- an [Access Request](just-in-time/access-requests/overview.mdx) by UUID
...
as per usual. See the [Admin Guide](../../reference/config-references/resources.mdx) for more
details.
</Details>

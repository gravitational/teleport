...
--attributes-to-roles groups,<>,access > keycloak-connector.yaml
```
...
- Verify that the public certificate used by Keycloak for signing SAML responses 
is correctly configured and valid.
- Ensure that the certificate is up-to-date and the private key is properly paired with it. 
  Refer to the **Client Certificate Signature validation** section for more information. 
- Once the above has been verified, temporarily add the `spec.provider: ping` parameter 
to the Keycloak auth connector to match Keycloak’s expectations.

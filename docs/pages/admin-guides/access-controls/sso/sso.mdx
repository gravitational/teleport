...
login screen prompts the user for their username. Teleport will use this username to identify the user,
and then only display the auth connector(s) that are relevant to them.

This feature can be enabled on a cluster by configuring any one or more SAML, OIDC, or
GitHub auth connector with a `spec.user_matchers` field, which takes a set of one or more
...
Once enabled, any users without an auth connector mapped to them will not be able to see
any auth connectors on the login page. For this reason, it is recommended to configure
one or more fallback connectors. A user matcher of `*` is a special case that marks the

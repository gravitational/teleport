...
Identifier-first login is a feature that simplifies the login experience in clusters with multiple authentication connectors. When identifier-first login
is enabled, the initial login screen only promps the user for their username. After entering their username, the user is only presented with the connector(s) that apply to them.

Any auth connector can be configured for identifier-first login by specifying the `spec.user_matchers` field with a set of one or more user
...
##### Match users that weren't matched to any other connectors, as well as any user with a username that ends in "@example.com", even if they also match other auth connectors.

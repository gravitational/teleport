...
- At least two groups in GitLab with users assigned. In our examples below,
  we'll assume a group named `company` with two subgroups, `admin` and `access`.
...
--claims-to-roles groups,company/access,access > oidc.yaml
```
...
This example maps the two subgroups `admin` and `access` of the parent group
`company` to the `admin` and `access` roles in Teleport, and creates the `oidc.yaml` file:
...
root and is capable of administrating the cluster and non-privileged access.

...
### Consume arbitrary JSON claims from an OIDC provider with a JSONPath expression

In some niche scenarios, an OIDC provider may return claims as arbitrary JSON
objects rather than the typical string values and lists usually seen. For
example, imagine a custom distributed IdP that aggregates claims for a user from
multiple different provider sources. The claims set might look like this:
...
A JSONPath expression can be used to interpolate standard OIDC claims from
the original JSON claims.
...
In addition to being stored as traits, the JSONPath interpolated claims can
then be used in the OIDC connector's `claims_to_roles` mappings.

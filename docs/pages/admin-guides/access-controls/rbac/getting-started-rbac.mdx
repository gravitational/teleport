...
description: Get started using Teleport Role-Based Access Control.
---
...
Users—whether they are local, signed in via Single Sign-On (SSO), or robots—can be assigned one or more roles. 
Each role defines specific permissions to ensure that users only have access to what they need. 

## What is a role?

In Teleport, a role defines the permissions and access controls assigned to users or groups of users. Roles determine what resources a user can access and what actions they can perform within the system. 
...
Interactive and non-interactive users (bots, defined below) can assume one or many roles. Roles govern access to your entire infrastructure, such as databases, SSH servers, Kubernetes clusters, web services and applications and Windows Desktops.

## Teleport users

In Teleport, users are entities that can authenticate and interact with the resources within a Teleport cluster. These users can have different levels of access depending on the roles assigned to them. 
...
External users are authenticated via third-party identity providers, such as Single Sign-On (SSO) platforms, through protocols like SAML or OAuth (OIDC). These are often referred to as SSO users.

External users authenticate using external Identity Providers (IdP), such as Okta, Google, GitHub, Microsoft Entra ID, or other SAML/OIDC providers.
These users do not manage their passwords or credentials within Teleport itself. Instead, they authenticate using their external SSO credentials.

Instead of manually assigning roles in Teleport, external users’ roles are mapped based on attributes from the identity provider (e.g., groups in Okta or teams in GitHub). 
For example, a user in the "admin" group in Okta can be mapped to the admin role in Teleport. External users are useful for larger organizations or those that want to centralize 
identity management via an existing SSO provider. This is often preferred for enterprise environments where identity management is handled outside Teleport.

### Bots

Machine ID provides machines with an identity that can authenticate to the Teleport cluster. This identity is known as a bot. Bots are represented in Teleport by a user and a role resource and can 
be created via the tctl bots add command. Unlike human users, who use a password, MFA, or SSO, bot users join the cluster as Teleport services using a join method. 
They can still join even if local auth is disabled.

See the [Machine ID introduction](../../../enroll-resources/machine-id/introduction.mdx for more information.

## Teleport roles

Roles in Teleport define the permissions and access controls for users within a Teleport cluster. They determine what resources (like SSH servers, databases, Kubernetes clusters, web apps, and desktops) 
a user can access and what actions they can perform on those resources.

Roles control access to specific resources and functions, such as viewing logs, editing configurations, or accessing specific servers, 
and are used for internal users (created directly in Teleport) and external users (authenticated via Single Sign-On, like Okta or GitHub).
Users can be assigned one or more roles, and the permissions of multiple roles combine to form a user’s overall capabilities.

Admins create and assign roles to manage who can access which parts of the infrastructure.
...
<Tabs>
<TabItem scope={["Teleport Community Edition"]} label="Teleport Community Edition">
...
Follow our [SAML Okta Guide](../../../admin-guides/access-controls/sso/okta.mdx) to

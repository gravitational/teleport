...
so that users can access Azure Portal and CLI by authenticating with Teleport.
...
When you create an identity provider in Microsoft Entra ID, you need to associate a 
domain name with the identity provider so the user accounts that belongs to the domain 
are federated to the associated identity provider for authentication.
...
you must update the DNS `TXT` record of the domain. Refer to the table below. 
...
| example.com | alice@example.com | https://mytenant.teleport.sh| Domain can be associated but DNS update is required. |

In the second example where the DNS update is required, you need to update `TXT` record of the `example.com` so that 
the `DirectFedAuthUrl` value of the `TXT` record points to SAML IdP SSO URL of `mytenant.teleport.sh` cluster.  
```
...
As a first step, the Web UI guides you through steps of adding 
Teleport as an identity provider in Microsoft Entra ID.
...
Once you add Teleport as SAML IdP in Microsoft Entra External ID, you will need to copy Entra ID `Tenant ID` from
Azure portal and provide the value to the configuration UI and move to the next step. 

As a second step of the service provider configuration, you will notice that the Web UI already configures 
...
In this step, we will create and invite an external user that authenticates with Teleport SAML IdP.
...
Fill up details for this user as needed to set up the account. You will notice the a default value for
"Invite redirect URL" field that points to `https://myapplications.microsoft.com/` is set for this user. This URL is where Microsoft will 
the user after their first authentication. You can update this URL if you want to redirect user to different URL specific 
to Azure portal.
Ensure that the default user type "Guest" remains unchanged. 

Click the `Review + invite` button to invite this user. 
...
By default, there aren't any permissions granted to the invited users. The general process to grant roles is 
to create a [Azure Resource Group](https://learn.microsoft.com/en-us/azure/azure-resource-manager/management/manage-resource-groups-portal#what-is-a-resource-group)
that maps Azure roles with users and groups.
...
User access logs can be found in both Teleport Web UI and Azure portal.

In Teleport, each user authentication with SAML IdP is logged and the event can be found in the audit log.
```json
{
  "cluster_name": "mytenant.teleport.sh",
  "code": "TSI000I",
  "ei": 0,
  "event": "saml.idp.auth",
...
### Configuring an existing Microsoft Entra ID users to to authenticate with Teleport

To configure an existing Microsoft Entra ID user to authenticate with Teleport, they must be converted to an "external" user.

First open user profile in Azure portal. 

![Entra ID convert internal user to external user](../../../../img/access-controls/saml-idp/ms-entra-external-id/entra-convert-user.png)

In the user profile, under the "B2B invitation" tile, click on a link that states "Convert to external user". In the following 
UI that opens after clicking the link, you will be able to convert this user to an external user.

Note that you cannot convert an already federated user account to authenticate with Teleport. This restriction is placed by 
Microsoft Entra ID because in this situation, the user account domain is already associated with another identity provider.

Also note that the user email cannot contain a domain that is already configured in Microsoft Entra ID, and that domain is not 
assigned to the Teleport SAML IdP. This restriction applies when inviting new "external" users too. See Step 1 of 
this guide to understand association of a domain with the IdP.
...
### External user collaboration setting

You can configure who can invite external users, what resources they have access to by default 
and self-service sign up to application configured in Azure tenant and account removal.

These configuration options can be found in Azure portal under **External Identities > External collaboration settings**.

### Associating multiple domain names
...
Note that the rules of DNS update applies here as well. 

...
the `DirectFedAuthUrl` value of the `TXT` record points to SAML IdP SSO URL of Teleport cluster `mytenant.teleport.sh`.  
```
...
Fill up details for this user as needed to set up the account. You will notice that the a default value for
"Invite redirect URL" field that points to `https://myapplications.microsoft.com/` is set for this user. 
...
First, in Azure portal, select **Groups** menu under Azure services.
...
resource under `teleport-dev-vm-pool` resource group with the privilege granted by the "Virtual Machine Contributor" role.
...
User access logs can be found in both Teleport Web UI and in Azure portal.
...
Refer to official [Microsoft Entra External ID SAML federation](https://learn.microsoft.com/en-us/entra/external-id/direct-federation) docs for general configuration.

### Issues related to user invitation and user type conversion
For issues related to user invitation, or internal to external user type conversion, please refer to [Microsoft Entra B2B collaboration](https://learn.microsoft.com/en-us/entra/external-id/troubleshoot) troubleshooting docs.

...
infrastructure they need to resolve incidents quickly-without longstanding admin
...
and approve or deny the requests via Teleport. You can also configure the plugin
to approve Role Access Requests automatically for select on-call users.
...
For users that are on-call for the "Teleport Access" team, we will configure the
Datadog Incident Management plugin to approve Access Requests automatically.

## Step 2/10. Define RBAC resources
...
## Step 6/10. Generate Datadog credentials
...
## Step 7/10. Configure the Teleport Datadog Incident Management plugin
...
value configures the Datadog users or teams to notify. The value can be a single
string or an array of strings. Each string must be either the name of a Datadog team
or the email address of a Datadog user.
...
## Step 8/10. Test your Datadog Incident Management plugin
...
## Step 9/10. Configure Automatic approvals

To enable access request automatic approvals, you will need to set up an on-call
schedule for your Datadog team. Navigate to **On-Call -> Teams** and click on
**Set Up Team** to set up an on-call schedule for the "Teleport Access" team.

Then, configure the Teleport `editor-requester` role and add the `datadog_auto_approvals`
annotation. This annotation expects a list of Datadog team handle names that allow
access request automatic approvals for that team. Add the "teleport-access" team
to the annotation.
...
You will also need to configure the `access-plugin` role to allow the role to
...
spec:
  allow:
    ...
    review_requests:
      ...
      roles: ['editor']
      where: 'contains(request.system_annotations["datadog_auto_approvals"], "teleport-access")
```

The `allow.review_requests.roles` field is allowing the plugin to only review requests
for the `editor` role. The role is further restricted to only reviewing requests
that contain the annotation `datadog_auto_approvals` with a value of `teleport-access`.

You will need to refresh the access plugin identity after modifying the role.

### Trigger an auto-approval

To trigger an auto-approval, login to Teleport as the current on-call user in Datadog
and create an Access Request for the `editor` role.

You will see a log similar to the following on your Datadog Incident Management
plugin host:

```
INFO   Successfully submitted a request approval
datadog_user_email:myuser@example.com datadog_user_name:My User
request_id:00000000-0000-0000-0000-000000000000 request_op:put
request_state:PENDING datadog/app.go:511
```

You Access Request will appear as `APPROVED`:
```code
$ tsh requests ls
ID                                   User               Roles     Created (UTC)       Status
------------------------------------ ------------------ --------- ------------------- --------
00000000-0000-0000-0000-000000000000 myuser@example.com editor    12 Aug 22 18:30 UTC APPROVED
```

## Step 10/10. Set up systemd

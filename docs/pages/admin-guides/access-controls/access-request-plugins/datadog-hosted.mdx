...
annotations on each Teleport Zero Trust Access Request and, depending on the annotations,
...
create a new Datadog team. Name it "Zero Trust Access".

We will configure the Datadog Incident Management plugin to route certain access
requests to the "Zero Trust Access" team.
...
and name it "Zero Trust Access". Copy the API key to paste in a later step.
...
on **New Service Account** and name it "Zero Trust Access". You can provide whatever
...
manage the Zero Trust Access Request incidents.

![Datadog dashboard for Service Accounts](../../../../img/enterprise/plugins/datadog/dashboard-service-accounts.png)

Now click on the "Zero Trust Access" service account that was just created and click
on the **Application Keys** tab and click on **New Key** and name it "Zero Trust Access"
...
select **Add Team to On-Call** to set up an on-call schedule for the "Zero Trust Access"
team.

...
- `Teleport Zero Trust Access Request Notifications`
- `My Critical Service`
...
- A role called `editor-requester`, which can request the built-in `editor`
  ...
  user requests it, notifying the on-call team for the `Zero Trust Access Request
    ...
    request:
      ...
      thresholds:
        - approve: 1
          deny: 1
      annotations:
        pagerduty_notify_service: ["Zero Trust Access Request Notifications"]
```
...
PagerDuty to open an incident in the specified service, `Zero Trust Access Request
...
# INFO   Starting Zero Trust Access PagerDuty extension 0.1.0-dev.1: pagerduty/main.go:124
...
pd_service_name:Zero Trust Access Request Notifications

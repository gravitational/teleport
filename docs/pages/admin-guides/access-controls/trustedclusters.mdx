...
As you learned in [Core Concepts](../../get-started/core-concepts.mdx), a Teleport cluster
...
through an external identity provider, see [Configure Single Sign-on](../authentication/sso.mdx).
...
- The `tctl` admin tool and `tsh` client tool version >= (=teleport.version=).
  ...
  For more information about installing Teleport, see [Installation](../../get-started/download-install/installation.mdx).
...
- A Teleport SSH server that is joined to the cluster you plan to use as the **leaf cluster**.
  ...
  [Join Services to your Cluster](../enroll-resources/agents/join-services-to-your-cluster/overview.mdx).
...
Reference](../../reference/rbac/teleport-roles.mdx).

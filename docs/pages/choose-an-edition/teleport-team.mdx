...
1. Open a browser and go to the address for your Teleport Team cluster, 
<Var name="cluster-url"/>`.teleport.sh` (for example, `mycluster.teleport.sh`).
...
of your cluster and the name of your Teleport user:
   ...
   ```code
   $ tsh login --proxy=<Var name="cluster-url" />.teleport.sh --user=<Var name="username" />
   ```
   ...
   ```code
   > Profile URL:      https://cluster-url.teleport.sh:443
   Logged in as:       admin@teleport.example.com
   Cluster:            cluster-url.teleport.sh

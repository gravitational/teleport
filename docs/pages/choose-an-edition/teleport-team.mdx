...
4. Type _server_ in the search box to filter the list of resources.
...
7. Copy the command to run the installation script from the Web UI and paste it 
...
1. Click **Start Session** to start an interactive session.
   ...
   your container's shell with **Ctrl-C**, then run `teleport start` to restart the process.
...
2. Sign in to your Teleport Team tenant by running the `tsh login` command  with the URL 
...
- [Access Controls for Servers](../server-access/rbac.mdx#role-configuration)

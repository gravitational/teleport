...
1. if you are using the `teleport-kube-agent` Helm chart, wipe out existing state and recycle the Kubernetes pod to
   rejoin a Kubernetes agent to a new cluster This will cause the agents to re-register with

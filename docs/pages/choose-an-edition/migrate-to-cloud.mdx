...
- The `tsh` and `tctl` client tools. This guide assumes that you are using
  ...
  Terraform provider](../admin-guides/configure-teleport/dynamic-resources/terraform-provider.mdx) and
  [Kubernetes
  operator](../admin-guides/configure-teleport/dynamic-resources/teleport-operator-standalone.mdx), in
  addition to custom scripts that use the [Teleport API](../index.mdx)
...
Review the [dynamic resources](../reference/config-references/resources.mdx)
...
1. For each agent and Machine ID bot, obtain a valid join token. We recommend
   ...
   method](../reference/operations/join-methods.mdx).
...
read the [full documentation](../admin-guides/enroll-resources/machine-id/deployment/overview.mdx) on deploying a
Machine ID Bot.
...
on:
- [Access Request plugins](../index.mdx)
- The [Teleport Event Handler](../index.mdx)

...
Teleport Enterprise (cloud-hosted) is PCI DSS (Payment Card Industry Data
Security Standard) compliant. The most recent PCI certificate can be viewed at
[trust.goteleport.com](https://trust.goteleport.com).

Teleport Enterprise (cloud-hosted) can be used to meet multiple security
requirements for PCI environments and provides customers details on how to
secure their Cardholder Data Environment when using Teleport Enterprise
(cloud-hosted). This includes a shared responsibilities matrix that outlines
Teleport's, the customer's, and shared responsibilities.

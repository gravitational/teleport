...
### Is Teleport Enterprise Cloud PCI compliant?

Teleport Enterprise Cloud is PCI DSS (Payment Card Industry Data Security Standard)
compliant. The most recent PCI certificate can be viewed at
[trust.goteleport.com](https://trust.goteleport.com).

Teleport Enterprise Cloud can be used to meet multiple security requirements for PCI
environments and provides customers details on how to secure their Cardholder
Data Environment when using Teleport Enterprise Cloud. This includes a shared
responsibilities matrix that outlines Teleport's, the customer's, and shared
responsibilities.

Additionally, Teleport takes reasonable steps to identify and remove sensitive
data that could be inadvertently stored or transmitted on systems it controls.
Any cardholder data received from a customer is considered an "unintended
channel."

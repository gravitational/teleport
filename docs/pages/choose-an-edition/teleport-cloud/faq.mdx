...
Teleport Team and Teleport Enterprise Cloud are upgraded after each Teleport release
is delivered. You can see the schedule for Teleport releases on [Teleport Upcoming
Releases](../../upcoming-releases.mdx).

- Major version upgrades for the Teleport Auth Service and Teleport Proxy Service on 
  Teleport Cloud occur after the first minor release (for example (=teleport.major_version=).1.0) 
  of Teleport.
...
For major version upgrades (for example, 12.x to 13.x), your Teleport account is NOT 
automatically upgraded if any Teleport agents connected to the cluster are not compatible 
(more than one major version behind). This version check is done to ensure backwards compatibility. 
...
If you want to proceed with an upgrade regardless of this compatibility check, contact 
[Teleport support](https://goteleport.com/support/).

### Are upgrades times configurable for Teleport Enterprise Cloud?
...
To set the window start time, select your username in the Teleport Web UI, then 
click **Help & Support**. Under Scheduled Upgrades, you can see the current
maintenance window start time. Click **Edit** to set a different start
...
Typically, Teleport agents are automatically updated from one to a few days after 
your Teleport cluster is upgraded during a scheduled maintenance period.

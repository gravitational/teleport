...
Teleport Enterprise Cloud is deployed within AWS, using S3 and DynamoDB for storage with AWS-managed encryption keys.
See the [available regions](./architecture.mdx#auth-service) for the DynamoDB. [External Audit Storage](./external-audit-storage.mdx) allows
for optionally storing audit events and session recordings on infrastructure in any AWS account.

...
Teleport Team and Teleport Enterprise Cloud follow the Teleport [Upcoming
Releases](../../upcoming-releases.mdx) timelines.

- Major version upgrades for cloud Teleport Auth Service and Teleport Proxy Service occur 
  after the first minor release (for example (=teleport.major_version=).1.0) of Teleport.
...
<Admonition type="note" title="Minimum agent version">
    For major version upgrades (for example, 12.x to 13.x), your Teleport tenant is NOT 
    automatically upgraded if any Teleport agents connected to the cluster are not compatible 
    (> 1 major version behind). This version check is done to ensure backwards compatibility. 
    To get version information for your agents, see [here](#how-can-i-find-version-information-on-connected-agents).

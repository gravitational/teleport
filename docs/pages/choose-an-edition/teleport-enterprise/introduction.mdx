...
| [Access Requests](../../access-controls/access-requests.mdx) | Request access to specific resources and a user interface for teams to create and review requests to access infrastructure with escalated privileges. |
...
With Teleport users can request access to specific resources and roles with requests publishable to tools such as Slack, Jira, PagerDuty that fits to your organization's workflows. Authorized users and automated processes can approve the requests. The commercial edition of Access Requests allows for
requesting specific resources such as a production server to allow fine-grain access for a targeted task.  The Web User Interface for Access Requests
is limited to the commercial edition.

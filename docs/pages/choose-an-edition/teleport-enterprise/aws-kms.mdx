...
a CA rotation must be performed.
...
### Attach the AWS IAM policy

Your Teleport Auth Server will need access to an IAM identity with this policy attached.
You can provide credentials via IAM Roles for Tasks, IAM Roles for EC2,
credential files, or environment variables.
See [Specifying Credentials](https://aws.github.io/aws-sdk-go-v2/docs/configuring-sdk/#specifying-credentials)
for details.

If your Teleport Auth Service is running on an EC2 instance and already has an
attached "IAM role for Amazon EC2", attach the above policy to the existing role.
If the instance does not already have an attached role, create an IAM role with
the above policy and attach it to your EC2 instance running the Teleport Auth
Service.

## Step 2/5. Configure your Auth Server to use KMS keys

CA key parameters are statically configured in the `teleport.yaml` configuration
file of the Teleport Auth Services in your cluster.

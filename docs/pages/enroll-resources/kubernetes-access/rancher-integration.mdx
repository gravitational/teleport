...
Teleport can be configured to interoperate with Rancher's RBAC system allowing you to centrally manage and audit access to Rancher-managed clusters.
 
This guide demonstrates how to configure Teleport's Kubernetes RBAC to grant access using Rancher's built-in roles — such as the Cluster Member role. 
This approach enables you to manage Kubernetes access through Teleport’s unified access control system while aligning with Rancher’s existing RBAC structure.
...
2. Discover Rancher RBAC bindings and identifiers:
 Run the command below to list Kubernetes RoleBindings and extract the group information that Teleport will use to match your existing Rancher access patterns:

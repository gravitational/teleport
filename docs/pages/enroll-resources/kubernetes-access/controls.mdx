...
to only allow access to pods with kind 'pod'. Later role versions extended the support.
</Admonition>
...
<Admonition type="tip">
Role V7 added support for more `kind` values. It uses singlar names while later role
...
the following kinds:
  ...
  <Admonition type="warning">
  The `*` kind behavior slightly differs from later versions. In V7, it allows access to all resources
  including the global ones regardless of the `namespace` field.
  In later versions, the `namespace` field is enforced, which means that when upgrading you may need to adjust
  your resources, especially on the deny side.
  </Admonition>
  <Admonition type="warning">
  The `namespace` kind covers the `namespace` resource as well as all resources within it.
  This behavior is different from later versions where it only covers the resource itself which means that
  when upgrading you may need to adjust your resources, especially on the deny side.
  </Admonition>

  | Kind | Grants access to |
  | ---- | ----------- |
  | `*` | All resources, including global ones regardless of the `namespace` field |
  | `pod` | Pods |
  | `secret` | Secrets |
...
- `namespace`: The Kubernetes namespace in which to allow access to a resource.
  In the `kube-access` role, we are allowing access to a pod in the
  `production` namespace.
  Note that when the `kind` field is set to `*`, the `namespace` field is ignored,
  which is a different behavior from later versions where the `namespace` field is
  enforced.
...
<Admonition type="tip">
Role V8 added support for for custom resources (CRDs) as well as any native resource that
were not supported earlier. To do so, it introduces a new field `api_group` and change the
`kind` field to require the plural form of the resource.
</Admonition>
<Admonition type="warning">
The `namespaces` kind doesn't have a special meaning and the `*` kind now enforces the `namespace` field.
As this behavior differs from earlier version, when upgrading, you may need to adjust your resources,
especially on the deny side.
</Admonition>
...
- `kind`: The kind of resource to enable access to. Currently. It can be `*` or
 the plural form of the kind (e.g. `pods`, `deployments`, `cronjobs`, `mycustomresources`).
 If the resource has a group, it must be specified in the `api_group` field, for example,
 while `pods` doesn't need a group, `deployments` does and hte `api_group` field should be set to `apps`.

<Admonition type="warning">
- The `namespaces` kind doesn't include all the namespaced resources, it only cover the namespace itself.
  This behavior is different from earlier versions where it covered the resource itself and everything in the namespace,
  which means that when upgrading you may need to adjust your resources, especially on the deny side.
- The kind, including `*` now enforces the `namespace` field. To match a global resource, the `namespace` field
must be empty.
  This behavior is different from earlier versions where it included global resources regardless of the `namespace` field.
</Admonition>

- `api_group`: The API group of the resource. In `kube-access`, this is the
  `""` for `pods` or `apps` for `deployments`. A wildcard can be used to match any API groups.

- `namespace`: The Kubernetes namespace in which to allow access to a resource. Must be empty for global resources.
  Note that when using `*`, it will not match any global resources.
  In the `kube-access` role, we are allowing access to a pod in the `production` namespace.
...
Thie following roles will grant full access to all namespaced resources in all namespaces
  ...
  allow:
    ...
    kubernetes_resources:
      - kind: "*"
        api_group: "*"
        name: "*"
        namespace: "*"  # In v8, having any value in the namespace means global resources will not be included.
        verbs: ["*"]
      - kind: namespaces  # In v8, namespaces mean the namespace itself. As it is considered a global resource, it must be added separately.
        name: "*"
        verbs: ["*"]
  deny:
    kubernetes_resources:
...
##### Full access to dev namespace and all global resources except clusterroles

The following roles will grant access to all resources in the `dev` namespace and all global resources
except `clusterroles`.
...
spec:
  allow:
    kubernetes_labels:
      "*": "*"
    kubernetes_resources:
      # In v7, "*" includes global resources regardless of the namespace.
      - kind: "*"
        name: "*"
        namespace: dev
        verbs: ["*"]
  deny:
    kubernetes_resources:
      - kind: clusterrole  # v7 uses singluar.
        name: "*"
        verbs: ["*"]
    # ...
```
...
spec:
  allow:
    ...
    kubernetes_resources:
      ...
      # In v8, "*" doesn't include globals if the namespace is set.
      - kind: "*"
        api_group: "*"
        name: "*"
        namespace: dev
        verbs: ["*"]
      # Grant access to all global resources.
      - kind: "*"
        api_group: "*"
        name: "*"
        namespace: ""  # Empty namespace means only global resources will match.
        verbs: ["*"]
  deny:
    kubernetes_resources:
  ...
  allow:
    ...
    kubernetes_resources:
      ...
      - kind: "*"
        api_group: "*"
        name: "*"
        namespace: "*"  # Namespace set means we match namespaced resources.
        verbs: ["*"]
      # Full access to global resources.
      - kind: "*"
        api_group: "*"
        name: "*"
        namespace: ""  # Empty namespace means we match global resources.
        verbs: ["*"]
    # ...
```

...
This documentation provides an overview of health checks to Kubernetes clusters with Teleport. Teleport Kubernetes Service instances periodically check the connectivity and permissions of enrolled Kubernetes clusters, and is available in Teleport version `18.3.0` and later.
...
- Running a pre-`18.3.0` version of Teleport Kubernetes Service
...
The default_kube `health_check_config` enables all Kubernetes clusters to participate in health checks from version `18.3.0` onward.
```yaml
kind: health_check_config
metadata:

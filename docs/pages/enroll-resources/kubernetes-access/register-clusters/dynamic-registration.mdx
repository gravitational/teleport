...
Teleport Enterprise tenant and `TOKEN` to the join token we created earlier:
...
command to retrieve the resource as it exists on the Auth Service instances backend:

...
- RBAC and resource filtering based on [dynamically updated labels](../../../zero-trust-access/rbac-get-started/labels.mdx)
- [Session recording without SSH connection termination](../../../reference/architecture/session-recording.mdx#record-at-the-proxy-service)
- [Session sharing](../../../zero-trust-access/authentication/joining-sessions.mdx)
- [Advanced session recording](../guides/bpf-session-recording.mdx)

## How it works

At a high level, Teleport supports OpenSSH servers by proxying SSH connections through the Proxy Service.
The OpenSSH server will be set up to trust connections from the Proxy Service rather than trusting direct
...
- In order to record the session and its audit events, the Proxy Service terminates (decrypts) the client SSH connection, and establishes its own connection to the OpenSSH server. It then acts as a pipe between the two connections, recording events and IO as it proceeds.
  - See [Recording Proxy Mode](../../../reference/architecture/session-recording.mdx#record-at-the-proxy-service) for more details.
...
processes that have joined the cluster.
...
<details>
<summary>Command not connecting, or not sure it worked?</summary>

If the command fails with a "connection refused" error on the first attempt,
verify that the Teleport Proxy Service is reachable:

```code
$ curl https://<Var name="teleport.example.com:443" />/webapi/ping
```

Retry the command afterward. Transient DNS or TLS handshake delays can cause
the first attempt to fail.

The command exits silently on success. To confirm the node registered, run
`tctl nodes ls` or `tsh ls` and check that the new node appears in the output.

If you need to retry the command, for example after a failed attempt, run it
again from the same host without deleting anything first. The command reuses
the host ID persisted in the local data directory, so retries update the
existing node entry instead of creating a duplicate one.

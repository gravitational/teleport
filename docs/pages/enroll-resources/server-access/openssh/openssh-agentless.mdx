...
At a high level, Teleport supports OpenSSH by proxying SSH connections through the Proxy Service.
The OpenSSH server will be set up to trust connections from the Proxy rather than trusting direct
connections from user clients. This ensures that all connections to OpenSSH servers go through the
...
- The Proxy Service is uniquely able to request certificates signed by Auth with the Teleport OpenSSH CA on behalf of users, and the OpenSSH server is set up to only trust certificates signed by this CA. Therefore, in order to connect to an OpenSSH Node, a user client must connect through the Proxy to request and use an OpenSSH certificate.
- Before generating an OpenSSH certificate for a user, the Proxy Service performs RBAC checks to see if the user should be allowed to access the node.

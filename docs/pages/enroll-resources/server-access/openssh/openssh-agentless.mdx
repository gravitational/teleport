...
If the command fails with a "connection refused" error on the first attempt, verify the Teleport Proxy Service is reachable with `curl https://<proxy-address>/webapi/ping` and retry. Transient DNS or TLS handshake delays can cause the initial connection to fail.

The command exits silently on success. To confirm the node registered, run `tctl nodes ls` on the Auth Service host or `tsh ls` from your workstation and verify the new node appears in the output.

<Admonition
  type="warning"
  title="Avoid duplicate registrations"
>
Running `teleport join openssh` multiple times for the same host creates
duplicate node entries in the cluster. Before re-running the command (for
example, to update labels), remove the existing node first:

```code
$ tctl rm nodes/<hostname>
```

You can find the node's hostname or UUID with `tctl nodes ls`.
</Admonition>

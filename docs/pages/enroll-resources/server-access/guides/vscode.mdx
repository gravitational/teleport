...
Configure your local SSH client to access Teleport Nodes. Replace <Var name="teleport.example.com" description="Path to your Teleport Proxy Service or Cloud tenant"/> with the address of your Teleport Proxy Service (e.g., `example.teleport.sh` for Teleport Enterprise users), and replace <Var name="alice" description="Your teleport user"/> with your Teleport user.

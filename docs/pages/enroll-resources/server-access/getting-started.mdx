...
<TabItem scope={["cloud"]} label="Teleport Enterprise">
 ...
 Change `example.teleport.sh` to your Teleport Enterprise tenant address. Assign the
...
On your local machine, log in to your cluster through `tsh`, assigning the
`--proxy` flag to the address of your Teleport Enterprise tenant:

```code
# Log in through tsh
$ tsh login --proxy=example.teleport.sh:443 --user=myuser
```
...
You'll be prompted to supply the password and multi-factor we set up
previously.
...
In this example, `myuser` is now logged into the `example.teleport.sh`

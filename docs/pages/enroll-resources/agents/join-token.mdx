...
- A Linux server that you will use to host your Teleport process, e.g., a
  ...
  <details>
  <summary>Running multiple Proxy Service instances behind a load balancer</summary>
...
- (!docs/pages/includes/iac-clients.mdx!)
  ...
  If you managing Teleport resources with the Teleport Kubernetes operator, this
  guide assumes that your resources manifests are placed together in a Helm
  chart.
...
exception of tokens with the `bot` type, which are used by Machine & Workload
Identity.
...
- If you have workloads split across different networks or clouds, we recommend
  setting up trusted clusters. Read how to get started in [Configure Trusted Clusters](../../zero-trust-access/deploy-a-cluster/trustedclusters.mdx).

...
description: Explains how to edit the services taht are running on a Teleport Agent.
---
...
new services you want to run. In our example, we want to create a join token for
the Teleport Kubernetes Service and Teleport Application Service, so we specify
the two services in the `--type` flag:
...
roles and token tyeps that the chart supports.
...
While this guide shows you how to create a token using `tctl`, we recommend that
admins of production clusters manage tokens using the Teleport Terraform
provider or Kubernetes operator. See the following documentation for information

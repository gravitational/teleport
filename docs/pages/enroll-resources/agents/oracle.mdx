...
Select the currently active domain, then select **Dynamic groups**. Click **Create dynamic
...
This error likely means that the instance does not have the required permissions.
Check if the identity domain you created the dynamic group in has the Compute
service active; if it doesn't, try creating the group in a domain that does.

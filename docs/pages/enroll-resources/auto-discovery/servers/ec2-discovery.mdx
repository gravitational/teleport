...
1. Add a new entry to `spec.allow` in the token created in
...
1. Edit the trust policy of the new role to allow the Discovery Service's role
  to assume it:
```json
{
    "Version": "2012-10-17",
...
```
1. Edit the Discovery Service's version of the [Step 2](#step-27-define-an-iam-policy)
...
```json
{
    ...
    "Statement": [
        ...
        },
        {
            "Effect": "Allow",
            "Action": "sts:AssumeRole",
            "Resource": "arn:aws:iam::123456789012:role/<Var name="new role ARN" />"
        }
    ]
}
```

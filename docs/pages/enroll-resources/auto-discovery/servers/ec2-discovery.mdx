...
spec:
  ...
  join_method: iam
  allow:
  # add an allow rule for each aws account
  - aws_account: "123456789"
    # ...
```
...
Create a new role and note its ARN. Create the following IAM policy and attach
it to the new role:
```json
{
    "Version": "2012-10-17",
...
```
Edit the Discovery Service's policy and add the following statement to allow
the Discovery Service to assume the new role:
```json
{
    "Version": "2012-10-17",
    "Statement": [
...
Add an entry to the `discovery_service.aws` section of your `teleport.yaml` file

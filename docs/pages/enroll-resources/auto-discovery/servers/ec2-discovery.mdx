...
## Step 1/4. Update EC2 invite token

Add a new entry to `spec.allow` in the token created in
[Step 1](#step17-create-an-ec2-invite-token) and set `aws_account` to the
account number of the new account.

## Step 2/4. Configure IAM permissions

Create a new role and note its ARN. Create an IAM policy as described in
[Step 2](#step-27-define-an-iam-policy) and attach it to the new role.
Edit the trust policy of the new role to allow the Discovery Service's role
to assume it:
```json
{
    "Version": "2012-10-17",
...
```
Edit the Discovery Service's version of the [Step 2](#step-27-define-an-iam-policy)
policy and add the following statement to allow the Discovery Service to assume the new role:
```json
{
    "Version": "2012-10-17",
    "Statement": [
...
## Step 3/4. Update Teleport configuration
...
## Step 4/4. Bootstrap the Discovery Service AWS configuration

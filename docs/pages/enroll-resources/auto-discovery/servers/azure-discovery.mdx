...
<TabItem label="Dynamic configuration (recommended)">
  Create a Discovery Config resource, that has the same discovery group as above, to enable Azure VM discovery.
  ...
  <Admonition type="tip">
  Discovery Config can be managed using terraform, see the [terraform `discovery_config` reference](../../../reference/infrastructure-as-code/terraform-provider/resources/discovery_config.mdx) for more information.
  </Admonition>
...
<TabItem label="Static configuration">
  <Admonition type="note">
  While simpler at first, this method has less flexibility because enrollment changes require edits to `teleport.yaml` and the restart of the Discovery Service.
  </Admonition>

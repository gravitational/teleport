...
- EC2 instances to enroll with your Teleport cluster. These instances must
  ...
  - Running Ubuntu/Debian/RHEL/Amazon Linux 2/Amazon Linux 2023, and SSM agent
    ...
    manually.)
  - Have the SSM agent installed and running, with the
    `AmazonSSMManagedInstanceCore` IAM policy attached to the instance
    profile so the agent can receive commands from the Discovery Service.
    Amazon Linux 2 and Amazon Linux 2023 include the SSM agent by default.
    For Ubuntu and Debian instances, install it manually or via user data
    before discovery can enroll them. See the [AWS Systems Manager
    documentation](https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-install-ssm-agent.html)
    for agent installation instructions, and the [AWS
    documentation](https://docs.aws.amazon.com/aws-managed-policy/latest/reference/AmazonSSMManagedInstanceCore.html)
    for a list of permissions included in the policy.
...
For this guided flow, use the **Static configuration** tab below, not

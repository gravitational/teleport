...
If you manage multiple AWS accounts in an AWS Organization, see [Organization-level EC2 Auto-Discovery Configuration](ec2-discovery-organization.mdx).
...
The SSM agent must be installed and running on target EC2 instances. Amazon
Linux 2 and Amazon Linux 2023 include it by default. For Ubuntu and Debian
instances, install it manually or via user data before discovery can enroll
them. See the [AWS Systems Manager documentation](https://docs.aws.amazon.com/systems-manager/latest/userguide/sysman-install-ssm-agent.html)
...
- Follow the [Terraform EC2 Auto-Discovery Configuration guide](./ec2-discovery-terraform.mdx) to manage these steps as Infrastructure as Code (IaC)

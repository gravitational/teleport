...
1. **Label matching:** The Database Service monitors `db` resources and the
  ...
  Use more specific label selectors when needed so that a Database Service only
  matches databases it can actually reach. Broad selectors can cause unreachable
  databases to be registered and lead to connection failures in HA deployments.
  </Admonition>

...
<Admonition type="tip">
You can override a discovered database's name by using provider metadata
attached to the database:

- **AWS and Azure key**: `TeleportDatabaseName`.
- **Google Cloud key**: `teleport-database-name`.
- **Value**: the desired name.

The Discovery Service checks the database metadata for the provider-specific key
and uses a valid value as the resource name in Teleport without appending
additional metadata.
</Admonition>
...
2. **Permissions:** The Database Service will assume the identity configured in
...
3. **Connection:** The Database Service, acting as either its own identity or

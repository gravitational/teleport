...
For more information you can take a look at [`discovery_service`](../../../reference/deployment/config.mdx) and [`app_service`](../../../reference/deployment/config.mdx) configuration references.
...
### `teleport.dev/session-recording-bots`

Controls whether session recordings will be generated for non-human (bot) users
of this application. Disabling session recording for bots provides a way to
mitigate the load caused by service-to-service use-cases. Supported values are
"on" and "off". Defaults to "on".

```yaml
annotations:
  teleport.dev/session-recording-bots: "on"
```

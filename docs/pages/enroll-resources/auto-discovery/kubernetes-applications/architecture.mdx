...
are considered of an "app" type, but it can be changed for a service by Kubernetes annotation [`teleport.dev/discovery-type`](../reference/kubernetes-applications.mdx#teleportdevdiscovery-type).
...
[`teleport.dev/name`](../reference/kubernetes-applications.mdx#teleportdevname).
...
Otherwise, this port is ignored. But if annotation [`teleport.dev/protocol`](../reference/kubernetes-applications.mdx#teleportdevprotocol) is used on the service and its value is

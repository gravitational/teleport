...
See the Azure VM auto-discovery [guide](../../../enroll-resources/auto-discovery/servers/azure-vm-discovery/azure-vm-discovery.mdx).
...
See the Google Cloud database auto-discovery
[guide](../../../enroll-resources/auto-discovery/databases/gcp.mdx).

| Label                                     | Description                                                                                               |
|-------------------------------------------|-----------------------------------------------------------------------------------------------------------|
| `endpoint-type`                           | Endpoint Teleport routes to: one of `public`, `private` (PSA), or `psc`.                                  |
| `engine-version`                          | Cloud SQL database version, e.g. `POSTGRES_16` or `MYSQL_8_4`.                                            |
| `engine`                                  | Database protocol: `postgres` or `mysql`.                                                                 |
| `instance-type`                           | Instance role: `primary` or `read-replica`.                                                               |
| `project-id`                              | GCP project ID the instance is running in.                                                                |
| `region`                                  | GCP region where the instance is running.                                                                 |
| `status`                                  | Instance state at discovery time, e.g. `RUNNABLE`.                                                        |
| `teleport.dev/cloud`                      | Always `GCP`.                                                                                             |
| `teleport.dev/discovery-type`             | `cloudsql` for Cloud SQL databases.                                                                       |
| `teleport.dev/origin`                     | Always `cloud`.                                                                                           |
| `teleport.internal/discovered-name`       | Original instance name when Teleport adds a suffix; otherwise absent.                                     |
| `teleport.internal/discovery-config-name` | Dynamic discovery configuration name. Absent for static Discovery Service matchers.                       |
| `teleport.internal/discovery-group-name`  | Discovery group name; absent when unset.                                                                  |

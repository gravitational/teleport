...
Note: AWS SSO/Identity Center assumed roles cannot be used as EKS Access Entry
principals. If you are using AWS SSO, create a dedicated IAM role for the
Discovery Service instead of using your SSO role. The IAM policy should also
include `eks:ListAccessEntries` for the auto-bootstrap to verify existing
entries.
</Admonition>

...
ARN as the Access Entry principal (e.g., `arn:aws:iam::ACCOUNT:role/aws-reserved/sso.amazonaws.com/REGION/AWSReservedSSO_RoleName_ID`), not the STS assumed-role session ARN (e.g., `arn:aws:sts::ACCOUNT:assumed-role/AWSReservedSSO_RoleName_ID/user@example.com`). EKS does not support STS session principals for access entries. If the Teleport services run under an SSO role, the auto-bootstrap will use the full IAM role ARN path to create entries. The IAM policy should also include `eks:ListAccessEntries` for the auto-bootstrap to verify existing entries.
</Admonition>

...
Federation is the ability for a trust relationship to be established between
two trust domains, allowing workloads in one trust domain to validate the
identities of workloads in the other trust domain.
...
in the first trust domain. It is typical to therefore establish the federation
...
Endpoint exposed on the Teleport Proxy.
...
The SPIFFE Bundle Endpoint is exposed on the Teleport Proxy at the path
...
To configure Teleport Workload Identity to trust another trust domain, the
`spiffe_federation` resource is used.

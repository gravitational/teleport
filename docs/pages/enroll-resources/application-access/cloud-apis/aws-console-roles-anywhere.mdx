...
Alternatively, the `tsh apps login` command supports interactive login when you pass
`--interactive` (or `-T`) or set `TELEPORT_AWS_LOGIN_INTERACTIVE=true`.

```code
$ tsh apps login aws-console --interactive
Available AWS roles:

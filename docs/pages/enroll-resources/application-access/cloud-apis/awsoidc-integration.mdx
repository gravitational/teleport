...
This command calls AWS STS `GetCallerIdentity` API using the OIDC token exchange, confirming that AWS trusts Teleport's identity provider and can assume the configured IAM role.

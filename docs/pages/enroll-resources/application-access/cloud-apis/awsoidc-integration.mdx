...
To validate the integration, confirm it was created successfully:
...
The `tctl integrations test` command is available in Teleport v19+. On earlier
versions, verify the integration by checking that the OIDC discovery endpoint
is accessible from AWS:

```code
$ curl https://<Var name="example.teleport.sh" />/.well-known/openid-configuration
```

This should return a JSON document with `issuer` and `jwks_uri` fields.
</Admonition>

...
When generating the credentials, Teleport checks the user's RBAC permissions, and if they are authorized, returns the credentials to the user.

Teleport uses AWS IAM Roles Anywhere to issue temporary credentials to users.
AWS IAM Roles Anywhere Profiles will appear as Teleport resources in the Web UI and users can access their roles through the Web UI or CLI.
...
## Step 1/3. Configure AWS IAM Roles Anywhere integration
...
## Step 2/3. Create an AWS IAM Roles Anywhere Profile and assign IAM Roles
...
## Step 3/3. Access AWS resources

Now that you have configured the Integration, users can access AWS resources through Teleport.
...
### Access the AWS CLI or other AWS SDK based tools

1. On your desktop, log into the AWS App Profile that was synced:

   ```code
   $ tsh apps login --aws-role arn:aws:iam::278576220453:role/ExampleReadOnlyAccess ExampleReadOnlyAccess
    Logged into AWS app "ExampleReadOnlyAccess".

    Your IAM role:
        arn:aws:iam::123456789012:role/ExampleReadOnlyAccess

    Example AWS CLI commands:
        aws --profile ExampleReadOnlyAccess s3 ls
        AWS_PROFILE=ExampleReadOnlyAccess aws s3 ls
   ```

1. You can now use the `aws` command line tool or any other AWS SDK based tools with the `ExampleReadOnlyAccess` role, as described in the output above.

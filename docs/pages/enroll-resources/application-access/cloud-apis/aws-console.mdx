...
1. Create a file called `aws-ro-access.yaml` with the following content,
   ...
   <Admonition type="important" title="AWS roles and App labels">
   Role needs permissions for `aws_role_arns` and `app_labels` for AWS access to work.

   If a role has `aws_role_arns` defined but not `app_labels`, attempting to access the AWS Console will result in a `Not Found` error.
   </Admonition>

...
To track the identity of the Teleport user across AWS role chaining in
CloudTrail, set `source_identity: true`. This sets the Teleport username as the
[AWS STS source identity](https://docs.aws.amazon.com/IAM/latest/UserGuide/id_credentials_temp_control-access_monitor.html)
on assumed role sessions. The target role's trust policy must include the

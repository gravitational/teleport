...
>
  ...
  - Verify the token hasn't expired (tokens are valid for 1 hour by default).
  - Ensure the Linux server can reach both the Teleport Proxy and the target application.
  - Test connectivity to the target app: `curl -v http://app.example.com:3000` from the Application Service host.
  - Run `tctl get app/my-app` to verify the application is registered in the cluster.
</Checkpoint>
...
<Checkpoint
  title="Verify DNS and TLS configuration"
  description="Confirm that DNS is correctly configured to route traffic through Teleport (if you completed this optional step)."
>
  Here are some troubleshooting tips:
  - Verify DNS resolution: `nslookup my-app.teleport.example.com` should resolve to your Teleport Proxy.
  - Test the application URL in your browser: `https://my-app.teleport.example.com`.
  - If using a wildcard certificate, ensure it covers `*.teleport.example.com`.
  - Check that your DNS provider has propagated the changes (this can take up to 48 hours).
</Checkpoint>
...
>
  ...
  - Verify the application appears in the **Applications** tab of the Teleport Web UI.
  - Check that the role's `app_labels` match the labels assigned to your application (e.g., `env: demo`).
  - If the application doesn't appear, run `tctl get roles/demo-app-access` to verify the role configuration.
  - Review Application Service logs for connection errors: `sudo journalctl -u teleport`.
  - Test that the Application Service can still reach the target app from its host.
</Checkpoint>

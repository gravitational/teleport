...
<Admonition
  ...
  title="Danger Zone"
>
  This disables TLS verification between the Application Service and your
  backend application. It does not affect the TLS connection between end users
  and the Teleport Proxy Service. Use this only for development or when the
  backend uses a self-signed certificate that cannot be added to the system
  trust store.
</Admonition>
...
The header rewrite syntax differs between static configuration
(`teleport.yaml`) and dynamic app resources (`tctl create`). Static config
uses a flat list of `"Name: Value"` strings, while dynamic resources use a
list of `name`/`value` mappings. Using the wrong format will cause a
validation error or silently drop headers.
</Admonition>

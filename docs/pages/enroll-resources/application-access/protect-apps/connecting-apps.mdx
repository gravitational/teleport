...
>
  Verify DNS resolution by running the following command:

  ```code
  $ nslookup my-app.teleport.example.com
  ```

  This should resolve to your Teleport Proxy. If it doesn't, check that your DNS provider has propagated the changes (this can take up to 48 hours). If using a wildcard certificate, ensure it covers `*.teleport.example.com`.

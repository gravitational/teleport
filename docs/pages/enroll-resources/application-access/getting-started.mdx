...
(for example, `teleport.example.com` or `example.teleport.sh`).
...
`teleport.example.com` or `example.teleport.sh`).
...
a password, set up a multi-factor for authentication, and sign in to the Teleport Web UI.
...
`https://grafana.teleport.example.com` or `https://grafana.example.teleport.sh`. 

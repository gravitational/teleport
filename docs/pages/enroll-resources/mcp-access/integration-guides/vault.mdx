...
description: Configure a HashiCorp Vault MCP server for access via Teleport
tags:
- how-to
- mcp
- zero-trust
...
(!docs/pages/includes/mcp-access/integration-how-it-works-service-token.mdx serviceName="HashiCorp Vault" token="a service token"!)

## Prerequisites

(!docs/pages/includes/edition-prereqs-tabs.mdx edition="Teleport (v18.3.0 or higher)" clients="\`tsh\` client"!)
- Access to your Grafana instance and sufficient privileges to manage service accounts.
...
## Step 1/3. Create a service in Vault
...
The [Vault MCP Server](https://github.com/hashicorp/vault-mcp-server) can be run
...
The Vault MCP Server now exposes a streamable-HTTP endpoint at `http://<Var name="MCP_HOST" initial="localhost" />:8080/mcp`.

## Step 3/3. Connect via Teleport

(!docs/pages/includes/mcp-access/integration-teleport-app.mdx service="vault" port="8080" !)

(!docs/pages/includes/mcp-access/integration-limit-tools.mdx service="vault" tool="^(read|list)_.*$" !)

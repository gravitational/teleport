...
The `"*"` wildcard grants access to all tools on all MCP servers and should only be used for temporary debugging. After identifying the issue, update the role to grant access to specific tools only. Leaving wildcard tool access in production bypasses the principle of least privilege.

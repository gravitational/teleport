...
app_service:
  ...
  - name: "everything"
    labels:
      env: dev
    description: "Everything MCP demo server for testing tool access"
    mcp:
...
The `run_as_host_user` field specifies a Linux user account that must exist on
the host before starting the Application Service. Create it with:

```code
$ sudo useradd --system --no-create-home teleport-mcp
```
...
>
Avoid adding this account to the host's `docker` group. If this host is later
enabled for [dynamic MCP server registration](../dynamic-registration.mdx),
group membership would give anyone who can create or update `app` resources
effective root access to the host, since Docker group membership allows
mounting the host filesystem into a container. Run Docker in
[Rootless mode](https://docs.docker.com/engine/security/rootless/) for the
`teleport-mcp` user instead, or use a pre-approved wrapper script in place of
raw Docker execution.
</Admonition>
...
Create the `teleport-mcp` host user as described above before starting the
service. Avoid adding it to the `docker` group for the same reason.

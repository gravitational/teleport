...
<Admonition
  type="warning"
  title="SSE transport is deprecated"
>
The SSE (Server-Sent Events) transport was deprecated in the MCP specification
on 2025-03-26 in favor of the Streamable HTTP transport. For new deployments,
use the [Streamable HTTP guide](streamable-http.mdx) instead.

Existing SSE deployments continue to work, but consider migrating to Streamable
HTTP for long-term support.
</Admonition>
...
Use a URI such as `mcp+sse+https://mcp-server.internal:8080/sse` to point at your
SSE MCP server. The URI format is `mcp+sse+<scheme>://<host>:<port><path>` where
the path points to the SSE endpoint exposed by your MCP server.

For path handling behavior when using streamable-HTTP transport instead of SSE, see the [Streamable HTTP](./streamable-http.mdx) guide.
...
- Configuration and CLI [reference](../reference.mdx).

...
<Admonition
  type="warning"
  title="Security warning: Docker group privileges"
>
In dynamic registration, any user with permission to create or update `app`
resources can specify arbitrary command arguments. Adding the
`run_as_host_user` account to the host's `docker` group grants that account —
and therefore anyone who can manage dynamic app resources — effective root
access to the host, since Docker group membership allows mounting the host

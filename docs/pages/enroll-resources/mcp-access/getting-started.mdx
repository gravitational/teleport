...
After saving the configuration file, restart Teleport to apply the change.

If your host uses systemd, run:

```code
$ sudo systemctl restart teleport
```

If your host doesn't use systemd, restart the Teleport process using the same
method you used to start it initially.

The MCP demo server will not appear in `tctl apps ls` until the service restarts.
...
(!docs/pages/includes/mcp-access/tsh-mcp-config.mdx appName="teleport-mcp-demo" !)
...
You can interact with it using sample questions like "can you show some details
on this Teleport demo?":
...
- Audit events [reference](../../reference/deployment/monitoring/audit.mdx#event-types).

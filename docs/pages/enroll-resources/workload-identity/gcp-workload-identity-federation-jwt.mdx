...
how external identities are mapped to GCP identities.
...
To grant our workloads access to resources within GCP, we can use the
principal string that is generated by the workload identity federation pool and
provider based on the SPIFFE ID of the workload. This string contains the
GCP project number, the name of the workload identity pool and the SPIFFE ID. It
...
This principal value can be specified in IAM policies to reference the workload
identity.

...
4. Enter a unique name for your identity that describes the workload it will
...
4. Enter the address of your Teleport Proxy service followed by
  `/workload-identity` for the "Issuer URL", for example
  `https://example.teleport.sh/workload-identity`.
5. Enter the SPIFFE ID you have decided on for the workload in the
...
6. Enter a unique, identifiable name for the federated credential. For example,

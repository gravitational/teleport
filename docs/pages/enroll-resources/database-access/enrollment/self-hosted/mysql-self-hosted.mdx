...
(!docs/pages/includes/database-access/database-service-host-note.mdx!)
...
<Admonition type="tip" title="Using identity files">
If you are using an identity file (`tsh -i identity.pem`) instead of interactive
`tsh login`, use `tsh proxy db --tunnel --port=<local-port>` instead of
`tsh db connect`. The `tsh db connect` command does not work with identity files
because it references certificate paths that do not exist on disk.
</Admonition>

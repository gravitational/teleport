...
(!docs/pages/includes/database-access/single-node-note.mdx!)
...
<Admonition type="tip" title="Step ordering">
Create your X.509 database users (e.g., `db.getSiblingDB("$external").runCommand(...)`)
before setting `net.tls.mode: requireTLS`. Once strict TLS mode is enabled, you
need a valid client certificate to connect. If you need to create users after
enabling mutual TLS, temporarily set `net.tls.allowConnectionsWithoutCertificates: true`.
</Admonition>

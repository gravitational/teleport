...
The protocol for Elasticsearch is `elasticsearch` when creating database
resources via `tctl create`. The abbreviated form `elastic` is accepted by
`teleport db configure create` but may not work with other Teleport APIs.
</Admonition>

...
(!docs/pages/includes/database-access/self-hosted-config-start.mdx dbName="myelastic" dbProtocol="elasticsearch" databaseAddress="elasticsearch.example.com:9200" !)
...
<TabItem label="Kubernetes Cluster">
  ...
  (!docs/pages/includes/database-access/self-hosted-db-helm-install.mdx dbName="myelastic" dbProtocol="elasticsearch" databaseAddress="elasticsearch.example.com:9200" !)
</TabItem>
</Tabs>

(!docs/pages/includes/database-access/multiple-instances-tip.mdx !)

<Admonition type="note" title="Protocol name">
The protocol name for Elasticsearch is `elasticsearch`. While `teleport db
configure create` accepts arbitrary protocol strings without validation, the
Database Service rejects any protocol not in the supported list at startup.
Always use `elasticsearch` as the protocol value.
</Admonition>
...
Complete Step 3 (role mapping creation) before setting `client_authentication:
required`. Once strict client authentication is enabled, you cannot use
username/password to manage Elasticsearch. If you need to create or update role
mappings after enabling mutual TLS, temporarily set `client_authentication:
optional`, make your changes, then set it back to `required`.
</Admonition>

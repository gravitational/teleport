...
For example, in the following steps, we will create `teleport-db-service` and `spanner-user` respectively.
...
$ gcloud iam service-accounts get-iam-policy \
  spanner-user@<Var name="project-id" />.iam.gserviceaccount.com \
  --format=json | grep "roles/iam.serviceAccountTokenCreator"
```
...
database, connect using the impersonated service account, and optionally
...
This error can come from either your client application or from the Teleport
Database Service.

**Client application.** Disable GCP credential loading in your client. The client
should not load credentials directly, because the Teleport Database Service
provides them.

**Teleport Database Service.** If the error appears in the Teleport Database
Service logs at DEBUG level, the service does not have GCP credentials configured
correctly.

If you are using a service account key, confirm that `GOOGLE_APPLICATION_CREDENTIALS`
is set to the path of your credentials file, then restart the Teleport Database
Service so it picks up the variable. For example, if the service runs under
`systemd`:
...
For other ways to authenticate as a service account, see

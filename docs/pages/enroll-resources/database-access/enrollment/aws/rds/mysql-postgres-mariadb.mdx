...
The Database Service requires outbound HTTPS access to
`rds.<region>.amazonaws.com` to fetch database metadata. If databases do not
appear in `tctl db ls` within 60 seconds of starting the service, enable DEBUG
logging (`teleport.log.severity: DEBUG` in `teleport.yaml`) and look for AWS
API timeout errors in the Teleport logs.
</Admonition>

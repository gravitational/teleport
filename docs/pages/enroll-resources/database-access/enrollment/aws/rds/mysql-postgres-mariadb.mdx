...
<Admonition type="warning" title="Databases not appearing">
...
and for China regions it is `truststore.pki.rds.cn-north-1.amazonaws.com.cn`.

If databases do not appear in `tctl db ls` within 60 seconds of starting the
service, ensure this endpoint is reachable and enable DEBUG logging
(`teleport.log.severity: DEBUG` in `teleport.yaml`) to check for download
failures.

Access to the RDS API (`rds.<region>.amazonaws.com`) is used on a best-effort
basis for fetching additional metadata (tags, engine versions) but is not

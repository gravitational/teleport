...
database and perform a MySQL handshake. For other protocols, Database Service
instances attempt to establish a TCP connection to the database's endpoint
and report whether the connection succeeded.
...
By default, health checks for MySQL databases send MySQL protocol messages
instead of relying on establishing and closing a TCP connection.

The reason is that MySQL tracks connection errors for each remote host that
tries to connect to the database. Each TCP health check is counted as a
connection error and eventually MySQL will block a host when 
`sum_connect_errors >= max_connect_errors`. As a result, TCP health checks

...
For details on configuring the `rds_iam` role, see the
[AWS documentation](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/UsingWithRDS.IAMDBAuth.DBAccounts.html#UsingWithRDS.IAMDBAuth.DBAccounts.PostgreSQL).
...
spec:
  ...
  # Optional database user to reassign objects to.
  # orphaned_resource_owner: bob
```
...
If `orphaned_resource_owner` is specified, user auto-provisioning is
in effect, and `create_db_user_mode` is set to `best_effort_drop`, all
database objects owned by the auto-provisioned user will be reassigned to
`orphaned_resource_owner` each time the auto-provisioned user's session ends.
`admin_user` must be a member of `orphaned_resource_owner` for this to work.

For auto-discovered cloud databases, the name of the admin user is taken from
the `teleport.dev/db-admin` label, the default database is taken from the
`teleport.dev/db-admin-default-database` label, and the orphaned resource owner
is taken from the `teleport.dev/db-orphaned-resource-owner` label.
...
- Read the automatic user provisioning [RFD](https://github.com/gravitational/teleport/blob/master/rfd/0113-automatic-database-users.md).
- Read the database permission management [RFD](https://github.com/gravitational/teleport/blob/master/rfd/0151-database-permission-management.md).

...
- An AWS account with the necessary permissions to create RDS instances and AWS Directory Service directories.
...
2. Enable TLS on your Oracle RDS instance by following the [AWS RDS Oracle SSL Setup documentation](https://docs.aws.amazon.com/AmazonRDS/latest/UserGuide/Appendix.Oracle.Options.SSL.html). Ensure `SQLNET.SSL_VERSION` is set to `1.2` for optimal security.
...
Connect to your Oracle as an administrative account (e.g. `admin`) and create
...
<Tabs>
  ...
  <TabItem label="Other">
  If `adutil` is not available for your distributon, you can use `ktutil` instead. It should be available as a standard Kerberos utility.
  ```code
  $ ktutil
...
Configure the Teleport Database Service. Make sure to update `--proxy` to
point to your Teleport Cloud tenant address and `--uri` to the Oracle
endpoint.
...
Enable standard auditing by modyfing the RDS Oracle parameter group `audit_trail` to contain [`db,extended`](https://aws.amazon.com/blogs/database/part-1-security-auditing-in-amazon-rds-for-oracle/#:~:text=Standard%20auditing%20with%20Amazon%20RDS%20for%20Oracle).
...
```code
$ tsh db connect --db-user=alice --db-name MYPDB oracle
```

...
- AWS account with a DocumentDB cluster.
  ...
  DocumentDB access requires IAM authentication which is only supported for
  DocumentDB 5.0 or higher.
...
metadata and assume IAM roles as DocumentDB IAM users. Note that the
...
## Step 5/6. Create a DocumentDB IAM user

In this step, we will create a DocumentDB IAM user that can be accessed by
Teleport. 
...
Login your DocumentDB cluster with your master username and password from a
machine that has network access to the DocumentDB cluster. Create a DocumentDB
user with the IAM role ARN and use the `MONGODB-AWS` mechanisms for
authentication:
```code
use $external;
db.createUser(

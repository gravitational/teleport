...
| `config`  | database resources manually defined in the `db_service.databases` section of `teleport.yaml`. |
...
| Tag or label name | Description |
| - | - |
| `TeleportDatabaseName`                   | (AWS and Azure) Overrides the name of the discovered database. |
| `teleport-database-name`                 | (Google Cloud) Overrides the name of the discovered database. |
...
| `vpc-id`           | ID of the Amazon VPC the resource resides in. Available for RDS, ElastiCache and MemoryDB databases. |
...
`dynamic_labels` fields respectively in database definition. See the

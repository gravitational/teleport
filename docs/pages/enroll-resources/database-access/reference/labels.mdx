...
| `teleport.dev/db-orphaned-resource-owner` | (AWS only) Specifies the database user that inherits database objects when an auto-provisioned user's session ends. See the [`db` resource reference](../../../reference/infrastructure-as-code/teleport-resources/database-v3.mdx) for more info. |

...
| `--protocol` | Proxied database protocol. Supported are: `postgres`, `mysql`, `mongodb`, `oracle`, `cockroachdb`, `redis`, `snowflake`, `sqlserver`, `cassandra`, `elasticsearch`, `opensearch`, `dynamodb`, `clickhouse`, `clickhouse-http` and `spanner`. |
...
| `--name` | Name of the proxied database. |
| `--protocol` | Proxied database protocol. Refer to the [`db` resource reference](../../../reference/infrastructure-as-code/teleport-resources/database-v3.mdx) for supported values. |
| `--uri` | Address the proxied database is reachable at. |
...
<Admonition type="note" title="Note">
  ...
  user must be able to impersonate the built-in `Db` role and user. See the

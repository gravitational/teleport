...
Setting **Default URL retrieval timeout** and **Default path validation cumulative retrieval timeout** to less than that
of Teleport's LDAP request timeout will fix this issue. These settings can be found in the Local Group Policy in Computer
Configuration > Windows Settings > Security Settings > Public Key Policies > Certificate Path Validation Settings Properties
then under the Network Retrieval tab.

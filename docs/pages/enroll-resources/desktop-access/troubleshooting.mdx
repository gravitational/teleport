...
#### The Kerberos Key Distribution Center (KDC) for the domain "Your Domain" does not have a certificate installed or does not support logon using certificates: 0xC0000320

You see this error in your *Security-Kerberos* Windows Event log on the client you're trying to connect to.

**Solution:** Review the Kerberos-Key-Distribution-Center Logs on each of your **Domain controller** for causes.

The *Kerberos-Key-Distribution-Center* Windows Event Log provides information on KDC configuration that's required for Smartcard Authentication. This Event Log might not be enabled by default.
Open the Windows Event Log and navigate to Event Viewer, Application and Services Logs,
Microsoft, Windows, Kerberos-Key-Distribution-Center, Operational. Enable this Event Log, restart the *Kerberos Key Distribution Center* service and review the logs
...
If the template name you see is *Domain Controller* this template does not include the EKU for Smart Card by default.

You can check in the local computer Personnal certificat store if the certificate has the "Smart Card Logon" listed in the intended purpose column. If it's not the case:
- Request a new certificat for your domain controller using the default "Kerberos Autentication" Template or any custom template allowing for Smart Card Logon
...
**Solution:** Review the Each certificate in the certification chain and ensure that all certificates are valid and that the Certificate Revocation List (CRL) URLs for each certificate are not expired and available.

If the *Kerberos-Key-Distribution-Center* logs doesn't list a certificate for authentication, Smart Card authentifcation will not work.
...
For checking CRL you can also use certutil GUI. Simply run the command below and click on the "Retrive". Below is an example of an expired revocation list that needs to be addressed
...
pkiview.msc is included with the feature "Active Directory Certificate Services Tools" under the "Remote Server Administration Tools => Role Administration Tools" in the feature installation wizard

pkiview.msc can be installed on any Domain Joined machine and lets you see the health and informations for the Ntauthstore, CDP containers and more. [Review this Microsoft article to learn more on how to use it](https://techcommunity.microsoft.com/blog/coreinfrastructureandsecurityblog/quick-check-on-adcs-health-using-enterprise-pki-tool-pkiview/1128638)

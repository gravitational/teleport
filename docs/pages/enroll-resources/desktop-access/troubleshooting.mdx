...
#### Solution: Ensure that group policy allows specifying credentials

The smart card PIN is sent to the target host using the password field in the
initial RDP connection sequence.
...
Open the Windows Event Log and navigate to **Event Viewer** > **Application and
Services Logs** > **Microsoft** > **Windows** > **Security-Kerberos** >
**Operational**. Enable this Event Log and attempt to connect with Teleport to
review log entries.

This is often an issue with the KDC certificates on your domain controllers not meeting
the requirements for smart card logon.

- Ensure that your KDC certificates are either issued with the "Domain
  Controller" built-in template (when using AD CS), or that they contain the
  Kerberos Authentication EKU (1.3.6.1.5.2.3.5)
- Ensure that the issuing CA certificate for the KDC certificates is also present
  in the NTAuth store of the domain controller.

### Your credentials could not be verified

You connect to a Windows host from the Teleport UI, land on the Windows login
screen and see an error message: **Your credentials could not be verified.**

This means that the smart card certificate presented by Teleport could not be validated
by the domain controller.

#### Solution: Review the CAPI2 logs on the domain controller

The Microsoft Crypto API (CAPI) service logs contain information about certificate
checks performed by domain controllers. This Event Log is not enabled by default.

Open the Windows Event Log and navigate to **Event Viewer** > **Application and
Services Logs** > **Microsoft** > **Windows** > **CAPI2** > **Operational**.
Enable this Event Log and attempt to connect with Teleport to review log
entries.

Note: The CAPI2 logs can be very verbose. We recommend increasing the max log size to
at least 4MB and disabling the logs when you complete troubleshooting.

Review the logs, looking for events with an error status and a task category of
"Build Chain." The details section will contain the username which failed logon.
When you locate the event(s) corresponding to the user who failed to log in,
look at the chain elements in order to identify the failure. Possible failures
here include revoked certificates, certificates that don't chain to a trusted
root, or failed certificate revocation checks.

#### The KDC does not support logon using certificates: 0xC0000320

You see this error `The Kerberos Key Distribution Center (KDC) for the domain "Your Domain" does not 
have a certificate installed or does not support logon using certificates: 0xC0000320
` in your *Security-Kerberos* Windows Event log on the client you're trying to connect to.

**Solution:** Review the Kerberos-Key-Distribution-Center Logs on each of your **Domain 
controller** for causes.

The *Kerberos-Key-Distribution-Center* Windows Event Log provides information on KDC configuration 
that's required for Smartcard Authentication. This Event Log might not be enabled by default.
...
Enable this Event Log, restart the *Kerberos Key Distribution Center* service and review the logs

You should see a log that looks like :
```
The Key Distribution Center (KDC) uses the below KDC certificate for smart card or 
certificate authentication.

Kdc Certificate Information:
  Issuer Name: <Your PKI>
  Serial Number: XXXXXXXXXXXXXXXXXXXXXXXXXXXXX
  Thumbprint: XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX
  Template: Kerberos Authentication
```

You can check in the local computer Personnal certificate store if the certificate has the 
"Smart Card Logon" listed in the intended purpose column. If it's not the case:
- Request a new certificate for your domain controller using the default "Kerberos Autentication" 
Template or any custom template allowing for Smart Card Logon
- Remove the old certificate
- Restart the *Kerberos Key Distribution Center* Windows service
- Repeat the operation for every Domain controller

![Certificate Manager view](../../../img/desktop-access/certificate-configuration.png)

#### The Key Distribution Center (KDC) cannot find a suitable certificate to use. This KDC is 
not enabled for smart card or certificate authentication.

You see this error in your *Kerberos-Key-Distribution-Center* Windows Event log on the domain controller

**Solution:** Review each certificate in the certification chain and ensure that : 
- Your domain controllers Certificats include the Smart Card EKU
- All Certificates are valid
- All Certificate Revocation List (CRL) URLs for each certificate are not expired and available.

If the *Kerberos-Key-Distribution-Center* logs doesn't list a certificate for authentication, Smart Card will not work.
To check if your certificates are not expired and CRL working you can use the following command
``` powershell
certutil -f -verify -URLfetch base64_exported_DomainControler_certificate.cer
```
Review the output with attention.

For checking CRL you can also use certutil GUI. Simply run the command below and click on 
the "Retrive". 
```
certutil -URL base64_exported_DomainControler_certificate.cer
```

Below is an example of an expired revocation list that needs to be addressed

![Certutil GUI](../../../img/desktop-access/certutilgui.png)

Note that you can click on the items listed to open them for review.


Another tool you can use for checking if everything is in order is *pkiview.msc*.

pkiview.msc is included with the feature "Active Directory Certificate Services Tools" under 
the "Remote Server Administration Tools => Role Administration Tools" in the feature installation wizard

pkiview.msc can be installed on any Domain Joined machine and lets you see the health and 
informations for the Ntauthstore, CDP containers and more. 
[Review this Microsoft article to learn more on how to use it](https://techcommunity.microsoft.com/blog/coreinfrastructureandsecurityblog/quick-check-on-adcs-health-using-enterprise-pki-tool-pkiview/1128638)
...
Teleport uses LDAP for several purposes:
1. To (optionally) discover Windows hosts in your Active Directory domain
2. In order to look up the Windows user's SID prior to starting an RDP session
3. To periodically publish Teleport CRLs to the domain

Teleport authenticates to the LDAP server using a client certificate it issues
for itself. If this mutual TLS connection fails, you will see errors similar to:
...
You should also verify that the service account Teleport is using is enabled and
not locked out, which sometimes happen when a policy requires that the account
first performs a password reset.

### Solution 4: Check for clock drift

An incorrect system clock (on either the Teleport side or in the Windows environment)
could result in one or both parties failing to accept the other's certificate.

Ensure that the system clocks are accurate. We recommend configuring NTP to
ensure that clocks don't drift over time.
...
#### Solution: Make sure the Windows host uses secure cipher suites

When the target host forcibly resets the connection, this usually means that the
desktop does not support secure cipher suites for TLS connections. This issue is
less common on modern versions of Windows, but is frequently observed on older versions
such as Windows Server 2012R2.

You can force the host to use secure cipher suites by [configuring a certificate for RDP
...
To enable NLA set the `TELEPORT_ENABLE_RDP_NLA` environment variable to `yes` on
hosts running Teleport's `windows_desktop_service`. More information on
Teleport's NLA support is available in the [Active Directory
guide](./active-directory.mdx#network-level-authentication-nla).
...
```
... "error":"connecting to LDAP server. no LDAP servers responded successfully for domain "example.com" ...
```

When Teleport attempts to authenticate using LDAP to a Domain Controller, the Domain Controller then attempts
to validate certificates by downloading Microsoft's Certificate Trust List from `ctldl.windowsupdate.com`.
If the Domain Controller request timeout (default 15 seconds) takes longer than Teleport's LDAP request timeout
(30 seconds for v18.2.5 and above or 15 seconds for v18.2.4 and earlier), the request will fail and the Domain
Controller will cache the failed retrieval for 35 minutes.

#### Solution: Decrease the Domain Controller's timeouts in Certificate Path Validation Settings Properties

Setting **Default URL retrieval timeout** and **Default path validation
cumulative retrieval timeout** to less than that of Teleport's LDAP request
timeout will fix this issue. These settings can be found in the Local Group
Policy in **Computer Configuration** > **Windows Settings** > **Security
Settings** > **Public Key Policies** > **Certificate Path Validation Settings**
under the **Network Retrieval** tab.
...
Device redirection is typically enabled by default, but may be disabled in group
policy. The relevant settings are located under **Computer configuration** >

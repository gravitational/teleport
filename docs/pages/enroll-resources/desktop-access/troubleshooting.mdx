...
#### The Kerberos Key Distribution Center (KDC) for the domain "Your Domain" does not 
havea certificate installed or does not support logon using certificates: 0xC0000320

You see this error in your *Security-Kerberos* Windows Event log on the client you're 
...
If the template name you see is *Domain Controller* this template does not include 
the EKU for Smart Card by default.

You can check in the local computer Personnal certificat store if the certificate has the 
"Smart Card Logon" listed in the intended purpose column. If it's not the case:
- Request a new certificat for your domain controller using the default "Kerberos Autentication" 
...
the "Retrive". Below is an example of an expired revocation list that needs to be addressed

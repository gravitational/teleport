...
Dynamic Windows desktop registration allows Teleport administrators to register new Windows desktops (or
update/unregister existing ones) without having to update the static
configuration files read by Teleport Windows Desktop Service instances.

Windows Desktop Service instances periodically query the Teleport Auth Service for
`dynamic_windows_desktop` resources, each of which includes the information that the Windows Desktop
Service needs to connect to a Windows desktop.
...
called `example` at `localhost:3089`.
...
The user creating the dynamic Windows desktop needs to have a role with access to the
Windows desktop labels and the `dynamic_windows_desktop` resource.  In this example role the user can only
create and maintain dynamic Windows desktops labeled `env: test`.

```yaml
kind: role
metadata:
  name: dynamicdesktopregexample
spec:
  allow:
...
Windows desktops (in `tsh desktops ls` or UI) as long as at least one Windows Desktop Service

...
```code
$ curl.exe -fo teleport-windows-auth-setup-v(=teleport.version=)-amd64.exe install --cert=teleport.cer -r
```
...
```code
$ curl.exe -fo teleport-windows-auth-setup-v(=cloud.version=)-amd64.exe install --cert=teleport.cer -r
```

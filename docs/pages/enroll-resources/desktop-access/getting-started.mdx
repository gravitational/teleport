...
<Checkpoint
  ...
  description="Confirm that the Windows computer has restarted and the Teleport certificate authority is installed."
>
  Here are some troubleshooting tips:
  - Verify the certificate was imported by opening `certlm.msc` and checking that the Teleport CA appears under **Trusted Root Certification Authorities > Certificates**.
  - Ensure the Teleport DLL is installed by checking that `teleport.dll` exists in `C:\Windows\System32\`.
  - Confirm NLA is disabled: open **System Properties > Remote**, and verify "Allow connections only from computers running Remote Desktop with Network Level Authentication" is unchecked.
  - Check that LSA protection is disabled if the DLL fails to load.
</Checkpoint>
...
>
  Here are some troubleshooting tips:
  - Check the service status: `systemctl status teleport` on the Linux server.
  - Review logs for connection errors: `sudo journalctl -u teleport`.
  - Verify the token hasn't expired (tokens are valid for 60 minutes by default).
  - Ensure the Linux server can reach the Teleport Proxy Service.
  - Ensure the Linux server can reach the Windows desktop on port 3389.
</Checkpoint>
...
>
  ...
  - If the connection fails, review the Windows Desktop Service logs on the Linux server: `sudo journalctl -u teleport`.
</Checkpoint>

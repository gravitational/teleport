...
2. Export the Teleport certificate authority by running the following command,
   ...
   CA certificates (one for each Auth Service instance). You can download a bundle
   containing all certificates by appending `&format=zip` to the URL.
...
4. Double-click the executable you downloaded to run the Teleport Windows Auth Setup program
  ...
  Note: official Teleport release installers include a Microsoft-signed Windows
  authentication package that can be loaded when
  ...
  is enabled. If you build the Windows authentication package from source instead
  of using an official release artifact, the resulting DLL is not Microsoft-signed
  and cannot be loaded while LSA protection is enabled.

...
Windows and Linux desktops connected to Teleport.
...
spec:
  options:
    ...
    # Note: this is applicable to local Windows users only and is not supported in
    # Active Directory environments.
    create_desktop_user: true
  allow:
    # Label selectors for Windows desktops this role has access to.
    ...
    # Label selectors for Linux desktops this role has access to.
    linux_desktop_labels:
      environment: ["dev", "stage"]

    # Linux user accounts this role can connect as.
    linux_desktop_logins: ["alice", "{{internal.linux_desktop_logins}}"]
```
...
In this case, the regular expression is evaluated against `192.168.1.105:3389`.
...
## Linux Desktop Labeling

Both `allow` and `deny` rules support `linux_desktop_labels` selectors. These
selectors are matched against the labels set on the Linux desktop resource.
Wildcards (`"*"`) can be used to match all label values.

Linux desktops acquire labels from the `labels` field in the
`linux_desktop_service` section of the Teleport configuration file on the host:
...
These labels are then used in role `allow` and `deny` rules:

```yaml
allow:
  # Allow access to all Linux desktops in the dev environment.
  linux_desktop_labels:
    environment: dev
```
...
```yaml
deny:
  # Deny access to production Linux desktops.
  linux_desktop_labels:
    environment: prod
```
...
The `linux_desktop_logins` role setting lists the Linux user accounts that the
role permits access to. The specified accounts must already exist on the Linux
host. The `{{internal.linux_desktop_logins}}` variable can be used as a placeholder for
the user's `linux_desktop_logins` trait. The `linux_desktop_logins` trait can be specified when
the user is created:

```code
$ tctl users add alice --linux-desktop-logins=alice,admin
```

Or updated for an existing user:

```code
$ tctl users update alice --set-linux-desktop-logins=alice,admin
```

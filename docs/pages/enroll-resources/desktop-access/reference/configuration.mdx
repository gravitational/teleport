...
`teleport.yaml` fields for the Linux Desktop Service:

```yaml
# Service responsible for Linux desktop access.
# The Linux Desktop Service runs directly on the Linux host being accessed
# and requires Xvfb to be installed on that host.
(!docs/pages/includes/config-reference/linux-desktop-config.yaml!)
```

`teleport.yaml` fields for the Windows Desktop Service:

```yaml
# Main service responsible for Windows desktop access.
#

...
Starting in Teleport 18.11.0, desktop session recordings can be exported to video.

```
$ tsh recordings export ccd2cfd0-f32c-467d-af49-cac52edd0ddb
```

For the most performant encoding and smallest file sizes, ensure that
[`ffmpeg`](https://www.ffmpeg.org/) is installed on your system. If `ffmpeg`
is installed, Teleport will use it to encode an MP4 video file containing
the session recording.

If `ffmpeg` is not available, Teleport will fall back to encoding an AVI video
file. These recordings take significantly longer to encode and often result in
much larger file sizes.

## Storage

Desktop session recordings contain images of changing sections of the screen,
which means they take up significantly more disk space than SSH or Kubernetes
session recordings. When using async recording modes, ensure that the host
running Teleport's Desktop Service has sufficient disk space to store recordings
...
Reference](../../../reference/deployment/config.mdx) for more information on session

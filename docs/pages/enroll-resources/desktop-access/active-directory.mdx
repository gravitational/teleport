...
>
  ...
  - Run `Get-ADUser -Identity svc-teleport` in PowerShell to confirm the account is enabled.
...
<Checkpoint
  title="Verify the GPO blocks interactive login"
  description="Confirm that the Group Policy Object is configured to prevent the service account from logging in interactively."
>
  Here are some troubleshooting tips:
  - Open **Group Policy Management** and verify the `Block teleport-svc Interactive Login` GPO is linked to your domain.
  - Run `gpupdate.exe /force` to apply the policy immediately.
  - Verify the policy is applied by running `gpresult /r` and checking that the GPO appears in the applied list.
  - Test by attempting to log in as `svc-teleport` on a domain computer—it should be denied.
</Checkpoint>
...
>
  Here are some troubleshooting tips:
  - Run `gpresult /r` on a target Windows host to confirm the GPO is being applied.
  - Verify the Teleport CA is in the NTAuth store: `certutil -viewstore -enterprise NTAuth`.
  - Verify the Teleport CA is in the Root store: `certutil -viewstore -enterprise Root`.
...
>
  ...
  - Test LDAP connectivity: `openssl s_client -connect $LDAP_SERVER:636`.
  - Verify the `der_ca_file` path is correct and the certificate is readable.
</Checkpoint>
...
<Checkpoint
  ...
  description="Confirm that you can connect to a Windows desktop in the domain and see the Windows desktop."
>
  Here are some troubleshooting tips:
  - Check that the desktop appears in the Teleport Web UI under **Resources > Desktops**.
  - Ensure your Teleport user is assigned the `windows-desktop-admins` role.
  - Review the Windows Desktop Service logs: `sudo journalctl -u teleport`.
</Checkpoint>

...
1. Configure a GPO named `Teleport Zero Trust Access Policy` to allow Teleport connections, including:
...
1. Create a new group policy object with the name `Zero Trust Access Policy` by running the following
   command:

   ```powershell
   $GPOName="Zero Trust Access Policy"
   New-GPO -Name $GPOName | New-GPLink -Target $((Get-ADDomain).DistinguishedName)
   ```
...
1. Select the new GPO—for example, `Zero Trust Access Policy`, right-click, then select **Edit**.
...
1. Verify that you have the `Zero Trust Access Policy` group policy object open in the
   group policy editor.
...
To finish configuring the `Zero Trust Access Policy` group policy object, you must
...
1. Open the `Zero Trust Access Policy` group policy object in the group policy editor.

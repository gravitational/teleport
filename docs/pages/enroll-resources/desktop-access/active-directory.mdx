...
- An Active Directory domain that is configured for LDAPS. Because Teleport requires an
  ...
  certificates.
- Each Domain Controller must have a valid certificate with Smart Card Logon EKU included. 
You can find more information on [Microsoft's Website](https://learn.microsoft.com/en-us/troubleshoot/windows-server/certificates-and-public-key-infrastructure-pki/enabling-smart-card-logon-third-party-certification-authorities)
...
<Checkpoint
  title="Verify the service account is created"
  description="Confirm that the Teleport service account exists in Active Directory and has the correct permissions."
>
  Here are some troubleshooting tips:

  - Open **Active Directory Users and Computers** and verify the `svc-teleport` account exists.

  - In PowerShell, confirm the account is enabled:
...
1. Create a new group policy object with the name `Teleport Access Policy` by running the following
   ...
   This command applies the GPO to the entire Active Directory domain.
...
<Checkpoint
  title="Verify the Teleport Access Policy GPO"
  description="Confirm that the Teleport Access Policy GPO is correctly configured with the certificate, smart card service, firewall rules, and RemoteFX settings."
>
  Here are some troubleshooting tips:
  - On a target Windows host, confirm the GPO is being applied by running:
  ...
  - In **Group Policy Management**, confirm the `Teleport Access Policy` GPO is linked to your domain and the Teleport CA appears under **Trusted Root Certification Authorities**.
...
>
  ...
  - Verify the `ldap_ca_cert` value contains a valid PEM-formatted certificate.
...
<Checkpoint
  ...
  description="Confirm that you can connect to a Windows desktop."
>
  Here are some troubleshooting tips:

  - Check that the desktop appears in the Teleport Web UI under **Resources > Desktops**.
...
## Configure Users and Groups to login through remote desktop services
In order for users to login through remote desktop services, they must be granted the
appropriate user rights, and they must be added to the 'Remote Desktop Users' AD group.

### Grant access through remote desktop
...
1. Type in the names of the users and/or groups for which you wish to grant access to desktops in this domain and click **Check Names** to validate.
...
domain to user's domain. For example, if Windows Desktop Service is configured against the root domain
`example.com` and there exists child domain, `dev.example.com`, Teleport can authenticate the user
`user@dev.example.com` for logon to a desktop in the root domain. Similarly, if `example.com` has an outgoing
trust relationship to an external domain `partner.com`, then `user@partner.com` can access desktops in `example.com`.

### Configure permissions for users from foreign domains

In order for users from foreign domains to login to desktops in the target domain, they must be granted
the appropriate user rights and must be added to the "Remote Desktop Users" group in Active Directory.
Follow the steps covered in the previous section ["Configure Users and Groups to login through remote desktop services"](#configure-users-and-groups-to-login-through-remote-desktop-services)
to grant appropriate permissions to foreign users and groups.


### Teleport Configuration for Cross-Domain Login

If the user belongs to another domain within the same forest of the Teleport configured domain,
LDAPS authentication works without further configuration because user's domain is enrolled in
the same forest-wide PKI hierarchy as the root domain. This is not the case for users belonging
to a domain in another forest.

In order to authenticate users from another forest, you must run the following steps for the domain controller(s)
in the external forest:
- [Export the Teleport Certificate](#export-the-teleport-certificate)
- [Publish the Teleport CA to the Active Directory domain](#publish-the-teleport-ca-to-the-active-directory-domain)
- [Publish the Teleport CA to the NTAuth Store](#publish-the-teleport-ca-to-the-ntauth-store)

Additionally, repeat the ["Export your LDAP CA Certificate"](#step-57-export-your-ldap-ca-certificate) step for the domain in
the external forest. Multiple CA certificates can be written to the `ldap_ca_cert` configuration field for the Teleport LDAPS client to trust.
...
By default, Windows Desktop Service assumes that users belong to the domain specified by the
`windows_desktop_service.ldap.domain` field in the service config. In order to login as a user from another
domain, the user's full UPN must be specified (e.g., `jsmith@dev.example.com`) in the `windows_desktop_logins`
section of the role that provides access to windows desktops.
   ...
   spec:
     allow:
       windows_desktop_labels:
         "*": "*"
       windows_desktop_logins: ["jsmith", "jsmith@dev.example.com", "jsmith@partner.com"]
   ```
...
NLA is not supported for cross-domain logins.

...
## A bot failed to renew a certificate due to a "generation mismatch", what does this mean?
...
In particular, note the message `auth API: access denied`. The Teleport auth
server will provide some additional context:
```text
...
renewable certificate, the counter is incremented and stored on the Teleport
Auth Server and embedded in the certificate itself.
...
accident if multiple bots are started using same internal data directory, or if
...
Additionally, ensure the internal data is not being copied to any other nodes.
If you'd like to share certificates between nodes, only copy content from
destination directories.

Once the underlying cause has been addressed, two steps required to reset a
locked bot:
...
Token-joined bots are unable to reauthenticate to the Teleport server once their
certificates have expired. Tokens in token-based joining (as opposed to AWS IAM
joining) are one-time-use, so when the bot's internal certificates expire, it
will not be able to connect.

When a bot's identity expires, certain server-side parameters like the
certificate generation counter must be reset. The simplest way to accomplish
this is by removing and recreating the bot, which purges all server-side data
and issues a new joining token.
...
```code
$ ssh -F /opt/machine-id/ssh_config bob@node.example.com                                                                                                                           timothyb89/v10/machine-id-app-access 
...
This can be verified by viewing the `tbot` logs and looking for the log message
...
... however, the SSH command attempted to login as `bob`.

### Resolution

Ensure the bot identity is allowed to login as the requested user by taking any
of the following actions:

 1. Changing the SSH command to login as an allowed user
 2. Modifying the `access` role to allow the `alice` principal
 3. Adding an additional role granting login via the `bob` principal
...
or more roles. Users should grant these permissions to the bot via a role at
first setup.
...
<Notice type="note">
By default, destinations are granted all roles provided to the bot via
`tctl bots add --roles=...`, but it's possible to grant only a subset of these
roles using the `roles: ...` parameter in `tbot.yaml`.

If permissions are unexpectedly missing, ensure `tbot.yaml` requests the
database role, either by relying on default behavior or adding the role to the
`roles: ...` list.
</Notice>

An example role might look like the following:
```yaml
kind: role
version: v5
metadata:
...
Ensure the bot has been granted a similar role. If desired you can examine bot
roles with `tctl` to ensure the necessary `rules` have been granted:

...
This implementation differs from using Teleport's Application Access to protect
...
- Workload ID works with any AWS client, including the command-line tool but also
  their SDKs
- Using Teleport Application Access to access AWS does not work with Machine ID

...
- A series of "outputs". An output is a set of configuration that specifies
...
For each output provided in `tbot`'s configuration, `tbot` then uses
impersonation to obtain credentials from the Auth Service for the roles
specified in the output's configuration. Once these are fetched, these are

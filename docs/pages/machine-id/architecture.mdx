...
2. For each destination provided in `tbot`'s configuration:
   1. Uses impersonation to obtain credentials from the Auth Service for the
      roles specified in the destination's configuration.
   2. Outputs these credentials in various formats.

...
There are two considerations to make when determing how to deploy Machine ID on
your infrastructure.

### Your infrastructure

The `tbot` binary runs as a container or on a Linux virtual machine. If you run
`tbot` on GitHub Actions, you can use a ready-made [GitHub Actions
workflow](https://github.com/marketplace/actions/teleport-auth).

### Initial authentication method

Machine ID authenticates itself with your Teleport cluster using one of the
following methods:

- **Static join token:** Your Teleport client tool generates a string and stores
  it on the Teleport Auth Service. Machine ID provides this string,
  demonstrating to the Auth Service that it belongs in your Teleport cluster.
- **Platform-signed document:** The platform that hosts `tbot`, such as a
  ...
  that Teleport can verify using the platform's certificate authority. 
...
guide](./platform-guides/linux.mdx) is compatible with most platforms. For

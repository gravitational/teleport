...
2. Create bot user
3. Start Machine ID
4. Use certificates issued by Machine ID
...
[Getting started](https://goteleport.com/docs/getting-started) guide to setup a
Teleport cluster.
...
Machine ID can join with a token or [IAM Method](https://goteleport.com/docs/setup/guides/joining-nodes-aws) on AWS.
...
<Tabs>
  ...
  First, create a IAM method token that specifies the AWS account from which
  the bot can join from.
...
* `token` is the token output by `tctl bots add` command or the name of your IAM method token
* `ca-pin` is the CA Pin for your Teleport cluster output by the `tctl bots add` command
* `destination-dir` is where Machine ID writes the renewable certificates to, this certificate is only used by Machine ID and should not be used by applications and tools
* `data-dir` is where Machine ID writes the short lived certificate to, this certificate should be used by applications and tools
<Details scope={["cloud"]} scopeOnly={true}>
...
OpenSSH configuration in `ssh_config` and `known_hosts` files to make it easy

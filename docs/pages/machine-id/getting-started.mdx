...
Machine ID is available starting from the `9.0.0` Teleport
release.
...
Install Teleport, which includes `tbot`, on the host where you will run Machine ID:
...
```
sudo ssh -F /opt/machine-id/ssh_config root@node-name.example.com
```

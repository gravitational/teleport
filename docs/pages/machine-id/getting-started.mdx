...
### Optional: Configure MachineID as a systemd service

Using the above example a configuration file for Machine ID would resemble the one below. The reference doc for Machine ID configuration can be found [here](./reference/configuration.mdx). 
...
The systemd unit file, placed at `/usr/lib/systemd/system/teleport-bot.service` as an example, would resemble the following.

```code
[Unit]
Description=Teleport Machine ID Auth Service for Plugin
...
```code
$ sudo systemctl enable teleport-bot
$ sudo systemctl start teleport-bot
```

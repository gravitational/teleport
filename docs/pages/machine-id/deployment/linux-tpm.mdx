...
Next, we need to create a Bot in the Teleport cluster. A Bot is an identity
for a machine or group of machines, and like a user, its access is controlled
by roles.
...
spec:
  ...
  #
  # If you don't know what roles your Bot will need yet, don't worry, the
  # access guides will help you determine this.
  roles: []
```

Ensure that you replace `example` with a unique descriptive name for your Bot.
...
Copy down the long hexadecimal string after `EKPub Hash`. This uniquely
identifies this TPM and will be used in the join token.
  ...
  tpm:
    ...
    allow:
      ...
      - description: "example-server-100"
        ...
        # for this check.
        ek_public_hash: "6c5aada1c5abee6d869369a0example2fd2beb41c850d3f0227f029c4fffc4ba"
```

Replace the following:

- `example-bot` with a unique name for the token.
- `example` with the name of the Bot that you created in step 2.
- `6c5aada1c5abee6d869369a0example2fd2beb41c850d3f0227f029c4fffc4ba` with the
   EKPub hash of your TPM.
- `example-server-100` with a human-readable description of the host that the
   TPM resides in.
...
- `example-bot` with the name of the token that you created in step 2.

...
`edit` Cluster Role that is preconfigured in most Kubernetes clusters to give
the bot read and write access resources in all namespaces.
...
The `kubeconfig.yaml` can be passed to `kubectl`. The `kubeconfig.yaml` contains
all the information necessary to automatically configure `kubectl` to connect
to the Kubernetes cluster through the Teleport Proxy. As well as being

...
Teleport Application Access secures access to TCP and HTTP applications.

Whilst humans may access these applications via the browser, machines may need
to access these applications with tools such as `curl`. In order to
authenticate, the machine will require credentials and Machine ID can be used to
produce these credentials.

In this guide, you will configure `tbot` to produce credentials that can be
used to access an application configured in Teleport Application Access.
...
- `tbot` must be installed and configured for your platform. Follow a
...
credentials needed for Application Access. To do this, the `application` output
type is used.

The specific application you want to the credentials to have access to must be
specified using the `app_name` field. In this example, the debug application
(`dumper`) will be used.
...
the Linux user that will be accessing applications.
...
Ensure you replace `grafana` with the name of the application as registered in
Teleport.

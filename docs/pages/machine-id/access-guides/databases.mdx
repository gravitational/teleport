...
However, it is possible to instead create an "authenticated tunnel". This will
automatically attach authentication to any incoming connection to the
tunnel. Whilst this is less secure, it can be necessary if the client you
wish to use with Database Access does not support client certificate
authentication.

To configure a authenticated tunnel, add the `--tunnel` flag to the
`tbot proxy db ...` command in the `machine-id-proxy.service` systemd unit file.

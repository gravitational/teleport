...
In this guide, you will learn how to create host certificates through Machine ID to allow Teleport users (manually or automated through Machine ID) to access OpenSSH servers. This allows for the benefits of
Machine ID with OpenSSH nodes and reduces risk by ensuring that short-lived host certificates on Teleport clusters can be successfully applied.
...
In order to create certificates `tbot` a role that provides the necessary permissions. Roles may restrict which **principals** can be requested using **predicates**,
...
| ----------- | ----------- | ----------- |
| all_equal | All created domains must be equal to the domain. | `all_equal(host_cert.principals, "foo.example.com")` |
| all_end_with | All created domains or subdomains must end with the selected domain. | `all_end_with(host_cert.principals, ".example.com")` |
| is_subset | All created domains must be a subset of the selected domain. This requires the certificate requested by tbot to be be listed with the role's predicate. | `is_subset(host_cert.principals, "foo.example.com", "bar.example.com")` |

...
Using a text editor of your choice, open the `sshd` configuration file generally found within `/etc/ssh/sshd_config` and add the following lines:
...
Once completed, restart `sshd` to apply the new configuration:
...
The connection should successfully complete without any additional credentials needing to be manually entered.

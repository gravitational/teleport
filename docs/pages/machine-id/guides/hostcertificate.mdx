...
In earlier versions of Teleport, host certificates for OpenSSH servers were required to be created through `tctl`. While host certificates created through `tctl`
are still a good option for some use cases, they must be manually rotated, and are not an ideal option where regular certificate rotation is a
requirement for a strong security posture.

Short-lived and regularly rotated host certificates provide a number of security improvements wherever applicable. Regular certificate rotation reduces risk
by ensuring that any potentially stolen certificates are usable for a shorter period of time. Additionally, when coupled with Teleport's [RBAC](https://goteleport.com/docs/access-controls/guides/role-templates/)
support and host certificate **Principals** and **Predicates**, limitations can be applied to both the process of creating host certificates, as well as the
host certificates themselves.
...
In this step, you will be downloading and installing Teleport binaries onto a remote machine. For the purposes of this guide, this machine will be referred to as the `Remote Device` and exists separately
...
- `tsh` is the client tool you will use to log in to the OpenSSH server.
- `tbot` is the Machine ID tool you will use to associate a bot user with a machine.

Machine ID is available starting from the Teleport 9.0.0 release. Download the appropriate Teleport package for your platform from our [downloads page](https://goteleport.com/teleport/download).

Next, log in to your cluster from the remote device using `tsh` to ensure that your device will be able to interact with the cluster:
...
In order to create certificates `tbot` requires a role that provides the necessary permissions. Roles may restrict which **principals** can be requested using **predicates**,
or strings which are used to reduce the scope of permissions for creating host certificates.
By using the format of `where: 'predicate(host_cert.principals, "domain.name")'`, a Teleport user can create a role that will only be able to able to affect the designated domain name
in a manner defined by the selected predicate. A list of supported predicates and their behavior are listed below:
...
`tbot` can join the Teleport Cluster using a number of supported methods. This guide will highlight both the `token` and the [IAM Method](https://goteleport.com/docs/setup/guides/joining-nodes-aws) on AWS.
Note that the following commands may be entered on any Teleport node with `tctl` access to the cluster.
...
The new configuration YAML will require a `destination` field and `ssh_host_cert` subfield that resembles the following, replacing `nodename.my.domain.com` with your own required domain name or IP addresses:

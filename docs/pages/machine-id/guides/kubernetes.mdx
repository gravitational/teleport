...
In this guide, we will demonstrate how to use Machine ID to access a Kubernetes
cluster protected by Teleport from an automated service.
...
Lastly, ensure both `tbot` and `kubectl` are installed on your Machine ID
client system. Refer to our [Installation guide](../../installation.mdx) for
`tbot`, and the [Kubernetes documentation](https://kubernetes.io/docs/tasks/tools/)
for `kubectl`.


## Step 1/3. Create a Machine ID bot and assign permissions

In this example, you'll create a bot named "demo" and assign it a role
...
Be sure to configure the `allow` fields to match your environment:
 ...
 - One (or both) of `kubernetes_groups` and `kubernetes_users` must be specified
   ...
   bot's name by default.

   Note that the specific user and group names are arbitrary and do not need to
   be explicitly created on the Kubernetes cluster. The running Teleport agent
   and matching Kubernetes RBAC rules are all that's needed to trust bot users
   as configured here.
...
opened={true}
>
If you don't have an existing Kubernetes `Role` or `RoleBinding` to use, create
one as follows.

First, create a role by running the following. If an existing
role on the cluster will suffice, this step can be skipped.
...
Next, create a role binding to connect the bot to the new role. You may bind
either the bot's mapped `User` or mapped `Group` to a role, however it must have
been granted the matching `User` or `Group` via a Teleport role.
```code
...
$ kubectl create rolebinding example-rolebinding-group \
  ...
  --role=example-role \
  --group=example
## This binds the new role to the user `alice`
...
Bots may be granted cluster-wide access by instead creating `ClusterRole` and
...
First, on a node with `tctl` access, create a new Machine ID bot using the
Teleport role created in the previous step:

```code
$ tctl bots add example --roles=kube-bot-example
```
...
`tctl bots add ...`. The `kubernetes_cluster` to match the cluster name as

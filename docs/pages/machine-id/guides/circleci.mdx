...
Service or cloud tenant (e.g. `example.teleport.sh`). Replace `my-node` with the
name of the Teleport node that you wish to connect to and replace `ci-user` with
the login you configured in Step 3.

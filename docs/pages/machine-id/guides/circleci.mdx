...
- A running instance of the Teleport SSH Service that you have registered with your Teleport cluster. For instructions on setting this up, see the [Getting Started Guide](../../server-access/introduction.mdx). The SSH node must include a
user you want to grant access to. In this guide, we will call the SSH node `my-node` and the user
...
["contexts"](https://circleci.com/docs/contexts/). Contexts allow an
organization to configure a series of secrets that should be exposed to a
workflow job, and can control which actors are allowed to trigger jobs
...
of Teleport determining which CircleCI jobs should be granted access to the
Teleport cluster.
...
you to configure the context. You will need to determine the ID of the context
to use when configuring Teleport. This value can be found in the URL of the
...
`tele.example.com:443` with the public-facing address of your Teleport Proxy Service or

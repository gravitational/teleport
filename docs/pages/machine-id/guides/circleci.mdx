...
ensuring that you replace `$ORGANIZATION_ID` and `$CONTEXT_ID` with the values
from step 1.
...
`tele.example.com:443` with the public-facing address of your Teleport Proxy.
Replace `my-node` with the name of the Teleport node that you wish to connect
to and replace `ci-user` with the login you configured in step 3.
      ...
      - run:
          ...
          command: |
            tbot start \
              ...
              --join-method=circleci --token=circleci-token \
              --oneshot \

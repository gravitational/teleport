...
ensuring that you replace `$ORGANIZATION_ID` and `$PROJECT_ID` with the values
from step 1.
...
to and replace `ci-user` with the
    ...
    steps:
      ...
      - run:
          ...
          command: |
            ...
            ./tsh -i ./certs/identity --proxy tele.example.com:443 ssh ci-user@my-node "echo $CIRCLE_BUILD_URL >> ~/circle_run_log"
workflows:
  teleport-workflow:
    jobs:
      - teleport

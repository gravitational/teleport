...
Once complete, the Machine ID host configuration will still require an output to be configured within the configuration file. The output specifies the destination where Machine ID will write certificates
...
To update the `tbot.yaml` configuration file, you can add `ssh_host` output configuration settings similar to the following, replacing the `principals` setting with your own required domain name:

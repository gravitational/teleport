...
First, on a node with `tctl` access (such as your local machine), create a file named `role.yaml` with the
following content:
...
the Teleport Proxy makes apps available via subdomains of its public web address. Given an app

...
You will need to be running the Teleport version 10.1.0 or later.
</ScopedBlock>
...
Lastly, ensure `tbot` is installed on your Machine ID client system. Refer to
our [Installation guide](../../installation.mdx) for more information.
...
First, on a node with `tctl` access, create a file named `role.yaml` with the
following content:
...
This role allows access to all apps. The label selector here can be used to
restrict access as desired and supports both wildcards and regular expressions.
Refer to the [Access Controls Reference](../../access-controls/reference.mdx#roles)
...
With Machine ID up and running, you should now have App certificates
written to `/opt/machine-id`. This can be verified by examining the `tbot` log:
...
It's not easily possible to use these certificates with browser-based
applications, however you can use these certificates to access remote APIs using
tools like `curl` as described in the
[API Access guide](../../application-access/guides/api-access.mdx).
...
Per the [Application Access Getting Started Guide](../../application-access/getting-started.mdx),
the Teleport Proxy makes apps available via a wildcard subdomain. Given an app
named `grafana-example` and a Teleport Proxy at
`https://teleport.example.com:443`, the app may be accessed at
`https://grafana-example.proxy.example.com:443`.
...
`tbot proxy app ...` in situations where the public proxy's app endpoints are
unavailable:
...
If your client application requires TLS certificates with a specific file
...
page when attempting to access an app through the public proxy without valid
credentials.

...
It's important to recognise that SPIFFE does not specify how to use SPIFFE IDs
authorization. This gives a high level of flexibility, allowing you to implement
authorization in a way that suits you.
...
The SVID is a document that encodes the SPIFFE ID and some form of cryptographic
proof which allows the SVID to be verified as issued by a trusted authority.
...
The data needed to verify an SVID is known as the trust bundle. This is a set of
certificates belonging to the trusted authorities within the trust domain.

Teleport Workload Identity currently only supports X.509 SVIDs. These are issued
by the Auth Server and signed using the cluster's `spiffe` CA.

### Workload API

### Teleport's Workload Identity

Teleport's Workload Identity is an implementation of a SPIFFE.

Each Teleport cluster acts as a SPIFFE trust domain.

The SPIFFE Workload API is implemented as a service within the `tbot` agent.
The ability to issue SVIDs is granted to the Bot associated with `tbot` through
Teleport's Roles.
...
- [TSH Support](./workload-identity/tsh.mdx): How to use `tsh` with Workload Identity.

...
# Onboarding values are used on first connect for token joining, and used for
# each renewal when using delegated joining methods (e.g IAM).
onboarding:
...
```yaml
# For a memory destination, type will always be `directory`.
type: memory
```
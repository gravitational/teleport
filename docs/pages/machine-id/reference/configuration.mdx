...
# Storage defines the destination where Machine ID internal data is stored.
storage:
    type: directory
...
A destination is something that `tbot` can read and write to. Destinations are
used in two places in the `tbot` configuration:

- Specifying where `tbot` should write and read it's own core state to
- Specifying where an output should write the generated files to

The most common destination type is `directory`, but new types may be introduced
to support other platforms (e.g Kubernetes Secrets).

...
use-cases will likely prefer to connect their own databases to a local proxy
...
| `--proxy`           | The `host:port` of the Teleport proxy to use to access resources. Required.                              |
...
run access the resources through a local proxy. The `tbot proxy` command wraps
`tsh proxy` to provide local proxy functionality for various protocols,
...
Though not recommended, to avoid the need to additional client authentication,
the `--tunnel` flag may be used to perform authentication at the proxy:
...
Refer to the [database guide](../guides/webapps.mdx) for more information on

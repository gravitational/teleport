...
We encourage everyone with an interest in improving Teleport documentation to contribute content, submit issues, and suggest changes.
This guide describes the process for making changes to the documentation whether you are an internal or external contributor.
...
=======
* You have Node.js, version 14.x or newer, installed. 
   Run `node --version` to check whether Node is installed.
   If you need to install or update Node, see [Installing Node.js](https://nodejs.org/en/download/package-manager) for instructions to download and install Node.js using a package manager.

* You have the `yarn` package manager installed.
   Run `yarn --version` to check whether `yarn` is installed. 
   If you need to install `yarn`, run the command `npm install --global yarn` to add it to your local system.
>>>>>>> fec612fdef (Spin through the steps for contributing to doc)
...
1. Initialize the submodules for documentation by running the following command:
    
    ```code
    $ git submodule init
    ```

1. Update the documentation to the latest version from the master branch by running the following command:
...
Different versions of the documentation are organized into the `content` directory of the `gravitational/docs` repository. 
Each subdirectory of the `content` directory includes a `git` submodule for a different branch of `gravitational/teleport`.
>>>>>>> fec612fdef (Spin through the steps for contributing to doc)
...
1. Create a new branch of `gravitational/teleport` for your changes by running a command similar to the following:
    ...
    Before making any changes, note how the content is organized so you know where to find the files to update.
    
    Within the `content/*.*` directory in your branch, there's a `docs` subdirectory. 
    The `content/*.*/docs` subdirectory contains the files and folders used to produce a specific version of the documentation. 
...
You can use `yarn` to run a development server that automatically detects changes to the documentation as you make them and refreshes the page within the browser.
...
If you're an internal contributor with permission to access the `gravitational/teleport` 
repository, you can commit changes and push branches directly to the repository.
If you're an external contributor, you can commit changes to your private branch and push 
private branches to a fork of the `gravitational/teleport` repository.
...
1. Open the [Teleport repository](https://github.com/gravitational/teleport), select your branch as the branch to merge into main, then click **New pull request**.
...
1. Pull all of the changes from the remote repository into your fork by running the following command:
    
    ```code
    $ git rebase origin/master
    ```

1. Push changes from your local branch to the remote fork of the [Teleport repository](https://github.com/gravitational/teleport) by running a command similar to the following:
...
1. Open the [Teleport repository](https://github.com/gravitational/teleport), select your branch as the branch to merge into main, then click **New pull request**.
    ...
    If you don't respond to reviewer feedback in a reasonable period of time, your pull request is likely to be deemed inactive and closed automatically. 

1. Wait for the minimum required approvals, then merge your pull request as soon as possible.

## Backport changes to earlier versions
...
- By using GitHub labels
- By using the make program
- By selecting commit hashes manually
...
<TabItem label="Select commits manually">

If automatic backport methods fail or you're an external contributor, you must backport changes manually.
...
1. Push the changes for the backport to the fork.
    ...
    ```code 
    $ git push -u fork my-branch-backport-11
    ```

...
- Use *sign in* and *sign out* as the preferred terminology when referring to actions involving 
  the Teleport Web UI.
- Use *login* as one word only when it's used as a noun, for example, when using *login* to mean a user 
...
really necessary to stop the reader's forward progress by adding `Admonition`, `Notice`, `Tabs`,
`Details`, or any other special component. In most cases, it's more effective to integrate the 
information in the main body of the text. If you use these types of components at all, use 
them sparingly.

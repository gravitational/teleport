...
- Tutorials are learning-oriented and designed to give a newcomer a positive, successful 
  ...
  that always provide a successful result.
- How-to guides are task-oriented with practical steps to achieve a specific goal. 
  These guides assume that readers familiar enough with Teleport that they know the task
...
- Explanations are the conceptual framework for understanding how or why a system works.
  ...
  how-toguide, or look up in reference material.

For more detailed information and examples of how to structure documentation using this information 
model, see [The Grand Unified Theory of Documentation](https://documentation.divio.com/).

### Structuring content into pages

In general, every page of documentation should have a purpose and self-contained. 
...
fined to cover the same information in more than one place to serve different purposes.
...
Avoid using “we” to unless you have a really good rationale for it. For example, if you 
...
- Avoid using dead language words and phrases even if they are generally accepted as English 
  words in practice. For example:
    ...
    - Instead of “via”, use an appropriate English substitute such as “by”, “through”, or 
      “using”.
...
- Summarize as succintly as possible the content they contain.
- Avoid generic headings like overview and introduction, if possible. While generic 
  heading can be conceptually useful, they add no value to the content or the navigational 
  experience.
...
it's rare you to need to use it.
...
notice. A paragraph highlighted as a note, tip, caution, or warning can be distracting and 
...
really necessary to stop the reader's forward progress by adding a Note, Caution, or Tip 
component. In most cases, it's more effective to integrate the information in the main
body of the text. If you use these types of components at all, use them sparingly! 

**Note**—Indicates neutral or positive information that emphasizes or supplements important 
points of the main text. A note supplies information that may apply only in special cases. Examples are memory limitations, equipment configurations, or details that apply to specific versions of a program.
...
When referring to elements in the user interface, follow the capitalization that is used for the element unless it is rendered as all capitals. 
Because using all capitals to typically consider aggressive in text and most
...
Don't capitalize common terms like server, node, agent, and so on.
...
In general, you should avoid writing about user interface elements. Instead, documentation 
should always focus on what the audience needs to do or wants to accomplish and not what 
is displayed on the screen.

| Element	| How to use it
| ------- | -------------
| button	| Use bold for the button label. Don't include "button" in the description. For example: Click **Submit**.
| checkbox	| Use checkbox, not box or check box, if you need to refer to a checkbox in the user interface. Use select and clear with checkboxes, not turn on and turn off, mark and unmark, check and uncheck, or unselect and deselect.
...
don't use links like click _here_ or see this _article_.
...
Tutorials—and, to some extent, Getting started guides—are designed to give first-time Teleport users
hands-on experience completing a set of explicit instructions. Tutorials should be
as simple and straightforward as possible. Tutotrials must provide a positive learning
experience and a successful outcome with no unexpected behavior or unexplained errors. 
It's okay for a tutorial to include a red herring to illustrate a common misstep but the 
author must provide a sufficient explanation of the error and how to correct it.

### What's in a tutorial?

Tutorial or getting started guides contain much of the same content as how-to guides 
but are focused on acheiving learning objectives:
...
- Learning objectives that sumarize what the reader will accomplish by completing the tutorial.
- Task steps that the reader must follow in dequence to successfully complete the lesson.
- Next steps section can be used to add links to logically related topics, if applicable, or
  to recommend related topics for further reading, such as reference topics related to the
  procedure completed.
...
- **Security** for audiences interested in security protocols, cryptographic primitives, and preventing cyberattacks.
...
- Full product names are treated as proper nouns and capitalized. 
  ...
  In general, avoid capitalizing terms and features. Overuse of capitalization

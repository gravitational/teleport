...
If a partial defines parameters but a page does not define them when including
the partial, the docs engine will leave the corresponding `{{ }}` expressions
untouched. The exception to this is when the partial defines default parameter

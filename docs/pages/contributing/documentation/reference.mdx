...
Opt for MermaidJS diagrams over static images. MermaidJS diagrams are much
easier to edit once they are added to the codebase, since there is no need to
find the original image file. 

It's also possible to apply standard stylesheets to MermaidJS diagrams and
perform static text analysis (e.g., spell checking and linting).
...
information by not seeing a `ScopedBlock`.

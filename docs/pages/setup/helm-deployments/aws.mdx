...
aws:
  ...
  autoScaling: false                              # Whether Teleport should configure DynamoDB's autoscaling.
highAvailability:
...
This can be done manually or delegated to Teleport.

This is step is usually done after a successful Teleport deployment, once you have gathered some data
about Teleport's DynamoDB usage and know what a regular usage looks like and how autoscaling should be tuned.
See [the DynamoDB autoscaling section](../../../setup/reference/backends.mdx#dynamodb-autoscaling) for more details.

To go further you have to know the desired read/write minimum, maximum and target capacity.
If you decide to let Teleport set up the autoscaling proceed as follow:

<Tabs>
  <TabItem label="Using values.yaml">
  Set the following fields in your existing `aws-values.yaml` file and replace the numeric values by yours:

  ```yaml
  aws:
    # [...] already present values under `aws`
    autoScaling: true
    readMinCapacity: 0     # integer
    readMaxCapacity: 1     # integer
    readTargetValue: 0.5   # float
    writeMinCapacity: 0    # integer
    writeMaxCapacity: 1    # integer
    writeTargetValue: 0.5  # float
  ```

  Then perform [a cluster upgrade](#upgrading-the-cluster-after-deployment) with the new values.
  ...
  $ helm upgrade teleport teleport/teleport-cluster \
    --namespace teleport \
    --set aws.autoScaling=true
    --set aws.readMinCapacity=0          `# integer` \
    --set aws.readMaxCapacity=1          `# integer` \
    --set aws.readTargetValue=0.5        `# float` \
    --set aws.writeMinCapacity=0         `# integer` \
    --set aws.writeMaxCapacity=1         `# integer` \
    --set aws.writeTargetValue=0.5       `# float`
  ```
  </TabItem>
</Tabs>
...
See the [high availability section of our Helm chart reference](../helm-reference/teleport-cluster.mdx#highavailability) for more details on high availability.

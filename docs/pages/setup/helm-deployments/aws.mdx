...
If you decide to let Teleport set up the autoscaling, proceed as follows:
...
<Tabs>
  ...
  $ helm upgrade teleport teleport/teleport-cluster \
    --namespace teleport \
    --set aws.dynamoAutoScaling=true \
    --set aws.readMinCapacity=5          `# integer` \
    --set aws.readMaxCapacity=100        `# integer` \
    --set aws.readTargetValue=50.0       `# float` \
    --set aws.writeMinCapacity=5         `# integer` \
    --set aws.writeMaxCapacity=100       `# integer` \
    --set aws.writeTargetValue=50.0      `# float`
  ```
  </TabItem>
</Tabs>

...
- Use `cert-manager` to provision and automatically renew ACME certs (described in [Step 4a](#step-4a-install-and-configure-cert-manager-to-handle-tls)).
...
- Use AWS Certificate Manager to handle TLS termination with AWS-managed certificates (described in [Step 4b](#step-4b-configure-teleport-to-use-acm-to-handle-tls)).
...
<Admonition type="warning" title="Command-line Teleport Application Access does not work with ACM">
  ...
  [Step 4a](#step-4a-install-and-configure-cert-manager-to-handle-tls) above).
</Admonition>

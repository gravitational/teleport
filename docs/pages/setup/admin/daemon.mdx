...
A Linux host where you will install Teleport. The host must be configured to run
userspace programs with systemd. 
...
Restart your Linux host and log in again. You can confirm that Teleport is
running as a systemd service with the following command, which should show an
`Active` status of `active (running)`.

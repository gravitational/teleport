...
See [the SSO documentation](../../entreprise/sso.mdx). Other possible values are `local` and
[`passwordless`](../../access-controls/guides/passwordless.mdx#optional-enabled-passwordless-by-default).
Defaults to `local`.
...
[Disabling local auth is required for FedRAMP / FIPS](../../enterprise/fedramp.mdx#teleport-auth-server).
...
`authentication.lockingMode` controls the locking mode cluster-wide.
See [the locking modes documentation](../../access-controls/guides/locking.mdx#next-steps-locking-modes)

...
You can specify multiple selectors by adding additional list elements.
...
<Admonition type="tip" title="Example">
  You can see [a dynamic application registration example here](../../application-access/guides/dynamic-registration.mdx).
</Admonition>
...
| Type | Default value | Required? |
| - | - | - |
| `list` | `[]` | When `db` chart role is used at least one of `databases`, `awsDatabases`, `dbResources` is required. |
...
<Admonition type="tip" title="Example">
  You can see [a dynamic database registration example here](../../database-access/guides/dynamic-registration.mdx).
</Admonition>

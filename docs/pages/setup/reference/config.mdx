...
teleport:
    ...
    # Invitation token or file path containing token used to join a cluster. it is not used on
    # subsequent starts.  
    # 
    # File path example:
    ...
    # Teleport provides HTTP endpoints for monitoring purposes. They are disabled by default, but you can enable them using the 
    # diagnosis address. See the Teleport Metrics of the Reference Manual (https://goteleport.com/docs/setup/reference/metrics/)
...
auth_service:
    ...
    # Sets a web idle timeout for browser users.  If not specified will default to 10m

...
auth_service:
    ...
    # If a session upload hasn't been uploaded for longer than the grace period,
    # the server will attempt to force the upload to completion. This ensures
    # that all session uploads will be completed, even if abandoned due to a server
    # error. If a session lives longer than the grace period, it will be forced to close.
    #
    # In synchronous session recording modes, the grace period is not used.
    #
    # Default is 24 hours.

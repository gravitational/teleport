...
```
[teleport]
addr = "localhost:3025"    # Equals to teleport-addr root key
identity = "tmp/identity"  # Equals to teleport-identity root key
```
...
Note that start time can be set only once, on the first run. If you want to change the start time frame later, remove plugin state dir which you had specified in `storage-dir` argument.s

## Export print events

Session print events are skipped by default. If you still want to export them, specify `--skip-session-types=''` argument.

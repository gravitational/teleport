...
each service's configuration file. The public address can take an IP or a DNS
name. It can also be a list of values:
...
daemon builds and establishes a reverse tunnel to the main cluster, it will
funnel all traffic through the selected proxy. 
...
within Teleport's environment.

You can also optionally set the `NO_PROXY`

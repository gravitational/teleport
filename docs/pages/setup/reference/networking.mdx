...
- `tsh` in all cases
- Reverse tunnel agents dialing back to proxies when TLS routing is enabled. `proxy_listener_mode` must be set to `multiplex` in `teleport.yaml` to enable TLS routing.

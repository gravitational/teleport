...
When setting up DynamoDB it's important to set up backup. Autoscaling is optional with Teleport,
it is advised to collect some production usage data before enabling autoscaling.

Autoscaling and backup can be configured on DynamoDB directly. We also make setup simpler by allowing AWS DynamoDB
settings to be set automatically during Teleport startup.

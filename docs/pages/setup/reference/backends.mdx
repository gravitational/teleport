...
`sqlite` or `dir`. It's not designed for high throughput and it's not capable of
serving the needs of Teleport's High Availability configurations; as such, it's
only recommended for deployments of moderate size.

A sign that the cluster has outgrown the capabilities of the SQLite backend in a
local directory is the presence of warning messages in the Auth Service's logs
that say `SLOW TRANSACTION`.

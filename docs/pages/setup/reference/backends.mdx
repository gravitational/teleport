...
<Admonition
  ...
  title="NOTE"
>
  The PostgreSQL/CockroachDB backend is currently in Preview mode.
</Admonition>
...
client authentication, configure all Teleport Auth servers to use the backend
...
not exit. Creating a database requires `CREATEDB` privileges for the Teleport

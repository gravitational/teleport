...
`sqlite` or `dir`. The SQLite backend is not designed for high throughput and it's not capable of
...
A sign that the cluster has outgrown the capabilities of the SQLite backend is the presence of warning messages in the Auth Service's logs

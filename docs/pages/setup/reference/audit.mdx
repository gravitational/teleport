...
`auth_service.session_record: node`.
</Notice>
...
|----|-------------------------|
|Best effort (`best_effort`)|Disables recording and keeps the session going on.|
|Strict (`strict`)|Immediatly terminates the session.|

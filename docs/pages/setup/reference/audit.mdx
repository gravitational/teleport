...
| session.data                  | Emitted at the end of a session, contains how much data was transmitted and received during the session. |
...
| session.rejected              | Emitted when a user's attempts to create an session has been rejected due to [session control limits](https://goteleport.com/blog/ssh-session-control/). |
...
| user.password_change          | A non-SSO users password has changed. |

...
you will loose connectivity after the transition `update_servers` -> `standby`. Make sure that all
...
<Admonition type="note">
Clients will temporarily loose connectivity during proxy and auth servers restarts.
</Admonition>
...
It currently does not track the states of the nodes and you can loose connectivity if things go wrong.
</Admonition>

...
<Admonition type="note">
  For cloud, login with a teleport user with editor privileges: ```code # tsh
  logs you in and receives short-lived certificates $ tsh login
  --proxy=myinstance.teleport.sh --user=email@example.com # try out the
  connection $ tctl get nodes ```
</Admonition>

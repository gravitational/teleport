...
- `update_servers`: Servers will reload and would start serving TLS and SSH
  certificates signed by the new certificate authority, but will still accept
  certificates issued by old certificate authority.
...
- **Semi-automatic:** Teleport automatically transitions between phases of the
  rotation after some amount of time (known as a _grace period_) elapses.
...
`update_servers`, they can rollback to the previous certificate authority, and
...
Make sure that all nodes are up to date with the transitions before proceeding.
</Admonition>
...
connectivity to nodes, [rollback](#rollback) to the old certificate authority.
</Admonition>
...
This is the last stage where you have the opportunity to rollback. If you have
lost connectivity to nodes, [rollback](#rollback) to the old certificate
authority.
</Admonition>
...
You can customize grace period and CA with additional flags:
...
because they were still and using the old cert authority. Connectivity to these
...
- How the
  [Teleport Certificate Authority](../../architecture/authentication.mdx#authentication-in-teleport)
  works

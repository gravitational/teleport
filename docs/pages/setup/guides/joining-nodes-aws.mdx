...
You can still use secret tokens to join the cluster, see the
[Getting Started](../../getting-started/linux-server.mdx#add-a-node-to-the-cluster)
guide. The following AWS join methods are meant to make deployment simpler by
eliminating the need to distribute secrets when your Nodes or Proxies are
running in AWS.
</Admonition>

There are two join methods available depending on your use-case.

The EC2 join method:
- is available in Teleport 7.3+ self-hosted.
- is available to any Teleport Node or Proxy running on an EC2 instance.
- only one Teleport Node or Proxy may use the EC2 join method per EC2 instance.
...
The IAM join method:
- is available in Teleport 8.1+ cloud or self-hosted.
- is available to any Teleport Node or Proxy running on an EC2 instance with an
  attached IAM role or with access to IAM credentials.
- no IAM credentials are required on the Teleport Auth server.

## Prerequisites

- Teleport v(=teleport.version=) Open Source or Enterprise.
- A running Teleport cluster (must be self-hosted if using the EC2 join method).
- An AWS EC2 instance with Teleport installed.

## Step 1/4. Setup AWS IAM Credentials
...
IAM policy or permissions are needed, any user or role can call this API.
...
configuration file or environment variables, see
https://aws.github.io/aws-sdk-go-v2/docs/configuring-sdk/#specifying-credentials
for details.
  </TabItem>
</Tabs>
...
<Tabs>
  <TabItem label="EC2 Method">
Nodes will prove that they are running in your AWS account by sending a signed
EC2 Instance Identity Document which matches an allow rule configured in your
AWS Node Joining Token.
...
```
  ...
  <TabItem label="IAM Method">
Nodes will prove that they are running in your AWS account by sending a
pre-signed `sts:GetCallerIdentity` request to the auth server. The Node's
identity must match an allow rule configured in your AWS Node Joining Token.
...
spec:
  ...
  # aws_arn is optional and supports wildcard characters (* and ?)
...
```
  ...
  <TabItem label="IAM Method">
The IAM join method can be used for Teleport services running SSH, Proxy,
Kubernetes, Application, or Database roles.
...
<Admonition type="note">
this section is not necessary when using the IAM join method.
</Admonition>

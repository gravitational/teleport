...
The IAM join method:
- is available in Teleport 8.3+ cloud or self-hosted.
- is available to any Teleport Node or Proxy running anywhere with access to IAM
  credentials, typically an EC2 instance with an attached IAM role.
- no IAM credentials are required on the Teleport Auth server.
- IAM credentials are required on the Nodes, but they do not require any
  permissions. An IAM role with no attached policies is sufficient.
...
sending a pre-signed `sts:GetCallerIdentity` request to the auth server. The
...
`*` (to match any combination of characters) and
`?` (to match any single character.) See the
...
You can check what your AWS identity looks like by calling

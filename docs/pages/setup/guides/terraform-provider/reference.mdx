...
description: Terraform Provider reference
---
...
Add the following configuration section into your `terraform` configuration block:
...
Provider supports following options in it's base configuration:

| Name                    | Type       | Description                                               |
|-------------------------|------------|-----------------------------------------------------------|
|                   `addr`|     string | Teleport auth proxy host:port                             |
...
# <a name="teleport_user"></a>`teleport_user`

User resource allows to manage Teleport users: create, update, delete, assign and revoke roles and permissions.

## `metadata`

`metadata` is a user metadata
...
# <a name="teleport_role"></a>`teleport_role`

Metadata is a role metadata
...
`spec` is a role spec. It consists of two sections: `allow` and `deny`. They are identical and contain the following fields:
...
|           `database_users`| string list | List of databaes users this role is allowed to connect as       |
...
`request` represents matcher for allow/deny restrictions on access-requests.
...
`review_requests` is a matcher for allow/deny restrictions on  access reviews.
...
`options` represents a set of role options.
...
| `request_access`          | string      | Access request stategy (optional|note|always) |
...
# <a name="teleport_provision_token"></a>`teleport_provision_token`

Teleport Github connector resource.

## `metadata`

`metadata` is a provision token metadata
...
`spec` is a provision token spec. It has a single field: `roles` (string list) which is a list of roles associated with the token,  that will be converted to metadata in the SSH and X509 certificates issued to the user of the token.

# <a name="teleport_github_connector"></a>`teleport_github_connector`

## `metadata`

`metadata` is a Github connector metadata
...
`spec` is a Github connector spec.
...
# <a name="teleport_oidc_connector"></a>`teleport_oidc_connector`

Teleport OIDC connector resource.

## `metadata`

`metadata` is an OIDC connector metadata
...
`spec` is an OIDC connector spec.
...
# <a name="teleport_saml_connector"></a>`teleport_saml_connector`

Teleport SAML connector resource

## `metadata`

`metadata` is a SAML connector metadata
...
`spec` is a SAML connector specification.
...
# <a name="teleport_trusted_cluster"></a>`teleport_trusted_cluster`
...
| Name                         | Type        | Description                                                                   |
|------------------------------|-------------|-------------------------------------------------------------------------------|
| `enabled`                    | bool        | Indicates if the TrustedCluster is enabled or disabled.  Setting Enabled to false has a side effect of deleting the user and host certificate  authority (CA). |

...
<Admonition
  ...
  title="IMPORTANT"
>
  The data directory (`/var/lib/teleport` by default) should be empty prior to launching the
  node.  An error will occur if this node had previously joined by another method (e.g. token or IAM) and is now switching to the EC2 method. 
</Admonition>

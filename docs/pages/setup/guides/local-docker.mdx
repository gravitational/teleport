...
You can now [follow this guide from **Step 4** onwards](../../getting-started.mdx#step-44-have-fun-with-teleport) to create your user and log into Teleport or continue below.
...
<Admonition type="tip" title="Tip">
  To recreate `testuser`:

  ```bash
  # Remove the current testuser, password, add token, and OTP
  docker exec teleport tctl users rm testuser 
...
To cleanup your Docker Teleport resources:

```bash
# Cleanup any unused images and containers
docker system prune

# Cleanup any unused images, volumes, and containers
# docker system prune --volumes

# Find the quay.io/gravitational/teleport Teleport IMAGE ID
docker images --all

# Remove the Teleport Docker image

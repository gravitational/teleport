...
Run the `configure` command to generate a sample configuration. Replace `mytenant.teleport.sh` with the DNS name of your Teleport server (Teleport Cloud or self-hosted):

```code
$ docker run -v `pwd`:/opt/teleport-plugin -w /opt/teleport-plugin quay.io/gravitational/teleport-plugin-event-handler:(=teleport.plugin.version=) configure . mytenant.teleport.sh
```

To create a Kubernetes secret that can be used with the Helm Chart later, use the following command:

...
<Tabs>
  ...
  tar -zxvf teleport-event-handler-v(=teleport.version=)-linux-amd64-bin.tar.gz -C .
  ```
  </TabItem>
  ...
  tar -zxvf terraform-provider-teleport-v(=teleport.version=)-linux-amd64-bin.tar.gz -C .
  ```
  </TabItem>
</Tabs>
...
to issue certificate for `teleport-event-handler` user again.

...
a deep dive into how to set up and run Teleport in production.
...
2. Using SSH public/private keys has some limitations. Read why [SSH Certificates are better](https://gravitational.com/blog/ssh-key-management/).
...
This guide will cover how to set up, configure and run Teleport on [AWS](https://aws.amazon.com/).
...
needed to set up a URL for the proxy - we recommend using a subdomain (e.g. `teleport.acmeinc.com`).

### NLB: Network Load Balancer

AWS provides many different load balancers. To set up Teleport, we recommend
...
To add new nodes to a Teleport Cluster, we recommend using a [strong static token](https://gravitational.com/teleport/docs/admin-guide/#example-configuration). SSM can be also used to store the Enterprise license.
...
more than happy to help you architect, set up, and deploy Teleport into your environment.
...
The below command is run on a machine with `kubectl` access to the cluster.
...
The following steps describe how to install and configure Teleport.
...
Modify the security group associated with that instance to allow port `22` inbound, so you can SSH to the instance after it’s running. You will need security group rules
...
Run this on the Teleport EC2 Host, see [Github authentication](../../admin-guide.mdx#github-oauth-20) for more info.
...
#### Testing Teleport and EKS setup
...
We've got a few other resources for Enterprise customers such as our

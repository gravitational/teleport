...
The Teleport Kubernetes Operator is deployed alongside its CustomResourceDefinitions. Once deployed, users
can use a Kubernetes client like `kubectl` or their existing CI/CD Kubernetes pipelines to create Teleport
CustomResources. The Teleport Kubernetes Operator watches for those resources and does API calls to Teleport to
...
cluster. All resources (except the CRDs which are cluster-scoped) are created in the `teleport-cluster` namespace.
...
Create a manifest `teleport-resources.yaml` that describes two CustomResources: a `TeleportUser` and a `TeleportRole`:
...
The operator watches for new resources or changes in Kubernetes. When a change happens, it triggers the reconciliation
...
Helm Chart parameters are documented in the [`teleport-cluster `Helm chart reference](../helm-reference/teleport-cluster.mdx).

See the [Helm Deployment guides](../helm-deployments.mdx) detailing specific setups like running teleport on AWS or GCP.

...
Roles using Kubernetes resources.
...
If Teleport returns some error it will be available in `status.conditions`.
...
If the status is not present or does not gives sufficient information to solve the issue, check the operator logs:

...
- [Issue and govern identities for automated systems](mwi/machine-id/introduction.mdx) using Teleport Machine Identity.
- Replace long-lived API keys and environment secrets that are vulnerable to breaches, mistakes, and exfiltration with short-lived cryptographic identities using x.509 certificates or JWTs via [Teleport Workload Identity](mwi/workload-identity/introduction.mdx).

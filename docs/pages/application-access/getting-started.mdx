...
This can be done by adding the application service to your teleport config, typically located at `/etc/teleport.yaml`.
...
instance, in this case `"http://localhost:3000"`.

Also update  `teleport.example.com` with the public domain of your teleport cluster. If you are a Teleport Cloud customer, use port 443 of your tenant's subdomain, e.g., `mytenant.teleport.sh:443`. Rewriting the headers is necessary for Grafana to recognize the Teleport reverse proxy.

Make sure to update the `app/name` and `app/uri` fields accordingly if you're using your own web application.

Then start your Teleport cluster by running `sudo systemctl teleport start`.

To check and see that the Teleport service has started successfully you can tail the logs with `journalctl -fu teleport`.

...
Create the Teleport Application Service configuration. Replace <Var name="teleport.example.com" /> with
your Teleport Proxy address (`teleport.example.com`) or Teleport Cloud tenant (e.g. `mytenant.teleport.sh`).
...
You can start the Teleport Application Service without configuration file using a
CLI command.  Replace <Var name="teleport.example.com" /> with
your Teleport Proxy address (`teleport.example.com`) or Teleport Cloud tenant (e.g. `mytenant.teleport.sh`).
...
$ sudo teleport app start \
  ...
  --auth-server=https://<Var name="teleport.example.com" />:443 \
  --labels=env=dev
```
...
your Teleport Proxy address (`teleport.example.com`) or your
...
Log in to the Teleport Web Interface at your Proxy Address. All available applications are displayed on the Applications tab.
...
or `https://grafana.mytenant.teleport.sh` for a Cloud tenant. You will need to log in

...
Next, let's create a user to access the application we've just connected. Teleport has a built-in role called `access` that allows users to access cluster resourcesCreate a local user assigned to these roles:
...
Next, let's create a user to access the application we've just connected. Teleport has a built-in role called `access` that allows users to access cluster resources,
along with a built-in role called `requester` that allows users to request resources. Create a local user assigned to these roles:

```code
$ tctl users add --roles=access,requester alice
```
</ScopedBlock>

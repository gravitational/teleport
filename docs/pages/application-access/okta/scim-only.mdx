...
interface between Teleport and Okta on the Okta side. For consistency, both of 
...
If installing from one of the above tarballs onto a self-hosted system, first 
back up your existing binaries then unpack the linked tarball and move the 
contained binaries into place.   
...
You can only install the SCIM integration with the build of `tctl` supplied with
the `v15.3.2-dev.scim.1` build of Teleport. Other builds of `tctl` do not yet
have the necessary commands.
 ...
 * `--role` defaults to `requester` if not supplied

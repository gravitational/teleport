...
![A Synchronized Okta Access List](../../../img/enterprise/plugins/okta/okta-access-list.png)

When first synchronizing an Access List, the owners for the Access List will be set as the
default owners configured during initial Okta integration enrollment, and the initial review date
will be set 6 months from the current date. These fields are modifiable, as well as the owner and

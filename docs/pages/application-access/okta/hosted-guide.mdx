...
If Teleport Identity is enabled, you will show you a guide to configure importing
...
![Set Default Owners](../../../img/enterprise/plugins/okta/okta-access-list-default-owners.png)

Access lists must have owners, so the first step of setting up Okta Access List
Synchronization is setting default owners for our new access lists. You can select
...
![Import Okta User Groups](../../../img/enterprise/plugins/okta/okta-access-list-import-user-groups-all.png)

After configurating default owners, you can customize which Okta User Groups should
be imported. By default, all user groups will be imported. If you would like more
control, you can uncheck the "Import All User Groups" checkbox at the top right of
of the panel and configure multiple filters which will be matched against the group
names.  For more information on filter syntax, please refer to the Import Filters section
below.

<Admonition type="note">
Only Okta User Groups with assignments will be imported as an Access List. If an Okta User
Group has no assignments, it will not be imported until it has assignments.
</Admonition>
...
Application has no assignments, it will not be imported until it has assignments.
</Admonition>
...
to a glob that matches names with a prefix of "app" and a suffix of "salesforce" would
look like `app*salesforce`.

You can also use raw regular expressions so long as the filter string is prefixed with `^`
and ends with `*`. This will support the Google RE2 regular expression syntax as defined
[https://github.com/google/re2/wiki/Syntax](here).
...
see the Okta [Architecture](./architecture.mdx) and [Synchronization with Okta and SCIM](./sync-scim.mdx)
guides.
...
All Access Lists imported by the the hosted integration will remain until they
are deleted by a Teleport Administrator. That is, they will **_not_** be deleted

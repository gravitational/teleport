...
Through the use of the Okta Service, Teleport is now able display and grant
access to Okta applications and user groups. Okta applications can now be
displayed and accessed through Teleport's application access UI, and access
to these applications along with user groups can be managed by Teleport's
RBAC along with access requests.

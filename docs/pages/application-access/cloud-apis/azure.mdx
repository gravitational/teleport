...
(!docs/pages/includes/application-access/azure-cli-intro.mdx credentialMethod="The Teleport Application Service uses Azure user-assigned managed identities to obtain authentication tokens from Azure"!)
...
- An Azure VM where you will run the Teleport Application Service. The Azure VM
  ...
  pod-managed identities feature will deprecate in September 2024.

  For running Teleport Application Service in AKS with Microsoft Entra Workload
  ID, see [Azure CLI Access on AKS with Workload ID](./azure-aks-workload-id).
  </Admonition>

...
1. Create a trust policy document, which authorizes an entity to assume the role
   ...
   assumes the `TeleportAWSAccess` role, then uses that role to assume the
...
### Give the Teleport Application Service permissions to assume roles
...
1. Create a file called `aws-ro-access.yaml` with the following content,
   replacing <Var name="AWS_ACCOUNT" /> with the account number where you
...
1. Retrieve the join token you created earlier in this guide by running the
   following command and copying a token with the `App` type:
...
1. Click the **Launch** button for the AWS Console application, then choose the
   role you would like to assume when signing in to the AWS Console:

   ![IAM role selector](../../../img/application-access/iam-role-selector.png)

1. Click on the role you want to assume and you will get redirected to the AWS
   Management Console, signed in with the selected role. You should see your
   Teleport user name as a federated login assigned to `ExampleReadOnlyRole` in
...
1. Log into the previously configured AWS console app on your desktop:
...
1. Log into the AWS Management Console app you configured earlier:
...
In the Teleport Application Service fails to assume the `ExampleReadOnlyAccess`
...
Now that you know how set up Teleport to protect access to the AWS Management

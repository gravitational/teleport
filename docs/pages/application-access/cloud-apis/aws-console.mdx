...
<Admonition type="note" title="Session Duration">
    ...
    running with temporary security credentials (e.g. EKS's IAM role as a
    service account).
</Admonition>
...
For example, if the Teleport Application Service is deployed in EKS with IAM
role with service account (IRSA) or if the Teleport Application Services

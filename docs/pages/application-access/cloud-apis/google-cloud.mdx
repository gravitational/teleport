---
title: "Protect Google Cloud CLIs with Teleport"
description: How to enable secure access to Google Cloud CLIs.
---
...
- A Google Cloud account with permissions to create IAM roles and service
  accounts, as well as to create IAM role bindings for service accounts and
  projects.
...
- A Google Compute Engine VM where you will run the Teleport Application
  Service. The VM must be running a Linux distribution.
...
account for Teleport users to authenticate as, `teleport-vm-lister`. If you would
like to enable access to any existing service accounts in your Google Cloud
project, you can replace `teleport-vm-lister` with these as you work through the
guide.
...
requests from Teleport users to Google Cloud's APIs. In this step, we will show
you how to configure these permissions before you launch the Teleport
Application Service.

### Create a service account for the Application Service

The Application Service uses a Google Cloud service account to generate signed
tokens for *other* service accounts in order to enable Teleport users to access
Google Cloud APIs. In this section, we will create this service account and
...
predefined "Service Account User" role. Assign `PROJECT_ID` to the name of your
Google Cloud project:

```code
$ PROJECT_ID=<Var name="google-cloud-project" />
$ gcloud projects add-iam-policy-binding ${PROJECT_ID?} \
   --member="serviceAccount:teleport-google-cloud-cli@${PROJECT_ID?}.iam.gserviceaccount.com" \
...
`teleport-google-cloud-cli` service account we created earlier to impersonate
another service account, the one chosen by the user. 

In this section, we will show you how to grant the Application Service
permissions to impersonate the Google Cloud service accounts you would like your
Teleport users to access.
...
Create a service account you will enable your Teleport user to access:

```code
$ gcloud iam service-accounts create teleport-vm-lister \
  --description="Sample service account to demonstrate Teleport" \
  --display-name="teleport-vm-lister"
```
...
$ PROJECT_ID=<Var name="google-cloud-project" />
$ gcloud projects add-iam-policy-binding ${PROJECT_ID?} \
   --member="serviceAccount:teleport-vm-lister@${PROJECT_ID?}.iam.gserviceaccount.com" \
   --role="roles/compute.viewer"
```

#### Enable the Application Service to impersonate your service account

Enable the `teleport-google-cloud-cli` service account to impersonate
`teleport-vm-lister` in order to authenticate user requests. To do so, bind the
`teleport-google-cloud-cli` account to the predefined "Service Account Token
Creator Role" for the `teleport-vm-lister` service account:
...
$ gcloud iam service-accounts add-iam-policy-binding \
 teleport-vm-lister@<Var name="google-cloud-project" />.iam.gserviceaccount.com \
...
At this point, you have created a service account for the Teleport Application
Service and enabled this service account to impersonate the service accounts you
would like Teleport users to access. 

In this step, you will attach the service account you created for the
Application Service to your Google Compute Engine VM, then run the Teleport
Application Service.

### Enable the Application Service VM to access Google Cloud

Now that you have created a Google Cloud service account and attached a role to
it, associate your service account with the Teleport Application Service VM.
...
application called `google-cloud-cli` with the `cloud` field set to to `GCP`.
The Teleport Application Service will forward requests to this application to
Google Cloud.
...
The next step is to authorize your Teleport user to access a Google Cloud
service account and execute Google Cloud CLI commands via Teleport. You will
protect access to the service account using Teleport's RBAC system, where a
user's roles determine which Google Cloud service accounts (if any) they can
access.
...
Assign the `teleport-vm-lister` service account we created earlier (or another
service account) to your Teleport user by running the following command:

```code
$ tctl users update <Var name="teleport-user" /> \
--set-gcp-service-accounts teleport-vm-lister@${PROJECT_ID?}.iam.gserviceaccount.com
```

This command uses the `--set-gcp-service-accounts` flag to add Google Cloud
service accounts to a user. You can assign `--set-gcp-service-accounts` to
multiple service account URIs, separated by commas.
...
For example, you can set a user's `gcp_service_accounts` to `teleport-vm-lister`
by using the following URI, replacing `my-project` with the name of your Google
Cloud project:

```text
teleport-vm-lister@my-project.iam.gserviceaccount.com
```
...
assume the `teleport-vm-lister` service account you created earlier.
...
`teleport-vm-lister` service account:

```code
$ tsh app login google-cloud-cli --gcp-service-account teleport-vm-lister
```
...
Your service account: teleport-google-cloud-cli@my-project.iam.gserviceaccount.com
...
Use the following credentials and HTTPS proxy setting to connect to the proxy:
  ...
  export CLOUDSDK_CORE_CUSTOM_CA_CERTS_FILE=/Users/myuser/.tsh/keys/teleport.example.com/myuser-app/teleport.example.com/google-cloud-cli-localca.pem
...
While you could run `gcloud iam service-accounts create` in a shell earlier in this
guide, after entering the `export` commands printed by `tsh proxy gcloud`, this
command results in authorization issues:
...
`gcloud` commands, view the [Google Cloud

...
- Either a Google Compute Engine VM where you will run the Teleport Application
  ...
  are using a pre-existing VM, it must be running a Linux distribution. You must
  have permissions to attach service accounts to Google Compute Engine VMs.
...
Application Service to a Google Compute Engine VM, then run the Teleport
Application Service.
...
pre-existing virtual machine for the Teleport Application Service or launching
a new one:

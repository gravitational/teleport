...
The Application Service requires its own Google Cloud service account, which it
uses to enable Teleport users to access Google Cloud APIs. It does so by
generating signed tokens for *other* service accounts, which it adds to user
requests before forwarding them to Google Cloud. This way, local Google Cloud
CLI tools have no access to these tokens.

In this section, we will create a service account for the Application Service
...
$ PROJECT_ID=<Var name="google-cloud-project" />
$ gcloud projects add-iam-policy-binding ${PROJECT_ID?} \
   --member="serviceAccount:teleport-vm-viewer@${PROJECT_ID?}.iam.gserviceaccount.com" \
...
Assign the `teleport-vm-viewer` service account we created earlier (or another
service account) to your Teleport user by running the following command:

```code
$ tctl users update <Var name="teleport-user" /> \
--set-gcp-service-accounts teleport-vm-viewer@${PROJECT_ID?}.iam.gserviceaccount.com
```

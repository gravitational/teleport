---
title: Access AWS Management Console and API With Teleport Application Access
description: How to access AWS Management Console and API with Teleport Application Access.
videoBanner: GVcy_rffxQw
---

# AWS Management Console And API Access
...
- Access other AWS applications through Teleport.
...
## Step 9. Using other AWS applications
...
To connect your AWS application, you can start either a local HTTPS proxy or a
local AWS Service Endpoint proxy.
...
<Tabs>
  ...
  Use the following credentials and HTTPS proxy setting to connect to the proxy:
    ...
    AWS_CA_BUNDLE=/Users/sampleuser/.tsh/keys/teleport.example.com/alice-app/teleport.example.com/awsconsole-test-localca.pem
  ...
  `terraform` or `eksctl` supports setting the above AWS credentials and the
  HTTPS proxy using environment variables:

  ```code
  $ export AWS_ACCESS_KEY_ID=8cd7ce58b871425e2e05
  $ export AWS_SECRET_ACCESS_KEY=0e08deb6ceb7538754567286922e7ada75021c40
  $ export AWS_CA_BUNDLE=/Users/sampleuser/.tsh/keys/teleport.example.com/alice-app/teleport.example.com/awsconsole-test-localca.pem
  ...
  SDK for Go v2) or require configuring HTTPS proxy through code (e.g. AWS SDK
  for JavaScript).
  </TabItem>
  ...
  Many tools support overwriting the AWS Service Endpoints through command
  flags or environment variables. For example, to use the AWS Service Endpoint
  with AWS CLI:
  ```code
  $ export AWS_ACCESS_KEY_ID=8cd7ce58b871425e2e05
  $ export AWS_SECRET_ACCESS_KEY=0e08deb6ceb7538754567286922e7ada75021c40
  $ export AWS_CA_BUNDLE=/Users/sampleuser/.tsh/keys/teleport.example.com/alice-app/teleport.example.com/awsconsole-test-localca.pem
  $ aws s3 ls --endpoint-url https://localhost:23457
  ```
  </TabItem>
</Tabs>
To log out of the aws application and remove credentials:

...
app_service:
  ...
  - name: "awsconsole-3rdparty"
    ...
    aws:
      external_id_map:
        "arn:aws:iam::1234554321:role/ExampleUser": "external-id-for-ExampleUser"
```

When showing available IAM roles, Teleport will display only role ARNs that
belong to the specific account.

If the account requires External IDs when assuming its AWS roles,
`aws.external_id_map` can be used to map the External IDs by the corresponding
AWS role ARN. A special key wildcard `"*"` can also be used to default an
External ID for all AWS roles for this application.

...
If TLS certificates are managed by files you can install
via the `https_keypairs` under the `proxy_service`.  Below is an example
for installing the `teleport.example.com` TLS certificate and the wildcard
TLS certificate `*.teleport.example.com`.
```yaml
proxy_service:
  enabled: "yes"

...
for the proxy and a wildcard certificate for its sub-domain.  You can have 
domains as application URIs that are not subdomains provided certificates are installed 
for those domains.
...
If you have obtained certificate/key pairs for your domains they can be provided directly
...
Teleport Cloud manages issuing TLS certificates for application address within
cloud tenants.  Application URIs must be subdomains of the Cloud tenant URI.
For example `grafana.example.teleport.sh` is a valid URI as a application URI for
the cloud tenant `example.teleport.sh`.
</TabItem>
</Tabs>

...
and AAAA queries are matched against `public_addr` of applications registered in available clusters.
If there's a match and the application is registered as a TCP application, VNet responds with a
virtual IP address over which the connection will be proxied to the cluster. In any other case, the
query is forwarded to the default DNS name server used by the OS.

If you want VNet to forward connections to an app that has a custom `public_addr` set, you need
to first update the VNet config in the Auth Server to include a matching DNS zone and authorize the
...
- A TCP application connected to the cluster. In this guide, we'll use the example app from
  [TCP Application Access guide](tcp.mdx).
...
VNet uses `100.64.0.0/10` as the IPv4 CIDR range by default. Each resource is assigned an IP address
from that range just before the first connection is established. If this range conflicts with other
software deployed on devices of your users, it can be changed by setting the `ipv4_cidr_range` field
of the VNet config.
...
app is [a TCP app](tcp.mdx), so it is possible to make those accessible over VNet as well. You'll
need to change `uri` of your application in the Application Service configuration file to use
`tcp://` instead of `http://`. There's also a couple of caveats:

- VNet does not terminate TLS when proxying connections to TCP apps. The application itself needs to
  handle TLS connections.
- Because the Teleport Web UI uses [HSTS](https://developer.mozilla.org/en-US/docs/Web/HTTP/Headers/Strict-Transport-Security),
  the application will not be accessible in browsers over plain HTTP, even with VNet running. Since
  the application needs to be accessible over HTTPS, it must return [a valid cert](connecting-apps.mdx#tls-requirements)
  for the domain it is served on.
- [JWT Token](../jwt/introduction.mdx), [redirect](connecting-apps.mdx#rewrite-redirect) and
  [header rewrites](connecting-apps.mdx#header-passthrough) are not available for TCP apps.

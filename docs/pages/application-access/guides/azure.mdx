...
reverse tunnel, so you can run the Application Service VM in a private network
and prevent unauthorized access to your organization's Azure credentials. 
...
running the Application Service. This lets the Application Service make requests
to the Azure API using those identities.
...
any CLI application that executes commands against the Azure API.
...
CLI application uses to authenticate requests to the Azure API.
...
variables in order to request an authentication token for the Azure API:

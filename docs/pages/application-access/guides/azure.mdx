...
You can use Teleport Application Access to manage access to Azure CLI tools.
This lets you control access to your infrastructure's management APIs using the
same RBAC system that you use to protect your infrastructure itself.
...
from the Azure CLI by requesting access to Azure's APIs under the user's chosen
...
When a user with the `azure-cli-access` role authenticates to the Azure CLI via
...
<Details title="Defining a Teleport role with a static list of identities">

The instructions above show you how to define a user's Azure identities by
configuring each user separately. Another approach is to define a role with
access to specific Azure identities, which means that Teleport users who assume
this role can use those (and only those) identites to execute commands via the
Azure CLI.

In this approach, the `azure-cli-access` role would look like this:

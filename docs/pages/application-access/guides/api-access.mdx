...
>
  ...
  Additionally, note that in most cases clients do not need to specify a CA certificate. The Teleport Proxy is usually configured with a wildcard certificate issued by a public certificate authority such as Let's Encrypt. If your Teleport Proxy has been configured to use a self-signed certificate, you will need to include it in your `curl` command using `--cacert <path>`.
</Admonition>
...
<Admonition type="note" title="CA certificate note">

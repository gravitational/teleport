...
<TabItem label="Teleport Cloud or via Proxy connection">
  If you are using Teleport Cloud or want to have the plugin only 
...
<TabItem label="On Premise">
  On premise where you can  to connect directly through the auth server (`auth.example.com:3025`) you can use the `tls` certificate files.
  ```bash
...
You should now see a new request in Teleport, and a message about the request on Slack. Based on your open source or licensed version you will get CLI or web console instructions on approving/denying the request.

...
from [source](https://github.com/gravitational/teleport-plugins/tree/master/access/slack).

<Notice scope={["oss","enterprise"]} type="tip">
...
Place the generated identity file(s) into `/var/lib/teleport/plugins/slack` where we
 can reference it when [configuring the plugin](#configuration-file).
...
generated in the [export identity](#export-the-access-plugin-identity) section.
...
Set `addr` to  your Proxy address.

Set `identity` to the identity file generated in the [export identity](#export-the-access-plugin-identity) section.
...
message for the access request, depending on the new resolve state.

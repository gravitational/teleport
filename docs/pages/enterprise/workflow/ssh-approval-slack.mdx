...
- Teleport 6.1.0 or newer (or Teleport Cloud)
...
Teleport Plugin use the `access-plugin` role and user to perform the approval. We export the identity files, using [`tctl auth sign`](https://goteleport.com/docs/cli-docs/#tctl-auth-sign).

<Tabs>
<TabItem label="Teleport Cloud or via Proxy connection">
  If you are using Teleport Cloud or want to have the plugin only connect through the proxy ports you must use a identity file.  When you `tctl auth sign` through Teleport Cloud or remotely you must have [impersonation](https://goteleport.com/docs/access-controls/guides/impersonation/) rights on your user's role.

  ```bash  
  $ tctl auth sign --user=access-plugin --format=file --out=access-plugin.pem --ttl=720h
  # ...
  ```  
  The above sequence will result in one PEM encoded files being generated: access-plugin.pem (certificate, private key, and CA certs respectively).  We'll reference the access-plugin.pem later when [configuring the plugins](#configuring-teleport-slack).
</TabItem>
<TabItem label="On Premise">
  On premise you can run this command directly on the auth server (`auth.example.com:3025`) or remotely. If you `tctl auth sign` for another user remotely you must have [impersonation](https://goteleport.com/docs/access-controls/guides/impersonation/) rights on your user's role.
  ```bash
...
<TabItem label="Download">
  ```bash
  $ wget https://get.gravitational.com/teleport-access-slack-v(=teleport.version=)-linux-amd64-bin.tar.gz
...
In the Teleport section, use the certificate(s) you've generated with `tctl auth sign` before. The plugin installer creates a folder for those certificates in `/var/lib/teleport/plugins/slack/` — so just move the certificates there and make sure the config points to them.
...
<TabItem label="Teleport Cloud or via Proxy connection">
  ```toml
  (!examples/resources/plugins/teleport-slack-proxyconnection.toml!)
  ```
</TabItem>  
<TabItem label="On Premise">
  ```toml
...
```bash
$ teleport-slack start
INFO   Starting Teleport Access Slack (=teleport.plugin.version=)-slack/main.go:145
```
...
The messages should automatically get updated to reflect the action you just clicked. You can also check the request status with `tsh`:
...
```bash
# tsh request new --request-roles=REQUESTED_ROLE
Seeking request approval... (id: 8f77d2d1-2bbf-4031-a300-58926237a807)
```

You should now see a new request in Teleport, and a message about the request on Slack. If you are running the community edition you will recieve instructions on approving/denying the request via CLI. The license version allows for approving from the Teleport web console and the CLI.

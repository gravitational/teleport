...
spec:
  options:
    # ...
    # Only allows the contractor to use this role for 1 hour from the time of the request.
    max_session_ttl: 1h
  allow:
    # ...
  deny:
...
the newly requested roles applied. In this case, `contractor` will now have the permission of the `dba`.

<Admonition type="warning">
  Granting a role with administrative abilities could allow a user to **permanently**
  upgrade their privileges (e.g. if the contractor was granted the admin role for some reason).
  We recommend only escalating to the next role of least privilege vs jumping directly to the "Super Admin" role.

  The `deny.request` block can help mitigate the risk of doing this by accident. See the example below.
</Admonition>
...
Teleport now has the ability for users to request additional roles. `tctl` or the Access Requests API makes it easy to dynamically approve or deny these requests.
...
<Admonition
  ...
  title="Note"
>
  Notice that the above role does not specify any logins. If a user's role specifies no logins, Teleport will now generate the user's initial SSH certificates with an invalid dummy login of the form `-teleport-nologin-<random-suffix>` (e.g. `-teleport-nologin-1e02dbfd`).
</Admonition>
...
```bash
sudo tctl request deny --reason='Please be more specific' --annotations=method=cli,unix-user=${USER} 28a3fb86-0230-439d-ad88-11cfcb213193
```

Because automatically generated requests always include all roles that the user is allowed to request, approvers can now specify a smaller subset of the requested roles that should be applied, allowing for sub-selection in cases where full escalation is not a desirable default:

```bash
sudo tctl request approve --roles=role-1,role-3 --reason='Approved, but not role-2 right now' 28a3fb86-0230-439d-ad88-11cfcb213193
```

...
<Details title="Installing via Helm?">
  ...
  See an example for `teleport-jira-helm.yaml` under [Running Teleport in Kubernetes?](#running-teleport-in-kubernetes)
</Details>
...
<Details title="Running Teleport in Kubernetes?">
  <Tabs>
    <TabItem label="Self-hosted" scope={["oss","enterprise","cloud"]}>
...
Use the following command to create a Kubernetes secret from the identity file
...
For installation instructions, see [Installing via Helm?](#installing-via-helm)
</Details>

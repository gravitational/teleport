...
- Jira Server installation with owner privileges, specifically to set up webhooks, issue types, and workflows. This plugin has been tested with Jira Software 8.8.0
...
>	
  By default, [`tctl auth sign`](https://gravitational.com/teleport/docs/cli-docs/#tctl-auth-sign) produces certificates with a relatively short lifetime. For production deployments, the `--ttl` flag can be used to ensure a more practical certificate lifetime. `--ttl=24h` exports a 24-hour token.
</Admonition>
...
The specific type of project you choose when you create it doesn't matter, as long as you can set up a Kanban Board for it, but we recommend that you go with Kanban Software Development — this will reduce the amount of setup work you'll have to do and provide the board out of the box.	

You'll need the project key for the Teleport plugin settings later on. It's usually a three-character code for the project.	
...
The default Jira Software workflow has a different board setup from what Teleport needs, so we'll set up another workflow and assign that workflow to the project board.	
...
- Newly created issues should be in the "Pending" state.	
- It should be possible to move from "Pending" to "Approved".	
- It should be possible to move from "Pending" to "Declined".	
- You can choose to make the workflow strict and restrict moving requests from the "Approved" state to the "Declined" state and vice versa, or leave that flexible. Teleport will only change the request status once, i.e. the first time the request is approved or denied on your Jira board.	

With Workflow editor you can set up who can approve or deny the request based on their Jira user permissions. We won't cover that in this guide as it mostly relates to Jira settings. By default, Teleport will allow anyone who can use the workflow to approve or deny the request.	
...
Run `sudo ./install` within `teleport-jira` or place the executable in the appropriate `/usr/bin` or `/usr/local/bin` on the server installation.
...
```bash
sudo teleport-jira configure > teleport-jira.toml
...
`[http]` setting block describes how the Plugin's HTTP server works. The HTTP server is responsible for listening for updates from Jira, and processing updates, like when someone drags a task from the "Inbox" to the "Approved" column.
...
The log output should look familiar to what Teleport service logs. You should see that it is connected to Teleport, and is listening for new Teleport requests and Jira webhooks.

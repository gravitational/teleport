...
<TabItem label="Download">
  ```code
  $ curl -L https://get.gravitational.com/teleport-access-jira-v(=teleport.version=)-linux-amd64-bin.tar.gz
  $ tar -xzf teleport-access-jira-v(=teleport.version=)-linux-amd64-bin.tar.gz
  $ cd teleport-access-jira
  $ ./install
  ```
</TabItem>
<TabItem label="From Source">
  To install from source you need `git` and [`go`](https://go.dev/) installed.
...
<TabItem label="Docker">
  ```code
  $ docker pull quay.io/gravitational/access-plugin-jira:(=teleport.version=)
  ```
</TabItem>
</Tabs>

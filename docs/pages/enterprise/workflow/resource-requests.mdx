...
All Teleport Nodes in the cluster must be running Teleport `v10.0.0` or greater
in order for Resource Access Requests to be properly enforced.
Nodes on older versions will only do RBAC checks based on roles and may not
respect resource restrictions.
</Admonition>

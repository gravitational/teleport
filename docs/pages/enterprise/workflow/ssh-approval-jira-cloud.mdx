...
When you drag the request task to "Approved" column on Jira, the request will be approved. 

If you drag the request task to the "Denied" column in Jira, the request will be denied.
...
Run `./install` witin `teleport-jira` or place the executable in the appropriate `/usr/bin` or `/usr/local/bin` on the server installation.

...
<Details title="Installing via Helm?">
  ...
  See an example for `teleport-mattermost-helm.yaml` under [Running Teleport in Kubernetes?](#running-teleport-in-kubernetes)
</Details>
...
```console
kubectl create secret generic teleport-plugin-mattermost-identity --from-file=auth_id=auth.pem
```

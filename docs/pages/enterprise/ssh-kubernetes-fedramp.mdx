...
With Teleport 4.0 we have built the foundation to meet FedRAMP requirements to access infrastructure. This includes support for [FIPS 140-2](https://en.wikipedia.org/wiki/FIPS\_140-2), also known as the Federal Information Processing Standard, which is the US government approved standard for cryptographic modules. This document outlines a high-level overview of how Teleport FIPS mode works and how it can help your company to become FedRAMP certified.
...
| [AU-10 Non-Repudiation](https://nvd.nist.gov/800-53/Rev4/control/AU-10) | Teleport audit logging supports both events as well as an audit of an entire SSH session. For non-repudiation purposes, a full session can be replayed back and viewed. |
| [CM-08 Information System Component Inventory](https://nvd.nist.gov/800-53/Rev4/control/CM-8) | Teleport maintains a live list of all nodes within a cluster. This node list can be queried by users (who see a subset they have access to) and administrators at any time. |
...
instructions on how to set up Teleport Enterprise. You'll need to start with the Teleport
...
teleport:
  ...
  # tokens.
  #
  # We recommend using tools like `pwgen` to generate sufficiently random
...
chosen, Teleport will fail to start.  Teleport also checks if the
binary was compiled against an approved cryptographic module (BoringCrypto) and
fails to start if it was not.

- For Teleport Open Source and Enterprise binaries not compiled with BoringCrypto, this flag will report that this version of Teleport is not compiled with the appropriate cryptographic module.
...
- Removes all uses of non-compliant algorithms like NaCl and replaces them with compliant algorithms like AES-GCM.

...
- Be a G Suite Super Admin. As Google Best Practices, we would recommend setting up a separate super admin with 2FA vs using your user.
...
Leave Service account user's roles and admin roles blank.
...
- Privileged role admin who can login as root and is capable of administrating
  the cluster
- Non-privileged dev

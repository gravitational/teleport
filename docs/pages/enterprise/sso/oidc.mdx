...
(also known as OIDC) to issue SSH credentials to specific groups of users.
...
<Admonition type="tip">
  For Google Workspace please follow our dedicated [Guide](ssh-google-workspace.mdx)
</Admonition>
...
Users are only allowed to log in to nodes labeled with `access: relaxed`
...
by setting the variable to `none`, Teleport will override the default `select_account`.

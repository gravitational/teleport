...
First, configure Teleport auth server to use OneLogin authentication via SAML instead of the local
user database. For self-hosted update `/etc/teleport.yaml` and restart the teleport daemon.  
Use the Dynamic resources for Teleport Cloud and any other deployments.

---
title: How To Use SSH Authentication With ADFS
...
First, configure the Teleport auth server to use ADFS authentication instead of the local
...
Also, if you are using dynamic roles (see below), it may be useful to map
...
Let's create two Teleport roles: one for administrators and the other is for
...
```bash
sudo tctl saml export adfs
```

Save the output to a file named `saml.crt`, return to ADFS, open the

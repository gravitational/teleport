...
First, configure Teleport auth server to use GitLab authentication via OIDC instead of the local
user database. For self-hosted update `/etc/teleport.yaml` and restart the teleport daemon.  
Use the Dynamtic resources for Teleport Cloud and any other deployments.

...
SSH credentials to specific groups of users with a SAML Authentication Connector. When used in combination with Role-Based Access Control (RBAC) it allows SSH administrators to define policies
like:
...
```bash
sudo tctl create azure-connector.yaml
```
...
>
  ...
  Alternatively, you can generate your key-pair when creating a connector using `openssl`:
...
We are going to create two roles:

- Privileged role `admin` who can log in as root and is capable of administrating
  the cluster
...
Devs are only allowed to log in to nodes labeled with the `access: relaxed`
...
<Admonition
  ...
  title="Tip"
>
  This is the Azure Active Directory Premium feature and requires a separate license.
...
Start with generating a public/private key and a certificate. You will configure Azure Active Directory with the public certificate and configure Teleport with the private key.
...
```bash
sudo tctl get saml --with-secrets > azure-out.yaml
```
...
```bash
sudo tctl create -f azure-out.yaml 
```

### Activate token encryption

- Navigate to the "Token Encryption" section
...
Example of a user being denied due as the role `clusteradmin` wasn't set up.

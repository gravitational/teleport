...
Manually pasting XML into the `entity_descriptor` field is error prone. We recommend using the `tctl sso configure saml -e entity_descriptor.xml ...` command instead.
Alternatively to a static XML file you can also provide URL where the XML file can be found. In this configuration Teleport will dynamically fetch its contents on each start. This URL should be provided via `entity_descriptor_url` field, which will be used by `tctl sso configure saml` command if the argument to the `-e` flag is URL.
We recommend a configuration utilising a fixed XML file.
...
- Double-check the role mapping in your connector definition for possible typos.
- Verify the configuration on your IdP's side (e.g., callback URLs, claim names, and scopes).

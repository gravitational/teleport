...
- `ping` (SAML and OIDC): Required for compatibility with Ping Identity (including
  PingOne and PingFederate). For SAML compatibility when "Enforced Signed Authn Requests" is enabled in
  the application settings. For OIDC compatibility to use client secret authentication method to avoid duplicate client ids.

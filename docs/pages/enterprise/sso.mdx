...
- `ping` (SAML): Required for compatibility with Ping Identity (including
  ...
  the application settings. (OIDC): Required for compatibility with Ping Identity (including
  PingOne and PingFederate) to use client secret authentication method to avoid duplicate client ids.

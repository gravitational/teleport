...
- Review the below log information and the troubleshooting sections for the above guides.
...
Servers will show if a user's roles include a matching server label that includes logins.
Make sure the user's assigned roles have populated `logins` and match to the `node_labels`.
In this example a user will have `ubuntu` and `debian` logins with a server that has `env: dev` tags.

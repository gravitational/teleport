...
Next, download the systemd service unit file from [examples directory](https://github.com/gravitational/teleport/tree/(=teleport.version=)/examples/systemd)
...
The easiest way to start Teleport Enterprise quickly is to use `docker-compose` with our [`teleport-ent-quickstart.yml`](https://github.com/gravitational/teleport/blob/(=teleport.version=)/docker/teleport-ent-quickstart.yml) file:
...
$ curl -Lso teleport-ent-quickstart.yml https://raw.githubusercontent.com/gravitational/teleport/(=teleport.version=)/docker/teleport-ent-quickstart.yml

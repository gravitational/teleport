...
```bsh
$ sudo tctl users add --roles=access joe --logins=joe,ubuntu,ec2-user
...
```bsh
# SSH into any host behind the proxy (joe user should already exist on the node):
...
```code
$ docker exec teleport tctl users add testuser --roles=access --logins=root,ec2-user=ubuntu
```

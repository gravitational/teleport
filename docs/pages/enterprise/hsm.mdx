...
- Teleport v(=teleport.version=) Open Source or Enterprise (self-hosted).
...
7. Install the AWS CloudHSM Client SDK 3 by following
...
## Step 3/3. (Re)start Teleport Auth

If this is a new auth server starting a brand new cluster, HSM keys will be
automatically generated at startup and no further steps are required. Check the
teleport logs for `Creating new HSM key pair` to confirm that the feature is
working. You can also check that keys were created in your HSM.
...
[Next Steps](#certificate-rotation-with-hsm). The auth server will not perform

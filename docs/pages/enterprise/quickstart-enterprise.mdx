...
- **Proxy servers** route client connection requests to the appropriate node and serve a
  Web UI can also be used to log into SSH nodes. Every client-to-node
...
- Mismatched tokens, i.e. "auth_token" on the node do not match "tokens/node" value on the auth server.

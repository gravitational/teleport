...
listening on  on [TCP/IP ports](../admin-guide.mdx#ports). On _auth.example.com_, it should
...
<Admonition type="note" title="Note">
See the [Kubernetes Guide](../kubernetes-access.mdx) and [Application Guide](../application-access.mdx) for enabling access to additional resources.
</Admonition>

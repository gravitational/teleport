...
that do not require 24/7 access or a guaranteed SLA. 
...
There is no option to provide your own key at the present time.
...
There is no way to provide your own bucket at the present time.
...
If you have a local file `/etc/teleport.yaml` on your machine `tctl` will attempt to use the local cluster. Prefix your commands with `TELEPORT_CONFIG_FILE=` so it will  not attempt to use that Teleport configuration file.

```bash
$ TELEPORT_CONFIG_FILE= tctl tokens add --type=node
```
...
We'll work on resolving any security issues that're identified and
publish the results in Q1 2021.
...
Our current SOCII Type2 report covers our organization, Teleport Open Source and Enterprise versions.
...
certificates for every customer. 

It is not possible to upload a custom

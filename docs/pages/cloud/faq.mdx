...
## Can I configure Teleport Cloud to only allow connections from a whitelist of IP addresses?
...
We're currently researching whether this can be done, so please contact support.

## Can I retrieve diagnostics from my hosted cluster?

We currently don't expose any metrics interfaces for a tenant.

For our own metrics collection, we're rolling out mTLS, so that only authorized clients may collect or scrape metrics from the running instances. 
So we would need to extend our certificate management to external clients.

Additionally, we would need to allow all components of a tenant to be individually addressable and reachable from outside our network (so a scraper can scrape the individual targets). 
This could allow those individual components to be selectively attacked, if an adversary is able to address requests to specific targets within our network.

## Is it possible to enable proxy recording mode?

No, having the proxy man-in-the-middle connections to record the session is disabled for cloud customers.
...
S3 are done so using the in-transit and encryption at rest provided by AWS. Each tenant has it's own credentials that isolate it to interacting with only it's own data.

...
S3, are established using encryption provided by AWS, both at rest and in transit. Each tenant has it's own credentials that isolate it to interacting with only its own data.

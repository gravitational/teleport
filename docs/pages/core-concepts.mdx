...
description: Learn the the key components that make up Teleport.
---
...
host](./try-out-teleport/linux-server.mdx).
...
infrastructure from the public internet without the need for a VPN.  The
Proxy Service provides connectivity for users, machines and resources
within the cluster.

It establishes reverse tunnels to the **Teleport Auth Service** for **Teleport
...
You can run multiple Proxy Service instances in a cluster for high availability.

The Proxy Service, and all Teleport Services, can run on the public internet,
isolated networks or even air-gapped infrastructure. You can also configure 
clients to bypass Proxy Service instances and connect to resources with 
Teleport-issued certificates directly. 
...
All Teleport Services, except for Teleport SSH Service, support running in a 
high availability configuration for multiple connections to the same resources.

### Teleport Application Service

Proxies HTTP and TCP traffic to user-configured endpoints, e.g., internal web
applications or the AWS, Azure, and Google Cloud APIs.
...
An SSH server implementation that allows users to open SSH sessions and
execute commands on remote machines while taking advantage of 
Teleport's built-in access controls, auditing, and session recording.
...
An authentication connector (also known as an auth connector) is a 
**configuration resource** that allows users to authenticate
to Teleport via a Single Sign-On (SSO) solution. 

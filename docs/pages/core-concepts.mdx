...
A **trusted cluster** consists of a **root cluster** and one or more **leaf clusters** 
that trust the root cluster certificate authority. The trust relationship between the
root and leaf clusters enables users authenticated in the root cluster to access resources 
...
For an overview of the trusted clusters architecture, see [Trusted Cluster 
Architecture]](./architecture/trustedclusters.mdx).

...
It establishes reverse tunnels to **Teleport Services**, enabling them to reach the
**Teleport Auth Service** and allowing users to access those services — all without
requiring open inbound ports. This means that, in the Proxy
...
Service](./enroll-resources/application-access/application-access.mdx).
...
The Jamf Service integrates with Jamf to enroll trusted devices for macOS endpoints. 
...
[Read more about the Debug Service](zero-trust-access/management/diagnostics/troubleshooting.mdx#step-13-enable-verbose-logging).
...
Unlike other **Teleport services**, Machine & Workload Identity runs via the
`tbot` binary, rather than the `teleport` binary.
...
[Read the documentation](installation/self-hosted/self-hosted.mdx) on
...
in the leaf cluster. The root and leaf cluster operate independently with their own
...
see [Configure Trusted Clusters](zero-trust-access/management/trustedclusters.mdx). For an overview of the architecture used in a trusted cluster relationship, see [Trusted Cluster Architecture](reference/architecture/trustedclusters.mdx).

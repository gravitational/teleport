...
Here are the core concepts describe a Teleport deployment. You will see these
terms often when setting up and managing Teleport, so you should get familiar
with them before following other pages in the documentation.
...
infrastructure, such as Kubernetes clusters and Windows desktops.
...
services from a single `teleport` binary on a [Linux
host](./deploy-a-cluster/open-source.mdx).
...
a backend, which it uses to store **configuration resources** and the
certificate authorities' private keys. **Teleport Services** are stateless
and interact with the Auth Service via a gRPC API.
...
The **Teleport Proxy Service** allows for secure access to resources in a your
...
A single running `teleport` binary can run one or more **Teleport Services**,
...
Services besides the **Teleport SSH Service**, an agent  and can enable access
to multiple resources, and can run on a separate host from the resources it
enables access to. All agents must run in the same network as their target
resources.
...
Teleport allows for three kinds of users:

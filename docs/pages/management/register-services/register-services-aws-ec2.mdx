...
services and Proxy Service instances to join your Teleport cluster without sharing
...
your Teleport Auth Service. No IAM credentials are required on the services or
Proxy Service instances.

<Details
opened
title="Other AWS service joining methods"
...
You can also configure services running in AWS to join a cluster via **secret
tokens**, which is useful when you don't want to rely on AWS-specific APIs. Read
more in the following guide:
[Adding services to the cluster](../admin/adding-nodes.mdx)
...
- An AWS EC2 instance to host your Teleport service, with the Teleport binary
...
- An AWS EC2 instance to host a Teleport service, with the Teleport binary
  ...
  (`/var/lib/teleport` by default). Remove the data directory if this instance
  has previously joined a Teleport cluster.
...
## Step 2/4. Create the AWS service joining token
...
configured in your AWS service joining token.
...
spec:
  ...
  # cluster.
  #
  # When launching your first service using the EC2 join method, you may need to
...
## Step 4/4. Launch your Teleport service
...
>
  ...
  launching the service. If this service had previously joined by another method (e.g.
...
In order for Teleport services to join from EC2 instances in AWS accounts other
...
When creating the AWS service joining token, include an allow rule for each foreign

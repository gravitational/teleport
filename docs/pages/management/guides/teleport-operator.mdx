...
In order to elect a leader, the Operator does not support multiple auth pod replicas that are not in the same Kubernetes
...
or loss of access to the Teleport Web UI, confirm where your auth pods are deployed.  All Operator sidecars connecting to the same
shared backend must be within the same teleport-cluster deployment and namespace to support the leader election process.
...
Kubernetes cluster or namespace.  Then redeploy the auth pods.  When the Operator starts, it deletes the cached information and recreates all required resources.

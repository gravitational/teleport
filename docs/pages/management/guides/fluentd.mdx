...
recommend starting the Teleport plugin daemon via an init
system like systemd. First copy `teleport-event-handler` to `/usr/local/bin`. Second
update the `teleport-event-handler.toml` file to use fully qualified paths and copy into `/etc/`.
...
Save this as `teleport-event-handler.service` in either `/usr/lib/systemd/system/` or

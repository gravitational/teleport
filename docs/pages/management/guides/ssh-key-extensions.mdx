...
```code
$ curl 'https://teleport.example.com/webapi/auth/export?type=user'  | sed 's/^cert-authority //g'
```
...
```code
$ curl 'https://mytenant.teleport.sh/webapi/auth/export?type=user'  | sed 's/^cert-authority //g'
```

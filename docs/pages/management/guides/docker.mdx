...
This docker installation includes just the Teleport Proxy and Auth Service.  You can try
accessing a server through Teleport by installing the Teleport SSH Service
on a Ubuntu docker container. Here are the steps.

First start a Ubuntu image:
```code
...
Open another terminal windows and confirm the node was added
...
Teleport provides a debug version that includes a Busybox shell. These are not intended

...
secrets when running in the same Kubernetes cluster than the Auth Service.
...
## Step 1/5. Create the Kubernetes join token
...
spec:
  ...
  # - "Kube" for Kube Service
  # - "App" for Application Access Service
  # - "Db" for Database Access Service
...
Kubernetes join tokens can be used by any non-Auth Teleport service:
Kubernetes, Application Access, Database Access, Windows Desktop, ... In this
guide, we restrict the token to Application Service joining.

It is not recommended to use a single token that can join everything. You should
restrict the token to the roles used by the joining instance. e.g. a Teleport
instance running both the App Service and DB Service should use a token with
`roles: [App, Db]`.
...
## Step 3/5. Configure your Application Access Service


Configure the `teleport-kube-agent` chart to deploy Teleport instances running
the Application Access Service by creating a `values.yaml` file with the
following content:
...
## Step 4/5. Deploy the Application Access Service
...
token mounted.
[The `teleport-kube-agent` chart](../../reference/helm-reference/teleport-kube-agent.mdx)
will take care of this by default.

Deploy the Teleport Application Access Service by running the following command:

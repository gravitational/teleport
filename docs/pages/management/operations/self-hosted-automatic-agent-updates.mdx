...
- `tctl` admin rights
- either:
  - a public S3 bucket, GCS bucket
  - a webserver reachable from all agents with valid TLS certificates
...
update to the provided version if possible. If the update is critical, updaters
will ignore the maintenance schedule and update as soon as possible.
...
Save the version server URL as it will be required later to configure the agent
updaters.
...
Create the following `cmc.yaml` file to allow maintenances on Monday, Wednesday

...
```code
$ apt-cache policy | grep teleport
# or
$ yum repolist | grep teleport
```

If the `teleport-ent` package is not found, add the appropriate repository and reinstall Teleport:

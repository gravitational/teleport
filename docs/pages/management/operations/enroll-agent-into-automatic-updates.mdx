...
If the agent was not installed using the system package manager, follow the instructions on the
[Teleport Enterprise Cloud Downloads page](../../choose-an-edition/teleport-cloud/downloads.mdx)
to add the teleport package repository and reinstall teleport. To check if teleport
was installed using the package manager:
```code

...
The `tctl get all` command must be run on a Teleport Auth Service host or existing user records will cause a permission error.

</Notice>

Teleport user secrets are only available when running `tctl` commands
on a Teleport Auth Service machine. 

As a workaround, retrieve resources
individually with the `--with-secrets` parameter except for users. Local user secrets
including passwords will need to be reset after being created without secrets.
...
records identifiable with `cluster_name` values that match to the leaf cluster names.
</Notice>
...
<Notice>
The `tctl get all` must be run on a Teleport Auth Service host 

...
```
<Notice>
The `tctl get all` is not fully supported remotely as retrieving user
resources with secrets is not allowed. As a workaround, retrieve resources
individually. Local user secrets including passwords would need resetting
after creating without secrets.

All trusted cluster certificate resources should be removed for use
in a bootstrap or you will receive an error.
</Notice>
...
<Notice>
The `tctl get all` is not fully supported remotely as retrieving user
resources with secrets is not allowed remotely. As a workaround, retrieve resources
individually. Local user secrets including passwords would need resetting after
creating without secrets.

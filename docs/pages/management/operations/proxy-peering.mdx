...
Teleport Proxy instances must be able to reach each other over the network on port
...
This ensures an agent is still reachable if one of the proxies it is connected to is not available.

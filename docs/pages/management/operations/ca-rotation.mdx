...
Joined OpenSSH servers will require reissuing and updating the HostKey
and HostCertificates installed.
...
configured in group policy for Active Directory integrations. For local Windows
user access the Teleport Windows Auth Setup must be run to update the trusted CA
certificate.

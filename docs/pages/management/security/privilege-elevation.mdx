...
- Restrict access to any command that may be run as part of dynamic labels.
- Follow the **principle of least privilege** in creating and assigning roles. 
  ...
  assign users the preset `access,editor` roles, which give them permission to access and edit 
...
Privileged users could manipulate the Teleport agents in ways that affect your authorization controls.
...
To prevent elevated privilages from resulting in security incidents, you should adhere to the 

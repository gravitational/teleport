---
title: Monitor Teleport Audit Events with Datadog
...
to establish a secure channel. While we can use this same configuration to connect
directly to [Splunk](./splunk.mdx) or [Elastic](./elastic-stack.mdx), Datadog uses
an API endpoint with keys, so we will use Fluentd as an intermediary.
...
- A server, virtual machine, Kubernets cluster, or Docker environment to run the
...
- Fluentd version v(=fluentd.version=) or greater. The Teleport Event Handler
  ...
  system, or use with a fresh setup. The instructions below assume a local Docker
...
$(pwd)/fluent.conf:/fluentd/etc/fluent.conf --entrypoint=/bin/sh -it --tty  fluent/fluentd:edge
...
- Adjust the `host` value to match your Datadog site. See their [Log Collection
  and Integrations](https://docs.datadoghq.com/logs/log_collection/?tab=host)

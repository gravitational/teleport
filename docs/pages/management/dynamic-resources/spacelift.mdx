...
{/*Uncomment when we fix issues with Mermaid support:
...
<Figure
  ...
  height="200"
>
  ![Example](../../../img/management/spacelift/spacelift.svg)
</Figure>

This guide will show you how to set up the GitOps platform Spacelift with the
Teleport Terraform provider. While following this guide, you will create a no-op
Teleport user and role in order to demonstrate using Spacelift to create
dynamic resources. 
...
- Upload a Teleport identify file to the GitOps platform that the GitOps
  platform will use to authenticate as the Teleport user and role you created.
...
After getting familiar with this guide, read our
[documentation](../machine-id/getting-started.mdx) to get started with Machine
ID. You will need to run your own compute workload to upload Teleport identity
files to Spacelift using the `spacectl stack environment mount` command of the
...
Commit the change and merge it to your `main` branch and push to your remote
repository (or use a pull request).

## Step 2/4. Create a Spacelift stack

Visit Spacelift, then click **Stacks > Add stack**. 
...
Click **Stacks > Teleport**. Click the **Environment** tab, then **Edit**. Set
the dropdown menu that follows to **Mounted file**, and the path to
`/mnt/workspace/auth.pem`.
...
In your clone of the GitHub repository you chose in order to follow this guide,
check out a branch from your main branch and add the following to `main.tf`:
...
Commit your changes and open a pull request against your repository. (Do not
merge it just yet.)

Click **Stacks > Teleport > PRs**, then click the name of the PR you opened.
...
Verify that Spacelift has created the new user and role:

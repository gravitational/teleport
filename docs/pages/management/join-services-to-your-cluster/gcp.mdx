...
- A GCP VM to host a Teleport service, with the Teleport binary installed.
...
Create the following `token.yaml` with a `gcp.allow` rule specifying your GCP
...
The GCP join method can be used for Teleport processes running the SSH, Proxy,

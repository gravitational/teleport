...
When a user tries to connect to any resource in the leaf cluster, the connection goes through the 
reverse tunnel using the root cluster's proxy.

## Who uses trusted clusters?

Most organizations don't need to configure trusted clusters. In most cases, you can enroll
resources and add reverse-tunnel proxies without creating a new cluster. Typically, a single 
teleport cluster can scale to hundreds of thousands of connected resources.

However, there are a few specific scenarios where trusted clusters can be paricularly useful.
...
After users sign in, they can be recognozied and trusted on the leaf clusters based on informtion in their
...
- Verifies that the certificate signature matches one of its trusted clusters.
...
Users from the leaf cluster can't see or connect to servers in the root cluster.

## Role relationships in a trusted cluster

As noted in [How it works](#how-it-works), leaf clusters are autonomous. They have their own state, roles, 
...
- Access to **two** Teleport cluster instances in your environment:

  - Teleport Community Edition or Teleport Enterprise if you manage the Teleport clusters on your own network.
...
- A Teleport agent that is joined to the cluster you plan to use as the **leaf cluster**.
  For information about how to launch a Teleport agent in a cluster, see 
...
your Teleport username:
   ...
   Replace `leafcluster.teleport.sh` with the Teleport leaf cluster domain and 
...
your Teleport username:
   ...
   Replace `rootcluster.teleport.sh` with the Teleport root cluster domain and 
   `myuser` with your Teleport username.
...
following content:
   ...
   version: v2
   metadata:
     name: rootcluster.teleport.sh
   spec:
     enabled: true
     token: (=presets.tokens.first=)
     tunnel_addr: rootcluster.teleport.sh:443
     web_proxy_addr: rootcluster.teleport.sh:443
     role_map:
       - remote: "access"
         local: ["visitor"]
   ```
   ...
   If you aren't sure of what values to set for the `tunnel_addr` or `web_proxy_addr`,
   see [Commands to lookup cluster addresses](commands-to-look-up-cluster-addresses.mdx).
   ...
   syntax, see [Role mapping  in trusted clusters](role-mapping-in-trusted-clusters).
...
running a command similar to the following:
   ...
   ```code
   $ tsh ls --cluster=<Var name="leafcluster.teleport.sh" />
   ```
...
## Role mapping in trusted clusters
...
TO remoe a leaf cluster completely:
...
or invalid HTTPS certificate, you are likey to see an error similar to the following: 
...
on the leaf cluster, it might indicate problems with their role ssignment, role mapping, or allowed logins.

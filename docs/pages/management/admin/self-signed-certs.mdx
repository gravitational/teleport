...
- Teleport is started with the `--auth-server` flag pointing to the Proxy
  Service endpoint:

  ```text
  teleport [app | db] start --auth-server=tele.example.com:443
  ```
...
- passing the `--auth-server` flag directly, as in:
  ...
  - `tctl --auth-server=tele.example.com:443`
...
flag](../../connect-your-client/teleport-connect.mdx#skipping-tls-certificate-verification).

## Further reading

- [Configuring Teleport TLS Certs](../../try-out-teleport/linux-server.mdx#configure-teleport)

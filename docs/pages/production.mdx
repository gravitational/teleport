...
- What does your existing network configuration look like?
  ...
  - Will the TLS certificate stop at a load balancer or at Teleport's Proxy Service?
  - Do you require port access only on `443` for web and desktop clients?
...
Teleport services listen on several ports. This table shows the default port numbers. See the [YAML reference](config-reference.mdx) for setting listen ports and public addresses for the services.

| Port | Service | Description | Ingress | Egress |
| - | - | - | - | - |
| `3080` | Proxy | Web port clients and nodes connect to. Optionally change to `443` that many of the Teleport examples demonstrate. Used to authenticate `tsh` users and web users into the cluster.  | Allow inbound connections from HTTP/HTTPS, nodes, and SSH clients with websockets allowed. | Allow outbound connections to HTTPS, nodes, and SSH clients. |
| `3023` | Proxy | SSH port clients connect to after authentication. Required for Teleport's `tsh` desktop client communication. | Allow inbound traffic from SSH clients. | Allow outbound traffic to SSH clients. |
| `3022` | Node | SSH port to the Node Service. This is Teleport's equivalent of port `22` for SSH.  Not required if a node is using a reverse tunnel configuration. | Allow inbound traffic from the proxy host. | Allow outbound traffic to the proxy host. |
| `3025` | Auth | SSH port used by the Auth Service to serve its Auth API to the Proxy and nodes in a cluster. | Allow inbound connections from all cluster nodes and proxy. | Allow outbound traffic to cluster nodes to the auth host. |
| `3024` | Proxy | Port used to create "reverse tunnels" from behind-firewall environments into a trusted proxy server. | Allow inbound traffic to nodes. | Allow outbound traffic to nodes to the proxy host.  |
| `3026` | Kubernetes | Port used for `kubectl` to access Teleport | Allow inbound traffic to kubectl clients.|  Allow outbound traffic to kubectl clients to the proxy host. |
| `3036` | Database | Port used for `mysql` clients to access Teleport | Allow inbound traffic to MySQL clients.| Allow outbound traffic to MySQL clients to the proxy host. |
| `5432` | Database | Port used for PostgreSQL clients to access Teleport.  This port is typically used when a Application Load Balancer is used that maintains the TLS certficate for the Teleport proxy public address. | Allow inbound traffic to PostgreSQL clients.| Allow outbound traffic to PostgreSQL clients to the proxy host.|
...
- [ACM](./setup/deployments/aws-terraform.mdx#acm) on AWS

...
description: How to configure SSH and Kubernetes access with SSO on AWS, GCE, and your infrastructure.
h1: Production Guide
---
...
We have a detailed [installation guide](installation.mdx) which shows how to install all available binaries or [install from source](installation.mdx#installing-from-source). Reference those guides to learn
the best way to install Teleport for your system and then come back here to finish your production install.
...
can be set via `https_key_file` and `https_cert_file`. Make sure any certificate

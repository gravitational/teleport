...
teleport-vdi-pod-0                  ⟵ Tunnel       data-science-vdi=true
...
you can add a PuTTY session using the following command:
...
No, Teleport will only modify the config parameters that it relies on, like proxy name, proxy command, hostname, username etc.
...
#### Can I use an alternative Teleport authentication method with saved sessions?
...
Advanced PuTTY users can use the Registry Editor to modify the PuTTY proxy command themselves under the `ProxyTelnetCommand`
key. Note that if you re-run `tsh puttyconfig` for the given hostname, this will be overwritten.
...
that your Teleport user/role has permissions to use. If you can log in successfully with `tsh ssh`, you should be able to
use the name login/hostname with `tsh puttyconfig`.
...
You are not logged into Teleport correctly. Run `tsh login --proxy=<proxy hostname>` to get correct certificates before
...
You do not have valid `tsh` credentials locally. Run `tsh login --proxy=<proxy hostname>` to log in.

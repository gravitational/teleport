...
This guide will show you how to use the Teleport client tool `tsh` to add saved sessions to PuTTY, then use
PuTTY as a client to connect to SSH nodes and benefit from Teleport's RBAC, auditing and session recording.

You will learn how to:

- Generate saved PuTTY sessions for remote cluster nodes.
- Log in to an interactive shell using these saved sessions.

## Prerequisites

- A client machine running Windows 10 or higher (support in `tsh` for adding saved sessions to PuTTY only works on Windows)

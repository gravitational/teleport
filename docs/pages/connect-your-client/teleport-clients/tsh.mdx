...
*profiles* in the `tsh` configuration file. A profile is a reusable bundle of
global connection settings (proxy address, cluster, user, and other global
overrides) that you select at invocation time with the `--profile` flag (or the
`TELEPORT_PROFILE` environment variable):
...
profiles:
  ...
  staging:
    ...
    user: alice
# Optionally select a profile to use when --profile is not provided.
...
```code
$ tsh --profile prod login
$ tsh --proxy teleport.prod.example.com:443 --cluster prod --user alice --mfa-mode sso login
```

Each profile supports the following fields, all optional. At minimum a profile
...
or environment variable, then the selected profile, then `tsh` defaults. In
other words, a value you pass explicitly always wins over a profile, so you can
select a profile and still override individual settings:

```code
# Use the "prod" profile but connect as a different user.
$ tsh --profile prod --user bob login
```

If `--profile` (or `TELEPORT_PROFILE`) is not set, `tsh` falls back to
`default_profile` when it is defined. Selecting a profile that does not exist,
or defining a `default_profile` that is not present in `profiles`, is an error.

<Admonition type="note">
A `tsh` configuration profile is distinct from the per-cluster login profiles
that `tsh` stores under `$TELEPORT_HOME` (`~/.tsh`) after you log in. The former
is a convenience bundle of connection settings you author yourself; the latter
is managed by `tsh` and shown by `tsh status`.
</Admonition>

...
When using [trusted clusters](../../zero-trust-access/management/trustedclusters.mdx), the cluster selector allows
...
Windows, and Linux. It enables any program on your computer to connect to TCP
...
1. Look for the application you wish to connect to.
  - For TCP applications, click the three dots next to the Connect button and select "Connect without VNet".
...
| `terminal.middleClick`        | `none` | `none` does nothing, `paste` pastes clipboard content. |
| `terminal.copyOnSelect`       | `false` | Automatically copies selected text to the clipboard.                                                                                                                                                                      |
| `terminal.setTeleportAuthServerEnvVar` | `true` | Whether to set `TELEPORT_AUTH_SERVER` in local terminal sessions. Setting it helps tctl choose the right profile. |
...
certificates](../../installation/self-hosted/self-signed-certs.mdx) or for demo purposes. We do not recommend

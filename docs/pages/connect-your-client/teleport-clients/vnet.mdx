...
<details>
<summary>Running tsh vnet on Linux without Teleport Connect</summary>
...
<Admonition type="note" title="Using Linux?">
This only applies to macOS. On Linux, VNet registers its DNS with systemd-resolved, and the
configuration is cleared automatically when the VNet link is removed.
</Admonition>
...
Once that's done, stop capturing `journalctl` output, then disable debug logs in VNet with:

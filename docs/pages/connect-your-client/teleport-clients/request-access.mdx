...
This page describes the `tsh` commands that allow you to create Teleport Access
Requests. 
...
resource by running the following command, where <Var name="resource-id" /> is
...
For example, to create a request to access all namespaces prefixed with `dev` that match

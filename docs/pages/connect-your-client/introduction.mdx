...
`tsh` lets you authenticate to Teleport, and list and connect to resoruces. After [downloading](/downloads)
...
```code
$ tsh login --proxy=teleport.example.com --user=alice@example.com --auth=your-idp-connector
Enter password for Teleport user alice:
Tap any security key
> Profile URL:        https://teleport.example.com:443
  Logged in as:       alice
...
Teleport provides a web interface for users to interact with. This is usually
found at the same URL used to connect to Teleport with (e.g.
<ScopedBlock scope={["oss", "enterprise"]}>`https://teleport.example.com`</ScopedBlock>
<ScopedBlock scope={["cloud"]}>`https://mytenant.teleport.sh`</ScopedBlock>),
but can be adjusted by Teleport Administrators. The Web UI provides similar
...
button will open a new tab with a terminal emulator to provide acesss to that server.

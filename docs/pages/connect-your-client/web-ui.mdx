...
## Secure Copy (SCP) files with the Web UI

Teleport provides two methods to copy local files to a remote system through the web interface.

### Teleport UI

You can use the upload functionality:

![Teleport UI](../../img/download-files.png)

You can also upload files from a system using drag and drop:

![drag and drop](../../img/upload-ui.png)

### Teleport Connect

Teleport Connect is a graphical user interface for Teleport that runs on Mac, Linux and Windows. This is another handy tool for quickly uploading local files to a remote server.

## ![Teleport Connect](../../img/download-with-connect.png)

```
NOTE: You can also use scp via the terminal, as rerefrenced in the chapter on [ssh connections](..connect-your-client/#transferring-files-scp).

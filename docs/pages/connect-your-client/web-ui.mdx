...
<Admonition type="note" title="Databases CLI compatibility">
  While the terminal interface looks similar to database CLIs like `psql`, it is
  a custom implementation provided by Teleport. As a result, it may have
  different behaviors and lack some features available in database CLI tools.
</Admonition>

...
#### Tunnel with VNet on macOS

VNet is a virtual network emulation tool available in Teleport Connect. It completely removes the
need to manually create tunnels for web and TCP applications.

1. Log in to the cluster.
1. To start VNet, open the connection list in the top left and click the connect icon to the right of
   VNet.
1. Now access any of your web and TCP apps over its regular address.

VNet automatically routes the connection through a tunnel authenticated with your user certificate.
See [the VNet section](#vnet) below for more details.

#### Creating a tunnel manually
...
VNet is a virtual network emulation tool available in Teleport Connect on macOS, with Windows and
Linux version coming soon. It automatically creates authenticated tunnels for web and TCP
applications. To learn how VNet works behind the scenes, see [VNet](../vnet.mdx).

VNet has a dedicated panel that can be accessed through the connection list in top left. From
there you can start and stop VNet and see recent connections made through VNet.

TODO: Screenshot of the VNet panel.

Connect prompts for a password on the first launch to install a privileged daemon that creates a
virtual network device and manages DNS setup. The daemon gets removed automatically when you remove
Teleport Connect from your computer.

To stop VNet, either close Teleport Connect or use the VNet panel in the connection list at the top left.
VNet will start automatically on app restart, unless you stop VNet before exiting.

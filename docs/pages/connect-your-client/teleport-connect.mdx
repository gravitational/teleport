...
sidebar_label: Teleport Connect
...
When using [trusted clusters](../zero-trust-access/management/admin/trustedclusters.mdx), the cluster selector allows
...
Teleport Connect can set up an authenticated tunnel on a random port on your
device to an MCP server in streamable-HTTP transport. All connections made to
that tunnel will be authenticated with your credentials.

1. Open a tab with cluster resources by clicking on the plus symbol at the end of the tab bar. You
  can also press `Ctrl+Shift+T/Cmd+T` to achieve the same result.
1. Click the Connect button of the MCP server resource.

A new tab will open with a new connection established between your device and the MCP server.

![MCP connection](../../img/use-teleport/connect-mcp-http.png)
...
On first launch, the app will automatically migrate your profiles to the new location.
</Admonition>
...
them on a file system. For more details, see [Hardware Key Support guide](../zero-trust-access/authentication/hardware-key-support.mdx).
...
| `headless.skipConfirm`        | `false`                                                                                                              | Skips the confirmation prompt for Headless Authentication approval and instead prompts for MFA immediately. |
...
1. Close Teleport Connect.
1. Remove the internal `tsh` folder and the `app_state.json` to clear all remembered tabs and connections. The internal
`tsh` folder was created by older versions of Teleport Connect and may not exist.
1. Remove the `tsh` folder to log out of all clusters. Its location may differ based on your app configuration.
...
certificates](../zero-trust-access/management/admin/self-signed-certs.mdx) or for demo purposes. We do not recommend
...
To remove the tsh directory (note: this directory is also used by the tsh tool):

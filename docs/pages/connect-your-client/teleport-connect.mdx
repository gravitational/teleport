...
The setup makes sure that you have all the necessary permissions to access the server. If the server
is not visible, it might indicate that you are assigned a role which happens to deny access to the
Connect My Computer node. Inspect the role list in the Management section of the Web UI to resolve
this problem. First, check what roles are assigned to your user object and then inspect each role.
Look for a role with broad `deny` rules.
...
Another solution is to restart the setup, as described in [the "Starting from scratch"

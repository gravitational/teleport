...
## Configuration
Teleport Connect can be configured by editing `app_config.json` file.
To open it, click More Options icon `⋮` -> Open Config File.
...
Below is the list of the config properties that can be configured.
...
| `keymap.openConnections`      | `Command+P` on macOS<br/>`Ctrl+P` on Windows/Linux                                                                                                                  | Shortcut to open the connections panel.                                |
| `keymap.openClusters`         | `Command+E` on macOS<br/>`Ctrl+E` on Windows/Linux                                                                                                                  | Shortcut to open the clusters panel.                                   |
| `keymap.openClusters`         | `Command+E` on macOS<br/>`Ctrl+E` on Windows/Linux                                                                                                                  | Shortcut to open the clusters panel.                                   |
| `keymap.openProfiles`         | `Command+I` on macOS<br/>`Ctrl+I` on Windows/Linux                                                                                                                  | Shortcut to open the profiles panel.                                   |
| `keymap.openCommandBar`       | `Command+K` on macOS<br/>`Ctrl+K` on Windows/Linux                                                                                                                  | Shortcut to open the command bar.                                      |
| `terminal.fontFamily`         | `Menlo, Monaco, 'Courier New', monospace` on macOS<br/>`'Consolas', 'Courier New', monospace` on Windows<br/>`'Droid Sans Mono', 'Courier New', monospace` on Linux | Font family for the terminal.                                          |
...
<Admonition
  ...
  title="Note"
>
  In the config file there is an additional `$schema` property. It is a link to the JSON schema that
  describes the available config options.
  It allows editors to provide autocompletion.
</Admonition>

### Configuring keyboard shortcuts
A valid shortcut contains at least one modifier and a single key code, for example `Shift+Tab`.
Function keys (`F21`-`F24`) do not require a modifier.
Modifiers and a key code must be combined by `+` character.

<Tabs>
  <TabItem label="macOS">
    Available platform modifiers: `Control`, `Option`, `Shift`, `Command`.
  </TabItem>
  <TabItem label="Windows/Linux">
    Available platform modifiers: `Ctrl`, `Alt`, `Shift`.
  </TabItem>
</Tabs>
...
If you initially agreed to share telemetry data, but now want to opt out, you need to update the `app_config.json` file (see [Configuration](#configuration)):
```json

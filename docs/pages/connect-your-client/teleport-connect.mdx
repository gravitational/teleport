...
- The **cluster selector** (to the left of the search bar), which shows up only if you have set up
  ...
  leaf cluster resources.
- The **more options menu** (to the left of the profile selector), containing options such as
...
<Tabs>
<TabItem label="macOS">
To add tsh to `PATH`, click the More Options icon `⋮` -> Install tsh in PATH. This will

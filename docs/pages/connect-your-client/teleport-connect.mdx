...
symlink tsh to `/usr/bin`. You can remove the symlink by selecting "Remove tsh from PATH".
...
`/usr/bin/tsh`, unless you have already installed the `teleport` package, which also creates
that symlink.
</TabItem>
<TabItem label="Windows">

...
<Checkpoint
  title="You can see your database"
  description="The output of `tsh db ls` includes the database you want to connect to."
>
- **No databases are listed:** Confirm the Database Service is running and has
  registered databases in the cluster. If you use leaf clusters, run:
  ```code
  $ tsh db ls --all
  ```
- **The database exists but does not appear for you:** Your role may restrict
  access via `db_names`, `db_labels`, or `db_users`. Ask your administrator to
  confirm your role allows the database and at least one database user.
- **A later command (`tsh db login`, `tsh proxy db`) reports the database is
  not found:** Confirm the database resource name matches the entry in
  `tsh db ls` exactly, including any prefixes or environment suffixes.
...
routing](../../zero-trust-access/management/tls-routing.mdx), run the
...
### Verify your local proxy or tunnel

Once you have started a tunnel, local proxy, or retrieved remote host and port
configuration, verify the result before configuring your GUI client.

<Checkpoint
  title="The local proxy or tunnel is running"
  description="`tsh` prints a `Started` message with a localhost address and port. For the local proxy server method, the `ca_file`, `cert_file`, and `key_file` paths are also printed and exist on disk. For the remote host and port method, `tsh db config` shows a Host and Port plus CA/Cert/Key paths."
>
...
- If you see **Port already in use**:
  - Re-run and let `tsh` pick a new port, or set one explicitly:
    ...
    ```
- If credential files do not exist (local proxy server method):
...
- If you see **certificate expired**:
  - Refresh your login and restart the proxy:
    ...
    ```
- If you see **Not logged into database** (remote host and port method):
  - Log in to the database, then view the configuration again:
    ```code
    $ tsh db login <database-name>
    $ tsh db config <database-name>
    ```
- If the port shown is the web/proxy port (for example `3080`) or TLS routing is enabled (remote host and port method):
  - Use **Authenticated tunnel** or **Local proxy server** instead.
...
This is `127.0.0.1:62652` in the example above (see the
[Authenticated tunnel](./gui-clients.mdx#authenticated-tunnel) section
...
local connection only as the connection to Redshift is encrypted by Teleport.
...
the `tsh` command you ran in [Get connection information](./gui-clients.mdx#get-connection-information).
...
Redis Insight connects to your Redis instance:
...
The driver also requires a password field even though Teleport ignores it. Use any placeholder value. The same requirements apply to the Snowflake JetBrains setup below.
</Admonition>
...
is `localhost:62652` in the example above. (See
[Get connection information](./gui-clients.mdx#get-connection-information) for
more information.)
...
After configuring any of the clients above, confirm that the connection works:

<Checkpoint
  title="Your GUI client is connected"
  description="The client authenticates and displays your database's schemas, tables, or keys."
>
...
- If you see **certificate verify failed / TLS errors**:
  - For the Authenticated tunnel, do not configure client certificates in the GUI (the tunnel handles TLS).
  - For the Local proxy server, confirm the CA, certificate, and key paths exactly match what `tsh proxy db` printed and include the `-----BEGIN` / `-----END` lines when pasted.
...
- If the GUI app cannot read certificate files:
  - On macOS, sandboxed apps may not see files under `~/.tsh/`. This affects multiple clients (MySQL Workbench, pgAdmin, and others). Copy the files to a user-accessible location and update the paths in your GUI.
...
- If the tunnel/proxy terminates unexpectedly:
  - Check `tsh status` to ensure your session hasn't expired and your certificates are still valid:
    ```code
    $ tsh status
    ```

...
<Checkpoint
  ...
  description="Run tsh db ls. Success looks like: the database you want appears in the list. An empty list means no databases are currently visible to your Teleport role."
>
...
- If a later command (such as `tsh db login` or `tsh proxy db`) reports the database is not found:
  - Confirm the database resource name matches the entry in `tsh db ls` exactly, including any prefixes or environment suffixes.
...
<Checkpoint
  ...
  description="Success looks like: tsh prints a Started message with a localhost address and port. For the local proxy server, all three credential file paths (ca_file, cert_file, key_file) are printed and exist on disk. For the remote host and port method, tsh db config shows a Host and Port plus CA/Cert/Key paths."
>
...
The driver also requires a password field even though Teleport ignores it. Use any placeholder value. The same requirements apply to the JetBrains setup in the next section.
</Admonition>
...
- If the GUI app cannot read certificate files:
  - On macOS, sandboxed apps may not see files under `~/.tsh/`. This affects multiple clients (MySQL Workbench, pgAdmin, and others). Copy the files to a user-accessible location (for example `/tmp/teleport-db/`) and update the paths in your GUI.

...
The TLS Routing configuration on the Teleport cluster impacts the ports that are available for database clients to connect to.

Follow the instructions below to see if TLS Routing is enabled on your cluster and get connection information for your database GUI clients.

(!docs/pages/includes/determinetls.mdx!)

<Tabs>
<TabItem label="Authenticated Proxy">

(!docs/pages/includes/database-access/tsh-db-tunnel.mdx!)

</TabItem>
<TabItem label="TLS Routing Enabled">

If TLS Routing is confirmed enabled use the following command to start a local TLS proxy your
GUI database client will connect to:
...
<TabItem label="TLS Routing Disabled">

If TLS Routing is not enabled, dedicated ports are available for the DB type connection in addition to running
a local database proxy. Run the following command to see the database

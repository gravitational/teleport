...
`localhost,62652` in the example above. Using the IP `127.0.0.1,62652` host could be required on
a Windows machine instead of `localhost`. (See
[Get connection information](./gui-clients.mdx#get-connection-information) for
more information.)

Create a connection with Microsoft SQL Server with the proxy endpoint `host`,`port`
for the Server, SQL Login for the Authentication Type, keep the Password field empty
and Encrypt set to `False`:
...
Create a connection to a database engine with the proxy endpoint `host`,`port`
for the Server name, SQL Server Authentication Type, keep the Password field empty
and do not enable encryption:

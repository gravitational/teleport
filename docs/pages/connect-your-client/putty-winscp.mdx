---
title: Using PuTTY to connect to SSH nodes and WinSCP to transfer files through Teleport
...
You will also optionally learn how to:
- Import a saved PuTTY session into WinSCP, then start an SFTP session to transfer files from an SSH node.
...
Yes, WinSCP 6.2+ supports validation using SSH host certificates which is needed to connect using Teleport.

...
is designed to mimic OpenSSH `scp` as much as possible:
...
Note that `tsh scp` always uses the SFTP protocol under the hood, no flags need
to be passed to enable this behavior.

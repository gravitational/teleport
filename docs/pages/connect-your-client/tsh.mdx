...
For a full list of `tsh` commands, flags, and arguments, read the [`tsh`
reference guide](../reference/cli/tsh.mdx) or execute `tsh` with no flags in
your terminal.
...
For the most part, the resource ID is the value you would see when listing
resources with `tsh`, such as running the `tsh apps ls` or `tsh db ls` commands.
For resources in a Kubernetes cluster, such as a service or pod, the procedure
is slightly different. The sections below describe how to request access to

...
Follow install instructions to obtain the `tsh` binary. Install
`tsh` of the same major version as the version used in your
Teleport cluster. 
<Details title="Finding your Teleport cluster version number">
To find the version number you can get it from:
- In the Web UI select user name in the upper right, then Help & Support.
  The Teleport Version and other information will show.
- Retrieve from the Teleport cluster using `curl` and `jq`. Replace the variable
  with your proxy address (e.g. mytenant.teleport.sh for Teleport cloud).
  ```code

...
# and saves them into ~/.tsh/example.teleport.sh
...
- Use `curl` and `jq`. Replace <Var name="teleport.example.com" />
  with your Proxy Service address (e.g. `example.teleport.sh` for Teleport
  Enterprise Cloud):
...
# Authenticate against the "work" cluster as joe and then
# log into the node as root:
$ tsh ssh --proxy=example.teleport.sh --user=joe root@node
```
...
```code
# Log in using the local Teleport 'admin' user:
$ tsh --proxy=example.teleport.sh --auth=local --user=admin login

# Log in using GitHub as an SSO provider, assuming the GitHub connector is called "github"
$ tsh --proxy=example.teleport.sh --auth=github login
```
...
```code
# Don't open the system default browser when logging in
$ tsh login --proxy=example.teleport.sh --browser=none
```
...
# > Profile URL:  example.teleport.sh:443
#   Logged in as: johndoe
...
```code
# Authenticate the user against example.teleport.sh and save the user
# certificate to joe.pem
$ tsh login --proxy=example.teleport.sh --out=joe

# Use joe.pem to log in to the server 'db'
$ tsh ssh --proxy=example.teleport.sh -i joe joe@db
```
...
```code
$ tsh login --proxy=example.teleport.sh --out=joe --format=openssh
$ ls -lh
...
```code
# Log in to your Teleport Enterprise cluster so you can use tctl locally.
...
# Login in to a cluster and retrieve your SSH certificate:
$ tsh --proxy=example.teleport.sh login
...
```code
$ tsh ssh -J example.teleport.sh telenode
```
...
Assuming the Teleport Enterprise account called `example.teleport.sh` is configured with a few trusted
clusters, a user may use the `tsh clusters` command to see a list of all Trusted Clusters on the server:

```code
$ tsh --proxy=example.teleport.sh clusters
...
```code
$ tsh --proxy=example.teleport.sh ls --cluster=production
...
```code
$ tsh --proxy=example.teleport.sh ssh --cluster=production db-1
```

This is possible even if Nodes in the `production` cluster are located behind a
firewall without open ports. This works because the `production` cluster
establishes a reverse SSH tunnel back into your Teleport Enterprise tenant's Proxy

...
Follow install instructions to obtain the `tsh` binary. Ideally, install
...
If you installed on MacOS with `brew` run `brew uninstall teleport`.  

For other installations and to remove user data, see our documentation
on [Uninstalling Teleport](../management/admin/uninstall-teleport.mdx).

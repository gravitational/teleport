...
We recommend setting up Teleport in High Availability (HA) mode. In HA mode DynamoDB
...
Instructions are provided below and can be found on [AWS Docs](https://docs.aws.amazon.com/eks/latest/userguide/install-kubectl.html).
...
While Teleport is fully compatible with third-party ACME clients (such as Let's Encrypt certbot), third-party clients are optional to get started with Teleport on AWS. Teleport will automatically acquire an ACME-compliant TLS certificate by default to make it easy to get set up.
...
Most of this guide has been designed for the Teleport Open Source Edition. Most of this guide also applies

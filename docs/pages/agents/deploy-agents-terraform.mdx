...
Depending on whether you assign the `is_community` input variable to `true` or
`false`, the Terraform module loads a user data script for either Teleport
Community Edition or a commercial edition of Teleport (Team, Enterprise, or
Enterprise Cloud):
...
<TabItem label="Commercial Editions of Teleport">
```text
(!examples/agent-pool-terraform/userdata-ent!)
```
</TabItem>
</Tabs>

The user data script, like the Terraform configuration file, assumes that you
are deploying your Teleport agents on EC2 instances running Amazon Linux 2023.
If your virtual machines are running another operating system, you will need to
change the installation script according to the instructions in the
[Installation](../installation.mdx) page.

This script installs the `teleport` binary on the host, then writes a
...
agent_count=2
is_community=true
proxy_service_address="mytenant.teleport.sh:443"
aws_region=""
...
proxy_service_address="mytenant.teleport.sh:443"
aws_region=""
teleport_version=(=teleport.major_version=)
subnet_id=""
```

For commercial editions, the startup script requires only the major version of
Teleport.
...
two agents have deployed successfully (it may take a few minutes for the EC2
instances to launch and run the installation script):

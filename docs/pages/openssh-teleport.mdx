...
- Recording proxy mode requires the use of SSH agent forwarding. Agent forwarding is required because without it, a proxy will not be able to establish the second connection to the destination node.
...
CA, Teleport will reject this connection with the error message: `"ssh:

...
`tbot` relies on [Machine & Workload Identity](../../../machine-workload-identity/machine-id/introduction.mdx) to obtain and automatically renew
...
Starting with 16.2, the Teleport Terraform provider can natively use Machine & Workload Identity (without `tbot`) to join a Teleport

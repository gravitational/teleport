...
|app_resources|[][object](#specallowapp_resources-items)|AppResources is the list of declarative, sugared per-request rules that scope access within an HTTP app to specific paths and methods. It is valid only in role version v9 and above, and only under allow.|
|app_resources_expressions|[]string|AppResourcesExpressions is the list of bare predicate rules that scope access within an HTTP app, the desugared counterpart of AppResources. It is valid only in role version v9 and above, and only under allow.|
...
|db_service_labels|[object](#specallowdb_service_labels)|DatabaseServiceLabels are used in RBAC system to allow/deny access to Database Services.|
...
|group_labels|[object](#specallowgroup_labels)|GroupLabels is a map of labels used as part of the RBAC system.|
...
|kubernetes_labels|[object](#specallowkubernetes_labels)|KubernetesLabels is a map of kubernetes cluster labels used for RBAC.|
...
|linux_desktop_labels|[object](#specallowlinux_desktop_labels)|LinuxDesktopLabels are used in the RBAC system to allow/deny access to Linux desktops.|
...
|node_labels|[object](#specallownode_labels)|NodeLabels is a map of node labels (used to dynamically grant access to nodes).|
...
|windows_desktop_labels|[object](#specallowwindows_desktop_labels)|WindowsDesktopLabels are used in the RBAC system to allow/deny access to Windows desktops.|
...
|workload_identity_labels|[object](#specallowworkload_identity_labels)|WorkloadIdentityLabels controls whether or not specific WorkloadIdentity resources can be invoked. Further authorization controls exist on the WorkloadIdentity resource itself.|
...
### spec.allow.app_resources items

|Field|Type|Description|
|---|---|---|
|allow_code|string|AllowCode is the structured code emitted on the allow audit event when the rule matches.|
|allow_encoded|[]string|AllowEncoded opts the rule's path match into the named encoded characters. The only supported value is the encoded slash "/".|
|allow_reason|string|AllowReason is the human-readable explanation emitted alongside AllowCode.|
|deny_code_hint|string|DenyCodeHint is the structured code emitted on the deny audit event when the rule's path and method matched but the where condition did not.|
|deny_reason_hint|string|DenyReasonHint is the human-readable explanation emitted alongside DenyCodeHint.|
|methods|[]string|Methods are the HTTP methods that further scope the rule, OR-ed. An empty list permits any method.|
|paths|[]string|Paths are the declarative path patterns the rule matches, OR-ed. A rule must set either Paths or UnsafeAllowAll.|
|unsafe_allow_all|boolean|UnsafeAllowAll grants unrestricted access to every path and method. It cannot be combined with any other field.|
|where|string|Where is a predicate over the caller identity, the request, and the rule's captures. It does not match paths.|
...
|Field|Type|Description|
|---|---|---|
|match|[object](#specallowdb_permissions-itemsmatch)|Match is a list of object labels that must be matched for the permission to be granted.|
...
### spec.allow.db_permissions items.match
...
### spec.allow.linux_desktop_labels
...
### spec.allow.request.annotations
...
|submit_for_users|[]string|SubmitForUsers is used by Teleport plugins to indicate they can submit reviews on behalf of other users. Reviews submitted for a user are evaluated against that user's own RBAC permissions. Supports wildcard (*).|
...
### spec.allow.windows_desktop_labels
...
### spec.allow.workload_identity_labels
...
|app_labels_expression|string|AppLabelsExpression is a predicate expression used to allow/deny access to Apps.|
|app_resources|[][object](#specdenyapp_resources-items)|AppResources is the list of declarative, sugared per-request rules that scope access within an HTTP app to specific paths and methods. It is valid only in role version v9 and above, and only under allow.|
|app_resources_expressions|[]string|AppResourcesExpressions is the list of bare predicate rules that scope access within an HTTP app, the desugared counterpart of AppResources. It is valid only in role version v9 and above, and only under allow.|
|aws_role_arns|[]string|AWSRoleARNs is a list of AWS role ARNs this role is allowed to assume.|
|azure_identities|[]string|AzureIdentities is a list of Azure identities this role is allowed to assume.|
...
|db_roles|[]string|DatabaseRoles is a list of databases roles for automatic user creation.|
|db_service_labels|[object](#specdenydb_service_labels)|DatabaseServiceLabels are used in RBAC system to allow/deny access to Database Services.|
|db_service_labels_expression|string|DatabaseServiceLabelsExpression is a predicate expression used to allow/deny access to Database Services.|
|db_users|[]string|DatabaseUsers is a list of databases users this role is allowed to connect as.|
|desktop_groups|[]string|DesktopGroups is a list of groups for created desktop users to be added to|
|gcp_service_accounts|[]string|GCPServiceAccounts is a list of GCP service accounts this role is allowed to assume.|
...
|group_labels|[object](#specdenygroup_labels)|GroupLabels is a map of labels used as part of the RBAC system.|
|group_labels_expression|string|GroupLabelsExpression is a predicate expression used to allow/deny access to user groups.|
|host_groups|[]string|HostGroups is a list of groups for created users to be added to|
|host_sudoers|[]string|HostSudoers is a list of entries to include in a users sudoer file|
...
|kubernetes_groups|[]string|KubeGroups is a list of kubernetes groups|
|kubernetes_labels|[object](#specdenykubernetes_labels)|KubernetesLabels is a map of kubernetes cluster labels used for RBAC.|
|kubernetes_labels_expression|string|KubernetesLabelsExpression is a predicate expression used to allow/deny access to kubernetes clusters.|
...
|kubernetes_users|[]string|KubeUsers is an optional kubernetes users to impersonate|
|linux_desktop_labels|[object](#specdenylinux_desktop_labels)|LinuxDesktopLabels are used in the RBAC system to allow/deny access to Linux desktops.|
|linux_desktop_labels_expression|string|LinuxDesktopLabelsExpression is a predicate expression used to allow/deny access to Linux desktops.|
|linux_desktop_logins|[]string|LinuxDesktopLogins is a list of desktop login names allowed/denied for Linux desktops.|
|logins|[]string|Logins is a list of *nix system logins.|
|mcp|[object](#specdenymcp)|MCPPermissions defines MCP servers related permissions.|
|node_labels|[object](#specdenynode_labels)|NodeLabels is a map of node labels (used to dynamically grant access to nodes).|
|node_labels_expression|string|NodeLabelsExpression is a predicate expression used to allow/deny access to SSH nodes.|
...
|windows_desktop_labels|[object](#specdenywindows_desktop_labels)|WindowsDesktopLabels are used in the RBAC system to allow/deny access to Windows desktops.|
|windows_desktop_labels_expression|string|WindowsDesktopLabelsExpression is a predicate expression used to allow/deny access to Windows desktops.|
|windows_desktop_logins|[]string|WindowsDesktopLogins is a list of desktop login names allowed/denied for Windows desktops.|
|workload_identity_labels|[object](#specdenyworkload_identity_labels)|WorkloadIdentityLabels controls whether or not specific WorkloadIdentity resources can be invoked. Further authorization controls exist on the WorkloadIdentity resource itself.|
|workload_identity_labels_expression|string|WorkloadIdentityLabelsExpression is a predicate expression used to allow/deny access to issuing a WorkloadIdentity.|
...
|Field|Type|Description|
|---|---|---|
|match|[object](#specdenydb_permissions-itemsmatch)|Match is a list of object labels that must be matched for the permission to be granted.|
|permissions|[]string|Permission is the list of string representations of the permission to be given, e.g. SELECT, INSERT, UPDATE, ...|

### spec.deny.db_permissions items.match
...
### spec.deny.linux_desktop_labels
...
|Field|Type|Description|
|---|---|---|
|annotations|[object](#specdenyrequestannotations)|Annotations is a collection of annotations to be programmatically appended to pending Access Requests at the time of their creation. These annotations serve as a mechanism to propagate extra information to plugins.  Since these annotations support variable interpolation syntax, they also offer a mechanism for forwarding claims from an external identity provider, to a plugin via `{{external.trait_name}}` style substitutions.|
...
### spec.deny.windows_desktop_labels
...
### spec.deny.workload_identity_labels
...
|web_terminal_clipboard_mode|string or integer|WebTerminalClipboardMode determines clipboard behavior in the Web UI terminal. Valid values are "unrestricted" and "no-copy". Defaults to "unrestricted" if unspecified. Can be either the string or the integer representation of each option.|

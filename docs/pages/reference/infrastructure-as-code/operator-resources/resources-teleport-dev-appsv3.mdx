...
|Field|Type|Description|
|---|---|---|
|allowed_cas|[]string|AllowedCas is the list of user provided CAs used for verifying upstream certificates. Accepted values are PEM-encoded CA certificates and Teleport CA aliases.|
|client_cert_mode|string|ClientCertMode specifies which client certificate mode to use for the upstream connection.|
|mode|string|Mode defines the TLS verification.|

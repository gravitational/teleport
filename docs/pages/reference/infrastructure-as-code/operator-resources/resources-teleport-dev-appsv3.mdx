...
|session_recording|[object](#specsession_recording)|SessionRecording contains per-app session recording configuration.|
...
### spec.session_recording

|Field|Type|Description|
|---|---|---|
|bots|string|Bots controls whether app session recording is enabled for bots. Disabling session recording for bots provides a way to mitigate the load on the agent caused by service-to-service use-cases. Supported values are "on" and "off".|

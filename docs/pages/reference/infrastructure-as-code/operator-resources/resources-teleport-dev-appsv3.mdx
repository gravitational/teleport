...
|http_protocol_priority|string or integer|HTTPProtocolPriority controls how the proxy orders `http/1.1` and `h2` in the ALPN list it advertises for this app. If unset, the server default applies. See HTTPProtocolPriority for details. Can be either the string or the integer representation of each option.|

...
|disconnect_expired_cert|boolean|DisconnectExpiredCert controls whether App sessions are disconnected when the user certificate expires.|
...
|lock|[object](#specapplock)|Lock configures the role's locking behavior for app sessions.|
...
|Field|Type|Description|
|---|---|---|
|labels|[][object](#specworkload_identitylabels-items)|Labels is the set of WorkloadIdentity labels used to dynamically select which WorkloadIdentity resources this role permits issuance with. Issuance additionally requires the caller to hold read_no_secrets (and, for the multi-issue RPC, list) rules for the workload_identity kind in the relevant scope.|

### spec.workload_identity.labels items

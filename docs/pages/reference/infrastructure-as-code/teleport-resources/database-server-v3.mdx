...
|ldap_host|The host used for LDAP queries, optionally including a port. (Optional, defaults to KDCHostName; useful when the LDAP server and KDC are reachable under different names.)|string|
...
|ldap_tls_server_name|The server name used for the TLS handshake with the LDAP server. (Optional, defaults to the host part of LDAPHost, falling back to KDCHostName.)|string|
|pki_domain|The Active Directory domain where CRLs are published. (Optional, defaults to Domain; useful when PKI lives in a root domain but Teleport accesses resources in a child domain.)|string|
...
|protocol|The database protocol. Valid options are: "cassandra" "clickhouse" "clickhouse-http" "cockroachdb" "dynamodb" "elasticsearch" "mongodb" "mysql" "oracle" "postgres" "redis" "snowflake" "spanner" "sqlserver"|string|

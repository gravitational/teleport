...
|source_identity|When true, sets the Teleport username as the AWS STS source identity on assumed role sessions. Persists across role chaining and is logged in CloudTrail. Requires `sts:SetSourceIdentity` in the target role's trust policy.|bool|

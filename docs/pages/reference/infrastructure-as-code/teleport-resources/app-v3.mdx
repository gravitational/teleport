...
Contains per-app session recording configuration.
...
|Field Name|Description|Type|
|---|---|---|
|bots|Controls whether app session recording is enabled for bots. Disabling session recording for bots provides a way to mitigate the load on the agent caused by service-to-service use-cases. Supported values are "on" and "off".|[App Session Recording Bots](#app-session-recording-bots)|

## App Session Recording Bots

Controls whether app session recording is enabled for bots.
...
|session_recording|Contains per-app session recording configuration.|[App Session Recording](#app-session-recording)|

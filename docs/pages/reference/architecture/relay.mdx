...
The connections from the client to the Relay, the connections between Relay instances, and the tunnel connections from the Teleport agents and the Relay instances are all encrypted and authenticated through mutual TLS, with X.509 certificates issued by the Teleport control plane. The tunnel connections from agents to the Relay service and the connections between Relay instances use a bespoke protocol, the Relay configuration discovery from agents and the SSH connections from clients use gRPC. The use of external TLS termination is not supported.

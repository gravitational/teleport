...
The Relay service is an optional Teleport cluster component that can be used to provide an alternate connectivity path between clients and resources similarly to what the Proxy service does when forwarding connections to resources [through reverse tunnels](proxy.mdx#tunnels): Teleport agents can be configured to open tunnels to both the Teleport Proxy service and to the Relay service, and clients can be configured to use the Relay service when connecting to resources; the Relay service will then route and forward the connection between the client and the requested resource without passing the connection through the Proxy service.
...
A Relay service deployment consists of one or more Relay instances configured with the same Relay group name, reachable by Teleport agents and clients through a L4 network load balancer. Relay instances of the same group receive connections from agents and clients through the load balancer, and can connect to each other directly. Relay instances listen on two separate ports to serve connections from agents and clients, and it's possible to use different load balancers and hostnames if necessary. The typical setup uses a single load balancer serving the two ports, reachable at a single hostname, with port 3042 used by agents and port 443 used by clients.
...
Use of the Relay service requires a desktop client (i.e. `tsh`) or the . It's not possible to use a Relay through the Teleport web UI.
...
When specifying `--relay` at `tsh login` time, the configured Relay address is stored in the `tsh` profile configuration and will then be used for all later invocations unless overridden. It's possible to configure a default Relay address on a per-user basis in the `default_relay_addr` user trait, which can be directly specified for each [local user](../../zero-trust-access/management/admin/users.mdx), passed by the SSO provider for [SSO users](../../zero-trust-access/sso/sso.mdx), granted through an [Access List](../../identity-governance/access-lists/access-lists.mdx), or added by a [Login Rule](../../zero-trust-access/authentication/login-rules/login-rules.mdx).

When using `tsh ssh`, if a Relay address was specified at login time or if `--relay` is passed in, the SSH connection will go through the specified Relay rather than going through the control plane. If an address was specified at login time, `--relay=none` will temporarily disable the use of the Relay service, and `--relay=default` will use the default Relay address for the user, even if a different address was specified at login time. When a Relay address is configured, only servers available through the Relay will be accessible, and no connection will go through the control plane.

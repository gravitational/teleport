...
Non-interactive users have to use Teleport Machine & Workload Identity product to receive and renew certificates.
Teleport Machine & Workload Identity's Bot runs alongside services and rotates SSH and X.509 certificates on behalf
...
When the proxy authorizes the attempt to connect to the Kubernetes cluster it interpolates
...
- [Teleport Predicate Language](../access-controls/predicate-language.mdx).

...
that this cluster trusts. In this case, Teleport activates [trusted cluster mapping logic](trustedclusters.mdx).
...
[trusted cluster mapping logic](trustedclusters.mdx) to support this use case.
...
Reference](../rbac/teleport-roles.mdx).
...
<Notice type="tip">
You can use `where` fields in all resource rules. Check out [the full role reference](../rbac/teleport-roles.mdx) contains full role spec for details.
</Notice>
...
- [Access Control Reference](../rbac/teleport-roles.mdx).
- [Teleport Predicate Language](../config-references/predicate-language.mdx).
- [Access Requests Guides](../../admin-guides/access-controls/just-in-time/access-requests/overview.mdx)

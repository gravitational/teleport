...
In TLS routing mode, the [Teleport Proxy Service](./proxy.mdx) multiplexes all
client connections on a single TLS port. This page describes the architecture of
TLS routing in Teleport.
...
and ALPN ([Application-Layer Protocol Negotiation](https://en.wikipedia.org/wiki/Application-Layer_Protocol_Negotiation))
TLS extensions.
...
See [GUI clients](../../connect-your-client/third-party/gui-clients.mdx) guide for a usage
example.
...
the Proxy Service does not require a Web TLS certificate using `https_keypairs`
or `acme`.
...
When transporting the SSH protocol over TLS routing, `tsh` makes connection

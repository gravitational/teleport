...
are considered of an "app" type, but it can be changed for a service by Kubernetes annotation [`teleport.dev/discovery-type`](../agent-services/kubernetes-application-discovery.mdx).
...
[`teleport.dev/name`](../agent-services/kubernetes-application-discovery.mdx).
...
Otherwise, this port is ignored. But if annotation [`teleport.dev/protocol`](../agent-services/kubernetes-application-discovery.mdx) is used on the service and its value is

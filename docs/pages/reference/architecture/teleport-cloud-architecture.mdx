...
Guide](../../get-started/get-started.mdx).
...
Customers can configure [External Audit Storage](../../zero-trust-access/management/external-audit-storage.mdx) to store audit logs and session recordings in their
own AWS account where retention period can be managed independently.

Teleport also supports [External CA Key Storage](../../zero-trust-access/management/external-ca-key-storage.mdx).

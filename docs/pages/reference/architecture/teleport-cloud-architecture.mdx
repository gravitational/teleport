---
title: Teleport Enterprise Architecture
description: Cloud security, availability, and networking details.
---

## Security

We have designed the Teleport Enterprise environment to be secure. We work with independent
...
The Teleport Enterprise environment is protected from network and transport layer DDoS attacks 
...
Teleport Enterprise Proxy does not terminate SSH sessions when using OpenSSH and `tsh` sessions.
The Cloud Proxy terminates TLS for Application, Database, and Kubernetes sessions.

## Data retention
All Teleport Enterprise customers have audit logs retained in S3 for 4 years, cluster
...
Teleport Enterprise can run in one of the following AWS regions:
...
Teleport Enterprise only serves the latest stable release of the Teleport software
for its customers.

Teleport Enterprise team upgrades the service with patch releases weekly and major
...
Teleport Enterprise and a loss of access to your infrastructure.
...
Teleport Enterprise commits to an SLA of (=cloud.sla.monthly_percentage=) of monthly uptime,

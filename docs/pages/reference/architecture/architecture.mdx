...
- [Proxy Peering](../../zero-trust-access/deploy-a-cluster/reliability/proxy-peering.mdx)
...
- **Machine & Workload Identity Bots** receive short-lived credentials from the
  `tbot` binary so service accounts can access infrastructure.
...
methods](../../installation/agents/agents.mdx) that
Agents use for this.

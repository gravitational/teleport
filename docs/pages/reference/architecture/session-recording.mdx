...
PostgreSQL database sessions can also be viewed using []`tsh play`](../cli/tsh.mdx)
and the Web UI. Sessions from other database protocols are only accessible with

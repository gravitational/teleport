...
environments where the connection to the Auth Service is unreliable or high-latency.
...
as a series of parts (in cloud storage or on the Auth Service instances disk) and it is the
responsibility of the Auth Service instances upload completer to detect the abandoned upload and
complete it.

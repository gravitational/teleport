...
modes. It can be enabled through the `teleport.yaml` configuration by setting
the `session_recording_encryption` flag in the `auth_service` section. It can
also be enabled dynamically by editing the `session_recording_config` resource
and setting the `spec.encryption.enabled` flag. Any required encryption keys
are provisioned using the CA key storage backend which is configured by setting
`ca_key_params` in the `auth_service` section of the `teleport.yaml`. Similar
to CAs, when no `ca_key_params` are provided then the encryption keys will be
software generated and saved in Teleport's storage backend.

...
Any encryption keys are provisioned using the same key storage backend
configured for CAs.
...
See the [reference](../deployment/monitoring/audit.mdx) to learn how to
...
specified by the `audit_sessions_uri` field in the `teleport.yaml` [configuration file](../deployment/config.mdx).
...
- [RBAC for session recordings](../access-controls/roles.mdx#rbac-for-session-recordings)

...
Auth Service instances, it is important that all instances have access to the same key
storage backend and keys. Otherwise the availlability of session replay may become
unstable. This holds true even when using [Manual Encryption Key Management]
</Admonition>
...
it to the storage backend. If recording encryption is enabled, the Auth Service
is also responsible for encrypting the event stream prior to upload. Since data
is streamed directly to the Auth Service, Teleport administrators don't need to
be concerned with disk space on their Teleport SSH Service and Proxy Service
instances, as no recording data is written to those disks.
...
Rotation of session recording encryption keys is facilitated through `tctl`.
...
1. Either complete the key rotation or rollback to the previous key
...
does not allow configuring manual encryption key management, and it is not recommended
for use unless it is explicitly requried.
</Admonition>
...
For this reason, it is possible to instead configure Teleport to use existing keys. This
is controlled using the `encryption.manual_key_management` config in either the
`teleport.yaml` config file or the dynamic `session_recording_config` resource.
...
recordings, but will not be used for any further encryptions.
...
`manual_key_management`. It is also important that these keys can be used for decryption
which varies per key storage backend. What this means can vary per key storage backend,
but generally must be defined when the key is first provisioned.
</Admonition>

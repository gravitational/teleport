...
Teleport allows running language models against session recordings to generate
session summaries. This feature supports SSH, Kubernetes, and database
sessions and is only available if Identity Security is enabled.

After each recording is uploaded, Teleport Auth Server matches session metadata
...
No more than 150 concurrent summary jobs will be executed per auth server. If

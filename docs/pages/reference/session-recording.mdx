...
This page provides an overview of how Teleport records interactive sessions and
the various configuration options that apply to session recordings. The concepts
here apply to:
...
TODO(zmb3): add note about app and db recordings  
...
(which may be the node or the proxy depending on your configuration) submits each
recording event to Teleport's Auth Server as it occurs. In this mode, failure to
emit a recording event is considered fatal - the session will be terminated if an
event cannot be recorded. This makes synchronous recording best suited for highly
...
and proxies, as no recording data is written to those disks.
...
the [`tsh play`](cli.mdx#tsh-play) command. Desktop session recordings can only be played back
in the Web UI.

...
sudo teleport start --roles=app --token=xyz --auth-server=proxy.example.com:443 \
...
| TELEPORT_PROXY | Address of the Teleport proxy server |cluster.example.com:443 |
...
# https://teleport.example.com:443/web/reset/8a4a40bec3a31a28db44fa64c0c70ca3
...
INFO [CLIENT]    RedirectURL set to "https://teleport.example.com:443/v1/webapi/github/callback"
kind: github
metadata:
  name: github
spec:
...
$ tctl status \
    --auth-server=192.168.99.102:443 \
...
```code
$ tbot proxy --destination-dir=./tbot-user --proxy=proxy.example.com:443 ssh alice@node:3022
```
...
```code
$ tbot proxy --destination-dir=./tbot-user --proxy=proxy.example.com:443 db --port=1234 example
```
...
```code
$ tbot proxy --destination-dir=./tbot-user --proxy=proxy.example.com:443 db --tunnel --port=1234 example
```

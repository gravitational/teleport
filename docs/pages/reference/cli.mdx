...
#After retrieving the recorded session can be played with tsh play
$ tsh play c0a02222-70dc-4be8-9308-7b7901d2d60

# Retrieve recorded sessions that occurred between Nov 1, 2022 to Nov 3, 2022
...
Recorded sessions are linked from the audit events to session recordings files in their [storage backend](../reference/backends.mdx).
The following error can occur if a session recording file is not available or when employing multiple auth servers with directory storage
backend for recorded sessions. When using directory storage backend for audit logs and recorded sessions then only the auth server with that
recorded session can retrieve it.
...
Using a Security information and event management (SIEM) service that combines the audit logs will help consolidate the list of available recordings. Downloaded recorded session are directly playable as a file. 

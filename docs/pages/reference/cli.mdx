...
Start a local TLS proxy for database connections when using Teleport in single-port mode.
Database connections can be made with certificates through database clients or tunnel mode
...
| Name | Default Value(s) | Allowed Value(s) | Description |
| - | - | - | - |
| `--cert-file` | none | Certificate file| Certificate file for proxy client TLS configuration |
| `--key-file` | none | Key file| Key file for proxy client TLS configuration |
| `tunnel` | none | Enable tunnel mode | Open authenticated tunnel using a database's client certificate so clients don't need to authenticate |
| `--db-user` | none | Database user| Optional database user to log in as |
| `--db-name` | none | Database name| Optional database name to log in to |
...
Start a local TLS proxy for `ssh` connections when using Teleport in single-port mode.
This is typically used as part of the SSH client configuration to use `ssh` as a client 
through Teleport.  See the [OpenSSH Guide](../server-access/guides/openssh.mdx)
...
| Name | Default Value(s) | Allowed Value(s) | Description |
| - | - | - | - |
| `--cluster` | none | Teleport Cluster | Specify the Teleport cluster to connect |
...
#### Examples
```code
#tsh proxy ssh used in combination with a ssh config
$ tsh config
```

```
# Produces the following output for a SSH config
...
```code
# The tsh proxy ssh will be used in the ProxyCommand
$ ssh jeff@node1.example.com
```
...
Start a local proxy for AWS access.  The user should already be logged into
at least one AWS application.
...
| Name | Default Value(s) | Allowed Value(s) | Description |
| - | - | - | - |
| `--app` | Currently logged in AWS app | AWS app| Optional name of the AWS application to use if logged into multiple |
| `-p, --port` | none | port number | Specify the source port for the local proxy |
| ` -e, --endpoint-url` | HTTP Proxy | Endpoint URL | Run the local proxy to serve as an AWS endpoint URL. If not specified, local proxy serves as an HTTPS proxy. |
| `-f, --format` | unix | AWS app| Optional Name of the AWS application to use if logged into multiple |

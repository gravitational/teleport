...
Print the version of your `tsh` binary and version of the Teleport proxy in current `tsh` profile
...
| `--client` | none | none | Only display the `tsh` binary version and do not retrieve the proxy version. |

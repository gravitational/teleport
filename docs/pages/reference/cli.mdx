...
- [teleport](#teleport): A CLI tool that supports the Teleport access plane by starting and configuring various Teleport services.
- [tsh](#tsh): A client CLI tool that allows end users to authenicate to Teleport and access resources in a cluster.
- [tctl](#tctl): An administrative CLI tool that can configure the Teleport Auth Service.
- [tbot](#tbot): A CLI tool that supports Machine ID on Teleport, used to apply short-lived certificates to service accounts (e.g, a CI/CD server).
...
The `teleport` CLI tool can be used to start all services defined in Teleport's required [configuration YAML file](config.mdx).
Services that can be started by the `teleport` CLI include the following:

- [Auth](../architecture/overview/#teleport-auth-service)
- [Node/SSH](../architecture/nodes/)
- [Proxy](../architecture/overview/#teleport-proxy-service)
...
The `teleport` CLI can additionally be used to take the following actions directly:

- [Start and configure the Application Proxy Service for web applications on Teleport](../application-access/guides/connecting-apps/#start-application-service-with-cli-flags).
- [Start and configure the Database Service for Database Access on Teleport](../database-access/introduction/)
- [Generate a systemd unit file for a systemd managed service daemon](../management/admin/daemon/)
- [Generate a sample configuration YAML file for an SSH Node](../architecture/nodes.mdx)

The primary commands for the `teleport` CLI are as follows:

| Command | Description |
| - | - |
| `teleport help` | Outputs guidance for using commands on Teleport. |
| `teleport start` | Starts the `teleport` service in the foregound using the current shell session, as well as any services configured by the [configuration YAML file](config.mdx). |
...
| `teleport app start` | Starts the application proxy service. |
...
The `teleport start` command includes a large number of optional configuration flags which change the behavior of the default command to support a wider spectrum of teleport products and services.
While configuration flags are a supported feature through our CLI for use cases that require it, we recommend using the [Teleport configuration file](config.mdx) in all cases where possible.
...
| `-r, --roles` | `proxy,node,auth,app,db` | **string** comma-separated list of `proxy, auth, node, db, app` or `windowsdesktop` | start listed services/roles. These roles are explained in the [Teleport Architecture](../architecture/overview.mdx) document. |
...
| [App](../application-access/introduction.mdx) | `app` | Provides access to applications using an SSH reverse tunnel. |
| [DB](../database-access/reference.mdx) | `db` | Provides access to databases. |
...
| `tbot configure` | Outputs a basic Machine ID client configuration file, to be adjusted and changed as needed. |
| `tbot start` | Starts the Machine ID client `tbot`, fetching and writing certificates to disk at a set interval. |
| `tbot init` |  Allows certificates to be written to disks other than a Machine ID client, configuring either file or POSIX ACL permissions. |
...
Allows certificates to be written to disks other than a Machine ID client, configuring either file or POSIX ACL permissions.
This allows you to lock down access to Machine ID's short-lived certificates from other users or applications on the system.
...
Allows for access to teleport resources via a proxy using various protocols. Requires running a local ALPN/SNI proxy to access the resources if using TLS.

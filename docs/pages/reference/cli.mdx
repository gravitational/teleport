...
Print the version of your `tsh` binary and current profile Teleport proxy version:
...
| `--client-only` | none | none | Only display the `tsh` binary version and do not retrieve the proxy version. |

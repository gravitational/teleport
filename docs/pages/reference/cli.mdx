...
Recorded sessions are linked from the audit events to session recordings files. Self-hosted high availability Teleport
deployments with directory storage for audit logs and session recordings can result in unavailable playback. That occurs
since each auth server has its own audit log and recorded session records.
...
Using a SIEM that combines the audit logs will help consolidate the list of available recordings. Downloading the 
recorded session is directly playable as a file. 

...
- [tbot](#tbot): A CLI tool that supports Machine ID, which provides short lived credentials to service accounts (e.g, a CI/CD server).
...
The `teleport start` command includes a large number of optional configuration flags which change the behavior of the default command to support a wider spectrum of Teleport services.
We recommend using the [Teleport configuration file](config.mdx) in all cases where possible.
...
teleport start \
   --roles=node \
   --token=010a38c7dab93356bd0a40062f376daa \
   --ca-pin=sha256:1cf31e6e7d3c7ef896b8d938d421ff77cef99d2c0cd026a4c49c5ed581b32e50 \
   --auth-server=172.29.143.214:3025

...
Resources](../../admin-guides/configure-teleport/dynamic-resources/overview.mdx).
...
| connector | Authentication connectors for [Single Sign-On](../../admin-guides/authentication/sso/overview.mdx) (SSO) for SAML, OIDC and GitHub. |
| node | A registered SSH node. The same record is displayed via `tctl nodes ls` |
| windows_desktop | A registered Windows desktop. |
| cluster | A trusted cluster. See [here](../../admin-guides/access-controls/trustedclusters.mdx) for more details on connecting clusters together. |
| [login_rule](#login-rules) | A Login Rule, see the [Login Rules guide](../../index.mdx) for more info. |
| [device](#device) | A Teleport Trusted Device, see the [Device Trust guide](../../admin-guides/authentication/device-trust/guide.mdx) for more info. |
...
Roles not [granting Kubernetes access](../../admin-guides/enroll-resources/kubernetes-access/introduction.mdx) are
...
see [the `kubernetes_resource` documentation](../agents/controls.mdx)
for a complete list).
...
See [Database Access Controls](../../admin-guides/enroll-resources/database-access/rbac.mdx) for more details.

...
<Admonition type="note" title="Note">
  It's best practice to create custom roles for each user according to the *Principle of Least Privilege (POLP)*. 
  Here, for convenience, we'll stick to using built-in roles to speed things along.
</Admonition>
...
sudo tsh login --user=api-admin --proxy=tele.example.com:443
```
...
Now you can run the program and connect the client to the Teleport Auth server and fetch the server version.

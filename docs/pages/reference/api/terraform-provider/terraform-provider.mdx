...
- This auth method does not work against Teleport clusters with MFA set to `webauthn`.
  ...
  [an additional MFA challenge for administrative
  actions](../../../zero-trust-access/authentication/mfa-for-admin-actions.mdx).
...
You can use any [delegated join method](../../deployment/join-methods.mdx#delegated-join-methods) by setting
...
- `join_method` (String) Enables the native Terraform MachineID support. When set, Terraform uses MachineID to securely join the Teleport cluster and obtain credentials. See [the join method reference](../../deployment/join-methods.mdx) for possible values. You must use [a delegated join method](../../deployment/join-methods.mdx#secret-vs-delegated). This can also be set with the environment variable `TF_TELEPORT_JOIN_METHOD`.

...
We recently created a public [Go client](https://pkg.go.dev/github.com/gravitational/teleport/api/client)
to programatically interact with the API. [tsh and tctl](../cli-docs/) use the same API.

## Go Client

The Go client can be used to programatically interact with Teleport. For example:
 - Integrating with external tools, which we have already done 
   for [several tools](../enterprise/workflow/#integrating-with-an-external-tool).
...
Create an API client in 3 minutes with the [Getting Started](./getting-started) Guide.
...
can be configured for use in any environment where a secret can be permanently
stored.
...
The `bound_keypair` was designed for and works best with Machine & Workload ID
bots where certificates are refreshed frequently, but can also be used to join
standard Teleport agents (as of Teleport v18.8) which are issued long-lived
certificates. Note that Bound Keypair for agents is subject to
[certain limitations](../machine-workload-identity/bound-keypair/admin-guide.mdx#limitations-of-bound-keypair-joining-for-teleport-agents)
that should be acknowledged before use.
...
The VM must have a [service
account](https://cloud.google.com/compute/docs/access/create-enable-service-accounts-for-instances)
assigned to it. No IAM roles are required
on the Teleport process joining the cluster. The default service account is only supported for GCP VMs.

For managed container and serverless platforms (e.g. GKE or Cloud Run), a
user-managed service account must be attached to the workload.

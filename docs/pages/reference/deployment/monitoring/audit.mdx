...
logs for the Teleport service recording the session. Depending on the recording
mode (see the next section), this is either the SSH Service or Proxy Service.
...
The Teleport service emits the log with an `error` field that indicates the
underlying cause.

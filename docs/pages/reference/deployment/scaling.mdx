...
To host Teleport on Kubernetes, our recommended deployment method, see
...
- The Auth Service needs a highly available backend. The default standalone mode
  ...
  or configure an external backend such as DynamoDB, Firestore, or etcd. See
  [Storage backends](backends.mdx) and the
  [High Availability guide](../../installation/self-hosted/deployments/high-availability.mdx).
- The Proxy Service needs a shared TLS certificate source. Unless you enable
  cert-manager (`highAvailability.certManager`), provide an existing secret
  (`tls.existingSecretName`), or use an ingress (`ingress.enabled`), the chart
  keeps the Proxy Service at a single replica regardless of the value below.

...
For example to spawn 32 requests per second for 2 minutes using a custom payload script against a specific agent:
```sh
tsh bench ssh --rate=32 --duration=2m user@node-agent -- /tmp/node-work.sh
```

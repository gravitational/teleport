...
Teleport instance is attempting to connect to an upstream host that has exited
the process that was listening on the required port.
...
expected a TLS handshake from the other side of the connection and did not
receive one, i.e., it was replying to the Teleport process in a different
protocol.

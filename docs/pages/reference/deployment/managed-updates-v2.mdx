...
sidebar_label: Managed Updates Resources
...
This page lists the fields you can configure in the dynamic resources related to
Managed Updates v2.

See also:
- the [Managed Updates Architecture](../architecture/agent-update-management.mdx) describing the agent update
  architecture and security invariants.
- the [Managed Updates Guide](../../upgrading/agent-managed-updates/agent-managed-updates.mdx) for more details on how to
...
These resources only track Teleport Agent installations.
However, if tbot is deployed alongside a Teleport Agent, a tbot update failure will cause the agent update to revert,
and the agent update will be marked as failed.
...
Each Teleport Auth Service instance generates one report every minute.

...
- `disablechecksum=true` - set to `true` or `false`. If `true`, Teleport will
  disable automatic trailing checksum calculation for S3 multipart uploads.
  This is useful for S3-compatible services that do not support trailing
  checksums. Default value is `false`.

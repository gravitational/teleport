...
Starting from PostgreSQL versions 18.6, 17.11, 16.15, 15.19, and 14.24, the correct setup of `wal2json` for use by Teleport requires setting the [`output_plugin_libraries` parameter](https://www.postgresql.org/docs/current/runtime-config-replication.html#GUC-OUTPUT-PLUGIN-LIBRARIES) in the PostgreSQL config file to include `wal2json`.

If this is not done during the first setup, or if PostgreSQL is later upgraded to a version that needs the `output_plugin_libraries` parameter, the Teleport Auth Service instances will periodically output error logs containing `Change feed stream lost.` and `ERROR: library "wal2json" may not be used as an output plugin (SQLSTATE 42501)`. The problem is temporary and no data is lost, but the Teleport cluster will not work correctly until the PostgreSQL configuration is fixed.

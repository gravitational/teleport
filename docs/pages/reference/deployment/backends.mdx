...
Determining an optimal defragmentation schedule is non-trivial, as it depends on factors such as workload characteristics,
key churn rate, storage backend behavior, and the specifics of the deployment environment. As a general starting point, it is
advisable to perform defragmentation on a weekly basis during periods of low cluster activity.
...
A typical Teleport resource is between 1 and 3 KiB in size.

For a cluster with 50k resources, each 2 KiB in size and retention history of 5 revisions we would expect
the dataset to be at least 488 MiB. An additional 20-40% safety margin is recommended for metadata and fragmentation overhead.

Given an existing typical resource you can estimate its size with the following command:

```sh
tctl get <Var name="resource" /> | base64  -w 0 | awk '{ total += length($0) } END { print total * 2 }'
```

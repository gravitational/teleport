...
This should be enough to configure mTLS on etcd side, but to configure it on
Teleport side, you need one more thing: client certificates. Here's one way to
do it:
...
1. For each Auth Service instance, generate the client certificate:
   ```sh
   cfssl gencert -ca=certs/ca.pem -ca-key=certs/ca-key.pem -config=config/ca-config.json -profile=client client.json | cfssljson -bare certs/client
   ```
...
etcd --cert-file=certs/host.pem \
  ...
  --peer-cert-file=certs/peer-host-key.pem \

...
|-------------------------|---------------------------------------------------------------------------------------------------------------------------------------------|-------------------------------------------------------------------------------------------------------------|
| core cluster state      | Cluster configuration (e.g. users, roles, auth connectors) and identity (e.g. certificate authorities, registered nodes, trusted clusters). | Local directory (SQLite), etcd, PostgreSQL, Amazon DynamoDB, GCP Firestore, CockroachDB, Amazon Aurora DSQL |
...
`/readyz` [diagnostics endpoint](../../zero-trust-access/management/diagnostics/monitoring.mdx) on
...
- Install etcd and configure peer and client TLS authentication using the [etcd
  security guide](https://etcd.io/docs/v3.6/op-guide/security/). See
  [Configuring mTLS](#configuring-mtls-authentication-with-etcd) below for
  details.
...
### Configuring mTLS authentication with etcd

We recommend using [cfssl](https://github.com/cloudflare/cfssl) and etcd's
[`tls-setup` script](https://github.com/etcd-io/etcd/tree/main/hack/tls-setup)
to configure mTLS between Teleport and etcd. It's helpful for creating
certificates and keys for the entire etcd cluster. After [installing
cfssl](https://github.com/cloudflare/cfssl?tab=readme-ov-file#installation),
clone the `tls-setup` repo and follow the `Readme.md` file to customize the
script configuration. Make sure to tweak the makefile if you change the number
of etcd nodes (and thus the number of `infra` variables).

After you run `make`, the following files will be generated:

- `tls-setup/certs/ca.pem` is the certificate authority and it should be
  referenced in Teleport's `tls_ca_file` option (see [Configuring Auth Service
  instances to use etcd](#configuring-auth-service-instances-to-use-etcd)
  below).
- `tls-setup/certs/peer-*` files are used internally by etcd for communicating
  between nodes.
- `tls-setup/certs/<host>.pem` and `tls-setup/certs/<host>-key.pem` are the
  host certificates and their corresponding private keys. Each corresponding
  etcd host needs to know them, and they are required for providing transport
  security on the etcd side.

This should be enough to configure mTLS on etcd side.

On the Teleport side, you need to configure client certificates. Here's one way
to do it:

1. Generate a default CSR:
   ```code
   $ cfssl print-defaults csr > client.json
   ```

1. Remove all hostnames from the hosts element of the `client.json` file, as
   this CSR doesn't represent any host, but a client. Also customize any other
   options if required.

1. For each Auth Service instance, generate the client certificate:
   ```code
   $ cfssl gencert -ca=certs/ca.pem \
     -ca-key=certs/ca-key.pem \
     -config=config/ca-config.json -profile=client client.json \
     | cfssljson -bare certs/client
   ```
   This assumes that the `tls-setup` tool has been previously run and generated
   the `certs/ca.pem` and `certs/ca-key.pem` files.

   The following files will be generated:

   - `certs/client.pem`, which will be referenced by the `tls_cert_file` option
...
$ etcd --cert-file=certs/host.pem \
  ...
  --advertise-client-urls=https://<Var name="etcd-host"/>:2379 --listen-client-urls=https://<Var name="etcd-host"/>:2379
  # ... other applicable options
 ```

### Configuring Auth Service instances to use etcd

Put these settings into the configuration file of all Auth Service instances:
...
teleport:
  storage:
     ...
     # Required path to TLS client certificate and key files to connect to
     # etcd.
     #
     # If you used cfssl as described above, these should be the
     # certs/client.pem and certs/client-key.pem files from the previous steps.
     ...
     #
     # If you used the tls-setup script, this CA certificate should be the
     # certs/ca.pem file.
...
The PostgreSQL backend requires PostgreSQL 14 or later, and, for the cluster
...
The `wal2json` plugin should be added to the [`output_plugin_libraries` parameter](https://www.postgresql.org/docs/current/runtime-config-replication.html#GUC-OUTPUT-PLUGIN-LIBRARIES) in `postgresql.conf`:

```
# the default is 'pgoutput, test_decoding', but if other plugins
# are in use, make sure to add wal2json to the existing list
...
The requirement for `output_plugin_libraries` is new starting from PostgreSQL versions 18.6, 17.11, 16.15, 15.19, and 14.24.

If the `output_plugin_libraries` parameter was not configured during the initial setup and PostgreSQL is later upgraded to a version that needs it, the Teleport Auth Service instances will periodically output error logs containing `Change feed stream lost.` and `ERROR: library "wal2json" may not be used as an output plugin (SQLSTATE 42501)`. The problem is temporary and no data is lost, but the Teleport cluster will not work correctly until the PostgreSQL configuration is fixed.
...
[on GitHub](https://github.com/gravitational/teleport/blob/branch/v(=teleport.major_version=)/examples/dynamoathenamigration/README.md).
...
teleport:
  storage:
    ...
    # parameters see https://www.cockroachlabs.com/docs/v25.3/connection-parameters.html
    #
    # If your certificates are not stored at the default ~/.postgresql
    # location, you will need to specify them with the sslcert, sslkey, and
    # sslrootcert parameters.
...
<Admonition type="warning" title="Enterprise-only feature">
  Use of the DSQL storage backend requires Teleport Enterprise.
</Admonition>

<Admonition type="note" title="Version requirement">
  The DSQL storage backend is supported in Teleport v19.0.0 and later.
</Admonition>

Teleport can use [Amazon Aurora DSQL](https://aws.amazon.com/rds/aurora/dsql/) as a storage backend to achieve high availability and survive regional failures.

### Infrastructure setup

The DSQL storage backend requires a DSQL cluster and a Kinesis stream receiving events from a DSQL [Change data capture stream](https://docs.aws.amazon.com/aurora-dsql/latest/userguide/cdc-streams.html). For multi-region operation, a multi-region DSQL cluster in two peered regions ([in the same region set](https://docs.aws.amazon.com/aurora-dsql/latest/userguide/what-is-aurora-dsql.html#aurora-dsql-multi-region-availability)) can be used by Teleport Auth Service instances from either region, and each cluster pair can be configured to emit CDC events to a regional Kinesis data stream.

Since it's not currently possible to configure the CDC stream in such a way that would restrict the changes available to any reader of the Kinesis data stream, we strongly recommend not using the same DSQL cluster for additional workloads, as the Auth Service instances will have access to the unrestricted change feed for the whole cluster, which is essentially equivalent to having full read permissions over every table in every schema of the cluster.

<Admonition type="danger">

No more than two Auth Service instances should be reading from the same Kinesis data stream, or it's possible that throttling will prevent Teleport from keeping up with the event stream; as such, you should deploy no more than two Auth Service instances per region.

</Admonition>

Teleport will only use a single schema in the DSQL cluster, expecting to be able to create tables and run migrations as needed. Because of this, we recommend either letting Teleport use the `admin` database user (requiring permissions to use the `dsql:DbConnectAdmin` action) and the pre-existing `public` schema, or you should [create a database user](https://docs.aws.amazon.com/aurora-dsql/latest/userguide/using-database-and-iam-roles.html#using-database-and-iam-roles-custom-database-roles) and grant it full permissions over a schema (Teleport will then need permissions to use the `dsql:DbConnect` action):

```sql
CREATE ROLE teleport WITH LOGIN;
AWS IAM GRANT teleport TO 'arn:aws:iam::0123456789012:role/<role name>';
CREATE SCHEMA teleport;
GRANT ALL PRIVILEGES ON SCHEMA teleport TO teleport;
```

It is not possible to grant all privileges on the `public` schema to a non-admin user.

The IAM role used by Teleport will need, at a minimum, permissions for `kinesis:ListShards`, `kinesis:GetShardIterator` and `kinesis:GetRecords` on the Kinesis data stream that's receiving the DSQL CDC stream, and either `dsql:DbConnectAdmin` or `dsql:DbConnect` on the DSQL cluster to use the `admin` database user or some other database user, respectively. Depending on the configuration, additional permissions might be required - for example, [user-generated KMS keys in Kinesis require `kms:Decrypt`](https://docs.aws.amazon.com/streams/latest/dev/permissions-user-key-KMS.html).

### Configuration

The cluster state backend configuration lives in the `teleport.storage` section of the `teleport.yaml` config file:
...
teleport:
  storage:
    ...
    # The identifier of the cluster, also shown as "Cluster ID" in the
    # AWS console. Required.
    dsql_identifier: ln7855l7l8l77ln4m5v8787lm7n7vl
    # The region that the DSQL cluster is in. In multi-region setups,
    # auths in different regions should be pointed at different DSQL
    # clusters in different regions. Required.
    dsql_region: us-west-2

    # The full hostname used to connect to the DSQL cluster over AWS
    # PrivateLink, it should be left unset or empty otherwise. The
    # hostname depends on the VPC endpoint service name, which can
    # change from cluster to cluster (see https://docs.aws.amazon.com/aurora-dsql/latest/userguide/privatelink-managing-clusters.html#privatelink-connecting-clusters).
    dsql_endpoint: ln7855l7l8l77ln4m5v8787lm7n7vl.dsql-7cwu.us-west-2.on.aws

    # The name of the AWS profile used as a source of credentials and
    # configuration for DSQL. If unset or blank, the default profile
    # is used. A non-default profile can be used to use a different
    # source for the credentials used to connect to DSQL, or to assume
    # a specific role. The profile configuration, by default, is read
    # from the ~/.aws/config file, but a different file can be used
    # by specifying the aws_config_file_path option (see below).
    dsql_aws_profile: profile-name

    # The name of the Kinesis data stream that is receiving CDC events
    # from the DSQL cluster. Required.
    kinesis_stream_name: change-feed-stream-name
    # The region that the Kinesis data stream is in. In multi-region
    # setups, each DSQL cluster peer should be configured to emit CDC
    # events to a regional Kinesis data stream. Required.
    kinesis_stream_region: us-west-2

    # The name of the AWS profile used as a source of credentials and
    # configuration for the Kinesis data stream. If unset or blank,
    # the default profile is used.
    kinesis_aws_profile: profile-name

    # The path to an AWS config file (see https://docs.aws.amazon.com/sdkref/latest/guide/file-format.html).
    # If set, only the specified file will be used as configuration for
    # AWS credentials for DSQL and Kinesis. If unset, the default
    # configuration file (at ~/.aws/config) is used.
    aws_config_file_path: path/to/aws/config

    # The username and schema name to use in the DSQL cluster. The
    # "admin" user requires permissions for the dsql:DbConnectAdmin
    # action rather than the dsql:DbConnect action. The schema must
    # already exist and the selected user should have full privileges
    # on it (as such, the public schema is only really suitable for
    # use by the admin user).
    database_user: admin
    database_schema: public

    # The maximum amount of parallel connections that Teleport will
    # open to use the DSQL cluster. A higher number of connections
    # can reduce latency but it will increase the memory requirements
    # of the Auth Service instance.
    max_conns: 1000
```

### Monitoring

In addition to the general Teleport backend metrics (see the [Teleport Metrics reference](./monitoring/metrics.mdx)), DSQL and Kinesis have specific CloudWatch metrics that should be monitored. The DSQL `CommitLatency` and `OccConflicts` metrics can warn about problems with the service, and the `BehindSourceLag` and `IsImpaired` metrics for the CDC stream can signal that something is wrong with the CDC configuration. The Kinesis metrics `ReadProvisionedThroughputExceeded` and `GetRecords.IteratorAgeMilliseconds` can warn about Teleport not being able to keep up (perhaps because of throughput throttling, but it could be caused by network problems between Teleport and Kinesis).

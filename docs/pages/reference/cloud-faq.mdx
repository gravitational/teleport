---
title: Teleport Enterprise FAQ
description: Teleport Enterprise frequently asked questions.
tocDepth: 3
---
...
### Can a customer deploy multiple clusters in Teleport Enterprise?

[Reach out to sales](https://goteleport.com/signup/enterprise) to discuss pricing.

### If I start with Teleport Enterprise, can I move to Teleport Enterprise or Teleport Community Edition, or do I need to start again?
...
### How long will Teleport Enterprise retain my data?
...
### Does your SOC 2 report include Teleport Enterprise?
...
### Can I configure an allowlist of inbound IP addresses to Teleport Enterprise?

We do not have plans to offer support for inbound connection IP allowlists.

We believe mTLS with strong user and [device identity](../admin-guides/access-controls/device-trust/guide.mdx) provides the best available
...
### How do I add resources to Teleport Enterprise?
...
After [connecting](#how-can-i-access-the-tctl-admin-tool) `tctl` to Teleport Enterprise, users can generate
...
### Is there a way to forward Teleport Enterprise audit events to my company's internal Security Information and Event Management (SIEM)?

Yes. We recommend Teleport's [event handler plugin](../admin-guides/management/export-audit-events/fluentd.mdx) to export Teleport Enterprise audit events.
...
- Teleport Auth Service and Teleport Proxy Service updates are pushed to Teleport Enterprise accounts
...
### Are updates times configurable for Teleport Enterprise?
...
for all Teleport Enterprise accounts. Updates for your cluster won't start earlier than the
...
Teleport recommends you check the status of Teleport Enterprise at
[status.teleport.sh](https://status.teleport.sh). From the status page, click
**Subscribe to Updates** to get email notifications whenever Teleport Enterprise creates,
updates, or resolves an incident and stay informed about scheduled maintenance and
new releases.

### When are agents automatically updated?

Teleport Enterprise must be set to receive automatic updates to use the
Teleport Enterprise version server for automatic agent updates. With automatic agent
...
### Which Proxy Service ports are open on my Teleport Enterprise tenant?

Teleport Enterprise allocates a different set of ports to each tenant. To see which
ports are available for your Teleport Enterprise tenant, run a command similar to the
...
### Where does Teleport Enterprise run?

Teleport Enterprise runs on Amazon Web Services (AWS). We run proxies in a variety 
...
### Is IPv6 Supported for connections to Teleport Enterprise?

We don't currently support IPv6 connections to Teleport Enterprise.
...
FIPS is not currently an option for Teleport Enterprise clusters.


## Performance and reliability

### Can I use Teleport Enterprise in production?

Yes. Large organizations leverage Teleport Enterprise to manage the vast number of resources in their organization. Teleport Enterprise is audited regularly to ensure the most reliable and secure service possible is available to our customers.
...
### Can I get push notifications of Teleport Enterprise downtime?

Yes. Customers can subscribe to Teleport Enterprise updates at [status.teleport.sh](https://status.teleport.sh).
...
Teleport Enterprise tenants are made up of a cluster of processes, with designated processes sitting behind a load balancer. To scrape the entire cluster would require each component of

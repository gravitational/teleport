...
`version` is an optional string that specifies the role version (default `v6`).
...
|     `kubernetes_resources`|         set | List of Kubernetes Resources the Role applies to                |
...
**spec.allow.kubernetes_resources** and **spec.deny.kubernetes_resources**

Kubernetes resources represents the resources the role allows/denies access to. The following fields are supported:

|                `kind`|      string | Resource kind. Supported values: `pod`                                          |
|                `name`|      string | Resource name in the Kubernetes cluster. Supports regular expressions and wildcards (`*`)       |
|           `namespace`|      string | Resource namespace in the Kubernetes cluster. Supports regular expressions and wildcards (`*`)  |

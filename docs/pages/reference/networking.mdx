...
the Teleport Auth Service](../enroll-resources/agents/join-services-to-your-cluster/join-token.mdx#start-your-teleport-process-with-the-invite-token).
...
Direct auth joining is only supported for SSH and Kubernetes agents. Application, database, and other discovery-based features are not supported when joining auth directly.
</Admonition>

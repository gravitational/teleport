...
In self-hosted Teleport deployments, you can configure the Teleport Proxy
Service to provide public addresses for specific listeners that are separate
from the main Proxy Service web address. See the following table for the public
addresses that are available to configure:

| Service | Configuration field |
| - | - |
| Proxy peering connections | `proxy_service.peer_public_addr`|
| Kubernetes traffic | `proxy_service.kube_public_addr` |
| SSH traffic | `proxy_service.ssh_public_addr` |
| Reverse tunnel traffic | `proxy_service.tunnel_public_addr` |
| MySQL database traffic | `proxy_service.mysql_public_addr` |
| PostgreSQL database traffic | `proxy_service.postgres_public_addr` |
| MongoDB traffic | `proxy_service.mongo_public_addr` |
...
| Port | Downstream Service | Description | Configuration Field |
| - | - | - | - |
| 27017 | Database Service | Traffic to MongoDB instances.| `proxy_service.mongo_listen_addr` |
| 3021 | Proxy Service | Port used by Teleport Proxy Service instances to dial agents in Proxy Peering mode. |`proxy_service.peer_listen_addr`|
| 3023 | All clients | SSH port clients connect to. The Proxy Service will forward this connection to port `3022` on the destination service. | `proxy_service.ssh_public_addr`|
| 3024 | Auth Service | SSH port used to create reverse SSH tunnels from behind-firewall environments into a trusted Proxy Service instance. | `proxy_service.tunnel_public_addr` |
| 3026 | Kubernetes Service | Traffic to Kubernetes clusters.| `proxy_service.kube_listen_addr` |
| 3036 | Database Service | Traffic to MySQL databases.| `proxy_service.mysql_listen_addr` |
| 3080 or 443 | Proxy Service | HTTPS connection to authenticate `tsh` users into the cluster. The same connection is used to serve a Web UI. | `proxy_service.web_listen_addr` |
| 5432 | Database Service | Traffic to Postgres databases.| `proxy_service.postgres_listen_addr` |

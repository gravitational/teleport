...
<Notice type="note">
Modes only take effect if session recording is enabled at the cluster-level, and
are only present for SSH sessions.
</Notice>

Modes specify whether the session is recorded and how Teleport handles recording
failures, such as a full disk error. They are configured per service at the role
level, where the strictest value takes precedence. The available modes are:
`off`, `best_effort` and `strict`.

To turn off the recordings *all* user's roles must set the value to `off`.
Otherwise, the strictest value will be used.

When enabling the recording, there are two available modes, which determine how
Teleport will proceed in case of recording failures:
...
If the user role doesn't specify a recording mode, `best_effort` will be used.
Here is an example of a role configured to use strict mode for SSH sessions:
...
<Notice type="note">
When using a `sync` recording (for example, `node-sync`), the recording mode
will always be set to `strict` when enabled.
</Notice>

In addition to specifying the record option per service, you can also set a
default value, which is used when the service doesn’t have a value. For example,
given the two roles below, the value used will be `strict`, which is the
strictest among all user roles.

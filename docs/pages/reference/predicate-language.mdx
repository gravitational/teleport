...
## Matching session summary inference policies

Teleport allows matching session recordings to summarization inference models
using `inference_policy` resources. In this case, the predicate language
supports the same functions and operators as in the [allow/deny rules in role
resources](#scoping-allowdeny-rules-in-role-resources-and-matching-session-summary-inference-policies). The following objects are always available:

| Field    | Meaning                                                     | Example                                        |
|----------|-------------------------------------------------------------|------------------------------------------------|
| user     | User that created the session                               | `contains(user.spec.traits["teams"], "alpha")` |
| resource | Resource that this session connects to (server or database) | `contains(user.spec.traits["teams"], "alpha")` |
| session  | The session that is being matched                           | `contains(session.participants, "admin")`      |

For more information, see:

- [Selecting sessions to summarize](../identity-security/session-summaries.mdx#selecting-sessions-to-summarize)
- [`inference_model` reference](./resources.mdx#inference-policy)

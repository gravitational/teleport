...
[supply chain attacks](https://www.cloudflare.com/en-gb/learning/security/what-is-a-supply-chain-attack/).
...
These signatures will be sent to the Teleport auth server when requesting a SVID.

If your `WorkloadIdentity` rules contain a call to `sigstore.policy_satisfied`,
the auth server will load and evaluate the named `SigstorePolicy` objects.
  ...
  keyless:
    identities:
      - issuer: https://token.actions.githubusercontent.com
        subject_regex: ^https://github.com/boxofrad/(.*)/\.github/workflows/(.*)@refs/heads/main$
  requirements:
    attestations:
      - predicate_type: https://slsa.dev/provenance/v1
```

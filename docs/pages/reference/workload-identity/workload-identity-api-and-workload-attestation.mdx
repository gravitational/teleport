...
- `tbot` can only reliably determine the a process' executable path if it's
  ...
  be empty or incorrect if the workload is containerized.
...
- Because `/proc/$pid/exe` is a symlink to an *inode* rather than a regular path,
  ...
  process' executable is on a network mount, it's possible for `binary_hash` to
  be out of date.

- `tbot` will only checksum binaries smaller than 100MiB in size.

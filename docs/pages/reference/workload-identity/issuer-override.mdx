...
description: Information about the `workload_identity_x509_issuer_override` resource
---
...
The public keys in the `issuer` certificates in the `spec.overrides` list must be unique across the whole list. When issuing an X509-SVID credential, the Teleport instance in the control plane serving the request will choose one of the active and usable X.509 certificates in the SPIFFE certificate authority, and then it will select the `issuer` certificate with the same public key in the default `workload_identity_x509_issuer_override`. Neither the `issuer` certificate nor the certificates in its `chain` are checked for expiration or validity, and if no `issuer` is found with the appropriate key, the X509-SVID issuance will fail, so it's important to keep the `workload_identity_x509_issuer_override` resource updated as time passes or if a CA rotation for the SPIFFE certificate authority is initiated. In usual conditions only a single certificate is present in the SPIFFE certificate authority, and only one override is thus required in the `workload_identity_x509_issuer_override` resource.

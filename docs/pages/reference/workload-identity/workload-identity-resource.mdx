...
Templating in the WorkloadIdentity resource's fields is built on top of Teleport's
[Predicate Language] and you can use its text manipulation functions such as
`strings.lower` and `regex.replace`.

[Predicate Language]: https://goteleport.com/docs/reference/predicate-language
...
Each rule consists of either a [Predicate Language] expression or a set of
conditions. When using the conditions form, all conditions within that rule must
pass in order for the rule to be considered a pass. If you specify multiple
rules, then at least one rule must pass in order for the WorkloadIdentity to be

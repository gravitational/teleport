...
- If multiple automatic review rules match an Access Request, `DENIED` rules
  take precedence.
...
- `!is_empty(access_request.spec.roles)` - Applies if the request contains at least one role.
- `access_request.spec.user == "example_user"` - Applies if created by `example_user`.
- `access_request.spec.roles.contains("example_role")` - Applies if the `example_role` role is requested.
- `set("role_1", "role_2").contains_all(access_request.spec.roles)` - Applies if all requested roles are either `role_1` or `role_2`.
- `contains_any(user.traits["team"], set("example_team"))` - Applies if the user has trait `team: example_team`.

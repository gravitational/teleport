...
### Key details about `client_idle_timeout`

The `client_idle_timeout` in Teleport is a configurable setting that helps improve security by terminating inactive sessions after a specified period. It can be applied globally or per role, allowing for flexibility based on your organization's security policies. 
This feature helps ensure that unattended sessions don’t pose a security risk, and can prevent unauthorized access in case a user leaves their session open and unattended. 
It enforces session timeouts when no activity is detected from the user.

#### Purpose
   - The `client_idle_timeout` configuration ensures that any SSH session, application session, or database connection that remains inactive for a certain period of time is automatically terminated.
...
#### Configuration
   The `client_idle_timeout` can be configured globally or per role, giving administrators flexibility in how they apply idle timeout rules.
   ...
   - Global configuration (applies to all users):
 	...
 	```
 	This example configures a global idle timeout of **30 minutes**. After 30 minutes of inactivity, the session will be terminated.
   ...
   - Per-role configuration (applies to specific users or groups):
 	...
 	```
 	In this example, the `example-role` has an idle timeout of **15 minutes**.
...
#### Example command to apply role-based timeout
   If you're updating an existing role to include a `client_idle_timeout`, you would modify the role configuration like this:
   ```bash
   tctl create -f example-role.yaml
   ```

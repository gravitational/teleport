...
spec:
  options:
    client_idle_timeout: 30m  # Idle timeout set to 30 minutes
  allow:
    node_labels:
      'env': 'stage'
...
#### Purpose
   The `client_idle_timeout` configuration ensures that SSH sessions, web application, kubectl, or database connection that remain inactive for a certain period of time are automatically terminated. You can also use it to set limits for 
   SSH sessions initiated *from* the web interface (note that this is distinct from `web_idle_timeout`, which sets idle timeout for the Web UI). This helps to mitigate risks associated with unattended sessions, such as unauthorized access.
...
You can set the `client_idle_timeout` globally in the Teleport cluster configuration (`teleport.yaml`) under the `auth_service` section:
 	...
 	```
 	This example configures a global idle timeout of **15 minutes**. After 30 minutes of inactivity, the session will be terminated.
...
You can also specify the timeout on a per-role basis, allowing different users or groups to have different timeout settings. For example, you might want a shorter timeout for higher-privileged roles.
 	...
 	In this example, the `admin-role` has an idle timeout of **10 minutes**.

...
copy loses all allow `app_labels` and `app_labels_expression`, so the role
no longer grants access to any app, including TCP, MCP, and cloud apps,
since a role does not distinguish app types. Each role is rewritten by
itself, so a separate v8-or-older role granting the same app is served
unchanged and still opens the app fully on older agents. Upgrade all
Application Service agents to Teleport 19 before using role v9 to restrict
app access.

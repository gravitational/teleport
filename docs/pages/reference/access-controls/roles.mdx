...
There are currently seven supported role versions: `v3`, `v4`, `v5`, `v6`, `v7`, `v8`,
and `v9`.
...
Roles with version `v5` or higher do not allow joining sessions by default. The
...
The SAML IdP role option `spec.idp.saml.enabled: true/false` is only supported in role version 7 and below.
...
Role v9 makes application access deny-by-default. When a v9 role grants an
app via `app_labels` or `app_labels_expression`, HTTP requests to that app
are denied unless a rule under `spec.allow.app_resources` grants them.
Currently, the only supported rule is `allow_all: true`, which restores
unrestricted access. Fine-grained path and method rules are not yet
supported. When v8 and v9 roles grant the same app, Teleport drops the v8
roles from the decision so they cannot re-open unrestricted access. Role
v9 does not affect TCP, MCP, cloud (AWS console, Azure, and GCP), or LLM
apps.

Only v9-aware agents enforce this. For agents running Teleport 18 or
earlier, the Auth Service serves a downgraded v8 copy of each role. With
`allow_all: true`, the copy grants exactly the v8 access. Without it, the
copy moves the allow `app_labels` and `app_labels_expression` to the deny
side, so the downgraded role denies those apps instead of granting them,
including TCP, MCP, and cloud apps, since a role does not distinguish app
types. Because deny rules override allow rules across all of a user's
roles, a separate v8-or-older role can no longer reopen the same app on
older agents. The moved selector denies conservatively, so in a
mixed-version cluster it can deny more apps than the v9 role allowed.
Upgrade all Application Service agents to Teleport 19 before using role v9
to restrict app access.
...
|`cluster_labels`|[Trusted Clusters](../../zero-trust-access/management/trustedclusters.mdx)|

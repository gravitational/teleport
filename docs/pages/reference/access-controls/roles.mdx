...
spec:
  options:
    client_idle_timeout: 30m  # Idle client timeout set to 30 minutes
  allow:
    node_labels:
      'env': 'stage'
...
This example configures a global client idle timeout of **30 minutes**. After 30 minutes of inactivity, the session will be terminated.
...
The `client_idle_timeout` configuration ensures that SSH sessions, desktop sessions, kubectl exec or database connections that remain inactive for a certain period of time

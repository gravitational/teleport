...
Role v9 is supported in Teleport 19 and above. It introduces `app_resources`,
which controls how requests for HTTP applications are authorized.

For details on access controls for HTTP applications, refer to
[Application Resources](./app-resources.mdx).
...
| `session.proto` | The proto used for the session (e.g., "ssh", "kube", "db", "tdp", "app"). | `string` |
...
<TabItem value="Application Sessions" label="Application Sessions">

The following application session fields can be used in `where` expressions to filter access
for application sessions together with the user and session fields above.

| Application Session Field | Description | Type |
|---|---|---|
| `session.app_name` | The name of the application where the session took place. | `string` |
| `session.app_uri` | The endpoint of the application where the session took place. | `string` |
| `session.app_public_addr` | The public address of the application where the session took place. | `string` |
| `session.app_labels` | The labels assigned to the application where the session took place. | `dict<string, string>` |

Only HTTP/HTTPS applications produce session recordings, so these fields are not
...
|`windows_desktop_labels`|[Windows desktops](../../enroll-resources/desktop-access/desktop-access.mdx)|
|`linux_desktop_labels`|[Linux desktops](../../enroll-resources/desktop-access/linux-desktop.mdx)|
...
|`{{internal.linux_desktop_logins}}`|List of allowed Linux desktop logins for the user.|

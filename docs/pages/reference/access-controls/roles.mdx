...
Role `v8` added support for kubernetes CRDs and introduced some change from `v7` on how `kubernetes_resources` are behave and are defined.

See See [Kubernetes Resources](../../enroll-resources/kubernetes-access/controls.mdx#kubernetes_resources) for more details about the difference between versions.

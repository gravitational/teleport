...
1. Restrict to Session Participants
...
4. Restrict by Session Creator

Allow viewing only sessions created by the user:

```yaml
version: v5
kind: role
metadata:
  name: own-created-sessions
spec:
  allow:
    rules:
    - resources: [session]
...
By leveraging these where expressions, you can implement sophisticated access control policies that precisely

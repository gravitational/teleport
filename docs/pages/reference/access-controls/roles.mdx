...
| `web_terminal_clipboard_mode` | Whether users can copy text from an SSH session to the clipboard in the Web UI terminal (`unrestricted` or `no-copy`) | Logical "AND" i.e. if any role is set to `no-copy`, copying is disabled |

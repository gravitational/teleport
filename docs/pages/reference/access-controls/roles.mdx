...
```yaml
where: contains(user.spec.traits["team"], "devops")
where: contains(user.spec.roles, "admin")
```
...
It contains metadata about the session, including the participants, protocol,
...
| `session.protocol` | The protocol used for the session (e.g., "ssh", "kube", "db", "rdp"). | `string` |
...
```yaml
# List only sessions initiated by the user
where: session.user == user.metadata.name
# List sessions where the user has the "devops" team trait
where: contains(session.user_traits["team"], "devops")
# List sessions initiated by users with the "admin" role
where: contains(session.user_roles, "admin")
# List sessions for users with the same "department" trait as the authenticated user
where: contains_any(session.user_traits["department"], user.spec.traits["department"])
# List sessions for users with the same roles as the authenticated user
where: contains_any(session.user_roles, user.spec.roles)
```
...
| `all_end_with` | `all_end_with(array<string>, string) -> bool` | Returns true if all elements in the array end with the specified suffix. |
| `all_equal` | `all_equal(array<string>, string) -> bool` | Returns true if all elements in the array are equal to the specified value. |
| `has_prefix` | `has_prefix(string, string) -> bool` | Returns true if the string starts with the specified prefix. |
| `is_subset` | `is_subset(array<string>, [string...]) -> bool` | Returns true if all elements of the second array are contained in the first array. |


## RBAC for active sessions

It is possible to further limit access to
[shared sessions](../../connect-your-client/tsh.mdx).

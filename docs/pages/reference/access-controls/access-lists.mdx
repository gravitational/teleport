...
For example, an organization might have the following two Access Lists:
"Engineering Team" and "Production Access". "Engineering Team" represents a
group of engineers, while "Production Access" is a higher-level Access List that
grants access to production resources.

### Creating a nested Access List

To create a nested Access List, which consists of a parent and a child:

1. Make sure the child Access List exists. If it does not, create it by
   navigating to **Identity Governance** > **Access Lists** > **Create New
   Access List** and configure your new Access List, selecting members in the
   **Members** section.
1. Create the parent Access List by following the same flow. When selecting
   members, select the name of the child Access List.

To confirm that members of the parent Access List have inherited member grants
from the child Access List:

1. Log in as a user who is a member of the child Access List. 
1. Verify that the user now has access to resources granted by both the parent
   and the child Access List. For example, if a Teleport Application Service
   instance with the debugging application enabled is set up, and the `access`
   role is granted through the child Access List, the "dumper" app should be
   visible to the user.
...
For example, if "Engineering Team" is added as a member of "Production Access",
all users who are members of "Engineering Team" inherit member grants (roles and
traits) from "Production Access".
...
For example, if "Engineering Team" is added as an owner of "Production Access",
all users who are members of "Engineering Team" inherit owner grants (roles and
traits) from "Production Access", and can perform owner actions, such as
modifying it or managing its members.

...
tool for privilege reduction. Relying on them for this is fragile, as changes to membership,
grants, membership requirements, or list structure could silently broaden a user's effective
permissions. Roles intended to reduce privileges should be assigned directly to users instead.

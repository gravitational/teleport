...
resource-based Access Requests that include resource(-s) granted by this Access
List. Learn more in the [Reviewing Access Requests](../../identity-governance/access-lists/reviewing-access-requests.mdx)
guide.
...
## Finding parent Access Lists

In some environments, Access Lists can be nested, where one list owns or references another. 
When multiple layers of Access Lists are involved, identifying all parent lists can become time-consuming.

If you have both `tctl` and `jq` installed, you can use the following command sequence to find which Access Lists are parents of a given list.

**Note:**
- The Access List title (spec.title) is a human-readable name.
- The Access List name (metadata.name) is a UUID used internally by Teleport.

```code
# Change the CHILD_TITLE variable
CHILD_TITLE="Human readable Access List title"

# Two lines below don't need to be changed

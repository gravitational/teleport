...
|`--log-format`|`text` (one of: `json`, `text`)|Controls the format of output logs. Can be `json` or `text`. Defaults to `text`.|
...
|`--[no-]trace`|`false`|Capture and export distributed traces. Overrides tracing.enabled in the configuration file.|
|`--trace-exporter`|*no default*|An OTLP exporter URL to send spans to. Overrides tracing.exporter_url in the configuration file.|

Global environment variables:

|Variable|Default|Description|
|---|---|---|
|`TBOT_CONFIG_PATH`|*no default*|Path to a configuration file.|
...
|Variable|Default|Description|
|---|---|---|
|`TELEPORT_AUTH_SERVER`|*no default*|Address of the Teleport Auth Server. Prefer using --proxy-server where possible.|
|`TELEPORT_BOT_TOKEN`|*no default*|A bot join token or path to file with token value, if attempting to onboard a new bot; used on first connect.|
|`TELEPORT_PROXY`|*no default*|Address of the Teleport Proxy Server.|

Flags:

|Flag|Default|Description|
|---|---|---|
|`-a`, `--auth-server`|*no default*|Address of the Teleport Auth Server. Prefer using --proxy-server where possible.|
|`--app`|*no default*|The name of the app in Teleport|
|`--ca-pin`|*no default*|CA pin to validate the Teleport Auth Server; used on first connect.|
|`--certificate-ttl`|*no default*|TTL of short-lived machine certificates.|
|`--destination`|*no default*|A destination URI, such as file:///foo/bar|
|`--diag-addr`|*no default*|If set and the bot is in debug mode, a diagnostics service will listen on specified address.|
|`--join-method`|*no default* (one of: `azure`, `azure_devops`, `bitbucket`, `circleci`, `gcp`, `github`, `gitlab`, `iam`, `kubernetes`, `spacelift`, `token`, `tpm`, `terraform_cloud`, `oracle`, `bound_keypair`, `env0`, `generic_oidc`)|Method to use to join the cluster.|
...
|`--reader-user`|*no default*|An additional user name or UID that should be allowed by ACLs to read this destination. Only valid for file destinations on Linux.|
|`--registration-secret`|*no default*|For bound keypair joining, specifies a registration secret for use at first join.|
|`--registration-secret-path`|*no default*|For bound keypair joining, specifies a file containing a registration secret for use at first join.|
...
|`--token`|*no default*|A bot join token or path to file with token value, if attempting to onboard a new bot; used on first connect.|
...
|`--ca-pin`|*no default*|CA pin to validate the Teleport Auth Server; used on first connect.|
|`--certificate-ttl`|*no default*|TTL of short-lived machine certificates.|
|`--database`|*no default*|The name of the database available in the requested database service.|
|`--destination`|*no default*|A destination URI, such as file:///foo/bar|
|`--diag-addr`|*no default*|If set and the bot is in debug mode, a diagnostics service will listen on specified address.|
|`--format`|`` (one of: ``, `tls`, `mongo`, `cockroach`)|The format of the credentials to generate.|
|`--join-method`|*no default* (one of: `azure`, `azure_devops`, `bitbucket`, `circleci`, `gcp`, `github`, `gitlab`, `iam`, `kubernetes`, `spacelift`, `token`, `tpm`, `terraform_cloud`, `oracle`, `bound_keypair`, `env0`, `generic_oidc`)|Method to use to join the cluster.|
|`--join-uri`|*no default*|An optional URI with joining and authentication parameters. Individual flags for proxy, join method, token, etc may be used instead.|
|`--[no-]oneshot`|`false`|If set, quit after the first renewal.|
|`--[no-]scoped`|`false`|Indicates whether tbot should run in scoped mode. This is required when authenticating as a scoped Bot.|
...
|`--ca-pin`|*no default*|CA pin to validate the Teleport Auth Server; used on first connect.|
|`--certificate-ttl`|*no default*|TTL of short-lived machine certificates.|
|`--cluster`|*no default*|The name of a specific cluster for which to issue an identity if using a leaf cluster.|
|`--destination`|*no default*|A destination URI, such as file:///foo/bar|
|`--diag-addr`|*no default*|If set and the bot is in debug mode, a diagnostics service will listen on specified address.|
|`--join-method`|*no default* (one of: `azure`, `azure_devops`, `bitbucket`, `circleci`, `gcp`, `github`, `gitlab`, `iam`, `kubernetes`, `spacelift`, `token`, `tpm`, `terraform_cloud`, `oracle`, `bound_keypair`, `env0`, `generic_oidc`)|Method to use to join the cluster.|
|`--join-uri`|*no default*|An optional URI with joining and authentication parameters. Individual flags for proxy, join method, token, etc may be used instead.|
...
|`--join-method`|*no default* (one of: `azure`, `azure_devops`, `bitbucket`, `circleci`, `gcp`, `github`, `gitlab`, `iam`, `kubernetes`, `spacelift`, `token`, `tpm`, `terraform_cloud`, `oracle`, `bound_keypair`, `env0`, `generic_oidc`)|Method to use to join the cluster.|
|`--join-uri`|*no default*|An optional URI with joining and authentication parameters. Individual flags for proxy, join method, token, etc may be used instead.|
|`--kubernetes-cluster`|*no default*|The name of the Kubernetes cluster in Teleport for which to fetch credentials.|
...
|`--join-method`|*no default* (one of: `azure`, `azure_devops`, `bitbucket`, `circleci`, `gcp`, `github`, `gitlab`, `iam`, `kubernetes`, `spacelift`, `token`, `tpm`, `terraform_cloud`, `oracle`, `bound_keypair`, `env0`, `generic_oidc`)|Method to use to join the cluster.|
|`--join-uri`|*no default*|An optional URI with joining and authentication parameters. Individual flags for proxy, join method, token, etc may be used instead.|
|`--label-selector`|*no default*|A set of Kubernetes labels to match in k1=v1,k2=v2 form. Repeatable.|
|`--name-selector`|*no default*|An explicit Kubernetes cluster name to include. Repeatable.|
|`--[no-]disable-exec-plugin`|`false`|If set, disables the exec plugin. This allows credentials to be used without the `tbot` binary.|
|`--[no-]oneshot`|`false`|If set, quit after the first renewal.|
|`--[no-]scoped`|`false`|Indicates whether tbot should run in scoped mode. This is required when authenticating as a scoped Bot.|
|`-o`, `--output`|*no default*|Path to write the generated configuration file to. If unspecified, the generated configuration is written to stdout.|
...
|`--ca-pin`|*no default*|CA pin to validate the Teleport Auth Server; used on first connect.|
|`--certificate-ttl`|*no default*|TTL of short-lived machine certificates.|
|`--data-dir`|*no default*|Directory to store internal bot data. Access to this directory should be limited.|
...
|`--proxy-server`|*no default*|Address of the Teleport Proxy Server.|
|`--proxy-templates-path`|*no default*|A path to a proxy template config file. Optional.|
|`--reader-group`|*no default*|An additional group name or GID that should be allowed by ACLs to read this destination. Only valid for file destinations on Linux.|
|`--reader-user`|*no default*|An additional user name or UID that should be allowed by ACLs to read this destination. Only valid for file destinations on Linux.|
|`--registration-secret`|*no default*|For bound keypair joining, specifies a registration secret for use at first join.|
|`--registration-secret-path`|*no default*|For bound keypair joining, specifies a file containing a registration secret for use at first join.|
...
|`--join-method`|*no default* (one of: `azure`, `azure_devops`, `bitbucket`, `circleci`, `gcp`, `github`, `gitlab`, `iam`, `kubernetes`, `spacelift`, `token`, `tpm`, `terraform_cloud`, `oracle`, `bound_keypair`, `env0`, `generic_oidc`)|Method to use to join the cluster.|
|`--join-uri`|*no default*|An optional URI with joining and authentication parameters. Individual flags for proxy, join method, token, etc may be used instead.|
|`--label-selector`|*no default*|A label-based selector for which workload identities to issue. Multiple labels can be provided using ','. Mutually exclusive with --name-selector.|
|`--listen`|*no default*|The address on which the workload identity API should listen. This should either be prefixed with 'unix://' or 'tcp://'.|
...
|`--reader-group`|*no default*|An additional group name or GID that should be allowed by ACLs to read this destination. Only valid for file destinations on Linux.|
|`--reader-user`|*no default*|An additional user name or UID that should be allowed by ACLs to read this destination. Only valid for file destinations on Linux.|
|`--region`|*no default*|The AWS region to use. If unset, value will be used from the AWS config or the AWS_REGION environment variable.|
|`--registration-secret`|*no default*|For bound keypair joining, specifies a registration secret for use at first join.|
|`--registration-secret-path`|*no default*|For bound keypair joining, specifies a file containing a registration secret for use at first join.|
|`--renewal-interval`|*no default*|Interval at which short-lived certificates are renewed; must be less than the certificate TTL.|
|`--role-arn`|*no default*|The ARN of the role to assume.|
|`--session-duration`|*no default*|The duration of the resulting AWS session and credentials. This may be up to 12 hours. When unset, this defaults to 6 hours.|
|`--session-renewal-interval`|*no default*|How often the session should be renewed. This should be less than the session duration. When unset, this defaults to 1 hour.|
|`--static-key-path`|*no default*|For bound keypair joining, specifies a path to a static key.|
|`--storage`|*no default*|A destination URI for tbot's internal storage, e.g. file:///foo/bar|
|`--token`|*no default*|A bot join token or path to file with token value, if attempting to onboard a new bot; used on first connect.|
|`--trust-anchor-arn`|*no default*|The ARN of the Roles Anywhere trust anchor to use.|
...
|---|---|---|
|`-a`, `--auth-server`|*no default*|Address of the Teleport Auth Server. Prefer using --proxy-server where possible.|
|`--audience`|*no default*|Specify the audiences to include in the JWT. At least one audience must be specified.|
|`--ca-pin`|*no default*|CA pin to validate the Teleport Auth Server; used on first connect.|
|`--certificate-ttl`|*no default*|TTL of short-lived machine certificates.|
|`--destination`|*no default*|A destination URI, such as file:///foo/bar|
|`--diag-addr`|*no default*|If set and the bot is in debug mode, a diagnostics service will listen on specified address.|
...
|---|---|---|
|destination-dir|*no default* (required)|The destination path to write the copy of the tbot binary.|
...
|`--proxy-server`|*no default*|The address of the Teleport proxy server to use, in host:port form.|

Arguments:

|Argument|Default|Description|
|---|---|---|
|args|*no default* (optional)|Arguments to `tsh db ...`; prefix with `-- ` to ensure flags are passed correctly.|
...
|---|---|---|
|`-a`, `--auth-server`|*no default*|Address of the Teleport Auth Server. Prefer using --proxy-server where possible.|
|`--bot-user`|*no default*|Enables POSIX ACLs and defines Linux user that can read/write short-lived certificates to "--destination-dir".|
|`--destination-dir`|*no default*|Directory to write short-lived machine certificates.|
|`--init-dir`|*no default*|If using a config file and multiple destinations are configured, controls which destination dir to configure.|
|`--[no-]clean`|`false`|If set, removes unexpected files and directories from the destination.|
|`--owner`|*no default*|Defines Linux "user:group" owner of "--destination-dir". Defaults to the Linux user running tbot if unspecified.|
|`--proxy-server`|*no default*|Address of the Teleport Proxy Server.|
|`--reader-user`|*no default*|Enables POSIX ACLs and defines Linux user that will read short-lived certificates from "--destination-dir".|
...
|`--pid-file`|*no default*|Overrides the PID file path that should be set in the systemd unit files.|
...
|`--proxy-server`|*no default*|The proxy server, which will be pinged to determine the current cryptographic suite in use.|
|`--static-key-path`|*no default*|If set, writes the static private key to a file.|
|`--storage`|*no default*|The internal storage URI to write the keypair to, such as `file:///var/lib/teleport/bot`.|
...
|Flag|Default|Description|
|---|---|---|
|`-o`, `--output`|*no default*|The path to write the generated configuration file to. If unset, it will be written to stdout.|
...
|Argument|Default|Description|
|---|---|---|
|args|*no default* (optional)|Arguments to `tsh proxy ...`; prefix with `-- ` to ensure flags are passed correctly.|
...
|Flag|Default|Description|
|---|---|---|
|`--path`|*no default*|The path to the SPIFFE Workload API endpoint to test.|
...
|Flag|Default|Description|
|---|---|---|
|`--diag-addr`|*no default*|The configured --diag-addr of a running bot, in host:port form.|
|`--service`|*no default*|An optional name. If set, waits for only the named service to become healthy. If unset, waits for all services.|
|`--timeout`|*no default*|An optional timeout. If set, returns an error if not all specified services have reported healthy by the timeout.|

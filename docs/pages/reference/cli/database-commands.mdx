...
| `--name` | Name of the proxied database. |
| `--protocol` | Proxied database protocol. Refer to the [configuration](../config-references/database-access.mdx) reference for supported values. |
| `--uri` | Address the proxied database is reachable at. |
...
<Admonition type="note" title="Note">
  ...
  [impersonation guide](../../admin-guides/access-controls/impersonation.mdx)
...
Displays only the databases a user has access to (see [RBAC](../../admin-guides/enroll-resources/database-access/rbac.mdx)).

...
# serviced by the auth server running on 192.0.2.1
sudo teleport start --roles=node --auth-server=192.0.2.1 --token=xyz --nodename=db

# Same as the above, but the node runs with db=master label and can be connected
# to using that label in addition to its name.
sudo teleport start --roles=node --auth-server=192.0.2.1 --labels=db=master

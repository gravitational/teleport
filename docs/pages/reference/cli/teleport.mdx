...
Adds required IAM permissions for Session Summaries feature using AWS Bedrock.
...
|`--aws-account-id`|none|The AWS account ID.|
|`--[no-]confirm`|`false`|Apply changes without confirmation prompt.|
|`--resource`|`*`|The AWS Bedrock resource to grant access to. Can be a full ARN or a model ID (e.g., 'anthropic.claude-v2' or '*' for all models).|
|`--role`|none|The AWS Role name used by the AWS OIDC Integration.|

...
|Argument|Default|Description|
|---|---|---|
|PROFILES|none (optional)|Comma-separated profile names to be exported. Supported profiles: allocs,cmdline,goroutine,heap,mutex,profile,trace,block,threadcreate. Default: goroutine,heap,profile|
...
|`--apply-on-startup`|none|Path to a non-empty YAML file containing resources to apply on startup. Works on initialized clusters, unlike --bootstrap. Only supports the following kinds: \[bot role user token cluster_networking_config cluster_auth_preference\].|
|`--auth-server`|none|Address of the auth server \[127.0.0.1:3025\]|

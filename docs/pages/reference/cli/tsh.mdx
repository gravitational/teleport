...
| `--auth` | `local` | Any defined [authentication connector](../operations/authentication.mdx), including `passwordless` and `local` (i.e., no authentication connector) | Specify the type of authentication connector to use. |
...
- `<cluster>` - the name of the cluster,  see [Trusted Cluster](../../admin-guides/access-controls/trustedclusters.mdx) for more information.
...
through Teleport.  See the [OpenSSH Guide](../../admin-guides/enroll-resources/server-access/openssh/openssh.mdx) guide
...
in their [storage backend](../operations/backends.mdx).
...
| `--query` | none | String | Query by [predicate language](../config-references/predicate-language.mdx) enclosed in single quotes |

...
- `id-or-file` The UUID of a past Teleport session, or the path to a local recording file. If the session is encrypted, it must be streamed from the auth service and local file playback is not possible.

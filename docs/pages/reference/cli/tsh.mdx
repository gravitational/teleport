...
|Variable|Default|Description|
|---|---|---|
|`TELEPORT_AWS_LOGIN_INTERACTIVE`|`false`|Prompt for AWS Roles interactively (--aws-role takes precedence).|
...
|Flag|Default|Description|
|---|---|---|
|`--dns-san`|*no default*|DNS SANs to include in the SVID. By default, none are included.|
|`--ip-san`|*no default*|IP SANs to include in the SVID. By default, none are included.|
|`--output`|*no default*|Path to the directory to write the SVID into.|
|`--svid-ttl`|`1h`|Sets the time to live for the SVID.|
|`--type`|`x509` (one of: `x509`)|Type of the SVID to issue (x509). Defaults to x509.|

Arguments:

|Argument|Default|Description|
|---|---|---|
|path|*no default* (required)|Path to use for the SVID SPIFFE ID. Must have a preceding '/'.|

...
For an MCP server that requires a pre-registered OAuth client, provide its
client ID, request a secure client-secret prompt, and use the callback port
registered with the provider:

```code
$ tsh mcp login --client-id=<client-id> --client-secret --callback-port=8080 <name>
```
...
|Flag|Default|Description|
|---|---|---|
|`--callback-port`|`0`|Local OAuth callback port. Set this to the exact port registered with the OAuth provider.|
...
|`--client-secret-file`|*no default*|Read the OAuth client secret from a file.|

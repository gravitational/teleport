...
|Flag|Default|Description|
|---|---|---|
|`--filter`|*no default*|Filter by comma-separated key=value pairs (keys: origin_type, kind, source). Pairs within one --filter are AND'd; repeat --filter to OR. Example: --filter kind=resource,source=AWS --filter origin_type=teleport_user.|
...
|Flag|Default|Description|
|---|---|---|
|`--limit`|`100`|Maximum number of detections to return (0 for unlimited).|

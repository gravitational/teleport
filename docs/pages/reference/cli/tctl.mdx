...
Reset local user account password and any associated multi-factor with expiring link to populate values. **Usage**: `tctl users reset <account>`
...
# Resets jeff's password and any associated multi-factor.  Jeff populates the password and confirms the token with the link.
```

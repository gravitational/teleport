...
Search session recordings with
[Session Recording Search](../../identity-security/session-search.mdx).
...
|`--kind`|none|Filter by session kind (ssh, db, k8s, desktop). Can be specified multiple times. Defaults to all kinds.|
...
|`--search-mode`|`hybrid`|Search strategy to use when search queries are provided. One of: `hybrid`, `keyword`, `embeddings`.|

...
|`--recovery-mode`|`standard`|If set, overrides the recovery mode for the bound keypair token. No effect if --token or --legacy is set.|
...
```code
$ tctl idp saml test-attribute-mapping [<flags>]
```
...
```code
$ tctl plugins install okta [<flags>]
```
...
```code
$ tctl plugins install scim [<flags>]
```
...
```code
$ tctl requests review [<flags>] <request-id>
```
...
```code
$ tctl sso configure github [<flags>]
```
...
```code
$ tctl tokens configure-kube [<flags>]
```
...
```code
$ tctl workload-identity revocations add [<flags>]
```
...
```code
$ tctl workload-identity revocations rm [<flags>]
```

...
```code
$ tctl integrations test --integration=INTEGRATION [<flags>]
```
...
|---|---|---|
|`--format`|`text` (one of: `text`, `json`, `yaml`)|Output format.|
|`--integration`|*no default*|Name of the integration to test. Use `tctl get integrations` to list integrations.|

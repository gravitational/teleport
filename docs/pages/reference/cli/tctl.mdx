...
|Flag|Default|Description|
|---|---|---|
|`--access-type`|*no default* (one of: `standing`, `request-based`)|How members are granted access: 'standing' (members get ongoing access to the resources described by the resource flags) or 'request-based' (members must request access; owners review). When omitted, a plain access list is created with no auto-generated roles.|

...
```code
$ tctl access-changes get [<flags>] <change-id>
```
...
Use --access-type to have Teleport create the access list's supporting roles
...
If --access-type is omitted, a plain access list is created with whatever grants
you set directly (--member-grant-roles, etc.) — there are no auto-generated
supporting roles.

Usage:

```code
$ tctl acl create [<flags>]
```
...
|`--description`|*no default*|Optional description.|
...
|`--iac`|*no default* (one of: `terraform`)|Dry-run: print the IaC config for the new access list without creating it. 'terraform' generates a static access list, which does not support audit reviews.|
...
|`--member-access-lists`|*no default*|Access list names to add as members; their members inherit this list's member grants (nested access lists).|
...
|`--members`|*no default*|Usernames or emails to add as members of the new access list. Members can also be added later with `tctl acl users add`.|
|`--node-labels`|*no default*|Selects SSH servers members may access by label match.|
|`--owner-access-lists`|*no default*|Access list names to add as owners; their members inherit owner permissions on this access list (nested access lists).|
...
|`--owners`|*no default*|Usernames or emails who own this access list and review membership.|
|`--title`|*no default*|Display name for the access list.|
...
|`--gcp-service-accounts`|*no default*|GCP service accounts members may assume via matched apps.|
|`--github-orgs`|*no default*|Selects git servers members may access by GitHub organization name.|
|`--iac`|*no default* (one of: `terraform`)|Dry-run: print the IaC config for the updated access list without applying the changes. 'terraform' generates a static access list, which does not support audit reviews.|
|`--kubernetes-groups`|*no default*|Kubernetes groups members may impersonate on matched clusters.|
|`--kubernetes-labels`|*no default*|Selects Kubernetes clusters members may access by label match.|
|`--kubernetes-users`|*no default*|Kubernetes users members may impersonate on matched clusters.|
|`--logins`|*no default*|OS logins members may use to connect to matched SSH servers.|
...
## tctl apps evaluate-request

Evaluate one HTTP request against the app_resources and
app_resources_expressions rules of v9 or newer roles and print the decision.

URL is the app URL, the request path included. The request is evaluated against
the cluster for the logged-in user.

Usage:

```code
$ tctl apps evaluate-request [<flags>] <url>
```
...
|---|---|---|
|`--format`|`yaml` (one of: `yaml`, `json`)|Output format.|
|`--method`|`GET`|Request method, in upper or lower case.|

Arguments:

|Argument|Default|Description|
|---|---|---|
|url|*no default* (required)|App URL, request path included.|

## tctl apps evaluate-resource

Evaluate one request for an app resource against the app_resources and
app_resources_expressions rules of v9 or newer roles and print the decision.

PATH is the path an app resource is reached at, such as /api/v4/projects.
The roles come from the --spec file and no cluster is contacted. Without a user
resource in the file, every role in it is evaluated. The file holds no app,
so app_labels are ignored and every role counts as granting the app.

Usage:

```code
$ tctl apps evaluate-resource --spec=SPEC [<flags>] <path>
```
...
|`--format`|`yaml` (one of: `yaml`, `json`)|Output format.|
|`--method`|`GET`|Request method, in upper or lower case.|
|`--spec`|*no default*|YAML file with one or more role resources and an optional user resource. The request is evaluated against the user's roles without contacting a cluster, or against every role in the file when there is no user resource.|

Arguments:

|Argument|Default|Description|
|---|---|---|
|path|*no default* (required)|Request path, such as /api/v4/projects.|
...
```code
$ tctl integrations test [<flags>] <integration>
```
...
|Argument|Default|Description|
|---|---|---|
|integration|*no default* (required)|Name of the integration to test. Use `tctl get integrations` to list integrations.|
...
|`--sort`|*no default* (one of: ``, `audit-queue`)|Sort output by column. Supported: audit-queue (largest audit queue backlog first, including dead-letter and corrupt events).|
...
Clean up the given plugin type.
...
|Argument|Default|Description|
|---|---|---|
|name|*no default* (optional)|The name of the plugin resource to delete|
...
## tctl plugins install slack

Install a Teleport Slack Access Request plugin.

Usage:

```code
$ tctl plugins install slack --bot-token-file=BOT-TOKEN-FILE --fallback-channel=FALLBACK-CHANNEL [<flags>]
```
...
|Flag|Default|Description|
|---|---|---|
|`--bot-token-file`|*no default*|Slack bot token used by the plugin. Accepts path to a file containing the token.|
|`--fallback-channel`|*no default*|Slack channel where unrouted alerts should be sent. For example #access-requests.|
|`--name`|`slack-default`|Name of the Slack plugin instance.|
...
Create an issuer override from the given certificate chains. Deprecated:
creation is rejected by Teleport v19 clusters; only --dry-run remains
functional.

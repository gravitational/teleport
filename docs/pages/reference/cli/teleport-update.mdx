...
`teleport-update` is a CLI tool that is used to update Teleport Agents & Bots installed on Linux servers.
...
Enables Managed Updates and performs an initial installation of the Teleport Agent & tbot.
...
**Example for a new Teleport Agent & tbot installation.**

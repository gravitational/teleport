...
`teleport-update` is a CLI tool that is used to update Teleport Agents and Bots installed on Linux servers.
...
Enables Managed Updates and performs an initial installation of the Teleport Agent and tbot.
...
- `/usr/local/bin/{teleport,tbot,tsh,...}` - Symbolic links into `/opt/teleport/default/versions/X.Y.Z/bin/`
- `/lib/systemd/system/teleport.service` - Teleport SystemD service
- `/etc/systemd/system/tbot.service` - Tbot SystemD service (not replaced if already present)
...
**Example for a new Teleport Agent installation.**
...
**Example for a new Teleport Agent and tbot installation.**

Install Teleport with Managed Updates enabled on a fresh system.

```code
# create /etc/teleport.yaml and /etc/tbot.yaml
$ sudo teleport-update enable
$ sudo systemctl enable teleport --now
...
Disable Managed Updates for the installed Teleport Agent and/or tbot.
...
Pin the installed Teleport Agent and/or tbot to a specific version of Teleport.
...
- Copying the Teleport systemd service file from `/opt/teleport/system/lib/systemd/system/teleport.service` into `/lib/systemd/system/teleport.service`.
...
- Removing the Teleport systemd service file from `/lib/systemd/system/teleport.service` (if symlinks are removed).

...
- [teleport-proxy](teleport-proxy.mdx): Deploy the
  `teleport` daemon on Kubernetes with preset configurations for Proxy Services
  only (no Auth). Generally prefer the teleport-cluster chart above instead.

...
| Teleport Jamf service        | `jamf`            | [`jamf`](#jamf) |
...
Kubernetes `Secret` containing the Jamf Pro API password.
...
containing the Jamf Pro API password used by the chart.
...
Jamf Pro API password stored in your Teleport chart values.

To create your own Jamf Pro API password secret, you can use a command like this:

```code
$ kubectl --namespace teleport create secret generic my-jamf-secret --from-literal=jamfPassword=<replace-with-actual-password>
```

<Admonition type="note">
  The key used for the Jamf Pro API password inside the secret must be `jamfPassword`, as in the command above.
</Admonition>

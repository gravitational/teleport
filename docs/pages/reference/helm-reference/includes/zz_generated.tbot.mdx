...
This is mandatory to connect to Teleport Cloud.
...
that the default output will be written to. Defaults to a secret named
`<fullname>-output`. The chart fullname is:
- `<release-name>-<chart-name>` if the release name doesn't contain the chart name already
- `<release-name>` if the release name contain the chart name

...
This includes deploying proxies, Auth Service instances, and [kubernetes-access](../../enroll-resources/kubernetes-access/introduction.mdx).
...
`authentication.secondFactor` controls the multi-factor used for local user authentication. Possible values supported by this chart
...
Changing the RP ID will invalidate all already registered webauthn multi-factors.
</Admonition>

### `authentication.webauthn`

See [multi-factor - WebAuthn](../../admin-guides/access-controls/guides/webauthn.mdx) for more details.
...
that monitors the workload (Auth Service and Proxy Service Services) deployed by the chart.

...
`auth` allows to set chart values only for the auth Kubernetes resources.
...
`proxy` allows to set chart values only for the proxy Kubernetes resources.
This is merged with chart-scoped values and takes precedence in case of conflict.

For example, to override the [`postStart`](#postStart) value only for proxy pods
and annotate the proxy Kubernetes Service:
```yaml
# By default all pods postStart command should be "echo starting"
postStart: ["echo", "starting"]
...
proxy:
  ...
  # We also annotate only the Kubernetes Service sending traffic to proxy pods.
  annotations:
    service:

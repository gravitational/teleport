...
<Admonition type="note" title="Azure IAM">
  ...
  After configuring a service principal with appropriate IAM permissions, you can use
  `extraEnv` to provide credentials as environment variables to your pods:
  ...
  - name: AZURE_CLIENT_SECRET
    value: "123456789abcdefghijklmnopqrstuvwxyzABCDE"
  ```
</Admonition>

| Type | Default value | Required? |
| - | - | - |
| `list` | `[]` | When the `db` chart role is used at least one of `databases`, `awsDatabases`, `azureDatabases`, `dbResources` is required. |
...
<Tabs>
  ...
  <Admonition type="note" title="YAML formatting">
    YAML is very sensitive to correct spacing. When specifying lists in a `values.yaml` file, you might find it helpful
    to [use a linter](https://codebeautify.org/yaml-validator) to validate your YAML list and ensure that it is correctly formatted.
  </Admonition>
  </TabItem>
  <TabItem label="--set">
  ```sh
  --set "azureDatabases[0].types={mysql,postgres}" \
  --set "azureDatabases[0].tags.*=*" \
  --set "azureDatabases[1].types[0]=mysql" \
  --set "azureDatabases[1].tags.env={dev,staging}" \
  --set "azureDatabases[1].tags.origin=alice" \
  --set "azureDatabases[1].regions={eastus,centralus}" \
  --set "azureDatabases[1].subscriptions={subID1,subID2}" \
  --set "azureDatabases[1].resource_groups={group1,group2}"
  ```

...
| [`discovery_service`](../../kubernetes-access/discovery/)       | `discovery`                                   | Uses Teleport to discover new resources<br/> and dynamically add them to the cluster             |
...
| Type   | Default value | Required?                                                                                                                             |
|--------|---------------|---------------------------------------------------------------------------------------------------------------------------------------|
| `list` | Discovery of all applications in all namespaces   | When the `discovery` chart role is used, `kubernetesDiscovery` is required. |
...
`kubernetesDiscovery` is a YAML list object detailing the filters for the services of the Kubernetes cluster that should be discovered
 and exposed as apps in Teleport. By default the Discovery Service will try to find all apps that are serving HTTP/HTTPS, but you can manually
 make it expose TCP apps as well, using Kubernetes annotations.

You can specify multiple filters, each of which can include these parameters:
- `types` is a list of types of services to be exposed in Teleport. Currently only `app` is supported.
- `namespaces` is a list of namespaces of the Kubernetes cluster to search in. The default value is `*`, which denotes all namespaces.
- `labels` is a list of Kubernetes labels based on which to filter out services. The default value is `*`: `*` that will match any labels.

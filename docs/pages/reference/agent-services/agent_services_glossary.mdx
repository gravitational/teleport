...
This glossary provides definitions and explanations for key terms related to Teleport Agent services and clients. Teleport Agent runs on 
target systems to facilitate secure access through the Teleport cluster. Most agent services can be enabled together or separately depending on deployment needs. 

...
You can also use [dynamic registration](../enroll-resources/desktop-access/dynamic-registration.mdx) using `dynamic_windows_desktop` resources.
This can be useful for managing inventories of hosts that are not joined to
an Active Directory domain.

You can also manage `windows_desktop` resources manually with `tctl`.

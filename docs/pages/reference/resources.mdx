...
which is currently supported but will eventually be deprecated.
The `autoupdate_config` and `autoupdate_version` resources below configure the
next-generation (v2) Teleport updater.
</Notice>
...
next-generation (v2) Teleport updater and original (v1) Teleport Agent updater.
...
<Notice type="warning">
The `autoupdate_config` and `autoupdate_version` resources configure the
next-generation (v2) Teleport updater.
`cluster_maintenance_config` above configures the original (v1) Teleport Agent updater,
which is currently supported but will eventually be deprecated.
</Notice>

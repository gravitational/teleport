...
- The [`tctl`](./cli/tctl.mdx) admin tool is used for configuring the Teleport
...
[`tctl`](./cli/tctl.mdx) admin tool to manage dynamic records like Teleport
users.
...
[`tctl`](./cli/tctl.mdx) has convenient sub-commands for dynamic
configuration, such as `tctl users` or `tctl nodes`.

For more advanced management tasks, like connecting clusters together or
troubleshooting trust, [`tctl`](./cli/tctl.mdx) offers the more powerful,
...
| - | - | - |
| [`tctl get`](./cli/tctl.mdx#tctl-get) | Get one or multiple resources. | `tctl get users` or `tctl get user/joe` |
| [`tctl rm`](./cli/tctl.mdx#tctl-rm) | Delete a resource by type/name. | `tctl rm user/joe` |
| [`tctl create`](./cli/tctl.mdx#tctl-create) | Create a new resource from a YAML file. Use the `-f` flag to update existing resources. | `tctl create -f joe.yaml` |

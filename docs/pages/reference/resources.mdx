...
spec:
  ...
  # (`non_ad: true`), this value MUST be the `name` of a `windows_desktop_service`
  # that can access the RDP port of the host (`spec.addr` above).
  #
  # `windows_desktop` resources are identified by the combination of `metadata.name` and `spec.host_id`.
  # To update a specific `windows_desktop` resource, ensure you use the same values for both fields
  # as the resource you intend to update.
  #
...
See [Database Access Controls](../database-access/auto-user-provisioning/database-access-controls.mdx) for more details.

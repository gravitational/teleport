...
The lock lasts 20 minutes. After the lock has expired the user may attempt to log in again.

Overriding a lock is available to users with rights to maintain `user` resources,
available in the built-in `editor` role. To turn off a lock, update the user entry
...
The file `user.yaml` should resembling the following:

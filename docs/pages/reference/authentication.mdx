...
A lock is placed on a local user if, within a 30 minute window, a local user has:

- 5 failed login attempts or
- 3 failed password resets

The lock lasts 20 minutes. After the lock has expired the user may attempt to login again.

Overriding a lock is available to users with rights to maintain `user` resources,
available in the builtin `editor` role. To turn off a lock, update the user entry
to disable the lock, following these steps.

Retrieve the user entry so you can edit the status:

```code
$ tctl get users/jeff > jeff.yaml`
```
...
Update the `is_locked` under `status` to `false` and save the file. Now
update the user entry with the below command.

```code
$ tctl create -f jeff.yaml
```

The user will now be unlocked and can attempt to authenticate again.

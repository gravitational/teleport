...
[Predicate Language](../../access-controls/predicate-language.mdx) for templating, allowing you to
...
Each rule consists of either a [Predicate Language](../../access-controls/predicate-language.mdx)
...
Some attributes, such as `workload.kubernetes.labels`, are maps of key-value
pairs. The conditions form cannot reference a value within a map attribute. To
match against one, use the expression form with the index syntax:
...
spec:
  ...
  rules:
    allow:
    - expression: workload.kubernetes.labels["app"] == "foo"
```

If the specified key is not present in the map, the rule evaluation fails and
the credential is not issued.
...
When using the [Predicate Language](../../access-controls/predicate-language.mdx), the attribute
...
- [Terraform provider reference: teleport_workload_identity](../../infrastructure-as-code/terraform-provider/resources/workload_identity.mdx)

...
`tbot` can export OpenTelemetry traces to a collector. This is disabled by default.

Tracing is configured with the `tracing` section of the configuration file. The
options mirror those of the Teleport `tracing_service`:
...
- `exporter_url` accepts the same formats as the Teleport `tracing_service`:
  `grpc://`, `http://`, `https://` and `file://`. See
  [Distributed Tracing](../../zero-trust-access/management/diagnostics/tracing.mdx)
  for details of each format.
- `sampling_rate_per_million` controls how many spans out of every million are
  exported. It defaults to `1000000`, which exports all spans. When `tbot` runs
  as a daemon, the sampling decision is made once at startup, so a lower rate
  results in either all or none of the spans from that process being exported.
- `ca_certs` and `keypairs` are optional and only valid with `grpc://` and
  `https://` exporters. A `grpc://` exporter is connected to with TLS only when
  one of them is specified, so to reach a TLS-enabled gRPC collector, list the CA
  that signed its certificate in `ca_certs`. An `https://` exporter always uses
  TLS, verifying against the system trust store unless `ca_certs` is specified.

The `--trace` and `--trace-exporter` flags override `enabled` and `exporter_url`
respectively, which is useful for capturing a trace of a single run:

```code
$ tbot start -c tbot.yaml --trace --trace-exporter=file:///var/lib/tbot/traces
```

---
title: Deploying tbot with Bound Keypair Static Keys
description: "How to install and configure tbot with Bound Keypair static keys"
tags:
 - mwi
 - how-to
...
<Admonition type="tip" title="Bound Keypair for Agents">
This guide applies to joining bots with the `tbot` client. For information on
joining standard Teleport agents with the `bound_keypair` join method, refer
[to the admin guide](./admin-guide.mdx#using-bound-keypair-for-standard-teleport-agents).
</Admonition>
...
First, `tbot` needs to be installed on the host that you wish to use Machine &
...
- Follow the [access guides](../../../machine-workload-identity/access-guides/access-guides.mdx)

...
Note that non-bot agents manage their certificates differently from bots: they
...
service role) or cluster CAs are rotated. Bound Keypair rotation can only happen
during the bound keypair challenge process, which for agents only happens during
these rare, manual events.
</Admonition>
...
lock it. It will not be able to reauthenticate, recover, interact with the
...
Find the desired host in the list and copy its ID from the "Server ID" column.
Then, create a lock for that ID:
```code
...
Note the different locks and lock targets shown above. Bots can be targeted
...
arbitrary - and potentially remote - keystore. However, it may still be possible
to automate rotation if your environment or secret store allows you to update
...
types as well as Machine & Workload Identity bots.
...
- Agents are ultimately still issued long-lived certificates and do not
  generally renew these identities outside of limited circumstances, namely
  during [cluster CA rotation](../../../zero-trust-access/management/security/ca-rotation.mdx)
  or when new system roles are added to an existing agent.
- As most Agents only join a single time, Bound Keypair's
  [Join State Verification](./concepts.mdx#join-state-verification) is
  effectively unused.
- As [keypair rotation](#requesting-a-keypair-rotation) can only take place at
  join time, agents will only honor a keypair rotation request under certain
  conditions, like a role change.
- Agent identities don't have access privileges for other Teleport resources, so
  they don't benefit from or take advantage of the additional protections
  provided by Bound Keypair joining.
...
- You do not want any shared secrets in your joining process. Bound Keypair's
  [static keys](../../../installation/agents/bound-keypair-static-keys.mdx)
...
If you'd like to join an agent using Bound Keypair joining, refer to these
guides:
- [Joining an agent using Bound Keypair](../../../installation/agents/bound-keypair.mdx)
- [Joining an agent using Bound Keypair Static Keys](../../../installation/agents/bound-keypair-static-keys.mdx)

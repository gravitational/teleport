...
- You do not want any shared secrets in your joining process. Bound Keypair's
  [static keys](#configuring-an-agent-with-a-static-key) allow you to generate a
  keypair on the target node itself. Only the public key would ever leave the
  system, and no secrets or private key material would ever have to leave the
  host.

...
Next, we'll use a helper in the `tbot` client to generate a static keypair. This
...
2. Via a standard file

If you intend to make the key available to `tbot` via an environment variable,
run the following:
...
Adjust the `--static-key-path` value as desired.
...
environment, as well as whether you'll be using an environment variable or file.

### Storing the key in a file on the bot host

Copy the generated private key from your local machine to the bot host via any
supported means. This might be as simple as copying via `scp`, copying and
pasting the file contents, or any other process.

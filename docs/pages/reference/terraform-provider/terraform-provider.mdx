...
`tbot` relies on [MachineID](../../mwi/machine-id/introduction.mdx) to obtain and automatically renew
...
- This auth method does not work against Teleport clusters with MFA set to `webauthn`.
  ...
  [an additional MFA challenge for administrative actions](../../zero-trust-access/access-controls/guides/mfa-for-admin-actions.mdx).

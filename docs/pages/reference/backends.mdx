...
reconnections). `wal_level` can only be set at server start, so it should be set
in `postgresql.conf`:
```
...
well as `login privilege`, of course). In the `psql` shell:
...
replication` role attribute, if given the ability to do so (with a `superuser
status` user, or similar privileged access as can be granted in managed
PostgreSQL services such as Azure Databases for PostgreSQL or Amazon RDS for
PostgreSQL).

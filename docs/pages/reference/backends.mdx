...
to achieve high available and survive regional failures. You must take steps to 
...
<Admonition
  ...
  title="IMPORTANT"
>
  `CockroachDB` can currently only be used to store Teleport's internal database in a
  highly-available way. This will allow you to have multiple auth servers in your
  cluster for a high availability deployment, but it will not also store Teleport audit events
  for you in the same way that [DynamoDB](#dynamodb) or
  [Firestore](#firestore) will.
</Admonition>
...
[multiregion survival goals](https://www.cockroachlabs.com/docs/stable/multiregion-survival-goals).
...
- Configure all Teleport Auth Service instances to use the CockroachDB backend in the
  `storage` section of `teleport.yaml` as shown below.
- Deploy several auth servers connected to the CockroachDB storage backend.
...
teleport:
  storage:
    ...
    # conn_string is a required pamater. It is a PostgreSQL connection string used
    ...
    # establishing a change feed conneciton.
    #
    # If your certificates are not stored at the default ~/.postgresql
    # location, you will need to specify them with the sslcert, sslkey, and
    # sslrootcert parameters.
    ...
    # 20 minutes. This is used by Teleport to cleanup old resources that are no longer
    # connected to or needed by Teleport. Note that configuring this to run more
    # frequently may have preformance implications for CockroachDB.
    ttl_job_cron: '*/20 * * * *'

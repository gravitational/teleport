...
- Configure all Teleport Auth Service instances to use DynamoDB backend in the "storage"
  section of `teleport.yaml` as shown below.
- Auth Service instances must be able to reach DynamoDB and DynamoDB Streams endpoints.
- Deploy up to two Auth Service instances connected to DynamoDB storage backend.
...
This config option applies to [AWS S3](#s3) and [Amazon DynamoDB](#dynamodb).
...
- Configure all Teleport Auth Service instances to use Firestore backend in the "storage"
  section of `teleport.yaml` as shown below.
- Deploy several Auth Service instances connected to Firestore storage backend.
- Deploy several proxy nodes.
...
monitor the number of warning messages in the Auth Service instances logs that say

...
S3 buckets must have versioning enabled, which ensures that a bad actor cannot
cover their tracks by replacing the session logs, as Teleport will always look
at the oldest version of a recording.

Below is an example of how to configure a Teleport auth server to store the

...
decoding plugin. Teleport needs separate databases for the cluster state and the
audit log, and it will attempt to create them if given permissions to do so; it
will also set up the database schemas as needed, so
...
must be set to at least as many Teleport auth servers as you'll be running (a
higher number is recommended, to account for network timeouts and
reconnections). In addition, the database user must have the `initiating
replication` [role
attribute](https://www.postgresql.org/docs/current/role-attributes.html) (as
well as `login privilege`, of course). Since replication permissions allow for
essentially full read access over the entire cluster (with a physical
replication connection) or to all databases that the user can connect to, it's
recommended to prevent the user from opening replication connections, or from
connecting to databases other than the ones used for Teleport, if the PostgreSQL
cluster is shared between Teleport and other applications.

To configure Teleport to use PostgreSQL:
- Configure all Teleport Auth servers to use the PostgreSQL back-end in the
  `storage` section of `teleport.yaml` as shown below.
- Deploy several auth servers connected to the PostgreSQL storage back-end.
- Deploy several proxy nodes.
- Make sure that the proxy instances and all Teleport resource services that
...
3766 hours (one year); it's possible to select a different retention period or
...
storing them in the configuration file.
...
to connect to an [Azure Database for
PostgreSQL](https://azure.microsoft.com/en-us/products/postgresql) server
without having to manage any secrets:

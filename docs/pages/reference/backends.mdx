...
- Each Teleport Auth Service instance will have individual audit logs within the `log` directory under the data
  directory (by default `/var/lib/teleport/log`). Using a SIEM to consolidate audit logs is recommended.
...
S3 and S3-compatible buckets can only be used as storage for the recorded sessions. S3 cannot

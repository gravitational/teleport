...
- Each Teleport Auth Service instance will have individual audit logs within the `log` directory under the data
  directory (by default `/var/lib/teleport/log`). Using a [SIEM to consolidate audit logs](../management/export-audit-events.mdx) is recommended.
  Using an S3-compatible storage solution will allow for a central platform for storing sessions. Otherwise each
  Teleport Auth Service instance will have sessions stored in the `records` directory under the same `log` directory 
  (by default `/var/lib/teleport/log/records`). See the S3 section.
- Deploy several Auth Service instances connected to an etcd backend.
...
S3 and S3-compatible buckets can be used as storage for the recorded sessions. S3 cannot
store the the cluster state.

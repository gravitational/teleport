...
the `TeleportOktaImportRule` resource, which you can apply after installing
...
|Field|Type|Description|
|---|---|---|
|mappings|[][object](#mappings-values)|Mappings is a list of matches that will map match conditions to labels.|
...
|Field|Type|Description|
|---|---|---|
|add_labels|[object](#`mappings` valuesadd_labels)|AddLabels specifies which labels to add if any of the previous matches match.|
|match|[][object](#match-values)|Match is a set of matching rules for this mapping. If any of these match, then the mapping will be applied.|

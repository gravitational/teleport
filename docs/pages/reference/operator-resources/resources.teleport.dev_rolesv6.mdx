...
|Field|Type|Description|
|---|---|---|
|allow|[object](#allow)|Allow is the set of conditions evaluated to grant access.|
|deny|[object](#deny)|Deny is the set of conditions evaluated to deny access. Deny takes priority over allow.|
|options|[object](#options)|Options is for OpenSSH options like agent forwarding.|
...
|impersonate|[object](#impersonate)|Impersonate specifies what users and roles this role is allowed to impersonate by issuing certificates or other possible means.|
...
|review_requests|[object](#review_requests)|ReviewRequests defines conditions for submitting access reviews.|
...
|create_db_user_mode||CreateDatabaseUserMode allows users to be automatically created on a database when not set to off. 0 is "unspecified", 1 is "off", 2 is "keep", 3 is "best_effort_drop".|
...
|create_host_user_mode||CreateHostUserMode allows users to be automatically created on a host when not set to off. 0 is "unspecified"; 1 is "off"; 2 is "drop" (removed for v15 and above), 3 is "keep"; 4 is "insecure-drop".|
...
|idp|[object](#idp)|IDP is a set of options related to accessing IdPs within Teleport. Requires Teleport Enterprise.|
...
|record_session|[object](#record_session)|RecordDesktopSession indicates whether desktop access sessions should be recorded. It defaults to true unless explicitly set to false.|
...
|require_session_mfa||RequireMFAType is the type of MFA requirement enforced for this user. 0 is "OFF", 1 is "SESSION", 2 is "SESSION_AND_HARDWARE_KEY", 3 is "HARDWARE_KEY_TOUCH", 4 is "HARDWARE_KEY_PIN", 5 is "HARDWARE_KEY_TOUCH_AND_PIN".|
...
|Field|Type|Description|
|---|---|---|
|saml|[object](#saml)|SAML are options related to the Teleport SAML IdP.|
...
|Field|Type|Description|
|---|---|---|
|mode||Mode is the type of extension to be used -- currently critical-option is not supported. 0 is "extension".|
|name|string|Name specifies the key to be used in the cert extension.|
|type||Type represents the certificate type being extended, only ssh is supported at this time. 0 is "ssh".|

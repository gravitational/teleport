...
the `TeleportSAMLConnector` resource, which you can apply after installing
...
|assertion_key_pair|[object](#`spec`assertion_key_pair)|EncryptionKeyPair is a key pair used for decrypting SAML assertions.|
|attributes_to_roles|[][object](#attributes_to_roles-values)|AttributesToRoles is a list of mappings of attribute statements to roles.|
...
|client_redirect_settings|[object](#`spec`client_redirect_settings)|ClientRedirectSettings defines which client redirect URLs are allowed for non-browser SSO logins other than the standard localhost ones.|
...
|signing_key_pair|[object](#`spec`signing_key_pair)|SigningKeyPair is an x509 key pair used to sign AuthnRequest.|

...
|Field|Type|Description|
|---|---|---|
|force_disable|boolean|ForceDisable specifies groups provider should be disabled, even if Entra ID responds with a groups claim source. User may choose to disable it if they are using integrations such as SCIM or similar groups importer as connector based role mapping may be not needed in such a scenario.|
|graph_endpoint|string|Microsoft Graph API endpoint. Defaults to "https://graph.microsoft.com".|

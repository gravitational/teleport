...
|entra_id_groups_provider|[object](#specentra_id_groups_provider)|EntraIDGroupsProvider configures out-of-band user groups provider.|
...
|Field|Type|Description|
|---|---|---|
|enabled|boolean|Enabled specifies groups provider enabled/disabled state. Defaults to false. If enabled, groups provider should honour groups claim source. User may choose to disable it if they are using integrations such as SCIM or similar groups importer as connector based role mapping may be not needed in such a scenario.|
|graph_endpoint|string|Optional Microsoft Graph API endpoint. Defaults to "graph.microsoft.com".|
|group_type|string|GroupType configures group filter behavior. Value can be "security-groups", "directory-roles", "all-groups".|

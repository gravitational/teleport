...
the `TeleportOpenSSHEICEServerV2` resource, which you can apply after installing
...
|cloud_metadata|[object](#`spec`cloud_metadata)|CloudMetadata contains info about the cloud instance the server is running on, if any.|
...
|rotation|[object](#`spec`rotation)|Rotation specifies server rotation|
...
|Field|Type|Description|
|---|---|---|
|aws|[object](#`cloud_metadata`aws)|AWSInfo contains attributes to match to an EC2 instance.|
...
|schedule|[object](#`rotation`schedule)|Schedule is a rotation schedule - used in automatic mode to switch between phases.|

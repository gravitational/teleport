...
Bound Keypair tokens are an improved alternative to
[ephemeral tokens](#ephemeral-tokens) that aims to improve reliability for
on-prem deployments

(!docs/pages/includes/provision-token/bound-keypair-spec.mdx!)

<Admonition type="note" title="See Also">
- [Deploying Machine ID on Bare-metal Nodes with Bound Keypair joining](../machine-workload-identity/machine-id/deployment/bound-keypair.mdx)

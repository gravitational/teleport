...
typically used with [Machine ID](../machine-id/introduction.mdx) to access
...
This join method is typically used with [Machine ID](../machine-id/introduction.mdx)
...
TPM (Trusted Platform Module) joining leverages the TPM hardware security module
on a host to prove its identity to the Auth Service. This method is only
available on Linux hosts with a TPM 2.0 device installed.

TPM joining requires that the Auth Service is running with a valid Enterprise
license.

This join method is useful in environments where no other form of identity is
available to the host, such as in a bare-metal environment.

(!docs/pages/includes/provision-token/tpm-spec.mdx!)

<Admonition type="note" title="See Also">
- [Deploying Machine ID on Linux: TPM](../machine-id/deployment/linux.mdx)
</Admonition>
...
auth_service:
    ...
    # Determines if Server and Database sessions to cluster nodes are forcefully
    # terminated after no activity from a client (idle client).
    ...
    # (Currently only supported for Server and Database connections)
    ...
    # certificates expire in the middle of an active Server or Database Access

...
- **Cluster Events:** Teleport logs events like successful user logins along
   with metadata like remote IP address, time, and the session ID.
- **Recorded Sessions:** Every SSH, desktop, or Kubernetes shell session is recorded and
   can be replayed later. Teleport Enterprise manages the storage of session
   recording data.
...
Teleport Enterprise manages the storage of audit logs for you. You can
...
Teleport Enterprise automatically stores recorded sessions.

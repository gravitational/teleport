...
For an existing cluster it's possible you could switch to the `hsm-v1` suite and
users and hosts could begin to use new key algorithms while your CAs continue to
use their existing RSA keys, but whenever a new CA key is generated it may fail
if the CA uses non-RSA keys.
New CA keys may be generated if a CA rotation is initiated, or during a version
upgrade when a new CA has been added in the new version.

### My YubiHSM2 authentication key does not have capabilites for ECDSA keys, what do I do?

Our [YubiHSM2 guide](../../admin-guides/deploy-a-cluster/hsm.mdx) recommends
creating an "authentication key" to be used by the Teleport Auth server to
authenticate with the YubiHSM2.
The example in the original version of that guide created an authentication key
without the necessary "capabilities" to create and sign with ECDSA keys.
...
your Auth server configuration to use the new key.
...
# Open a session with an admin authentication key that has capabilites to create
...
authentication key, then restart your Auth server.

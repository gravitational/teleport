...
<TabItem label="Okta Integration Network" value="oin">
  ...
  4. In *Teleport cluster domain* filed type in your proxy domain, e.g. `example.teleport.sh` and click **Done**.
...
By default Okta SSO integration supports SP-initiated flow. To enable IdP-initiated flow you will have to edit the Okta SAML connector with `tctl edit saml/<Var name="okta" />` and set `spec.allow_idp_initiated` to `true`.

## Attribute mapping

All attributes sent with the SAML assertion will be stored as user's traits which then can be used to build RBAC rules. It is also possible to map SAML attributes to Teleport roles. They can be set in the web UI by editing the Okta SAML connector yaml or using the CLI `tctl edit saml/<Var name="okta" />`.

In the example below there are 2 mappings:

- *groups* attribute containing value *Everyone* is mapped to Teleport *okta-requester* role.
- *groups* attribute containing value *okta-admin* Okta group is mapped to Teleport *editor* role.

At least 1 attribute to role mapping is required however we encourage to use [Access Lists](../../access-lists/access-lists.mdx) for access management instead.

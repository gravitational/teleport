...
To login using Okta connector, open the web UI and click the **Okta** button at the bottom of the page to authenticate.

To login using CLI:

```
tsh login --proxy=<Var name="example.teleport.sh" /> --auth=<Var name="okta" /> --user=<Var name="okta_login" />
```

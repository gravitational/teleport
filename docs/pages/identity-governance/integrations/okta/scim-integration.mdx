...
<summary>Editing Okta SCIM configuration</summary>
   ...
   Note that saving the new SCIM configuration will invalidate the existing SCIM token.

</details>

Save the current settings by clicking **Save SCIM Configuration** button to make the generated **API Token** active. It will be needed in the next step.

   ![Generate SCIM token](../../../../img/okta/scim-gen-token.png)

## Step 2/2. Configure SCIM details in the Okta app

<Tabs groupId="scim-app" defaultValue="oin">
<TabItem label="Okta Integration Network" value="oin">
   ...
   3. Click **Configure API Integration**, check the **Enable API integration** box and:
      1. Paste the API Token generated in Step 1 of this guide.
...
If you have users assigned to the Okta Teleport app before enabling SCIM, you may notice there are red exclamation marks next to each user in the *Assignments* tab. To learn more about that read the **Retrospective user provisioning** section below.

## Supported features

Teleport supports only *To App* (Okta -> Teleport) provisioning and does not support importing from Teleport to Okta.

Supported *To App* provisioning features include:
...
Please note that SCIM attributes are stored in an internal user label,
`teleport.internal/scim-attrs`, which is subject to change. Teleport uses the
name and email attributes to derive [display
names](../../../reference/access-controls/user-display-names.mdx). The remaining
attributes are only used to construct compliant SCIM resource responses.
All attributes sent to Teleport are stored and then replied in subsequent SCIM
resource responses. The only exception is the "password" attribute.

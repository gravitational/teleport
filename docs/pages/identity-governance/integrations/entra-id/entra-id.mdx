...
<Admonition type="tip" title="Review access to Entra ID resources">
Once Teleport imports your Entra ID groups and enterprise applications, you can
use Teleport Identity Security to review which identities can access them and
whether that access is used. See [Review Identity Access from the
CLI](../../../identity-security/cli/access-review.mdx) (`tctl access-review`).
For example, to see who can reach the `Payroll` enterprise application:

```code
$ tctl access-review --query "SELECT * FROM access_path WHERE resource = 'Payroll'"
```
</Admonition>
...
This method is best suited for an air-gapped Teleport cluster where the Teleport Proxy Service

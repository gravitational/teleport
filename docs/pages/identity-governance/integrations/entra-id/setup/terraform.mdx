...
The example Terraform module provides Entra ID configuration options that are based on the 
...
the Entra ID tenant ID values as an output. You will need these values in the next step.
...
if you wish to set up system credential based authentication.

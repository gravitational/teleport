...
The `region` field is used to configure the region used by the
Microsoft Bot Framework. This should match the region set when creating your Azure
app, and will default to 'emea' when left unspecified.
...
The `teams_app_id` is a UUID you must specify that will be used as an identifier

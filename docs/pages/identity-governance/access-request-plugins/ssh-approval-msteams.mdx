...
Note: The `region` field is used to configure to configure the region used by the
Microsoft Bot Framework, this should match the region set when creating your Azure
app. This will default to 'emea' when left unspecified.
...
The `teams_app_id` is a unique UUID you can specify that will be used as an identifier
for the app's manifest within Teams.

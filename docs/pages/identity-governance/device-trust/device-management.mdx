...
Enable auto-enrollment in your cluster settings. To do so, modify the dynamic config resource.
...
Because this is an enterprise feature, it is not included in the set of supported settings in our default Helm chart.
In order to enable it, you will need to add the following snippet to your Helm values file under `auth.teleportconfig.auth_service`:
```diff
auth:
  teleportConfig:
    # ...

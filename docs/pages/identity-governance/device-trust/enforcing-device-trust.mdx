...
- `required-human` - enables device authentication and device-aware audit.
  Additionally, it requires a trusted device for all SSH, Database and
...
the `device_trust_mode` field assigned to `"required"` or `"required-human"`.
...
spec:
  options:
    device_trust_mode: "required-human"
  allow:
    db_labels:
      '*': '*'
    db_names:

...
<Admonition type="warning" title="Access Monitoring is being replaced by Identity Security">
Access Monitoring is deprecated beginning in Teleport 19 and is scheduled for
removal in Teleport 20, when Teleport Identity Security fully replaces it. The
Access Monitoring UI is removed from Identity Governance starting in Teleport
19.4. For the full plan, timeline, and rationale, see the [Access Monitoring
deprecation discussion](https://github.com/gravitational/teleport/discussions/68403).
</Admonition>

Identity Security already covers many Access Monitoring use cases — through its
Web UI and the [Identity Security CLI](../identity-security/cli/cli.mdx) — and we
are actively expanding coverage. The table below maps each Access Monitoring
capability to its Identity Security replacement and current status:

| Access Monitoring capability | Identity Security replacement | Status |
| --- | --- | --- |
| Pre-built risk reports (root SSH, MFA-less sessions, privileged accounts) | [Alerts](../identity-security/usage/alerts.mdx) / [`tctl detections`](../identity-security/cli/detections.mdx) | Available |
| Investigating who did what, from where, and when | [Investigate](../identity-security/usage/investigate.mdx) / [`tctl investigate`](../identity-security/cli/investigate.mdx) | Available |
| Reviewing which identities can access which resources | [`tctl access-review`](../identity-security/cli/access-review.mdx) | Available |
| Monitoring access-path changes to sensitive resources | [Crown Jewels](../identity-security/usage/crown-jewels.mdx) / [`tctl access-changes`](../identity-security/cli/access-changes.mdx) | Available |
| Reporting, charts, and aggregations over audit activity (Query Editor, `tctl audit query exec`) | Work in progress. In the meantime, [`tctl investigate`](../identity-security/cli/investigate.mdx) retrieves the underlying events, which you can export to build your own reports. | In progress |
...
<details>
<summary>Check this with `tctl investigate`</summary>

```code
$ tctl investigate --event-type=exec --from=30d --limit=0 --format=json \
  | jq '[.data[] | select((.event_data.command // "") | test("/etc/passwd"))]'
```
...
```code
$ tctl investigate --event-type=cert.create --from=30d --limit=0 --format=json \
  | jq '[.data[] | {event_date: .time[0:10], user: .event_data.identity.user, ip: .event_data.identity.client_ip}]
        | group_by([.event_date, .user])
        | map({event_date: .[0].event_date, user: .[0].user, unique_ip_count: ([.[].ip] | unique | length)})'
```
...
```code
$ tctl investigate --event-type=cert.create --user='admin-annie' --from=30d --limit=0 --format=json \
  | jq '[.data[].event_data.identity.client_ip] | unique'
```
...
```code
$ tctl investigate --event-type=access_request.create --event-type=access_request.review --from=30d --limit=0 --format=json
```
...
```code
$ tctl investigate --event-type=access_request.create --event-type=access_request.review --from=30d --limit=0 --format=json \
  | jq '[.data[] | {event: .event_type, id: .event_data.id, user: .event_data.user, reason: .event_data.reason, roles: .event_data.roles, resource_ids: .event_data.resource_ids, reviewer: .event_data.reviewer, state: .event_data.state}]'
```
...
```code
$ tctl investigate --event-type=db.session.start --from=30d --limit=0 --format=json \
  | jq '[.data[] | select(
      (.event_data.with_mfa // "") == "" and
      ((.event_data.access_requests // []) | length) == 0 and
      (.event_data.impersonator // "") == "" and
      ((.event_data.trusted_device.device_id) // "") == ""
    )]'
```
...
```code
$ tctl investigate --event-type=session.start --from=30d --limit=0 --format=json \
  | jq '[.data[] | select(
      .event_data.proto == "ssh" and
      (.event_data.with_mfa // "") == "" and
      ((.event_data.access_requests // []) | length) == 0 and
      (.event_data.impersonator // "") == "" and
      ((.event_data.trusted_device.device_id) // "") == ""
...
```code
$ tctl investigate --event-type=kube.request --from=30d --limit=0 --format=json \
  | jq '[.data[] | select(
      (.event_data.with_mfa // "") == "" and
      ((.event_data.access_requests // []) | length) == 0 and
      (.event_data.impersonator // "") == "" and
...
```code
$ tctl investigate --event-type=db.session.start --from=30d --limit=0 --format=json \
  | jq '[.data[] | select(.event_data.db_protocol == "postgres" and .event_data.db_user == "postgres")]'
```
...
```code
$ tctl investigate --event-type=exec --from=30d --limit=0 --format=json \
  | jq '[.data[] | select(.event_data.proto == "kube")]'
```
...
```code
$ tctl investigate --event-type=cert.create --from=30d --limit=0 --format=json \
  | jq '[.data[] | select(((.event_data.identity.expires | sub("\\.[0-9]+Z$";"Z") | fromdateiso8601) - (.time | sub("\\.[0-9]+Z$";"Z") | fromdateiso8601)) > 86400)]'
```

</details>

**Suggestion:**  Use short lived certificates less than a working day. Check out Machine & Workload Identity to get short lived certificates for your automation.
...
```code
$ tctl investigate --event-type=instance.join --from=30d --limit=0 --format=json \
  | jq '[.data[] | select(((.event_data.token_expires | sub("\\.[0-9]+Z$";"Z") | fromdateiso8601) - (.time | sub("\\.[0-9]+Z$";"Z") | fromdateiso8601)) > 86400)]'
```
...
```code
$ tctl investigate --event-type=session.start --from=30d --limit=0 --format=json \
  | jq '[.data[] | select(.event_data.login == "root")]'
```
...
```code
$ tctl investigate --event-type=kube.request --from=30d --limit=0 --format=json \
  | jq '[.data[] | select((.event_data.kubernetes_groups // []) | index("system:masters"))]'
```

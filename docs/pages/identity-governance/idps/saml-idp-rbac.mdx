---
title: Configure Access to the SAML IdP Service Provider
description: This page explains how to configure access to the SAML IdP service provider (SAML application) resource in Teleport.
sidebar_position: 2
labels:
 - identity-governance
---

This page explains how access to a SAML IdP service provider resource can be managed in Teleport. 
...
- **Logging in to the SAML service provider.** For example, when a Teleport user attempts to log in to the 
  SAML IdP service provider by authenticating with Teleport. 
...
## RBAC behaviour between different Teleport role versions

SAML IdP service provider resource have a different RBAC behavior applied to them in 
...
- If role version 7 allows access but role version 8 does not explicitely allow access (via matching app labels), access is denied. 
...
the SAML IdP service proivder resource. 
...
In this case, at minimum, user needs `read,list` access to the `saml_idp_service_provider` resource and must have 
an `app_labels` value matching with the resource label defined for the `saml_idp_service_provider` resource. 

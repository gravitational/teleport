...
process. User audit events in Azure are tagged on behalf of the Teleport user account,
providing better audit visibility.

Alternatively, you can also manage access to the Azure API and CLI by deploying the 
...
Teleport user account, limiting user account visibility on the audit events.
...
Note that while adding Teleport as an identity provider, you need to provide a domain name 
...
Enter the details for this user as needed to set up the account. You will notice that the default value for
...
This is required so the users can access and be billed for Azure resources available under the linked subscription. 
...
Refer to the official the [Microsoft Entra External ID SAML federation](https://learn.microsoft.com/en-us/entra/external-id/direct-federation) docs for general configuration.

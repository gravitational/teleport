...
In the Teleport Web UI, from side the navigation menu, select **Zero Trust Access > Roles** 
...
## Step 2/3. Configure the service provider
...
metadata:
  # For RBAC, role allow "app_labels" rule will be matched with this resource label.
  labels:
    env: saml-test
...
Then modify the spec and update it using the `tctl create -f` command:
```code

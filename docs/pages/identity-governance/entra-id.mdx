...
Identity Security product. If you enable it, you [link access graph]
...
Designed for air-gapped Teleport clusters that are not publicly accessible, this setup accommodates 
environments where Azure cannot validate OIDC tokens issued by Teleport. Instead, Teleport relies 
on Azure credentials available on the VMs where Teleport Auth Service is running.

In both the cases, the credential should have the following API permissions at minimum.
```code 
...
By default, the guided OIDC configuration sets a broader scoped permission, which 
is required by the Idenitty Security product to perform policy and access path analysis.
```code
...
![Integrations](../../img/igs/entraid/entra-default-owners.png)
...
![Integrations](../../img/igs/entraid/entra-finish.png)
...
![Integrations](../../img/igs/entraid/entra-create-enterprise-app.png)
...
- **Entity ID and ACS URL:** SAML ACS endpoint of your Teleport cluster. E.g.  `https://<teleport cluster addr>/v1/webapi/saml/acs/entra-id`

For “Attributes & Claims”, attributes with user will already be available for you but you will need to 
add a `groups` claim. 

![Integrations](../../img/igs/entraid/entra-sso-config.png)


## Step 3/5. (Optional) Setting up OIDC IDP
...
![Integrations](../../img/igs/entraid/entra-setup-oidc.png)
...
![Integrations](../../img/igs/entraid/entra-api-permission.png)


## Step 5/5. Install Entra ID plugin

Now run `tctl plugins install entraid` command.

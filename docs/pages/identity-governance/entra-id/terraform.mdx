...
description: Describes how to set up Teleport Entra ID integration using Terraform
sidebar_position: 3
labels:
 - how-to
 - identity-governance
...
In this step, you configure authentication method for the Microsoft Graph client used 
by Teleport. 
...
Teleport Auth service is updated with permissions that grants access to the Graph API.  

<Tabs>
<TabItem default label="Teleport as an OIDC provider" name="entra-oidc">

To set up Teleport as OIDC IdP, you must set up a federated credential for the enterprise 
...
Next, grant Microsoft Graph API permissions to the enterprise application.
...
To set up a system credential, you need to grant Graph API permissions to the Managed Identity 
that is assigned to the Teleport Auth service. 

Azure supports two kinds of Managed Identities - system assigned and user assigned. 
For simplicity, a method to update a system assigned Managed Identity, which is assigned to a 
VM that hosts Teleport Auth Service is demonstrated below. 

First, ensure that a system assigned Managed Identity is enabled for your VM.
This can be configured by setting up the [`identity`](https://registry.terraform.io/providers/hashicorp/azurerm/latest/docs/resources/virtual_machine#identity-1)
...
To assign permissions, first, you will need the ID of the Graph API permission
...
resource "azuread_app_role_assignment" "example_permission_application" {
...
resource "azuread_app_role_assignment" "example_permission_group" {
  # ID of the API permission retrieved using PowerShell. 
...
resource "azuread_app_role_assignment" "example_permission_user" {
  # ID of the API permission retrieved using PowerShell. 

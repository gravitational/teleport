...
To request access to a namespace and all resources within it, you can use the
following command:
...
The first resource, `/teleport.example.com/kube:cw:namespaces/mycluster/prod`
requests access to the namespace resource itself. It is only needed if you want
to gain write access. Read-only access to namespace is automatically granted
when you have access to any resource within it.

The second resource, `/teleport.example.com/kube:ns:*.*/mycluster/prod/*`
requests access to all resources within the namespace.

- `teleport.example.com` is the Teleport cluster name.
- `kube:ns:*.*`
  - `kube:ns:` means we are requesting access to namespaced resources.
  - The first `*` represent the kind, so we request access to all kinds
  within the namespace.
  - The second `*` represents the api group, so we request access to all
  api groups.
- `mycluster/prod/*`
  - `mycluster` is the target kubernetes cluster.
  - `prod` is the namespace we are requesting access to. It could also
  be `*` to request access to all namespaces.
  - The final `*` represents the resource name, so we request access to all
  resources.
</Admonition>

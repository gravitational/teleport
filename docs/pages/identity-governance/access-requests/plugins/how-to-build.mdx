...
In this guide, you will explore a number of Teleport's API client libraries by
writing a plugin that lets you manage Access Requests via Google Sheets. The
plugin lists new Access Requests in a Google Sheets spreadsheet, with links to
allow or deny each request. 

...
1. Write your SMTP service's password to a local file called `smtp-password`.
1. Create a Kubernetes secret from the file:

  ```code
  $ kubectl -n teleport create secret generic smtp-password --from-file=smtpPassword=smtp-password
  ```

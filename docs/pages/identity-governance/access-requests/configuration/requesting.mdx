...
This guide describes configuration options for the kinds of access a user can
request. You can also require a user to request access on login. See [Teleport
Users with Zero Standing Privileges](no-standing-privileges.mdx).
...
1. If the maximum duration is greater than zero, set the duration of elevated
   ...
   Teleport only considers the maximum duration greater than zero if either:

   - The requestor provided a value for the `--max-duration` flag
   - At least one of the requested roles includes a `request.max_duration` field

   If these values are unset, Teleport only uses the session TTL.
...
1. Begin with the base expiration of the Access Request, which a user can set
   with the `--request-ttl` flag of the [`tsh request
   create`](../../../reference/cli/tsh.mdx) command. If this is unset, the
   request TTL is one hour.

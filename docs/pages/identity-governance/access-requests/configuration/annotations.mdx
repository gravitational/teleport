...
|ServiceNow|`teleport.dev/schedules`|The integration approves a user's Access Request if the user is on call for one of the services listed in `teleport.dev/schedules`.|
...
If you are expecting a plugin to behave a certain way in response to an Access
Request, but it has not, you can begin troubleshooting by checking whether the
Teleport Auth Service has added the correct annotations to an Access Request. If
it has not, you will need to ensure users who rely on your Access Request plugin
have the appropriate [role settings](#allowing-and-denying-annotations).

Run the following command, which lists open Access Requests along with any
system annotations:

```code
$ tctl get access_requests --format=text --verbose
```

The output will resemble the following example, in which the user `alice`
created an Access Request that received a correct annotation for the Pagerduty
plugin:

```
Name                                 User   Roles        Annotations
------------------------------------ ------ ------------ ----------------------------------
00000000-0000-0000-0000-000000000000 alice  access       pagerduty_notify_service/myservice
```

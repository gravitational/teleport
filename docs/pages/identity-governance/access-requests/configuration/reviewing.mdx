...
Unlike `options.request_access`, which configures whether a user needs to
provide a reason when first authenticating ([Zero Standing
Privileges](no-standing-privileges.mdx)), the `spec.allow.request.reason.mode`
field only configures a request prompt when a user who is already authenticated
creates a request.
...
For resource-based Access Requests, owners of Access Lists that grant access to
the requested resources can be automatically added as suggested reviewers.
...
Owners of an applicable Access List can also be automatically added as suggested reviewers
when the requester is already a member of the Access List.
...
For [short-term requests](../access-requests.mdx), an Access List is applicable when its member grants include a role that allows requesting one of the requested roles.

For [long-term requests](../../access-lists/access-lists.mdx), an Access List is applicable when the Access List directly grants one of the requested roles.
...
Note that in both cases those owners still need to have an appropriate Teleport role that
allows them to review such Access Requests.

You can learn more about these use-cases in the [Reviewing Access Requests](../../access-lists/reviewing-access-requests.mdx)
guide.
...
applies the allow or deny rules for the `review_requests.claims_to_roles` and
...
You can grant permissions to Access Request reviewers to inspect requested
resources, giving them the context they need to approve or deny an request.

...
sidebar_label: Notification Routing Rules
...
routed to plugins based on several conditions. For example, you may
...
the AMR and uses it to process incoming events. 
...
Here is an example Access Monitoring Rule. This rule will notify
`your_slack_channel` via the Slack plugin if an Access Request containing the
role `your_role_name` is made. Open a file called `notify-slack.yaml` with the
following content:
...
```code
$ tctl create -f notify-slack.yaml
```

...
`dba` roles, and specifies some more complex restrictions that we will explain
below:
...
1. Calculate the maximum duration of elevated privileges if the Access Request
   ...
   - The lowest value of the `request.max_duration` field included in one of
     ...
     If neither the `--max-duration` flag nor any `request.max_duration` values are set, the maximum duration is considered unset during calculation.
...
1. Begin with the base expiration of the Access Request, which a user can
...
Request with the key `my-annotation`.

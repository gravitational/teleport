...
You can specify a custom prompt to remind the user to provide a reason for specific requestable
...
It is possible to specify a custom prompt for all or none of the Access Requests for a particular user.
This is achieved by setting the `spec.options.request_prompt` to a non-empty string. In order to
specify a custom prompt for Access Requests for a particular requestable role or resource, set
`spec.allow.request.reason.prompt` to a non-empty string.

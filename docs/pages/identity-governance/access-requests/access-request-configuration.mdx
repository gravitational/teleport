...
It is possible to specify a global prompt for all or none of the Access Requests for a particular user.
This is achieved by setting the `spec.options.request_prompt` to a non-empty string.

In order to specify a scoped prompt rule for a particular requestable role or resource, set
`spec.allow.request.reason.prompt` to a non-empty string. This will affect only Access Requests that
contain the specific role or resource.

If both global prompt rules and multiple scoped prompt rules apply to the same Access Request, all
prompts will be listed in order within the reason box.
...
If you [write your own Access Request plugin](../access-request-plugins/access-plugin.mdx), the

...
- Teleport user (`alice` in this guide) with an `editor` or equivalent administrative role.
- Teleport user (`bob` in this guide) acting as a low-privileged requester user.

## Step 1/5. Create roles

First, let's create 3 roles:
...
Assign the `ssh-access-requester` role to `bob`.
...
Now, let's create an access list that allows its owner to review requests to
these SSH nodes.

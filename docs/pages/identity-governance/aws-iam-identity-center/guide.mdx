...
- Teleport preset `editor` role or equivalent to configure plugin resource, integration 
  resource and SAML IdP service provider resource.
...
To resolve RBAC error, first, identify what roles are assigned to the user. 

If the user has been assigned with a Teleport role of version 8, 
the role must define `app_labels` matching `teleport.dev/origin: aws-identity-center`
label.
...
If the user is assigned with Teleport role version 7 or below, 

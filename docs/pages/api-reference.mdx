...
Or if you prefer, follow the Authentication, Go Client, and Go Packages sections below to add the necessary files to a new directory called `/api-examples`. At the end of the file, you should have this structure:
...
To interact with the API, you will need to provision appropriate TLS certificates. To provision certificates, you will need to create a user with appropriate permissions. You should only give the API user permissions for what it actually needs.
...
Every user in Teleport is assigned a set of [roles](./access-controls/reference.mdx#roles). A user's roles define what actions or resources the user is allowed or denied to access. We offer a wide array of permissions, allowing you to safely and precisely give developers access to the resources they need.
...
<Admonition type="note">
  The open-source edition of Teleport automatically assigns every user to the built-in `admin` role, but Teleport Enterprise allows administrators to define their roles with far greater control over the user permissions.
</Admonition>
...
To see a role example in `yaml` form, look at the the admin role in the [RBAC documentation](./access-controls/reference.mdx#roles). You'll notice that the role object below has the same nested structure as its `yaml` counterpart.
...
However, this also allows you to take a modular approach to define roles. Splitting roles up into logical parts will allow you to manage roles for many developers more effectively. It may also be effective to keep your deny and allow conditions in separate roles so that conflicting roles are obvious.
...
type RoleConditions struct {
  ...
  NodeLabels Labels
  // Rules is a list of rules and their access levels. Rules represent allowed or denied rules
  // that is executed to check if the user or service has access to a resource
  Rules []Rule
...
Suppose you wanted to create a role for an auditor that could view all sessions but could not access any servers. Using `tctl`, you would first create a role that allows reading and listing of the session resource like below. The user is also explicitly denied access to all nodes in the deny block.
...
These tokens can be predefined static tokens or dynamic tokens with a short lifetime. The latter can be generated by `tctl` or this API, and is more secure.
...
Cluster Labels can be used to differentiate between leaf clusters in a [Trusted Cluster](trustedclusters.mdx). These can be useful when defining [roles](./access-controls/reference.mdx#roles) within a trusted cluster, where each cluster has its requirements for access.

For example, if each cluster corresponds to a different product that should only be accessed by its product team, you can give each cluster a label like `product: A`. Then create a role for each product that allows access to clusters with its respective product label.
...
For example, you could have a team of contractors who need database access for some tasks but should not have it permanently. To do this, you can give them the `contractor` role below, which allows them to request the `dba` role.
...
4. `Update Servers`: Servers will have to reload and should start serving TLS and SSH certificates signed by the new CA (retrieved in the previous phase).
...
- You could catch rotation errors and automatically trigger the `Rollback` phase to try again (though this shouldn't be necessary outside of specific scenarios, such as servers going offline for long periods).

...
teleport-cluster-auth-57989d4cbd-4q2ds    1/1     Running   0          46s
...
```code
$ kubectl exec -it deployment/teleport-cluster-auth -- tctl users add --roles=access appuser
...
```code
$ kubectl exec -i deployment/teleport-cluster-auth -- bash -c "cat<<EOF>>/home/cp.yaml
...
$ kubectl exec -it deployment/teleport-cluster-auth -- tctl tokens add \
--type=app \
--app-name=kube-dash \
...
```code
$ kubectl exec -it deployment/teleport-cluster-auth -- tctl tokens ls
```

...
description: How to configure Teleport in High Availability (HA) mode for AWS deployments
...
Gravitational automatically builds and publishes Open Source, Enterprise, and Enterprise FIPS 140-2 AMIs when we
...
(Open Source Edition users can provide any valid local file path here - it isn't used by the auth server in a Teleport Open Source Edition install)
...
useful information. We recommend a generic ops/support email address that the team deploying Teleport has access to.
...
- The client is the same for both the Open Source and Enterprise editions of Teleport.

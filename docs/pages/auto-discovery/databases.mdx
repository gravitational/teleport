...
1. One or more Teleport Discovery Services polling APIs to discover and register
  databases with your Teleport cluster.
1. One or more Teleport Database Service instances forwarding database connections to
  the discovered databases.

Both Discovery and Database service can be configured in the same Teleport
...
discovery_service:
    ...
    aws:
      # AWS resource types. Valid database discovery types are:
      ...
      # 'elasticache' - discovers and registers Amazon ElastiCache Redis databases.
      # 'memorydb' - discovers and registers Amazon MemoryDB Redis databases.
      # 'opensearch' - discovers and registers Amazon OpenSearch Redis databases.
    - types: ["rds"]
...
db_service:
  ...
  resources:
    - labels:
        ...
        # Cloud resources are also labelled to indicate which cloud provider
        # they came from.

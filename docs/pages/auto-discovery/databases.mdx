...
Now that you know how to set up Teleport auto-discovery for AWS databases, learn
how to configure Teleport to manage access to the AWS databases in your
infrastructure. 

### Filter out specific endpoints

You can use the Teleport generated label `endpoint-type` on your `aws` matcher
...
### Connect more AWS databases

- Learn about [Dynamic Registration](./dynamic-registration.mdx) by the
  Teleport Database Service, which you can use to enroll databases in Teleport
  as you deploy them.
- Connect AWS databases in [external AWS accounts](./aws-cross-account.mdx).
- See [all of the databases](../guides.mdx) that Teleport can protect.

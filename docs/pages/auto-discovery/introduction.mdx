...
infrastructure. You can configure the Discovery Service to filter resources
based on labels. The Discovery Service then reconciles these resources with
Teleport resources registered on the Auth Service backend.

Other agent services are responsible for monitoring dynamic resources created by
the Discovery Service. As with any dynamic resource that represents a component
of your infrastructure, like a database or internal web application, agents can
start proxying discovered resources once the Discovery Service creates them on

...
The insecure mode skips the verification of the HTTPS certificate when talking to a proxy service of
a Teleport cluster. This is useful in test environments or for demo purposes, we do not recommend
using this mode in production.

To launch the app in the insecure mode, open a terminal first. From there you can launch the app in
one of the two ways:

...
Local users are a reliable fallback for cases when the SSO provider is down. Let's create a local admin `alic`e` who has access to Kubernetes group `system:masters`.

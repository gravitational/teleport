...
cluster automatically. The following diagram provides a simplified overview of this deployment
scenario with  the Teleport Kubernetes Service running on the Kubernetes cluster:
...
- Access to a running Teleport cluster, `tctl` admin tool, and `tsh` client tool, 
  ...
  use the Enterprise version of `tctl` and `tsh`. 
...
The guided enrollment simplifies the deployment process by prepopulating commands
...
see [Enroll Kubernetes manually](#enroll-kubernetes-manually).

## Step 2/4. Follow guided enrollment

To enroll a Kubernetes cluster using the Teleport Web UI:

1. Type the address for your Teleport cluster in a browser.

1. Sign in using your administrative account and second factor authentication.
...
1. Copy the command to add the `teleport-agent` chart to your charts repository and paste 
   it in a terminal on the target computer where Kubernetes and Helm are installed.
...
1. Type a Kubernetes group names and, optionally, user names that should have access to Kubernetes
   resources, then click **Next**.

   You must specify at least one Kubernetes group. If you don't specify a Kubernetes user, you can
   connect to the cluster using your Teleport user is by default.
...
1. View pods using the `kubectl` command routed through the Teleport cluster:
   ...
   balanced-567b5f87b5-pvkvr         1/1     Running   0          143m
   hello-minikube-59d4768566-tcnv6   1/1     Running   0          144m
   ```

## Enroll Kubernetes manually

If you can't use the Teleport Web UI or want to work exclusively in the terminal,
you can enroll a Kubernetes cluster manually.

To enroll manually:

1. Sign in to your Teleport cluster from your administrative workstation
   using your administrative account:

   ```code
   $ tsh login --proxy=<Var name="teleport.example.com"/> --user=<Var name="email@example.com"/>
   ```

1. Request a token from the Teleport Auth Service for the Teleport Kubernetes Service to use
   to join the cluster:
...
1. Allow Helm to install charts from the Teleport Helm repository:

   ```code
   $ helm repo add teleport https://charts.releases.teleport.dev && helm repo update
   ```

1. Set the environment variables for the Teleport cluster address and Kubernetes cluster name:

   ```code
   PROXY_ADDR=<Var name="teleport.example.com"/>:443
   CLUSTER=<Var name="my-k8s-cluster"/>
   ```

1. Create the `values.yaml` file using the appropriate settings for your Kubernetes and Teleport clusters:
   ...
   enterprise: true # Remove this field for Teleport Community Edition
   EOF
   ```

1. Install the helm chart with the `values.yaml` settings:
   ...
   The Teleport Kubernetes Service version should be the same as the Teleport cluster version or 
   no more than one major version behind it. You can override the version by adding 
   `--set teleportVersionOverride=(=teleport.version=)` as a command-line option.

1. Verify that the Teleport agent pod is running:

   ```code
   $ kubectl -n teleport-agent get pods
   ```

   You should see one Teleport agent pod pod with a single ready container:
...
Take a look at a [kube-agent helm chart reference](../reference/helm-reference/teleport-kube-agent.mdx) for 

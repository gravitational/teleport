...
style telekube fill: #844cff,color: #ffffff
style kube fill: #326ce5,color: #ffffff
  telekube[Teleport Kubernetes Service]<--> kube[Kubernetes API Server]
end

subgraph pub[ ]
style pub fill:none,stroke:none
style proxy fill: #844cff,color: #ffffff
  Users-- "kubectl get pods (mTLS)" -->proxy[Teleport Proxy Service\nteleport.example.com]
end

telekube<-- Encrypted Tunnel -->proxy
```

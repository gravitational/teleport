...
Let's create a local admin `teleport-admin` who has access to Kubernetes group `system:masters`.
...
$ kubectl exec -ti ${POD?} -- tctl users add teleport-admin --roles=editor,access --kubernetes_groups=system:masters --logins=k8s-keep-this-login

User "teleport-admin" has been created but requires a password. Share this URL with the user to
...
{"error":"user \"teleport-admin\" does not belong to any teams configured in \"github\" connector","method":"github","attributes":{"octocats":["devs"]}}
```
</Admonition>

...
Similarly to [EKS Auto-Discovery](./aws.mdx), AKS Auto-Discovery can automatically 
...
- Azure identity with permissions to create AD Groups 
...
In some configurations, Teleport has the ability to automatically set up its 
access to the cluster, if you include the permissions required for it to do so.
...
To grant access to AKS cluster running with this configuration, create an AD role 
...
- Teleport identity has access to the static [cluster administrator credentials](https://learn.microsoft.com/en-us/rest/api/aks/managed-clusters/list-cluster-admin-credentials) (Local Accounts).
- Teleport identity belongs to the cluster's administrator group.
- Teleport identity has permissions to create `ClusterRole` and `ClusterRoleBinding` on the 
cluster and permissions to execute [remote commands](https://learn.microsoft.com/pt-pt/rest/api/aks/managed-clusters/run-command).
...
Connect to every cluster with admin permissions and create the following resources 
using `kubectl``.
...
Link the previously created `ClusterRole` into a Teleport AD group id.
...
Please replace `{azure_ad_group_id}` with the appropriate Azure AD group id
...
Once configured, Teleport can be started and AKS clusters matching the tags and regions

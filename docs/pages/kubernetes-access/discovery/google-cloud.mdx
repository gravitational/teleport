...
Create the role, replacing `project-id` with the name of your project:
...
Create the role, replacing `project-id` with the name of your project. If you
receive a prompt indicating that certain permissions are in `TESTING`, enter
`y`:
...
Grant the roles you defined earlier to your service account, replacing
...
Grant the roles you defined earlier to your service accounts, replacing
`project-id` with the ID of your project.
...
(e.g., via Amazon EC2 or on a home network).
...
Attach your service account to the VM running the Kubernetes Service:
...
Attach your service account to the VM running the Discovery Service:
...
clusters in project `myproj-prod` running in `us-east2`, but _not_ clusters in

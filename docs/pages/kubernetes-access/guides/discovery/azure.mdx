...
The Teleport Kubernetes Discovery requires a valid auth token for the `discovery` and `kube` roles 
...
include at least one entry and must specify the `aks` type. On the other hand, it 

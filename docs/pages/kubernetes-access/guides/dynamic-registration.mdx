...
Kubernetes clusters in your instrastructure.
...
```code
$ PROXY_SERVICE=teleport.example.com:443;
$ TOKEN=(=presets.tokens.first=);
...
resources with any label key and any label value.
...
match, _all_ of the `labels` entries within that item must match the cluster's
labels.

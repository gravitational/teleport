...
In release 5.0, Teleport has changed the [Kubernetes integration](../../kubernetes-access/introduction.mdx) to improve configuration and user experience.
These changes are backward compatible and there is no *required* migration.
...
- Support for multiple Kubernetes clusters per Teleport cluster (replaces the
  need to use [Trusted Clusters](../../setup/admin/trustedclusters.mdx) to achieve this)
...
This approach requires you to manually craft a `kubeconfig` file that contains
...
Users with a role that matches a specific Kubernetes cluster label will be

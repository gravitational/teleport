...
`ClusterRoleBindings` or `RoleBindings` resources. If by any chance they do not
exist in the cluster, Kubernetes RBAC will ignore them.

An example of a `ClusterRoleBinding` resource that assigns the built-in `view`
...
Teleport supports Service Account impersonation by using the Service Account
fully qualified name in the `kubernetes_users` field.

The Service Account fully qualified name consists of the following pattern:
...
An example of a role that impersonates a Service Account can be found bellow.

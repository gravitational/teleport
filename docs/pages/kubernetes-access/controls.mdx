...
<Notice type="tip" >
  For a user to have Kubernetes access they must be asssigned a Teleport role that
  contains `kubernetes_groups` or `kubernetes_users`. The groups or user assigned
  must be associated to roles with access to the pods within that Kubernetes
  cluster. Teleport does not alter those roles to allow or deny access.
  Teleport confirms access based on the Teleport `pod` role settings at 

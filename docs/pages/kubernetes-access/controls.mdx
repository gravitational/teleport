...
Here are examples of connectors that map SSO users to Kubernetes groups in the Open Source Edition
...
<Tabs>
  <TabItem label="Github (Open Source)">
    ```yaml
    kind: github
    version: v3
    metadata:
...
spec:
  allow:
    # Allow access to kubernetes clusters in a production environment
    kubernetes_labels:
      'env': 'prod'
```
...
- [Google Workspace](../enterprise/sso/ssh-google-workspace.mdx)

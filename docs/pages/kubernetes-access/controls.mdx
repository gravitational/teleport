...
<Notice type="tip" >
  ...
  must also be assigned within a role to a user.  `kubernetes_resources` configuration
  requires the users or groups within the Kubernetes clusters have access to the pods
  and does not grant additional access if the Kubernetes role does not.
</Notice>

...
within the user's roles. If one of these fields does not allow the user to
...
For eample, this combination of roles allows a user to view all pods in all
...
group, which can execute commands and retrieve logs, but only `nginx` on pods. 

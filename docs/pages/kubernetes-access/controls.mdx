...
<Notice type="tip" >
  For a user to have Kubernetes access `kubernetes_groups` or `kubernetes_users`
  must also be assigned within a role to a user. The groups or user assigned must
  be associated to roles with access to the pods within that Kubernetes
  cluster. Teleport does not alter the those roles to allow or deny access.

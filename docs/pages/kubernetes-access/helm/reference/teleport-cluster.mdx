...
<Admonition type="note" title="Installing cert-manager">
  ...
  and the relevant sections of the [AWS](../guides/aws.mdx#step-47-configure-tls-certificates-for-teleport) and [GCP](../guides/gcp.mdx#step-47-install-and-configure-cert-manager) guides for more information.
</Admonition>
...
### `highAvailability.certManager.createIssuer`

| Type | Default value | Can be used in `custom` mode? | `teleport.yaml` equivalent |
| - | - | - | - |
| `bool` | `false` | ❌ | None |

Setting `highAvailability.certManager.createIssuer` to `true` will create `Issuer` or `ClusterIssuer` to use for issuing certificates.
...
<Tabs>
  ...
  ```code
  $ --set highAvailability.certManager.enabled=true \
  --set highAvailability.certManager.createIssuer=true \
  --set highAvailability.certManager.issuerName=letsencrypt-production \
  --set highAvailability.certManager.issuerSpec.acme.email=email@address.com \
  --set highAvailability.certManager.issuerSpec.acme.server=https://acme-v02.api.letsencrypt.org/directory \
  --set highAvailability.certManager.issuerSpec.acme.privateKeySecretRef.name=letsencrypt-production \
  --set highAvailability.certManager.issuerSpec.acme.solvers[0].selector.dnsZones[0]=example.com \
  --set highAvailability.certManager.issuerSpec.acme.solvers[0].dns01.route53.region=us-east-1 \
  --set highAvailability.certManager.issuerSpec.acme.solvers[0].dns01.route53.hostedZoneID=Z0159221358P96JYAUAA4
  ```
  </TabItem>
</Tabs>
...
<Admonition type="note" title="Configuring an Issuer">
  ...
  of the [AWS](../guides/aws.mdx#step-47-configure-tls-certificates-for-teleport) and [GCP](../guides/gcp.mdx#step-47-install-and-configure-cert-manager) guides for more information.
</Admonition>
...
### `highAvailability.certManager.issuerSpec`

| Type | Default value | Can be used in `custom` mode? | `teleport.yaml` equivalent |
| - | - | - | - |
| `object` | `{}` | ❌ | None |

Spec of `Issuer` or `ClusterIssuer`.
...
<Admonition type="note" title="Installing cert-manager">
  ...
  You must create this yourself when `highAvailability.certManager.createIssuer` is `true`.
</Admonition>

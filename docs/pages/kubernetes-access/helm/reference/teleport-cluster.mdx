...
### `highAvailability.certManager.usages`

| Type | Default value | Can be used in `custom` mode? | `teleport.yaml` equivalent |
| - | - | - | - |
| `array` | `[]` | ❌ | None |

Sets the `usages` to be used when issuing certificates with `cert-manager`. Defaults to `[]` to use built-in usages.
...
<Tabs>
  ...
  <TabItem label="--set">
  ```code
  --set highAvailability.certManager.usages=[]
  ```
  </TabItem>
</Tabs>

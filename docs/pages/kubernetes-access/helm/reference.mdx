...
<Admonition type="tip" title="Database auto-discovery">
  This section configures database auto-discovery, which is only currently supported on AWS. You can configure databases for other platforms using the `databases` section below this.
</Admonition>

<Admonition type="note" title="IAM roles">
  For AWS database auto-discovery to work, your agent pods will need to use a role which has appropriate IAM permissions as per the [database documentation](../../database-access/guides/rds.mdx).

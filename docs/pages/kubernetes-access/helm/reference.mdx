...
| Type | Default value | Can be used in `ownconfig` mode? | `teleport.yaml` equivalent |
| - | - | - | - |
| `string` | `nil` | ❌ | `auth_service.cluster_name`, `proxy_service.public_addr` |
...
| Type | Default value | Can be used in `ownconfig` mode? |
| - | - | - |
| `bool` | `false`| ✅ |
...
<Admonition type="note" title="Installing cert-manager">
  ...
  and the relevant sections of the [AWS](./guides/aws.mdx#install-and-configure-cert-manager) and [GCP](./guides/gcp.mdx#install-and-configure-cert-manager) guides for more information.
</Admonition>
...
Sets the name of the `cert-manager` `Issuer` or `ClusterIssuer` to use for to use for issuing certificates.
...
<Admonition type="note" title="Configuring an Issuer">
  ...
  of the [AWS](./guides/aws.mdx#install-and-configure-cert-manager) and [GCP](./guides/gcp.mdx#install-and-configure-cert-manager) guides for more information.
</Admonition>
...
<Admonition type="note">
  These annotations will not be applied in `ownconfig` mode, as the `ConfigMap` is not managed by the chart.
...
```bash
kubectl --namespace teleport create secret generic teleport-kube-agent-join-token --from-literal=auth-token=D4Z280P1mHiY78bKcfTGNKN52Gg5wc8q
```

...
<br />
While the Kubernetes cluster is being provisioned, follow the "Get started" guide as shown below:
...
```code
$ CLUSTERNAME=tele.teleporters.dev(replace with your preferred domain name)
...
```
Once you get the value for external IP (it may take a few minutes for this field to be populated), update your DNS record such that the clusterName's A record points to this IP address. For example `192.168.200.200` is the external IP in the above case.
...
# https://tele.teleporters.dev:443/web/invite/<invite-token>
...
Second, update **tadmin** user role to assign **member** role:
<Figure align="left" bordered caption="Update role">
  ![Update role](../../../../img/helm/digitalocean/update-role.png)
</Figure>

We've updated the user **tadmin** to have the **member** role, whish is allowed to access Kubernetes cluster with privilege `system:master`.
...
First, authenticate to Teleport using `tsh` as a **tadmin** user which we created earlier in the third step. 

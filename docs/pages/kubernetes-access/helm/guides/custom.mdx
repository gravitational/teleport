...
For this example, we'll be using this `teleport.yaml` configuration file (with appropriately complex [static tokens](../../../admin-guide.mdx#static-tokens)):
...
<Admonition type="warning">
  ...
  It's important to specify any settings under the `acme`, `aws`, `gcp` and `logLevel` sections of the chart in your own
  `teleport.yaml` file that you upload yourself.
</Admonition>
...
<Admonition type="tip">
  ...
  Don't worry if you can't - you'll just have to remember to replace `custom.example.com` with the external IP or hostname
...
NOTE: Make sure custom.example.com:443 points at a Teleport proxy which users can access.
```
...
<Admonition type="note">
  ...
  You should modify your command accordingly and replace `custom.example.com` with either the IP or hostname depending on which you have
...
You can follow our [Getting Started with Teleport guide](../../../getting-started.mdx#step-24-create-a-teleport-user-and-set-up-two-factor-authentication) to finish setting up your
Teleport cluster.

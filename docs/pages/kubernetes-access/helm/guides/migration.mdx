...
In this guide, we'll detail a way to migrate an existing Teleport cluster using the older `teleport` Helm chart
...
## Migrating from the `teleport` chart to `teleport-cluster`

### Get the Teleport configuration file from your existing cluster
...
### Extracting the contents of Teleport's database
...
### Start the new cluster with your old config file and backup
...
### Remove the bootstrap data and update the chart deployment

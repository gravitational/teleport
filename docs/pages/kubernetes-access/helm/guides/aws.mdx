...
In this guide, we'll go over setting up a High Availability Teleport cluster with multiple replicas on Kubernetes
...
For Teleport to be able to create its required DynamoDB tables, indexes, and S3 storage bucket,
...
| `teleport-helm-state` | DynamoDB table name to use for the Teleport cluster state and backend |
| `teleport-helm-events` | DynamoDB table name to use for the Teleport audit log (**must** be different from the state table)
    ...
    "Statement": [
        {
            ...
            "Resource": [
                "arn:aws:dynamodb:us-west-2:1234567890:table/teleport-helm-state",
                "arn:aws:dynamodb:us-west-2:1234567890:table/teleport-helm-state/stream/*"
            ]
        },
        {
...
<Tabs>
  ...
  helm install teleport teleport/teleport-cluster \
    ...
    --set aws.backendTable=teleport-helm-state                            `# DynamoDB table to use for the Teleport cluster state and backend` \

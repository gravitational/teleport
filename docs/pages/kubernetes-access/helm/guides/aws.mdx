...
We now need to configure TLS certificates for Teleport to secure its communications.

There are two options when using AWS:
- Use `cert-manager` to provision and automatically renew ACME certs (described in [Step 4a](#step-4a-install-and-configure-cert-manager-to-handle-tls))
  - This is better supported overall and will enable all Teleport services such as PostgreSQL database access
    and CLI application access to work correctly. This is the recommended deployment option.
- Use [AWS ACM](https://aws.amazon.com/certificate-manager/) to handle TLS termination with AWS-managed certificates (described in [Step 4b](#step-4b-use-acm-to-handle-tls))
  - The use of ACM will not allow Teleport to handle PostgreSQL database connections or application access via CLI, as
    Teleport will not be handling its own TLS termination.
...
If you are planning to use `cert-manager`, you will need to add one additional IAM policy to your cluster to enable it
...
## Step 4b. Use ACM to handle TLS

To use ACM to handle TLS, add additional annotations to the chart specifying the ACM certificate ARN to use.

We also use the `extraArgs` value to disable Teleport's TLS listener, making the backend connection use plain TCP from the load balancer
to the pod.
...
<Tabs>
  ...
  ```code
    --set extraArgs[0]="--insecure-no-tls" \
    ...
    --set "annotations.service.service.\.beta\.kubernetes\.io/aws-load-ssl-ports:\"443\""
  ```
  </TabItem>
</Tabs>

<Admonition type="note" title="Internal load balancers">
  To use an internal AWS network load balancer (as opposed to the default internet-facing NLB), you should add two additional annotations:

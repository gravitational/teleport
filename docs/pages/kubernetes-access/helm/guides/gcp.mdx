...
<Admonition type="note">
  You cannot change the `clusterName` after the cluster is configured, so make sure you choose wisely. We recommend using the
...
<Admonition type="note">
  As you have not configured TLS certificates for your cluster, you will need to accept insecure warnings in your browser to view
  the user creation page successfully. You can use services like `cert-manager` to provision certificates for your cluster.
</Admonition>

...
Teleport `discovery` and `app` services are required to run in the same Kubernetes cluster which services are being discovered.
...
Discovery service by default will only try to expose ports that serve HTTP/HTTPS. To understand if this port serves HTTP, discovery 

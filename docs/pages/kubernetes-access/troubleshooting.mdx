---
title: Kubernetes Access Troubleshooting Guide
...
The agent's state needs to be reset, so it requests a new certificate and joins
the cluster again.
...
non-GKE clusters, so please don't change the `kubernetes_groups` parameter to

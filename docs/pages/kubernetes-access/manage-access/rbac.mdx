...
send requests to to their authorized Kubernetes clusters via the Teleport
...
Install the Teleport Kubernetes Service in your cluster, assigning 
<Var name="proxy-address" /> to the host and port of your Teleport Proxy Service
...
- `kubernetes_resources`: Allows access to the `webapp` pod in the `production`
  namespace and all pods in the `development` namespace.
...
To access your Kubernetes cluster via Teleport, authenticate to it to update
your kubeconfig:
...
development   loadbalancer   1/1     Running   0          65m
development   webapp         1/1     Running   0          65m
production    webapp         1/1     Running   0          65m
```

You can access information about the `webapp` pod in the `production` namespace:

```code
$ kubectl -n production get pods/webapp -o json
```

If you attempt to get information about the `loadbalancer` pod in the
`production` namespace, you will see an error:

```code
$ kubectl -n production get pods/loadbalancer -o json

Error from server (Forbidden): pods "loadbalancer" is forbidden: User "myuser" cannot get resource "pods" in API group "" in the namespace "production"
```

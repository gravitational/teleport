...
1. Sign in to your Teleport cluster by running the `tsh login` command  with the
   URL of your cluster and the name of your Teleport user. 
   Replace <Var name="example" /> with your account's subdomain of
   `teleport.sh` and <Var name="username" /> with your Teleport username:
...
1. Access your server as the `root` user, replacing <Var name="node-name" />
   with the name of your container as displayed by `tsh ls`:

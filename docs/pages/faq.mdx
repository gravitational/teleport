...
### Can I set a default username for my SSH connections?

Yes, just set the `TELEPORT_LOGIN` environment variable in your terminal or user profile settings (~/.bashrc). 
```bash
# example of setting jeff as the default username instead of using the logged in username
whoami
jeffery
export TELEPORT_LOGIN=jeff
tsh ssh myserver
jeff@myserver:~#
```

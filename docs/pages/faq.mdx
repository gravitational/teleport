...
## Can I copy files on a machine without being logged in, i.e. headless authentication?

Yes, Teleport supports [Headless WebAuthn authentication](.access-controls/guides/headless.mdx) that allows you to perform operations like
downloading or uploading via `scp` or opening `ssh` sessions on remote stations you aren't
logged into. You confirm the operation easily from your local browser or terminal.

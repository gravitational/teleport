...
Gravitational offers this feature for all [Enterprise versions of Teleport](enterprise/introduction.mdx).

### What features ship with Teleport?

![Teleport feature overview](../img/feature-overview.png)

Teleport is a flexible SSH tool whose features are designed to assist users in four main areas (organized into products):

1. **Server Access**: server and cluster management scenarios.
2. **Kubernetes Access**: kubernetes management.
3. **Application Access**: application access. 
4. **Database Access**: access to database resources.

These products have several in-built features that are common to those use scenarios but customers can pick and chose which features are needed for their specific needs. Some features are toggleable, enabled through configuration, or via a plugin.

Some of the most common features include:

1. **Identity Awareness** - Single Sign-On (SSO), Short-lived certs, and Multi-Factor Authentication (MFA).
2. **Access Controls** - Role-Based Access Control (RBAC), Session Controls, and Access Requests.
3. **Visibility** - Events, Session Recording, and Session Player.

These features are typically available in both Teleport Open Source and Commercial Editions though often with enhanced functionality in the latter.

## Commercial Teleport Editions

### What is included in the commercial version, Teleport Enterprise?

Teleport Enterprise Edition includes your favorite features from the Open Source Edition:

- [Role-Based Access Control (RBAC)](enterprise/introduction.mdx#role-based-access-control).

In addition to the following commercial features:

- Authentication via SAML and OpenID with providers such as Okta, Active
  Directory, Auth0, or your preferred method of [SSO](enterprise/sso/ssh-sso.mdx).
- Premium support.

We also offer implementation services, to help you integrate
Teleport with your existing systems and processes.

Read more about [Teleport Enterprise Edition](enterprise/introduction.mdx).

### Does Teleport send any data to Gravitational?

The Teleport Open Source Edition does not send any information to

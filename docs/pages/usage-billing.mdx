...
description: Provides a detailed breakdown of Teleport usage reporting.
tags:
 - conceptual
 - platform-wide
---
...
calculate usage metrics. This guide describes the anonymized usage data and
the usage metrics we calculate.
...
anonymized log, which Teleport aggregates over the cycle.
...
calculate these types of usage metrics:
...
1. Click on "Usage Reporting".
1. Usage data for the current cycle will be displayed. Example:

![Cycle](../img/webui_usage_cycle.png)
...
A "resource" is any distinct computing resource, including a Kubernetes cluster,
SSH server, Windows or Linux desktop, database instance, or serverless endpoint,
that registers with the Teleport cluster at least once a month.
...
IG MAU is calculated separately from standard MAU and represents users specifically engaging with Teleport's Identity Governance features. A user can contribute to both standard MAU and IG MAU usage metrics within the same period if they perform activities that qualify for both metrics.

## Usage measurements

We aggregate all counts of the usage metrics on a monthly basis starting on
...
aggregate amount of a usage metric in a given month during the term of the

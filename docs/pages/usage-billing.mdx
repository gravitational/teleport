...
[ephemeral](reference/user-types.mdx#temporary-users). Teleport deletes an SSO
...
includes [how-to
guides](./admin-guides/management/export-audit-events/export-audit-events.mdx)
for exporting audit events to common log management solutions so you can
identify users that have authenticated using an SSO provider.

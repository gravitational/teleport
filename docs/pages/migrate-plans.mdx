...
[self-hosted](installation/self-hosted/self-hosted.mdx) Teleport Enterprise cluster
...
1. If you are migrating to a self-hosted Teleport Enterprise account, plan and
   ...
   assist with this, read the documentation on [Self-Hosting
   Teleport](installation/self-hosted/self-hosted.mdx).
...
To migrate services such as Teleport Agents, Machine & Workload Identity Bots,
and plugins, start by cataloging the various services you're managing with
Teleport. The following resources should be considered for migration:

 - Teleport Agents 
 - Machine & Workload Identity Bots
...
1. For each Agent and Machine & Workload Identity Bot, obtain a valid join
   token. We recommend using a [delegated join
...
### Migrate Machine & Workload Identity Bots

In general, you can migrate a Machine & Workload Identity Bot using the
following steps:
...
To learn how to restart and configure a Bot for your infrastructure, read the
...
1. If you are using Machine & Workload Identity to produce credentials for the
   plugin, reconfigure the Bot to connect to the new Teleport cluster and
   restart the Bot.
...
Enterprise](installation/self-hosted/self-hosted.mdx).

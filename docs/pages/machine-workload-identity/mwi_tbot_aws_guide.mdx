...
Teleport Workload Identity and Machine ID eliminate the need for long-lived static credentials like SSH keys, API tokens, or cloud access keys. 
Instead, they provide short-lived, automatically rotated identities tied to the workloads and machines that use them.
...
- An AWS EC2 instance with the IAM role attached that you'll install `tbot` onto.

## Step 1/8. Install `tbot`
...
## Step 5/8. Configure Workload Identity
...
## Step 6/8. Expose a Workload API endpoint
...
## Step 7/9. Testing the Workload API with `tbot spiffe-inspect`
...
## Step 8/8. Configuring your workload to use the Workload API
...
- Follow one of the [access guides](../machine-workload-identity/machine-id/access-guides/access-guides.mdx) to configure `tbot` so that meets your access needs.
- Read the [configuration reference](../reference/machine-workload-identity/machine-id/configuration.mdx) to explore all available configuration options.

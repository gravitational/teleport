...
Choose one of the following sources for the JWT that `tbot` uses to join:

- An environment variable that contains a JWT (`env`).
- A command that fetches a JWT and writes it to stdout (`command`).
- An HTTP endpoint that returns a JWT, either directly or in a JSON response (`http_request`).

...
In this guide, you will deploy Machine & Workload Identity's agent, `tbot`, on a
Linux host, and use the secure identity of the onboard TPM 2.0 chip for
...
can add additional rules to the `allow` list, one for each host.

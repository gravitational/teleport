...
The bot must have at least one role assigned before running an identity or
output service. You'll create a role and assign it to the bot when you follow
the access guide for your use case (see Next steps), using a command similar
to `tctl bots update example --add-roles <role-name>`.

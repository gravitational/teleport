---
title: Machine & Workload Identity Troubleshooting Guide
description: Troubleshooting common issues with Machine & Workload Identity
...
This page describes common issues that you may come across when setting up
Machine & Workload Identity (MWI) and how to work around or resolve them.
...
```text
[AUTH]      WARN lock targeting BotInstanceID:"0fa4d626-1837-4aee-a5d0-79856d4223ab" is in force: The bot instance example/0fa4d626-1837-4aee-a5d0-79856d4223ab has been locked due to a certificate generation mismatch, possibly indicating a stolen certificate. auth/apiserver.go:224
```
...
Machine & Workload Identity (with token-based joining) uses a certificate
generation counter to detect potentially stolen renewable certificates. Each
time a bot fetches a new renewable certificate, the Auth Service increments the
counter, stores it on the backend, and embeds a copy of the counter in the
certificate.

If the counter embedded in your bot certificate doesn't match the counter
stored in Teleport's Auth Service, the renewal will fail and the bot instance
will be automatically [locked](../identity-governance/locking.mdx).
...
locked bot:
 1. Remove the lock on the bot instance
 1. Reset the bot's generation counter by creating a new bot instance

To remove the lock, first find and remove the lock targeting the bot instance.
For this example, we'll assume the bot is named `example`:
...
spec:
  message: The bot instance example/0fa4d626-1837-4aee-a5d0-79856d4223ab has been locked
    due to a certificate generation mismatch, possibly indicating a stolen certificate.
  target:
    bot_instance_id: 0fa4d626-1837-4aee-a5d0-79856d4223ab
version: v2
...
when impersonated certificates for the matching services were renewed.
...
of the following actions:
 ...
 - Modifying the `access` role to allow the `bob` principal
...
Unlike regular Teleport users, Machine & Workload Identity bot users are granted
only minimal Teleport [RBAC permissions](../reference/access-controls/roles.mdx) and are not
...
Per the [Machine & Workload Identity Database Access Guide](./access-guides/databases.mdx), ensure at
least one role providing database permissions has been granted to the
output service listed in the error.

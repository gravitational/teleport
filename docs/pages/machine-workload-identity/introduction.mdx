...
Establish a root certificate authority within your Teleport cluster that issues short-lived JWTs and X509 certificates to workloads. These identities ([SPIFFE](../machine-workload-identity/spiffe.mdx) Verifiable Identity Documents or SVIDs) contain the workload's identity encoded as a URI (SPIFFE ID).
...
A [Zero Trust strategy](../machine-workload-identity/getting-started.mdx) is applied across workloads and automation:
...
- [Workload Identity](../machine-workload-identity/use-cases/spiffe-workload-api.mdx): Information on Teleport Workload

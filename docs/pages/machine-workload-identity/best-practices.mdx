...
### Using the `spiffe-svid` output service
...
To configure the `spiffe-svid` output service type follow the instructions in
[Getting Started with Workload Identity](../machine-workload-identity/use-cases/spiffe-workload-api.mdx).

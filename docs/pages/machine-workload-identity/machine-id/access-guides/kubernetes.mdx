...
Now, `tbot` needs to be configured with an output service to produce the
Kubernetes credentials and client configuration file. This is done using the
`kubernetes/v2` service type.
...
Output services must also be configured with a destination. In this example, the

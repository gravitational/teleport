...
node. This node could be a bare-metal machine, a VM, a container, or any other
host - the only requirement is that the node has persistent storage.
...
Replace:
- `example` in `spec.bot_name` with the name of the bot you created in the
  second step

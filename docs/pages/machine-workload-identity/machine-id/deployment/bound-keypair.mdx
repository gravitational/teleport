...
host using Bound Keypair Joining. This host could be a bare-metal machine, a
VM, a container, or any other host - the only requirement is that the host has
persistent storage.
...
With Bound Keypair Joining, Machine ID bots generate a unique keypair which is
persistently stored in their internal data directory. Teleport is
...
<Admonition type="tip" title="Onboarding and Recovery">
To learn more about the various onboarding and recovery options, refer to the

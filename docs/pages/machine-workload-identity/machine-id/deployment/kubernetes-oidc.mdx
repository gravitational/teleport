...
https://oidc.eks.us-west-2.amazonaws.com/id/$myClusterID/.well-known/openid-configuration
```
...
Teleport:
```code
$ curl https://oidc.eks.us-west-2.amazonaws.com/id/$myClusterID/.well-known/openid-configuration | jq
{
  "issuer": "https://oidc.eks.us-west-2.amazonaws.com/id/$myClusterID",
  "jwks_uri": "https://oidc.eks.us-west-2.amazonaws.com/id/$myClusterID/keys",
...
```code
$ curl https://oidc.eks.us-west-2.amazonaws.com/id/$myClusterID/keys
{"keys":[{...snip...}]}
```

There's no need to record anything from this response - we just need to be
...
- [Amazon EKS](https://docs.aws.amazon.com/eks/latest/userguide/enable-iam-roles-for-service-accounts.html)
- [Azure AKS](https://learn.microsoft.com/en-us/azure/aks/use-oidc-issuer)
...
Create `bot-token.yaml`, ensuring you insert the value from the OIDC issuer
in `spec.kubernetes.oidc.issuer`:

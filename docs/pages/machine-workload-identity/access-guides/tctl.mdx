---
title: Machine & Workload Identity with tctl
sidebar_label: tctl
description: How to use Machine & Workload Identity with tctl to manage your Teleport configuration
tags:
 - how-to
 - mwi
...
Now, `tbot` needs to be configured with a service that will produce the
credentials needed by `tctl`. As `tctl` will be accessing the Teleport API, the
correct service type to use is `identity`.
...
Alternatively, pass `--config=/dev/null` to prevent `tctl` from reading the
local Teleport configuration file:

```code
$ tctl --config=/dev/null --auth-server example.teleport.sh:443 -i /opt/machine-id/identity create -f roles/*.yaml
```

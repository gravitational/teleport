...
- A GitHub repository where you have permissions to create GitHub Actions workflows.
  ...
  - Self-hosted
    - Your Teleport Auth Server must be able to reach your GitHub Enterprise instance.
...
methods](../reference/deployment/join-methods.mdx#delegated-join-methods)
specific to the platform the bot is running on. Since we are using GitHub
Actions, we will use the `github` join method.
...
spec:
  ...
  github:
    allow:
    - repository: "your-github-username/my-repo" # if you cloned the example repo, if not, use your repo name

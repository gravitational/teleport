...
<iframe width="560" height="315"
  src="https://www.youtube.com/embed/YzVK2tr6u-U?si=Oc4jGs4abodZiRiC"
...
supporting everything from [Infrastructure-as-Code](../machine-workload-identity/iac-mwi.mdx) workflows to [AI agent](../machine-workload-identity/ai-agents-mwi.mdx)
...
can reference the dedicated use case pages for tool-specific guidance.
...
![Example ssh server resource with label](../../img/machine-id/ssh-target-example.png)
...
![Example kubernetes cluster resource with label](../../img/machine-id/k8s-target-example.png)
...
-------- ----------------------------- -------
crumpet  env=dog-food,region=us-west-2  18.1.5
```
...
Teleport supports a number of secure [join methods](../reference/join-methods.mdx#delegated-join-methods)
...
We have several published [Actions](../reference/machine-id/github-actions.mdx#github-actions-helpers)
...
```yaml
version: v2
proxy_server: mycluster.cloud.gravitational.io:443
onboarding:
  join_method: github
  token: github-bot
certificate_ttl: 5m
...
jobs:
  check_resource_usage:
    ...
    - name: Fetch Teleport binaries
      ...
      with:
        proxy: mycluster.cloud.gravitational.io:443
        version: auto
    - name: Export ssh config
      run: tbot start --oneshot -c ./action_support/server_bot.yaml
    - name: Run mpstat
      run: |
        ssh -F ./ssh_out/ssh_config ubuntu@myinstance.mycluster.cloud.gravitational.io mpstat
```

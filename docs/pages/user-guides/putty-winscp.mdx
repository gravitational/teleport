...
[`teleport join openssh`](../admin-guides/enroll-resources/server-access/openssh/openssh.mdx)), you must specify the `sshd` port
...
For more information about Teleport authentication, see [Authentication options](../reference/operations/authentication.mdx).

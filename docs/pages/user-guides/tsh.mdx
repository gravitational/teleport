...
A Teleport cluster can be configured for multiple user identity sources. For example, a cluster may have a local user called `admin` while regular users should [authenticate via GitHub](../admin-guides/authentication/sso/github-sso.mdx). In this case, you have to pass `--auth` flag to `tsh login` to specify which identity storage to use:
...
[Machine ID](../admin-guides/enroll-resources/machine-id/introduction.mdx). This ensures that your automation
...
>
  ...
  Refer them to the [Moderated Sessions guide](../admin-guides/access-controls/moderated-sessions.mdx) for more information on configuring join permissions.
</Admonition>
...
[Configure Trusted Clusters](../admin-guides/access-controls/trustedclusters.mdx).
...
storage](../admin-guides/common-operations/external-audit-storage.mdx).
...
[Uninstalling Teleport](../admin-guides/common-operations/uninstall-teleport.mdx).

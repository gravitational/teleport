...
When using [trusted clusters](../admin-guides/access-controls/trustedclusters.mdx), the cluster selector allows
...
When it comes to [cloud APIs secured with Application Access](../index.mdx),
...
(!docs/pages/includes/connect-your-client/connect-my-computer-prerequisites.mdx!)
...
- Permissions to read and update user objects in the backend (verbs `read` and `update` for [the
  `user` resource](../reference/rbac/teleport-roles.mdx)).
- Permissions to read, update, and create roles in the backend (verbs `read`, `update`, and `create`
  for [the `role` resource](../reference/rbac/teleport-roles.mdx)).

The agent runs as the current system user, not as root. Some features are thus not available, such
as logging in as other system users or [host user creation](../admin-guides/enroll-resources/server-access/guides/host-user-creation.mdx).
...
During the setup, Connect My Computer creates [a join token](../admin-guides/enroll-resources/agents/join-services-to-your-cluster/join-token.mdx)
...
Controls Reference Documentation](../reference/rbac/teleport-roles.mdx) for more
...
locks](../admin-guides/access-controls/locking.mdx) on those nodes.
...
(!docs/pages/includes/connect-your-client/launch-connect-with-flags-macos.mdx flags="--connect-debug"!)
</TabItem>
<TabItem label="Linux">
...
certificates](../admin-guides/self-hosting/demo-clusters/self-signed-certs.mdx) or for demo purposes. We do not recommend
...
(!docs/pages/includes/connect-your-client/launch-connect-with-flags-macos.mdx flags="--insecure"!)

...
- **Restricted Cipher Suite Selection**
  ...
  algorithms that are approved under the FIPS certification program and
  supported by BoringCrypto. Due to the rigorous and lengthy certification
  process, FIPS-approved algorithms often lag behind the state of the art.
  As a result, the FIPS build lacks support for newer algorithms with improved
  performance or forward-looking security properties (e.g. ChaCha20-Poly1305, Ed25519).
...
- **Operational Complexity**
  ...
  troubleshooting connection issues where peers expect different cipher
  support.

- **No Security Benefit Without Compliance Need**

  Outside of regulated environments, using the FIPS binary does not increase
  Teleport’s security posture. In fact, it may result in a net negative effect
  by limiting use of modern cryptographic algorithms.

...
- A Linux host running Red Hat Enterprise Linux (RHEL) 8 or 9. The Teleport SSH
...
- Root or `sudo` access on the host. The `install-selinux.sh` script must be run
  as `root`.
- SELinux installed and enabled on the host, in either permissive or enforcing
  mode. Teleport exits with an error if SELinux is disabled. Check the current
  mode with `getenforce` or `sestatus`. Enforcing mode is required for production
  use.
- The `selinux-policy-devel` package installed on the host, which provides the
  toolchain used to compile the module. The [Installation](#installation)
  section below covers this.
- A Teleport SSH Service agent installed from a Teleport tarball and configured
  to point at your cluster. Follow
  [SSH getting started](../../../enroll-resources/server-access/getting-started.mdx)
  to enroll the host. The `install-selinux.sh` script and SELinux module ship
  only in the Linux tarball, not in package-manager or container installs, and
  the agent must run only the SSH Service.
- The Teleport SSH Service agent must run as a **systemd service**. Teleport only
  runs under the correct `teleport_ssh_t` SELinux domain (and is therefore only
  enforced) when started by systemd, not when launched manually.

...
- Root or `sudo` access on the host. The `install-selinux.sh` script must be run
  as `root`.
- SELinux enabled on the host in permissive or enforcing mode (not disabled). See
  [SELinux modes](#selinux-modes) for the difference between the two.
...
- A Teleport SSH Service agent configured to point at your cluster and enrolled by
  ...
  - **Runs as a systemd service.** Teleport only runs under the correct
    `teleport_ssh_t` SELinux domain — and is therefore only enforced — when started
    by systemd, not when launched manually.

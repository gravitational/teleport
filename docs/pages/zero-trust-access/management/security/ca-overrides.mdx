...
Teleport stores CA overrides in the `cert_authority_override` resource.
...
- Some commands that interact with CAs are not yet aware of overrides.
  For example:
  - `tctl get cas/windows` still returns the self-signed CA certificate.
    Use `tctl get ca_overrides/windows` to get override data.
  - CA rotation does not account for active overrides. Administrators must track
    active overrides and manually create new overrides at the appropriate steps
    during rotation.
</Admonition>
...
externally signed one, while keeping the private key managed by Teleport.
...
## Supported CAs and downstream trust
...
## Step 2. Issue override certificates using your external CA
...
See the [Supported CAs](#supported-cas-and-downstream-trust) section for notes on each CA.

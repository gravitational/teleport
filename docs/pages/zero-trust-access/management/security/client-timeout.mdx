...
sidebar_label: Client Timeout Enforcement
...
Obtain your existing `cluster_networking_config` resource:

```code
$ tctl get cluster_networking_config > cnc.yaml
```

Include `client_idle_timeout` in `cnc.yaml`:
...
Create the `cluster_networking_config` resource via `tctl`:

```code
$ tctl create -f cnc.yaml
```

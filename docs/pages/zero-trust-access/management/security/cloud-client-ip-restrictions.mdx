...
- One of the following client tools for managing the `client_ip_restriction`
  resource:
  - The `tctl` CLI.
  - The [Teleport Terraform provider](../../../configuration/terraform-provider/terraform-provider.mdx).
...
Assign the role to a user. Replace `alice` with the Teleport username and
include any roles that user already needs:

```code
$ tctl users update --set-roles editor,client-ip-restriction-editor alice
```

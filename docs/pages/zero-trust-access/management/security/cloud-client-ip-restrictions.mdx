...
- `expires` is an optional [RFC 3339](https://www.rfc-editor.org/rfc/rfc3339)
  timestamp that bounds enforcement. Setting it starts a *test run*: the
  allowlist is enforced until this time and not after it. Use it as a safety net
  so that an allowlist that locks you out stops being enforced on its own instead
  of requiring a support ticket. An unset value never expires.

When `expires` passes, Teleport Cloud stops enforcing the allowlist and reports
`status.state` as `expired`. It does not rewrite `spec`: `mode` still reads
`enforced` and `expires` still holds the time you set, so enforcing again always
takes a new write.
...
that a mistake stops enforcing itself instead of becoming a permanent lockout
that requires a support ticket.
...
wrong and locks you out, you do not need to do anything: enforcement stops when
the test run expires, the cluster is reachable from any IP again, and your
allowlist is still saved so you can correct it.
...
In the IP Allowlist panel, enter your CIDR blocks and save. A newly saved
allowlist is stored as a draft, and the status banner reads "Draft".
...
[RFC 3339](https://www.rfc-editor.org/rfc/rfc3339) format and must be at least 20
...
Cloud enforces the allowlist, but if it locks you out, enforcement stops at the
expiry time, `status.state` becomes `expired`, and access is restored.
...
In the IP Allowlist panel, select "Test Run". This enforces the draft for 30
minutes and shows a countdown while the test run is active.
...
removing `expires` (or extending it) and reapplying. In the Web UI, "Confirm"
does the same thing. With `expires` unset, enforcement never lapses:
...
its own when the test run expires. See
[Test and roll out the allowlist safely](#test-and-roll-out-the-allowlist-safely).

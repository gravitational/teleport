...
In the event a rollback is required in response to a failed upgrade to v19, then the cluster
state should also be restored to prevent any further issues. Teleport major releases may alter the
backend state in a way that previous major versions cannot reconcile. Since the clone was
already taken this can be achieved by altering the teleport.yaml of the Auth service to point at the prefix
of the backup before it is restarted with v18.

...
verification](../../../reference/machine-workload-identity/machine-id/bound-keypair/concepts.mdx#join-state-verification).
...
Machine ID bots, if active, should detect the rotation event and renew their
certificates, which will automatically fetch a new join state JWT signed with
the new CA. Bots that are not currently running will need to be started once
and allowed to fetch new certificates, which will also refresh the join state
JWT.

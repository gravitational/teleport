...
This step will config and start a [Teleport Machine ID bot](../../../machine-workload-identity/deployment/deployment.mdx) as a background daemon,
which will continually generate and refresh a short-lived key and certificate for break-glass access on the machine where it runs. The configuration
below generates a certificate which is valid for 24 hours from the time of issuance, refreshed every 2 hours. This means that depending on the exact
time of usage, the certificate will be valid for break-glass access for between 22 and 24 hours.

...
| Is the cluster aware of this agent right now? | `tctl inventory ls` |
...
that disappear from the inventory. `tctl inventory status --connected` is
not suitable for this check in HA deployments, because each Auth Service
instance only sees the agents directly connected to it, and agents connected
to other Auth instances would intermittently appear missing.

...
runs `tctl inventory status --connected` against the control plane and
alerts on agents that disappear from the connected list. This catches both
local-process failures (caught by the liveness probe) and cluster-side
disconnects (caught by the inventory check) that the probes alone would miss.

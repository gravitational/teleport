...
- Confirm PVCs are bound: 
  ...
  ```code
  $ kubectl get pvc -n teleport-cluster
  ```
...
- Verify the namespace exists: 
  ...
  ```code
  $ kubectl get namespace teleport-cluster
  ```
...
>
  ...
  - Get the load balancer address: 
    ...
    ```code 
    $ kubectl get svc teleport-cluster-proxy -o jsonpath='{.status.loadBalancer.ingress[0]}'
    ```
  ...
  - If TLS fails, check Proxy Service logs: 
  
    ```code 
    $ kubectl logs deployment/teleport-cluster-proxy
    ```

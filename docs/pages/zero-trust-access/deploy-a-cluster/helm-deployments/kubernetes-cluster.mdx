...
>
  ...
  - If pods are in `Pending` state, check for persistent volume issues: `kubectl describe pod <pod-name>`.
  - Confirm PVCs are bound: `kubectl get pvc -n teleport-cluster`.
  - If pods are in `CrashLoopBackOff`, check logs: `kubectl logs deployment/teleport-cluster-auth`.
  - Verify the namespace exists: `kubectl get namespace teleport-cluster`.
  - Check events for errors: `kubectl get events --sort-by='.metadata.creationTimestamp'`.
  - Ensure your cluster has a default StorageClass: `kubectl get storageclasses`.
</Checkpoint>
...
>
  Here are some troubleshooting tips:
  - Verify DNS resolution: `nslookup teleport.example.com` (using your cluster name) should resolve to your load balancer IP.
  - Get the load balancer address: `kubectl get svc teleport-cluster-proxy -o jsonpath='{.status.loadBalancer.ingress[0]}'`.
  ...
  - If TLS fails, check Proxy Service logs: `kubectl logs deployment/teleport-cluster-proxy`.
  - DNS propagation can take time. Wait a few minutes and try again.
  - Verify ACME is enabled in your values file with `acme: true`.
</Checkpoint>
...
>
  Here are some troubleshooting tips:
  - If `tsh login` fails, verify DNS is resolving correctly and TLS certificates are valid.
  - If `tsh kube ls` shows no clusters, ensure the `member` role was created successfully.
  - If `kubectl` commands fail after `tsh kube login`, check that KUBECONFIG is set correctly.
  - Verify your user has the correct roles: `tctl get users/<username>`.
  - Check Auth Service logs for authentication errors: `kubectl logs deployment/teleport-cluster-auth`.

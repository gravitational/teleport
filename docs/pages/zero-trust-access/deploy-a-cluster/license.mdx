...
If the Teleport service can’t write the new license to disk due to permissions or 
the environment setup, Teleport will maintain an up-to-date record of your
organization's subscription. This means that new features will 
be enabled in the cluster even though the license file is not updated. The next 
time the service starts, the old license features will be available, but 
shortly after it will update again. You will observe warnings in the logs, but 

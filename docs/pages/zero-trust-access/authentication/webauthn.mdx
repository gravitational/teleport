...
<Admonition type="warning" title="Machine & Workload Identity and Device Trust">

  We do not currently support Machine & Workload Identity and Device Trust.
  Requiring Device Trust cluster-wide or for roles impersonated by Machine &
  Workload Identity will prevent credentials produced by Machine & Workload
  Identity from being used to connect to resources.

  As a work-around, configure Device Trust enforcement on a role-by-role basis and
  ensure that it is not required for roles that you will impersonate using
...
- [Configuring SSO for MFA checks](../sso/sso-for-mfa.mdx): Delegate per-session MFA to your IdP. 

...
- Authenticating with [WebAuthn](../management/security/idp-compromise.mdx#set-up-cluster-wide-webauthn) or [SSO MFA](../sso/sso-for-mfa.mdx) from a remote dev box
...
- A Teleport cluster with [WebAuthn](../management/security/idp-compromise.mdx#set-up-cluster-wide-webauthn) or [SSO MFA](../sso/sso-for-mfa.mdx) configured.
- Machines for Headless Authentication activities have [Linux](../../installation/single-machine/single-machine.mdx), [macOS](../../installation/single-machine/single-machine.mdx) or [Windows](../../installation/single-machine/single-machine.mdx) `tsh` binary installed.
...
--------------------- ----------------- --------------------------------
teleport-cluster      example-cluster   teleport-cluster-example-cluster
...
When per-session MFA is required, each Kubernetes cluster needs its own MFA check
under headless authentication. A `tsh proxy kube --headless` run across several
clusters performs one check per cluster. A run without `--headless` performs a
single check that covers all of them, as long as both `tsh` and the Teleport
cluster are running Teleport (=kubernetes.multi_cluster_mfa_min_version=) or
later. See
[Per-session MFA](./per-session-mfa.mdx#per-session-mfa-for-kubernetes-access)
for a full explanation of per-session MFA for Kubernetes sessions.

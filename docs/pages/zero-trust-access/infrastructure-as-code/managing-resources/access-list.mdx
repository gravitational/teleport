...
nested Access List. The name of the memeber is the ID of another Access List
...
apps](../../../identity-governance/okta/app-and-group-sync.mdx) as Teleport Access Lists, while the
Microsoft Entra ID integration allows [synchronizing
groups](../../../identity-governance/entra-id/entra-id.mdx) as Teleport Access Lists. 
...
[Okta](../../../identity-governance/okta/app-and-group-sync.mdx) or [Microsoft Entra ID](../../../identity-governance/entra-id/entra-id.mdx)).
An existing static list created by another Terraform setup (with a different
state) could be imported, and its members can be managed by Terraform, but

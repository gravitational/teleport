...
This guide relies on the OIDC variant of [the Kubernetes join method](./TODO-LINK).
...
- a working Teleport cluster v18.1.X
- a kubernetes cluster you want to enroll
- the kube cluster has an OIDC provider
...
Note: We recommend naming the token after the kubernetes cluster it's trusting, this will allow you to enroll other
clusters without facing token name conflicts.
...
Finally deploy the Teleport Kube agent using the values we previously set in the provision token:
...
Validate the agent successfully joined the cluster by checking its pod readiness

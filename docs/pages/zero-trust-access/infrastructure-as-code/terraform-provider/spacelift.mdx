...
authenticate to a Teleport cluster using Machine & Workload Identity when
running on Spacelift.
...
First, we'll create a Machine & Workload Identity Bot for our Spacelift job to
act as. We'll grant it the `terraform-provider` role, which automatically grants
access to every resource supported by the Teleport terraform provider.
...
authenticate to the Teleport cluster.
...
- To find out more about Spacelift's OIDC implementation, which Machine &
  ...
  [the Spacelift documentation](https://docs.spacelift.io/integrations/cloud-providers/oidc/).

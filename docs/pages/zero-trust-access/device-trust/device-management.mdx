...
This guide describes the commands you can run in order to perform common Device
Trust management operations, such as registering a trusted device, creating a
device enrollment token, and removing a trusted device.
...
<Admonition type="tip" title="Sync from an MDM">
Teleport supports device synchronization through [MDM integrations](device-trust.mdx#mdm-integrations).
Once configured, devices are automatically updated in Teleport's device inventory.
</Admonition>
...
[manual](#register-a-trusted-device) or performed using [an MDM
integration](./device-trust.mdx#mdm-integrations).

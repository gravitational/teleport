...
- Scoped WorkloadIdentity resources for issuing SPIFFE credentials to
...
A "materialized" scoped role assignment is an one that is automatically created
...
SubKind      ID                                                          User    Assigns                        
------------ ----------------------------------------------------------- ------- --------------------------------------------------
materialized /examples/basic::acl-4ezohtYllrk-gbWjn0hj_npCNwhujMurruDLRg charlie /examples/basic::example-access -> /examples/basic
materialized /examples/basic::acl-XMyMVGoNZopPQ9WxuZ5RH6GM5FPHVwLSTFUsBg bob     /examples/basic::example-access -> /examples/basic
```
...
set, the WorkloadIdentity is considered scoped. Scoped WorkloadIdentities
...
SPIFFE credentials to workloads.

A scoped WorkloadIdentity must specify a SPIFFE ID that encodes the scope
in which it is defined. The path of the SPIFFE ID consists of three
sections:

- The scope section: segments that match the scope in which the
  WorkloadIdentity is defined.
- The separator segment: `/_/`.
- The administratively-defined section: one or more segments freely chosen
  by the administrator.

For example, a WorkloadIdentity defined in `/staging` may specify the
SPIFFE ID `/staging/_/payments/api`, which is issued as
`spiffe://example.teleport.sh/staging/_/payments/api`.

The following rules apply to the SPIFFE ID of a scoped WorkloadIdentity:

- The scope section must exactly match the scope in which the
  WorkloadIdentity is defined. It cannot be an ancestor or descendant of
  that scope. This means the scope responsible for a SPIFFE ID can always
  be determined from the ID alone, and admins of one scope cannot issue
  SPIFFE IDs that appear to belong to another scope.
...
Additionally, the following rules apply to the resource itself:

- A scoped WorkloadIdentity cannot be defined in the root scope `/`.
- The scope of a WorkloadIdentity cannot be changed by an update. To move a
  WorkloadIdentity between scopes, or to convert between scoped and
  unscoped, delete and re-create the resource.

Issuance is controlled by the `workload_identity` section of a scoped role,
which selects the WorkloadIdentities that the role permits issuance with by
label. Issuance additionally requires the `readnosecrets` verb (and, when
issuing by label selector, `list`) for the `workload_identity` resource in
the role's `rules`.

In scoped mode, `tbot` supports the `workload-identity-x509`,
`workload-identity-jwt`, and `workload-identity-api` services. When
selecting a scoped WorkloadIdentity by name, use its scope-qualified name,
...
scoped Bot `my-scoped-bot` the ability to issue SPIFFE credentials. It
assumes the scope administrator's role also grants write access to the
`workload_identity` resource.

As the scope administrator, create a scoped WorkloadIdentity:
...
Create a scoped role permitting issuance with WorkloadIdentities labelled
...
previous example, selecting the WorkloadIdentity by its scope-qualified
name:
...
with the SPIFFE ID `spiffe://example.teleport.sh/staging/_/payments/api`.

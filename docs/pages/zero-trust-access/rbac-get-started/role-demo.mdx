...
1. Copy the following Teleport configuration to a file called `local-dev.yaml`:
   ...
   to the server called `env:local-dev`.
...
1. Start a Docker container on your workstation to prepare a server that you
   want to enroll as a resource in your Teleport cluster:
...
In this step, you will create a local Teleport user with the `local-dev-only`

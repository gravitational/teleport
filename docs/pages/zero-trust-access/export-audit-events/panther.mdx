...
cluster. It authenticates to your Teleport cluster and Fluentd instance using mutual
TLS. In this section, you will set up the Event Handler plugin 
...
## Step 3/5. Create a Dockerfile with Fluentd and the S3 plugin
...
Update the following fields. If you are using the `teleport-plugin-event-handler`
Helm chart with Teleport Kubernetes Operator enabled, you can ignore the `teleport`
section, as we replaced it with the `crd` and `tbot` sections to connect to Teleport.
...
(!docs/pages/includes/plugins/fluentd-config.mdx!)
...
## Step 5/5. Configure Panther to ingest logs from S3

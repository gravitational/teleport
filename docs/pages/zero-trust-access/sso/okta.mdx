...
integration](../../identity-governance/okta/okta.mdx).
...
Then follow the instructions for the [guided Okta single sign-on integration](../../identity-governance/okta/guided-sso.mdx).
...
Teleport, you can skip to the [next step](#step-12-configure-okta).
...
7. Now you should be in your newly created SAML app configuration screen in the **Sign On** tab. Scroll down to the **Attribute statements** and click **Add expression**. Provide values like below and click **Save**.
   ...
   {/* NOTE: This image is shared in multiple guides. */}
   ![Set groups attribute](../../../img/sso/okta/legacy-group-expression.png)

Notice that we have set "NameID" to the email format and mapped the groups with
an expression returning all groups. We have also set the "Audience"
...
## Step 2/2. Connect Okta to Teleport

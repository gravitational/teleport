---
title: Teleport Authentication with Microsoft Entra ID (formerly Azure AD)
description: How to configure Teleport access with Microsoft Entra ID.
tags:
 - how-to
 - zero-trust
---
...
- Only members of the "DBA" Entra ID group can connect to PostgreSQL databases.
...
- An Entra ID admin account with access to creating non-gallery applications
  (P2 License).
- To register one or more users in the directory.
- To create at least two security groups in Entra ID and assign one or more
  users to each group.
...
Entra ID connector to determine which Linux logins to allow on a host.
...
is passed in from the Entra ID connector using the `windowsaccountname`
attribute.

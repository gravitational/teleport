...
Any claims that is present in the OIDC ID Token is preserved as user traits. 
...
<Admonition type="info" title="New Group">
  You may skip this step if you are using an existing Microsoft Entra ID 
...
Teleport uses OIDC authorization code flow to exchange OIDC authorization code 
for the user's ID Token. To exchange the ID Token, a client credential must be 
set up for Teleport. 
...
If user's group membership exceeds 200 groups limit, Microsoft Entra ID issues a
...
<details>
<summary>Example Auth Connector resource file created by the `tctl sso configure` command</summary>
...
User may receive this error if a groups overage claim was issued for the user but 

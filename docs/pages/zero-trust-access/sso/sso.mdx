...
- [Moderated sessions](../access-controls/guides/joining-sessions.mdx)
- [Admin actions](../access-controls/guides/mfa-for-admin-actions.mdx)
...
In order to consume these JSON claims, an administrator must create a [login rule](../../admin-guides/access-controls/login-rules/login-rules.mdx)
with a [JSONPath](https://support.smartbear.com/alertsite/docs/monitors/api/endpoint/jsonpath.html)
expression.

For example, imagine a custom distributed IdP that aggregates claims for a user
from multiple different provider sources. The claims set might look like a JSON object of the following format:

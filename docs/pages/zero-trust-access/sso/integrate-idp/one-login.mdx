...
1. Obtain SAML metadata for your authentication connector. Once the application
   is set up, navigate to the **More Actions** menu and find the **SAML
   Metadata** option:
...
## Step 2/3. Connect OneLogin to Teleport

In this section, you will create an authentication connector that provides
Teleport with the information it needs to exchange SAML messages with OneLogin
and issue certificates to users.
...
--attributes-to-roles <Var name="mapping_2" /> > onelogin.yaml
```

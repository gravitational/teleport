...
## Step 2/3. Connect Keycloak to Teleport

In this section, you will create an authentication connector that provides
Teleport with the information it needs to exchange SAML messages with KeyCloak
...
If you define a role mapping using a Keycloak group, you must include an
explicit leading `/` in the group name.
...
--audience https://example.teleport.sh/v1/webapi/saml/acs/keycloak \
--acs https://example.teleport.sh/v1/webapi/saml/acs/keycloak > keycloak-connector.yaml
```
...
metadata:
  name: keycloak
spec:
  acs: https://example.teleport.sh/v1/webapi/saml/acs/keycloak
  attributes_to_roles:
  - name: groups
    roles:
    - editor
  ...
  issuer: ""
  service_provider_issuer: https://example.teleport.sh/v1/webapi/saml/acs/keycloak
  sso: ""
version: v2
```

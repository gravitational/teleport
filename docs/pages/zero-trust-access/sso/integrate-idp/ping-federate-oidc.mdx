...
```
https://<pingfed-hostname>/pf/.well-known/openid-configuration
```

A common issuer format is:

```
https://<pingfed-hostname>/as
```
...
Create a file named `pingfed-connector.yaml` with the following content:
...
```bash
tctl create -f pingfed-connector.yaml
```

...
h1: Migrating from Self-Hosted to Teleport Enterprise Cloud
---
...
- Verify tsh and tctl access to the newly created Teleport Cloud tenant by using the tsh command-line tool to log in to your new Teleport Cloud cluster:


# Login to your Teleport Cloud cluster.
$ tsh login --proxy=<example.teleport.sh> --user=<your-admin-user>

# Run "tsh status" and "tctl status" to verify you've logged into your cluster.
$ tsh status && tctl status

2. ##Migrate Teleport Configurations.
...
The actionable steps required to perform the migration are listed below:
...
For example, to list all nodes registered with the Teleport cluster, you would use:
`tctl nodes ls`

Similarly, to list all registered Kubernetes clusters, you can use:

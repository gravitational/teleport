...
In this guide we'll be using Moderated Sessions to required participants for a Kubernetes session.
...
Require and allow policies have to specify which sessions they apply to. Currently, only `ssh` and `k8s`

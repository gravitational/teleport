...
1. Authenticate to your Teleport cluster. This depends on whether your shell is
   ...
   **On non-interactive environments:** If you are running `tsh` and `tctl` as
   ...
   ```code
   $ tctl status --auth-server=<Var name="teleport.example.com" />
   ```
   ...
   Export the `TELEPORT_IDENTITY_FILE` environment variable and add the
   `--auth-server` or `--proxy` flags to all subsequent `tctl` and `tsh`
   commands.

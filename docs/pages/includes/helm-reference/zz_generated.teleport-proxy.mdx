...
`auth_server` the address this proxy should use to contact the Auth
service. This should be
be an <IP>:<PORT> for an internal network path to the auth service. Port
defaults to 3025 if not specified.
...
`licenseSecretName` overrides name for the Kubernetes secret that stores the Enterprise license
...
### `extraLabels.joinTokenSecret`

| Type | Default |
|------|---------|
| `object` | `{}` |

`extraLabels.joinTokenSecret` labels to add to the join token secret

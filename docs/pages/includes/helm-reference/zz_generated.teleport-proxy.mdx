...
service ([clusterName in the teleport-cluster chart](../../reference/helm-reference/teleport-cluster.mdx#clustername)
/ [auth_service.cluster_name in the Auth Service config](../../reference/deployment/config.mdx#auth-service))
...
> to your geography-aware DNS service. We do not recommend using a load balancer
> with an Anycast VIP because Teleport clients maintain long-live persistent
> connections that may be disrupted during Anycast routing changes.
...
See [Join Methods](../../reference/deployment/join-methods.mdx)
...
`joinParams.azure` Azure config params. See [Joining Services via Azure](../../installation/agents/azure.mdx)
...
`serviceAccountTokenAudience` should ALMOST ALWAYS be left empty to
let the chart insert the correct value.

optionally overrides the audience on the projected Kubernetes service account
token mounted into proxy pods.
...
`serviceAccountTokenExpirationSeconds` should ALMOST ALWAYS be omitted
to let the chart insert the correct value.

seconds until expiry on the projected Kubernetes service account token mounted
into proxy pods. This must be sufficiently long to allow the proxy to join the
cluster.

When omitted the effective Teleport join method is `kubernetes`, the chart
defaults this to 600 seconds.
...
See the [Teleport Configuration Reference](../../reference/deployment/config.mdx)
...
> (`proxy_service.enabled` in [teleport.yaml](../../reference/deployment/config.mdx#proxy-service),
> as deployed by this chart), does not require the license file even when
> running an enterprise build. Your license file only needs to be deployed on
> the auth service with the
> [teleport-cluster chart](../../reference/helm-reference/teleport-cluster.mdx).
...
- "scratch": Generate a minimal config file. Only values derived from the
   mandatory helm values are inserted into the config. Use teleportConfig to
   supply additional configuration.
...
> [!WARNING]
> You **must** meet the replication criteria, else the deployment will
> result in errors and inconsistent data.
...
> [!DANGER]
> Setting CPU limits is an anti-pattern and harmful in most cases.

...
`updater` controls whether the Kube Updater should be deployed alongside.
The updater fetches the target version, validates the image signature, and updates the teleport deployment.
...
`enabled` Enables the Kube Updater and deploys it alongside the Teleport Agent.
You can enable this when:

- using Teleport Cloud and your tenant is enrolled into automatic updates by default.
- using self-hosted Teleport and managed updates are enabled.

## `containerName`

`containerName` sets the name of the main container in the tbot pod.
This value is used to identify the container in Kubernetes and is typically set to "tbot"
for the tbot chart.

...
`crd` controls whether the chart creates custom resources managed by the Teleport Operator.
If enabled, the chart will create TeleportRoleV7, TeleportBotV1, and TeleportProvisionToken
resources to automatically connect the plugin to Teleport.

<Admonition type="warning" title="Interaction with Teleport Operator">
The Teleport Operator must already be enabled in the Teleport cluster
for the custom resources to be applied successfully.
</Admonition>
...
### `crd.tokenJoinOverride`

`crd.tokenJoinOverride` configures the join token for tbot and the plugin to use to join the Teleport cluster.

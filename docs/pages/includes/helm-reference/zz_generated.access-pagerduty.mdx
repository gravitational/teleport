...
PagerDuty](../../identity-governance/access-request-plugins/ssh-approval-pagerduty.mdx) guide
...
`pagerduty` contains the configuration used by the plugin to authenticate to PagerDuty.

You can pass the PagerDuty API key:
- via the chart Values by setting `pagerduty.apiKey`
- via an existing Kubernetes Secret by setting `pagerduty.apiKeyFromSecret`
...
`pagerduty.apiKey` is the PagerDuty API key used by the plugin to interact
with PagerDuty. When set, the Chart creates a Kubernetes Secret for you.

This value has no effect if `pagerduty.apiKeyFromSecret` is set.
...
`pagerduty.apiKeyFromSecret` is the name of the Kubernetes Secret
...
`pagerduty.apiKeySecretPath` is the Kubernetes Secret key
...
`pagerduty.userEmail` is the PagerDuty bot email.
...
See [the join method reference](../../reference/join-methods.mdx) for a list of supported values and detailed explanations.
...
| Type | Default |
|------|---------|
| `string` | `"public.ecr.aws/gravitational/teleport-plugin-pagerduty"` |

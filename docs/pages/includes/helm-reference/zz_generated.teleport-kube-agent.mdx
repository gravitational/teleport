...
<Admonition type="tip" title="Supported values">
  You can see a list of all the supported [values which can be used in a Teleport Database Service configuration here](../../reference/deployment/config.mdx#database-service).
</Admonition>
...
#### `highAvailability.podDisruptionBudget.unhealthyPodEvictionPolicy`

| Type | Default |
|------|---------|
| `string` | `"AlwaysAllow"` |

`highAvailability.podDisruptionBudget.unhealthyPodEvictionPolicy` allows
eviction of unhealthy pods even if it falls below minAvailable. When all
pods are unhealthy (e.g. join token missing, CrashLoopBackoff, can't reach
server due to firewall/networking), the default policy (IfHealthyBudget)
prevents draining a node.

#### `highAvailability.podDisruptionBudget.forceUnhealthyPodEvictionPolicy`

| Type | Default |
|------|---------|
| `bool` | `false` |

`highAvailability.podDisruptionBudget.forceUnhealthyPodEvictionPolicy` enables unhealthyPodEvictionPolicy for kubernetes clusters < v1.31. The
feature became [stable in that version](https://kubernetes.io/docs/tasks/run-application/configure-pdb/#unhealthy-pod-eviction-policy)
but is available in k8s 1.26-1.30 if explicitly enabled, and may require
the PDBUnhealthyPodEvictionPolicy feature gate.

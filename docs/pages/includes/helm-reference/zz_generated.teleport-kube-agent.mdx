...
| Teleport Enterprise cluster        | `example.teleport.sh:443`           |
...
the Teleport Community Edition version or the enterprise version of the container image.
This must be set to `true` when connecting to Teleport Enterprise or self-hosted
...
- using Teleport Enterprise and your tenant is enrolled into automatic updates.
...
- you are a Teleport Enterprise customer not enrolled in automatic updates.
...
You must set this if the updater is enabled, and you are not a Teleport Enterprise user.

You must not change the default values if you are a Teleport Enterprise user.
...
You must not change the default value if you are a Teleport Enterprise user unless
...
`image` sets the container image used for Teleport Community Edition agent pods
...
<Admonition type="warning" title="Interaction with Teleport Kube Agent Updater">
  ...
  using automatic updates. Teleport Enterprise uses automatic updates by default.
</Admonition>

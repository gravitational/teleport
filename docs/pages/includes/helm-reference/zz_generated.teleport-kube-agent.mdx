...
DEPRECATED: This value is deprecated and will be removed in a future version.
Use [`rbac`](#rbac) instead.
...
`rbac` controls the RBAC resources created by the chart.
...
- When `true`, the chart creates both `ClusterRole` and `ClusterRoleBinding`
  resources for the agent, impersonation admin, and `Role`/`RoleBinding` for
  the updater if enabled.
...
where the user deploying the agent is not allowed to edit RBAC resources,
in which case you are responsible for creating the RBAC resources, see
...
`rbac.roleName` provides a custom name for the `Role` resource that the
`teleport-kube-agent` chart creates for the Teleport pod. By default, the `Role`
has the name of the Helm release.
...
`rbac.roleBindingName` provides a custom name for the `RoleBinding` resource that the
`teleport-kube-agent` chart creates for the Teleport pod. By default, the
`RoleBinding` has the name of the Helm release.
...
`rbac.clusterRoleName` can be optionally used to override the name of the
Kubernetes `ClusterRole` used by the agent's `ServiceAccount`. By default, the
`ClusterRole` has the name of the Helm release.

You should set this value if there is a `ClusterRole` resource your cluster with
the same name as your `teleport-kube-agent` release.
...
`rbac.clusterRoleBindingName` can be optionally used to override the name
of the Kubernetes `ClusterRoleBinding` used by the agent's `ServiceAccount`.
By default, the `ClusterRoleBinding` has the name of the Helm release.

You should set this value if there is a `ClusterRoleBinding` in your cluster with
the same name as your `teleport-kube-agent` release.

### `rbac.adminClusterRoleName`

| Type | Default |
|------|---------|
| `string` | `""` |

`rbac.adminClusterRoleName` can be optionally used to override the name
of the Kubernetes `ClusterRole` used for admin impersonation. By default, the
`ClusterRole` has the name of the Helm release with `-cluster-admin` suffix.

You should set this value if there is a `ClusterRoleBinding` resource in your cluster
with the same name as your `teleport-kube-agent` release with `-cluster-admin` suffix.

### `rbac.adminClusterRoleBindingName`

| Type | Default |
|------|---------|
| `string` | `""` |

`rbac.adminClusterRoleBindingName` can be optionally used to override the name
of the Kubernetes `ClusterRoleBinding` used for admin impersonation. By default, the
`ClusterRoleBinding` has the name of the Helm release with `-cluster-admin` suffix.
...
`rbac.adminGroupName` can be optionally used to override the name
of the Kubernetes `Group` used for admin impersonation. By default, the
`Group` has the name of the Helm release with `-cluster-admin` suffix.

You should set this value if there is a `Group` resource in your cluster
with the same name as your `teleport-kube-agent` release with `-cluster-admin` suffix.
...
`extraLabels.clusterRole` are labels to set on the agent ClusterRole.
...
`extraLabels.clusterRoleBinding` are labels to set on the agent ClusterRoleBinding.

### `extraLabels.adminClusterRole`

| Type | Default |
|------|---------|
| `object` | `{}` |

`extraLabels.adminClusterRole` are labels to set on the admin ClusterRole.

### `extraLabels.adminClusterRoleBinding`

| Type | Default |
|------|---------|
| `object` | `{}` |

`extraLabels.adminClusterRoleBinding` are labels to set on the admin ClusterRoleBinding.

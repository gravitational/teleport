...
1. By default the operator only watches for resources in its own namespace,
   you can either put the CR in the same namespace as the operator or run the 
   operator with the `--watch-all-namespaces` flag.

...
Has format `<channel>/<version>` e.g. stable/v(=teleport.major_version=). See [installation](../../get-started/download-install/installation.mdx) for more details.
- `{{ .AutomaticUpgrades }}`: indicates whether Automatic Updates are enabled or disabled.
  Its value is either `true` or `false`. See
  [self-hosted automatic agent updates](../../get-started/download-install/upgrading/self-hosted-automatic-agent-updates.mdx)

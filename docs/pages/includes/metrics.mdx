...
The following table identifies all metrics available when the service connects to upstream servers.
In the case of `proxy`, the upstream server can be a `kubernetes_service` or Kubernetes Cluster if it's running
in legacy mode.
...
| `teleport_kubernetes_client_got_conn_duration_seconds`            | histogram | Teleport Kubernetes Proxy | Latency distribution of time to dial to the upstream server - using reversetunnel or direct dialer.       |
...
The following table identifies all metrics available when the service connects to upstream servers.
In the case of `kubernetes_service`, the upstream server is always a Kubernetes cluster.

...
| Value                     | Description                                                                       |
|---------------------------|-----------------------------------------------------------------------------------|
| `read_entra_groups`       | Reading groups from the Microsoft Graph API during a full sync. Emitted only on full sync.                  |
| `read_entra_groups_delta` | Reading group changes from the Microsoft Graph API during a delta sync. Emitted only on delta sync.        |
| `read_entra_members`      | Reading group memberships from the Microsoft Graph API.  Emitted only on full sync.                         |
| `read_entra_users`        | Reading users from the Microsoft Graph API during a full sync.     Emitted only on full sync.               |
| `read_entra_users_delta`  | Reading user changes from the Microsoft Graph API during a delta sync.   Emitted only on delta sync.         |
...
The `operation` and `result` labels breaks down into the following values:

| Label       | Values                                                                                                                                                            |
|-------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| `operation` | `create`, `update`, or `delete`.                                                                                                                                  |
| `result`    | `success`, `error`, or `noop`. `noop` is emitted when user or Access List scheduled for deletion is not found.  |

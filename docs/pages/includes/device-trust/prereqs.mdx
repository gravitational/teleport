- To enroll a macOS device, you need:
  - A signed and notarized `tsh` binary.
    [Download the macOS tsh installer](https://goteleport.com/download).
- To enroll a Windows device, you need:
  ...
  - The `tsh` client. [Download the Windows tsh
    installer](https://goteleport.com/download).
- To enroll a Linux device, you need:
  ...
  - The `tsh` client. [Install tsh for Linux](../../installation/single-machine/linux.mdx).

    WSL users should use the Windows binary instead.
    [Download the Windows tsh installer](https://goteleport.com/download).
...
- Correct end-user IP propagation to your Teleport deployment:
  [X-Forwarded-For header](../../reference/deployment/config.mdx#proxy-service) (L7 load

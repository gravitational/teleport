- To enroll a macOS device, you need:
  - A signed and notarized `tsh` binary.
    [Download the macOS tsh installer](../../includes/cloud/install-macos.mdx).
...
- To enroll a Linux device, you need:
  ...
  - The `tsh` client. [Install tsh for Linux](../install-linux.mdx).

    WSL users should use the Windows binary instead.
    [Download the Windows tsh installer](../install-windows.mdx).

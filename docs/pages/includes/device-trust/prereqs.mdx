...
- To enroll a Windows device, you need:
  ...
  - The `tsh` client. [Download the Windows tsh installer](../../installation.mdx#windows-tsh-tbot-and-tctl-clients-only).
- To enroll a Linux device, you need:
  ...
  - The `tsh` client. [Install tsh for Linux](../../installation.mdx#linux).
    ...
    [Download the Windows tsh installer](../../installation.mdx#windows-tsh-tbot-and-tctl-clients-only).
...
- Correct end-user IP propagation to your Teleport deployment:
  ...
  balancer) or [PROXY protocol](../../zero-trust-access/management/security/proxy-protocol.mdx) (L4 load balancer)

...
`example.teleport.sh` with the DNS name of your Teleport Enterprise
tenant:

```code
$ teleport-event-handler configure . example.teleport.sh:443
```
...
```code
$ TELEPORT_CLUSTER_ADDRESS=example.teleport.sh:443

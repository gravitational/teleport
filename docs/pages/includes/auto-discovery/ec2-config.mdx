...
<TabItem label="Dynamic configuration (recommended)">
  Create a Discovery Config resource, that has the same discovery group as above, to enable EC2 instance discovery.
...
<TabItem label="Static configuration">
  <Admonition type="tip">
  While simpler, this method has less flexibility because enrollment changes require configuration changes and the restart of the Discovery Service.
  </Admonition>

...
```
  ...
  <Admonition type="tip">
  `document_name` defaults to `TeleportDiscoveryInstaller`, a custom SSM
  document created by `teleport discovery bootstrap` (or manually, as shown in
  the [Manual EC2 Auto-Discovery
  guide](../../enroll-resources/auto-discovery/servers/ec2-discovery/ec2-discovery-manual.mdx)).
  You can also set `document_name` to the built-in `AWS-RunShellScript`
  document, which is available in every AWS account and requires no setup.
  </Admonition>

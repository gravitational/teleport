...
<TabItem label="Dynamic configuration (recommended)">
  ...
  The `document_name` must refer to an SSM document that accepts `token`,
  `scriptName`, and `env` parameters. The built-in AWS document
  `AWS-RunShellScript` does not accept these parameters and will cause
  `InvalidParameters` errors. Use the `TeleportDiscoveryInstaller` document
  created by `teleport discovery bootstrap`, or create a custom document as
  shown in the [Manual EC2 Auto-Discovery
  guide](../../enroll-resources/auto-discovery/servers/ec2-discovery/ec2-discovery-manual.mdx).
  </Admonition>

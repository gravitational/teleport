...
- `readyz remained in starting state after ...` means the agent process kept starting but did not become ready within the retry window.
- `readyz reported not ready: bad token` means Auth rejected join credentials.
- `service teleport remained in transition ... state="deactivating"` means systemd stayed in a transition state across retries.
- `systemd reported service teleport is not active (state: "failed"|"inactive")` means the service stopped or failed before join completed.
- `join failure: token is expired or not found; ...` means Teleport detected an explicit token failure signal in journal logs and surfaced it in the primary error line.
...
```json
{
  ...
  "platform_type": "Linux",
  "platform_version": "24.04",
  "status": "Teleport was installed successfully but the agent failed to join the cluster. Check the readyz status in the standard error output for details.",
  "stderr": "ERROR: join failure: token is expired or not found; readyz remained in starting state after wait (attempts=2, status=\"teleport is starting and hasn't joined the cluster yet\")\n\nJournal output:\n...",
  "stdout": "",
  "exit_code": 150
}
```

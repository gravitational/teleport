<Tabs>
<TabItem label="Login Errors in Audit Log" scope={["cloud"]}>
...
All editions can provide the ability to review the Audit Log in the Web Console. In self hosted
you can monitor the audit log file entries and the process logs on the Teleport Auth Server as a additional option.
...
process logs to see `ERROR` and `INFO` related entries.
</Admonition>

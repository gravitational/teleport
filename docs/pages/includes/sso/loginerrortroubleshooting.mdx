...
###Failed to verify JWT: oidc: unable to verify JWT signature: no matching keys

This error typically indicates a discrepancy between the algorithm used to sign the JWT token and the algorithm(s) supported by the JWKS (JSON Web Key Set). Specifically, the token might be signed with one algorithm, e.g., HS256, while the JWKS only lists keys for a different algorithm, e.g., RS256. Here are the things to check:

Here are some things to check:
- Verify the JWT header specifies the correct signing algorithm (alg). This should match one of the algorithms listed in the keys section of the JWKS endpoint response. Common algorithms are RS256.
- Ensure the JWKS endpoint is returning all relevant public keys. Sometimes key rotation can cause valid keys to be omitted.

To resolve the issue, align the JWT algorithm header (alg) with a supported algorithm in the JWKS. Rotate keys if necessary. Verify the JWKS only publishes the active public keys. With proper configuration, the signature should validate successfully.
...
spec:
  ...
  allow:
    ...
    request:
      ...
      # can also be matchers. this example leverages Teleport's regular expression
      # support that allows for dynamic mapping from claims. the below mapping
      # says that users with claims that match with `projects: project-(.*)` can request
      # roles that match with '$1-admin' with the first match group.
      # Example: `projects: project-foo` claim allows requesting `foo-admin` role
      claims_to_roles:
        - claim: 'projects'

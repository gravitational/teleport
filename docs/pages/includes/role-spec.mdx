...
spec:
  ...
  options:
    ...
    # Specify whether or not to record the user's SSH sessions and how to deal
    # with session recording failures, such as a full disk error.
    #
    # Session recording is enabled if any of the user's roles specify
    # `best_effort` or `strict`, where, in case of recording failures, the
    # session will be immediately terminated only if the mode is set to `strict`.
    #
    # The configuration is done per service. The `default` will be used if the
    # service doesn’t specify a value. When set in multiple roles, the value
    # used will always be the strictest, following the rule: `strict` > `best_effort` > `off`.
    # To turn off the session recording, all user’s roles must set the value to
    # `off`; otherwise, the strictest value will be used.
    record_session:
      ...
      # roles has enabled recording. Supported values are `true` (enables
      # recording) and `false` (turns off recording). Defaults to `true` if
      # unspecified.
      #
      ...
      # protocol-specific mode is not set. Defaults to `best_effort`
      default: off|best_effort|strict
      # Optional: Session recording mode for SSH sessions.
      # If not set, the value set on default will be used.
      ssh: off|best_effort|strict

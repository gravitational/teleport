...
spec:
  ...
  options:
    ...
    # Defines session recording options.
    record_session:
      ...
      desktop: true
      # Specify whether or not to record the user's sessions (currently only
      # available for SSH sessions) and how to deal with session recording
      # failures, such as a full disk error.
      #
      ...
      # session will be immediately terminated only if the mode is set to
      # `strict`. This option is optional and the default value is `best_effort`.
      #

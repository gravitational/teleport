The first step is to create an IAM role that provides access to your
{{ database }} resources. Teleport Application Service will assume this IAM
role on behalf of the Teleport user that accesses these {{ database }}
resources.
...
Create a file with the following trust policy:
```code
...
Add the following resources to your Terraform deployment:
```code

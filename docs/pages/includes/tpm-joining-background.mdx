...
a Teleport cluster without using any shared secrets as with the`token` join
method. Instead of using a shared secret, the unique identity of the host's
Trusted Platform Module (TPM) and public key cryptography is used to
authenticate the host.

This is the most secure way of joining on-prem hosts to a Teleport cluster.
...
it, and then creating a join token based on the EK public key of the TPM.
Even if the host operating system is reinstalled, the EK public key will not
change, meaning that the TPM will still be usable to join your Teleport

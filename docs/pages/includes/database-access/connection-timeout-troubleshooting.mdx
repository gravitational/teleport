...
In a high-availability (HA) deployment where your database is registered
dynamically or via auto-discovery, repeat the above connectivity test for
**every** Teleport Database Service that proxies this database. To list all
Teleport Database Services associated with a given database, run the `tctl get
db_server/<db_name>` command. For example:
```
...
If any of the Database Services listed here **should not** proxy the database,
(for example, a Database Service in a different VPC or AWS region without
connectivity), locate and update their configurations so they only receive or
discover databases they can reach. In most cases, you can achieve this by
refining your tag filters, such as adding the a `vpc-id` label.
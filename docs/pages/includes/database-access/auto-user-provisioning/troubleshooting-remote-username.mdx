...
cluster:
```shell
> tsh db connect mysql --db-name <database> example
...
cluster. Please use this name from the error message as the database user for
when connecting through `tsh` or the GUI clients.

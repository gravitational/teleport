...
<Admonition type="note" title="Teleport database certificate authorities">
Teleport maintains two separate CAs for database access:

- **Database CA (`db`)**: Signs certificates that databases present to the
  Teleport Database Service. Export with `tctl auth export --type=db`.
- **Database Client CA (`db_client`)**: Signs certificates that the Teleport
  Database Service presents to databases. Export with
  `tctl auth export --type=db-client`.

When using `tctl auth sign`, the output files are:
- `server.cas`: A CA bundle for your database to trust - contains the Database
  Client CA certificate, which your database uses to verify connections from
  the Teleport Database Service.
- `server.crt`: A server certificate for your database, signed by the Database CA.

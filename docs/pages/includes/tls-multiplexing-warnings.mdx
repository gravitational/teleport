Prior to Telport `13.0`, using Teleport's TLS routing mode behind a layer 7 (HTTP/HTTPS) proxy is generally not supported, due to
...
support is available in Preview starting from Teleport `13.0`. Please ensure

...
[Auth Guide](teleport-auth.mdx#issuing-node-certificates).
...
[Auth Server API](teleport-auth.mdx#auth-api).
...
* [Teleport Proxy UI](teleport-proxy.mdx#web-to-ssh-proxy) accessed via a web browser.

Each client is authenticated via the [Auth Service](teleport-auth.mdx#authentication-in-teleport) before being granted access to a Node.
...
Guide](teleport-auth.mdx#authentication-in-teleport).
...
a node in the [Architecture Overview](teleport-architecture-overview.mdx).
...
Read more about what is stored in the [Auth Guide](teleport-auth.mdx#auth-state)

...
- Have access to a Teleport Auth server ([quickstart](quickstart.mdx))
...
<Admonition type="note">
By default, `tctl auth sign` produces certificates with a relatively short lifetime. See our [Kubernetes Section](kubernetes-ssh.mdx#using-teleport-kubernetes-with-automation) for more information on automating the signing process for short lived certificates.

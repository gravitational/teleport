...
must be enabled by using the `--diag-addr` flag to teleport start: `teleport start --diag-addr=127.0.0.1:3434`
The http://127.0.0.1:3434/readyz endpoint will reply `{"status":"ok"}` if the Teleport service

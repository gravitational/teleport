...
* Users can login once using [`tsh login`](cli-docs.mdx#tsh-login) and
  switch between multiple clusters using [`tsh kube login`](cli-docs.mdx#tsh-kube-login).
...
[Github section in the Admin Manual](admin-guide.mdx#github-oauth-20).
</Admonition>
...
[Okta integration guide](enterprise/sso/ssh-okta.mdx).
</Admonition>
...
a kubeconfig using [`tctl auth sign`](cli-docs.mdx#tctl-auth-sign)
...
We've a complete guide on setting up Teleport with EKS. Please see the [Using Teleport with EKS Guide](aws-oss-guide.mdx#using-teleport-with-eks).
...
You can take advantage of the [Trusted Clusters](trustedclusters.mdx) feature of

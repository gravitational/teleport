...
[Auth Guide](authentication.md#issuing-node-certificates).
...
* [Teleport Proxy UI](proxy.md#web-to-ssh-proxy) accessed via a web browser.

Each client is authenticated via the [Auth Service](authentication.md#authentication-in-teleport) before being granted access to a Node.
...
a node in the [Architecture Overview](overview.md).
...
Read more about what is stored in the [Auth Guide](authentication.md#auth-state)

...
1. Create a static or dynamic token on `main` that will be used by `cluster-b`
...
1. Create a `TrustedCluster` resource on `cluster-b` that tells `cluster-b`
...
1. Notice how we defined `roles` in the `TrustedCluster` resource. This is
...
1. Now inject both roles into the Teleport "auth service" on `cluster-b` using
   `tctl`:
...
That's it, with that done both clusters have exchanged certificate authorities
and `cluster-b` has built a reverse tunnel to `main`. To verify, run this on
"cluster-b":

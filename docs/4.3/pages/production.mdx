...
2. The **ExecReload** command allows admins to run `systemctl reload teleport`.
   ...
   [etc 3.3](admin-guide.mdx#using-etcd) is configured*_. Graceful Restarts will
...
During our [quickstart](quickstart.mdx) guide we skip over setting up TLS so that you can quickly try Teleport.

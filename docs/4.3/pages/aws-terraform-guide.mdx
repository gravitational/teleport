...
- [Install the Teleport binary on the Server](admin-guide.mdx#installing)
- [Run Teleport - we recommend using systemd](admin-guide.mdx#systemd-unit-file)
- [Set the correct settings in /etc/teleport.yaml](admin-guide.mdx#configuration-file)
- [Add nodes to the Teleport cluster](admin-guide.mdx#adding-nodes-to-the-cluster)
...
You can also generate a node join token using `tctl tokens add --type=node` [as detailed here in our admin guide](admin-guide.mdx#adding-nodes-to-the-cluster).
...
for this document) or join your nodes using [Teleport's node tunnelling](admin-guide.mdx#adding-a-node-located-behind-nat) functionality.

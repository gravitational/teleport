...
func (r resourceTeleportHealthCheckConfig) ModifyPlan(ctx context.Context, req tfsdk.ModifyResourcePlanRequest, resp *tfsdk.ModifyResourcePlanResponse) {
	...
	healthCheckConfig := &healthcheckconfigv1.HealthCheckConfig{}
	resp.Diagnostics.Append(schemav1.CopyHealthCheckConfigFromTerraform(ctx, plan, healthCheckConfig)...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	resp.Diagnostics.Append(schemav1.CopyHealthCheckConfigToTerraform(ctx, healthCheckConfig, &normalized)...)
	if resp.Diagnostics.HasError() {
		return
	}

...
func (r resourceTeleportAuthPreference) ModifyPlan(ctx context.Context, req tfsdk.ModifyResourcePlanRequest, resp *tfsdk.ModifyResourcePlanResponse) {
	...
	authPreference := &apitypes.AuthPreferenceV2{}
	resp.Diagnostics.Append(tfschema.CopyAuthPreferenceV2FromTerraform(ctx, plan, authPreference)...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	resp.Diagnostics.Append(tfschema.CopyAuthPreferenceV2ToTerraform(ctx, authPreference, &normalized)...)
	if resp.Diagnostics.HasError() {
		return
	}

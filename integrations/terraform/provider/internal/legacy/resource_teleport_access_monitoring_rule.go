...
func (r resourceTeleportAccessMonitoringRule) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if !trace.IsNotFound(err) {
		if err == nil {
			...
			resp.Diagnostics.Append(tfdiag.DiagFromErr("AccessMonitoringRule exists in Teleport", trace.Errorf(existErr)))
			return
		}

		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading AccessMonitoringRule", trace.Wrap(err), "access_monitoring_rule"))
		return
	}

	_, err = r.p.Client().AccessMonitoringRulesClient().CreateAccessMonitoringRule(ctx, accessMonitoringRuleResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating AccessMonitoringRule", trace.Wrap(err), "access_monitoring_rule"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading AccessMonitoringRule", trace.Wrap(ctx.Err()), "access_monitoring_rule"))
				return
			case <-retry.After():
			}
...
func (r resourceTeleportAccessMonitoringRule) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading AccessMonitoringRule", err, "access_monitoring_rule"))
		return
	}

	_, err = r.p.Client().AccessMonitoringRulesClient().UpdateAccessMonitoringRule(ctx, accessMonitoringRuleResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating AccessMonitoringRule", err, "access_monitoring_rule"))
		return
	}
		var accessMonitoringRuleI *accessmonitoringrulesv1.AccessMonitoringRule
...
func (r resourceTeleportAccessMonitoringRule) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting AccessMonitoringRule", trace.Wrap(err), "access_monitoring_rule"))
		return
	}

...
func (r resourceTeleportInferenceModel) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if !trace.IsNotFound(err) {
		if err == nil {
			...
			resp.Diagnostics.Append(tfdiag.DiagFromErr("InferenceModel exists in Teleport", trace.Errorf(existErr)))
			return
		}
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating InferenceModel", trace.Wrap(err), "inference_model"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading InferenceModel", trace.Wrap(ctx.Err()), "inference_model"))
				return
			case <-retry.After():
			}
...
func (r resourceTeleportInferenceModel) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading InferenceModel", err, "inference_model"))
		return
	}

	_, err = r.p.Client().SummarizerClient().UpsertInferenceModel(ctx, inferenceModelResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating InferenceModel", err, "inference_model"))
		return
	}
		var inferenceModelI *summarizerv1.InferenceModel
...
func (r resourceTeleportInferenceModel) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting InferenceModel", trace.Wrap(err), "inference_model"))
		return
	}

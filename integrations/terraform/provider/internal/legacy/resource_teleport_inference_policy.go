...
func (r resourceTeleportInferencePolicy) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if !trace.IsNotFound(err) {
		if err == nil {
			...
			resp.Diagnostics.Append(tfdiag.DiagFromErr("InferencePolicy exists in Teleport", trace.Errorf(existErr)))
			return
		}
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating InferencePolicy", trace.Wrap(err), "inference_policy"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading InferencePolicy", trace.Wrap(ctx.Err()), "inference_policy"))
				return
			case <-retry.After():
			}
...
func (r resourceTeleportInferencePolicy) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading InferencePolicy", err, "inference_policy"))
		return
	}

	_, err = r.p.Client().SummarizerClient().UpsertInferencePolicy(ctx, inferencePolicyResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating InferencePolicy", err, "inference_policy"))
		return
	}
		var inferencePolicyI *summarizerv1.InferencePolicy
...
func (r resourceTeleportInferencePolicy) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting InferencePolicy", trace.Wrap(err), "inference_policy"))
		return
	}

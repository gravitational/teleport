...
func (r resourceTeleportHealthCheckConfig) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if !trace.IsNotFound(err) {
		if err == nil {
			...
			resp.Diagnostics.Append(tfdiag.DiagFromErr("HealthCheckConfig exists in Teleport", trace.Errorf(existErr)))
			return
		}
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating HealthCheckConfig", trace.Wrap(err), "health_check_config"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading HealthCheckConfig", trace.Wrap(ctx.Err()), "health_check_config"))
				return
			case <-retry.After():
			}
...
func (r resourceTeleportHealthCheckConfig) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading HealthCheckConfig", err, "health_check_config"))
		return
	}

	_, err = r.p.Client().UpsertHealthCheckConfig(ctx, healthCheckConfigResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating HealthCheckConfig", err, "health_check_config"))
		return
	}
		var healthCheckConfigI *healthcheckconfigv1.HealthCheckConfig
...
func (r resourceTeleportHealthCheckConfig) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting HealthCheckConfig", trace.Wrap(err), "health_check_config"))
		return
	}

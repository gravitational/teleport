...
func (r resourceTeleportSAMLConnector) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error setting SAMLConnector defaults", trace.Wrap(err), "saml"))
		return
	}
	...
	if !trace.IsNotFound(err) {
		if err == nil {
			...
			resp.Diagnostics.Append(tfdiag.DiagFromErr("SAMLConnector exists in Teleport", trace.Errorf(existErr)))
			return
		}
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating SAMLConnector", trace.Wrap(err), "saml"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading SAMLConnector", trace.Wrap(ctx.Err()), "saml"))
				return
			case <-retry.After():
			}
	...
	if !ok {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading SAMLConnector", trace.Errorf("Can not convert %T to SAMLConnectorV2", samlConnectorI), "saml"))
		return
	}
...
func (r resourceTeleportSAMLConnector) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err := samlConnectorResource.CheckAndSetDefaults(); err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating SAMLConnector", err, "saml"))
		return
	}
...
func (r resourceTeleportSAMLConnector) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting SAMLConnectorV2", trace.Wrap(err), "saml"))
		return
	}

...
func (r resourceTeleportWorkloadIdentity) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if !trace.IsNotFound(err) {
		if err == nil {
			...
			resp.Diagnostics.Append(tfdiag.DiagFromErr("WorkloadIdentity exists in Teleport", trace.Errorf(existErr)))
			return
		}

		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading WorkloadIdentity", trace.Wrap(err), "workload_identity"))
		return
	}

	_, err = r.p.Client().CreateWorkloadIdentity(ctx, workloadIdentityResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating WorkloadIdentity", trace.Wrap(err), "workload_identity"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading WorkloadIdentity", trace.Wrap(ctx.Err()), "workload_identity"))
				return
			case <-retry.After():
			}
...
func (r resourceTeleportWorkloadIdentity) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading WorkloadIdentity", err, "workload_identity"))
		return
	}

	_, err = r.p.Client().UpsertWorkloadIdentity(ctx, workloadIdentityResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating WorkloadIdentity", err, "workload_identity"))
		return
	}
		var workloadIdentityI *workloadidentityv1.WorkloadIdentity
...
func (r resourceTeleportWorkloadIdentity) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting WorkloadIdentity", trace.Wrap(err), "workload_identity"))
		return
	}

...
func (r resourceTeleportLoginRule) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if !trace.IsNotFound(err) {
		if err == nil {
			...
			resp.Diagnostics.Append(tfdiag.DiagFromErr("LoginRule exists in Teleport", trace.Errorf(existErr)))
			return
		}

		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading LoginRule", trace.Wrap(err), "login_rule"))
		return
	}

	_, err = r.p.Client().UpsertLoginRule(ctx, loginRuleResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating LoginRule", trace.Wrap(err), "login_rule"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading LoginRule", trace.Wrap(ctx.Err()), "login_rule"))
				return
			case <-retry.After():
			}
...
func (r resourceTeleportLoginRule) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading LoginRule", err, "login_rule"))
		return
	}

	_, err = r.p.Client().UpsertLoginRule(ctx, loginRuleResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating LoginRule", err, "login_rule"))
		return
	}
		var loginRuleI *loginrulev1.LoginRule
...
func (r resourceTeleportLoginRule) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting LoginRule", trace.Wrap(err), "login_rule"))
		return
	}

...
func (r resourceTeleportDeviceV1) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if !trace.IsNotFound(err) {
		...
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading DeviceV1", trace.Wrap(err), "device"))
		return
	}

	_, err = r.p.Client().UpsertDeviceResource(ctx, trustedDeviceResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating DeviceV1", trace.Wrap(err), "device"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading DeviceV1", trace.Wrap(ctx.Err()), "device"))
				return
			case <-retry.After():
			}
...
func (r resourceTeleportDeviceV1) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading DeviceV1", err, "device"))
		return
	}

	_, err = r.p.Client().UpsertDeviceResource(ctx, trustedDeviceResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating DeviceV1", err, "device"))
		return
	}
		var trustedDeviceI *apitypes.DeviceV1
...
func (r resourceTeleportDeviceV1) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting DeviceV1", trace.Wrap(err), "device"))
		return
	}

...
func (r resourceTeleportStaticHostUser) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if !trace.IsNotFound(err) {
		if err == nil {
			...
			resp.Diagnostics.Append(tfdiag.DiagFromErr("StaticHostUser exists in Teleport", trace.Errorf(existErr)))
			return
		}

		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading StaticHostUser", trace.Wrap(err), "static_host_user"))
		return
	}

	_, err = r.p.Client().StaticHostUserClient().CreateStaticHostUser(ctx, staticHostUserResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating StaticHostUser", trace.Wrap(err), "static_host_user"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading StaticHostUser", trace.Wrap(ctx.Err()), "static_host_user"))
				return
			case <-retry.After():
			}
...
func (r resourceTeleportStaticHostUser) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading StaticHostUser", err, "static_host_user"))
		return
	}

	_, err = r.p.Client().StaticHostUserClient().UpsertStaticHostUser(ctx, staticHostUserResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating StaticHostUser", err, "static_host_user"))
		return
	}
		var staticHostUserI *userprovisioningv2.StaticHostUser
...
func (r resourceTeleportStaticHostUser) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting StaticHostUser", trace.Wrap(err), "static_host_user"))
		return
	}

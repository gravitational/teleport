...
func (r resourceTeleportClientIPRestriction) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating ClientIPRestriction", trace.Wrap(err), "client_ip_restriction"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading ClientIPRestriction", trace.Wrap(ctx.Err()), "client_ip_restriction"))
				return
			case <-retry.After():
			}

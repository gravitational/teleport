...
func (r resourceTeleportDynamicWindowsDesktop) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error setting DynamicWindowsDesktop defaults", trace.Wrap(err), "dynamic_windows_desktop"))
		return
	}
	...
	if !trace.IsNotFound(err) {
		if err == nil {
			...
			resp.Diagnostics.Append(tfdiag.DiagFromErr("DynamicWindowsDesktop exists in Teleport", trace.Errorf(existErr)))
			return
		}

		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading DynamicWindowsDesktop", trace.Wrap(err), "dynamic_windows_desktop"))
		return
	}

	_, err = r.p.Client().DynamicDesktopClient().CreateDynamicWindowsDesktop(ctx, desktopResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating DynamicWindowsDesktop", trace.Wrap(err), "dynamic_windows_desktop"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading DynamicWindowsDesktop", trace.Wrap(ctx.Err()), "dynamic_windows_desktop"))
				return
			case <-retry.After():
			}
	...
	if !ok {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading DynamicWindowsDesktop", trace.Errorf("Can not convert %T to DynamicWindowsDesktopV1", desktopI), "dynamic_windows_desktop"))
		return
	}
...
func (r resourceTeleportDynamicWindowsDesktop) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err := desktopResource.CheckAndSetDefaults(); err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating DynamicWindowsDesktop", err, "dynamic_windows_desktop"))
		return
	}
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading DynamicWindowsDesktop", err, "dynamic_windows_desktop"))
		return
	}
...
func (r resourceTeleportDynamicWindowsDesktop) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting DynamicWindowsDesktopV1", trace.Wrap(err), "dynamic_windows_desktop"))
		return
	}

...
func (r resourceTeleportWorkloadCluster) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if !trace.IsNotFound(err) {
		if err == nil {
			...
			resp.Diagnostics.Append(tfdiag.DiagFromErr("WorkloadCluster exists in Teleport", trace.Errorf(existErr)))
			return
		}
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating WorkloadCluster", trace.Wrap(err), "workload_cluster"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading WorkloadCluster", trace.Wrap(ctx.Err()), "workload_cluster"))
				return
			case <-retry.After():
			}
	...
	if _, err := stateConf.WaitForStateContext(ctx); err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error waiting for WorkloadCluster", trace.Wrap(err), "workload_cluster"))
	}
}
...
func (r resourceTeleportWorkloadCluster) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading WorkloadCluster", err, "workload_cluster"))
		return
	}

	_, err = r.p.Client().UpsertWorkloadCluster(ctx, workloadclusterResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating WorkloadCluster", err, "workload_cluster"))
		return
	}
		var workloadclusterI *workloadclusterv1.WorkloadCluster
...
func (r resourceTeleportWorkloadCluster) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting WorkloadCluster", trace.Wrap(err), "workload_cluster"))
		return
	}

...
func (r resourceTeleportDiscoveryConfig) Create(ctx context.Context, req tfsdk.CreateResourceRequest, resp *tfsdk.CreateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading DiscoveryConfig", trace.Errorf("Can not convert %T to DiscoveryConfig: %s", discoveryConfigResource, err), "discovery_config"))
		return
	}
	...
	if !trace.IsNotFound(err) {
		if err == nil {
			...
			resp.Diagnostics.Append(tfdiag.DiagFromErr("DiscoveryConfig exists in Teleport", trace.Errorf(existErr)))
			return
		}
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error creating DiscoveryConfig", trace.Wrap(err), "discovery_config"))
		return
	}
	...
	for {
		...
		if trace.IsNotFound(err) {
		    select {
			case <-ctx.Done():
			    resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading DiscoveryConfig", trace.Wrap(ctx.Err()), "discovery_config"))
				return
			case <-retry.After():
			}
...
func (r resourceTeleportDiscoveryConfig) Update(ctx context.Context, req tfsdk.UpdateResourceRequest, resp *tfsdk.UpdateResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error reading DiscoveryConfig", err, "discovery_config"))
		return
	}

	_, err = r.p.Client().DiscoveryConfigClient().UpsertDiscoveryConfig(ctx, discoveryConfigResource)
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error updating DiscoveryConfig", err, "discovery_config"))
		return
	}
		var discoveryConfigI = discoveryConfigResource
...
func (r resourceTeleportDiscoveryConfig) Delete(ctx context.Context, req tfsdk.DeleteResourceRequest, resp *tfsdk.DeleteResourceResponse) {
	...
	if err != nil {
		resp.Diagnostics.Append(tfdiag.DiagFromWrappedErr("Error deleting DiscoveryConfig", trace.Wrap(err), "discovery_config"))
		return
	}

...
func (r resourceTeleportOIDCConnector) ModifyPlan(ctx context.Context, req tfsdk.ModifyResourcePlanRequest, resp *tfsdk.ModifyResourcePlanResponse) {
	...
	oidcConnector := &apitypes.OIDCConnectorV3{}
	resp.Diagnostics.Append(tfschema.CopyOIDCConnectorV3FromTerraform(ctx, plan, oidcConnector)...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	resp.Diagnostics.Append(tfschema.CopyOIDCConnectorV3ToTerraform(ctx, oidcConnector, &normalized)...)
	if resp.Diagnostics.HasError() {
		return
	}

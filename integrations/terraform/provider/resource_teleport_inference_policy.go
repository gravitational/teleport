...
func (r resourceTeleportInferencePolicy) ModifyPlan(ctx context.Context, req tfsdk.ModifyResourcePlanRequest, resp *tfsdk.ModifyResourcePlanResponse) {
	...
	inferencePolicy := &summarizerv1.InferencePolicy{}
	resp.Diagnostics.Append(schemav1.CopyInferencePolicyFromTerraform(ctx, plan, inferencePolicy)...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	resp.Diagnostics.Append(schemav1.CopyInferencePolicyToTerraform(ctx, inferencePolicy, &normalized)...)
	if resp.Diagnostics.HasError() {
		return
	}

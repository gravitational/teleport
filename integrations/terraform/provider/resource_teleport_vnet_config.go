...
func (r resourceTeleportVnetConfig) ModifyPlan(ctx context.Context, req tfsdk.ModifyResourcePlanRequest, resp *tfsdk.ModifyResourcePlanResponse) {
	...
	vnetConfig := &vnet.VnetConfig{}
	resp.Diagnostics.Append(schemav1.CopyVnetConfigFromTerraform(ctx, plan, vnetConfig)...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	resp.Diagnostics.Append(schemav1.CopyVnetConfigToTerraform(ctx, vnetConfig, &normalized)...)
	if resp.Diagnostics.HasError() {
		return
	}

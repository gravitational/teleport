...
func (r resourceTeleportOktaImportRule) ModifyPlan(ctx context.Context, req tfsdk.ModifyResourcePlanRequest, resp *tfsdk.ModifyResourcePlanResponse) {
	...
	oktaImportRule := &apitypes.OktaImportRuleV1{}
	resp.Diagnostics.Append(tfschema.CopyOktaImportRuleV1FromTerraform(ctx, plan, oktaImportRule)...)
	if resp.Diagnostics.HasError() {
		return
	}
	...
	resp.Diagnostics.Append(tfschema.CopyOktaImportRuleV1ToTerraform(ctx, oktaImportRule, &normalized)...)
	if resp.Diagnostics.HasError() {
		return
	}

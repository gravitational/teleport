						...
						if !ok {
							...
							if !ok {
								if tf.Attrs["mode"] != nil {
									diags.Append(attrWriteUnexpectedExistingTypeDiag{"ClientIPRestriction.spec.mode", "github.com/hashicorp/terraform-plugin-framework/types.String"})
								}
								i, err := t.ValueFromTerraform(ctx, github_com_hashicorp_terraform_plugin_go_tftypes.NewValue(t.TerraformType(ctx), nil))
								if err != nil {

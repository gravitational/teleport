		...
		Steps: []resource.TestStep{
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(name, "spec.rules.#", "1"),
					resource.TestCheckResourceAttr(name, "spec.rules.0.name", "backups"),
					resource.TestCheckResourceAttr(name, "spec.rules.0.criteria", "The destroyed resource was a backup or snapshot."),
					resource.TestCheckResourceAttr(name, "spec.rules.0.actions.risk_level_floor", "critical"),
					resource.TestCheckResourceAttr(name, "spec.rules.0.actions.emit_audit_event", "true"),
				),
			},
			{
				Config:   s.getFixture("classifier_0_create.tf"),
			...
			},
			{
				...
				Check: resource.ComposeTestCheckFunc(
					...
					resource.TestCheckResourceAttr(name, "spec.disabled", "true"),
					resource.TestCheckResourceAttr(name, "spec.actions.flag_for_review", "true"),
					resource.TestCheckResourceAttr(name, "spec.rules.#", "2"),
					resource.TestCheckResourceAttr(name, "spec.rules.1.name", "unjustified"),
					resource.TestCheckResourceAttr(name, "spec.rules.1.filter", `equals(user.metadata.name, "alice")`),
					resource.TestCheckResourceAttr(name, "spec.rules.1.actions.flag_for_review", "true"),
				),
			},
			{
				Config:   s.getFixture("classifier_1_update.tf"),

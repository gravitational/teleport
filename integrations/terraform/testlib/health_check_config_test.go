...
func (s *TerraformSuiteOSS) TestImportHealthCheckConfig() {
	...
	healthCfg.Metadata.Expires = timestamppb.New(time.Now().Add(time.Hour))
	...
	healthCfg, err = s.client.CreateHealthCheckConfig(ctx, healthCfg)
	require.NoError(t, err)
		...
		Steps: []resource.TestStep{
			{
				...
				ImportStateCheck: func(state []*terraform.InstanceState) error {
					...
					require.Equal(t, existing.Metadata.Expires.String(), state[0].Attributes["metadata.expires"])

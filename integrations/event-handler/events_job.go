...
func (j *EventsJob) runPolling(ctx context.Context) error {
	...
	concurrency := j.app.Config.Concurrency
	if concurrency > 32 {
		// limit concurrency to 32. values beyond this tend to have diminishing returns for the bulk event export
		// api. since the concurrency config option also controls concurrent session event extraction, we want users
		// to be able to specify much larger concurrency values if their audit data is disproportionately session-heavy,
		// without the primary event export logic eating up excess resources.
		concurrency = 32
	}

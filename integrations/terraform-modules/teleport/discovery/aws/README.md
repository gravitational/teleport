...
| [aws_iam_policy_document.teleport_discovery_service_organization](https://registry.terraform.io/providers/hashicorp/aws/latest/docs/data-sources/iam_policy_document) | data source |
...
| aws\_iam\_role\_name\_for\_child\_accounts | Name for the AWS IAM role to assume in child accounts, when using organization-wide discovery. | `string` | `"teleport-discovery-from-organization"` | no |
...
| enroll\_organization\_accounts | Discover resources in all the AWS accounts under the organization. | `bool` | `false` | no |
...
| iam\_role\_to\_create\_in\_child\_accounts | AWS IAM role that must be created in child AWS accounts under the organization for discovering resources. |

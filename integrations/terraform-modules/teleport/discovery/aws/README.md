...
|-------|------|---------|-------------|
| `types` | `list(string)` | (required) | AWS resource types to discover. Allowed values: `ec2`, `eks`, `rds`, `rdsproxy`. |
...
| aws\_matchers | AWS resource discovery matchers. Each matcher supports the following fields: - `types` (required): AWS resource types to discover. Allowed values: `ec2`, `eks`, `rds`, and `rdsproxy`. - `regions` (optional): AWS regions to search. Defaults to `["*"]`, which searches all regions. - `tags` (optional): AWS resource tags to match. Defaults to `{ "*" = ["*"] }`, which matches all resources. - `setup_access_for_arn` (optional): ARN to configure access for discovered EKS clusters. Only supported for EKS matchers. - `kube_app_discovery` (optional): Configures Kubernetes App Discovery for discovered EKS clusters. | ```list(object({ types = list(string) regions = optional(list(string), ["*"]) tags = optional(map(list(string)), { "*" : ["*"] }) setup_access_for_arn = optional(string, "") kube_app_discovery = optional(bool) }))``` | `[]` | no |

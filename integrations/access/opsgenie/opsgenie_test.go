...
func (s *OpsgenieSuite) assertNewEvent(watcher types.Watcher, opType types.OpType, resourceKind, resourceName string) types.Event {
	t := s.T()
	t.Helper()
...
func (s *OpsgenieSuite) assertNoNewEvents(watcher types.Watcher) {
	t := s.T()
	t.Helper()
...
func (s *OpsgenieSuite) assertReviewSubmitted() {
	t := s.T()
	t.Helper()

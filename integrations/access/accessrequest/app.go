...
func (a *App) onPendingRequest(ctx context.Context, req types.AccessRequest) error {
	...
	case err == nil:
		...
		// Try to approve the request if user is currently on-cal
...
func (a *App) tryApproveRequest(ctx context.Context, reqID string, reqData pd.AccessRequestData) error {
	...
	if _, err := a.apiClient.SubmitAccessReview(ctx, types.AccessReviewSubmission{
		...
		Review: types.AccessReview{
			...
			Reason:        fmt.Sprintf("Access request has been automatically approved by %q plugin because user %q is on-call.", a.pluginName, reqData.User),
			Created:       time.Now(),
		},
	}); err != nil {

locals {
  ...
  # Split okta_org_url into the parts the okta provider block wants:
  # https://integrator-1563488.okta.com -> org_name=integrator-1563488, base_url=okta.com
  okta_host_parts = split(".", replace(var.okta_org_url, "https://", ""))
  okta_org_name   = local.okta_host_parts[0]
  okta_base_url   = join(".", slice(local.okta_host_parts, 1, length(local.okta_host_parts)))
}

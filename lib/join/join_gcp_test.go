...
func TestJoinGCP(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			convertGCP := func(gcp *types.ProvisionTokenSpecV2GCP) *joiningv1.GCP {
				t.Helper()
				allow := make([]*joiningv1.GCP_Rule, len(gcp.Allow))
				for i, rule := range gcp.Allow {
			...
			scopedToken := &joiningv1.ScopedToken{
				...
				Spec: &joiningv1.ScopedTokenSpec{
					...
					Gcp:           convertGCP(token.GetGCPRules()),
				},
			}

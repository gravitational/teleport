...
func (s *clientStream) Send(msg messages.Request) (err error) {
	...
	if sendErr := s.grpcStream.Send(req); sendErr != nil {
		if errors.Is(sendErr, io.EOF) {
			// From the docs on Send: if the error was generated by the client,
			// the status is returned directly. Otherwise, io.EOF is returned,
			// and the status of the stream may be discovered using Recv().
			//
			// Returning the error from Recv in this case makes sure the client
			// sees the real error in cases where the RPC returned early with
			// an error after the server sent a message and before the client
			// sends a message. This has been known to occur when the server
			// does not support a given join method.
			_, recvErr := s.grpcStream.Recv()
			if recvErr == nil {
				// Sanity check.
				return trace.Errorf("gRPC Send returned EOF and Recv returned nil")
			}
			return trace.Wrap(recvErr, "gRPC stream failed")
		}
		return trace.Wrap(sendErr, "sending request to gRPC stream")
	}
	return nil
}

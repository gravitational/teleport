...
func (s *Server) makeResult(
	...
	case types.RoleInstance:
		return s.makeHostResult(ctx, diag, authCtx, clientParams.HostParams, provisioner)
	case types.RoleBot:
		result, _, err := s.makeBotResult(ctx, diag, authCtx, clientParams.BotParams, rawClaims, provisioner)
...
func (s *Server) makeHostResult(
	...
	certsParams, err := makeHostCertsParams(ctx, diag, authCtx, hostParams, configuredJoinMethod(provisioner))
	if err != nil {
		return nil, trace.Wrap(err)
	}

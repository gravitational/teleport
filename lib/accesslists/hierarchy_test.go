...
func TestAccessListHierarchyIsOwner(t *testing.T) {
	...
	require.Error(t, err)
	require.ErrorIs(t, err, trace.AccessDenied("%s", "User is not member of the access list, directly or via nested list\nWhen resolving access, the following access paths were ignored:\n * \"1\" because user did not meet list requirements"))
...
func TestAccessListIsMember(t *testing.T) {
	...
	membershipType, reason, err = IsAccessListMember(ctx, stubMember1, acl1, accessListAndMembersGetter, locksGetter, clock)
	require.NoError(t, err)
	require.Equal(t, reason, fmt.Sprintf("User %q is currently locked", member1))

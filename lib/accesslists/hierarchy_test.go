...
func TestAccessListHierarchyDepthCheck(t *testing.T) {
	...
	for i := 0; i < accesslist.MaxAllowedDepth; i++ {
		...
		acls[i+1].Spec.MemberOf = append(acls[i+1].Spec.MemberOf, acls[i].GetName())
...
func TestAccessListValidateWithMembers(t *testing.T) {
	...
	for i := 0; i < accesslist.MaxAllowedDepth; i++ {
		...
		nestedAcls[i+1].Spec.MemberOf = append(nestedAcls[i+1].Spec.MemberOf, nestedAcls[i].GetName())
	...
	for i := 0; i < Length; i++ {
		...
		nestedAcls1[i+1].Spec.MemberOf = append(nestedAcls1[i+1].Spec.MemberOf, nestedAcls1[i].GetName())
	...
	for i := 0; i < Length; i++ {
		...
		nestedAcls2[i+1].Spec.MemberOf = append(nestedAcls2[i+1].Spec.MemberOf, nestedAcls2[i].GetName())
...
func TestAccessListHierarchyCircularRefsCheck(t *testing.T) {
	...
	acl2.Spec.MemberOf = append(acl2.Spec.MemberOf, acl1.GetName())
	acl2m1 := newAccessListMember(t, acl2.GetName(), acl3.GetName(), accesslist.MembershipKindList, clock)
	acl3.Spec.MemberOf = append(acl3.Spec.MemberOf, acl2.GetName())
	...
	accessListAndMembersGetter.accessLists[acl3.GetName()].Spec.MemberOf = []string{}
	err = ValidateAccessListMember(ctx, acl3, acl3m1, accessListAndMembersGetter)
	require.NoError(t, err)
	...
	acl5.Spec.MemberOf = append(acl5.Spec.MemberOf, acl4.GetName())
	...
	acl4.Spec.OwnerOf = append(acl4.Spec.OwnerOf, acl5.GetName())
...
func TestAccessListHierarchyIsOwner(t *testing.T) {
	...
	acl1.Spec.OwnerOf = append(acl1.Spec.OwnerOf, acl4.GetName())
...
func TestGetOwners(t *testing.T) {
	...
	acl2.Spec.OwnerOf = append(acl2.Spec.OwnerOf, acl1.GetName())
	...
	acl3.Spec.OwnerOf = append(acl3.Spec.OwnerOf, acl2.GetName())
...
func TestGetInheritedGrants(t *testing.T) {
	...
	acl1.Spec.OwnerOf = append(acl1.Spec.OwnerOf, aclroot.GetName())

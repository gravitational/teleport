...
func validateAccessListMemberBasic(parent *accesslist.AccessList, member *accesslist.AccessListMember) error {
	...
	if member.Scope != "" {
		...
		if memberSQN.Scope != "" {
			if err := ToScopesQualifiedName(memberSQN).StrongValidate(); err != nil {
				return trace.Wrap(err)
			}
		}
		...
		if memberParentName != parent.GetScopeQualifiedName() {
			return trace.BadParameter("member spec.access_list %q must match parent list name %q",
				member.Spec.AccessList, ScopeQualifiedNameToString(parent.GetScopeQualifiedName()))
		}
	} else {
		// The member must belong to the parent access list.
...
func validateAccessListOwner(
	...
	if ownerName.Scope != "" {
		if err := ToScopesQualifiedName(ownerName).StrongValidate(); err != nil {
			return trace.Wrap(err)
		}
	}

...
func (cfg *Config) CheckAndSetDefaults() error {
	if cfg.FileSystem == nil {
		cfg.FileSystem = &localFileSystem{}
	}
...
func CreateCommand(cfg Config) (Command, error) {
	...
	cmd.Entry = log.WithFields(log.Fields{
		trace.Component: "SCP",
...
func (cmd *command) processCommand(ch io.ReadWriter, st *state, b byte, line string) error {
	cmd.Debugf("[SCP] <- %v %v", string(b), line)
	switch b {
	case WarnByte:
...
func (cmd *command) receiveFile(st *state, fc newFileCmd, ch io.ReadWriter) error {
	cmd.Debugf("scp.receiveFile(%v)", cmd.Flags.Target)
	...
	cmd.Debugf("file %v(%v) copied to %v", fc.Name, fc.Length, path)
	return nil
}

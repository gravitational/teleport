...
func (process *TeleportProcess) runRelayService() (retErr error) {
	...
	peerListener, err := process.importOrCreateListener(
		ListenerRelayPeer,
		netip.AddrPortFrom(netip.IPv4Unspecified(), defaults.RelayPeerListenPort).String(),
	)
	if err != nil {
		return trace.Wrap(err)
	}
	...
	peerAddrPort := netip.AddrPortFrom(hostNetIP.Unmap(), peerListener.Addr().(*net.TCPAddr).AddrPort().Port())

...
func (process *TeleportProcess) runRelayService() (retErr error) {
	...
	tunnelGRPCServer := grpc.NewServer(
		...
		grpc.Creds(tunnelServer.GRPCServerCredentials()),
	)
	defer tunnelGRPCServer.Stop()

	relaytunnelv1alpha.RegisterDiscoveryServiceServer(tunnelGRPCServer, &relayapi.StaticDiscoverServiceServer{
	...
	peerServer, err := relaypeer.NewServer(relaypeer.ServerConfig{
		Log: log.With("subcomponent", "peer_server"),
		GetCertificate: func(ctx context.Context) (*tls.Certificate, error) {
			return conn.serverGetCertificate()
		},
		GetPool: func(ctx context.Context) (*x509.CertPool, error) {
	...
	transportCreds, err := auth.NewTransportCredentials(auth.TransportCredentialsConfig{
	...
	relayRouter, err := proxy.NewRelayRouter(proxy.RelayRouterConfig{
		...
		PeerDial: relaypeer.ClientConfig{
			...
			AccessPoint: accessPoint,
			Log:         log.With("subcomponent", "relay_router"),
	...
	// TODO(espadolini): improve guessing and make it configurable
	hostIP, err := utils.GuessHostIP()
	if err != nil {
		return trace.Wrap(err)
	}
	hostNetIP, _ := netip.AddrFromSlice(hostIP)
	peerAddrPort := netip.AddrPortFrom(hostNetIP.Unmap(), uint16(peerListener.Addr().(*net.TCPAddr).Port))

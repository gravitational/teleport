...
func (process *TeleportProcess) initKubernetes() {
	logger := process.logger.With(trace.Component, teleport.CompoundComponent(teleport.ComponentKube, process.id))
...
func (process *TeleportProcess) initKubernetesService(logger *slog.Logger, conn *Connector) (retErr error) {
	...
	if len(cfg.Kube.DynamicLabels) != 0 {
		dynLabels, err = labels.NewDynamic(process.ExitContext(), &labels.DynamicConfig{
			...
			Log:    process.log.WithField(trace.Component, teleport.CompoundComponent(teleport.ComponentKube, process.id)),
		})
		if err != nil {
			return trace.Wrap(err)
		}
	...
	authorizer, err := authz.NewAuthorizer(authz.AuthorizerOpts{
		...
		Logger:      process.log.WithField(trace.Component, teleport.CompoundComponent(teleport.ComponentKube, process.id)),
	})
	if err != nil {
		return trace.Wrap(err)
	}

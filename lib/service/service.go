...
func validateScopedAppRegistration(isScoped bool, appName string, publicAddr, cloud string, mcp *types.MCP, llm *types.LLM, requiredApps []string, aws *servicecfg.AppAWS) error {
	...
	if len(requiredApps) > 0 {
		return trace.BadParameter("app %q is scoped and does not specifying required app names", appName)
	}
	if aws != nil {
		return trace.BadParameter("app %q is scoped and does not aws applications", appName)
	}
	return nil
}

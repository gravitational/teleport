...
func (process *TeleportProcess) initInstance() error {
	...
	if process.Config.Auth.Enabled && !hasNonAuthRole {
		...
		fmt.Printf("---> Not creating instance client (local auth only)\n")
		return nil
	}
	fmt.Printf("---> Creating instance client (contains non-auth system role)\n")
...
func (process *TeleportProcess) RegisterWithAuthServer(role types.SystemRole, eventName string) {
	...
	process.RegisterCriticalFunc(fmt.Sprintf("register.%v", serviceName), func() error {
		...
		if role != types.RoleInstance && role != types.RoleAuth {
			fmt.Printf("---> Waiting for instance identity event in order to furnish %q register with client...", role)
			conn, err := process.WaitForConnector(InstanceIdentityEvent, process.log)
			if conn == nil {
				return trace.Wrap(err)
			}
			connector.Client = conn.Client
			fmt.Printf("---> Successfully furnished %q connector with instance client...", role)
		}

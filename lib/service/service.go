...
func (process *TeleportProcess) initProxyEndpoint(conn *Connector) error {
	...
	if (!listeners.db.Empty() || alpnRouter != nil) && !process.Config.Proxy.DisableReverseTunnel {
		...
		if alpnRouter != nil && !cfg.Proxy.DisableDatabaseProxy {
			alpnRouter.Add(alpnproxy.HandlerDecs{
				MatchFunc:           alpnproxy.MatchByALPNPrefix(alpncommon.ProtocolMySQL),
	...
	if !cfg.Proxy.DisableTLS && !cfg.Proxy.DisableALPNSNIListener && listeners.web != nil {
		authDialerService := alpnproxyauth.NewAuthProxyDialerService(tsrv, clusterName, utils.NetAddrsToStrings(process.Config.AuthServers)[0])
		alpnRouter.Add(alpnproxy.HandlerDecs{
			MatchFunc:           alpnproxy.MatchByALPNPrefix(alpncommon.ProtocolAuth),
...
func (process *TeleportProcess) setupProxyTLSConfig(conn *Connector, tsrv reversetunnel.Server, accessPoint auth.ReadProxyAccessPoint, clusterName string) (*tls.Config, error) {
	...
	tlsConfig.NextProtos = apiutils.Deduplicate(append(tlsConfig.NextProtos, alpncommon.SupportedProtocols...))
	...
	tlsConfig.GetConfigForClient = func(info *tls.ClientHelloInfo) (*tls.Config, error) {
		...
		// access CLI clients.
		var err error
		tlsClone.ClientCAs, _, err = auth.DefaultClientCertPool(info.Context(), accessPoint, clusterName)
		if err != nil {
			return nil, trace.Wrap(err)
		}
...
func setupALPNRouter(listeners *proxyListeners, serverTLSConf *tls.Config, cfg *Config) *alpnproxy.Router {
	...
	router.Add(alpnproxy.HandlerDecs{
		MatchFunc: alpnproxy.MatchByALPNPrefix(alpncommon.ProtocolProxyGRPC),
...
func (process *TeleportProcess) singleProcessModeResolver(mode types.ProxyListenerMode) reversetunnel.Resolver {
	return func(context.Context) (*utils.NetAddr, bool, error) {
		...
		if !ok {
			return nil, false, trace.BadParameter(`failed to find reverse tunnel address, if running in single process mode, make sure "auth_service", "proxy_service", and "app_service" are all enabled`)
		}
		return addr, mode == types.ProxyListenerMode_Multiplex, nil
	}
}

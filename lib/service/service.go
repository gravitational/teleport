...
func (process *TeleportProcess) initAuthService() error {
	...
	// and outages.
	{
		...
		process.RegisterFunc("auth.wait-for-event-stream", func() error {
			if err := WaitForEventStream(process.ExitContext(), WaitForEventStreamConfig{
				Watcher: authServer,
...
// EventWatcher is an interface for creating event watchers.
type eventWatcher interface {
	NewWatcher(ctx context.Context, watch types.Watch) (types.Watcher, error)
}

// WaitForEventStreamConfig contains configuration for waiting for an event stream.
type WaitForEventStreamConfig struct {
...
func (c *WaitForEventStreamConfig) checkAndSetDefaults() error {
	if c.Watcher == nil {
...
// WaitForEventStream waits for an event stream to be ready by creating a watcher
...
func WaitForEventStream(ctx context.Context, cfg WaitForEventStreamConfig) error {
...
func waitForInitEvent(ctx context.Context, w types.Watcher, cfg WaitForEventStreamConfig, attempt int, start time.Time) error {
	defer w.Close()
	select {
	case evt := <-w.Events():

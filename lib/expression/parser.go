...
func DefaultParserSpec[evaluationEnv any]() typical.ParserSpec[evaluationEnv] {
	return typical.ParserSpec[evaluationEnv]{
		Functions: map[string]typical.Function{
			...
			"regexp.not_match_any": typical.BinaryFunction[evaluationEnv](
				func(inputs Set, expression string) (bool, error) {
					match, err := utils.RegexNotMatchesAny(inputs.items(), expression)
					if err != nil {
						return false, trace.Wrap(err, "invalid regular expression %q", expression)
					}
					return match, nil
				}),
			"regexp.not_match_all": typical.BinaryFunction[evaluationEnv](
				func(inputs Set, expression string) (bool, error) {
					match, err := utils.RegexNotMatchesAll(inputs.items(), expression)
					if err != nil {
						return false, trace.Wrap(err, "invalid regular expression %q", expression)
					}
					return match, nil

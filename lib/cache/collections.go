...
func (userNotificationExecutor) upsert(ctx context.Context, cache *Cache, notification *notificationsv1.Notification) error {
	...
	if _, err := cache.notificationsCache.CreateUserNotification(ctx, notification); err != nil {
		if !trace.IsAlreadyExists(err) {
			return trace.Wrap(err)
		}

		if err := cache.notificationsCache.DeleteUserNotification(ctx, username, notification.GetMetadata().GetName()); err != nil {
			return trace.Wrap(err)
		}
...
func (userNotificationExecutor) delete(ctx context.Context, cache *Cache, resource types.Resource) error {
	...
	switch r := resource.(type) {
	case types.Resource153Unwrapper:
		switch notification := r.Unwrap().(type) {
...
func (globalNotificationExecutor) upsert(ctx context.Context, cache *Cache, notification *notificationsv1.GlobalNotification) error {
	if _, err := cache.notificationsCache.CreateGlobalNotification(ctx, notification); err != nil {
		if !trace.IsAlreadyExists(err) {
			return trace.Wrap(err)
		}

		if err := cache.notificationsCache.DeleteGlobalNotification(ctx, notification.GetMetadata().GetName()); err != nil {
			return trace.Wrap(err)
		}
...
func (globalNotificationExecutor) delete(ctx context.Context, cache *Cache, resource types.Resource) error {
	...
	case types.Resource153Unwrapper:
		...
		case *notificationsv1.GlobalNotification:
			notificationId := notification.GetSpec().GetNotification().GetSpec().GetId()

...
func testResources[T types.Resource](t *testing.T, p *testPack, funcs testFuncs[T]) {
	...
	if funcs.update != nil {
		// If cache get is specified get the fresh object state.
		// This handles the case when the backend updated flow
		// does the CAS operation. So before updating we need to
		// have up-to-date object state with valid revision.
		if funcs.cacheGet != nil {
			var err error

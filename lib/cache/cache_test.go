...
// legacyListAdapter adapts collection getters that do not support pagination to conform to [testFuncs] interface
func legacyListAdapter[T any](fn func(context.Context) ([]T, error)) func(context.Context, int, string) ([]T, string, error) {
...
func testResourcePagination[T any](t *testing.T, p *testPack, funcs testFuncs[T]) {
	...
	require.EventuallyWithT(t, func(t *assert.CollectT) {
		...
		assert.Equal(t, len(expected), len(items))

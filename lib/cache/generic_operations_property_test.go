...
func TestGenericLister_ReturnsEmptyTokenWhenNoMoreItems(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		returned := 0
		for returned < n {
			pageSize := rapid.IntRange(1, defaults.DefaultChunkSize*2).Draw(t, "pageSize")
...
func TestGenericLister_BadPageSizeFallsBackToDefault(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		require.Empty(t, cmp.Diff(wantItems, gotItems, cmpopts.SortSlices(func(a, b int) bool {
			return a < b
		})))
...
func TestGenericLister_NeverReturnsMoreThanValidLimit(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		items, _, err := fixture.lister.list(t.Context(), pageSize, start)
		require.NoError(t, err)
...
func TestGenericLister_ReturnsAllItems(t *testing.T) {
	rapid.Check(t, func(t *rapid.T) {
		...
		// All items are returned but the order is unspecifed.
		require.Empty(t, cmp.Diff(want, got, cmpopts.SortSlices(func(a, b int) bool {
			return a < b
		})))
	})
}

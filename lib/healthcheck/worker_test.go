...
func Test_newWorker(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			cfg: workerConfig{
				...
				Target: Target{
					Resource:             db,
					GetUpdatedResourceFn: func() types.ResourceWithLabels { return db },
					ResolverFn: func(ctx context.Context) ([]string, error) {
						return []string{db.GetURI()}, nil
					},
				},
				healthUpdateCh: make(chan types.TargetHealth, 1),
			},
			wantHealth: types.TargetHealth{
				Address:          "",
				Protocol:         "",
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			...
			select {
			case health := <-w.healthUpdateCh:
				require.Empty(t, cmp.Diff(test.wantHealth, health,
					cmpopts.IgnoreFields(types.TargetHealth{}, "TransitionTimestamp")),
				)
			case <-time.After(time.Second):
				require.FailNow(t, "timed out waiting for health update")
			}
		})
	}
}
	...
	for _, test := range tests {
		t.Run(test.desc, func(t *testing.T) {
			w := &worker{
				...
				healthCheckCfg: &healthCheckConfig{},
				log:            slog.Default(),
				target:         Target{ResolverFn: test.resolverFn},
			}

...
type workerConfig struct {
	...
	// healthUpdateCh is used to send/receive each target health update - only
	// used in tests.
	healthUpdateCh chan types.TargetHealth
}
...
func (cfg *workerConfig) checkAndSetDefaults() error {
	...
	cfg.Log = cfg.Log.With(
		"target_name", cfg.Target.Resource.GetName(),
		"target_kind", cfg.Target.Resource.GetKind(),
		"target_origin", cfg.Target.Resource.Origin(),
	)
	return nil
}
...
func newWorker(ctx context.Context, cfg workerConfig) (*worker, error) {
	...
	w := &worker{
		...
		log:                       cfg.Log,
		target:                    cfg.Target,
		healthUpdateCh:            cfg.healthUpdateCh,
	}
...
type worker struct {
	...
	// log emits logs.
	log *slog.Logger
	// dialFn is used to override the dialer in tests.
	dialFn dialFunc
	...
	// mu guards concurrent access to the target resource and its health.
	mu sync.RWMutex
...
func (w *worker) GetTargetResource() types.ResourceWithLabels {
	w.mu.RLock()
	defer w.mu.RUnlock()
	return w.target.GetUpdatedResourceFn()
}
...
func (w *worker) dialEndpoints(ctx context.Context) error {
	...
	case 1:
		return w.dialEndpoint(ctx, dialFn, endpoints[0])
	default:
		group, ctx := errgroup.WithContext(ctx)
		for _, ep := range endpoints {
			group.Go(func() error {
				return trace.Wrap(w.dialEndpoint(ctx, dialFn, ep))
			})
		}
		return group.Wait()
	}
...
func (w *worker) dialEndpoint(ctx context.Context, dialFn dialFunc, endpoint string) error {
	conn, err := dialFn(ctx, "tcp", endpoint)
	if err != nil {
		return trace.Wrap(err)
	}
...
func (w *worker) setTargetHealthStatus(newStatus types.TargetHealthStatus, reason types.TargetHealthTransitionReason, message string) {
	...
	if w.healthUpdateCh != nil {
		w.healthUpdateCh <- w.targetHealth
	}
}

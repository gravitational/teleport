...
func TestDialCygwinEvictsStaleUID(t *testing.T) {
	...
	const staleUID = 9999
	socketFile, _ := startFakeCygwinAgent(t, keyring, func(uint32) bool {
		return false
	})

	storeCygwinUID(staleUID, true)

	// Reject the cached UID and all fallback UIDs so the dial fails on every
	// platform.

...
// TestSingleRequestClient verifies that a single request agent client
...
func TestSingleRequestClient(t *testing.T) {
	...
	// Serve the keyring over a unix socket.
	...
	// client closes the connection before a request returns, so the counts can be compared
	...
	src := sshagent.NewSingleRequestClient(func() (sshagent.Client, error) {
		return sshagent.NewClient(func() (io.ReadWriteCloser, error) {
			...
			opens.Add(1)
			return &countingConn{Conn: conn, closes: &closes}, nil
		})
	})

	requireNoOpenConns := func(t *testing.T) {
		t.Helper()
		require.Equal(t, opens.Load(), closes.Load(), "single-request agent client leaked a connection to the agent")
	}

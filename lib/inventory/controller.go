...
func (c *Controller) handleControlStream(handle *upstreamHandle) {
	...
	var winServiceKeepAliveDelay *delay.Delay
	...
	for {
		...
		case msg := <-handle.Recv():
			...
			case *proto.InventoryHeartbeat:
				...
				if m.HasWindowsDesktopService() {
					if winServiceKeepAliveDelay == nil {
						winServiceKeepAliveDelay = c.createKeepAliveDelay(c.winServiceHBVariableDuration)
					}

					if err := c.handleWindowsDesktopServiceHB(handle, m.GetWindowsDesktopService(), winServiceKeepAliveDelay); err != nil {
						handle.CloseWithError(err)
						return
					}
				}
		...
		case now := <-winServiceKeepAliveDelay.Elapsed():
			winServiceKeepAliveDelay.Advance(now)

			if err := c.keepAliveWindowsDesktopService(handle, now); err != nil {
				handle.CloseWithError(err)
				return
			}
			c.testEvent(keepAliveWindowsServiceTick)
	...
	if handle.windowsDesktopService != nil {
		...
		if err == nil || trace.IsNotFound(err) {
			...
			slog.WarnContext(c.closeContext, "Failed to delete windows desktop service on termination",
				"windows_desktop_service", handle.Hello().GetServerID(),
				"error", err,
			)
		}
	}
...
func (c *Controller) handleWindowsDesktopServiceHB(handle *upstreamHandle, windowsDesktopService *types.WindowsDesktopServiceV3, windowsServiceDelay *delay.Delay) error {
	...
	if handle.PeerAddr() != "" {
		windowsDesktopService.Spec.Addr = utils.ReplaceLocalhost(windowsDesktopService.GetAddr(), handle.PeerAddr())
	}
	...
	handle.windowsDesktopService.resource.SetExpiry(c.clock.Now().Add(c.serverTTL).UTC())
...
func (c *Controller) keepAliveWindowsDesktopService(handle *upstreamHandle, now time.Time) error {
	...
	handle.windowsDesktopService.resource.SetExpiry(now.Add(c.serverTTL).UTC())
	if _, err := c.auth.UpsertWindowsDesktopService(c.closeContext, handle.windowsDesktopService.resource); err == nil {
		if handle.windowsDesktopService.retryUpsert {
		...
		handle.windowsDesktopService.keepAliveErrs++
		closing := handle.windowsDesktopService.keepAliveErrs > c.maxKeepAliveErrs
		slog.WarnContext(c.closeContext, "Failed to upsert windows desktop service on keepalive",
			"server_id", handle.Hello().GetServerID(),
			"error", err,
			"count", handle.windowsDesktopService.keepAliveErrs,
			"closing", closing,
		)

...
func Func[T any](fn func() (T, error)) iter.Seq2[T, error] {
...
func CollectPages[T any, S ~[]T](stream iter.Seq2[S, error]) ([]T, error) {
	var c []T
...
func FilterMap[A, B any](stream iter.Seq2[A, error], fn func(A) (B, bool)) iter.Seq2[B, error] {
...
func Chain[T any](streams ...iter.Seq2[T, error]) iter.Seq2[T, error] {
	return func(yield func(T, error) bool) {
...
func Chunks[T any](stream iter.Seq2[T, error], size int) iter.Seq2[[]T, error] {
	if size < 1 {
		return Empty[[]T]()
	}
	return func(yield func([]T, error) bool) {
...
func Once[T any](item T) iter.Seq2[T, error] {
	return func(yield func(T, error) bool) {
		yield(item, nil)
	}
}
...
func OnceFunc[T any](fn func() (T, error)) iter.Seq2[T, error] {
	return func(yield func(T, error) bool) {
		item, err := fn()
		if errors.Is(err, io.EOF) {
			return
...
func Drain[T any](stream iter.Seq2[T, error]) error {
...
func Slice[T any, S ~[]T](items S) iter.Seq2[T, error] {
	return func(yield func(T, error) bool) {
...
func PageFunc[T any](fn func() ([]T, error)) iter.Seq2[T, error] {
	return func(yield func(T, error) bool) {
		for {
			items, err := fn()
			if err != nil {
...
// have no effect.
func Skip[T any](stream iter.Seq2[T, error], n int) iter.Seq2[T, error] {
	return func(yield func(T, error) bool) {
		n := n // copy
		for item, err := range stream {
			if err != nil {
...
func Flatten[T any](stream iter.Seq2[iter.Seq2[T, error], error]) iter.Seq2[T, error] {
	return func(yield func(T, error) bool) {
...
func MapErr[T any](stream iter.Seq2[T, error], fn func(error) error) iter.Seq2[T, error] {
	return func(yield func(T, error) bool) {
		for item, err := range stream {
			if err != nil {
				mappedErr := fn(err)
...
func RateLimit[T any](stream iter.Seq2[T, error], wait func() error) iter.Seq2[T, error] {
	return func(yield func(T, error) bool) {
		for item, err := range stream {
			if err != nil {
				yield(*new(T), trace.Wrap(err))
...
func MergeStreams[T any](
	streamA iter.Seq2[T, error],
	streamB iter.Seq2[T, error],
...
func TakeWhile[T any](stream iter.Seq2[T, error], predicate func(T) bool) iter.Seq2[T, error] {
	return func(yield func(T, error) bool) {
		for item, err := range stream {
			if err != nil {
				yield(*new(T), trace.Wrap(err))

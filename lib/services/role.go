...
func (set RoleSet) checkAccess(
	...
	if state.MFARequired == MFARequiredAlways && (os.Getenv(teleport.EnvVarForceInBandMFA) == "yes" || !state.MFAVerified) {
	...
	bypassMFAChecks := state.MFARequired == MFARequiredNever ||
		(state.MFAVerified && (os.Getenv(teleport.EnvVarForceInBandMFA) != "yes" || !state.ReturnPreconditions))

...
func (set RoleSet) checkAccess(
	...
	if state.MFARequired == MFARequiredAlways && (os.Getenv(mfa.ForceInBandEnv) == "yes" || !state.MFAVerified) {
	...
	bypassMFAChecks := state.MFARequired == MFARequiredNever ||
		(state.MFAVerified && (os.Getenv(mfa.ForceInBandEnv) != "yes" || !state.ReturnPreconditions))

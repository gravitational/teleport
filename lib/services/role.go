...
func ApplyTraits(r types.Role, traits map[string][]string) types.Role {
	for _, condition := range []types.RoleConditionType{Allow, Deny} {
		...
		// apply template to requestable roles
		inRequestCond := r.GetAccessRequestConditions(condition)
		var outRequestCond types.AccessRequestConditions
		outRequestCond.Roles = applyValueTraitsSlice(inRequestCond.Roles, traits, "access request role")
		r.SetAccessRequestConditions(condition, outRequestCond)

		// apply template to access review permissions
		inReviewCond := r.GetAccessReviewConditions(condition)
		var outReviewCond types.AccessReviewConditions
		outReviewCond.Roles = applyValueTraitsSlice(inReviewCond.Roles, traits, "access review role")
		r.SetAccessReviewConditions(condition, outReviewCond)
	}

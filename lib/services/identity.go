...
type Identity interface {
	...
	// user specified with this identity
	GetUserBySAMLIdentity(id SAMLIdentity) (User, error)
	...
	UpsertSAMLConnector(connector SAMLConnector, ttl time.Duration) error
	...
	GetSAMLAuthRequest(stateToken string) (*SAMLAuthRequest, error)
...
// SAMLIdentity is OpenID Connect identity that is linked
// to particular user and connector and lets user to log in using external
// credentials, e.g. google
type SAMLIdentity struct {
	// ConnectorID is id of registered SAML connector, e.g. 'google-example.com'
	ConnectorID string `json:"connector_id"`

	// Email is SAML verified email claim
	// e.g. bob@example.com
	Email string `json:"username"`
}

const SAMLIDentitySchema = `{
  "type": "object",
...
func (i *SAMLIdentity) String() string {
	return fmt.Sprintf("SAMLIdentity(connectorID=%v, email=%v)", i.ConnectorID, i.Email)
}

// Equals returns true if this identity equals to passed one
func (i *SAMLIdentity) Equals(other *SAMLIdentity) bool {
	return i.ConnectorID == other.ConnectorID && i.Email == other.Email
}

// Check returns nil if all parameters are great, err otherwise
func (i *SAMLIdentity) Check() error {
	if i.ConnectorID == "" {
...
type SAMLAuthRequest struct {
	// ConnectorID is ID of SAML connector this request uses
	ConnectorID string `json:"connector_id"`

...
type OIDCAuthRequest struct {
	...
	// State is the state of this request which holds information about user session
	State OIDCAuthRequestState `json:"state"`
...
// OIDCAuthRequestState describes a state of OIDC request
type OIDCAuthRequestState struct {
	// ID is this auth. request state ID
	ID string
	// CSRFToken is this auth. request CSRF token
	CSRFToken string
}
...
func (i *OIDCAuthRequest) Check() error {
	...
	if i.State.ID == "" {
		return trace.BadParameter("State: missing ID")
	}

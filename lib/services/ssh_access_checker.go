...
// scoped role proto with ssh.client_idle_timeout takes precedence over defaults.client_idle_timeout). If the
// role specifies a more restrictive value it is returned; otherwise the global value is returned unchanged.
func (c *SSHAccessChecker) AdjustClientIdleTimeout(timeout time.Duration) time.Duration {
	if !c.checker.isScoped() {

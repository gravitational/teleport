...
type UserNotificationCache struct {
	...
	primaryCache *sortcache.SortCache[*notificationsv1.Notification, notificationsCacheIndex, string]
...
type GlobalNotificationCache struct {
	rw           sync.RWMutex
	cfg          NotificationCacheConfig
	primaryCache *sortcache.SortCache[*notificationsv1.GlobalNotification, notificationsCacheIndex, string]
	ttlCache     *utils.FnCache
	initC        chan struct{}
	closeContext context.Context
	cancel       context.CancelFunc
...
func (c *UserNotificationCache) fetch(ctx context.Context) (*sortcache.SortCache[*notificationsv1.Notification, notificationsCacheIndex, string], error) {
	cache := sortcache.New(sortcache.Config[*notificationsv1.Notification, notificationsCacheIndex, string]{
...
func (c *UserNotificationCache) read(ctx context.Context) (*sortcache.SortCache[*notificationsv1.Notification, notificationsCacheIndex, string], error) {
	c.rw.RLock()
	...
	temp, err := utils.FnCacheGet(ctx, c.ttlCache, "user-notification-cache", func(ctx context.Context) (*sortcache.SortCache[*notificationsv1.Notification, notificationsCacheIndex, string], error) {
		return c.fetch(ctx)
	})
...
// to create a temporary cache as a fallback in case the primary cache is ever unhealthy.
func (c *GlobalNotificationCache) fetch(ctx context.Context) (*sortcache.SortCache[*notificationsv1.GlobalNotification, notificationsCacheIndex, string], error) {
	cache := sortcache.New(sortcache.Config[*notificationsv1.GlobalNotification, notificationsCacheIndex, string]{
...
// read gets a read-only view into a valid cache state. it prefers reading from the primary cache, but will fallback
// to a periodically reloaded temporary state when the primary state is unhealthy.
func (c *GlobalNotificationCache) read(ctx context.Context) (*sortcache.SortCache[*notificationsv1.GlobalNotification, notificationsCacheIndex, string], error) {
	c.rw.RLock()
	primary := c.primaryCache
	c.rw.RUnlock()
	...
	temp, err := utils.FnCacheGet(ctx, c.ttlCache, "global-notification-cache", func(ctx context.Context) (*sortcache.SortCache[*notificationsv1.GlobalNotification, notificationsCacheIndex, string], error) {
		return c.fetch(ctx)
	})

...
func NewReviewPermissionChecker(
	...
	// load all roles for the user and
...
// push combines statically assigned review roles with
// review roles generated by traits interpolation.
func (c *ReviewPermissionChecker) push(role types.Role) error {
	...
	var err error
	outDenied := apiutils.Deduplicate(applyValueTraitsSlice(deny.Roles, c.UserState.GetTraits(), "deny review roles traits interpolation"))
	c.Roles.DenyReview[deny.Where], err = appendRoleMatchers(c.Roles.DenyReview[deny.Where], outDenied, deny.ClaimsToRoles, c.UserState.GetTraits())
	if err != nil {
		return trace.Wrap(err)
	}

	outAllowed := apiutils.Deduplicate(applyValueTraitsSlice(allow.Roles, c.UserState.GetTraits(), "allow review roles traits interpolation"))
	c.Roles.AllowReview[allow.Where], err = appendRoleMatchers(c.Roles.AllowReview[allow.Where], outAllowed, allow.ClaimsToRoles, c.UserState.GetTraits())
	if err != nil {
		return trace.Wrap(err)
	}

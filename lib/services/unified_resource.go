	...
	return func(yield func(iteratedItem, error) bool) {
		...
		err := c.read(ctx, func(cache *UnifiedResourceCache) error {
			...
			iterateRange(&item{Key: startKey}, &item{Key: endKey}, func(item *item) bool {
				...
				if c.itemKindMatches(r, kindsMap) {
					return yield(iteratedItem{key: item.Key, resource: r}, nil)
				}
...
// IterateResources iterates over all resources from the start key that match
// one of the provided kinds. If no kinds are provided, resources of all supported
// kinds are returned.
func (c *UnifiedResourceCache) IterateResources(ctx context.Context, start string, sortBy types.SortBy, kinds ...string) iter.Seq2[types.ResourceWithLabels, error] {
...
// IterateNodes iterates over all cached nodes starting from the provided key.
func (c *UnifiedResourceCache) IterateNodes(ctx context.Context, start string, sortBy types.SortBy) iter.Seq2[types.Server, error] {
	return iterateUnifiedResourceCache(ctx, c, start, sortBy, types.KindNode, types.Server.DeepCopy)
}

// IterateAppServers iterates over all cached app servers starting from the provided key.
func (c *UnifiedResourceCache) IterateAppServers(ctx context.Context, start string, sortBy types.SortBy) iter.Seq2[types.AppServer, error] {
	return iterateUnifiedResourceCache(ctx, c, start, sortBy, types.KindApp, types.AppServer.Copy)
}

// IterateDatabaseServers iterates over all cached database servers starting from the provided key.
func (c *UnifiedResourceCache) IterateDatabaseServers(ctx context.Context, start string, sortBy types.SortBy) iter.Seq2[types.DatabaseServer, error] {
	return iterateUnifiedResourceCache(ctx, c, start, sortBy, types.KindDatabase, types.DatabaseServer.Copy)
}

// IterateKubernetesServers iterates over all cached Kubernetes servers starting from the provided key.
func (c *UnifiedResourceCache) IterateKubernetesServers(ctx context.Context, start string, sortBy types.SortBy) iter.Seq2[types.KubeServer, error] {
	return iterateUnifiedResourceCache(ctx, c, start, sortBy, types.KindKubernetesCluster, types.KubeServer.Copy)
}

// IterateWindowsDesktops iterates over all cached windows desktops starting from the provided key.
func (c *UnifiedResourceCache) IterateWindowsDesktops(ctx context.Context, start string, sortBy types.SortBy) iter.Seq2[types.WindowsDesktop, error] {
	return iterateUnifiedResourceCache(ctx, c, start, sortBy, types.KindWindowsDesktop, func(desktop types.WindowsDesktop) types.WindowsDesktop { return desktop.Copy() })
}

// IterateGitServers iterates over all cached git servers starting from the provided key.
func (c *UnifiedResourceCache) IterateGitServers(ctx context.Context, start string, sortBy types.SortBy) iter.Seq2[types.Server, error] {
	return iterateUnifiedResourceCache(ctx, c, start, sortBy, types.KindGitServer, types.Server.DeepCopy)
}

// IterateSAMLIdPServiceProvider iterates over all cached sAML IdP service providers starting from the provided key.
func (c *UnifiedResourceCache) IterateSAMLIdPServiceProvider(ctx context.Context, start string, sortBy types.SortBy) iter.Seq2[types.SAMLIdPServiceProvider, error] {
	return iterateUnifiedResourceCache(ctx, c, start, sortBy, types.KindSAMLIdPServiceProvider, types.SAMLIdPServiceProvider.Copy)
}

// IterateIdentityCenterAccount iterates over all cached identity center accounts starting from the provided key.
func (c *UnifiedResourceCache) IterateIdentityCenterAccount(ctx context.Context, start string, sortBy types.SortBy) iter.Seq2[*identitycenterv1.Account, error] {
	...
	return func(yield func(*identitycenterv1.Account, error) bool) {
		for account, err := range iterateUnifiedResourceCache[IdentityCenterAccount](ctx, c, start, sortBy, types.KindIdentityCenterAccount, cloneFn) {
			if err != nil {
				yield(nil, err)
				return
			}
...
// IterateIdentityCenterAccountAssignment iterates over all cached identity center account assignments starting from the provided key.
func (c *UnifiedResourceCache) IterateIdentityCenterAccountAssignment(ctx context.Context, start string, sortBy types.SortBy) iter.Seq2[*identitycenterv1.AccountAssignment, error] {
	// cloning is performed on the concrete resource below instead of
	// on the wrapper type.
	...
	return func(yield func(*identitycenterv1.AccountAssignment, error) bool) {
		for assignment, err := range iterateUnifiedResourceCache[IdentityCenterAccountAssignment](ctx, c, start, sortBy, types.KindIdentityCenterAccountAssignment, cloneFn) {
			if err != nil {
				yield(nil, err)
				return
			}
...
func iterateUnifiedResourceCache[T any](ctx context.Context, c *UnifiedResourceCache, start string, sortBy types.SortBy, kind string, cloneFn func(T) T) iter.Seq2[T, error] {
	return func(yield func(T, error) bool) {
		for i, err := range c.iterateItems(ctx, start, sortBy, kind) {
			if err != nil {
				var t T
				yield(t, err)
				return
...
func (c *UnifiedResourceCache) IterateUnifiedResources(ctx context.Context, matchFn func(types.ResourceWithLabels) (bool, error), req *proto.ListUnifiedResourcesRequest) ([]types.ResourceWithLabels, string, error) {
	...
	for item, err := range c.iterateItems(ctx, req.StartKey, req.SortBy, req.Kinds...) {
		...
		if match {
			if len(resources) == int(req.Limit) {
...
func (c *UnifiedResourceCache) itemKindMatches(r resource, kinds map[string]struct{}) bool {
	...
	case types.KindKubeServer, types.KindKubernetesCluster:
		_, ok := kinds[types.KindKubernetesCluster]
		return ok
	case types.KindDatabaseServer:
		_, ok := kinds[types.KindDatabase]
		return ok
...
func (c *UnifiedResourceCache) GetUnifiedResources(ctx context.Context) ([]types.ResourceWithLabels, error) {
	var resources []types.ResourceWithLabels
	for resource, err := range c.IterateResources(ctx, prefix, types.SortBy{IsDesc: false, Field: sortByName}) {
		if err != nil {
			return nil, trace.Wrap(err)
		}

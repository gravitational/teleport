...
func (c *UnifiedResourceCache) IdentityCenterAccounts(ctx context.Context, params UnifiedResourcesIterateParams) iter.Seq2[types.AppServer, error] {
	return func(yield func(types.AppServer, error) bool) {
		for account, err := range iterateUnifiedResourceCache(ctx, c, params, types.KindIdentityCenterAccount, types.AppServer.Copy) {
			if err != nil {
				yield(nil, err)
				return
			}
...
func (c *UnifiedResourceCache) itemKindMatches(r resource, kinds map[string]struct{}) bool {
	...
	case types.KindAppServer:
		if r.(types.AppServer).GetApp().GetSubKind() == types.KindIdentityCenterAccount {

...
func newAccessRequestPack(t *testing.T, ctx context.Context) (accessRequestServices, *services.AccessRequestCache) {
...
func TestAccessRequestCacheSorting(t *testing.T) {
	ctx, cancel := context.WithCancel(context.Background())
	defer cancel()

	svcs, cache := newAccessRequestPack(t, ctx)

	// describe a set of basic test resources that we can use to
	// verify various sort scenarios.
	rrs := []struct {
		id    string
	...
	for _, tt := range tts {
		...
		require.Equal(t, tt.Expect, reqIDs)
	}
}

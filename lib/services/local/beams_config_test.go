...
func TestBeamsConfigService(t *testing.T) {
	...
	config := services.DefaultCloudManagedBeamsConfig()

	// Create
	created, err := service.CreateBeamsConfig(ctx, config)
	require.NoError(t, err)
	require.NotEmpty(t, created.GetMetadata().GetRevision())
	require.Equal(t, "anthropic", created.GetSpec().GetLlm().GetAnthropic().GetAppName())
	require.Equal(t, types.BeamsProvisionModeCloudManaged, created.GetSpec().GetLlm().GetAnthropic().GetProvisionMode())
	...
	require.Equal(t, "anthropic", got.GetSpec().GetLlm().GetAnthropic().GetAppName())
	require.Equal(t, "openai", got.GetSpec().GetLlm().GetOpenai().GetAppName())

	// Update
	toUpdate := proto.Clone(got).(*beamsv1.BeamsConfig)
	toUpdate.GetSpec().GetLlm().GetAnthropic().SetAppName("anthropic-byo-bedrock")
	toUpdate.GetSpec().GetLlm().GetAnthropic().SetProvisionMode(types.BeamsProvisionModeAWSOIDC)
	...
	require.Equal(t, types.BeamsProvisionModeAWSOIDC, updated.GetSpec().GetLlm().GetAnthropic().GetProvisionMode())
	...
	_, err = service.GetBeamsConfig(ctx)

...
// protojson marshal/unmarshal used by the generic backend service.
func TestDiscoveryServiceRoundTrip(t *testing.T) {
	...
	original := discoveryservicev1.DiscoveryService_builder{
		...
		Metadata: headerv1.Metadata_builder{
			...
			Expires: timestamppb.New(clock.Now().UTC().Add(90 * time.Second)),
		}.Build(),
		Spec: discoveryservicev1.DiscoveryServiceSpec_builder{
			...
			DiscardedMatchers: []*discoveryservicev1.DiscardedMatcher{
				discoveryservicev1.DiscardedMatcher_builder{
					Cloud:    "aws",
					Position: 1,
					Reason:   "ambient credentials are not allowed on Teleport Cloud; matcher has no integration",
				}.Build(),
			},
			BoundDiscoveryConfigs: []*discoveryservicev1.DiscoveryConfigBinding{
				discoveryservicev1.DiscoveryConfigBinding_builder{
					Name:  "rds-prod",
					State: "LOADED",
				}.Build(),
				discoveryservicev1.DiscoveryConfigBinding_builder{
					Name:           "eks-prod",
					State:          "ERROR",
					Error:          "integration \"acme-old\" not found",
					DiscardedTotal: 1,
				}.Build(),
			},
		}.Build(),

...
func oktaAssignmentValidationSuite(
	...
	a1.SetResourceStatus(types.OktaAssignmentStatus{
		Phase:       types.OktaAssignmentPhaseProvisioned,
		...
		Targets: types.OktaAssignmentStatusTargets{
			...
			Status: []types.OktaAssignmentStatusTargetStatus{
				...
				},
				{
					Type:        types.OktaAssignmentTargetTypeApplication,
					ID:          "test_id_2",
					Phase:       types.OktaAssignmentTargetPhaseImported,
					ProcessedAt: clock.Now(),
				},
			},
		},
	...
	msg = "invalid status.phase"

	status := newFilledStatus()
	status.Phase = "test_invalid_phase_1"
	a1.SetResourceStatus(status)
	_, err = testedFn(ctx, a1)
	require.ErrorContains(t, err, "status.phase: unknown value \"test_invalid_phase_1\"", msg)
	...
	msg = "when status not empty: .processed_at before any of .targets.status[*].processed_at"

	status = newFilledStatus()
	status.Targets.Status[1].ProcessedAt = status.ProcessedAt.Add(1)
	a1.SetResourceStatus(status)
	_, err = testedFn(ctx, a1)
	require.ErrorContains(t, err, "status.targets.status[1].processed_at: is after status.processed_at", msg)

	msg = "when status not empty: .targets.stats.total smaller than number of target statuses"
	...
	a1.SetResourceStatus(status)
	_, err = testedFn(ctx, a1)
	require.ErrorContains(t, err, "status.targets.stats.total: [2] not equal to len(status.targets.status) [3]", msg)

	msg = "when status not empty: status.targets.stats.total smaller than number of spec.targets"
	...
	msg = "when status not empty: status.targets.stats.total bigger than number of spec.targets"
	...
	msg = "when status not empty: .targets.stats.total smaller than .provisioned + .failed"
	...
	a1.SetResourceStatus(status)
	_, err = testedFn(ctx, a1)
	require.ErrorContains(t, err, "status.targets.stats: .total [2] not equal to .provisioned [1] + .failed [3]", msg)
}

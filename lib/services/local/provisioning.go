...
func (s *ProvisioningService) GetToken(token string) (services.ProvisionToken, error) {
	...
	return services.UnmarshalProvisionToken(item.Value, services.SkipValidation(), services.WithResourceID(item.ID))
}
...
func (s *ProvisioningService) GetTokens(opts ...services.MarshalOption) ([]services.ProvisionToken, error) {
	...
	for i, item := range result.Items {
		t, err := services.UnmarshalProvisionToken(item.Value, services.AddOptions(opts, services.SkipValidation(), services.WithResourceID(item.ID))...)
		if err != nil {
			return nil, trace.Wrap(err)
		}
		tokens[i] = t

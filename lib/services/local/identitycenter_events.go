...
type identityCenterCustomPermissionSetParser struct {
	baseParser
	prefix backend.Key
}

func newIdentityCenterCustomPermissionSetParser() *identityCenterCustomPermissionSetParser {
	prefix := backend.NewKey(awsResourcePrefix, awsCustomPermissionSetPrefix)
	return &identityCenterCustomPermissionSetParser{
		baseParser: newBaseParser(prefix),
		prefix:     prefix,
	}
}

func (p *identityCenterCustomPermissionSetParser) parse(event backend.Event) (types.Resource, error) {
	switch event.Type {
	case types.OpDelete:
		name := event.Item.Key.TrimPrefix(p.prefix).String()
		if name == "" {
	...
	case types.OpPut:
		r, err := services.UnmarshalProtoResource[*identitycenterv1.CustomPermissionSet](
			event.Item.Value,
			services.WithExpires(event.Item.Expires),
			services.WithRevision(event.Item.Revision),
		)

...
// GetDynamicWindowsDesktops returns all Windows desktops matching filter.
func (s *WindowsDesktopService) GetDynamicWindowsDesktops(ctx context.Context, filter types.DynamicWindowsDesktopFilter) ([]types.DynamicWindowsDesktop, error) {
	...
	if filter.Name != "" && len(desktops) == 0 {
		return nil, trace.NotFound("dynamic windows desktop \"%s\" doesn't exist", filter.Name)
	}

	return desktops, nil
}

// CreateDynamicWindowsDesktop creates a windows desktop resource.
...
// UpdateDynamicWindowsDesktop updates a windows desktop resource.
...
// DeleteDynamicWindowsDesktop removes the specified windows desktop resource.
...
// DeleteAllDynamicWindowsDesktops removes all windows desktop resources.
...
// ListDynamicWindowsDesktops returns all Windows desktops matching filter.

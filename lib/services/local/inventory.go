...
func (s *PresenceService) GetInstances(ctx context.Context, req types.InstanceFilter) iter.Iter[types.Instance] {
	const pageSize = 10_000
	if req.ServerID != "" {
		instance, _, err := s.GetRawInstance(ctx, req.ServerID)
		if err != nil {
			if trace.IsNotFound(err) {
				return iter.Empty[types.Instance]()
			}
		...
		return iter.Once(instance)
	}

	startKey := backend.Key(instancePrefix, "")
	endKey := backend.RangeEnd(startKey)
	items := backend.IteratorOnRange(ctx, s, startKey, endKey, pageSize)
	return iter.FilterMap(items, func(item backend.Item) (types.Instance, bool) {

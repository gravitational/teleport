...
func TestRecoveryCodesCRUD(t *testing.T) {
	...
	t.Run("upsert, get, delete recovery codes", func(t *testing.T) {
		...
		codes, err := identity.GetRecoveryCodes(ctx, username, true)
		require.NoError(t, err)
		...
		codes, err = identity.GetRecoveryCodes(ctx, username, false)
		require.NoError(t, err)
	...
	t.Run("deleting user deletes recovery codes", func(t *testing.T) {
		...
		require.NoError(t, err)
		_, err = identity.GetRecoveryCodes(ctx, username, true)

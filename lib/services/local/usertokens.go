...
func (s *IdentityService) GetUserTokens(ctx context.Context) ([]services.UserToken, error) {
...
func (s *IdentityService) GetUserToken(ctx context.Context, tokenID string) (services.UserToken, error) {
	item, err := s.Get(ctx, backend.Key(userTokensPrefix, tokenID, paramsPrefix))
	if err != nil {
		if trace.IsNotFound(err) {
			return nil, trace.NotFound("usertoken(%v) not found", tokenID)
		}
		return nil, trace.Wrap(err)
	}
...
func (s *IdentityService) CreateUserToken(ctx context.Context, usertoken services.UserToken) (services.UserToken, error) {
...
func (s *IdentityService) GetUserTokenSecrets(ctx context.Context, tokenID string) (services.UserTokenSecrets, error) {
	item, err := s.Get(ctx, backend.Key(userTokensPrefix, tokenID, secretsPrefix))
	if err != nil {
		if trace.IsNotFound(err) {
			return nil, trace.NotFound("usertoken(%v) secrets not found", tokenID)
		}
		return nil, trace.Wrap(err)
	}
...
func (s *IdentityService) UpsertUserTokenSecrets(ctx context.Context, secrets services.UserTokenSecrets) error {

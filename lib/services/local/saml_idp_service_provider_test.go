...
func TestEntityDescriptorConfiguredPerServiceProviderSubkind(t *testing.T) {
	ctx := context.Background()
	backend, err := memory.New(memory.Config{
		Context: ctx,
		Clock:   clockwork.NewFakeClock(),
	...
	const acsURL = "https://sphost/saml"
	const entityID = "https://sphost/saml/metadata"
	tests := []struct {
		name             string
		subkind          string
		wantNameIDFormat string
	}{
		{
			name:             "Google Workspace",
			subkind:          samlserviceprovider.GoogleWorkspace,
			wantNameIDFormat: types.SAMLEmailAddressNameIDFormat,
		},
		{
			name:             "default type (empty subkind)",
			subkind:          "",
			wantNameIDFormat: types.SAMLUnspecifiedNameIDFormat,
		},
	}
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			require.Equal(t, test.wantNameIDFormat, getNameIDFormatFromSPSSODescriptor(ed.SPSSODescriptors))
		})
	}
}
	...
	for _, spSSODescriptor := range spSSODescriptors {
		for _, acs := range spSSODescriptor.AssertionConsumerServices {
			if acs.Binding == "urn:oasis:names:tc:SAML:2.0:bindings:HTTP-POST" {
				// exactly one NameID field is generated by Teleport.
				return string(spSSODescriptor.NameIDFormats[0])
			}
		}
	}
	return ""

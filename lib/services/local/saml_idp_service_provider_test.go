...
func TestCreateSAMLIdPServiceProvider_fetchOrGenerateEntityDescriptor(t *testing.T) {
	...
	testSPServer := httptest.NewTLSServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		default:
			...
			// than generated.
			spMetadata = newEntityDescriptor(fullURL)
			w.WriteHeader(http.StatusOK)
			fmt.Fprintln(w, spMetadata)
		}
	...
	require.Equal(t, strings.TrimSpace(spMetadata), strings.TrimSpace(spFromBackend.GetEntityDescriptor()))
	...
	spMetadata2 := newSAMLSPMetadata(notFoundURL, testSPServer.URL)

	expected, err := samlsp.ParseMetadata([]byte(spMetadata2))
	require.NoError(t, err)

	actual, err := samlsp.ParseMetadata([]byte(sp2FromBackend.GetEntityDescriptor()))
	require.NoError(t, err)

	// ignoring ValidUntil as the duration is set using time.Now() and will create flaky test.

...
import (
	...
	sr "github.com/gravitational/teleport/lib/scopes/role"
...
//
// Known holes: this model does not provide means of making one assignment invalidate another (e.g. in the case of OIDC assignments,
// for which only one should be valid at a time), and does no invalidate assignments on user deletion.
...
func (s *ScopedRoleService) CreateScopedRole(ctx context.Context, req *srpb.CreateScopedRoleRequest) (*srpb.CreateScopedRoleResponse, error) {
	...
	// TODO(fspmarshall): switch this to weak validate and add strong validate in API layer instead?
...
func (s *ScopedRoleService) UpdateScopedRole(ctx context.Context, req *srpb.UpdateScopedRoleRequest) (*srpb.UpdateScopedRoleResponse, error) {
	...
	revision, err := s.bk.AtomicWrite(ctx, []backend.ConditionalAction{
		{
			Key:       item.Key,
			Condition: backend.Revision(extant.Role.Metadata.Revision),
			Action:    backend.Put(item),
		},
		{
			Key:       scopedRoleRoleLockKey(role.Metadata.Name),
...
func (s *ScopedRoleService) CreateScopedRoleAssignment(ctx context.Context, req *srpb.CreateScopedRoleAssignmentRequest) (*srpb.CreateScopedRoleAssignmentResponse, error) {
	...
	for _, subAssignment := range assignment.Spec.Assignments {
		...
		// as such modification could theoretically invalidate the assignment. technically we consider
		// invalid assignments something we're willing to be resilient to, but we still need to take
		// every reasonable effort to prevent them. this restriction may be limited if we solve the
		// "poison pill" problem of role options.
...
func scopedRoleToItem(role *srpb.ScopedRole) (backend.Item, error) {
	...
	if role.Metadata.Expires != nil {
		// TODO(fspmarshall): formalize a model for scoped role expiration.
...
func scopedRoleAssignmentToItem(assignment *srpb.ScopedRoleAssignment) (backend.Item, error) {
	...
	if assignment.Metadata.Expires != nil {
		// TODO(fspmarshall): formalize a model for scoped role assignment expiration.
...
func scopedRoleWithRevision(role *srpb.ScopedRole, revision string) *srpb.ScopedRole {
	roleCopy := *role
	roleCopy.Metadata = &(*role.Metadata)
	roleCopy.Metadata.Revision = revision
	return &roleCopy
}
...
func scopedRoleAssignmentWithRevision(assignment *srpb.ScopedRoleAssignment, revision string) *srpb.ScopedRoleAssignment {
	...
	assignmentCopy.Metadata = &(*assignment.Metadata)
	assignmentCopy.Metadata.Revision = revision

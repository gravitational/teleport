...
func (s *AppService) GetApp(ctx context.Context, name string) (types.Application, error) {
	item, err := s.Get(ctx, backend.Key(appPrefix, backend.NewSafeString(name)))
	if err != nil {
...
func (s *AppService) CreateApp(ctx context.Context, app types.Application) error {
	...
	item := backend.Item{
		Key:     backend.Key(appPrefix, backend.NewSafeString(app.GetName())),
		Value:   value,
		Expires: app.Expiry(),
...
func (s *AppService) DeleteApp(ctx context.Context, name string) error {
	err := s.Delete(ctx, backend.Key(appPrefix, backend.NewSafeString(name)))
	if err != nil {
		if trace.IsNotFound(err) {

...
// GetBeamsConfig returns the singleton BeamsConfig resource. If no resource
// exists in the backend, a virtual default is returned.
func (s *BeamsConfigService) GetBeamsConfig(ctx context.Context) (*beamsv1.BeamsConfig, error) {
	...
	if err != nil {
		if trace.IsNotFound(err) {
			return services.DefaultBeamsConfig(), nil
		}
		return nil, trace.Wrap(err)
	}
	return config, nil

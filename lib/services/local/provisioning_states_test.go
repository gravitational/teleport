...
func TestProvisioningUpdate(t *testing.T) {
	...
	uut, err := NewProvisioningStateService(backend, ProvisioningStateServiceModeStrict)
	require.NoError(t, err)
	...
	t.Run("downstream is honoured", func(t *testing.T) {
		...
		stateA, err := uut.CreateProvisioningState(ctx, downstreamA, mkUserProvisioningState(
			"someuser@example.com",
			downstreamA,
			provisioningv1.Status_STATUS_STALE))
		require.NoError(t, err)

		// WHEN I try to create a provisioning state for the same user on a
		// *different* downstream
		stateB, err := uut.CreateProvisioningState(ctx, downstreamB, mkUserProvisioningState(
			"someuser@example.com",
			downstreamB,
			provisioningv1.Status_STATUS_PROVISIONED))
		...
		require.Equal(t, provisioningv1.Status_STATUS_STALE, retrievedStateA.Status.Status)
		...
		require.Equal(t, provisioningv1.Status_STATUS_STALE, retrievedStateB.Status.Status)
	...
	t.Run("locking", func(t *testing.T) {
		...
		s0, err := uut.CreateProvisioningState(ctx, downstreamID, mkUserProvisioningState(
			"someuser@example.com",
			downstreamID,
			provisioningv1.Status_STATUS_STALE))
		require.NoError(t, err)
		...
		_, err = uut.UpdateProvisioningState(ctx, downstreamID, s1)
		require.NoError(t, err)

		// WHEN I try to update the resource based on the original version...
		s0.Status.Status = provisioningv1.Status_STATUS_STALE
		s0.Status.Error = "I can't find the database"
		_, err = uut.UpdateProvisioningState(ctx, downstreamID, s0)
...
func mkUserProvisioningState(username string, downstream services.DownstreamID, initialStatus provisioningv1.Status) *provisioningv1.PrincipalState {

...
func newCloudClusterParser() *cloudClusterParser {
	return &cloudClusterParser{
		baseParser: newBaseParser(backend.NewKey(cloudClusterPrefix)),
	}
}

type cloudClusterParser struct {
	baseParser
}

func (p *cloudClusterParser) parse(event backend.Event) (types.Resource, error) {
	switch event.Type {
	case types.OpDelete:
		name := event.Item.Key.TrimPrefix(backend.NewKey(cloudClusterPrefix)).String()
		if name == "" {
			return nil, trace.NotFound("failed parsing %v", event.Item.Key.String())
		}
	...
	case types.OpPut:
		r, err := services.UnmarshalCloudCluster(event.Item.Value,
			services.WithExpires(event.Item.Expires),
			services.WithRevision(event.Item.Revision),
		)
		if err != nil {

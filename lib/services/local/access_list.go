...
func (a *AccessListService) runOpWithLock(ctx context.Context, accessList *accesslist.AccessList, op opType) (*accesslist.AccessList, error) {
	...
	validateAccessList := func() error {
		...
		expectedMemberOf := []string{}
		expectedOwnerOf := []string{}
		...
		if op == opTypeUpdate {
			...
			expectedMemberOf = existingList.Spec.MemberOf
			expectedOwnerOf = existingList.Spec.OwnerOf
		}

		// If the memberOf or ownerOf fields have been updated, return an error – these fields are managed by the service.
		if !cmp.Equal(expectedMemberOf, accessList.Spec.MemberOf) {
			return trace.BadParameter("memberOf field may not be set explicitly")
		}
		if !cmp.Equal(expectedOwnerOf, accessList.Spec.OwnerOf) {
			return trace.BadParameter("ownerOf field may not be set explicitly")
		}
...
func (a *AccessListService) updateAccessListRefField(
	...
	targetName string,
	new bool,
	fieldSelector func(spec *accesslist.Spec) *[]string,
) error {
	...
	field := fieldSelector(&targetAccessList.Spec)
...
func (a *AccessListService) updateAccessListMemberOf(ctx context.Context, accessListName, memberName string, new bool) error {
	return a.updateAccessListRefField(ctx, accessListName, memberName, new, func(spec *accesslist.Spec) *[]string {
		return &spec.MemberOf
	})
}
...
func (a *AccessListService) updateAccessListOwnerOf(ctx context.Context, accessListName, ownerName string, new bool) error {
	return a.updateAccessListRefField(ctx, accessListName, ownerName, new, func(spec *accesslist.Spec) *[]string {
		return &spec.OwnerOf
	})
}
...
func (a *AccessListService) UpsertAccessListWithMembers(ctx context.Context, accessList *accesslist.AccessList, membersIn []*accesslist.AccessListMember) (*accesslist.AccessList, []*accesslist.AccessListMember, error) {
	...
	validateAccessList := func() error {
		// If the memberOf or ownerOf fields are already set, return an error – these fields are managed by the service.
		if len(accessList.Spec.MemberOf) > 0 {
			return trace.BadParameter("memberOf field may not be set explicitly")
		}
		if len(accessList.Spec.OwnerOf) > 0 {

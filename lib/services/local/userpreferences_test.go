...
func TestUserPreferencesCRUD2(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			res, err := identity.GetUserPreferences(ctx, username, &userpreferencesv1.GetUserPreferencesRequest{})
			require.NoError(t, err)
			...
			if test.req != nil {
				err := identity.UpsertUserPreferences(ctx, username, test.req)
				require.NoError(t, err)
			}
...
func TestLayoutUpdate(t *testing.T) {
	...
	prefs, err := identity.GetUserPreferences(ctx, "test", &userpreferencesv1.GetUserPreferencesRequest{})
	require.NoError(t, err)

...
type CommonAccessChecker interface {
	...
	CheckAccessToRules(resource string, verbs ...string) error
}
...
func (c *accessCheckerShim) CheckAccessToRules(resource string, verbs ...string) error {
	return checkAccessToRulesImpl(c.AccessChecker, resource, verbs...)
}

func checkAccessToRulesImpl(checker AccessChecker, resource string, verbs ...string) error {
	if len(verbs) == 0 {
...
func (c *SplitAccessCheckerContext) CheckMaybeHasAccessToRules(resource string, verbs ...string) error {
	if c.scopedContext == nil {
		shim := accessCheckerShim{c.unscopedChecker}
		return shim.CheckAccessToRules(resource, verbs...)
	}
...
func (c *SplitAccessCheckerContext) Decision(ctx context.Context, scope string, fn func(*SplitAccessChecker) error) error {
	for checker, err := range c.CheckersForResourceScope(ctx, scope) {
		if err != nil {
			return trace.Wrap(err)
		}

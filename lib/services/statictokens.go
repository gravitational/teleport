...
func MarshalStaticTokens(t StaticTokens, opts ...MarshalOption) ([]byte, error) {
	cfg, err := CollectOptions(opts)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	...
	default:
		return nil, trace.BadParameter("unrecognized static token version %T", t)
	}
}

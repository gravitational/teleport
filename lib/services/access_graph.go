...
// AccessGraphSecrets is an interface for managing access graph secrets.
type AccessGraphSecrets interface {
	AccessGraphSecretsGetter
	// UpsertAuthorizedKey upserts a new authorized key.
	UpsertAuthorizedKey(ctx context.Context, in *accessgraphsecretspb.AuthorizedKey) (*accessgraphsecretspb.AuthorizedKey, error)
	// DeleteAuthorizedKey deletes a specific authorized key.
	DeleteAuthorizedKey(ctx context.Context, hostID, name string) error
	// DeleteAllAuthorizedKeys deletes all authorized keys.
	DeleteAllAuthorizedKeys(ctx context.Context) error
	// UpsertPrivateKey upserts a new private key.
	UpsertPrivateKey(ctx context.Context, in *accessgraphsecretspb.PrivateKey) (*accessgraphsecretspb.PrivateKey, error)
	// DeletePrivateKey deletes a specific private key.
	DeletePrivateKey(ctx context.Context, deviceID, name string) error
	// DeleteAllPrivateKeys deletes all private keys.
	DeleteAllPrivateKeys(ctx context.Context) error
}

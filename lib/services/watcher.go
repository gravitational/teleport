...
type ProxyWatcherConfig struct {
	...
	// ProxiesC is a channel used to report the current resourxe set. It receives
...
func (p *genericCollector[T, R]) getResourcesAndUpdateCurrent(ctx context.Context) error {
	...
	newCurrent := make(map[string]T, len(resources))
	for _, resource := range resources {
		newCurrent[p.ResourceKey(resource)] = resource
	}
	p.rw.Lock()
	defer p.rw.Unlock()
	...
	if len(resources) > 0 || p.isInitialized() ||
		(!p.RequireResourcesForInitialBroadcast && len(resources) == 0) {
...
func (p *genericCollector[T, R]) processEventsAndUpdateCurrent(ctx context.Context, events []types.Event) {
	...
	for _, event := range events {
		if event.Resource == nil || event.Resource.GetKind() != p.ResourceKind {
		...
		case types.OpDelete:
			...
			delete(p.current, event.Resource.GetMetadata().Description+event.Resource.GetName())
		...
		case types.OpPut:
			...
			current := p.current[key]
			p.current[key] = resource
			updated = p.ResourceDiffer(current, resource)
		default:

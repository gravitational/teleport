...
func (cfg *DynamicWindowsDesktopWatcherConfig) CheckAndSetDefaults() error {
	...
	if cfg.DynamicWindowsDesktopGetter == nil {
		...
		if !ok {
			return trace.BadParameter("missing parameter DynamicWindowsDesktopGetter and Client not usable as DynamicWindowsDesktopGetter")
		}
...
func (p *dynamicWindowsDesktopCollector) getResourcesAndUpdateCurrent(ctx context.Context) error {
	dynamicWindowsDesktops, err := p.DynamicWindowsDesktopGetter.GetDynamicWindowsDesktops(ctx, types.DynamicWindowsDesktopFilter{})
	if err != nil {
		return trace.Wrap(err)
	}

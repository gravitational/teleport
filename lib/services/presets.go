...
func NewAWSIAMRolesAnywhereProfileTemplateRole(buildType string) types.Role {
	if buildType != modules.BuildEnterprise {
		return nil
	}
	...
	return &types.RoleV6{
		...
		Metadata: types.Metadata{
			...
			Namespace:   apidefaults.Namespace,
			Description: "Template Role to be used by Access Lists in AWS IAM Roles Anywhere Profile resources",
			Labels: map[string]string{
				types.TeleportInternalResourceType: types.SystemResource,
			},
		},

...
func (g *GithubSuite) TestMapClaims(c *check.C) {
	...
	logins, roles, kubeGroups, kubeUsers := connector.MapClaims(types.GithubClaims{
		OrganizationToTeams: map[string][]string{
			"gravitational": {"admins"},
		},
	})
	c.Assert(logins, check.DeepEquals, []string{"admin", "dev"})
	c.Assert(roles, check.DeepEquals, []string{"system"})
	...
	c.Assert(roles, check.DeepEquals, []string(nil))

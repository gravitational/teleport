...
func UnmarshalEncryptionConfig(bytes []byte, opts ...MarshalOption) (types.ClusterEncryptionConfig, error) {
	var recConfig types.ClusterEncryptionConfigV3
...
func MarshalEncryptionConfig(recConfig types.ClusterEncryptionConfig, opts ...MarshalOption) ([]byte, error) {
	if err := recConfig.CheckAndSetDefaults(); err != nil {
		return nil, trace.Wrap(err)
	}
	...
	switch recConfig := recConfig.(type) {
	case *types.ClusterEncryptionConfigV3:
		if version := recConfig.GetVersion(); version != types.V2 {
			return nil, trace.BadParameter("mismatched encryption config version %v and type %T", version, recConfig)
		}
		if !cfg.PreserveResourceID {
			// avoid modifying the original object
			// to prevent unexpected data races
		...
		return utils.FastMarshal(recConfig)
	default:
		return nil, trace.BadParameter("unrecognized encryption config version %T", recConfig)
	}
}
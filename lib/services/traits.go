...
func (ms TraitMappingSet) TraitsToRoles(traits map[string][]string) []string {
	var roles []string
	ms.traitsToRoles(traits, func(role string, modified bool) {
...
func (ms TraitMappingSet) TraitsToRoleMatchers(traits map[string][]string) ([]parse.Matcher, error) {
	...
	var firstErr error
	ms.traitsToRoles(traits, func(role string, modified bool) {
		if modified || utils.ContainsExpansion(role) {
...
func (ms TraitMappingSet) traitsToRoles(traits map[string][]string, collect func(role string, modified bool)) {

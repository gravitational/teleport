...
import (
	...
	"github.com/gravitational/teleport/lib/utils/oncemap"
	"github.com/gravitational/trace"
)

// ScopedAccessCheckerContext is the top-level scoped access checker state. It builds and caches scoped access checkers "at" specific
// scopes, based on a user's identity.
type ScopedAccessCheckerContext struct {
	builder  scopedAccessCheckerBuilder
	checkers *oncemap.OnceMap[string, *ScopedAccessChecker]
}

// NewScopedAccessCheckerContext builds a scoped access checker context for a given identity. The context is used to build scoped access checkers at various scopes,
// and to evaluate access to resources within those scopes. Note that the supplied context.Context is captured and used to propagate cancellation during
// loading of scoped roles. Cancellation of the context while access checks are still in progress may result in spurious access denied errors.
func NewScopedAccessCheckerContext(ctx context.Context, info *AccessInfo, localCluster string, reader ScopedRoleReader) (*ScopedAccessCheckerContext, error) {
	...
	return &ScopedAccessCheckerContext{
		builder: builder,
		checkers: oncemap.New(func(scope string) (*ScopedAccessChecker, error) {
...
func (c *ScopedAccessCheckerContext) checkersForResourceScope(ctx context.Context, scope string, enforcePin bool) stream.Stream[*ScopedAccessChecker] {
	return func(yield func(*ScopedAccessChecker, error) bool) {
		// deny immediately if the resource scope is not subject to the pinned scope. note that this denial isn't just an optimization,
		// we have to perform this check separately from whatever access checks are performed by particular checkers. This is vital as
		// the pin scope itself may deny access to a resource that would be permitted by any particular checker. For example, if a user
		// has a scoped role assigned at /foo which grants access to all ssh nodes, but they are pinned to scope /foo/bar, the checker at
		// /foo needs to permit access to all nodes subject to /foo, but the pin only permits access to resources subject to /foo/bar,
		// even if the final allow decision is determined by a checker at /foo.
		...
		for checkerScope := range scopes.DescendingScopes(scope) {
			checker, err := c.checkers.Get(ctx, checkerScope)
			if err != nil {
...
// DoScopedDecision is a helper function that takes care of boilerplate for simple scoped decisions.  It calls the provided decision function
...
//
// The generic parameter may be used to propagate additional information/parameterization of the decision. For basic checks, conventionally
// set D to struct{} to indicate that the presence/absence of the error alone is the entire decision state. For logging/debuggin purposes,
// returning checker.Scope() as the decision value may be convenient.
...
func (b *scopedAccessCheckerBuilder) newCheckerAtScope(ctx context.Context, scope string) (*ScopedAccessChecker, error) {
	...
	// validate that the target scope is compatible with the scope pin. note that this check is conceptually distinct from checking
	// wether or not the pinned scope permits access to resources at the target scope. scope pinning rules disallow access to resources
	// that are not equivalent or descendant to the pinned scope, but during access evaluation we must create checkers for all parent
	// scopes as well. this check just verifies that we aren't creating a checker for an *orthogonal* scope. a full access-control check
	// implementation must use [ScopedAccessCheckerContext.CheckersForResourceScope] in order to ensure that pinning is properly enforced
	...
	// fetch all assigned scoped roles and convert them to classic roles to drive the underlying access checks. we need to use the unchecked
	// variant of assignments for resource scope because the standard variant refuses to produce an iterator for a scope that is a parent of
	// the pinned scope, and this method will get called to build parent checkers as part of legitimate access checks. it is the responsibility
	// of the caller of this method to determine that it is being used in a context that respectes correct pinning rules.
...
// RiskyNewScopedAccessCheckerAtScope creates a scoped access checker at the specified scope. Note that creating a scoped access checker at a specific
// target scope rather than using [ScopedAccessCheckerContext] is typically incorrect as it bypasses scope pin enforcement. There are some
...
// Scope returns the scope "at" which the checker was created. A scoped access checker created at a given scope includes all scoped roles assigned to
// the user at that scope, and all ancestral scopes.

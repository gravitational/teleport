...
// at that same scope, even if that scope is a parent of the node's scope rather than the node's scope itself.
type ScopedAccessChecker struct {
	scope   string
	checker *accessChecker
}

// NewScopedAccessCheckerAtScope creates a scoped access checker at the specified scope. Note that we rarely actually want to manually create a scoped access
// checker at a scope other than root. Scoped access checks must halt at the first scope that meets an explicit decision. The only currently known exception
// is pinned certificate creation, which requires creating an access checker at the pinned scope directly in order to determine the certificate parameters
// that the user should have at that scope.
...
func fetchAndConvertScopedRoles(ctx context.Context, scope string, names []string, reader ScopedRoleReader) ([]types.Role, error) {
	...
	for _, name := range names {
		...
		// TODO(fspmarshall): figure out how/when we want to support trait interpolation in scoped roles. When we do, that will likely need
		// to be done here. Wether we should perform trait interpolation on the per-conversion scoped role and add support piecemeal, or inherit
		// identical trait interpolation from classic role behavior isn't clear yet, so at this stage we're just opting to skip entirely.

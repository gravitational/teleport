...
func TestRolesForResourceRequest(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			desc:                 "denied with trait template",
			currentRoles:         []string{"splunk-response-team", "db-admins-with-traits", "db-admins-denied-with-traits"},
			requestResourceIDs:   resourceIDs,
			expectRequestedRoles: []string{"splunk-admins", "splunk-super-admins"},
		},
	}
	...
	roleDesc := map[string]types.RoleSpecV6{
		...
		"kube-access-deployment": {
			Allow: types.RoleConditions{
				...
				KubernetesResources: []types.KubernetesResource{
					{Kind: "deployments", Namespace: "*", Name: "*", Verbs: []string{"*"}, APIGroup: "*"},
				},
			},
		},
		"db-access-wildcard": {

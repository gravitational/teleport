...
// RemoteSite is an alias to Cluster which exists for the sole
// purpose of preventing enterprise builds from failing until
// references there can be updated.
// Deprecated: Use Cluster instead.
// TODO(tross): Remove when enterprise is updated to use Cluster.
type RemoteSite = Cluster

// Cluster represents a teleport cluster that can be accessed via
...
type Cluster interface {
	...
	// GetLastConnected returns last time the remote cluster was seen connected
...
// clusters connect and register with it.

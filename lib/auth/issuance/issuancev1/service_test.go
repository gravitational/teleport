...
func TestIssueScopedBotCerts(t *testing.T) {
	...
	botClient, err := srv.NewClient(authtest.TestScopedBot(bot.Status.UserName, botScope))
	require.NoError(t, err)
...
func TestIssueScopedBotCerts_Unauthorized(t *testing.T) {
	...
	t.Run("non-bot user with scope", func(t *testing.T) {
		...
		scopedUserClient, err := srv.NewClient(authtest.TestScopedUser(user.GetName(), testScope))
		require.NoError(t, err)
	...
	t.Run("unscoped bot", func(t *testing.T) {
		...
		botClient, err := srv.NewClient(authtest.TestIdentity{
			I: authz.LocalUser{
				Username: unscopedBot.Status.UserName,

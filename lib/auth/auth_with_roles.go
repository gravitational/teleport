...
func (a *ServerWithRoles) ListNotificationsForUser(ctx context.Context, req *notificationsv1.ListUserNotificationsRequest) (*notificationsv1.ListUserNotificationsResponse, error) {
	req.Username = a.context.User.GetName()
	...
	var startKey string
	for {
		rsp, err := a.authServer.ListUserNotificationStates(ctx, &notificationsv1.ListUserNotificationStatesRequest{
		...
		notificationStates = append(notificationStates, rsp.NotificationStates...)

		if rsp.NextPageToken == "" {
			break
		}
		startKey = rsp.NextPageToken
	}
	...
	notifications, userNotificationsNextKey, globalNotificationsNextKey, err := a.authServer.ListNotificationsForUser(ctx, req, userNotifMatchFn, globalNotifMatchFn)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	userLastSeenNotification, err := a.authServer.GetUserLastSeenNotification(ctx, &notificationsv1.GetUserLastSeenNotificationRequest{
		Username: req.Username,
	})
	if err != nil {
		return nil, trace.Wrap(err)
	...
	response := &notificationsv1.ListUserNotificationsResponse{

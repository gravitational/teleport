...
func (a *ServerWithRoles) actionNamespace(namespace, resource string, verb string, extraVerbs ...string) error {
	...
	if a.scopedContext != nil {
		// paths that support scoped identities should call preAuthorizeScopedAction in response to
...
func (a *ServerWithRoles) remoteServerAction() error {
	...
	if a.scopedContext != nil {
		...
		if !isUnscoped {
			return trace.Wrap(services.ErrScopedIdentity, "checking remote server role")
		}
	}
...
func (a *ServerWithRoles) authContextForSearch(ctx context.Context, req *proto.ListResourcesRequest) (*authz.ScopedContext, error) {
	...
	if a.scopedContext != nil {
		...
		unscopedCtx, isUnscoped = a.scopedContext.UnscopedContext()
		if !isUnscoped {
			// scoped identities don't have search_as_roles or preview_as_roles, so we can just return the scoped
			// context unmodified
			return a.scopedContext, nil
		}
	}
...
func (r *resourceChecker) CanAccess(ctx context.Context, resource types.ResourceWithLabels) error {
	...
	case types.KubeServer:
		return r.scopedCtx.CheckerContext.Decision(ctx, cmp.Or(rr.GetScope(), scopes.Root), func(checker *services.ScopedAccessChecker) error {
...
// GetAllowedLoginsForResource returns all of the allowed logins for the passed resource. For scoped identities,
// this will only return logins for resources that support scopes.
func (r *resourceChecker) GetAllowedLoginsForResource(ctx context.Context, resource services.AccessCheckable) ([]string, error) {
	if unscopedCtx, isUnscoped := r.scopedCtx.UnscopedContext(); isUnscoped {
		return unscopedCtx.Checker.GetAllowedLoginsForResource(resource)
	}
	if server, ok := resource.(types.Server); ok {
		return r.scopedCtx.CheckerContext.GetPossibleLoginsForSSHServer(ctx, server)
	}
...
func (a *ServerWithRoles) checkAccessToApp(app types.Application) error {
	...
	if a.scopedContext != nil {
		...
		unscopedCtx, isUnscoped = a.scopedContext.UnscopedContext()
		if !isUnscoped {
			return trace.Wrap(services.ErrScopedIdentity, "checking access to app")
		}
	}
...
func (a *ServerWithRoles) GetKubernetesCluster(ctx context.Context, name string) (types.KubeCluster, error) {
	if err := a.authorizeActionWithScopedPreAuth(types.KindKubernetesCluster, types.VerbRead); err != nil {
		return nil, trace.Wrap(err)
	}
...
func (a *ServerWithRoles) DeleteKubernetesCluster(ctx context.Context, name string) error {
	if err := a.authorizeActionWithScopedPreAuth(types.KindKubernetesCluster, types.VerbDelete); err != nil {
		return trace.Wrap(err)
	}
...
func (a *ServerWithRoles) checkAccessToDatabase(database types.Database) error {
	...
	if a.scopedContext != nil {
		...
		unscopedCtx, isUnscoped = a.scopedContext.UnscopedContext()
		if !isUnscoped {
			return trace.Wrap(services.ErrScopedIdentity, "checking access to database")
		}
	}
...
func (a *ServerWithRoles) checkAccessToWindowsDesktop(w types.WindowsDesktop) error {
	...
	if a.scopedContext != nil {
		...
		unscopedCtx, isUnscoped = a.scopedContext.UnscopedContext()
		if !isUnscoped {
			return trace.Wrap(services.ErrScopedIdentity, "checking access to windows desktop")
		}
	}
...
func (a *ServerWithRoles) checkAccessToSAMLIdPServiceProvider(ctx context.Context, sp types.SAMLIdPServiceProvider) error {
	...
	if a.scopedContext != nil {
		...
		unscopedCtx, isUnscoped = a.scopedContext.UnscopedContext()
		if !isUnscoped {
			return trace.Wrap(services.ErrScopedIdentity, "checking access to SAML IdP service provider")
		}
	}
...
func (a *ServerWithRoles) checkAccessToUserGroup(userGroup types.UserGroup) error {
	...
	if a.scopedContext != nil {
		...
		unscopedCtx, isUnscoped = a.scopedContext.UnscopedContext()
		if !isUnscoped {
			return trace.Wrap(services.ErrScopedIdentity, "checking access to user group")
		}
	}

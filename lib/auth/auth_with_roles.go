...
func (a *ServerWithRoles) GetDynamicWindowsDesktops(ctx context.Context) ([]types.DynamicWindowsDesktop, error) {
	if err := a.action(apidefaults.Namespace, types.KindDynamicWindowsDesktop, types.VerbList, types.VerbRead); err != nil {
		return nil, trace.Wrap(err)
	}
	hosts, err := a.authServer.GetDynamicWindowsDesktops(ctx)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func (a *ServerWithRoles) GetDynamicWindowsDesktop(ctx context.Context, name string) (types.DynamicWindowsDesktop, error) {
	if err := a.action(apidefaults.Namespace, types.KindDynamicWindowsDesktop, types.VerbList, types.VerbRead); err != nil {
		return nil, trace.Wrap(err)
	}
	host, err := a.authServer.GetDynamicWindowsDesktop(ctx, name)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	filtered, err := a.filterDynamicWindowsDesktops([]types.DynamicWindowsDesktop{host})
	if err != nil {
		return nil, trace.Wrap(err)
	}
	if len(filtered) == 0 {
		return nil, trace.AccessDenied("insufficient permissions")
	}
	return host, nil
}
...
func (a *ServerWithRoles) UpdateDynamicWindowsDesktop(ctx context.Context, s types.DynamicWindowsDesktop) error {
	...
	existing, err := a.authServer.GetDynamicWindowsDesktop(ctx, s.GetName())
	if err != nil && !trace.IsNotFound(err) {
		return trace.Wrap(err)
	}
	if trace.IsNotFound(err) {
	...
	if err := a.checkAccessToDynamicWindowsDesktop(existing); err != nil {
		return trace.Wrap(err)
	}
...
func (a *ServerWithRoles) DeleteDynamicWindowsDesktop(ctx context.Context, name string) error {
	...
	desktop, err := a.authServer.GetDynamicWindowsDesktop(ctx, name)
	if err != nil && !trace.IsNotFound(err) {
		return trace.Wrap(err)
	}
	if trace.IsNotFound(err) {
...
func (a *ServerWithRoles) DeleteAllDynamicWindowsDesktops(ctx context.Context) error {
	...
	desktops, err := a.authServer.GetDynamicWindowsDesktops(ctx)
	if err != nil {
		return trace.Wrap(err)
	}

...
// flushCache is a helper for waiting until preceeding changes have propagated to the
...
// tryFlushCache is equivalent to flushCache except that failures are expressed by returning
// and error instead of failing the encompassing test.
func tryFlushCache(clt flushClt) error {
	name := strings.ReplaceAll(uuid.NewString(), "-", "")
	defer clt.DeleteNamespace(name)
	...
	for {
		...
		case <-timout:
			return trace.Errorf("failed to flush cache within timeout")
		}
	}
}

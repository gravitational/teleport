...
func (a *TestAuthServer) NewRemoteClient(identity TestIdentity, addr net.Addr, pool *x509.CertPool) (*Client, error) {
	...
	return NewClient(client.Config{Addrs: addrs, Creds: client.TLSCreds(tlsConfig)})
}
...
func (t *TestTLSServer) CloneClient(clt *Client) *Client {
	...
	newClient, err := NewClient(client.Config{Addrs: addr, Creds: client.TLSCreds(clt.TLSConfig())})
	if err != nil {
		panic(err)
	}
	return newClient

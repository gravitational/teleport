...
func (a *TestAuthServer) NewRemoteClient(identity TestIdentity, addr net.Addr, pool *x509.CertPool) (*Client, error) {
	...
	return NewClient(client.Config{Addrs: addrs, Credentials: client.LoadTLS(tlsConfig)})
}
...
func (t *TestTLSServer) CloneClient(clt *Client) *Client {
	...
	newClient, err := NewClient(client.Config{Addrs: addr, Credentials: client.LoadTLS(clt.TLSConfig())})
	if err != nil {
		panic(err)
	}
	return newClient

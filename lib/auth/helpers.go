...
func (a *TestAuthServer) NewRemoteClient(identity TestIdentity, addr net.Addr, pool *x509.CertPool) (*Client, error) {
	...
	return NewClient(client.Config{Addrs: addrs, Credentials: &client.Credentials{TLS: tlsConfig}})
}
...
func (t *TestTLSServer) CloneClient(clt *Client) *Client {
	...
	newClient, err := NewClient(client.Config{Addrs: addr, Credentials: &client.Credentials{TLS: clt.TLSConfig()}})
	if err != nil {
		panic(err)
	}
	return newClient

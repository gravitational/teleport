...
// Users manages users on auth server
...
func (s *IdentityService) CreateUser(user services.User) error {
	...
	value, err := resource.MarshalUser(user.WithoutSecrets().(services.User))
	if err != nil {
		return trace.Wrap(err)
	}
...
func (s *IdentityService) AddUserLoginAttempt(user string, attempt auth.LoginAttempt, ttl time.Duration) error {
...
func (s *IdentityService) GetUserLoginAttempts(user string) ([]auth.LoginAttempt, error) {
	startKey := backend.Key(webPrefix, usersPrefix, user, attemptsPrefix)
	result, err := s.GetRange(context.TODO(), startKey, backend.RangeEnd(startKey), backend.NoLimit)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	out := make([]auth.LoginAttempt, len(result.Items))
	...
	sort.Sort(auth.SortedLoginAttempts(out))
	return out, nil
}
...
func (s *IdentityService) GetOIDCConnector(ctx context.Context, name string, withSecrets bool) (services.OIDCConnector, error) {
	...
	conn, err := resource.UnmarshalOIDCConnector(item.Value,
...
func (s *IdentityService) GetOIDCConnectors(ctx context.Context, withSecrets bool) ([]services.OIDCConnector, error) {
	...
	for i, item := range result.Items {
		conn, err := resource.UnmarshalOIDCConnector(
			item.Value, resource.WithExpires(item.Expires))
		if err != nil {
			return nil, trace.Wrap(err)
		}
		if !withSecrets {
...
func (s *IdentityService) CreateOIDCAuthRequest(req auth.OIDCAuthRequest, ttl time.Duration) error {
...
func (s *IdentityService) GetOIDCAuthRequest(stateToken string) (*auth.OIDCAuthRequest, error) {
	if stateToken == "" {
...
func (s *IdentityService) GetSAMLConnector(ctx context.Context, name string, withSecrets bool) (services.SAMLConnector, error) {
	...
	conn, err := resource.UnmarshalSAMLConnector(
		item.Value, resource.WithExpires(item.Expires))
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func (s *IdentityService) CreateSAMLAuthRequest(req auth.SAMLAuthRequest, ttl time.Duration) error {
	if err := req.Check(); err != nil {
		return trace.Wrap(err)
	}
	value, err := json.Marshal(req)
...
func (s *IdentityService) GetSAMLAuthRequest(id string) (*auth.SAMLAuthRequest, error) {
	if id == "" {
...
func (s *IdentityService) CreateGithubConnector(connector services.GithubConnector) error {
	...
	value, err := resource.MarshalGithubConnector(connector)
	if err != nil {
		return trace.Wrap(err)
	}
	item := backend.Item{
...
func (s *IdentityService) GetGithubConnectors(ctx context.Context, withSecrets bool) ([]services.GithubConnector, error) {
	...
	for i, item := range result.Items {
		connector, err := resource.UnmarshalGithubConnector(item.Value)
		if err != nil {
			return nil, trace.Wrap(err)
		}
		if !withSecrets {
...
func (s *IdentityService) CreateGithubAuthRequest(req auth.GithubAuthRequest) error {
	err := req.Check()
	if err != nil {
		return trace.Wrap(err)
	}
...
func (s *IdentityService) GetGithubAuthRequest(stateToken string) (*auth.GithubAuthRequest, error) {
	if stateToken == "" {
		return nil, trace.BadParameter("missing parameter stateToken")
	}

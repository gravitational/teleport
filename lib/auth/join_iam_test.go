...
func TestValidateSTSHost(t *testing.T) {
	...
	for _, validHost := range []string{
		...
		t.Run(validHost, func(t *testing.T) {
			err := validateSTSHost(validHost)
			require.NoError(t, err, "unexpected error validating hostname %s: %v", validHost, err)
		})
	}
	for _, invalidHost := range []string{
		...
		"sts.example.com",
		"sts.us-east-1.com",
		"sts.us-attacker-1.amazonaws.com",
		"sts.cn-attacker-1.amazonaws.com.cn",
		"sts.us-east-1.amazonaws.com.cn",
		"sts.us-attacker-1.c2s.ic.gov",
		"sts.us-attacker-1.sc2s.sgov.gov",
		"sts.us-east-1.attacker.amazonaws.com",
		"sts.cn-north-1.attacker.amazonaws.com.cn",
		"sts.us-iso-east-1.attacker.c2s.ic.gov",
		"sts.us-isob-east-1.attacker.sc2s.sgov.gov",
		"example.com",
		"com",
		"sts",
		"sts..amazonaws.com",
		...
		t.Run(invalidHost, func(t *testing.T) {
			err := validateSTSHost(invalidHost)
			require.True(t, trace.IsAccessDenied(err), "expected AccessDenied error while validating hostname %s, got: %T %v", invalidHost, err, err)
		})
	}
}

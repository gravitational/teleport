...
func GenerateIdentity(a *Server, id identity.IdentityID, additionalPrincipals, dnsNames []string) (*identity.Identity, error) {
	...
	return identity.ReadIdentityFromKeyPair(priv, certs)
}

// ReadLocalIdentity reads, parses and returns the given pub/pri key + cert from the
// key storage (dataDir).
func ReadLocalIdentity(dataDir string, id identity.IdentityID) (*identity.Identity, error) {

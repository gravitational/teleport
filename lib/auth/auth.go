...
func (a *Server) checkForReleases(ctx context.Context) {
	...
	log.Debugf("Checking for new teleport releases via github api.")
...
func makeUpgradeSuggestionMsg(current, latest, oldestInstance string) string {
	...
	if semver.Compare(latest, current) == 1 {
		...
		if semver.Major(latest) != semver.Major(current) {
			...
			msg = fmt.Sprintf("A new major version of Teleport is available. Please consider upgrading your cluster to %s.", semver.Major(latest))
		} else {
	...
	if oldestInstance != "" && semver.Compare(latest, oldestInstance) == 1 {
		...
		return "Cluster contains some older Teleport instances. Please consider upgrading."
	}

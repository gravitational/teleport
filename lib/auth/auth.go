...
// GetKubeClusterStream streams a list of registered Kubernetes clusters.
func (a *Server) GetKubeClusterStream(ctx context.Context) stream.Stream[types.KubeCluster] {
	var done bool
	startKey := ""
	return stream.PageFunc(func() ([]types.KubeCluster, error) {
		if done {
			return nil, io.EOF
		}
		resp, err := a.ListResources(ctx, proto.ListResourcesRequest{
		...
		resources := types.ResourcesWithLabels(resp.Resources)
		clusters, err := resources.AsKubeClusters()
		return clusters, trace.Wrap(err)
	})
}

// GetAppStream streams a list of registered applications.
func (a *Server) GetAppStream(ctx context.Context) stream.Stream[types.Application] {
	var done bool
	startKey := ""
	return stream.PageFunc(func() ([]types.Application, error) {
		if done {
			return nil, io.EOF
		}
		resp, err := a.ListResources(ctx, proto.ListResourcesRequest{
		...
		resources := types.ResourcesWithLabels(resp.Resources)
		appServers, err := resources.AsAppServers()
		apps := make([]types.Application, len(appServers))
		for i, appServer := range appServers {
			apps[i] = appServer.GetApp()
		}
...
// GetDatabaseStream streams a list of registered databases.
func (a *Server) GetDatabaseStream(ctx context.Context) stream.Stream[types.Database] {
	var done bool
	startKey := ""
	return stream.PageFunc(func() ([]types.Database, error) {
		if done {
			return nil, io.EOF
		}
		resp, err := a.ListResources(ctx, proto.ListResourcesRequest{
...
// GetWindowsDesktopStream streams a list of registered Windows desktops.
func (a *Server) GetWindowsDesktopStream(ctx context.Context) stream.Stream[types.WindowsDesktop] {
	var done bool
	startKey := ""
	return stream.PageFunc(func() ([]types.WindowsDesktop, error) {
		if done {
		...
		resources := types.ResourcesWithLabels(resp.Resources)
		desktops, err := resources.AsWindowsDesktops()

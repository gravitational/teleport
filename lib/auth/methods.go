...
func (a *Server) authenticateUserLogin(ctx context.Context, req authclient.AuthenticateUserRequest) (services.UserState, *services.SplitAccessChecker, error) {
	...
	// parameter is untrusted user input, but we reject attempts to login to a scope for a which a user has no
	// assigned privileges.
	if req.Scope != "" {
		...
		// scoped roles into the access checker that apply to all possibly resources the resulting identity may have
		// access to.

...
func (p *ProcessStorage) GetState(role types.SystemRole) (*StateV2, error) {
	item, err := p.Get(context.TODO(), backend.ExactKey(statesPrefix, strings.ToLower(role.String()), stateName))
	if err != nil {
...
func (p *ProcessStorage) CreateState(role types.SystemRole, state StateV2) error {
	...
	item := backend.Item{
		Key:   backend.ExactKey(statesPrefix, strings.ToLower(role.String()), stateName),
		Value: value,
	}
...
func (p *ProcessStorage) ReadIdentity(name string, role types.SystemRole) (*Identity, error) {
	...
	item, err := p.Get(context.TODO(), backend.ExactKey(idsPrefix, strings.ToLower(role.String()), name))
	if err != nil {
		return nil, trace.Wrap(err)
	}
	var res IdentityV2
...
func (p *ProcessStorage) WriteIdentity(name string, id Identity) error {
	...
	item := backend.Item{
		Key:   backend.ExactKey(idsPrefix, strings.ToLower(id.ID.Role.String()), name),
		Value: value,
	}
	_, err = p.Put(context.TODO(), item)
	return trace.Wrap(err)

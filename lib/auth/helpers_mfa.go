...
func (d *TestDevice) solveAuthnKey(c *proto.MFAAuthenticateChallenge) (*proto.MFAAuthenticateResponse, error) {
	...
	resp, err := d.Key.SignAssertion(d.Origin(), webauthntypes.CredentialAssertionFromProto(c.WebauthnChallenge))
	if err != nil {
...
func (d *TestDevice) solveRegisterWebauthn(c *proto.MFARegisterChallenge) (*proto.MFARegisterResponse, error) {
	...
	resp, err := d.Key.SignCredentialCreation(d.Origin(), webauthntypes.CredentialCreationFromProto(c.GetWebauthn()))
	if err != nil {
		return nil, trace.Wrap(err)
	}

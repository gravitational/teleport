...
func TestGenerateAppToken(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			inJwtAuthority: "", // defaults to "jwt"
			outKey:         defaultKey,
			outError:       false,
		},
		{
			...
			inJwtAuthority: types.JwtAuthorityDefault,
			outKey:         defaultKey,
			outError:       false,
		},
		{
			...
			inJwtAuthority: types.JwtAuthorityOIDC,
			outKey:         oidcKey,
	...
	for _, ts := range tests {
		...
		token, err := client.GenerateAppToken(
			...
			types.GenerateAppTokenRequest{
				...
				URI:          "https://localhost:8080",
				Expires:      clock.Now().Add(1 * time.Minute),
				JwtAuthority: ts.inJwtAuthority,
			})

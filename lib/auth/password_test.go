...
func TestCheckPassswordWOToken(t *testing.T) {
	t.Parallel()
...
func TestCheckPassswordWOToken_previousPasswordStateUnspecified(t *testing.T) {
	...
	_, err = s.a.UpsertUser(ctx, u)
	require.NoError(t, err)

	// Double check if the password state is indeed unspecified.
	...
	// Password state should still be unspecified.
	u, err = s.a.GetUser(ctx, "user1", false /* withSecrets */)
	require.NoError(t, err)
...
func TestCheckPasswordWOToken_userNotFound(t *testing.T) {
	t.Parallel()

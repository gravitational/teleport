...
// GetTokenV3 retrieves a v3 token by name.
func (g *GRPCServer) GetTokenV3(ctx context.Context, req *types.ResourceRequest) (*types.ProvisionTokenV3, error) {
	...
	provisionTokenV3, ok := t.(*types.ProvisionTokenV3)
	if !ok {
		return nil, trace.Errorf("encountered unexpected token type: %T", t)
	}
	// TODO: Consider transforming a V2 token to V3 ?
	return provisionTokenV3, nil
}
...
// GetTokensV3 retrieves all V3 tokens.
func (g *GRPCServer) GetTokensV3(ctx context.Context, _ *empty.Empty) (*types.ProvisionTokenV3List, error) {
	...
	// TODO: Duplication issues of user tokens and static tokens
	// Calling GetTokensV2 and GetTokensV3 will return all of these potentially.
	// expand on this TODO in PR and solicit feedback.
	provisionTokensV3 := make([]*types.ProvisionTokenV3, len(ts))
	for i, t := range ts {
		var ok bool
		if provisionTokensV3[i], ok = t.(*types.ProvisionTokenV3); !ok {
			return nil, trace.Errorf("encountered unexpected token type: %T", t)
		}
	}
	return &types.ProvisionTokenV3List{
		ProvisionTokens: provisionTokensV3,
	}, nil
}
...
// CreateToken creates a v3 token.

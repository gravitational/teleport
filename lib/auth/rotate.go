...
type RotateRequest struct {
	...
	// InPlace indicates that the rotation steps should be performed without
	// actually swapping CA keys/certs.  The intent of this option is to trigger
	// re-issue of server certificates without invalidating existing certs.
...
func startNewRotation(req rotationReq, ca services.CertAuthority) error {
	...
	if req.inPlace {
		// We are rotating "in place", meaning that the rotation steps will be observed, but the
		// cert/key values won't actually be changed.
		sshPrivPEM = signingKeys[0]
		sshPubPEM = checkingKeys[0]
		certPEM = keyPairs[0].Cert
		keyPEM = keyPairs[0].Key
	} else if len(req.privateKey) != 0 {

...
func updatePreservedAttributes(ctx context.Context, uls *userloginstate.UserLoginState, ulsService services.UserLoginStates) error {
	...
	mergeSAMLIdentities(uls, existingULS)
	return nil
}

// mergeSAMLIdentities merges SAML identities from u2 to u1.
func mergeSAMLIdentities(u1, u2 *userloginstate.UserLoginState) {
	ids1 := u1.Spec.SAMLIdentities
	ids2 := u2.Spec.SAMLIdentities

	for _, id2 := range ids2 {
		if !slices.ContainsFunc(ids1, func(id1 userloginstate.ExternalIdentity) bool {

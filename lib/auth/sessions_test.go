...
func TestCreateAppSession_ResourceExpiry(t *testing.T) {
	t.Parallel()
	fakeclock := clockwork.NewFakeClock()
	...
	t.Cleanup(func() { assert.NoError(t, testAuthServer.Close()) })

	authServer := testAuthServer.AuthServer
	username := "user1"
	sessionTTL := 12 * time.Hour

	user, _, err := authtest.CreateUserAndRole(authServer, username, []string{username}, nil)
	require.NoError(t, err)

	session, err := authServer.CreateAppSessionFromReq(t.Context(), auth.NewAppSessionRequest{
	...
	// Check if ResourceExpiry is properly set and not defaulted to Metadata.Expires.
	sessV2, ok := session.(*types.WebSessionV2)
	require.True(t, ok)
	require.NotNil(t, sessV2.Spec.ResourceExpiry, "ResourceExpiry must be populated during session creation")

	expectedExpiry := fakeclock.Now().Add(sessionTTL)
	require.WithinDuration(t, expectedExpiry, session.Expiry(), time.Second)

	require.Equal(t, session.Expiry(), session.GetExpiryTime())
}

...
// RecordingMetadataConfig defines the configuration for the RecordingMetadataService.
...
// NewRecordingMetadata creates a new instance of RecordingMetadataService with the provided configuration.
...
func (s *RecordingMetadataService) ProcessSessionRecording(ctx context.Context, sessionID session.ID) error {
	...
	activeUsers := make(map[string]*durationpb.Duration)
	...
	addInactivityEvent := func(start, end time.Time) {
		...
		metadata.Metadata.Events = append(metadata.Metadata.Events, &pb.SessionRecordingEvent{
	...
	sampler := newThumbnailBucketSampler(maxThumbnails)

	recordThumbnail := func(start, end time.Time) {
		state := vt.DumpState()

		sampler.add(&thumbnailEntry{
			state:       &state,
			startOffset: start.Sub(startTime),
			endOffset:   end.Sub(startTime),
		})
	}
...
loop:
	for {
		...
		case evt := <-evts:
			...
			case *apievents.SessionEnd:
				metadata.Metadata.Duration = durationpb.New(e.EndTime.Sub(e.StartTime))
				...
				endTime := durationpb.New(e.EndTime.Sub(startTime))

				for user, userStartOffset := range activeUsers {
					metadata.Metadata.Events = append(metadata.Metadata.Events, &pb.SessionRecordingEvent{
						StartOffset: userStartOffset,
						EndOffset:   endTime,
			...
			case *apievents.SessionJoin:
				activeUsers[e.User] = durationpb.New(e.Time.Sub(startTime))

			case *apievents.SessionLeave:
				if joinTime, ok := activeUsers[e.User]; ok {
					metadata.Metadata.Events = append(metadata.Metadata.Events, &pb.SessionRecordingEvent{
						StartOffset: joinTime,
			...
			case *apievents.SessionPrint:
				...
				if e.Time.After(nextThumbnailTime) {
					eventStartTime := e.Time
					endTime := e.Time.Add(thumbnailInterval).Add(-1 * time.Millisecond)

					recordThumbnail(eventStartTime, endTime)
			...
			case *apievents.SessionStart:
				...
				metadata.Metadata.StartCols = int32(cols)
				metadata.Metadata.StartRows = int32(rows)
	...
	if metadata.Metadata.Duration == nil {
		metadata.Metadata.Duration = durationpb.New(lastEvent.GetTime().Sub(startTime))
	}
	...
	for _, t := range thumbnails {
		...
		metadata.Frames = append(metadata.Frames, thumbnail)
	}

	thumbnail := getRandomThumbnail(metadata.Frames)

...
func (c *Client) ValidateOIDCAuthCallback(ctx context.Context, q url.Values) (*OIDCAuthResponse, error) {
	protoResp, err := c.APIClient.ValidateOIDCAuthCallback(ctx, ValidateOIDCAuthCallbackRequestToProto(q))
	if err != nil {
		if trace.IsNotImplemented(err) {
			return c.HTTPClient.validateOIDCAuthCallback(ctx, q)
		}
		return nil, trace.Wrap(err)
	}
	return OIDCAuthResponseFromProto(protoResp), nil
}

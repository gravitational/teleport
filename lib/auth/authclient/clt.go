...
var (
	...
	ErrNoMFADevices = &trace.AccessDeniedError{
		Message: "MFA is required to access this resource but user has no MFA devices; see Account Settings in the Web UI or use 'tsh mfa add' via CLI to register MFA devices",
	}
...
func waitForWebSession(ctx context.Context, sessionID, user string, evenSubKind string, req waitForWebSessionReq) error {
	...
	logger := logrus.WithField("session", sessionID)

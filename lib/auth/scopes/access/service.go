...
func (s *Server) CreateScopedRoleAssignment(ctx context.Context, req *scopedaccessv1.CreateScopedRoleAssignmentRequest) (*scopedaccessv1.CreateScopedRoleAssignmentResponse, error) {
	...
	if name := req.GetAssignment().GetMetadata().GetName(); name != "" {
		return nil, trace.BadParameter("cannot create assignment with preset name %q, assignment names are generated automatically", name)
	}

...
func (bs *DatabaseObjectImportRuleService) CreateDatabaseObjectImportRule(
	...
	if err := databaseobjectimportrule.ValidateDatabaseObjectImportRule(req.Item); err != nil {
		return nil, trace.Wrap(err, "validating rule")
	}
	out, err := bs.backend.CreateDatabaseObjectImportRule(ctx, req.Item)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	return out, nil
...
func (bs *DatabaseObjectImportRuleService) UpdateDatabaseObjectImportRule(
	...
	rule, err := bs.backend.UpdateDatabaseObjectImportRule(ctx, req.Item)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	return rule, nil

...
func TestBootstrapDevices(t *testing.T) {
	conf := setupConfig(t)

	device := resourceFromYAML(t, deviceYAML).(*types.DeviceV1)
	conf.BootstrapResources = []types.Resource{device}
	...
	_, err = conf.Backend.Get(context.Background(), backend.Key("devices", "id", "1ef60cb1-9a59-400f-8ec3-a53c40127d19"))
	require.NoError(t, err)

	// TODO(sshah) test device details.

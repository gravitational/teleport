...
type InitConfig struct {
	...
	DynamicAccessExt auth.DynamicAccessExt
...
func Init(cfg InitConfig, opts ...Option) (*Server, error) {
	if cfg.DataDir == "" {
	...
	if trace.IsNotFound(err) || len(jwtSigner.GetJWTKeyPairs()) == 0 {
		...
		jwtSigner, err = auth.NewJWTAuthority(cfg.ClusterName.GetClusterName())
		if err != nil {
			return nil, trace.Wrap(err)
		}
...
func migrateOSS(ctx context.Context, asrv *Server) error {
	...
	role := auth.NewDowngradedOSSAdminRole()
...
func migrateOSSGithubConns(ctx context.Context, role types.Role, asrv *Server) (int, error) {
	...
	for _, conn := range conns {
		...
		for i, team := range teams {
			r := auth.NewOSSGithubRole(team.Logins, team.KubeUsers, team.KubeGroups)
...
func (i *Identity) TLSConfig(cipherSuites []uint16) (*tls.Config, error) {
	...
	tlsConfig.ServerName = auth.EncodeClusterName(i.ClusterName)
...
func migrateMFADevices(ctx context.Context, asrv *Server) error {
	...
	for _, user := range users {
		...
		if len(la.TOTPKey) > 0 {
			d, err := auth.NewTOTPDevice("totp", la.TOTPKey, asrv.clock.Now())
			if err != nil {
				return trace.Wrap(err)
			}

...
func TestListUnifiedResources_ScopedNodes(t *testing.T) {
	...
	srv := newTestTLSServer(
		...
		withScopesFeatures(scopes.Features{Enabled: true, AgentPinEnabled: true}),
	)
	...
	inlineEventually(t, func() bool {
		testNodes, err := srv.Auth().GetNodes(t.Context(), apidefaults.Namespace)
		require.NoError(t, err)
		return len(testNodes) == 5
	}, time.Second*5, time.Millisecond*200)

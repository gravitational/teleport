...
func TestListResources_KindUserGroup(t *testing.T) {
	...
	// Add user groups.
	testUg1 := createUserGroup(t, srv.Auth(), "c", map[string]string{"label": "value"})
	testUg2 := createUserGroup(t, srv.Auth(), "a", map[string]string{"label": "value"})
	testUg3 := createUserGroup(t, srv.Auth(), "b", map[string]string{"label": "value"})

	client, err := srv.NewClient(authtest.TestUserWithRoles(user.GetName(), []string{role.GetName()}))
	require.NoError(t, err)

	// Test create.
	userGroups, _, err := client.ListUserGroups(ctx, 0, "")
	require.NoError(t, err)

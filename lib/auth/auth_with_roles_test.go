...
func TestOktaImportRules(t *testing.T) {
	...
	role, err := types.NewRole("import-rules", types.RoleSpecV6{
		Allow: types.RoleConditions{
			Rules: []types.Rule{
				{
					Resources: []string{types.KindOktaImportRule},
	...
	require.NoError(t, err)
	user, err := CreateUser(srv.AuthServer.AuthServer, "test-user", role)
	require.NoError(t, err)

	client, err := srv.NewClient(TestUser(user.GetName()))
	require.NoError(t, err)

	rules, token, err := client.ListOktaImportRules(ctx, 0, "")
	require.NoError(t, err)
	...
	require.NoError(t, client.CreateOktaImportRule(ctx, r1))
	require.NoError(t, client.CreateOktaImportRule(ctx, r2))
	require.NoError(t, client.CreateOktaImportRule(ctx, r3))

	rules, token, err = client.ListOktaImportRules(ctx, 0, "")
	require.NoError(t, err)
	require.Empty(t, token)
	require.Empty(t, cmp.Diff([]types.OktaImportRule{r1, r2, r3}, rules,
		cmpopts.IgnoreFields(types.Metadata{}, "ID")))

	r1.SetExpiry(time.Now().Add(30 * time.Minute))
	require.NoError(t, client.UpdateOktaImportRule(ctx, r1))

	r, err := client.GetOktaImportRule(ctx, r1.GetName())
	require.NoError(t, err)
	require.Empty(t, cmp.Diff(r1, r,
		cmpopts.IgnoreFields(types.Metadata{}, "ID")))

	require.NoError(t, client.DeleteOktaImportRule(ctx, r1.GetName()))

	rules, token, err = client.ListOktaImportRules(ctx, 0, "")
	require.NoError(t, err)
	require.Empty(t, token)
	require.Empty(t, cmp.Diff([]types.OktaImportRule{r2, r3}, rules,
		cmpopts.IgnoreFields(types.Metadata{}, "ID")))

	require.NoError(t, client.DeleteAllOktaImportRules(ctx))
...
func TestOktaAssignments(t *testing.T) {
	...
	role, err := types.NewRole("assignments", types.RoleSpecV6{
		Allow: types.RoleConditions{
			Rules: []types.Rule{
				{
					Resources: []string{types.KindOktaAssignment},
					Verbs:     []string{types.VerbList, types.VerbRead, types.VerbUpdate, types.VerbCreate, types.VerbDelete},
				},
			},
		},
	...
	assignments, token, err := client.ListOktaAssignments(ctx, 0, "")
	require.NoError(t, err)
	require.Empty(t, token)
	...
	require.NoError(t, client.CreateOktaAssignment(ctx, a1))
	require.NoError(t, client.CreateOktaAssignment(ctx, a2))
	require.NoError(t, client.CreateOktaAssignment(ctx, a3))

	assignments, token, err = client.ListOktaAssignments(ctx, 0, "")
	require.NoError(t, err)
	require.Empty(t, token)
	require.Empty(t, cmp.Diff([]types.OktaAssignment{a1, a2, a3}, assignments,
		cmpopts.IgnoreFields(types.Metadata{}, "ID")))

	a1.SetExpiry(time.Now().Add(30 * time.Minute))
	require.NoError(t, client.UpdateOktaAssignment(ctx, a1))

	a, err := client.GetOktaAssignment(ctx, a1.GetName())
	require.NoError(t, err)
	require.Empty(t, cmp.Diff(a1, a,
		cmpopts.IgnoreFields(types.Metadata{}, "ID")))

	require.NoError(t, client.DeleteOktaAssignment(ctx, a1.GetName()))

	assignments, token, err = client.ListOktaAssignments(ctx, 0, "")
	require.NoError(t, err)
	require.Empty(t, token)
	require.Empty(t, cmp.Diff([]types.OktaAssignment{a2, a3}, assignments,
		cmpopts.IgnoreFields(types.Metadata{}, "ID")))

	require.NoError(t, client.DeleteAllOktaAssignments(ctx))
...
func newOktaImportRule(t *testing.T, name string) types.OktaImportRule {

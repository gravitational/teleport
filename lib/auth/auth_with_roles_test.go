...
func TestInstaller(t *testing.T) {
	...
	inst, err := types.NewInstallerV1(libdefaults.InstallerScriptName, "contents")
	require.NoError(t, err)
	...
	for _, tc := range []struct {
		...
		installerAction: func(c *Client) error {
			_, err := c.GetInstaller(ctx, libdefaults.InstallerScriptName)
			return err
		},
	}, {
		...
		assert: require.NoError,
		installerAction: func(c *Client) error {
			inst, err := types.NewInstallerV1(libdefaults.InstallerScriptName, "new-contents")
			require.NoError(t, err)
		...
		assert: require.NoError,
		installerAction: func(c *Client) error {
			err := c.DeleteInstaller(ctx, libdefaults.InstallerScriptName)
			return err
		},
	}} {

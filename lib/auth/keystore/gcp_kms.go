...
func newGCPKMSKeyStore(cfg *GCPKMSConfig) (*gcpKMSKeyStore, error) {
	var kmsClient kmsClientI
	if cfg.kmsClientOverride != nil {
		kmsClient = cfg.kmsClientOverride
	} else {
		realClient, err := kms.NewKeyManagementClient(context.Background())
		if err != nil {
			return nil, trace.Wrap(err)
		}
		kmsClient = &kmsClientWrapper{realClient}
	}
	...
	return &gcpKMSKeyStore{
		...
		kmsClient:       kmsClient,
		log:             logrus.WithFields(logrus.Fields{trace.Component: "GCPKMSKeyStore"}),
		clock:           clock,
	}, nil
}
...
func (g *gcpKMSKeyStore) generateRSA(opts ...RSAKeyOption) ([]byte, crypto.Signer, error) {
	...
	resp, err := doGCPRequest(ctx, g.clock, g.kmsClient.CreateCryptoKey, req)
	if err != nil {
...
func (g *gcpKMSKeyStore) deleteKey(ctx context.Context, rawKey []byte) error {
	...
	_, err = retryWhilePending(ctx, g.clock, g.kmsClient.DestroyCryptoKeyVersion, req)
...
func (g *gcpKMSKeyStore) newKmsSigner(ctx context.Context, keyID gcpKMSKeyID) (*kmsSigner, error) {
	...
	resp, err := retryWhilePending(ctx, g.clock, g.kmsClient.GetPublicKey, req)
	if err != nil {
...
func (s *kmsSigner) Sign(rand io.Reader, digest []byte, opts crypto.SignerOpts) (signature []byte, err error) {
	...
	resp, err := doGCPRequest(ctx, s.clock, s.kmsClient.AsymmetricSign, &kmspb.AsymmetricSignRequest{
...
func parseGCPKMSKeyID(key []byte) (gcpKMSKeyID, error) {
	...
	// strip gcpkms: prefix
	keyID.keyVersionName = string(key[len(gcpkmsPrefix):])
	return keyID, nil
}

// withTimeout is like context.WithTimeout, but with mockable time
func withTimeout(ctx context.Context, clock clockwork.Clock, d time.Duration) (context.Context, func()) {
...
func retryWhilePending[reqType, optType, respType any](
	...
	ctx, cancel := withTimeout(ctx, clock, defaultGCPPendingTimeout)
	defer cancel()

	ticker := clock.NewTicker(defaultGCPPendingRetryInterval)
	defer ticker.Stop()
	for {
		resp, err := doGCPRequest(ctx, clock, f, req)
		if err == nil {
			return resp, nil
		}
...
func doGCPRequest[reqType, optType, respType any](
	...
	ctx, cancel := withTimeout(ctx, clock, defaultGCPRequestTimeout)
	defer cancel()
	resp, err := f(ctx, req)
	return resp, trace.Wrap(err)
}

// kmsClientI is a mockable interface for the GCP KMS client.
type kmsClientI interface {
	...
	ListCryptoKeys(ctx context.Context, req *kmspb.ListCryptoKeysRequest, opts ...gax.CallOption) cryptoKeyIteratorI
...
// cryptoKeyIteratorI is a mockable interface for the GCP KMS crypto key
// iterator type.
type cryptoKeyIteratorI interface {
	Next() (*kmspb.CryptoKey, error)
}

// kmsClientWrappers wraps the real kms client so that we can override
// ListCryptoKeys to return a mockable interface instead of a custom iterator
// type that would be difficult/impossible to mock.
type kmsClientWrapper struct {
	*kms.KeyManagementClient
}

func (r *kmsClientWrapper) ListCryptoKeys(ctx context.Context, req *kmspb.ListCryptoKeysRequest, opts ...gax.CallOption) cryptoKeyIteratorI {
	return r.KeyManagementClient.ListCryptoKeys(ctx, req, opts...)
}

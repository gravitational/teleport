...
func getCertRequest(req *GenerateCredentialsRequest) (*certRequest, error) {
	...
	if req.ActiveDirectorySID != "" {
		fmt.Printf("req.ActiveDirectorySID = %v\n", req.ActiveDirectorySID)
		adUserMapping, err := asn1.Marshal(SubjectAltNameAdSid{
			otherNameAdSid{
...
// SubjectAltNameUPN is a struct that can be marshaled as ASN.1
...
type SubjectAltNameUPN struct {
	OtherName otherNameUPN `asn1:"tag:0"`
}
...
type SubjectAltNameAdSid struct {
	OtherName otherNameAdSid `asn1:"tag:0"`
}

type otherNameAdSid struct {
	OID   asn1.ObjectIdentifier
	Value adSid `asn1:"tag:0"`
}

...
// CreateSessionChallenge implements the mfav2.MFAServiceServer.CreateSessionChallenge method.
func (s *Service) CreateSessionChallenge(
	ctx context.Context,
	req *mfav2.CreateSessionChallengeRequest,
) (*mfav2.CreateSessionChallengeResponse, error) {
	...
	if req.GetTargetCluster() != "" {
		if err := s.clusterExists(ctx, req.GetTargetCluster()); err != nil {
			return nil, trace.Wrap(err)
		}
	}
	...
	targetClusterName := cmp.Or(req.GetTargetCluster(), sourceClusterName)

	// Build the MFA challenge components.
	var webauthnChallenge *webauthnv2.CredentialAssertion
	var ssoChallenge *mfav2.SSOChallenge
	...
	if enableWebauthn && len(supportedMFADevices.Webauthn) > 0 {
		...
		assertion, err := webLogin.Begin(
			...
			wanlib.BeginParams{
				...
				SessionIdentifyingPayload: req.GetPayload(),
		...
		webauthnChallenge = wantypes.CredentialAssertionToProtoV2(assertion)
	}

	// If SSO is enabled, the user has an SSO device and the client provided a redirect URL and proxy address, create an
	// SSO challenge.
	if enableSSO && supportedMFADevices.SSO != nil && req.GetSsoClientRedirectUrl() != "" && req.GetProxyAddressForSso() != "" {
		ssoChallengeResp, err := s.authServer.BeginSSOMFAChallenge(
			ctx,
			mfatypes.BeginSSOMFAChallengeParams{
				User:                 username,
				SSO:                  supportedMFADevices.SSO.GetSso(),
				SSOClientRedirectURL: req.GetSsoClientRedirectUrl(),
				ProxyAddress:         req.GetProxyAddressForSso(),
		...
		ssoChallenge = mfav2.SSOChallenge_builder{
			Device: mfav2.SSOMFADevice_builder{
				ConnectorId:   supportedMFADevices.SSO.GetSso().ConnectorId,
				ConnectorType: supportedMFADevices.SSO.GetSso().ConnectorType,
				DisplayName:   supportedMFADevices.SSO.GetSso().DisplayName,
			}.Build(),
			RequestId:   ssoChallengeResp.RequestId,
			RedirectUrl: ssoChallengeResp.RedirectUrl,
		}.Build()
	}

	authenticateChallenge := mfav2.AuthenticateChallenge_builder{
		Name:              challengeName,
		WebauthnChallenge: webauthnChallenge,
		SsoChallenge:      ssoChallenge,
	}.Build()
	challenge := mfav2.CreateSessionChallengeResponse_builder{
	...
	s.logger.DebugContext(
		...
		"Created challenge",
		"user", username,
		"has_webauthn_challenge", webauthnChallenge != nil,
		"has_sso_challenge", ssoChallenge != nil,
	)

	return challenge, nil
}

// ValidateSessionChallenge implements the mfav2.MFAServiceServer.ValidateSessionChallenge method.
func (s *Service) ValidateSessionChallenge(
	ctx context.Context,
	req *mfav2.ValidateSessionChallengeRequest,
) (*mfav2.ValidateSessionChallengeResponse, error) {
	authCtx, err := s.authorizer.Authorize(ctx)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
// ReplicateValidatedMFAChallenge implements the mfav2.MFAServiceServer.ReplicateValidatedMFAChallenge method.
func (s *Service) ReplicateValidatedMFAChallenge(
	ctx context.Context,
	req *mfav2.ReplicateValidatedMFAChallengeRequest,
) (*mfav2.ReplicateValidatedMFAChallengeResponse, error) {
	authCtx, err := s.authorizer.Authorize(ctx)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	...
	return mfav2.ReplicateValidatedMFAChallengeResponse_builder{
...
func validateCreateSessionChallengeRequest(req *mfav2.CreateSessionChallengeRequest) error {
	...
	switch {
	case req.GetSsoClientRedirectUrl() != "" && req.GetProxyAddressForSso() == "":
...
func validateValidateSessionChallengeRequest(req *mfav2.ValidateSessionChallengeRequest) error {
	if req == nil {
	...
	if mfaResp.GetWebauthn() == nil && mfaResp.GetSso() == nil {
		return trace.BadParameter("missing Webauthn or SSO response in AuthenticateResponse")
	}
...
func (s *Service) validateWebauthnResponse(
	ctx context.Context,
	username string,
	mfaResp *mfav2.AuthenticateResponse,
) (*authz.MFAAuthData, error) {
	pref, err := s.cache.GetAuthPreference(ctx)
	if err != nil {
		return nil, trace.Wrap(err)
	...
	loginData, err := webLogin.Finish(
		ctx,
		username,
		wantypes.CredentialAssertionResponseFromProtoV2(mfaResp.GetWebauthn()),
...
func (s *Service) validateSSOResponse(
	...
	authData, err := s.authServer.VerifySSOMFASession(
		ctx,
		username,
		mfaResp.GetSso().GetRequestId(),
		mfaResp.GetSso().GetToken(),
		&mfav1.ChallengeExtensions{Scope: mfav1.ChallengeScope_CHALLENGE_SCOPE_USER_SESSION},
	)
	if err != nil {
...
func (s *Service) waitForValidatedMFAChallenge(ctx context.Context, targetCluster, challengeName string) (*mfav2.ValidatedMFAChallenge, error) {
	...
	if err := retry.For(
		...
		func() error {
			...
			chal, err = s.storage.GetValidatedMFAChallenge(ctx, targetCluster, challengeName)
			if err != nil {
				return trace.Wrap(err)
			}
...
func checkPayload(sip *mfav2.SessionIdentifyingPayload) error {
	if sip == nil {
...
// CompleteBrowserMFAChallenge returns NotImplemented; Browser MFA is served by mfav1.
//
// TODO(danielashare): Port browser MFA implementation from mfav1 to mfav2.
func (s *Service) CompleteBrowserMFAChallenge(_ context.Context, _ *mfav2.CompleteBrowserMFAChallengeRequest) (*mfav2.CompleteBrowserMFAChallengeResponse, error) {
	return nil, trace.NotImplemented("browser MFA is handled by mfav1.Service")
}

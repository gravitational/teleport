...
func (c *Client) GetAuthPreference(ctx context.Context) (types.AuthPreference, error) {
	...
	if err != nil {
		...
		out, err := c.Get(ctx, c.Endpoint("authentication", "preference"), url.Values{})
		if err != nil {
			return nil, trace.Wrap(err)
		}
...
func (c *Client) SetAuthPreference(ctx context.Context, cap types.AuthPreference) error {
	...
	_, err = c.PostJSON(ctx, c.Endpoint("authentication", "preference"), &setClusterAuthPreferenceReq{ClusterAuthPreference: data})
	if err != nil {
		return trace.Wrap(err)
	}

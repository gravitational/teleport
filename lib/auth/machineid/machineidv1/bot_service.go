...
func UpsertBot(
	...
	if existingUser != nil {
		// UpsertUser overwrites unconditionally, so this check is the only
		// protection against clobbering an unrelated user named bot-<name>.
		if botLabel, _ := existingUser.GetLabel(types.BotLabel); botLabel != bot.GetMetadata().GetName() {
			return nil, trace.BadParameter(
				"a user %q exists but does not belong to this bot, delete it before recreating the bot",

...
func TestIssueTeleportWorkloadIdentity(t *testing.T) {
	...
	aliceExpiredSessionCertBytes, aliceSessionExpiresAt := createAppSessionCertBytes(t, aliceClt, appName, alice.GetName())
	tp.clock.Advance(tp.clock.Until(aliceSessionExpiresAt))
	...
	for name, tc := range map[string]struct {
		...
		"app service generates workload identity for a session": {
			...
			expectResp: func(tt require.TestingT, i1 any, i2 ...any) {
				...
				td, err := spiffeid.TrustDomainFromString("_teleport_app." + tp.srv.ClusterName())
				require.NoError(tt, err)

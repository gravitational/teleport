...
func (f *LoginFlow) Begin(ctx context.Context, user string, scope wanpb.ChallengeScope, allowReuse bool) (*wantypes.CredentialAssertion, error) {
	...
	return lf.begin(ctx, user, scope, allowReuse)
}
...
// the returned device.
func (f *LoginFlow) Finish(ctx context.Context, user string, resp *wantypes.CredentialAssertionResponse, scope wanpb.ChallengeScope, allowReuse bool) (*types.MFADevice, error) {
	lf := &loginFlow{
		U2F:         f.U2F,
		Webauthn:    f.Webauthn,
		identity:    mfaIdentity{f.Identity},
	...
	dev, _, err := lf.finish(ctx, user, scope, allowReuse, resp)

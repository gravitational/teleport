...
func (f *PasswordlessFlow) Finish(ctx context.Context, resp *wantypes.CredentialAssertionResponse) (*WebauthnSessionData, error) {
	lf := &loginFlow{
		Webauthn:    f.Webauthn,
		identity:    passwordlessIdentity{f.Identity},
		sessionData: (*globalSessionStorage)(f),
	}
	return lf.finish(ctx, "" /* user */, wanpb.ChallengeScope_CHALLENGE_SCOPE_PASSWORDLESS_LOGIN, resp)
}

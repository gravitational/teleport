...
func TestMaybeDowngradeRoleVersionToV8(t *testing.T) {
	...
	// Built directly rather than via NewRoleWithVersion: write-time
	// validation currently rejects every rule field except
	// unsafe_allow_all, and the downgrade must strip the new fields even
	// on roles written by a later release during a rolling upgrade.
	newV9Role := func(rules []types.AppResource, expressions []string) *types.RoleV6 {
		return &types.RoleV6{
			Kind:     types.KindRole,
			...
			Spec: types.RoleSpecV6{
				Allow: types.RoleConditions{
					...
					AppResources:            rules,
					AppResourcesExpressions: expressions,
				},
			},
		}
	}
	...
	unsafeAllowAll := []types.AppResource{{UnsafeAllowAll: true}}
	pathRules := []types.AppResource{{Paths: []string{"/health"}}}
	...
	for _, tc := range []struct {
		...
		{
			desc:          "v18 client downgrades to v8",
			clientVersion: "18.1.2",
			wantVersion:   types.V8,
		},
		{
			desc:          "v17 client downgrades to v8",
			clientVersion: "17.0.0",
			wantVersion:   types.V8,
		},
	} {
		t.Run(tc.desc, func(t *testing.T) {
			input := newV9Role(unsafeAllowAll, expressions)
			...
			if tc.wantVersion == types.V9 {
				...
				require.NotEmpty(t, got.Spec.Allow.AppResourcesExpressions)
	...
	t.Run("unsafe_allow_all keeps app access", func(t *testing.T) {
		got := auth.MaybeDowngradeRoleVersionToV8(newV9Role(unsafeAllowAll, nil), clientVersion(t, "18.1.2"))
	...
	t.Run("path rules fail closed", func(t *testing.T) {
		got := auth.MaybeDowngradeRoleVersionToV8(newV9Role(pathRules, nil), clientVersion(t, "18.1.2"))
		require.Empty(t, got.Spec.Allow.AppLabels)
		require.Empty(t, got.Spec.Allow.AppLabelsExpression)
		require.Contains(t, got.GetMetadata().Labels[types.TeleportDowngradedLabel], "fail closed")
	})

	t.Run("default-deny role fails closed", func(t *testing.T) {
		got := auth.MaybeDowngradeRoleVersionToV8(newV9Role(nil, nil), clientVersion(t, "18.1.2"))
		require.Empty(t, got.Spec.Allow.AppLabels)
		require.Empty(t, got.Spec.Allow.AppLabelsExpression)
		require.Contains(t, got.GetMetadata().Labels[types.TeleportDowngradedLabel], "fail closed")
	})

	t.Run("expressions without unsafe_allow_all fail closed", func(t *testing.T) {
		got := auth.MaybeDowngradeRoleVersionToV8(newV9Role(nil, expressions), clientVersion(t, "18.1.2"))
		require.Empty(t, got.Spec.Allow.AppLabels)
		require.Empty(t, got.Spec.Allow.AppLabelsExpression)
		require.Contains(t, got.GetMetadata().Labels[types.TeleportDowngradedLabel], "fail closed")
	})

	t.Run("deny app rules fail closed even with unsafe_allow_all", func(t *testing.T) {
		input := newV9Role(unsafeAllowAll, nil)
		input.Spec.Deny.AppResources = pathRules
...
func TestRoleVersionV9Downgrade(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{
			desc:          "v18 client gets v8 with app labels kept for unsafe_allow_all",
			clientVersion: "18.1.2",
			roleName:      "v9_allow_all",
			wantVersion:   types.V8,
			wantAppLabels: true,

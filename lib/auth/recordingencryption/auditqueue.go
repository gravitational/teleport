...
type AuditQueueSealer struct {
	...
	wg           sync.WaitGroup
	state        atomic.Pointer[encryptionState]
	fetchFailing bool
}
...
func NewAuditQueueSealer(ctx context.Context, client SessionRecordingConfigWatcher) (*AuditQueueSealer, error) {
	...
	if err := sealer.refreshWithTimeout(ctx); err != nil {
...
func (s *AuditQueueSealer) encryptionState() (*encryptionState, error) {
	state := s.state.Load()
	if state == nil {
		return nil, trace.Wrap(errors.New("audit queue sealer has not resolved the encryption keys"))
	}
	return state, nil
}
...
func (s *AuditQueueSealer) watch() error {
	...
	if err := s.refreshWithTimeout(s.loopCtx); err != nil {
		s.warnOnFailureStreak(err)
	}
...
func (s *AuditQueueSealer) handleEvent(event types.Event) error {
	...
	case types.OpDelete:
		return trace.Wrap(s.refreshWithTimeout(s.loopCtx))
	default:
		return nil
	}
}

func (s *AuditQueueSealer) refreshWithTimeout(ctx context.Context) error {
	ctx, cancel := context.WithTimeout(ctx, auditQueueSealerRefreshTimeout)
	defer cancel()
...
func (s *AuditQueueSealer) applyConfig(config types.SessionRecordingConfig) error {
	state, err := parseRecipients(config)
	if err != nil {
		return trace.Wrap(err)
	}
...
func parseRecipients(config types.SessionRecordingConfig) (*encryptionState, error) {
	...
	if len(keys) == 0 {
		return nil, trace.BadParameter("session recording encryption is enabled but no encryption keys are available")
	}
	recipients := make([]age.Recipient, 0, len(keys))
	for _, key := range keys {
		recipient, err := ParseRecordingRecipient(key.PublicKey)
		if err != nil {
			return nil, trace.Wrap(err, "parsing session recording encryption key")
		}
		recipients = append(recipients, recipient)
	}
	return &encryptionState{encrypted: true, recipients: recipients}, nil
}

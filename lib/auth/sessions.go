...
func (a *Server) CreateWebSessionFromReq(ctx context.Context, req sessionreq.NewWebSessionRequest) (types.WebSession, error) {
	if req.Scope != "" {
...
func (a *Server) CreateAppSessionFromReq(ctx context.Context, req sessionreq.NewAppSessionRequest) (types.WebSession, error) {
	if !modules.GetModules().Features().GetEntitlement(entitlements.App).Enabled {
		return nil, trace.AccessDenied(
			"this Teleport cluster is not licensed for application access, please contact the cluster administrator")
	}

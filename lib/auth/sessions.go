...
func (a *Server) generateAppToken(ctx context.Context, req types.GenerateAppTokenRequest) (string, error) {
	...
	switch req.JwtAuthority {
	case "", types.JwtAuthorityDefault:
		caType = types.JWTSigner
	case types.JwtAuthorityOIDC:
		caType = types.OIDCIdPCA
	default:
		return "", trace.BadParameter("unsupported jwt authority %q", req.JwtAuthority)
	}

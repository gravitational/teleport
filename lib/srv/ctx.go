...
func (c *ServerContext) setSession(sess *session, ch ssh.Channel) {
	...
	go func() {
		c.Logger.DebugContext(c.CancelContext(), "Sending current session ID")
		_, err := ch.SendRequest(teleport.CurrentSessionIDRequest, false, []byte(sess.ID()))
		if err != nil {
			c.Logger.DebugContext(c.CancelContext(), "Failed to send the current session ID", "error", err)
		}
	}()
}
		...
		for {
			...
			if err != nil {
				if !utils.IsOKNetworkError(err) {
					c.Logger.DebugContext(c.CancelContext(), "Encountered error accepting XServer connection", "error", err)
				}
				return
			}
			...
			go func() {
				...
				if err != nil {
					c.Logger.DebugContext(c.CancelContext(), "Failed to open a new X11 channel", "error", err)
					return
				}
				defer xchan.Close()
...
func (c *ServerContext) SendExecResult(r ExecResult) {
	...
	default:
		c.Logger.InfoContext(c.CancelContext(), "Blocked on sending exec result", "code", r.Code, "command", r.Command)
	}
}
...
func (c *ServerContext) SendSubsystemResult(r SubsystemResult) {
	...
	default:
		c.Logger.InfoContext(c.CancelContext(), "Blocked on sending subsystem result")
	}
}

...
// NewHealthChecker returns an endpoint health checker for BigQuery.
...
// getEndpoint returns the BigQuery endpoint to dial.
func getEndpoint(db types.Database) string {
	if uri := db.GetURI(); uri != "" {
		return uri
	}
	return gcputils.BigQueryEndpoint
}

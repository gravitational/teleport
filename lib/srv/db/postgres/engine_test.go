...
// testEnv is the shared fixture for the provisioning tests. Every connection
// is closed via t.Cleanup, so callers never close one explicitly.
type testEnv struct {
	db                   types.Database
	engine               *Engine
...
// randomPassword returns a cryptographically random hex password. Generated
// per run so no test credential is hardcoded (the RDS instance is publicly
// reachable); hex is safe in both a URL and a SQL PASSWORD literal.
...
func runUserAutoProvisioningTests(t *testing.T, env *testEnv) {
	...
	t.Run("DeleteUser with reassignment", func(t *testing.T) {
		...
		t.Run("Refused: source user does not exist", func(t *testing.T) {
			// Called directly: the procedure rejects a source_user with no
			// pg_roles row. DeleteUser only ever passes an existing user.
			_, err := env.adminConn.Exec(t.Context(),

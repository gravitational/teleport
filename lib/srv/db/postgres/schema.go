...
const schemaInfoQuery = "SELECT schemaname, tablename FROM pg_tables"
	...
	for schemaName, schemaVal := range s {
		for _, table := range schemaVal.tables {
			...
			obj, err := databaseobject.NewDatabaseObject(name, &dbobjectv1.DatabaseObjectSpec{
				...
				DbServiceName: session.Database.GetName(),
			...
			obj.Metadata.Expires = timestamppb.New(time.Now().Add(permissions.ObjectTTL))

...
func (s *TestServer) handleDeactivateUser(client *pgproto3.Backend, sendDeleteResponse bool) error {
	...
	// Extract object inheritor user.
	objectInheritorUser := ""
	if sendDeleteResponse {
		objectInheritorUser, err = getVarchar(bind.ParameterFormatCodes[1], bind.Parameters[1])
		if err != nil {
			return trace.Wrap(err)
		}
	}
	...
	// Mark the user as active.
	if sendDeleteResponse {
		s.log.DebugContext(context.Background(), "Deleted user.", "user", name, "object_inheritor_user", objectInheritorUser)
	} else {

...
func TestApplyDatabaseObjectImportRules(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			database: mkDatabase("dummy", map[string]string{"env": "prod"}),
			objs: []*dbobjectv1.DatabaseObject{
				mkDatabaseObject("foo", &dbobjectv1.DatabaseObjectSpec{ObjectKind: ObjectKindTable}),
			},
			want: []*dbobjectv1.DatabaseObject{
				mkDatabaseObject("foo", &dbobjectv1.DatabaseObjectSpec{ObjectKind: ObjectKindTable}, func(db *dbobjectv1.DatabaseObject) error {

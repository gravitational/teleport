...
func newTestTrace(runCommand func(name string, arg ...string) ([]byte, error)) *Trace {
	clock := clockwork.NewFakeClockAt(time.Date(2024, time.December, 4, 0, 0, 0, 0, time.UTC))
	testTrace := NewTrace(clock)
	testTrace.runCommand = runCommand

	testTrace.AddPacket(ClientToTeleport, []byte("hello server, this is client"))
	clock.Advance(time.Second)

	testTrace.AddPacket(TeleportToServer, []byte("hello server, this is Teleport"))
	clock.Advance(time.Second)

	testTrace.AddPacket(ServerToTeleport, []byte("hello Teleport, this is server"))
	clock.Advance(time.Second)

	testTrace.AddPacket(TeleportToClient, []byte("hello client, this is server (actually Teleport, wink)"))
	clock.Advance(time.Second)

	return testTrace
}

func TestTrace_SaveAsText(t *testing.T) {
	testTrace := newTestTrace(func(name string, arg ...string) ([]byte, error) {
		assert.FailNow(t, "runCommand shouldn't be called for SaveAsText")
		return nil, trace.BadParameter("not expected")
	})

	tmp := t.TempDir()
	fileOut := path.Join(tmp, "trace.txt")

	err := testTrace.SaveAsText(fileOut)
...
func TestTrace_SaveToPCAP(t *testing.T) {
	tmp := t.TempDir()
	fileOut := path.Join(tmp, "trace.pcap")

	runCommandCount := 0

	testTrace := newTestTrace(func(progName string, args ...string) ([]byte, error) {
		runCommandCount++
...
`, string(hexData))
	...
	err := testTrace.SaveToPCAP(fileOut, 1111)
	require.NoError(t, err)

...
func ConvertConnectError(err error, sessionCtx *Session) error {
	...
	case strings.Contains(errString, "x509: certificate has expired or is not yet valid"):
		return trace.AccessDenied("Database service cannot validate the database's certificate: %s. Please verify if the database certificate is outdated and the database service time and date are set correctly.", err)

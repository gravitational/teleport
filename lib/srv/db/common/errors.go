...
// extra information/recommandations.
func ConvertConnectError(err error, sessionCtx *Session) error {
	...
	case strings.Contains(errString, "x509: certificate signed by unknown authority"):
		return trace.AccessDenied("Database service cannot validate database's certificate: %v. Please verify if the correct CA bundle is used in the database config.")

	case strings.Contains(errString, "tlsca: unknown certificate authority"):

...
func (e *Engine) connect(ctx context.Context, sessionCtx *common.Session) (*serverConnection, error) {
	deployment, err := depCache.get(ctx, sessionCtx, e.connectDeployment)
	if err != nil {
...
// handshake performs the actual handshake only once and saves the reply to replay
// for new clients in the same session.
func (c *serverConnection) handshake(ctx context.Context, clientMessage protocol.Message, maxMessageSize uint32) replyIterator {
	return func(yield func(protocol.Message, error) bool) {
		c.deployment.handshakeOnce.Do(func() {
			for reply, err := range c.roundTrip(ctx, clientMessage, maxMessageSize) {
				c.deployment.handshakeReplies = append(c.deployment.handshakeReplies, reply)
				c.deployment.handshakeErrors = append(c.deployment.handshakeErrors, err)
			}
		})
		for i := range len(c.deployment.handshakeReplies) {
			reply := c.deployment.handshakeReplies[i]
			err := c.deployment.handshakeErrors[i]
			if !yield(reply, err) {
				return
			}
		}
...
func (c *serverConnection) close(ctx context.Context) error {
	return trace.NewAggregate(c.deployment.close(ctx), c.connection.Close())
}
...
// It is used to select and connect to a server in the topology, but only
// handshake once.
type deployment struct {
	...
	handshakeReplies []protocol.Message
	handshakeErrors  []error
}
...
func (d *deployment) close(ctx context.Context) error {
	shouldClose := depCache.put(d.sessionCtx)
	if shouldClose {
		if err := d.top.Disconnect(ctx); err != nil {

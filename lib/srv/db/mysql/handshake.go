...
func waitForAgent(ctx context.Context, mode handshakeMode, serviceConn net.Conn, handshakeOpts ...client.Option) error {
	...
	case handshakeWhenSupported:
		...
		if !isHandshakeV10 {
			...
			if err != nil || !isMagicOKResult(pkt) {
				return trace.Wrap(err)
			}
...
func notifyProxy(ctx context.Context, log *slog.Logger, mode handshakeMode, proxyConn *server.Conn) error {
	...
	case handshakeWhenSupported:
		...
		found, err := waitForMagicProxyReply(bufConn, 10*time.Second)
		if err != nil {
			log.DebugContext(ctx, "Proxy did not reply to handshake request, assuming that this proxy does not support MySQL handshakes",
				"error", err,
			)
			return nil
		}

...
func (s *Server) stopDatabase(ctx context.Context, db types.Database) error {
	var errors []error
	if err := s.stopDatabaseObjectsImporter(db); err != nil {
...
// stopDatabaseObjectsImporter stops the object importer for a database.
func (s *Server) stopDatabaseObjectsImporter(db types.Database) error {
	err := s.cfg.DatabaseObjects.StopImporter(db.GetName())
	if err != nil && !trace.IsNotFound(err) {
		return trace.Wrap(err)
	}
	return nil

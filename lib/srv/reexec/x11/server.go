...
func OpenNewXServerListener(displayOffset int, maxDisplay int, screen uint32) (net.Listener, Display, error) {
	if displayOffset > maxDisplay {
		return nil, Display{}, fmt.Errorf("displayOffset (%d) cannot be larger than maxDisplay (%d)", displayOffset, maxDisplay)
	} else if maxDisplay > MaxDisplayNumber {
		return nil, Display{}, fmt.Errorf("maxDisplay (%d) cannot be larger than the max int32 (%d)", maxDisplay, math.MaxInt32)
	}
	...
	return nil, Display{}, trace.Errorf("No more X11 sockets are available")
}

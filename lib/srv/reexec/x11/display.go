...
func (d *Display) getNetAddr() (net.Addr, error) {
	...
	return nil, errors.Join(unixErr, tcpErr)
}
...
func (d *Display) unixSocket() (*net.UnixAddr, error) {
	...
	return nil, fmt.Errorf("display is not a unix socket")
}
...
func ParseDisplayFromUnixSocket(socket string) (Display, error) {
	if filepath.Dir(socket) != x11SockDir() {
		return Display{}, fmt.Errorf("parsing x11 sockets outside of the standard /tmp/.X11-unix path is not supported")
	}
...
func (d *Display) tcpSocket() (*net.TCPAddr, error) {
	if d.HostName == "" {
		return nil, fmt.Errorf("display is not a tcp socket, hostname can't be empty")
	}
...
func GetXDisplay() (Display, error) {
	...
	if displayString == "" {
		return Display{}, fmt.Errorf("$DISPLAY not set")
	}
...
func ParseDisplay(displayString string) (Display, error) {
	if displayString == "" {
		return Display{}, fmt.Errorf("display cannot be an empty string")
	}
	...
	for _, c := range displayString {
		if !unicode.IsLetter(c) && !unicode.IsNumber(c) && !strings.ContainsRune(allowedSpecialChars, c) {
			return Display{}, fmt.Errorf("display contains invalid character %q", c)
		}
	}
	...
	if colonIdx == -1 || len(displayString) == colonIdx+1 {
		return Display{}, fmt.Errorf("display value is missing display number")
	}

...
func (x *XAuthCommand) ReadEntry(display Display) (*XAuthEntry, error) {
	...
	if len(out) == 0 {
		return nil, fmt.Errorf("no xauth entry found")
	}
	...
	if len(splitEntry) != 3 {
		return nil, fmt.Errorf("invalid xAuthEntry, expected entry to have three parts")
	}
...
func (x *XAuthCommand) output() ([]byte, error) {
	...
	if err := x.Wait(); err != nil {
		return nil, fmt.Errorf("command %+q failed with stderr: %+q: %w", x.Cmd.Args, errOut, err)
	}
...
func ReadAndRewriteXAuthPacket(xreq io.Reader, spoofedXAuthEntry, realXAuthEntry *XAuthEntry) ([]byte, error) {
	if spoofedXAuthEntry.Proto != realXAuthEntry.Proto || len(spoofedXAuthEntry.Cookie) != len(realXAuthEntry.Cookie) {
		return nil, fmt.Errorf("spoofed and real xauth entries must use the same xauth protocol")
	}
	...
	if _, err := io.ReadFull(xreq, initBuf); err != nil {
		return nil, fmt.Errorf("X11 channel initial packet buffer missing or too short: %w", err)
	}
	...
	if _, err := io.ReadFull(xreq, authPacket); err != nil {
		return nil, fmt.Errorf("X11 channel auth packet missing or too short: %w")
	}
	...
	if string(proto) != spoofedXAuthEntry.Proto || hex.EncodeToString(authData) != spoofedXAuthEntry.Cookie {
		return nil, fmt.Errorf("X11 channel has the wrong authentication data")
	}
	...
	return append(initBuf, authPacket...), err
}
...
func readXauthPacketInitBuf(initBuf []byte) (protoLen int, dataLen int, err error) {
	...
	default:
		return 0, 0, fmt.Errorf("X11 channel auth packet has invalid byte order: %v", initBuf[0])
	}

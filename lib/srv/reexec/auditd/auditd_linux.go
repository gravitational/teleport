...
func (c *Client) isEnabledUnderMutex() (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to get auditd status: %w", (err))
	}
...
func getAuditStatus(conn NetlinkConnector) (*auditStatus, error) {
	...
	if len(msgs) != 1 {
		return nil, fmt.Errorf("returned wrong messages number, expected 1, got: %d", len(msgs))
	}
...
func (c *Client) SendMsg(event EventType, result ResultType) error {
	...
	return (c.sendMsg(netlink.HeaderType(event), buf.Bytes()))
}

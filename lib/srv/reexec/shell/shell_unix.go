...
func getLoginShell(username string) (string, error) {
	...
	if bufSize <= 0 || bufSize > 1<<20 {
		return "", fmt.Errorf("unreasonable _SC_GETPW_R_SIZE_MAX of %d", bufSize)
	}
	...
	if rv != 0 || result == nil {
		...
		return "", fmt.Errorf("cannot determine shell for %s", username)
	}

	// If no shell was found, return fmt.Errorf to allow the caller to set
	// the default shell.
	shellCmd := strings.TrimSpace(C.GoString(pwd.pw_shell))
	if len(shellCmd) == 0 {
		return "", fmt.Errorf("no shell specified for %v", username)
	}

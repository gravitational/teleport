...
func FileContexts(dataDir, configPath, execPathOverride string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("failed to parse file contexts template: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("failed to expand symlinks for the executable: %w", err)
	}
	...
	if err != nil {
		return "", fmt.Errorf("failed to expand file contexts template: %w", err)
	}
...
func parseLabel(label string) (*selinuxContext, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse SELinux context: %w", err)
	}
...
func CheckConfiguration(ensureEnforced bool, logger *slog.Logger) error {
	...
	if !ocselinux.GetEnabled() {
		return fmt.Errorf("SELinux is disabled or not present")
	}
	if ocselinux.EnforceMode() == ocselinux.Disabled {
		return fmt.Errorf("SELinux mode is disabled, SELinux will not enforce or log anything")
	}
	if ocselinux.EnforceMode() == ocselinux.Permissive {
		if ensureEnforced {
			return fmt.Errorf("SELinux mode is permissive, SELinux will not enforce rules only log denials")
		} else {
	...
	if err != nil {
		return fmt.Errorf("failed to get SELinux context: %w")
	}
	...
	if !installed {
		return fmt.Errorf("the SSH SELinux module %s is not installed", moduleName)
	}
	if disabled {
		return fmt.Errorf("the SSH SELinux module %s is disabled", moduleName)
	}
	if permissive {
		if ensureEnforced {
			return fmt.Errorf("the SSH SELinux module %s is permissive, denials will be logged but not enforced", moduleName)
		} else {
...
func diagnoseWrongDomain(procCtx *selinuxContext, logger *slog.Logger) error {
	...
	fallbackErr := fmt.Errorf(fallbackErrMsg, procCtx.domain, domain)

	execPath, err := os.Executable()
	if err != nil {
		return errors.Join(fmt.Errorf("failed to get executable path: %w", err), fallbackErr)
	}
	label, err := ocselinux.FileLabel(execPath)
	if err != nil {
		return errors.Join(fmt.Errorf("failed to get SELinux label of executable: %w", err), fallbackErr)
	}
	fileCtx, err := parseLabel(label)
	if err != nil {
		return errors.Join(fmt.Errorf("failed to parse SELinux context: %w", err), fallbackErr)
	}
	if fileCtx.domain != domain {
		...
		return fmt.Errorf(binaryLabelErrMsg, execPath, fileCtx.domain, execType)
	}
...
func ModuleInstalled() (bool, error) {
	...
	if err != nil {
		return false, fmt.Errorf("failed to find SELinux type: %w", err)
	}
...
func moduleStatus(modulesDir string) (installed bool, disabled bool, permissive bool, err error) {
	...
	if err != nil {
		return false, false, false, fmt.Errorf("failed to list modules directory: %w", err)
	}

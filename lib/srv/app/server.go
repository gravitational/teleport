...
type Config struct {
	...
	// Scope is the scope the agent is pinned to if a full scope pin is not present.
	Scope string
...
func (c *Config) CheckAndSetDefaults() error {
	...
	if c.Scope != "" {
		if err := scopes.WeakValidate(c.Scope); err != nil {
			return trace.Wrap(err)
		}
	}
	if c.ScopePin.GetScope() != "" && c.Scope != "" {
		return trace.BadParameter("either a scope pin or a bare scope must be set for a scoped app_service, not both")
	}
	return nil
}

// GetScope returns the scope the agent is pinned to, whether it comes from a
// bare scope or a full scope pin.
func (c *Config) GetScope() string {
	return cmp.Or(c.ScopePin.GetScope(), c.Scope)
}
...
func (s *Server) getServerInfo(app types.Application) (*types.AppServerV3, error) {
	...
	server, err := types.NewAppServerV3(types.Metadata{
		...
		types.AppServerWithScope(s.c.GetScope()),
	)
	if err != nil {
		return nil, trace.Wrap(err)
	}

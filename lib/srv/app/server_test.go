...
func TestShutdown(t *testing.T) {
	...
	for _, test := range tests {
		...
		t.Run(test.name, func(t *testing.T) {
			...
			require.Empty(t, cmp.Diff(appServers[0].GetApp(), app0, cmpopts.IgnoreFields(types.Metadata{}, "Expires")))
			...
			if test.wantAppServersAfterShutdown {
				require.Empty(t, cmp.Diff(appServersAfterShutdown[0].GetApp(), app0))
			} else {

...
func (c *ConnectionsHandler) handleConnection(conn net.Conn) (func(), error) {
	...
	if err != nil {
		...
		case app.IsTCP():
			return nil, trace.Wrap(err)
		case app.IsMCP():
			return nil, trace.Wrap(c.mcpServer.HandleUnauthorizedAppConnection(ctx, tlsConn, err))
		default:
	...
	case app.IsMCP():
		defer cancel(nil)
		return nil, trace.Wrap(c.mcpServer.HandleAuthorizedAppConnection(ctx, tlsConn, authCtx, app))
	default:
		cleanup := func() {
			cancel(nil)

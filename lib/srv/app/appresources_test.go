...
func TestEnforceMinimalV9(t *testing.T) {
	...
	versionSkew := newTestRole(t, "version-skew", types.V9, types.RoleConditions{
		AppLabels:    devLabels,
		AppResources: []types.AppResource{{}},
	})
	...
	defaultDeny := newTestRole(t, "default-deny", types.V9, types.RoleConditions{AppLabels: devLabels})
	...
	newHandler := func(log *slog.Logger) (*ConnectionsHandler, *eventstest.MockRecorderEmitter) {
	...
	t.Run("denies a plain request", func(t *testing.T) {
		handler, _ := newHandler(slog.New(slog.DiscardHandler))
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "http://dev-app/", nil)
		denied, err := handler.enforceMinimalV9(rec, req, authContext(defaultDeny), app)
		require.NoError(t, err)
		require.True(t, denied)
		require.Equal(t, http.StatusForbidden, rec.Code)
		require.Contains(t, rec.Body.String(), denyBodyRequestNotAllowed)
	})

	t.Run("denies version skew with a 500", func(t *testing.T) {
		handler, _ := newHandler(slog.New(slog.DiscardHandler))
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "http://dev-app/", nil)
		denied, err := handler.enforceMinimalV9(rec, req, authContext(versionSkew), app)
		require.NoError(t, err)
		require.True(t, denied)
		require.Equal(t, http.StatusInternalServerError, rec.Code)
		require.Contains(t, rec.Body.String(), denyBodyInternalAccessError)
		require.NotContains(t, rec.Body.String(), string(appresource.DenyRoleVersionUnsupported), "the client must not read the skew reason")
	})

	t.Run("allows allow_all", func(t *testing.T) {
		handler, emitter := newHandler(slog.New(slog.DiscardHandler))
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "http://dev-app/", nil)
	...
	t.Run("denies and warns once on CORS preflights", func(t *testing.T) {
		...
		handler, emitter := newHandler(slog.New(slog.NewTextHandler(logs, nil)))
		for range 2 {
			rec := httptest.NewRecorder()
		...
		require.Len(t, emitted, 2, "every preflight denial emits an event, even though the warning is logged once")
		for _, e := range emitted {
			event, ok := e.(*apievents.AppSessionRequestDenied)
			require.True(t, ok, "expected AppSessionRequestDenied, got %T", e)
			require.Equal(t, http.MethodOptions, event.Method)
			require.Equal(t, string(appresource.DenyNotAllowed), event.DenyKind)
		}
		require.Equal(t, 1, strings.Count(logs.String(), "Denied CORS preflight"))
	})

	t.Run("warns once about version skew", func(t *testing.T) {
		logs := &strings.Builder{}
		handler, _ := newHandler(slog.New(slog.NewTextHandler(logs, nil)))
		for range 2 {
			rec := httptest.NewRecorder()
			req := httptest.NewRequest(http.MethodGet, "http://dev-app/", nil)
			denied, err := handler.enforceMinimalV9(rec, req, authContext(versionSkew), app)
		...
		require.Equal(t, 1, strings.Count(logs.String(), "Denied app request"))
	})
	...
	t.Run("emits an audit event per denied request", func(t *testing.T) {
		...
		require.Equal(t, string(appresource.DenyRoleVersionUnsupported), event.DenyKind)
		require.Empty(t, event.SessionID, "SessionID must stay unset so denials get unique audit keys")
		require.Equal(t, "app-session-id", event.AppSessionId, "AppSessionId carries the cert's session ID instead")
	})

	t.Run("names the deny kind for an intended default-deny", func(t *testing.T) {
		handler, emitter := newHandler(slog.New(slog.DiscardHandler))
		rec := httptest.NewRecorder()
		req := httptest.NewRequest(http.MethodGet, "http://dev-app/", nil)
		denied, err := handler.enforceMinimalV9(rec, req, authContext(defaultDeny), app)

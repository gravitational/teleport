...
func (m *KubeMiddleware) resolveClusterKey(req *http.Request) (teleportCluster, kubeCluster string, err error) {
	...
	// Legacy fallback: kubeconfigs generated by older tsh have the cluster
	// pair encoded into the TLS SNI rather than the URL path.
	// TODO(jakealti): DELETE IN v20.0.0
	if req.TLS != nil {
		if ltc, lkc, lerr := common.ClustersFromLegacyKubeLocalProxySNI(req.TLS.ServerName); lerr == nil {
			key := kubeClusterKey{teleportCluster: ltc, kubeCluster: lkc}
			*req = *req.WithContext(context.WithValue(req.Context(), kubeClusterKeyContextKey{}, key))
			return ltc, lkc, nil
		}
	}
	...
	if err != nil {
		...
		writeKubeError(req.Context(), rw, &apierrors.StatusError{
			ErrStatus: metav1.Status{
				...
				Message:  "this kubeconfig was generated by an older version of tsh and is no longer compatible with the running local proxy. Re-run `tsh proxy kube` to regenerate it.",
			},
		}, m.logger)
		return true
	}

...
func (h *AuthHandlers) fetchAccessInfo(ctx context.Context, cert *ssh.Certificate, ca types.CertAuthority, clusterName string) (*services.AccessInfo, error) {
	...
	if clusterName == ca.GetClusterName() {
		accessInfo, err = services.AccessInfoFromLocalCertificate(ctx, cert, h.c.AccessPoint)
	} else {
		accessInfo, err = services.AccessInfoFromRemoteCertificate(ctx, cert, h.c.AccessPoint, ca.CombinedMapping())
	}

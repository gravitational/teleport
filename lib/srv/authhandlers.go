...
func (a *ahLoginChecker) evaluateScopedSSHAccess(ident *sshca.Identity, ca types.CertAuthority, clusterName string, target types.Server, osUser string) (*decisionpb.SSHAccessPermit, error) {
	...
	// for prototyping purposes we determine agent scope via an env var. for the final feature, this will be
	// determined via static assignment at agent join time. note that defaulting a resource scope to root when
	// its scope is unknown is compatible with the scopes model (for the purposes of access checks), as it is
	// a strictly privilege-reducing fallback.
	agentScope := scopes.Root
	if scope := os.Getenv("TELEPORT_UNSTABLE_AGENT_SCOPE"); scope != "" {
		if err := scopes.StrongValidate(scope); err != nil {
			slog.WarnContext(ctx, "ignoring invalid TELEPORT_UNSTABLE_AGENT_SCOPE value (will default to root)", "error", err, "value", scope)
		} else {
			agentScope = scope
		}
	}

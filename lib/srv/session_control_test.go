...
func TestSessionController_AcquireSessionContext(t *testing.T) {
	...
	getEvents := func(emitter *eventstest.ChannelEmitter) []apievents.AuditEvent {
	...
	cases := []struct {
		...
		assertion func(t *testing.T, ctx context.Context, err error, emitter *eventstest.ChannelEmitter)
	}{
		{
			...
			identity: IdentityContext{
				...
				RoleSet: services.RoleSet([]types.Role{maxConnections1}),
			},
			assertion: func(t *testing.T, ctx context.Context, err error, emitter *eventstest.ChannelEmitter) {
				require.NoError(t, err)
				require.NotNil(t, ctx)
				require.Empty(t, getEvents(emitter))
			},
		},
		{
		...
		},
		{
			...
			assertion: func(t *testing.T, ctx context.Context, err error, emitter *eventstest.ChannelEmitter) {
				...
				require.NotNil(t, ctx)
				emittedEvents := getEvents(emitter)

				require.Len(t, emittedEvents, 1)

				evt, ok := emittedEvents[0].(*apievents.SessionReject)
				require.True(t, ok)
		...
		},
		{
			...
			identity: IdentityContext{
				...
				RoleSet: services.RoleSet([]types.Role{maxConnections0}),
			},
			assertion: func(t *testing.T, ctx context.Context, err error, emitter *eventstest.ChannelEmitter) {
				require.NoError(t, err)
				require.NotNil(t, ctx)
				require.Empty(t, getEvents(emitter), 0)
			},
		},
	}

...
func (s *Server) handleStreamableHTTP(ctx context.Context, sessionCtx *SessionCtx) error {
	...
	reverseProxy, err := reverseproxy.New(
		...
		reverseproxy.WithResponseModifier(func(resp *http.Response) error {
			// Attribute passed-through 5xx responses to the remote server so
			// clients can tell them apart from errors generated by Teleport.
			// Responses synthesized by streamableHTTPTransport are never 5xx.
...
// makeProxyErrorHandler returns an error handler for the streamable HTTP
// reverse proxy. Unlike reverseproxy.DefaultHandler, which collapses every
// non-network error into a bare "500 Internal Server Error", it logs the
// underlying error, includes it in the response body, and sets the error
// origin header so clients can attribute the failure.

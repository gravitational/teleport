...
func (s *Service) ProxySSH(stream transportv1pb.TransportService_ProxySSHServer) (err error) {
	...
	if unscopedCtx, ok := authzContext.UnscopedContext(); ok {
		// SSH idle timeout is enforced by the SSH server handling the proxied connection.
		// The transport proxy still monitors lock and cert-expiry controls, but does not
		// run a second idle timer because it can race session setup and observe different
		// activity than the SSH server.

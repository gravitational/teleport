...
func MakeEKSFetchersFromAWSMatchers(logger *slog.Logger, clients cloud.AWSClients, matchers []types.AWSMatcher, discoveryConfig string) (kubeFetchers []common.Fetcher, _ error) {
...
func (a *eksFetcher) getMatchingKubeCluster(ctx context.Context, clusterName string) (types.KubeCluster, error) {
	...
	case eks.ClusterStatusUpdating, eks.ClusterStatusActive:
		a.Logger.DebugContext(ctx, "EKS cluster status is valid", "status", st, "cluster_name", clusterName)
	default:

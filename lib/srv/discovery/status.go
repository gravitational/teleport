...
func (s *Server) acquireSemaphore(ctx context.Context, kind, semaphoreName string) (releaseFn func(), stopCtx context.Context, err error) {
	...
	releaseFn = func() {
		...
		if err := lease.Wait(); err != nil {
			s.Log.WithError(err).
				...
				WithField("semaphore", semaphoreName).
...
// - merge the current UserTask with the one stored in the backend, so that instances are not lost
...
// Note: UpsertingUserTask triggers the creation of a new Notification.
func (s *Server) acquireSemaphoreForIntegration(integration string) (releaseFn func(), ctx context.Context, err error) {
	return s.acquireSemaphore(s.ctx, types.KindIntegration, integration)
}

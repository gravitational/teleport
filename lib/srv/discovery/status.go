...
type awsAWSSyncTaskKey struct {
	integration string
	issueType   string
	accountID   string
}

func (s *Server) mergeUpsertDiscoverAWSSyncTask(taskGroup awsAWSSyncTaskKey, failedInstances []*usertasksv1.RiskFactor) error {
	userTaskName := usertasks.TaskNameForDiscoverAWSync(usertasks.TaskNameForDiscoverAWSSyncParts{
		Integration: taskGroup.integration,
		IssueType:   taskGroup.issueType,
		AccountID:   taskGroup.accountID,
	})
	...
	var severity usertasksv1.Severity
	...
	task, err := usertasks.NewDiscoverEC2UserTask(
		&usertasksv1.UserTaskSpec{
			Integration: taskGroup.integration,
			TaskType:    usertasks.TaskTypeDiscoverTAGSync,
			IssueType:   taskGroup.issueType,
			State:       usertasks.TaskStateOpen,
			AccessGraph: &usertasksv1.AccessGraph{
				Severity:    severity,
				AccountId:   taskGroup.accountID,
				RiskFactors: failedInstances,
			},
		},
		usertasks.WithExpiration(taskExpiration),
	)

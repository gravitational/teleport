...
// VMs were all filtered out (e.g. an entire resource group is Windows) do not
// produce a spurious AzureInstances in GetInstances' result. Emitting empty
// groups would cascade into downstream "no instances found, skipping" log
// entries per (rg, region) and, if any future caller ever treats an empty
...
func TestAzureWatcher_GetInstances_SkipsEmptyBuckets(t *testing.T) {
	...
	buildVM := func(rg, name string, osType armcompute.OperatingSystemTypes) *armcompute.VirtualMachine {
	...
	// rg-windows holds only Windows VMs (fully filtered to empty by
	// filterSupportedOS). rg-linux holds one Linux VM. GetInstances must
	// emit exactly one AzureInstances group (rg-linux), not two.
	clients := mockClients{
		vmClients: map[string]azure.VirtualMachinesClient{
			sub: azure.NewVirtualMachinesClientByAPI(&azure.ARMComputeMock{
				VirtualMachines: map[string][]*armcompute.VirtualMachine{
					"rg-windows": {
						buildVM("rg-windows", "vm-w1", armcompute.OperatingSystemTypesWindows),
						buildVM("rg-windows", "vm-w2", armcompute.OperatingSystemTypesWindows),
					},
					"rg-linux": {
						buildVM("rg-linux", "vm-l1", armcompute.OperatingSystemTypesLinux),
					},
				},
			}, nil),
		},
	...
	require.Equal(t, "rg-linux", collected[0].ResourceGroup,
		"the single emitted group should be the one that had a surviving VM")
	require.Len(t, collected[0].Instances, 1,
		"the emitted group should hold exactly the one Linux VM")
	require.Equal(t, "vm-l1", *collected[0].Instances[0].Name)
}

...
func TestInstallerScriptContainsJoinFailureMessage(t *testing.T) {
	script := installer.NewDefaultInstaller.GetScript()
	// This substring must match what detectIssueType in ssm_install.go
	// looks for (JoinFailureMessage constant) to classify join failures.
	require.Contains(t, script, "failed to join the cluster",
		"installer script must contain the join failure detection substring used by detectIssueType")
}

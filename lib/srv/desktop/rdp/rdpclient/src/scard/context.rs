...
use crate::scard::backend::PendingGetStatusChange;
use crate::scard::piv::Card;
...
impl Contexts {
    ...
    pub(super) fn take_scard_cancel_response(
        &mut self,
        id: u32,
    ) -> PduResult<Option<PendingGetStatusChange>> {
    ...
    pub(super) fn get_card(
        &mut self,
        handle: &ScardHandle,
    ) -> PduResult<&mut Card<TRANSMIT_DATA_LIMIT>> {
        self.get_internal_mut(handle.context.value)?
            .get(handle.value)
...
struct ContextInternal {
    handles: HashMap<u32, Card<TRANSMIT_DATA_LIMIT>>,
    next_id: u32,
...
impl ContextInternal {
    ...
    fn connect(
        ...
        let card = Card::new(uuid, cert_der, key_der, pin)?;
        let id = self.next_id;
        self.next_id += 1;
    ...
    fn get(&mut self, id: u32) -> Option<&mut Card<TRANSMIT_DATA_LIMIT>> {

...
func (s *WindowsService) handleConnection(proxyConn *tls.Conn) {
	...
	desktops, err := stream.Collect(clientutils.Resources(ctx, func(ctx context.Context, pageSize int, pageToken string) ([]types.WindowsDesktop, string, error) {
...
func (s *WindowsService) nameForStaticHost(addr string) (string, error) {
	desktops, err := stream.Collect(clientutils.Resources(s.closeCtx, func(ctx context.Context, pageSize int, pageToken string) ([]types.WindowsDesktop, string, error) {
		resp, err := s.cfg.AccessPoint.ListWindowsDesktops(ctx, types.ListWindowsDesktopsRequest{
			Limit:          pageSize,
			StartKey:       pageToken,

...
func TestUserMgmt_CreateTemporaryUser(t *testing.T) {
	...
	closer, err := users.UpsertUser("bob", userinfo, false /* take ownership */)
	...
	secondCloser, err := users.UpsertUser("bob", userinfo, false /* take ownership */)
	require.NoError(t, err)
	...
	closer, err = users.UpsertUser("simon", userinfo, false /* take ownership */)
...
func Test_UpdateUserGroups_Keep(t *testing.T) {
	...
	// Create user
	closer, err := users.UpsertUser("alice", userinfo, false /* take ownership */)
...
func Test_DontUpdateUnmanagedUsers(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			closer, err := users.UpsertUser("alice", tc.userinfo, false /* take ownership */)
			assert.ErrorIs(t, err, unmanagedUserErr)
			assert.Equal(t, nil, closer)
			assert.Zero(t, backend.setUserGroupsCalls)
...
func Test_AllowExplicitlyManageExistingUsers(t *testing.T) {
	...
	closer, err := users.UpsertUser("alice-keep", userinfo, false /* take ownership */)
	assert.NoError(t, err)
	assert.Equal(t, nil, closer)
	assert.Equal(t, 1, backend.setUserGroupsCalls)
	...
	userinfo.Mode = decisionpb.HostUserMode_HOST_USER_MODE_STATIC
	closer, err = users.UpsertUser("alice-static", userinfo, true /* take ownership */)
	assert.NoError(t, err)
	assert.Equal(t, nil, closer)
	assert.Equal(t, 2, backend.setUserGroupsCalls)
	...
	userinfo.Mode = decisionpb.HostUserMode_HOST_USER_MODE_DROP
	closer, err = users.UpsertUser("alice-drop", userinfo, false /* take ownership */)
	assert.ErrorIs(t, err, unmanagedUserErr)
	assert.Equal(t, nil, closer)
	assert.Equal(t, 2, backend.setUserGroupsCalls)
...
func TestCreateUserWithExistingPrimaryGroup(t *testing.T) {
	...
	closer, err = users.UpsertUser("fred", userinfo, false /* take ownership */)
	assert.NoError(t, err)
	assert.NotEqual(t, nil, closer)
	assert.Zero(t, backend.setUserGroupsCalls)

...
func (s *SessionRegistry) UpsertHostUser(identityContext IdentityContext) (bool, io.Closer, error) {
	...
	if trace.IsAccessDenied(accessErr) {
		...
		if ui != nil && slices.Contains(ui.Groups, types.TeleportKeepGroup) {
			return false, nil, trace.WrapWithMessage(accessErr, "insufficient permissions for migrating unmanaged user, create_host_user_mode must be set to 'keep'")
		}

...
func (s *SessionRegistry) CheckJoinSession(sid rsession.ID) error {
	if _, found := s.findSession(sid); !found {
		return trace.NotFound("session with ID %q not found", sid.String())
	}
	return nil
}

...
func TestSharedSessionOwnerDisconnect(t *testing.T) {
	f := newFixtureWithoutDiskBasedLogging(t)
	ctx := t.Context()

	ownerClient, err := tracessh.Dial(ctx, "tcp", f.ssh.srvAddress, f.ssh.cltConfig)
	require.NoError(t, err)
	t.Cleanup(func() { _ = ownerClient.Close() })

	joinerClient, err := tracessh.Dial(ctx, "tcp", f.ssh.srvAddress, f.ssh.cltConfig)
	require.NoError(t, err)
	t.Cleanup(func() { _ = joinerClient.Close() })

	ownerSession, err := ownerClient.NewSession(ctx)
	require.NoError(t, err)
	t.Cleanup(func() { _ = ownerSession.Close() })

	require.NoError(t, ownerSession.RequestPty(ctx, "xterm", 40, 80, ssh.TerminalModes{}))
	require.NoError(t, ownerSession.Shell(ctx))
	...
	joinSession, err := joinerClient.NewSessionWithParams(ctx, &tracessh.SessionParams{
		JoinSessionID: sessionID,
		JoinMode:      types.SessionPeerMode,
	})
	require.NoError(t, err)
	t.Cleanup(func() { _ = joinSession.Close() })

	require.NoError(t, joinSession.RequestPty(ctx, "xterm", 40, 80, ssh.TerminalModes{}))
	joinStdin, err := joinSession.StdinPipe()
	require.NoError(t, err)
	require.NoError(t, joinSession.Shell(ctx))

	require.Eventually(t, func() bool {
		tracker, err := f.ssh.srv.termHandlers.SessionRegistry.SessionTrackerService.GetSessionTracker(ctx, sessionID)
		if err != nil {
			return false
		}
		return len(tracker.GetParticipants()) == 2
	}, 5*time.Second, 50*time.Millisecond)

	waitErr := make(chan error, 1)
	go func() {
		waitErr <- joinSession.Wait()
	}()

	require.NoError(t, ownerClient.Close())

	select {
	case err := <-waitErr:
		t.Fatalf("joiner session exited after owner disconnect: %v", err)
	case <-time.After(300 * time.Millisecond):
	}

	_, _ = joinStdin.Write([]byte("exit\n"))
	select {
	case err := <-waitErr:
		require.NoError(t, err)
	case <-time.After(5 * time.Second):
		t.Fatal("timed out waiting for joiner session to exit")
	}
}

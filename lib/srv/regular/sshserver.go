...
type ServerAccessPoint interface {
	srv.AccessPoint
...
func (s *Server) Shutdown(ctx context.Context) error {
	...
	if s.heartbeat != nil {
		...
		// Explicitly deleting proxy heartbeats helps the behavior of reverse
		// tunnel agents during rollouts, as otherwise they'll keep trying to
		// reach proxies until the heartbeats expire; there's also only a few
		// proxies in a cluster at a given time, so the extra load generated by
		// the delete operations is negligible. Deleting node heartbeats might
		// generate too much load in large clusters, so it's better to just rely
		// on the TTL there.

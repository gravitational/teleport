...
type Server struct {
	...
	*logrus.Entry
	...
	// fwdSocketDir is the directory in which sockets are created for direct-tcpip
	// forwarder subprocesses.
	fwdSocketDir string

	// fwdSocketCleaner performs periodic health checks against the sockets
	// in fwdSocketDir and removes them as necessary.
	fwdSocketCleaner *tcpip.SocketCleaner
...
func (s *Server) close() {
	...
	if s.fwdSocketCleaner != nil {
		s.fwdSocketCleaner.Close()
	}
}
...
func New(
	...
	fwdSocketDir := filepath.Join(dataDir, "sock/fwd")
	...
	s := &Server{
		...
		dataDir:            dataDir,
		fwdSocketDir:       fwdSocketDir,
		fwdSocketCleaner:   tcpip.NewSocketCleaner(fwdSocketDir, time.Minute /* cleanup interval */),
...
func (s *Server) getDirectTCPIPForwarder(ctx *srv.ServerContext) (func(string) (net.Conn, error), error) {
	if f, ok := ctx.Parent().GetDirectTCPIPForwarder(); ok {
		return f, nil
	}

	// set up socket (dir is created as part of setup if it does not exist).
	sock, fd, err := tcpip.SetupListenerFD(s.fwdSocketDir)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	ctx.ForwardSocketName = sock
	...
	cmd, err := srv.ConfigureCommand(ctx, fd)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	...
	keepaliver := tcpip.NewSocketKeepaliver(sock, time.Minute /* keepalive interval */)

	go func() {
		if err := keepaliver.Run(); err != nil && !explicitlyClosed.Load() {
			s.Logger.Warnf("Forwarder socket keepaliver exited early with unexpected error: %v", err)
		}
	}()
	...
	closer := utils.CloseFunc(func() error {
		...
		select {
		case <-cdone:
			// child already exited. this will have been called out in the
			// logs above, so we can ignore it here. nothing more to do.
			return nil
		default:
		}

		// give the child a brief opportunity to exit gracefully.
		cmd.Process.Signal(os.Interrupt)
		select {
		case <-cdone:
		case <-time.After(time.Second * 3):
			// forcibly kill the child (the only real downside to this is that
			// it leaves the socket hanging, but we have a periodic that will
	...
	// set up closure for dialing through the child
	f := func(addr string) (net.Conn, error) {
		return tcpip.DialThroughForwarder(sock, addr)
	}

	if ef, ok := ctx.Parent().GetDirectTCPIPForwarder(); ok {
		// another forwarder was concurrently registered. this isn't actually a problem, multiple forwarders
	...
	// register forwarder and associated closer with connection context
	ctx.Parent().SetDirectTCPIPForwarder(f)
...
func (s *Server) handleDirectTCPIPRequest(ctx context.Context, ccx *sshutils.ConnectionContext, identityContext srv.IdentityContext, channel ssh.Channel, req *sshutils.DirectTCPIPReq) {
	...
	forwardFunc, err := s.getDirectTCPIPForwarder(scx)
	if err != nil {
	...
	conn, err := forwardFunc(scx.DstAddr)
	if err != nil {
		writeStderr(channel, err.Error())
		return
	}

...
func init() {
	utils.RegisterPrometheusCollectors(proxiedSessions, failedConnectingToNode, connectingToNode)
}
...
func (t *proxySubsys) proxyToHost(ctx context.Context, ch ssh.Channel, remoteAddr net.Addr) error {
	...
	conn, err := t.router.DialHost(ctx, remoteAddr, t.host, t.port, t.clusterName, t.ctx.Identity.RoleSet, t.ctx.StartAgentChannel)
	if err != nil {

...
func (c *Client) discoverOnce(ctx context.Context) error {
	...
	resp, err := relayapi.Discover(ctx, relayapi.DiscoverParams{
	...
	c.updateDiscovery(ctx, resp.GetRelayGroup(), resp.GetTunnelPublicAddr(), resp.GetTargetConnectionCount())

	return nil
}

func (c *Client) updateDiscovery(ctx context.Context, relayGroup string, tunnelAddr string, connectionCount int32) {
	c.mu.Lock()
	defer c.mu.Unlock()
...
func (c *Client) dialLoopGrouped() {
	...
	for {
		...
		// TODO(espadolini): figure out a nice way to wait for "events" rather than just checking repeatedly
		if !c.shouldDial() {
...
func (c *Client) dialRelayGrouped() error {
	...
	c.updateDiscovery(helloCtx, serverHello.GetRelayGroup(), serverHello.GetTunnelPublicAddr(), serverHello.GetTargetConnectionCount())

...
// randomValidByte generates a byte that could be valid in a scope segment,
// targeting bytes that would be considered valid by both WeakValidate and
// EncodeForKey. This means it must a printable ASCII character greater than
// the backend separator that is not a breaking char.
func randomValidByte() byte {
	const (
		min = backend.Separator + 1
		max = 126
	)
	for {

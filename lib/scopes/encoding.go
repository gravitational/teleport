...
func EncodeForKey(scope string) (string, error) {
	...
	// the backend separator, which would break sorting for composed keys
	// commonly used in the backend or cache, e.g. <encoded-scope>/<resource-name>
	...
	// Enforce that the scope does not contain any empty segments, which can
	// also break sorting of composed keys. For example:
	// +++/resource
	// +++++/resource
	// the sort order would invert depending on whether the resource suffix was present.

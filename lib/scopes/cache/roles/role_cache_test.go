...
func TestListScopedRoleASsignmentsScenarios(t *testing.T) {
	...
	rsp, err := cache.ListScopedRoles(t.Context(), &srpb.ListScopedRolesRequest{
	...
	_, err = cache.ListScopedRoles(t.Context(), &srpb.ListScopedRolesRequest{
		ResourceScope: &scopespb.Filter{
			Mode:  scopespb.Mode_MODE_RESOURCES_SUBJECT_TO_SCOPE,
			Scope: "/aa",
		},
...
func TestListScopedRolesBasics(t *testing.T) {
	...
	tts := []struct {
		name   string
		req    *srpb.ListScopedRolesRequest
		expect [][]string
	}{
		{
...
func collectRoleNames(roles []*srpb.ScopedRole) []string {
	var names []string
	for _, role := range roles {

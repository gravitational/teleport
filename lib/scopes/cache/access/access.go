...
func (c *Cache) fetch(ctx context.Context) (state, error) {
	...
	for role, err := range scopeutils.RangeScopedRoles(ctx, c.cfg.Reader, &scopedaccessv1.ListScopedRolesRequest{}) {
		if err != nil {
	...
	for assignment, err := range scopeutils.RangeScopedRoleAssignments(ctx, c.cfg.Reader, &scopedaccessv1.ListScopedRoleAssignmentsRequest{}) {
		if err != nil {
			return state{}, trace.Wrap(err)
		}

...
func TestScopedAccessCacheReplication(t *testing.T) {
	...
	for role, err := range scopeutils.RangeScopedRoles(ctx, cache, &scopedaccessv1.ListScopedRolesRequest{}) {
		require.NoError(t, err)
	...
	for assignment, err := range scopeutils.RangeScopedRoleAssignments(ctx, cache, &scopedaccessv1.ListScopedRoleAssignmentsRequest{}) {
		require.NoError(t, err)

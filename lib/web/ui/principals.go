...
// SplitPrincipalSet builds one principal dimension from a computed
// granted/visible split. Returns zero-valued set when ps is nil.
func SplitPrincipalSet(principalType string, ps *PrincipalSet) ResourcePrincipalSet {
...
func MakeResourcePrincipalSets(sets []types.ResourcePrincipalSet) []ResourcePrincipalSet {
	...
	for _, ps := range sets {
		...
		out = append(out, ResourcePrincipalSet{
			...
			Granted:       slices.Sorted(slices.Values(ps.Granted)),
			Requestable:   slices.Sorted(slices.Values(ps.Requestable)),
			ByRole:        byRole,
		})
	}
	return out

...
// - retrieving a u2f authn challenge with token
...
func TestAddU2FDeviceWithPrivilegeToken(t *testing.T) {
	...
	// Create a privilege token that re-auths with existing otp.
	...
	endpoint = pack.clt.Endpoint("webapi", "u2f", "signuptokens", token1)
	...
	endpoint = pack.clt.Endpoint("webapi", "mfa", "authnchallenge", token1)
	...
	// Create another privilege token that re-auths with the new u2f.
	endpoint = pack.clt.Endpoint("webapi", "users", "privilege", "token")
	resp, err = pack.clt.PostJSON(context.Background(), endpoint, privilegeTokenRequest{
		U2FSignResponse: u2fSignResp,
	})
...
// - creating a privilege token
// - creating totp secretes for pvivlege token
...
func TestAddTOTPDeviceWithPrivilegeToken(t *testing.T) {
	...
	// Create totp secrets for this token.
	endpoint = pack.clt.Endpoint("webapi", "mfa", "qrcode", token1)
	resp, err = pack.clt.Get(context.Background(), endpoint, nil)
	require.NoError(t, err)
	...
	secrets, err := env.server.Auth().GetUserTokenSecrets(context.Background(), token1)
	require.NoError(t, err)
	...
	var newTOTPDevice *types.MFADevice
	for _, device := range devices {
		if device.GetName() == "new-otp" {
			newTOTPDevice = device
		}
	}
	require.NotEmpty(t, newTOTPDevice)

	newTOTPToken, err = totp.GenerateCode(newTOTPDevice.GetTotp().Key, env.clock.Now().Add(30*time.Second))
	require.NoError(t, err)
...
func TestDeleteMFADeviceWithPrivilegeToken(t *testing.T) {
	...
	endpoint = pack.clt.Endpoint("webapi", "mfa", token1, devices[0].GetName())

...
func (s *WebSuite) TestCreateAppSession(c *C) {
	...
	for _, tt := range tests {
		...
		c.Assert(response.FQDN, check.Equals, tt.outFQDN, tt.inComment)
		...
		c.Assert(session.GetUser(), check.Equals, tt.outUsername, tt.inComment)
		c.Assert(session.GetName(), check.Equals, response.CookieValue, tt.inComment)
	}
}
...
func TestWebSessionsRenewAllowsOldBearerTokenToLinger(t *testing.T) {
	...
	_, err = proxy.client.GetWebSession(context.TODO(), services.GetWebSessionRequest{
		User:      "foo",

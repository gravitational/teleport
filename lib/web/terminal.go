...
func newMFACeremony(stream *terminal.WSStream, createAuthenticateChallenge mfa.CreateAuthenticateChallengeFunc) *mfa.Ceremony {
	return &mfa.Ceremony{
		...
		SSOMFACeremonyConstructor: func(ctx context.Context) (mfa.SSOMFACeremony, error) {
			// The WebUI doesn't use an end-to-end SSO ceremony. Instead we just provide the
			// WebUI with the redirect URL to begin the flow in the challenge, and the URL
			// to finish the flow at /web/sso_confirm. HandleRedirect and GetCallbackMFAToken
			// are not needed as the WebUI will handle this logic manually.
			return &sso.MFACeremony{
				ClientCallbackURL: "/web/sso_confirm",
			}, nil
		},
	}
}

...
// getMFADevicesHandle gets mfa devices for authenticated user.
...
// createAuthenticateChallengHandle creates auth challenges for the authenticated user.
func (h *Handler) createAuthenticateChallengHandle(w http.ResponseWriter, r *http.Request, params httprouter.Params, ctx *SessionContext) (interface{}, error) {
	clt, err := ctx.GetClient()
	if err != nil {
		return nil, trace.Wrap(err)
	}

	res, err := clt.CreateAuthenticateChallenge(r.Context(), &proto.CreateAuthenticateChallengeRequest{})
	if err != nil {

...
type Terminal interface {
	...
	WriteWithChanges(p []byte) (int, []int, error)

	// ResizeWithChanges changes the size of the virtual terminal and returns the line numbers that were changed.
	ResizeWithChanges(cols, rows int) []int
...
type View interface {
	...
	// Line returns the segments of the line at index n.
	Line(n int) []Segment
	...
	// ANSI returns a string representation of the terminal state.
	ANSI() string
}

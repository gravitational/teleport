...
// Default SVG rendering settings
const (
	defaultFontSize   = 14
...
// TerminalStateToSVG converts a terminal state to an SVG representation
func TerminalStateToSVG(state vt10x.TerminalState) string {
	var buf bytes.Buffer

	cols, rows := state.Cols, state.Rows
	charWidth := 100.0 / (float64(cols) + 2.0)
	fontSize := float64(defaultFontSize)
	rowHeight := fontSize * defaultLineHeight

...
func (proxy *ProxyClient) IssueUserCertsWithMFA(ctx context.Context, clt auth.ClientI, params ReissueParams, promptMFAChallenge PromptMFAChallengeHandler) (*Key, error) {
	ctx, span := proxy.Tracer.Start(
		ctx,
...
func (proxy *ProxyClient) CreateAccessRequest(ctx context.Context, req types.AccessRequest) error {
	...
	return proxy.currentCluster.CreateAccessRequest(ctx, req)
}
...
func (proxy *ProxyClient) GetAccessRequests(ctx context.Context, filter types.AccessRequestFilter) ([]types.AccessRequest, error) {
	...
	reqs, err := proxy.currentCluster.GetAccessRequests(ctx, filter)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	return reqs, nil
...
func (proxy *ProxyClient) GetRole(ctx context.Context, name string) (types.Role, error) {
	...
	role, err := proxy.currentCluster.GetRole(ctx, name)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	return role, nil
...
func (proxy *ProxyClient) NewWatcher(ctx context.Context, watch types.Watch) (types.Watcher, error) {
	...
	watcher, err := proxy.currentCluster.NewWatcher(ctx, watch)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	return watcher, nil
...
func (proxy *ProxyClient) isAuthBoring(ctx context.Context, clt auth.ClientI) (bool, error) {
	ctx, span := proxy.Tracer.Start(
		ctx,
...
func (proxy *ProxyClient) CreateAppSession(ctx context.Context, req types.CreateAppSessionRequest) (types.WebSession, error) {
	...
	err = auth.WaitForAppSession(ctx, ws.GetName(), ws.GetUser(), authClient)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	return ws, nil
...
func (proxy *ProxyClient) ListResources(ctx context.Context, namespace, resource, startKey string, limit int) ([]types.ResourceWithLabels, string, error) {
	...
	resp, err := proxy.currentCluster.ListResources(ctx, proto.ListResourcesRequest{
		Namespace:    namespace,
...
// authClient wraps an auth.ClientI and prevents closing
// the underlying client. Used by the ProxyClient to ensure
// that there is only one auth.ClientI for the current cluster
// for the duration of a tsh command and that any users of the
// client don't accidentally close it
...
func (proxy *ProxyClient) AuthClient() auth.ClientI {
...
func (proxy *ProxyClient) ConnectToNode(ctx context.Context, clt auth.ClientI, nodeAddress NodeAddr, user string) (*NodeClient, error) {
	ctx, span := proxy.Tracer.Start(
		ctx,
	...
	if len(proxy.teleportClient.JumpHosts) > 0 {
		return proxy.PortForwardToNode(ctx, clt, nodeAddress, user)
	}

	authMethods, err := proxy.sessionSSHCertificate(ctx, clt, nodeAddress)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
// It returns connected and authenticated NodeClient
func (proxy *ProxyClient) PortForwardToNode(ctx context.Context, clt auth.ClientI, nodeAddress NodeAddr, user string) (*NodeClient, error) {
	ctx, span := proxy.Tracer.Start(
		ctx,
...
func (proxy *ProxyClient) sessionSSHCertificate(ctx context.Context, clt auth.ClientI, nodeAddr NodeAddr) ([]ssh.AuthMethod, error) {

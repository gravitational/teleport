...
func (p *ForwardedPort) ToString() string {
	...
	if netutils.IsLocalhost(p.SrcIP) {
...
func (c *Config) KubeProxyHostPort() (string, int) {
	if c.KubeProxyAddr != "" {
		addr, err := netutils.ParseAddr(c.KubeProxyAddr)
		if err == nil {
...
func (c *Config) WebProxyHostPort() (string, int) {
	if c.WebProxyAddr != "" {
		addr, err := netutils.ParseAddr(c.WebProxyAddr)
		if err == nil {
...
func (c *Config) SSHProxyHostPort() (string, int) {
	if c.SSHProxyAddr != "" {
		addr, err := netutils.ParseAddr(c.SSHProxyAddr)
		if err == nil {
...
func (c *Config) PostgresProxyHostPort() (string, int) {
	if c.PostgresProxyAddr != "" {
		addr, err := netutils.ParseAddr(c.PostgresProxyAddr)
		if err == nil {
...
func (c *Config) MongoProxyHostPort() (string, int) {
	if c.MongoProxyAddr != "" {
		addr, err := netutils.ParseAddr(c.MongoProxyAddr)
		if err == nil {
...
func (c *Config) MySQLProxyHostPort() (string, int) {
	if c.MySQLProxyAddr != "" {
		addr, err := netutils.ParseAddr(c.MySQLProxyAddr)
		if err == nil {
...
func (tc *TeleportClient) applyProxySettings(proxySettings webclient.ProxySettings) error {
	...
	if proxySettings.Kube.Enabled {
		...
		case proxySettings.Kube.PublicAddr != "":
			if _, err := netutils.ParseAddr(proxySettings.Kube.PublicAddr); err != nil {
		...
		case proxySettings.Kube.ListenAddr != "" && !proxySettings.TLSRoutingEnabled:
			addr, err := netutils.ParseAddr(proxySettings.Kube.ListenAddr)
			if err != nil {
				return trace.BadParameter(
					"failed to parse value received from the server: %q, contact your administrator for help",
	...
	if proxySettings.SSH.PublicAddr != "" {
		addr, err := netutils.ParseAddr(proxySettings.SSH.PublicAddr)
		if err != nil {
			return trace.BadParameter(
				"failed to parse value received from the server: %q, contact your administrator for help",
	...
	if proxySettings.SSH.ListenAddr != "" {
		addr, err := netutils.ParseAddr(proxySettings.SSH.ListenAddr)
		if err != nil {
			return trace.BadParameter(
				"failed to parse value received from the server: %q, contact your administrator for help",
	...
	if proxySettings.SSH.SSHPublicAddr != "" {
		addr, err := netutils.ParseAddr(proxySettings.SSH.SSHPublicAddr)
		if err != nil {
			return trace.BadParameter(
				"failed to parse value received from the server: %q, contact your administrator for help",
	...
	case proxySettings.DB.PostgresPublicAddr != "":
		addr, err := netutils.ParseAddr(proxySettings.DB.PostgresPublicAddr)
		if err != nil {
	...
	case proxySettings.DB.PostgresListenAddr != "" && !proxySettings.TLSRoutingEnabled:
		addr, err := netutils.ParseAddr(proxySettings.DB.PostgresListenAddr)
		if err != nil {
	...
	case proxySettings.DB.MongoPublicAddr != "":
		addr, err := netutils.ParseAddr(proxySettings.DB.MongoPublicAddr)
		if err != nil {
	...
	case proxySettings.DB.MongoListenAddr != "":
		addr, err := netutils.ParseAddr(proxySettings.DB.MongoListenAddr)
		if err != nil {
	...
	case proxySettings.DB.MySQLPublicAddr != "":
		addr, err := netutils.ParseAddr(proxySettings.DB.MySQLPublicAddr)
		if err != nil {
	...
	case proxySettings.DB.MySQLListenAddr != "":
		addr, err := netutils.ParseAddr(proxySettings.DB.MySQLListenAddr)
		if err != nil {

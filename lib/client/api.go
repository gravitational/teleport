...
type Config struct {
	...
	// ParticipantRequirements is set if debug information about participants requirements
...
func (tc *TeleportClient) runShell(ctx context.Context, nodeClient *NodeClient, mode types.SessionParticipantMode, sessToJoin *session.Session, beforeStart func(io.Writer)) error {
	...
	env[teleport.EnvSSHSessionParticipantRequirements] = strconv.FormatBool(tc.Config.ParticipantRequirements)

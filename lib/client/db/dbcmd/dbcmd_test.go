...
func TestCLICommandBuilderGetConnectCommand(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:         "postgres missing psql tolerated",
			dbProtocol:   defaults.ProtocolPostgres,
			databaseName: "mydb",
			opts:         []ConnectCommandFunc{WithTolerateMissingCLIClient()},
			execer:       &fakeExec{},
			cmd: []string{"psql",
				"postgres://myUser@localhost:12345/mydb?sslrootcert=/tmp/keys/example.com/cas/root.pem&" +
					"sslcert=/tmp/keys/example.com/bob-db/db.example.com/mysql.crt&" +
		...
		{
			name:         "cockroach missing cockroach and psql",
			dbProtocol:   defaults.ProtocolCockroachDB,
			databaseName: "mydb",
			execer:       &fakeExec{execOutput: map[string][]byte{}},
			wantErr:      true,
		...
		{
			name:         "cockroach missing cockroach tolerated",
			dbProtocol:   defaults.ProtocolCockroachDB,
			databaseName: "mydb",
			opts:         []ConnectCommandFunc{WithTolerateMissingCLIClient()},
			execer:       &fakeExec{},
		...
		},
		{
			...
			opts:            []ConnectCommandFunc{WithNoTLS()},
			execer: &fakeExec{
				execOutput: map[string][]byte{"mongosh": []byte("1.1.6")},
			},
			cmd: []string{"mongosh",
				"mongodb://localhost:12345/mydb?directConnection=true&serverSelectionTimeoutMS=5000",
			},
		...
		{
			name:            "mongo missing mongosh tolerated",
			dbProtocol:      defaults.ProtocolMongoDB,
			databaseName:    "mydb",
			getDatabaseFunc: withMongoDBAtlasDatabase(),
			opts: []ConnectCommandFunc{
				WithTolerateMissingCLIClient(),
				WithNoTLS(),
			},
			execer: &fakeExec{},
			cmd: []string{"mongosh",
		...
		{
			name:         "sqlserver missing sqlcmd tolerated",
			dbProtocol:   defaults.ProtocolSQLServer,
			databaseName: "mydb",
			opts:         []ConnectCommandFunc{WithTolerateMissingCLIClient()},
			execer:       &fakeExec{},
		...
		{
			name:         "snowflake missing snowsql",
			dbProtocol:   defaults.ProtocolSnowflake,
			databaseName: "mydb",
			execer:       &fakeExec{execOutput: map[string][]byte{}},
			wantErr:      true,
		...
		{
			name:         "snowflake missing snowsql tolerated",
			dbProtocol:   defaults.ProtocolSnowflake,
			databaseName: "warehouse1",
			opts: []ConnectCommandFunc{
				WithTolerateMissingCLIClient(),
		...
		{
			name:       "spanner missing spanner-cli",
			dbProtocol: defaults.ProtocolSpanner,
			opts: []ConnectCommandFunc{
				WithNoTLS(),
			},
		...
		},
		{
			...
			databaseName: "",
			execer: &fakeExec{execOutput: map[string][]byte{
				clickHouseNativeClientBin: {},
			}},
			cmd: []string{"clickhouse-client",
				"--host", "localhost",
				"--port", "12345",
				"--user", "myUser",
			},
		...
		{
			name:         "clickhouse missing clickhouse-client",
			dbProtocol:   defaults.ProtocolClickHouse,
			databaseName: "",
			execer:       &fakeExec{execOutput: map[string][]byte{}},
			wantErr:      true,

...
// PlayerController is the interface that provides specific event handling and comunication method for specific player implementation.
// It might be tsh play or web based player.
type PlayerController interface {
	...
	// ReceiveAction returns ActionMessage from client used to control playback
	ReceiveAction() (ActionMessage, error)

	// Send message to client
	Send(msg []byte) error

	// Close cleans everything up
	Close() error

	// Reset sends a message to client that session should be reseted to clear state
	// applies only to terminal playbacks.
	Reset() error

	// Move informs client that player had moved playback position to given timestamp in milliseconds
	Move(position int64) error
...
func (pp *Player) Play(ctx context.Context) {
	defer pp.LogDebug("playbackPlayer.Play returned")
...
const (
	...
	// ActionCancel closes the playback
	ActionCancel = playbackAction("cancel")

	// ActionRewind moves the playback few milliseconds back
	ActionRewind = playbackAction("rewind")

	// ActionForward moves the playback few milliseconds forward
	ActionForward = playbackAction("forward")
)

// ActionMessage is a message passed from the playback client
// to the server over the connection in order to
// control playback.
...
func NewPlayer(sID string, streamer Streamer, log logrus.FieldLogger, controller PlayerController) *Player {
	return &Player{
		SID:               sID,
		streamer:          streamer,
...
func (pp *Player) close(cancel context.CancelFunc) {
	pp.closeOnce.Do(func() {
		...
		if err := pp.playerController.Close(); err != nil {
			pp.LogWithError(err, "playerControler.Close() failed")
		}
...
func (pp *Player) LogWithError(err error, format string, args ...interface{}) {
	if !pp.LoggingEnabled {
		return
	}

	pp.Log.WithError(err).Errorf(format, args...)
}

func (pp *Player) LogDebug(format string, args ...interface{}) {
	if !pp.LoggingEnabled {
		return
	}

	pp.Log.Debugf(format, args...)
}
...
func (pp *Player) receiveActions(cancel context.CancelFunc) {
	defer pp.LogDebug("playbackPlayer.ReceiveActions returned")
	defer pp.close(cancel)

	for {
		action, err := pp.playerController.ReceiveAction()
		if err != nil {
			...
			if !utils.IsOKNetworkError(err) {
				pp.LogWithError(err, "error reading action")
			}
			return
		}
		pp.LogDebug("received playback action: %+v", action)
		...
		case ActionCancel:
			return
		default:
			pp.LogDebug("received unknown action: %v", action.Action)
			return
		}
	}
}

func (pp *Player) forward() {
	action := ActionMessage{
		Action:       ActionMove,
		MovePosition: pp.lastDelay + 10000,
	}

	pp.setPlayerToMoveState(action)
}

func (pp *Player) rewind() {
	position := pp.lastDelay - 10000
	if position < 0 {
		position = 0
	}
...
func (pp *Player) MarshalAndSend(e interface{}) error {
	...
	if err != nil {
		pp.LogWithError(err, "failed to marshal %T event into JSON: %v", e, e)
		if err := pp.playerController.Error("server error"); err != nil {
			pp.LogWithError(err, "failed to write \"error\" message")
		}
		return err
	}
	...
	if err := pp.Send(msg); err != nil {
		...
		if !errors.Is(err, net.ErrClosed) {
			pp.LogWithError(err, "failed to write %T: %v", e, e)
		}
...
func (pp *Player) Send(msg []byte) error {
	return pp.playerController.Send(msg)
}
...
func (pp *Player) streamSessionEvents(ctx context.Context, cancel context.CancelFunc) {
	defer pp.LogDebug("playbackPlayer.StreamSessionEvents returned")
	defer pp.close(cancel)
	...
	for {
		...
		if !pp.moveState.initialized && pp.playState == playStateMove {
			...
			if pp.moveState.position <= pp.lastDelay {
				...
				if err := pp.playerController.Reset(); err != nil {
					return
				}
			}
		}
		...
		case err := <-errC:
			if err != nil && !errors.Is(err, context.Canceled) {
				pp.LogWithError(err, "streaming session %v", pp.SID)
				...
				if err := pp.playerController.Error(errorText); err != nil {
					pp.LogDebug("Could not send error to client: %v", errorText)
				}
			}
			return
		case evt := <-eventsC:
			if evt == nil {
				pp.LogDebug("reached end of playback")
			...
			if err := pp.playerController.HandleEvent(ctx, payload); err != nil {
				return
			}
		}
	}
...
func (pp *Player) handleMoveState(delayMilliseconds int64) bool {
	...
	if isFirstEventAfterMove {
		...
		if err := pp.playerController.Move(pp.moveState.position); err != nil {
			pp.Log.Warnf("Could not move player to position: %v", pp.moveState.position)
		}
	}
...
func (pp *Player) IsPlaying() bool {
	pp.mu.Lock()
	defer pp.mu.Unlock()

	return pp.playState == playStatePlaying
}

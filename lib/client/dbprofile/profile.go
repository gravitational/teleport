...
// Package dbprofile contains methods for working with database connection
// profiles that combine connection parameters for a particular database.
//
...
import (
	...
	"github.com/gravitational/teleport/lib/client/dbprofile/mysql"
	"github.com/gravitational/teleport/lib/client/dbprofile/postgres"
...
func Add(tc *client.TeleportClient, db tlsca.RouteToDatabase, clientProfile client.ProfileStatus, quiet bool) error {
	...
	connectProfile := profile.ConnectProfile{
		...
		SSLRootCert: clientProfile.CACertPath(),
		SSLCert:     clientProfile.DatabaseCertPath(db.ServiceName),
		SSLKey:      clientProfile.KeyPath(),
	}
...
// load loads the appropriate database connection profile based.

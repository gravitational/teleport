...
func WithAzureIntegrationCredentials(integration string, auth AzureOIDCCredentials) AzureClientsOption {
	return func(cfg *azureClients) {
		cfg.azureIntegrationName = integration
		cfg.azureOIDCCredentials = auth
	}
}
...
type azureClients struct {
	...
	// azureOIDCCredentials is optional reference to auth RPC methods.
	azureOIDCCredentials AzureOIDCCredentials
	// azureIntegrationName is the optional Azure integration name.
	azureIntegrationName string
...
func (c *azureClients) initAzureCredential() (azcore.TokenCredential, error) {
	...
	if c.azureIntegrationName != "" {
		integration, err := c.azureOIDCCredentials.GetIntegration(ctx, c.azureIntegrationName)
		if err != nil {
			return nil, trace.Wrap(err)
		}
		...
		cred, err := azidentity.NewClientAssertionCredential(spec.TenantID, spec.ClientID, func(ctx context.Context) (string, error) {
			return c.azureOIDCCredentials.GenerateAzureOIDCToken(ctx, c.azureIntegrationName)
		}, nil)
		if err != nil {
			return nil, trace.Wrap(err)
		}

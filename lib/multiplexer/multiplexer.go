...
func signPROXYHeader(in signPROXYHeaderInput) ([]byte, error) {
	...
	logger := log.WithFields(log.Fields{
		"src_addr":        fmt.Sprintf("%v", sAddr),
		"dst_addr":        fmt.Sprintf("%v", dAddr),
		"allow_downgrade": in.allowDowngrade,
		"cluster_name":    in.clusterName})

	logger.Debug("signed PROXY header requested")
	...
	if isDifferentTCPVersion(sAddr, dAddr) {
		...
		logger = logger.WithField("pseudo_src_addr", fmt.Sprintf("%v", sAddr))
		logger.Debug("generated pseudo IPv4")
	}

	logger.Debug("signing PROXY JWT")
	...
	logger.Debug("adding teleport TLVs")
...
func NewPROXYSigner(signingCert *x509.Certificate, jwtSigner JWTPROXYSigner, allowDowngrade bool) (*PROXYSigner, error) {
	log.WithField("allow_downgrade", allowDowngrade).Debug("initializing PROXYSigner")

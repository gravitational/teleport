...
func (s *Service) OpenBPFSession(ctx *srv.ServerContext) (uint64, error) {
	...
	err = s.cgroup.Place(sessionID.String(), ctx.GetTerm().PID())
	if err != nil {
		return 0, trace.Wrap(err)
	}
...
func (s *Service) emitCommandEvent(eventBytes []byte) {
	...
	case eventRet:
		...
		if err := ctx.GetRecorder().EmitAuditEvent(ctx.Context, sessionCommandEvent); err != nil {
...
func (s *Service) emitDiskEvent(eventBytes []byte) {
	...
	_ = ctx.GetRecorder().EmitAuditEvent(ctx.Context, sessionDiskEvent)
}
...
func (s *Service) emit4NetworkEvent(eventBytes []byte) {
	...
	if err := ctx.GetRecorder().EmitAuditEvent(ctx.Context, sessionNetworkEvent); err != nil {

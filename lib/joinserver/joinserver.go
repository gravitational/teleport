...
type joinServiceClient interface {
	...
	RegisterUsingTPMMethod(ctx context.Context, initReq *proto.RegisterUsingTPMMethodInitialRequest, challengeResponse client.RegisterTPMChallengeResponseFunc) (*proto.Certs, error)
}
...
func (s *JoinServiceGRPCServer) RegisterUsingTPMMethod(srv proto.JoinService_RegisterUsingTPMMethodServer) error {
	...
	case <-timeout:
		...
		logrus.Warnf("TPM join attempt timed out, node at (%s) is misbehaving or did not close the connection after encountering an error.", nodeAddr)
		// Returning here should cancel any blocked Send or Recv operations.
		return trace.LimitExceeded("RegisterUsingTPMMethod timed out after %s, terminating the stream on the server", iamJoinRequestTimeout)
	case <-ctx.Done():
		return trace.Wrap(ctx.Err())
	}
}

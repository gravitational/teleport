...
type server struct {
	...
	// leafClusters is the list of connected remote clusters
	leafClusters []*leafCluster

	// localCluster is the  local (our own cluster) tunnel client.
...
func NewServer(cfg Config) (reversetunnelclient.Server, error) {
	...
	localCluster, err := newLocalCluster(srv, cfg.ClusterName, cfg.LocalAuthAddresses)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	srv.localCluster = localCluster
...
func (s *server) disconnectClusters(connectedRemoteClusters []*leafCluster, remoteMap map[string]types.RemoteCluster) error {
...
// remote cluster, or if no match, a cluster peer.
...
func (s *server) reportClusterStats(connectedRemoteClusters []*leafCluster, remoteMap map[string]types.RemoteCluster) error {
	...
	// resource in the backend but no associated remote cluster
	for cluster := range remoteMap {
		var exists bool
		for _, remoteCluster := range connectedRemoteClusters {
			if remoteCluster.GetName() == cluster {
				exists = true
				break
			}
		}
...
func (s *server) DrainConnections(ctx context.Context) error {
	...
	for _, cluster := range s.leafClusters {
		s.logger.DebugContext(ctx, "Advising reconnect to remote cluster", "remote_cluster", cluster.GetName())
...
func (s *server) upsertRemoteCluster(conn net.Conn, sshConn *ssh.ServerConn) (*leafCluster, *remoteConn, error) {
	...
	if leafCluster != nil {
		if remoteConn, err = leafCluster.addConn(conn, sshConn); err != nil {
			return nil, nil, trace.Wrap(err)
		}
	} else {
		leafCluster, err = newLeafCluster(s, domainName, sshConn.Conn)
		if err != nil {
			return nil, nil, trace.Wrap(err)
		}
		if remoteConn, err = leafCluster.addConn(conn, sshConn); err != nil {
		...
		s.leafClusters = append(s.leafClusters, leafCluster)
	}
	leafCluster.logger.InfoContext(s.ctx, "Processed inbound connection from remote cluster",
	...
	// heartbeat delay
	go leafCluster.registerHeartbeat(s.Clock.Now())
	return leafCluster, remoteConn, nil
}

func (s *server) GetSites() ([]reversetunnelclient.Cluster, error) {
	s.RLock()
	defer s.RUnlock()
	out := make([]reversetunnelclient.Cluster, 0, len(s.leafClusters)+len(s.clusterPeers)+1)
...
func (s *server) getRemoteClusters() []*leafCluster {
	s.RLock()
	defer s.RUnlock()
...
// GetSite returns a Cluster. The first attempt is to find and return a
// remote cluster and that is what is returned if a remote agent has
// connected to this proxy. Next we loop over local cluster and try and try and
// return a local cluster. If that fails, we return a cluster peer. This happens
...
// newLeafCluster helper creates and initializes 'leafCluster' instance
func newLeafCluster(srv *server, domainName string, sconn ssh.Conn) (*leafCluster, error) {
	...
	leafCluster.localClient = srv.localAuthClient
	leafCluster.localAccessPoint = srv.localAccessPoint

	clt, _, err := leafCluster.getRemoteClient()
	if err != nil {
		return nil, trace.Wrap(err)
	}
	leafCluster.remoteClient = clt
	...
	leafCluster.remoteAccessPoint = accessPoint
	...
	leafCluster.nodeWatcher = nodeWatcher
	...
	leafCluster.certificateCache = certificateCache
	...
	remoteWatcher, err := services.NewCertAuthorityWatcher(srv.ctx, services.CertAuthorityWatcherConfig{
		ResourceWatcherConfig: services.ResourceWatcherConfig{
			...
			Logger:    srv.logger,
			Clock:     srv.Clock,
			Client:    leafCluster.remoteAccessPoint,
		},
	...
	go func() {
		leafCluster.updateCertAuthorities(caRetry, remoteWatcher, remoteVersion)
	}()
	...
	go leafCluster.updateLocks(lockRetry)
...
func createRemoteAccessPoint(srv *server, clt authclient.ClientI, domainName string) (authclient.RemoteProxyAccessPoint, error) {
	...
	// cluster this remote cluster provides access to.

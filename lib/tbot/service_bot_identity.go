...
func (s *identityService) Run(ctx context.Context) error {
	...
	s.log.InfoContext(
		...
		"ttl", s.cfg.CertificateLifetime.TTL,
		"interval", s.cfg.CertificateLifetime.RenewalInterval,
	)
...
func renewIdentity(
	...
	if oldIdentity.TLSIdentity.Renewable {
		...
		newIdentity, err := botIdentityFromAuth(
			ctx, log, oldIdentity, authClient, botCfg.CertificateLifetime.TTL,
		)
		if err != nil {
...
func botIdentityFromToken(
	...
	expires := time.Now().Add(cfg.CertificateLifetime.TTL)

	...
	case SymlinksTrySecure:
		...
		if errors.Is(err, unix.ENOSYS) {
			missingSyscallWarning.Do(func() {
				log.Warn("Failed to open file securely due to missing syscall; falling back to regular file handling. Configure `symlinks: insecure` to disable this warning", "path", path)
			})
...
func createSecure(path string, isDir bool) error {
	...
	if err := f.Close(); err != nil {
		log.Warn("Failed to close file", "path", path, "err", err)
	}
...
func Create(path string, isDir bool, symlinksMode SymlinksMode) error {
	...
	case SymlinksTrySecure:
		...
		missingSyscallWarning.Do(func() {
			log.Warn("Failed to create file securely due to missing syscall; falling back to regular file handling. Configure `symlinks: insecure` to disable this warning", "path", path)
		})
...
func ConfigureACL(path string, owner *user.User, opts *ACLOptions) error {
	if owner.Uid == opts.BotUser.Uid && owner.Uid == opts.ReaderUser.Uid {
		...
		log.Warn("The owner, bot, and reader all appear to be the same "+
			"user. This is an unusual configuration: consider setting "+
	...
	log.Debug("Configuring ACL on path", "path", path, "acl", desiredACL)
...
func HasSecureWriteSupport() bool {
	...
	if err != nil {
		log.Info("Failed to determine kernel version. It will be assumed secure write support is not available", "err", err)
		return false
	}

...
import (
	...
	"github.com/gravitational/teleport/lib/client/loginagent"
...
func TestAgentService(t *testing.T) {
	...
	loginClient, err := loginagent.NewClient(outputDir)
	require.NoError(t, err)
	t.Cleanup(func() { require.NoError(t, loginClient.Close()) })

	loginResp, err := loginClient.Login(t.Context(), &loginagent.LoginRequest{})
	require.NoError(t, err)
	require.Equal(t, "bot-test-bot", loginResp.Username)
	require.NotEmpty(t, loginResp.SSHCert)
	require.NotEmpty(t, loginResp.TLSCert)
	require.NotEmpty(t, loginResp.PrivateKey)
	...
	privateKey, err := keys.ParsePrivateKey(
		loginResp.PrivateKey,
		keys.WithHardwareKeyService(hwks),
	)
	require.NoError(t, err)

	trustedCerts := make([]authclient.TrustedCerts, len(loginResp.HostSigners))
	for idx, cert := range loginResp.HostSigners {
		trustedCerts[idx] = authclient.TrustedCerts{
			ClusterName:     cert.ClusterName,
			AuthorizedKeys:  cert.SSHAuthorizedKeys,
			TLSCertificates: cert.TLSCACerts,
		}
	}

	keyRing := &client.KeyRing{
		SSHPrivateKey: privateKey,
		Cert:          loginResp.SSHCert,
		TLSPrivateKey: privateKey,
		TLSCert:       loginResp.TLSCert,

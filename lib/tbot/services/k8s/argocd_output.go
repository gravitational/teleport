...
//
// TODO(boxofrad): make these options work for the other Kubernetes services too.
type ArgoCDServiceOption func(*ArgoCDOutput)

// WithDefaultCredentialLifetime sets the service's default credential lifetime.
func WithDefaultCredentialLifetime(lifetime bot.CredentialLifetime) ArgoCDServiceOption {
	return func(svc *ArgoCDOutput) {
		svc.defaultCredentialLifetime = lifetime
	}
}

// WithKubernetesClient sets the service's Kubernetes clients. It's used in tests.
func WithKubernetesClient(k8s kubernetes.Interface) ArgoCDServiceOption {
	return func(svc *ArgoCDOutput) {
		svc.k8s = k8s
	}
}
...
func WithInsecure(insecure bool) ArgoCDServiceOption {
	return func(svc *ArgoCDOutput) {
...
func WithALPNUpgradeCache(cache *internal.ALPNUpgradeCache) ArgoCDServiceOption {
	return func(svc *ArgoCDOutput) {
		svc.alpnUpgradeCache = cache
	}
}
	...
	if proxyPong.Proxy.TLSRoutingEnabled {
		...
		if required {
			return nil, trace.Errorf(
				"Teleport proxy %q appears to be behind a Layer 7 load balancer that does not support ALPN. "+

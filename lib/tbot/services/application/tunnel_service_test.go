...
// service works in scoped mode, selects an exact scope among same-named apps,
// issues scoped app certs, and proxies traffic through to the backend.
func TestE2E_ScopedApplicationTunnelService(t *testing.T) {
	...
	// Register a same-named app in the parent scope. It is deliberately not
	// backed by an agent, so the tunnel must select the live child-scope app.
	parentApp, err := types.NewAppV3(types.Metadata{
		Name: appName,
	}, types.AppSpecV3{
		PublicAddr: scopedapp.ScopedAppPublicAddr(scopeName, appName, proxyPublicAddr),
		URI:        parentAppURI,
	}, scopeName)
	require.NoError(t, err)
	parentAppServer, err := types.NewAppServerV3FromApp(parentApp, "parent-app-host", "parent-app-host-id")
	require.NoError(t, err)
	_, err = process.GetAuthServer().UpsertApplicationServer(ctx, parentAppServer)
	require.NoError(t, err)

	// Create a scoped role that grants app access.
	makeScopedRole(t, rootClient, scopedRoleName, scopeName)
	...
	// Start the live app agent in a child scope.
	_ = makeScopedAppAgent(t, process, log, childScopeName, appName, httpSrv.URL)

	// Wait for both same-named apps to be visible before checking lookup behavior.
	require.EventuallyWithT(t, func(ct *assert.CollectT) {
		servers, err := rootClient.GetApplicationServers(ctx, apidefaults.Namespace)
		require.NoError(ct, err)
		gotApps := make(map[string]string)
		for _, server := range servers {
			app := server.GetApp()
			if app.GetName() == appName {
				gotApps[app.GetScope()] = app.GetURI()
			}
		}
		require.Equal(ct, map[string]string{
			scopeName:      parentAppURI,
			childScopeName: httpSrv.URL,
		}, gotApps)
	}, 10*time.Second, 100*time.Millisecond)

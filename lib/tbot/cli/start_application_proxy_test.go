...
// TestApplicationTunnelCommand tests that the ApplicationProxyCommand
// properly parses its arguments and applies as expected onto a BotConfig.
func TestApplicationProxyCommand(t *testing.T) {
	testStartConfigureCommand(t, NewApplicationProxyCommand, []startConfigureTestCase{
		{
			name: "success",
			args: []string{
				"start",
			...
			assertConfig: func(t *testing.T, cfg *config.BotConfig) {
				...
				svc := cfg.Services[0]
				appSvc, ok := svc.(*application.ProxyServiceConfig)
				require.True(t, ok)

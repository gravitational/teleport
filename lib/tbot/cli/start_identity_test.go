...
func TestIdentityCommand(t *testing.T) {
	...
	cmd := NewIdentityCommand(subcommand, mockAction.action)
	...
	require.Equal(t, "foo", cmd.Token)
	require.Len(t, cmd.CAPins, 1)
	require.Equal(t, "bar", cmd.CAPins[0])
	require.Equal(t, time.Minute*10, cmd.CertificateTTL)
	require.Equal(t, time.Minute*5, cmd.RenewalInterval)
	require.True(t, cmd.Oneshot)
	require.Equal(t, "0.0.0.0:8080", cmd.DiagAddr)
	require.Equal(t, "/foo", cmd.Storage)
	require.Equal(t, "file:///bar", cmd.Destination)
	require.Equal(t, "example.com:443", cmd.ProxyServer)

...
func (b *Bot) buildServices(ctx context.Context) ([]Service, func(), error) {
	...
	identityService, closeIdentityService, err := b.buildIdentityService(ctx, reloadBroadcaster, clientBuilder)
	if err != nil {
	...
	heartbeatService, err := b.buildHeartbeatService(identityService, startedAt)
	if err != nil {
		return nil, closeFn, trace.Wrap(err, "building heartbeat service")
	}
	services = append(services, heartbeatService)

	caRotationService, err := b.buildCARotationService(reloadBroadcaster, identityService)
	if err != nil {
...
func (b *Bot) buildHeartbeatService(identityService *identity.Service, startedAt time.Time) (*heartbeat.Service, error) {
...
func (b *Bot) buildCARotationService(reloadBroadcaster *internal.ChannelBroadcaster, identityService *identity.Service) (*carotation.Service, error) {

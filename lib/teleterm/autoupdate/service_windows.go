...
const (
	...
	teleportConnectAppGUID       = "22539266-67e8-54a3-83b9-dfdca7b33ee1"
	teleportConnectKeyPath       = `SOFTWARE\` + teleportConnectAppGUID
	...
	registryValueDownloadBaseURL   = "BaseUrl"
)
...
func platformGetConfig() (*api.GetConfigResponse, error) {
	...
	config := &api.GetConfigResponse{
		DownloadBaseUrl:      machineValues.baseUrl,
		RequiredToolsVersion: machineValues.version,
	}

	// If per-machine config is fully set, there's no need to check if there are values in the per-user hive.
	perMachineConfigFullySet := machineValues.baseUrl != "" && machineValues.version != ""
	...
	if machineValues.baseUrl == "" {
		config.DownloadBaseUrl = userValues.baseUrl
	}

	if machineValues.version == "" {
		config.RequiredToolsVersion = userValues.version
	}
...
func readRegistryPolicyValues(key registry.Key) (*policyValue, error) {
	...
	url, err := readyRegistryValue(key, teleportConnectPoliciesKeyPath, registryValueDownloadBaseURL)
	if err != nil && !trace.IsNotFound(err) {
		return nil, trace.Wrap(err)
	}

...
// makePipeServerSecurityDescriptor allows SYSTEM/Admins Full Control and grants
// Authenticated Users the passed access mask.
...
func (h *handler) Execute(ctx context.Context, _ []string) (err error) {
	...
	hash, err := downloadChecksum(ctx, h.testCfg.HTTPClient, updaterConfig.CDNBaseURL, updateMeta.Version)
	if err != nil {
...
func downloadChecksum(ctx context.Context, client *http.Client, baseUrl string, version string) ([]byte, error) {

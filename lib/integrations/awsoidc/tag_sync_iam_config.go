...
const (
	...
	defaultPolicyNameForTAGSync = "AWSTAGSyncAccess"
)

// TAGAWSIAMConfigureRequest is a request to configure the required Policies to use the TAG AWS Sync.
type TAGAWSIAMConfigureRequest struct {
	...
	// Defaults to "AWSTAGSyncAccess"
	IntegrationRoleTAGPolicy string
}

// CheckAndSetDefaults ensures the required fields are present.
func (r *TAGAWSIAMConfigureRequest) CheckAndSetDefaults() error {
...
// TAGIAMConfigureClient describes the required methods to create the IAM Policies required for enrolling TAG Sync into Teleport.
type TAGIAMConfigureClient interface {
...
// NewTAGIAMConfigureClient creates a new TAGIAMConfigureClient.
func NewTAGIAMConfigureClient(ctx context.Context) (TAGIAMConfigureClient, error) {
...
// ConfigureTAGSyncIAM sets up the roles required for Teleport to be able to pool
// AWS resources into Teleport.
// The following actions must be allowed by the IAM Role assigned in the Client.
//   - iam:PutRolePolicy
func ConfigureTAGSyncIAM(ctx context.Context, clt TAGIAMConfigureClient, req TAGAWSIAMConfigureRequest) error {
	if err := req.CheckAndSetDefaults(); err != nil {
		return trace.Wrap(err)
	}

	policyDocument, err := awslib.NewPolicyDocument(
		awslib.StatementTAGAWSSync(),
	).Marshal()
	if err != nil {
		return trace.Wrap(err)
	}

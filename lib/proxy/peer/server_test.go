...
func TestServerTLS(t *testing.T) {
	...
	stream, _, err := client1.dial([]string{"s1"}, "", "", &netutils.Addr{}, &netutils.Addr{}, "")
	require.NoError(t, err)
	...
	_, _, err = client2.dial([]string{"s2"}, "", "", &netutils.Addr{}, &netutils.Addr{}, "")
	require.Error(t, err)
	...
	require.NoError(t, err)
	stream, _, err = client3.dial([]string{"s3"}, "", "", &netutils.Addr{}, &netutils.Addr{}, "")
	require.NoError(t, err)
	require.NotNil(t, stream)
	stream.Close()
}

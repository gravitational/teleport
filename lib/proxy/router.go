...
func (r *Router) DialHost(ctx context.Context, from net.Addr, host, port, clusterName string, roleSet services.RoleSet, agentGetter teleagent.Getter) (net.Conn, error) {
	...
	if clusterName != r.clusterName {
		remoteSite, err := r.getRemoteCluster(ctx, clusterName, roleSet)
		if err != nil {
			return nil, trace.Wrap(err)
		}
		site = remoteSite
...
func (r *Router) getRemoteCluster(ctx context.Context, clusterName string, roleSet services.RoleSet) (reversetunnel.RemoteSite, error) {
	...
	if err := roleSet.CheckAccessToRemoteCluster(rc); err != nil {
...
func getServer(ctx context.Context, host, port string, site site) (types.Server, error) {
	...
	matches, err := site.GetNodes(func(server services.Node) bool {
		...
		if (host == ip || host == server.GetHostname() || apiutils.SliceContainsStr(ips, ip)) &&

...
func (c *RouterConfig) CheckAndSetDefaults() error {
	...
	if c.ClusterGetter == nil {
		return trace.BadParameter("ClusterGetter must be provided")
	}
...
func NewRouter(cfg RouterConfig) (*Router, error) {
	...
	localCluster, err := cfg.ClusterGetter.GetSite(cfg.ClusterName)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func (r *Router) DialHost(ctx context.Context, clientSrcAddr, clientDstAddr net.Addr, host, port, clusterName string, clusterAccessChecker func(types.RemoteCluster) error, agentGetter teleagent.Getter, signer agentless.SignerCreator) (_ net.Conn, err error) {
	...
	target, err := r.serverResolver(ctx, host, port, teleportCluster{cluster})
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
// getRemoteCluster looks up the provided clusterName to determine if a cluster exists with
...
func (r *Router) getRemoteCluster(ctx context.Context, clusterName string, clusterAccessChecker func(types.RemoteCluster) error) (reversetunnelclient.Cluster, error) {
	...
	cluster, err := r.clusterGetter.GetSite(clusterName)
	if err != nil {
...
// teleportCluster is a cluster implementation that wraps
...
func (r teleportCluster) GetNodes(ctx context.Context, fn func(n readonly.Server) bool) ([]types.Server, error) {
...
func (r teleportCluster) GetGitServers(ctx context.Context, fn func(n readonly.Server) bool) ([]types.Server, error) {
...
func (r teleportCluster) GetClusterNetworkingConfig(ctx context.Context) (types.ClusterNetworkingConfig, error) {
...
func getServerWithResolver(ctx context.Context, host, port string, cluster cluster, resolver apiutils.HostResolver) (types.Server, error) {
	if cluster == nil {
		return nil, trace.BadParameter("invalid cluster provided")
	}

...
func NewRelayRouter(clusterName string, dial relayTunnelDialFunc, accessPoint relayAccessPoint, nodeWatcher nodeWatcher) (*relayRouter, error) {
	return &relayRouter{
...
func (r *relayRouter) DialHost(ctx context.Context, clientSrcAddr net.Addr, clientDstAddr net.Addr, host string, port string, cluster string, _ func(types.RemoteCluster) error, _ teleagent.Getter, _ agentless.SignerCreator) (net.Conn, error) {
	if cluster != r.clusterName {
		return nil, trace.Errorf("dialing nodes for a different cluster through the Relay service is not supported")
	}

	src, err := r.accessPoint.GetSessionRecordingConfig(ctx)
	if err != nil {
		// deliberately not wrapping the error to not return a NotFound or something like that
		return nil, trace.Errorf("unable to determine recording mode: %s", err.Error())
	}
	if services.IsRecordAtProxy(src.GetMode()) {
		return nil, trace.Errorf("connectivity to SSH servers through the Relay service is not supported in Proxy recording mode")
	}
	...
	if server == nil {
		return nil, trace.Errorf("direct dialing to nodes through the Relay service is not supported")
	}

	if server.IsOpenSSHNode() {
		return nil, trace.Errorf("connectivity to agentless servers through the Relay service is not supported")
	}

	rwc, err := r.dial(ctx, server.GetName()+"."+r.clusterName, types.NodeTunnel, clientSrcAddr, clientDstAddr)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	return utils.NewConnWithAddr(rwc, clientDstAddr, clientSrcAddr), nil
}
...
func (r *relayRouter) DialSite(context.Context, string, net.Addr, net.Addr) (net.Conn, error) {
	return nil, trace.Errorf("connectivity to Auth services through the Relay service is not supported")
}
...
func (r *relayRouter) DialWindowsDesktop(context.Context, net.Addr, net.Addr, string, string, func(types.RemoteCluster) error) (net.Conn, error) {
	return nil, trace.Errorf("connectivity to Windows desktops through the Relay service is not supported")
}

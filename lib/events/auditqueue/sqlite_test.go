...
func marshalTestBatch(t *testing.T, index int64) []byte {
	...
	require.NoError(t, err)
	payload, err := marshalOneOf(oneOf)
	require.NoError(t, err)
	return encodeBatch([][]byte{payload})
}

func TestMarshalOneOf_RoundTrip(t *testing.T) {
	t.Parallel()

	oneOf, err := apievents.ToOneOf(newTestEvent(42))
	require.NoError(t, err)
	...
	require.Equal(t, int64(42), decoded[0].GetIndex())
	require.Equal(t, "user.login", decoded[0].GetType())
}
...
func TestEnqueue_ConcurrentGoodEnqueuesUnaffectedByOneToOneOfFailure(t *testing.T) {
	t.Parallel()
	q := newSqliteTestQueue(t)
	...
	go func() {
		defer wg.Done()
		bad := &badJSONEvent{
			UserLogin: newTestEvent(int64(N)).(*apievents.UserLogin),
			Extra:     math.NaN(),
		}
		badErr = q.Enqueue(bad)
	}()
	for i := 0; i < N; i++ {
		go func(i int) {
			defer wg.Done()
	...
	require.ErrorContains(t, badErr, "unsupported value")

	seen := make(map[int64]int)
	for i, err := range errs {
		require.NoError(t, err, "Enqueue %d must not be affected by the concurrently enqueued bad event", i)
	}
	...
	require.Len(t, seen, N, "exactly the N good events must be durably stored, no drops or duplicates")
	for i := int64(0); i < N; i++ {
		require.Equal(t, 1, seen[i], "index %d must appear exactly once", i)
	}
}

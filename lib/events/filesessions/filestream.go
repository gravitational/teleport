...
func (h *Handler) UploadPart(ctx context.Context, upload events.StreamUpload, partNumber int64, partBody io.ReadSeeker) (*events.StreamPart, error) {
	...
	if err := h.fileOps.WriteReservation(ctx, reservationPath, partBody); err != nil {
...
Loop:
	...
	if err := h.fileOps.CombineParts(ctx, f, partNames); err != nil {
		return trace.Wrap(err)
	}
...
func (h *Handler) ReserveUploadPart(ctx context.Context, upload events.StreamUpload, partNumber int64) error {
	reservationPath := h.reservationPath(upload, partNumber)
	if err := h.fileOps.CreateReservation(ctx, reservationPath, reservationSize); err != nil {
		return trace.Wrap(err)
	}

...
func (l *Log) EmitAuditEvents(ctx context.Context, batch []apievents.AuditEvent) error {
	...
	// Chunks that were committed before a failure are intentionally not rolled
	// back. The caller redelivers the whole batch (at-least-once delivery), and
	// the insert is idempotent due to de-duplication on database backends, or
	// on search queries in the case of Athena.

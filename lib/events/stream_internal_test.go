...
func TestReadOversizedMessage(t *testing.T) {
	...
	makePart := func(t *testing.T, messageSize uint32) []byte {
		...
		// append the ProtoStreamV1PartHeader
		var header []byte
		header = binary.BigEndian.AppendUint64(header, ProtoStreamV1)    // version
		header = binary.BigEndian.AppendUint64(header, uint64(gz.Len())) // part size
		header = binary.BigEndian.AppendUint64(header, uint64(0))        // padding
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			reader := NewProtoReader(bytes.NewReader(makePart(t, tc.messageSize)))
			defer reader.Close()

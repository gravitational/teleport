...
// detailsBlob returns a BlockBlobClient for the blob of the session details.
func (h *Handler) detailsBlob(sessionID session.ID) *blockblob.Client {
	return h.session.NewBlockBlobClient(detailsName(sessionID))
}

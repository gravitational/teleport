...
func TestFileLogCheckpoint(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			checkpoint: fmt.Sprintf("%s/%s", eventsInTimeFrame[0].GetID(), inTimeFrame.Format(time.RFC3339)),
			eventOrder: types.EventOrderAscending,
		...
		},
		{
			...
			checkpoint: fmt.Sprintf("%s/%s", eventsInTimeFrame[2].GetID(), inTimeFrame.Format(time.RFC3339)),
			eventOrder: types.EventOrderDescending,
		...
		},
		{
			...
			eventOrder:     types.EventOrderAscending,
			checkpoint:     fmt.Sprintf("%s/%s", eventsBeforeTimeFrame[0].GetID(), beforeTimeFrame.Format(time.RFC3339)),
			expectedEvents: eventsInTimeFrame,
		},
		{
			name:           "checkpoint after range (asc)",
			eventOrder:     types.EventOrderAscending,
			checkpoint:     fmt.Sprintf("%s/%s", eventsAfterTimeFrame[0].GetID(), afterTimeFrame.Format(time.RFC3339)),
			expectedEvents: nil,
		},
		{

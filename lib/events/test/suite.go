...
func (s *EventsSuite) EventPagination(c *check.C) {
	...
	err = utils.RetryStaticFor(time.Minute*5, time.Second*5, func() error {
		arr, checkpoint, err = s.Log.SearchEvents(baseTime, toTime, defaults.Namespace, nil, 100, types.EventOrderAscending, checkpoint)
		return err
	})
	c.Assert(err, check.IsNil)
	...
	for _, name := range names {
		arr, checkpoint, err = s.Log.SearchEvents(baseTime, toTime, defaults.Namespace, nil, 1, types.EventOrderAscending, checkpoint)
		c.Assert(err, check.IsNil)
	...
	for _, i := range []int{0, 2} {
		nameA := names[i]
		nameB := names[i+1]
		arr, checkpoint, err = s.Log.SearchEvents(baseTime, toTime, defaults.Namespace, nil, 2, types.EventOrderAscending, checkpoint)
		c.Assert(err, check.IsNil)
	...
	for i := len(names) - 1; i >= 0; i-- {
		arr, checkpoint, err = s.Log.SearchEvents(baseTime, toTime, defaults.Namespace, nil, 1, types.EventOrderDescending, checkpoint)
		c.Assert(err, check.IsNil)
		c.Assert(arr, check.HasLen, 1)
...
func (s *EventsSuite) SessionEventsCRUD(c *check.C) {
	...
	err = utils.RetryStaticFor(time.Minute*5, time.Second*5, func() error {
		history, _, err = s.Log.SearchEvents(s.Clock.Now().Add(-1*time.Hour), s.Clock.Now().Add(time.Hour), defaults.Namespace, nil, 100, types.EventOrderAscending, "")
		return err
	})
	c.Assert(err, check.IsNil)

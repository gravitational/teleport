...
func testDateExporterBasics(t *testing.T, randomFlake bool) {
	...
	// note that we do a lot more events here just to make absolutely certian
...
func (c *fakeClient) setFailExport(n int) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.failExport = n
}

func (c *fakeClient) setFailChunks(n int) {
	c.mu.Lock()
	defer c.mu.Unlock()
	c.failChunks = n
}
...
func (c *fakeClient) ExportUnstructuredEvents(ctx context.Context, req *auditlogpb.ExportUnstructuredEventsRequest) stream.Stream[*auditlogpb.ExportEventUnstructured] {
	...
	if c.failExport > 0 {
		c.failExport--
		return stream.Fail[*auditlogpb.ExportEventUnstructured](trace.NotFound("export failed as test condition"))
	}
...
func (c *fakeClient) GetEventExportChunks(ctx context.Context, req *auditlogpb.GetEventExportChunksRequest) stream.Stream[*auditlogpb.EventExportChunk] {
	...
	if c.failChunks > 0 {
		c.failChunks--
		return stream.Fail[*auditlogpb.EventExportChunk](trace.NotFound("chunks failed as test condition"))
	}
	chunks, ok := c.data[req.Date.AsTime().Format(time.DateOnly)]
	if !ok {

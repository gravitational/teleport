...
func (e *DateExporter) run(ctx context.Context) {
	...
	for chunk, cursor := range e.GetState().Cursors {
		slog.InfoContext(ctx, "resuming processing of partially completed chunk", "date", e.cfg.Date.Format(time.DateOnly), "chunk", chunk, "cursor", cursor)
	...
	for {
		...
		if err != nil {
			...
			if e.mainLogLimiter.Allow() {
				slog.WarnContext(ctx, "fetch and process of event chunks failed", "date", e.cfg.Date.Format(time.DateOnly), "error", err)
			}
		...
		if (n > 0 && e.mainLogLimiter.Allow()) || !firstFullCycleCompleted {
			slog.InfoContext(ctx, "successful fetch and process of event chunks", "date", e.cfg.Date.Format(time.DateOnly), "chunks", n, "idle", idle)
		}
	...
	for {
		...
		if err := e.exportEvents(ctx, events, entry); err != nil {
			...
			if e.chunkLogLimiter.Allow() {
				slog.WarnContext(ctx, "event chunk export failed", "date", e.cfg.Date.Format(time.DateOnly), "chunk", chunk, "failures", failures, "error", err)
			}
			retry.Inc()

...
type ExporterState struct {
	// Dates is a map of dates to their respective state.
...
type ExporterConfig struct {
	...
	// PreviousState is an optional paramter used to resume from a previous date export run.
...
func NewExporter(cfg ExporterConfig) (*Exporter, error) {
	...
	var initError error
	e.withLock(func() {
		if cfg.PreviousState.IsEmpty() {
			// no previous state, start at the beginning
			initError = e.startExportLocked(ctx, cfg.StartDate)
			return
		}
...
func (e *Exporter) run(ctx context.Context) {
	...
	for {
		...
		if err != nil && logLimiter.Allow() {
			...
			slices.Sort(dates)
			slog.WarnContext(ctx, "event exporter poll failed", "error", err, "dates", dates)
		}
...
func (e *Exporter) poll(ctx context.Context) (bool, error) {
	...
	if e.current.IsIdle() {
		if normalizeDate(time.Now()).After(e.currentDate) {
			...
			slog.InfoContext(ctx, "event exporter advancing to next target", "date", nextDate.Format(time.DateOnly))
		} else {
			caughtUp = true
		}
	}
...
func (e *Exporter) pruneBacklogLocked(ctx context.Context) {
	...
	for _, date := range dates[e.cfg.BacklogSize:] {
		...
		slog.InfoContext(ctx, "event exporter halted historical event processing", "date", date)
	}
}

...
type UploadHandler interface {
	...
	// Download downloads a session recording and returns a ReadCloser for the content.
	Download(ctx context.Context, sessionID session.ID) (io.ReadCloser, error)
	...
	// DownloadSummary downloads a session summary and returns a ReadCloser for
	// the content. Returns a "not found" error if there's no such summary.
	DownloadSummary(ctx context.Context, sessionID session.ID) (io.ReadCloser, error)

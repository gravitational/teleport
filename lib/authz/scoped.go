...
// AuthorizeAdminActionAllowReusedMFA is equivalent to [Context.AuthorizeAdminActionAllowReusedMFA], but will
// automatically deny in the case that the underlying identity is scoped.
func (s *ScopedContext) AuthorizeAdminActionAllowReusedMFA() error {
	if s.unscopedContext != nil {
		return s.unscopedContext.AuthorizeAdminActionAllowReusedMFA()
	}

	// we currently categorically disallow admin actions with scoped identities. if we choose
	// to revisit this policy in the future we will need to audit all currently scope-enabled
	// APIs that support conditional admin actions to verify that a scoped admin action model
	// does not introduce any security regressions.
	return trace.AccessDenied("admin action cannot be performed with scoped identity")
}

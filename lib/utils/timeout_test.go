...
func TestObeyDelayedIdleTimeout(t *testing.T) {
	clock := clockwork.NewFakeClock()
	t.Cleanup(func() { clock.Advance(24 * time.Hour) })

	c1, c2 := net.Pipe()
	c1, enableWatchdog := obeyDelayedIdleTimeout(c1, clock)
...
func TestObeyDelayedIdleTimeoutNeverEnabled(t *testing.T) {
	clock := clockwork.NewFakeClock()
	ctx := t.Context()
	t.Cleanup(func() { clock.Advance(24 * time.Hour) })

	c1, c2 := net.Pipe()
	c1, _ = obeyDelayedIdleTimeout(c1, clock)

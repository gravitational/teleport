...
// FindMatchingDNS checks if a given request host or app fqdn matches any of the specified proxy DNS names.
...
//
// Parameters:
//   - requestHost: A string representing the host in the request, which may include a port.
...
func FindMatchingProxyDNS(requestHostOrFQDN string, proxyDNSNames []string) string {
	if requestHostOrFQDN == "" || len(proxyDNSNames) == 0 {
		return ""
	}

	// Remove port from request host if present.
	normalizedRequestHost := strings.Split(requestHostOrFQDN, ":")[0]

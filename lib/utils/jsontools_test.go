...
func TestStreamJSONArray(t *testing.T) {
	...
	err := StreamJSONArrayFromIter(iter.Slice(objects), &objBuf, true)
	require.NoError(t, err)
	...
	err = StreamJSONArrayFromIter(iter.Slice(numbers), &numBuf, false)
	require.NoError(t, err)
	...
	for i := 0; i < 100; i++ {
		...
		err = StreamJSONArrayFromIter(iter.Slice(iterative), &iterBuf, false)
		require.NoError(t, err)

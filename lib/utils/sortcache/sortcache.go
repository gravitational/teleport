...
type Config[T any, I comparable, E comparable] struct {
...
type SortCache[T any, I comparable, E comparable] struct {
	rw      sync.RWMutex
	indexes map[I]func(T) E
	trees   map[I]*btree.BTreeG[entry[E]]
	values  map[uint64]T
	counter uint64
}
...
func New[T any, I comparable, E comparable](cfg Config[T, I, E]) *SortCache[T, I, E] {
	const (
		// bTreeDegree of 8 is standard across most of the teleport codebase
		bTreeDegree = 8
	)

	trees := make(map[I]*btree.BTreeG[entry[E]], len(cfg.Indexes))

	for index := range cfg.Indexes {
		trees[index] = btree.NewG(bTreeDegree, func(a, b entry[E]) bool {
			return cfg.LessFunc(a.key, b.key)
		})
	}

	return &SortCache[T, I, E]{
		indexes: cfg.Indexes,
		trees:   trees,
...
func (c *SortCache[T, I, E]) Get(index I, key E) (value T, ok bool) {
	c.rw.RLock()
	defer c.rw.RUnlock()
...
func (c *SortCache[T, I, E]) HasIndex(name I) bool {
...
func (c *SortCache[T, I, E]) KeyOf(index I, value T) E {
	// index map is treated as immutable once created, so no lock is required.
...
func (c *SortCache[T, I, E]) lookup(index I, key E) (ref uint64, ok bool) {
	tree, exists := c.trees[index]
	if !exists {
		return 0, false
	}

	entry, exists := tree.Get(entry[E]{key: key})
	if !exists {
		return 0, false
	}
...
func (c *SortCache[T, I, E]) Put(value T) (evicted int) {
	c.rw.Lock()
	defer c.rw.Unlock()
	...
	for index, fn := range c.indexes {
		...
		c.trees[index].ReplaceOrInsert(entry[E]{
			key: key,
			ref: c.counter,
		})
	}
...
func (c *SortCache[T, I, E]) Delete(index I, key E) {
	c.rw.Lock()
	defer c.rw.Unlock()
...
func (c *SortCache[T, I, E]) deleteValue(ref uint64) {
	...
	for idx, fn := range c.indexes {
		c.trees[idx].Delete(entry[E]{key: fn(value)})
	}
}
...
// sequence is.
func (c *SortCache[T, I, E]) readItems(tree *btree.BTreeG[entry[E]], start, stop E, desc bool, out []T) (n int, next E) {
	c.rw.RLock()
	defer c.rw.RUnlock()

	fn := func(ent entry[E]) bool {
		if n == len(out) {
			next = ent.key
			return false
		}
	...
	if desc {
		...
		case start == e && stop == e:
			tree.Descend(fn)
		case start == e:
			tree.DescendGreaterThan(entry[E]{key: stop}, fn)
		case stop == e:
			tree.DescendLessOrEqual(entry[E]{key: start}, fn)
		default:
			tree.DescendRange(entry[E]{key: start}, entry[E]{key: stop}, fn)
		}
	} else {
		switch {
		case start == e && stop == e:
			tree.Ascend(fn)
		case start == e:
			tree.AscendLessThan(entry[E]{key: stop}, fn)
		case stop == e:
			tree.AscendGreaterOrEqual(entry[E]{key: start}, fn)
		default:
			tree.AscendRange(entry[E]{key: start}, entry[E]{key: stop}, fn)
		}
	}
...
func (c *SortCache[T, I, E]) items(index I, start, stop E, desc bool) iter.Seq[T] {
...
func (c *SortCache[T, I, E]) Ascend(index I, start, stop E) iter.Seq[T] {
...
func (c *SortCache[T, I, E]) Descend(index I, start, stop E) iter.Seq[T] {
	return c.items(index, start, stop, true)
}

// Len returns the number of values currently stored.
func (c *SortCache[T, I, E]) Len() int {
	c.rw.RLock()
	defer c.rw.RUnlock()
	return len(c.values)
}

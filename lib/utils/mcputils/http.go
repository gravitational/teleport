...
const (
	// TeleportErrorOriginHeader is set on HTTP error responses produced or
	// forwarded by the Teleport Application Service so that clients (tsh) can
	// attribute the failure to the right component instead of guessing.
	TeleportErrorOriginHeader = "X-Teleport-Mcp-Error-Origin"

	// ErrorOriginAppService indicates the error response was generated by the
	// Teleport Application Service itself while proxying the request.
	ErrorOriginAppService = "app-service"
	// ErrorOriginUpstream indicates the error response came from the remote
	// MCP server and was passed through by the Application Service.
	ErrorOriginUpstream = "upstream"
	// ErrorOriginUpstreamUnreachable indicates the Application Service could
	// not reach the remote MCP server (dial failure or timeout).

...
type ReconfigureRequest struct {
	...
	NodeLabels string
	// SourceHostUUID sets the node label types.ReconfigureSourceUUIDLabel,
	// recording the host the input config was read on.
...
func Reconfigure(fc *FileConfig, req ReconfigureRequest) error {
	...
	// After the node-labels merge, so the recorded source wins over both a
	// stale label carried in the input and a --node-labels value.
	if req.SourceHostUUID != "" {
		if fc.SSH.Labels == nil {
			fc.SSH.Labels = make(map[string]string, 1)
		}
		fc.SSH.Labels[types.ReconfigureSourceUUIDLabel] = req.SourceHostUUID
	}

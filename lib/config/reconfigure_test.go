...
// reconfigureStaleSourceUUID carries a source-uuid label from an earlier
// reconfigure run on another host.
const reconfigureStaleSourceUUID = `version: v3
teleport:
  data_dir: /var/lib/teleport
  proxy_server: old.example.com:443
ssh_service:
  enabled: yes
  labels:
    teleport.dev/reconfigure-source-uuid: old-uuid
`
...
func TestReconfigureLabelsAndFields(t *testing.T) {
	...
	t.Run("source host uuid becomes a label", func(t *testing.T) {
		fc := parseTestConfig(t, reconfigureBaseV3) // ssh_service has labels: {env: prod}
		err := Reconfigure(fc, ReconfigureRequest{SourceHostUUID: "uuid-x"})
		require.NoError(t, err)
		require.Equal(t, map[string]string{
			"env":                            "prod",
			types.ReconfigureSourceUUIDLabel: "uuid-x",
		}, fc.SSH.Labels)
		requireRoundTrips(t, fc)
	})

	t.Run("source host uuid creates the labels map when absent", func(t *testing.T) {
		fc := parseTestConfig(t, reconfigureLegacyToken) // no ssh_service section
		err := Reconfigure(fc, ReconfigureRequest{SourceHostUUID: "uuid-x"})
		require.NoError(t, err)
		require.Equal(t, map[string]string{types.ReconfigureSourceUUIDLabel: "uuid-x"}, fc.SSH.Labels)
		requireRoundTrips(t, fc)
	})

	t.Run("source host uuid overwrites the input's stale label", func(t *testing.T) {
		fc := parseTestConfig(t, reconfigureStaleSourceUUID)
		err := Reconfigure(fc, ReconfigureRequest{SourceHostUUID: "new-uuid"})
		require.NoError(t, err)
		require.Equal(t, "new-uuid", fc.SSH.Labels[types.ReconfigureSourceUUIDLabel])
		requireRoundTrips(t, fc)
	})

	t.Run("source host uuid wins over node-labels on the same key", func(t *testing.T) {
		fc := parseTestConfig(t, reconfigureBaseV3)
		err := Reconfigure(fc, ReconfigureRequest{
			NodeLabels:     types.ReconfigureSourceUUIDLabel + "=user-set,team=a",
			SourceHostUUID: "real-uuid",
		})
		require.NoError(t, err)
		require.Equal(t, "real-uuid", fc.SSH.Labels[types.ReconfigureSourceUUIDLabel])
		require.Equal(t, "a", fc.SSH.Labels["team"])
		requireRoundTrips(t, fc)
	})

	t.Run("empty source host uuid preserves the input's label", func(t *testing.T) {
		fc := parseTestConfig(t, reconfigureStaleSourceUUID)
		err := Reconfigure(fc, ReconfigureRequest{})
		require.NoError(t, err)
		require.Equal(t, "old-uuid", fc.SSH.Labels[types.ReconfigureSourceUUIDLabel])
		requireRoundTrips(t, fc)
	})
	...
	for _, tc := range []struct {
	...
	}{
		{
			...
			req: ReconfigureRequest{
				...
				SourceHostUUID: "8a2b5c70-97d8-4d97-b3c4-6e54b4a63cfe",
				DataDir:        "/var/lib/teleport_new",
			},
		},
		{

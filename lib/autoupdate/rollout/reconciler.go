	...
	for tries < maxConflictRetry {
		...
		default:
			...
			case trace.IsCompareFailed(err), trace.IsNotFound(err):
				...
				r.log.WithError(err).Debug("retrying reconciliation")
...
func (r *reconciler) computeStatus(
	...
	if timeOverride := status.GetTimeOverride().AsTime(); !(timeOverride.IsZero() || timeOverride.Unix() == 0) {
		r.log.WithFields(logrus.Fields{
			...
			"real_time":     now,
		}).Debug("reconciling with synthetic time instead of real time")
		now = timeOverride
	}
	...
	if err != nil {
		r.log.WithError(err).Error("Errors encountered during rollout progress. Some groups might not get updated properly.")
	}

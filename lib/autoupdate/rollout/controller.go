...
func NewController(client Client, log *logrus.Entry, clock clockwork.Clock, period time.Duration, reg prometheus.Registerer) (*Controller, error) {
	if client == nil {
	...
	log = log.WithField(teleport.ComponentLabel, teleport.ComponentRolloutController)
...
func (c *Controller) Run(ctx context.Context) error {
	...
	c.log.WithField("period", c.period).Info("Starting autoupdate_agent_rollout controller")
	for {
		select {
		case <-ctx.Done():
			c.log.WithField("reason", ctx.Err()).Info("Stopping autoupdate_agent_rollout controller")
			return ctx.Err()
		case <-ticker.Next():
			c.log.Debug("Reconciling autoupdate_agent_rollout")
			if err := c.tryAndCatch(ctx); err != nil {
				c.log.WithError(err).Error("Failed to reconcile autoudpate_agent_controller")
			}
		}
	}
}
...
func (c *Controller) tryAndCatch(ctx context.Context) (err error) {
	...
	defer func() {
		if r := recover(); r != nil {
			c.log.Errorf("Recovered from panic in the autoupdate_agent_rollout controller: %#v", r)

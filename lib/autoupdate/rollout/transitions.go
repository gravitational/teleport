...
func TriggerGroups(rollout *autoupdatev1pb.AutoUpdateAgentRollout, groupNames []string, desiredState autoupdatev1pb.AutoUpdateAgentGroupState, now time.Time) error {
	...
	err := checkSchedule(rollout)
	if err != nil {
		return trace.Wrap(err)
	}

	err = strategySupportsManualTransition(rollout)
	if err != nil {
		return trace.Wrap(err)
	}
	...
	default:
		return trace.BadParameter("unsupported desired state: %s, supported states are 'unspecified' and 'active'.", getGroupStateName(desiredState))
	}
	...
	for _, groupName := range groupNames {
		...
		default:
			return trace.BadParameter("group %q in unexpected state %s", groupName, getGroupStateName(group.GetState()))
		}
...
func RollbackGroups(rollout *autoupdatev1pb.AutoUpdateAgentRollout, groupNames []string, now time.Time) error {
	...
	for _, groupName := range groupNames {
		...
		if group.GetState() == autoupdatev1pb.AutoUpdateAgentGroupState_AUTO_UPDATE_AGENT_GROUP_STATE_ROLLEDBACK {
			return trace.AlreadyExists("group %q is already rolledback", groupName)
		}
...
func getGroup(groups []*autoupdatev1pb.AutoUpdateAgentRolloutStatusGroup, groupName string) (*autoupdatev1pb.AutoUpdateAgentRolloutStatusGroup, error) {
	...
	return nil, trace.NotFound("group %v not found", groupName)
}

// getGroupStateName converts a group state enum into a string. This is used
// to craft user-friendly error messages.
func getGroupStateName(state autoupdatev1pb.AutoUpdateAgentGroupState) string {
	if name, ok := autoupdatev1pb.AutoUpdateAgentGroupState_name[int32(state)]; ok {
		return name
	}

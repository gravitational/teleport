...
[Service]
ExecStart=
ExecStart=-/bin/echo "The deprecated teleport-upgrade script is disabled by teleport-update. Please remove the teleport-ent-updater package."
`
...
func NewNamespace(ctx context.Context, log *slog.Logger, name, installDir string) (ns *Namespace, err error) {
	...
	if name == "" {
		...
		return &Namespace{
			...
			deprecatedDropInFile: filepath.Join(systemdAdminDir, "teleport-upgrade.service.d", BinaryName+".conf"),

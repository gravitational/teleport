...
[Service]
Type=oneshot
ExecStart={{.UpdaterBinary}} --install-suffix={{.InstallSuffix}} --install-dir="{{escape .InstallDir}}" --link-dir="{{escape .LinkDir}}" update
`
...
type Namespace struct {
	...
	dataDir string
	// proxyAddr parsed from teleport.yaml, if present
	proxyAddr string
	...
	pidFile string
	// updaterVersionsDir for Teleport versions (ns: /opt/teleport/myns/versions)
	updaterVersionsDir string
...
// linkDir overrides the destination directory for namespace setup (i.e., /usr/local)
func NewNamespace(ctx context.Context, log *slog.Logger, name, installDir, linkDir string) (ns *Namespace, err error) {
	...
	if name == "" {
		nsDir := filepath.Join(installDir, defaultNamespace)
		if linkDir == "" {
		...
		return &Namespace{
			...
			updaterVersionsDir:  filepath.Join(nsDir, versionsDirName),
			...
			updaterLockFile:     filepath.Join(nsDir, lockFileName),
			updaterConfigFile:   filepath.Join(nsDir, updateConfigName),
	...
	nsDir := filepath.Join(installDir, name)
	prefix := "teleport_" + name
	if linkDir == "" {
		linkDir = filepath.Join(nsDir, "bin")
	}
	return &Namespace{
		log:                 log,
		name:                name,
...
func (ns *Namespace) Init() (lockFile string, err error) {
	if err := os.MkdirAll(ns.updaterVersionsDir, systemDirMode); err != nil {
...
func (ns *Namespace) writeConfigFiles(ctx context.Context) error {
	...
	params := confParams{
		...
		UpdaterBinary:     ns.updaterBinFile,

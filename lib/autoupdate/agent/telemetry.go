...
func HelloUpdaterInfo() (*proto.UpdaterV2Info, error) {
	...
	// TODO: After rebasing on top of Stephen's status PR, read UUID and report it

	switch {
	case conf.Spec.Enabled == false:
		info.UpdaterStatus = proto.UpdaterStatus_UpdaterStatusDisabled
	case conf.Spec.Pinned == true:
		info.UpdaterStatus = proto.UpdaterStatus_UpdaterStatusPinned
	default:
		info.UpdaterStatus = proto.UpdaterStatus_UpdaterStatusOK
	}

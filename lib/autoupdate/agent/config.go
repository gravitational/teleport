...
type UpdateSpec struct {
	...
	// InsecureSkipArtifactSignature disables artifact signature verification and falls back
	// to checksum-only verification. This is intended for development builds that are not signed.
	InsecureSkipArtifactSignature bool `yaml:"insecure_skip_artifact_signature,omitempty"`
...
func updateConfigSpec(spec *UpdateSpec, override OverrideConfig) error {
	...
	if override.InsecureSkipArtifactSignatureChanged {
		spec.InsecureSkipArtifactSignature = override.InsecureSkipArtifactSignature
	}
	if override.Enabled {
		spec.Enabled = true
	}

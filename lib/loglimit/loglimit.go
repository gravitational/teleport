...
type Config struct {
	...
	// Handler is forwarded any messages that should be emitted.
...
type LogLimiter struct {
	...
	// lastEviction indicates the last time the full windows mapping
...
func (l *LogLimiter) Handle(ctx context.Context, record slog.Record) error {
	...
	shouldLog := func(now time.Time) bool {
		...
		defer func() {
			...
			if now.After(l.lastSweep.Add(timeWindow * 3)) {
				for key, lastSeen := range l.windows {
					if now.After(lastSeen.Add(timeWindow * 2)) {

...
func newClusterSchemaBuilder(client *kubernetes.Clientset) (serializer.CodecFactory, rbacSupportedResources, error) {
	...
	supportedResources := maps.Clone(allowedResources)

	if err := registerDefaultKubeTypes(kubeScheme); err != nil {
		return kubeCodecs, supportedResources, trace.Wrap(err)
	}
	...
	for _, apiGroup := range apiGroups {
		...
		for _, apiResource := range apiGroup.APIResources {
			...
			supportedResources[resourceKey] = utils.KubeCustomResourceDefinition
...
func getKubeAPIGroupAndVersion(groupVersion string) (string, string) {

...
func (f *Forwarder) deleteResourcesCollection(sess *clusterSession, w http.ResponseWriter, req *http.Request) (resp any, err error) {
	...
	case !isLocalKubeCluster:
		...
		status = rw.Status()
	case kubeObjType == utils.KubeCustomResourceDefinition && namespace != "":
...
func (f *Forwarder) handleDeleteCustomResourceCollection(w http.ResponseWriter, req *http.Request, sess *clusterSession) (int, error) {
	// Access to custom resources is controlled by the KidKubeNamespace.

	...
	tests := []struct {
	...
	}{
		{
			...
			assert: func(t *testing.T, restConfig *rest.Config) {
				...
				_, err = clientB.CoreV1().Pods(metav1.NamespaceDefault).List(t.Context(), metav1.ListOptions{})
				require.NoError(t, err)

				clientA := pathRoutedKubeClient(t, restConfig, clusterName, "a")
				_, err := clientA.CoreV1().Pods(metav1.NamespaceDefault).List(t.Context(), metav1.ListOptions{})
				require.NoError(t, err)
		...
		},
		{
			...
			assert: func(t *testing.T, restConfig *rest.Config) {
				...
				_, err = client.CoreV1().Pods(metav1.NamespaceDefault).Get(t.Context(), "foo", metav1.GetOptions{})

...
func (s *session) disconnectPartyOnErr(idString string, err error) {
	if idString == sessionRecorderID {
		s.log.ErrorContext(s.forwarder.ctx, "Failed to write to session recorder, closing session")
		s.Close()
		return
	}
	...
	if uuidParseErr != nil {
		s.log.ErrorContext(s.forwarder.ctx, "Unable to decode party id",
			"party_id", idString,
			"error", uuidParseErr,
		)
		return
	...
	if leaveErr != nil {
		s.log.ErrorContext(s.forwarder.ctx, "Failed to disconnect party from the session",
			"party_id", idString,
			"error", leaveErr,
		)
	}
	if wasActive {
		...
		s.log.ErrorContext(s.forwarder.ctx, "Encountered error with party, disconnecting them from the session",
			"error", err,
			"party_id", idString,
		)
	}
...
func (s *session) checkPresence() error {
	...
	for _, participant := range s.tracker.GetParticipants() {
		...
		if participant.Mode == string(types.SessionModeratorMode) && time.Now().UTC().After(participant.LastActive.Add(PresenceMaxDifference)) {
			s.log.DebugContext(s.forwarder.ctx, "Participant is not active, kicking", "participant_id", participant.ID)
			id, _ := uuid.Parse(participant.ID)
			_, err := s.unlockedLeave(id)
			if err != nil {
				s.log.WarnContext(s.forwarder.ctx, "Failed to kick participant for inactivity",

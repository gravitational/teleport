...
func newSession(ctx authContext, forwarder *Forwarder, req *http.Request, params httprouter.Params, initiator *party, sess *clusterSession) (*session, error) {
	...
	s := &session{
		...
		// An in-band validated MFA challenge marks the session MFA-verified the same way
		// an MFA-stamped certificate does.
		PresenceEnabled:                ctx.Identity.GetIdentity().MFAVerified != "" || ctx.inBandMFAVerified,

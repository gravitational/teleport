...
type Action struct {
	// Item is the item to be written (only used when Kind is Put). The item's Key
	// field will be disregarded, and the actual key will be taken from the
	// conditional action.
	Item Item
...
// Put builds an action that writes the provided item to the target key. The
// item's Key field will be disregarded, and the actual key will be taken from
// the conditional action. The item's Revision field will also be disregarded
// and substituted by a value generated by the AtomicWrite function.

...
func (b *Backend) pollStreams(externalCtx context.Context) error {
	...
	shouldStartPoll := func(shard *dynamodbstreams.Shard) bool {
		...
		if _, ok := set[aws.StringValue(shard.ParentShardId)]; ok {
			b.Warnf("Skipping child shard: %s, still polling parent %s", sid, aws.StringValue(shard.ParentShardId))
	...
	refreshShards := func(init bool) (bool, error) {
	...
	if closing, err := refreshShards(true); closing || err != nil {
		return trace.Wrap(err)
	}
	...
	for {
		...
		case <-ticker.C:
			if closing, err := refreshShards(false); closing || err != nil {
				return trace.Wrap(err)
			}
		case <-ctx.Done():

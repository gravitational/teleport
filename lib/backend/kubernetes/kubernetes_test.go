...
func TestBackend_Put(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name: "secret does not exist and should not be created",
			fields: fields{
				objects:     nil,
				namespace:   "test",
				replicaName: "agent-0",
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if _, err = b.Put(t.Context(), tt.args.item); tt.wantErr {
				require.Error(t, err)
			...
			if got, err := b.getSecret(t.Context()); tt.wantErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
...
func TestBackend_Create(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			// Create upserts the content in the secret.
			if _, err = b.Create(t.Context(), tt.args.item); tt.wantErr {
				require.Error(t, err)
			} else {
				require.NoError(t, err)
			}

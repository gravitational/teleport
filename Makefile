...
CARGO_GET_VERSION = $(if $(BIN_JQ),$(CARGO_GET_VERSION_JQ),$(CARGO_GET_VERSION_AWK))
...
ensure-wasm-bindgen: VERSION = $(shell $(call CARGO_GET_VERSION,wasm-bindgen))
ensure-wasm-bindgen: INSTALLED_VERSION = $(lastword $(shell wasm-bindgen --version 2>/dev/null))
ensure-wasm-bindgen:
	@: $(or $(VERSION),$(error Unknown wasm-bindgen version. Is it in Cargo.lock?))
	$(if $(filter-out $(INSTALLED_VERSION),$(VERSION)),\
		cargo install wasm-bindgen-cli --locked --version "$(VERSION)", \
		@echo wasm-bindgen-cli up-to date: $(INSTALLED_VERSION) \
	)

ensure-wasm-pack: VERSION = $(shell $(MAKE) -s -C build.assets print-wasm-pack-version)
	...
	$(if $(filter-out $(INSTALLED_VERSION),$(VERSION)),\
		...
		@echo wasm-pack-cli up-to date: $(INSTALLED_VERSION) \
	)

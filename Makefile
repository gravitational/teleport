...
test-go-unit:
	$(CGOFLAG) GOEXPERIMENT=synctest go test -cover -json -tags "enablesynctest $(PAM_TAG) $(FIPS_TAG) $(BPF_TAG) $(LIBFIDO2_TEST_TAG) $(TOUCHID_TAG) $(PIV_TEST_TAG) $(VNETDAEMON_TAG) $(EXTRA_TAGS)" $(PACKAGES) $(SUBJECT) $(FLAGS) $(ADDFLAGS) \

...
integration:
	...
	$(CGOFLAG) go test -p 2 -timeout 30m -tags "$(PAM_TAG) $(FIPS_TAG) $(BPF_TAG) $(ROLETESTER_TAG) $(RDPCLIENT_TAG)" $(PACKAGES) $(FLAGS)
...
integration-root:
	$(CGOFLAG) go test -p 2 -run "$(INTEGRATION_ROOT_REGEX)" $(PACKAGES) $(FLAGS)
endif

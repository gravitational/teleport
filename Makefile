...
test-go-bench: $(BENCHFIND) | $(TEST_LOG_DIR)
	...
	go test -run ^$$ -bench . -skip $(BENCHMARK_SKIP_PATTERN) -benchtime $(BENCH_TIME) -count $(BENCH_COUNT) $(BENCH_ARGS) $$PKGS \
		| tee $(OUT_FILE)
...
test-go-bench-root: $(BENCHFIND) | $(TEST_LOG_DIR)
	...
	go test -run ^$$ -bench $(BENCHMARK_PATTERN) -skip $(BENCHMARK_SKIP_PATTERN) -benchtime $(BENCH_TIME) -count $(BENCH_COUNT) $(BENCH_ARGS) $$PKGS \
		| tee $(OUT_FILE)
...
benchstat:
ifndef FILES
	$(error "Please provide FILES=<file1> <file2> ...")
endif
	@$(BENCHSTAT) $(FILES) | tee test-logs/benchstat.txt
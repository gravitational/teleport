...
import { SideNavPage } from '@gravitational/e2e/helpers/pages/SideNav';
...
test('Verify navigation with no allow rules', async ({ page }, testInfo) => {
  ...
  await test.step('Audit section only contains Active Sessions', async () => {
    const panel = await sideNav.openSection('Audit');
  ...
  await test.step('Identity Governance contains Access Requests with Enterprise CTA', async () => {
    const panel = await sideNav.openSection('Identity Governance');
  ...
  await test.step('Add New section contains Resource and Integration', async () => {
    const panel = await sideNav.openSection('Add New');
...
test('Verify read-only access to resources', async ({ page }, testInfo) => {
  // We add a longer timeout to add some buffer because this includes a full signup flow and injecting a session recording.
  test.setTimeout(30_000);
  const username = `test-user-${testInfo.workerIndex}`;
  ...
  await test.step('User can see roles but cant create/delete/update', async () => {
    const ztaPanel = await sideNav.openSection('Zero Trust Access');
  ...
  await test.step('User can see auth connectors', async () => {
    const ztaPanel2 = await sideNav.openSection('Zero Trust Access');
    await expect(
  ...
  await test.step('User can access Users screen but cant create/delete/update', async () => {
    const ztaPanel3 = await sideNav.openSection('Zero Trust Access');
    await expect(ztaPanel3.getByRole('link', { name: 'Users' })).toBeVisible();
  ...
  await test.step('User can access Trusted Root Clusters but cant create/delete/update', async () => {
    const ztaPanel4 = await sideNav.openSection('Zero Trust Access');
    await expect(
      ztaPanel4.getByRole('link', { name: 'Trusted Root Clusters' })
    ).toBeVisible();
    await page.goto('/web/trust');
    await expect(page.getByText('Trusted Root Clusters').first()).toBeVisible();
    ...
    await page.getByRole('button', { name: 'Edit Trusted Cluster' }).click();

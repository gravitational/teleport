...
func testRDS(t *testing.T) {
	...
	opts := []testOptionsFunc{
		...
		withUserRole(t, autoUserDropWithReassignment, "db-auto-user-dropper-reassignment", makeAutoUserReassignAndDropRoleSpec(autoRole3)),
		...
		withUserRole(t, autoUserDropWithReassignment2, "db-auto-user-dropper-reassignment", makeAutoUserReassignAndDropRoleSpec(autoRole3)),
	}
				...
				for name, test := range map[string]struct {
					...
					"auto user drop with reassignment": {
						...
						testFunc: func(t *testing.T, pgConn *pgconn.PgConn) {
							...
							checkPostgresTableReassignment(t, ctx, conn, testSchema, tableForReassignment)
						},
					},
					"db permissions": {
...
func checkPostgresTableReassignment(
	...
	require.Equal(t, "teleport-object-inheritor", owner)
}
...
func cloneAndCustomizeDB(t *testing.T, db types.Database, admin *types.DatabaseAdminUser) types.Database {
	t.Helper()
	clone := db.Copy()

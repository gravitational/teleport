...
func pullImage(ctx context.Context, image string, arch nodeArch) error {
	slog.InfoContext(ctx, "pulling docker image", "image", image, "arch", arch)
	...
	if err := rc.Wait(ctx); err != nil {
		...
		slog.WarnContext(ctx, "could not refresh docker image, using the cached one", "image", image, "error", err)
	}

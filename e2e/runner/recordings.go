...
func seedRecordings(ctx context.Context, e2eDir, dataDir string, recordingOwners map[string][]recordingOwner) error {
	...
	recIDs := make([]string, 0, len(recordingOwners))
	for id := range recordingOwners {
	...
	for _, recID := range recIDs {
		...
		for _, owner := range recordingOwners[recID] {

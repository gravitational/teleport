...
func Run(args []string, stdout io.Writer) error {
	...
	supportJoinMethods := []string{"token", "iam", "oidc-gcp"}
	...
	startCmd.Flag("join-method", "Method to use to join the cluster (token, iam, oidc-gcp).").Default(config.DefaultJoinMethod).EnumVar(&cf.JoinMethod, supportJoinMethods...)
	...
	configureCmd.Flag("join-method", "Method to use to join the cluster (token, iam, oidc-gcp).").Default(config.DefaultJoinMethod).EnumVar(&cf.JoinMethod, supportJoinMethods...)

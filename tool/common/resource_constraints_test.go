...
func TestParseResourceValues(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:      "double slash in resource name stays in name",
		...
		{
			name:      "question mark in name without key shape stays in name",
			values:    []string{"/main/node/web?1"},
			assertErr: require.NoError,
			want: []types.ResourceAccessID{
				{Id: types.ResourceID{ClusterName: "main", Kind: types.KindNode, Name: "web?1"}},
...
func TestSplitInlineConstraints(t *testing.T) {
	...
	tests := []struct {
		...
		{"question mark in name, no key shape", "/main/node/web?1", "/main/node/web?1", ""},
		{"question mark in name before constraint", "/main/node/web?1?logins=root", "/main/node/web?1", "logins=root"},
		// Keys this build has never seen still split off cleanly instead of
		// being folded into the resource name. An older client then reports
		// an unknown key rather than a mangled resource ID.
...
func TestParseResourceAccessIDListJSON(t *testing.T) {
	...
	t.Run("wildcard in list is rejected", func(t *testing.T) {
		t.Parallel()
		data := []byte(`{"resources":[
			{"id":{"cluster":"main","kind":"node","name":"web-1"},"constraints":{"version":"v1","ssh":{"logins":["*"]}}}
		]}`)
		_, err := ParseResourceAccessIDListJSON(data)
...
func TestParseResourceValuesEscaping(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:  "slashes in value are never special",
			value: `/main/app/aws-console?role_arns=arn:aws:iam::123:role/x//y/Admin`,
			want:  awsRAID("main", "aws-console", "arn:aws:iam::123:role/x//y/Admin"),
		},
		{
			name:  "ampersand in value without a key shape stays literal",
			value: `/main/app/aws-console?role_arns=arn:aws:iam::123:role/a&b`,
			want:  awsRAID("main", "aws-console", "arn:aws:iam::123:role/a&b"),
		},
		{
			name:  "escaped ampersand keeps a key-shaped value in one pair",
			value: `/main/app/aws-console?role_arns=arn:aws:iam::123:role/a\&logins=b`,
			want:  awsRAID("main", "aws-console", "arn:aws:iam::123:role/a&logins=b"),
		},
		{
		...
		{
			name:    "slash escape is no longer recognized",
			value:   `/main/node/web-1?logins=a\/b`,
			wantErr: "unsupported escape",
		},
		{
		...
		{
			name:    "wildcard rejected for logins",
			value:   `/main/node/web-1?logins=*`,

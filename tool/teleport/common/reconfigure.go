...
func runReconfigure(flags reconfigureFlags, stdout io.Writer) error {
	...
	// The uuid names the host the input config was read on, if present. Best effort: when the uuid is
	// unreadable (a machine that never ran an agent, an unprivileged user),
	// ReadFile returns "" and the label is not added.
	sourceDataDir := fc.DataDir
	if sourceDataDir == "" {
		sourceDataDir = defaults.DataDir
	}
	sourceHostUUID, _ := hostid.ReadFile(sourceDataDir)

...
// The input fixtures take a data dir instead of hardcoding one, so
// runReconfigure's host UUID probe reads a per-test temp dir and never the
// machine's real /var/lib/teleport.

func reconfigureTestInput(dataDir string) string {
	return fmt.Sprintf(`version: v3
teleport:
  data_dir: %s
...
// --pid-file collides.
func reconfigureCollisionTestInput(dataDir string) string {
	return fmt.Sprintf(`version: v3
teleport:
  data_dir: %s
  proxy_server: old.example.com:443
...
func writeReconfigureInput(t *testing.T) string {
	t.Helper()
	return writeReconfigureInputFile(t, reconfigureTestInput(t.TempDir()))
}

// writeHostUUID plants a host_uuid file in dataDir, with the 0400 mode the
// agent writes it with.
func writeHostUUID(t *testing.T, dataDir, id string) {
	t.Helper()
	require.NoError(t, os.WriteFile(filepath.Join(dataDir, hostid.FileName), []byte(id), 0o400))
}
...
func TestRunReconfigure(t *testing.T) {
	...
	t.Run("collision with the input agent is a hard error", func(t *testing.T) {
		var stdout bytes.Buffer
		err := runReconfigure(reconfigureFlags{
			input:       writeReconfigureInputFile(t, reconfigureCollisionTestInput(t.TempDir())),
			proxyServer: "new.example.com:443",
		}, &stdout)
	...
	t.Run("source host uuid from the input's data dir becomes a label", func(t *testing.T) {
		var stdout bytes.Buffer
		dataDir := t.TempDir()
		writeHostUUID(t, dataDir, "uuid-x\n") // trailing newline exercises trimming
		err := runReconfigure(reconfigureFlags{
			input:       writeReconfigureInputFile(t, reconfigureTestInput(dataDir)),
			proxyServer: "new.example.com:443",
		}, &stdout)
		require.NoError(t, err)
		fc, err := config.ReadConfig(strings.NewReader(stdout.String()))
		require.NoError(t, err)
		require.Equal(t, "uuid-x", fc.SSH.Labels[types.ReconfigureSourceUUIDLabel])
	})

	t.Run("unreadable host uuid omits the label", func(t *testing.T) {
		var stdout bytes.Buffer
		err := runReconfigure(reconfigureFlags{
			input:       writeReconfigureInput(t), // data dir with no host_uuid file
			proxyServer: "new.example.com:443",
		}, &stdout)
		require.NoError(t, err)
		fc, err := config.ReadConfig(strings.NewReader(stdout.String()))
		require.NoError(t, err)
		require.NotContains(t, fc.SSH.Labels, types.ReconfigureSourceUUIDLabel)
	})

	t.Run("data-dir does not change where the source uuid is read from", func(t *testing.T) {
		var stdout bytes.Buffer
		oldDataDir := t.TempDir()
		newDataDir := t.TempDir()
		writeHostUUID(t, oldDataDir, "old-host-uuid")
		writeHostUUID(t, newDataDir, "new-host-uuid")
		err := runReconfigure(reconfigureFlags{
			input:       writeReconfigureInputFile(t, reconfigureTestInput(oldDataDir)),
			proxyServer: "new.example.com:443",
			dataDir:     newDataDir,
		}, &stdout)
		require.NoError(t, err)
		fc, err := config.ReadConfig(strings.NewReader(stdout.String()))
		require.NoError(t, err)
		require.Equal(t, "old-host-uuid", fc.SSH.Labels[types.ReconfigureSourceUUIDLabel])
		require.Equal(t, newDataDir, fc.DataDir)
	})
}
...
func TestTeleportReconfigure(t *testing.T) {
	dir := t.TempDir()
	dataDir := t.TempDir()
	writeHostUUID(t, dataDir, "e2e-host-uuid")
	input := filepath.Join(dir, "teleport.yaml")
	require.NoError(t, os.WriteFile(input, []byte(reconfigureTestInput(dataDir)), 0o600))
	...
	require.Equal(t, map[string]string{
		"team":                           "a",
		types.ReconfigureSourceUUIDLabel: "e2e-host-uuid",
	}, fc.SSH.Labels)

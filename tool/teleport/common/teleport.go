...
func Run(options Options) (app *kingpin.Application, executedCommand string, conf *servicecfg.Config) {
	...
	dbConfigureCreate.Flag("output",
		`Write to stdout with "-o stdout", default config file with "-o file" or custom path with --output=file:///path`).Short('o').Default(
	...
	systemdInstall.Flag("output", `Write to stdout with "-o stdout" or custom path with --output=file:///path`).Short('o').Default(teleport.SchemeStdout).StringVar(&systemdInstallFlags.output)
...
func onIntegrationConfAWSOIDCIdP(params config.IntegrationConfAWSOIDCIdP) error {
	...
	err = awsoidc.ConfigureIdPIAM(ctx, iamClient, awsoidc.IdPIAMConfigureRequest{
		...
		S3BucketLocation:   params.S3BucketURI,
		S3JWKSContentsB64:  params.S3JWKSContentsB64,
	})
	if err != nil {
		return trace.Wrap(err)
	}

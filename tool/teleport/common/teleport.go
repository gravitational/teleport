...
func Run(options Options) (app *kingpin.Application, executedCommand string, conf *servicecfg.Config) {
	...
	integrationConfAWSOIDCIdPCmd.Flag("s3-bucket-uri", "The S3 URI(format: s3://<bucket>/<prefix>) used to store the openid configuration and public keys. "+
		"").StringVar(&ccf.IntegrationConfAWSOIDCIdPArguments.S3BucketURI)
	integrationConfAWSOIDCIdPCmd.Flag("s3-jwks-base64", `The JWKS base 64 encoded. Required when using the S3 Bucket as the Issuer URL. Format: base64({"keys":[{"kty":"RSA","alg":"RS256","n":"<value of n>","e":"<value of e>","use":"sig","kid":""}]}).`+
		"").StringVar(&ccf.IntegrationConfAWSOIDCIdPArguments.S3JWKSContentsB64)

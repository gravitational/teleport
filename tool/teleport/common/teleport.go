...
func Run(options Options) (app *kingpin.Application, executedCommand string, conf *servicecfg.Config) {
	...
	dbStartCmd.Flag("gcp-project-id", "(Only for Cloud SQL/AlloyDB) Project identifier.").StringVar(&ccf.DatabaseGCPProjectID)
	dbStartCmd.Flag("gcp-instance-id", "(Only for Cloud SQL/AlloyDB) Instance identifier.").StringVar(&ccf.DatabaseGCPInstanceID)
	dbStartCmd.Flag("gcp-cluster-id", "(Only for AlloyDB) Cluster identifier.").StringVar(&ccf.DatabaseGCPClusterID)
	dbStartCmd.Flag("gcp-region", "(Only for AlloyDB) Region identifier.").StringVar(&ccf.DatabaseGCPRegion)
	dbStartCmd.Flag("gcp-alloy-db", "(Only for AlloyDB) Marks the database as AlloyDB instance.").BoolVar(&ccf.DatabaseIsAlloyDB)

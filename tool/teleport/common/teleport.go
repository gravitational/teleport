...
func Run(options Options) (app *kingpin.Application, executedCommand string, conf *service.Config) {
	...
	dbStartCmd.Flag("aws-elasticache-replication-group-id", "(Only for ElastiCache Redis) ElastiCache replication group identifier.").StringVar(&ccf.DatabaseAWSRDSClusterID)

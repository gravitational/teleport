...
func Run(options Options) (app *kingpin.Application, executedCommand string, conf *service.Config) {
	...
	dbConfigureCreate.Flag("azure-sqlserver-discovery", "List of Azure regions in which the agent will discover Azure Cache For Redis servers.").StringsVar(&dbConfigCreateFlags.AzureSQLServerDiscoveryRegions)

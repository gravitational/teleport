...
// matching template.
func (t ProxyTemplates) Apply(fullHostname string) (proxy, host, cluster, query, search string, matched bool) {
	for _, template := range t {
		proxy, host, cluster, query, search, matched := template.Apply(fullHostname)
		if matched {
			return proxy, host, cluster, query, search, true
		}
	}
	return "", "", "", "", "", false
}
...
func (t ProxyTemplate) Apply(fullHostname string) (proxy, host, cluster, query, search string, matched bool) {

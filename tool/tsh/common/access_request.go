...
type requestableRoleRow struct {
	Role        string `asciitable:"Role"`
	Description string `asciitable:"Description"`
}
...
type kubeResourceRow struct {
	...
	Namespace  string `asciitable:"Namespace"`
	Labels     string `asciitable:"Labels"`
	ResourceID string `asciitable:"Resource ID"`
}

type dbResourceRow struct {
	DatabaseName string `asciitable:"Database Name"`
	Labels       string `asciitable:"Labels"`
	ResourceID   string `asciitable:"Resource ID"`
}
...
type genericResourceRow struct {
	Name       string `asciitable:"Name"`
	Hostname   string `asciitable:"Hostname"`
	Labels     string `asciitable:"Labels"`
	ResourceID string `asciitable:"Resource ID"`
}
...
func printRequestableRoles(cf *CLIConf, rows []requestableRoleRow) error {
	...
	case teleport.Text, "":
		...
		columns, rows, err := asciitable.MakeAsciitableColumnsAndRows(rows, nil)
		if err != nil {
			return err
		}
...
func printRequestableResources[T resourceRow](cf *CLIConf, rows []T, resourceIDs []string) error {
	...
	switch format {
	case teleport.Text, "":
		columns, tableRows, err := asciitable.MakeAsciitableColumnsAndRows(rows, nil)
		if err != nil {
			return err
		}

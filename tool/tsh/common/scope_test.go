...
func TestResolveDesiredScope(t *testing.T) {
	tests := []struct {
		...
		},
		{
			...
			cf:              &CLIConf{},
			profile:         &client.ProfileStatus{ScopePin: scopesv1.Pin_builder{Scope: "/staging/west"}.Build()},
		...
		},
		{
			...
			profile:         &client.ProfileStatus{ScopePin: scopesv1.Pin_builder{Scope: "/staging/east"}.Build()},
			wantScope:       "/staging/east",
			wantScopeChange: false,
		},
		{

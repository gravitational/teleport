...
func dataDirFor(t *testing.T, shared bool) string {
	...
	return newSharedFixtureTempDir(t)
}

// newSharedFixtureTempDir returns a fresh temp dir that outlives the calling test.
// Unlike t.TempDir it is removed by TestMain, so a fixture created in it can be reused by later tests.
func newSharedFixtureTempDir(t *testing.T) string {
	t.Helper()

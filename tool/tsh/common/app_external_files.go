...
func writeFilesForExternalApps(stdout io.Writer, appInfo *appInfo, exportCredentials bool) error {
	if appInfo.RouteToApp.AWSCredentialProcessCredentials != "" {
		if exportCredentials {
			return trace.Wrap(exportAWSCredentials(appInfo.RouteToApp.AWSCredentialProcessCredentials, stdout))
		}

		return trace.Wrap(addAWSProfileToConfig(appInfo.RouteToApp.Name))
	}
...
// exportAWSCredentials writes a shell script that, when executed, exports the required permissions to access AWS APIs.
// Receives the credentials as a JSON string, which are formatted as "credential_process".
//
// This output is meant to be consumed by the `eval` shell utility.
func exportAWSCredentials(credentialsAsJsonString string, stdout io.Writer) error {

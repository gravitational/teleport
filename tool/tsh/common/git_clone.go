...
func onGitClone(cf *CLIConf) error {
	...
	if app.IsGitHub() {
		return onGitCloneGitHub(cf, tc, app)
	}
	if app.IsAWSConsole() {
		return onGitCloneAWS(cf, tc, app)
	}

	return trace.BadParameter("app %v of type %v is not supported for %v", app.GetName(), app.GetProtocol(), cf.command)
}

func onGitCloneGitHub(cf *CLIConf, tc *client.TeleportClient, app types.Application) error {
...
func onGitCloneAWS(cf *CLIConf, tc *client.TeleportClient, app types.Application) error {
	...
	// TODO ensureGitRemoteIsInstalled(cf)

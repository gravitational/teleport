...
func Run(ctx context.Context, args []string, opts ...CliOption) error {
	...
	dbExec.Flag("max-connections", "Run queries on target databases concurrently. Defaults to 1, and maximum allowed is 10.").IntVar(&cf.MaxConnections)
	dbExec.Flag("log-dir", "Directory to log separated command output.").StringVar(&cf.SSHLogDir)
	dbExec.Flag("dbs", "List of comma separated database services. Mutually exclusive with --search or --labels.").StringVar(&cf.DatabaseServices)

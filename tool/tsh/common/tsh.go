...
func Run(ctx context.Context, args []string, opts ...CliOption) error {
	...
	benchDBQuery := benchDB.Command("query", "Run Database query benchmark. It will evaluate the time to execute queries on the target database.").Hidden()
	benchDBQuery.Arg("database", "Database to run the benchmark on. Can be obtained from 'tsh db ls' output.").StringVar(&cf.DatabaseService)
	benchDBQuery.Flag("create-sql", "For SQL databases only. Query used for creating the benchmark table. It can also be used to load data. The time to execute this query is not counted on the benchmark results.").StringVar(&benchDBOpts.CreateSQL)
	benchDBQuery.Flag("query-sql", "For SQL databases only. Query database clients will execute during benchmark. Can rely on table and data generated by --create query.").StringVar(&benchDBOpts.QuerySQL)
	...
	case benchDBQuery.FullCommand():
		err = onBenchmark(
			&cf,
			&benchmark.DBQueryBenchmark{
				Config:    dbBenchBase,
				CreateSQL: benchDBOpts.CreateSQL,
				QuerySQL:  benchDBOpts.QuerySQL,
			},
		)

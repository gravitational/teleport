...
func Run(ctx context.Context, args []string, opts ...CliOption) error {
	...
	case resolve.FullCommand():
		err = onResolve(&cf)
		// If quiet was specified from this command and
		...
		// if no matches were found that's okay and we
		// should add additional spam to stderr.
		if err != nil && cf.Quiet {
			os.Exit(1)
		}
...
// from a true hostname into a search, or predicate query.

...
func onLogin(cf *CLIConf, reExecArgs ...string) error {
	...
	if cf.Scope != "" {
		// client-side validation of scopes isn't strictly necessary, but scope syntax is easy to mess up (especially accidentally omitting
		// the leading slash), so its nice to find out if the scope is malformed before going through authentication.
		if err := scopes.StrongValidate(cf.Scope); err != nil {
			return trace.Wrap(err)
		}

		// TODO(fspmarshall/scopes): this is a very clunky way to handle the forced reauth on scope change, look into

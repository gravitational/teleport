...
func Run(ctx context.Context, args []string, opts ...CliOption) error {
	...
	sessionsSummarize := sessions.Command("summarize", "Retrieve a session recording summary.")
	sessionsSummarize.Arg("session-id", "ID of a session whose summary will be retrieved.").Required().StringVar(&cf.SessionID)
...
func onSummarizeSession(cf *CLIConf) error {
	...
	res, err := clt.AuthClient.SummarizerServiceClient().GetSummary(
		cf.Context, &summarizerv1.GetSummaryRequest{SessionId: cf.SessionID})
	if err != nil {
		return trace.Wrap(err)
	}
	...
	case summarizerv1.SummaryState_SUMMARY_STATE_SUCCESS,
		...
		fmt.Print("The following summary was generated by AI from the session transcript.\n\n")
		fmt.Println(res.GetSummary().GetContent())
		println("finish date ", res.Summary.GetInferenceFinishedAt())
	}

...
func Run(ctx context.Context, args []string, opts ...CliOption) error {
	...
	app.Flag("home-dir", "Directory where client state, credentials, and config are stored. Defaults to \"~/.tsh\".").Envar(types.HomeEnvVar).StringVar(&cf.HomePath)
	app.Flag("config-path", "Path to file where global client configuration is stored. Defaults to \"/etc/tsh.yaml\".").Envar(globalTshConfigEnvVar).StringVar(&cf.HomePath)

...
type CLIConf struct {
	...
	AppName string
	// AppHTTPS enables a special mode to tunnel https for HTTP apps. Mainly
	// used for debugging purpose so the flag should be hidden.
	AppHTTPS bool
...
func Run(ctx context.Context, args []string, opts ...CliOption) error {
	...
	proxyApp.Flag("https", "Tunnels HTTPS instead of HTTP for HTTP apps.").Hidden().BoolVar(&cf.AppHTTPS)

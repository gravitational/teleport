...
func (c *scopesLSCommand) run(cf *CLIConf) error {
	...
	if err := tc.WithRootClusterClient(ctx, func(clt authclient.ClientI) error {
		assignments, err = stream.Collect(scopeutils.RangeScopedRoleAssignments(ctx, clt.ScopedAccessServiceClient(), &scopedaccessv1.ListScopedRoleAssignmentsRequest{

...
type AVIEncoder struct {
	...
	outputPrefix     string
	// files holds the paths of the AVI files written so far, in order.
	files []string
}
...
// openNextFile creates the AVI writer for the current file count and records
// its path in the list of output files.
func (a *AVIEncoder) openNextFile() error {
	filename := makeAVIFileName(a.outputPrefix, a.currentFileCount)
	writer, err := mjpeg.New(filename, a.maxWidth, a.maxHeight, a.framesPerSecond)
	if err != nil {
		return trace.Wrap(err)
	}
	a.currentWriter = writer
	a.files = append(a.files, filename)
	return nil
}

// OutputFiles returns the paths of the AVI files written, in order.
func (a *AVIEncoder) OutputFiles() []string {
	return a.files
}

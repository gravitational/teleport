...
type CLIConf struct {
	...
	// ParticipantReq is set if verbose participant requirement information should be printed for moderated sessions.
	ParticipantReq bool
}
...
func Run(args []string, opts ...cliOption) error {
	...
	ssh.Flag("participant-req", "Displays a verbose list of required participants in a moderated session.").BoolVar(&cf.ParticipantReq)
...
func makeClient(cf *CLIConf, useProfileLogin bool) (*client.TeleportClient, error) {
	...
	tc.Config.VerboseParticipantRequirements = cf.ParticipantReq
	return tc, nil
}

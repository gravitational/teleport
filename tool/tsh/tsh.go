...
type CLIConf struct {
	...
	// ConfigProxyLeaf is the name of the leaf cluster, if any.
	ConfigProxyLeaf string
}
...
func Run(args []string, opts ...cliOption) error {
	...
	configProxy.Arg("target", "Target node name").Required().StringVar(&cf.ConfigProxyTarget)
	configProxy.Arg("leaf-cluster", "Leaf cluster name, if any").StringVar(&cf.ConfigProxyLeaf)

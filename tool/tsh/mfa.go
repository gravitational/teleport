...
`, otpKey.URL(), c.Account, c.Secret, c.Issuer, c.Algorithm, c.Digits, c.PeriodSeconds)
	...
	// length.
	for {
		totpCode, err = client.ReadPassword(ctx, os.Stdout, "Once created, enter an OTP code generated by the app")
		if err != nil {
			return nil, trace.Wrap(err)
		}
...
func (p *mfaAddPrompt) PromptPIN() (string, error) {
	return client.ReadPassword(p.ctx, os.Stdout, "Enter your *new* security key PIN")
}

...
`, otpKey.URL(), c.Account, c.Secret, c.Issuer, c.Algorithm, c.Digits, c.PeriodSeconds)
	...
	// length.
	for {
		totpCode, err = client.ReadPassword(os.Stdout, "Once created, enter an OTP code generated by the app")
		if err != nil {
			return nil, trace.Wrap(err)
		}
...
func (m mfaAddPrompt) PromptPIN() (string, error) {
	return client.ReadPassword(os.Stdout, "Enter your *new* security key PIN")
}

...
func (p *ExecOptions) Run() error {
	...
	fn := func() error {
		...
		req := restClient.Post().
			...
			SubResource("exec").
			Param("displayDisplayParticipantRequirements", strconv.FormatBool(p.displayDisplayParticipantRequirements))
...
func newKubeExecCommand(parent *kingpin.CmdClause) *kubeExecCommand {
	...
	c.Flag("participant-req", "Displays a verbose list of required participants in a moderated session.").BoolVar(&c.displayDisplayParticipantRequirements)
...
func (c *kubeExecCommand) run(cf *CLIConf) error {
	...
	p.displayDisplayParticipantRequirements = c.displayDisplayParticipantRequirements

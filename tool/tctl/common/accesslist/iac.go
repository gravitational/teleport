...
func (c *Command) writeIACConfig(al *accesslist.AccessList, accessRoles []*types.RoleV6, members []*accesslist.AccessListMember, alPresetType string) error {
	switch c.iac {
	case iacTerraform:
		return trace.Wrap(c.writeIACTerraform(al, accessRoles, members, alPresetType))
	default:
		return trace.BadParameter("unsupported --iac value %q", c.iac)
	}
}

func (c *Command) writeIACTerraform(al *accesslist.AccessList, accessRoles []*types.RoleV6, members []*accesslist.AccessListMember, alPresetType string) error {

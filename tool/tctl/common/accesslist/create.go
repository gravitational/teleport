...
func (c *Command) Create(ctx context.Context, client *authclient.Client) error {
	...
	// Requested dry-run: generate and print the IaC config instead of
	// creating anything.
	if c.iac != "" {
...
func (c *Command) validateCreate() error {
	if c.iac != "" && c.formatSet {
		return trace.BadParameter("--iac and --format cannot be combined")
	}
...
func (c *Command) dryRunCreate(newAccessList *accesslist.AccessList, members []*accesslist.AccessListMember) error {
	...
	return trace.Wrap(c.writeIACConfig(newAccessList, accessRoles, members, presetType(c.accessType)))
}

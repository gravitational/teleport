...
func (rc *ResourceCommand) createDatabaseObjectImportRule(ctx context.Context, client auth.ClientI, raw services.UnknownResource) error {
	...
	if existingRule != nil {
		...
		_, err = client.DatabaseObjectImportRuleClient().UpdateDatabaseObjectImportRule(ctx, &dbobjectimportrulev1.UpdateDatabaseObjectImportRuleRequest{Item: rule})
		if err != nil {
			return trace.Wrap(err)
		}
		fmt.Printf("rule %q has been updated\n", ruleName)
	} else {
		_, err = client.DatabaseObjectImportRuleClient().CreateDatabaseObjectImportRule(ctx, &dbobjectimportrulev1.CreateDatabaseObjectImportRuleRequest{Item: rule})
		if err != nil {
			return trace.Wrap(err)
		}

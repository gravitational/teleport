...
func (rc *ResourceCommand) getCollection(ctx context.Context, client *authclient.Client) (resources.Collection, error) {
	...
	case scopedaccess.KindScopedRole:
		...
		items, err := stream.Collect(scopeutils.RangeScopedRoles(ctx, client.ScopedAccessServiceClient(), &scopedaccessv1.ListScopedRolesRequest{}))
		if err != nil {
			return nil, trace.Wrap(err)
		}
	...
	case scopedaccess.KindScopedRoleAssignment:
		...
		items, err := stream.Collect(scopeutils.RangeScopedRoleAssignments(ctx, client.ScopedAccessServiceClient(), &scopedaccessv1.ListScopedRoleAssignmentsRequest{}))
		if err != nil {
			return nil, trace.Wrap(err)
		}

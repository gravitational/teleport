...
func TestFilterICAccounts(t *testing.T) {
	...
	resources := []*types.EnrichedResource{
		{ResourceWithLabels: newICAppServer("222222222222", "Bravo")},
		{ResourceWithLabels: regularApp},
		{ResourceWithLabels: node},
		{ResourceWithLabels: newICAppServer("111111111111", "Alpha")},
	}
	...
	require.Len(t, got, 2)
	require.Equal(t, "Alpha", icAccountName(got[0].GetApp()))
	require.Equal(t, "Bravo", icAccountName(got[1].GetApp()))
}

func TestAWSICWriteText(t *testing.T) {
	t.Parallel()
	...
	t.Run("ls", func(t *testing.T) {
		...
		c := &AWSICCommand{}
		require.NoError(t, c.writeText(&buf, servers))

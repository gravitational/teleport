...
func (c *VersionControlCommand) Initialize(app *kingpin.Application, config *service.Config) {
	...
	c.vcDirectiveDelete = directive.Command("delete", "Manually set version directive status").Alias("rm").Hidden()
...
func (c *VersionControlCommand) ListDirectives(ctx context.Context, client auth.ClientI) error {
	...
	// simplify handling of directives by aggregating them into a map
	dm := make(map[types.VersionDirectivePhase][]*types.VersionDirectiveV1)

	if directives.Active != nil {
		dm[types.DirectivePhaseActive] = append(dm[types.DirectivePhaseActive], directives.Active)
	}

	for _, d := range directives.Pending {
		dm[types.DirectivePhasePending] = append(dm[types.DirectivePhasePending], d)
	}

	for _, d := range directives.Draft {
		dm[types.DirectivePhaseDraft] = append(dm[types.DirectivePhaseDraft], d)
	}
	...
	case teleport.Text:
		...
		// iterate through phases in order so that table ends up being sorted
		for _, phase := range []types.VersionDirectivePhase{types.DirectivePhaseActive, types.DirectivePhasePending, types.DirectivePhaseDraft} {
			for _, d := range dm[phase] {
				origin := "-"
				if d.Spec.Origin != "" {
					origin = d.Spec.Origin
				}
				table.AddRow([]string{string(phase), d.GetSubKind(), d.GetName(), string(d.Spec.Status), origin})
			}
		}
		...
		return trace.Wrap(err)
	case teleport.JSON:
		if phaseArg == types.DirectivePhaseNone {
			// print the entire set object, so that output is categorized by phase
			err = utils.WriteJSON(os.Stdout, directives)
			return trace.Wrap(err)
		}

		ds := dm[phaseArg]
		if ds == nil {
			// ensure that we print an empty sequence, rather than nil
			ds = []*types.VersionDirectiveV1{}
		}
	...
	case teleport.YAML:
		if phaseArg == types.DirectivePhaseNone {
			// print the entire set object, so that the output is categorized by phase
			err = utils.WriteYAML(os.Stdout, directives)
			return trace.Wrap(err)
		}
...
func (c *VersionControlCommand) ListInstallers(ctx context.Context, client auth.ClientI) error {
	...
	case teleport.Text:
		...
		for _, i := range installers.LocalScript {
			table.AddRow([]string{string(types.InstallerKindLocalScript), i.GetName(), string(i.Spec.Status)})
		}
		_, err = table.AsBuffer().WriteTo(os.Stdout)
		return trace.Wrap(err)
	case teleport.JSON:
		if kind == types.InstallerKindNone {
			// print the entire set, so that output is categorized by kind
			err = utils.WriteJSON(os.Stdout, installers)
			return trace.Wrap(err)
		}
		err = utils.WriteJSON(os.Stdout, installers.LocalScript)
		return trace.Wrap(err)
	case teleport.YAML:
		if kind == types.InstallerKindNone {
			// print the entire set, so that output is categorized by kind
			err = utils.WriteYAML(os.Stdout, installers)
			return trace.Wrap(err)
		}
		err = utils.WriteYAML(os.Stdout, installers.LocalScript)
		return trace.Wrap(err)
	default:
		return trace.BadParameter("unknown format %q, must be one of [%q, %q]", c.format, teleport.Text, teleport.JSON)
	}

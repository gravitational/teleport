...
func (p *PluginsCommand) initInstallOkta(parent *kingpin.CmdClause) {
	...
	p.install.okta.cmd.
		Flag("assign-okta-requester-role", "If user synchronization is enabled, assign builtin okta-requester role to synchronized users").
		Default("true").
		BoolVar(&p.install.okta.assignOktaRequesterRole)
	p.install.okta.cmd.
...
func (p *PluginsCommand) InstallOkta(ctx context.Context, args installPluginArgs) error {
	...
	settings := &types.PluginOktaSettings{
		...
		SyncSettings: &types.PluginOktaSyncSettings{
			...
			DisableOktaRequesterRole: (oktaSettings.userSync || oktaSettings.scimEnabled) && !oktaSettings.assignOktaRequesterRole,

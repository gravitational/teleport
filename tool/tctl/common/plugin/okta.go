...
func (p *PluginsCommand) InstallOkta(ctx context.Context, args installPluginArgs) error {
	...
	creds, err := generateSCIMCreds(p.install.name, oktaSettings)
	if err != nil {
		return trace.Wrap(err)
	}
	settings := &types.PluginOktaSettings{
		...
		SyncSettings: &types.PluginOktaSyncSettings{
			...
			AppGroupSyncDisabled: !oktaSettings.appGroupSync,
		},
	}
...
func generateSCIMCreds(pluginName string, oktaSettings oktaArgs) ([]*types.PluginStaticCredentialsV1, error) {

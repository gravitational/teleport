...
func (a *awsICInstallArgs) validate(ctx context.Context, auth authClient, log *slog.Logger) error {
	...
	if err := a.validateCredentialInput(ctx, auth, log); err != nil {
...
func (a *awsICInstallArgs) validateCredentialInput(ctx context.Context, integrations integrationGetter, log *slog.Logger) error {
	...
	_, err := integrations.GetIntegration(ctx, a.oidcIntegration)
	if err != nil {
		if !a.forceOIDCIntegration {
			return trace.Wrap(err, "OIDC integration validation failed")
		}
		log.WarnContext(ctx, "Using supplied OIDC integration name even though it failed validation.",
			"error", err.Error())
	}
...
func (p *PluginsCommand) initInstallAWSIC(parent *kingpin.CmdClause) {
	...
	cmd.Flag("force-oidc-integration", "Force the use of the supplied OIDC integration name, even if validation fails.").
		Default("false").
		BoolVar(&p.install.awsIC.forceOIDCIntegration)

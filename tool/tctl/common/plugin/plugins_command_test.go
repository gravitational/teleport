...
func TestPluginsInstallOkta(t *testing.T) {
	...
	testCases := []struct {
		...
		{
			name: "scim-only can disable okta-requester role assignment",
			cmd: PluginsCommand{
				install: pluginInstallArgs{
					name: "okta",
					okta: oktaArgs{
		...
		{
			name: "disable okta-requester role assignment",
			cmd: PluginsCommand{
				install: pluginInstallArgs{
					name: "okta",
					okta: oktaArgs{

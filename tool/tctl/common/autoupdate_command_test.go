...
func TestClientToolsAutoUpdateCommands(t *testing.T) {
	...
	_, err := runAutoUpdateCommand(t, ctx, authClient, []string{"client-tools", "update", "--set-mode=enabled"})
	...
	_, err = runAutoUpdateCommand(t, ctx, authClient, []string{"client-tools", "update", "--set-mode=disabled"})
	require.NoError(t, err)
	...
	_, err = runAutoUpdateCommand(t, ctx, authClient, []string{"client-tools", "update", "--set-target-version=1.2.3"})
	require.NoError(t, err)
	...
	// Test the watch command that returns the target version.
	timeoutCtx, cancel := context.WithTimeout(ctx, time.Second)
	defer cancel()
	watchBuf, err := runAutoUpdateCommand(t, timeoutCtx, authClient, []string{"client-tools", "watch"})
	require.NotErrorIs(t, err, context.Canceled)
	response = mustDecodeJSON[versionResponse](t, watchBuf)
	assert.Equal(t, "1.2.3", response.TargetVersion)
}

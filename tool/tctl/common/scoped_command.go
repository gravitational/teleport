...
func (c *ScopedCommand) Status(ctx context.Context, client *authclient.Client) error {
	...
	for role, err := range scopeutils.RangeScopedRoles(ctx, client.ScopedAccessServiceClient(), &scopedaccessv1.ListScopedRolesRequest{}) {
		if err != nil {
	...
	for assignment, err := range scopeutils.RangeScopedRoleAssignments(ctx, client.ScopedAccessServiceClient(), &scopedaccessv1.ListScopedRoleAssignmentsRequest{}) {
		if err != nil {
			return trace.Wrap(err)
		}
	...
	sortedScopes := slices.SortedFunc(maps.Keys(rows), scopes.SortCmp)

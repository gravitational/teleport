...
func (c *AccessGraphCommand) initAccessReview(parent *kingpin.CmdClause) {
	...
	reviewCmd.Flag("from", fmt.Sprintf("Include activity at or after this time. (Examples: %s, 24h, 7d; negative durations like -1h are future-relative. Default: 30d)", time.RFC3339)).
		Default("30d").
		SetValue(timeValue{target: &c.access.review.from})
	reviewCmd.Flag("to", fmt.Sprintf("Include activity at or before this time. (Examples: %s, 24h, 7d; negative durations like -1h are future-relative. Default: now)", time.RFC3339)).
		Default("now").
...
func (c *AccessGraphCommand) AccessReviewResource(ctx context.Context, client *accessgraph.ClientWithResponses) error {
	search := c.access.review.resource.name
	subjects, err := resolveSubjectNodes(ctx, client, fmt.Sprintf(
	...
	pathNodes, pathEdges, err := queryAccessPath(ctx, client, subjects)
	if err != nil {
		return trace.Wrap(err)
	}
	...
	return c.runReview(ctx, client, "resource", subjects, resourceMap, identityMap)
}

// AccessReviewACL executes `tctl access review acl <name>`.
func (c *AccessGraphCommand) AccessReviewACL(ctx context.Context, client *accessgraph.ClientWithResponses) error {
	...
	return c.reviewIdentityGroup(ctx, client, "acl", subjects)
}

// AccessReviewRole executes `tctl access review role <name>`.
func (c *AccessGraphCommand) AccessReviewRole(ctx context.Context, client *accessgraph.ClientWithResponses) error {
	...
	return c.reviewIdentityGroup(ctx, client, "role", subjects)
}

func (c *AccessGraphCommand) AccessReviewIdentity(ctx context.Context, client *accessgraph.ClientWithResponses) error {
	...
	return c.runReview(ctx, client, "identity", subjects, resourceMap, identityMap)
}
...
func (c *AccessGraphCommand) reviewIdentityGroup(
	...
	return c.runReview(ctx, client, subjectType, subjects, resourceMap, identityMap)
}
...
// how to handle them.
func resolveSubjectNodes(ctx context.Context, client *accessgraph.ClientWithResponses, query string) ([]models.Node, error) {
	slog.DebugContext(ctx, "resolving subject nodes", "query", query)
	rsp, err := doRequest(client.ExecuteQueryV1WithResponse(ctx, &accessgraph.ExecuteQueryV1Params{
		Query: query,
	}))
	if err != nil {
...
func queryAccessPath(ctx context.Context, client *accessgraph.ClientWithResponses, subjects []models.Node) (*[]models.Node, *[]models.Edge, error) {
	if len(subjects) == 0 {
		empty := []models.Node{}
...
func (c *AccessGraphCommand) runReview(
	...
	t := time.Now()
	activity, err := fetchResourceActivity(ctx, client, resourceNodes, identities, c.access.review.from, c.access.review.to)
	if err != nil {
		return trace.Wrap(err)
	}
	...
	if subjectType != "resource" {
		altAccess, err = computeAlternativeAccess(ctx, client, subjects, resourceNodes)
		if err != nil {
			return trace.Wrap(err)
		}
	}
...
func buildSessionActivityQuery(identities []*models.Node, resources []*models.Node) string {
	...
	return fmt.Sprintf("%s AND (%s AND %s)",
		dslClause("identity_id", identityNames),
		dslClause("resource", resourceNames),
		dslClause("event_type", eventTypes),
	)
}
...
func computeAlternativeAccess(
	...
	pathNodes, pathEdges, err := queryAccessPath(ctx, client, resourceNodes)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	g := newTraversalGraph(pathNodes, pathEdges)

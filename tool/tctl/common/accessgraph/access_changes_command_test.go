...
func TestAccessChangesList(t *testing.T) {
	...
	t.Run("request shaping carries filter envelope and search", func(t *testing.T) {
		t.Parallel()
		...
		// type_filter is a JSON-encoded array of one envelope; decode and assert.
		...
		require.NotNil(t, filters[0].OriginType)
...
func TestInitAccessChangesFlags(t *testing.T) {
	...
	t.Run("ls defaults", func(t *testing.T) {
		...
		require.Empty(t, got.ls.kind)
		require.Empty(t, got.ls.typ)
		require.Empty(t, got.ls.source)
	})

	t.Run("ls accepts every filter flag", func(t *testing.T) {
		t.Parallel()
		got, err := parse(t, "access-changes", "ls",
			"--search", "prod",
			"--kind", "resource",
			"--type", "teleport_database",
			"--source", "Teleport",
			"--limit", "250",
		)
		require.NoError(t, err)
		require.Equal(t, "prod", got.ls.search)
		require.Equal(t, "resource", got.ls.kind)
		require.Equal(t, "teleport_database", got.ls.typ)
		require.Equal(t, "Teleport", got.ls.source)
...
func TestConstructAccessChangesListQuery(t *testing.T) {
	...
	t.Run("no filter flags leaves TypeFilter and Search nil", func(t *testing.T) {
		t.Parallel()
	...
	t.Run("single axis populates only that axis in the envelope", func(t *testing.T) {
		t.Parallel()
		args := accessChangesArgs{}
		args.ls.kind = "resource"
		...
		require.NotContains(t, *params.TypeFilter, `"origin_type":`)
		require.NotContains(t, *params.TypeFilter, `"source":`)
	})

	t.Run("multiple axes populate every supplied axis", func(t *testing.T) {
		t.Parallel()
		args := accessChangesArgs{}
		args.ls.kind = "resource"
		args.ls.source = "Teleport"
		args.ls.typ = "teleport_database"
		params, err := constructAccessChangesListQuery(args)
		require.NoError(t, err)
		require.NotNil(t, params.TypeFilter)

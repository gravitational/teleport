...
func TestFetchLogsFacets(t *testing.T) {
	...
	a := &investigateArgs{}
	luceneToFlagMap := a.luceneToFlagMap()
	...
	t.Run("renames + sorts + drops non-filterable columns", func(t *testing.T) {
		...
		facets, total, err := fetchLogsFacets(context.Background(), ag,
			accessgraph.ExecuteLogsStatsQueryV1Params{}, luceneToFlagMap)
...
func TestInvestigate(t *testing.T) {
	...
	t.Run("--limit caps events and surfaces truncated", func(t *testing.T) {
		h := &investigateHandler{
			stats: statsResponse(
				statsColumn{name: "event_type", values: []statsValue{{value: "session.start", count: 2}}},
			),
			// One page with more events than --limit, so fetchAllLogs trims and flags truncated.
			logsPages: []fetchAllLogsPage{{data: []map[string]any{
				{"uuid": "11111111-1111-1111-1111-111111111111"},
...
func TestInitInvestigateFlags(t *testing.T) {
	...
	t.Run("boolean flags flip to true", func(t *testing.T) {
		got, err := parse(t, "investigate", "--show-unmatched", "--facets-only")
		require.NoError(t, err)

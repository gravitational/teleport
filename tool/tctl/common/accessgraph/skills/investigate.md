...
Output (JSON/YAML) is `{total, truncated, facets, data}`:
...
- `truncated` — `true` when more events matched than were returned under `--limit`; raise `--limit` (or pass `0`) for the full set. Always `false` under `--facets-only`.
...
- **Different flags are ANDed.** `--user alice --status failure` matches events from Alice that errored.
...
Run `tctl investigate --help` for the full flag list. Each structured filter `--<flag>` has a matching `--exclude-<flag>`

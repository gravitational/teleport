...
// These constants represent flags for the [userInfo] structure.
const (
	PasswordNotRequired = uint32(0x20)    // UF_PASSWD_NOTREQD
...
type userInfo struct {
	name        *uint16
	password    *uint16
	passwordAge uint32 // ignored by NetUserAdd
	priv        uint32
	homeDir     *uint16 // optional
	comment     *uint16 // optional TODO(probakowski): set to "created by teleport"
	flags       uint32
	scriptPath  *uint16 // optional
}
...
// userLevel1 is the level argument for the NetUserAdd call.
// It indicates that we are providing a USER_INFO_1 structure.
const userLevel1 = uint32(1)

// userPrivUser is a member of the NetUserEnum which indicates
// that we are creating a user account.
const userPrivUser = uint32(1)
...
func ensureUser(name string) error {
	...
	if err = NetUserAdd(nil, /* server name (use local machine) */
		userLevel1,
		&info,
		&invalidFieldIndex); err != nil {
		err = fmt.Errorf("can't create user: %w (field %d)", err, invalidFieldIndex)
		return err
	}
...
var procNetUserAdd = modNetapi32.NewProc("NetUserAdd")

// NetUserAdd creates a local user account.
// This code was generated by mkwinsyscall, but copied out from the generated
// code so that we can expose the return value.
func NetUserAdd(servername *uint16, level uint32, buf *userInfo, errIndex *uint32) error {
	rc, _, e1 := syscall.Syscall6(
		procNetUserAdd.Addr(),
		4,
		uintptr(unsafe.Pointer(servername)),
		uintptr(level),
		uintptr(unsafe.Pointer(buf)),
		uintptr(unsafe.Pointer(errIndex)), 0, 0)
	if rc != 0 {
		return fmt.Errorf("NetUserAdd failure %v: %w", rc, errnoErr(e1))
	}

...
func (p *Plugin) generateAccessListTerraformConfig(_ http.ResponseWriter, r *http.Request, params httprouter.Params, ctx *web.SessionContext) (any, error) {
	...
	accessRoles := make([]terraform.AccessRole, 0, len(req.AccessRoles))
	for _, roleReq := range req.AccessRoles {
		if roleReq.Role == nil {
			return nil, trace.BadParameter("access role is nil")
		}
		accessRole := terraform.AccessRole{
			Role:         roleReq.Role,
			BlockComment: roleReq.BlockComment,
		}
	...
	tfConfig, err := terraform.GenerateConfigWithPresetBuilder(terraform.ConfigParams{
		PresetType:   preset.PresetType(req.PresetType),
	...
	return ui.GenerateAccessListTerraformConfigResponse{Terraform: tfConfig}, nil
}

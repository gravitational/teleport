...
const (
	...
	// defaultReportReadyRerunSchedulerThreshold limits how often report can be re-run by scheduler.
	defaultReportReadyRerunSchedulerThreshold = time.Hour
)
...
func (s *Service) schedulesReportsUpdate(ctx context.Context) error {
	...
	if now.After(t) || now.Equal(t) {
		...
		if err := s.maybeUpdateSecurityReports(ctx, defaultReportReadyRerunSchedulerThreshold); err != nil {

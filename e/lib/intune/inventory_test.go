...
func TestClient_ListManagedDevices(t *testing.T) {
	...
	api.SetManagedDevices([]*intune.ManagedDevice{dev1, dev2, dev3})

	client := env.MustNewClient(t)

	t.Run("list and filter by lastSyncDateTime", func(t *testing.T) {
		resp, err := client.ListManagedDevices(t.Context(), &intune.ListManagedDevicesRequest{})
		require.NoError(t, err)
		require.Len(t, resp.ManagedDevices, 3)
		var lastSyncedDevice *intune.ManagedDevice
		...
		resp2, err := client.ListManagedDevices(t.Context(), &intune.ListManagedDevicesRequest{
	...
	t.Run("pagination", func(t *testing.T) {
		...
		resp3, err := client.ListManagedDevices(t.Context(), &intune.ListManagedDevicesRequest{

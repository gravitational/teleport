...
func getInfoFieldsSAML(diag *types.SSODiagnosticInfo, debug bool) []string {
	return []string{
		tester.GetDiagMessage(
			...
			true,
			tester.FormatYAML("[SAML] Attributes to roles", diag.SAMLAttributesToRoles),
		),
		tester.GetDiagMessage(
		...
		tester.GetDiagMessage(
			...
			true,
			tester.FormatYAML("[SAML] Attributes statements", diag.SAMLAttributeStatements),
		),
		tester.GetDiagMessage(
			diag.SAMLAssertionInfo != nil,
			debug,
			tester.FormatJSON("[SAML] Assertion info", diag.SAMLAssertionInfo),
		),
		tester.GetDiagMessage(
			diag.SAMLTraitsFromAssertions != nil,
			debug,
			tester.FormatJSON("[SAML] Calculated user traits", diag.SAMLTraitsFromAssertions),
		),
		tester.GetDiagMessage(
			diag.SAMLConnectorTraitMapping != nil,
			debug,
			tester.FormatYAML("[SAML] Connector trait mapping", diag.SAMLConnectorTraitMapping),
		),
	}
}

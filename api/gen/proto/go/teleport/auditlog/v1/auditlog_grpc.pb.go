...
type AuditLogServiceClient interface {
	...
	// GetEventExportChunks returns a paginated list of chunks to be exported via ExportUnstructuredEvents for a given
	// date. The returned list isn't necessarily in time order. Subsequent calls may return a different order with additional
	// chunks in it if events for the target date are still being uploaded. Polling for new chunks must consume the entire list.

...
// TestFn tests the IterFn iterator.
func TestFn(t *testing.T) {
	// normal usage
	var n int
	s, err := Collect(IterFn(func() (int, error) {
		n++
		if n > 3 {
			return 0, io.EOF
		}
	...
	// immediate error
	err = Drain(IterFn(func() (int, error) {
...
func TestPageFn(t *testing.T) {
	// basic pages
	var n int
	s, err := Collect(PageFn(func() ([]int, error) {
		n++
		if n > 3 {
			return nil, io.EOF
		}
	...
	// immediate failure
	err = Drain(PageFn(func() ([]int, error) {

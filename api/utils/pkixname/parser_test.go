...
func TestParse(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			// "in" is the exact same as want.String().
		...
		},
		{
			...
			in:   "C=US,C=CA,CN=Multiple Countries",
			want: &pkix.Name{
				Country:    []string{"US", "CA"},
				CommonName: "Multiple Countries",
			},
		},
		{
		...
		},
		{
			...
			in:      `C==`,
			wantErr: "special character '=' not quoted",
		},
		{
			name:    "malformed DN: ",
			in:      `C+1`,
		...
		},
		{
			...
			in:      `CN=Before\0DAfter,O=Test,C=GB`,
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			...
			if test.want != nil {
				t.Logf("PKIX=[%s]", test.want.String())
			}
			...
			if diff := cmp.Diff(test.want, got); diff != "" {
				t.Errorf("ParseDistinguishedName() mismatch (-want +got)\n%s", diff)
			}
		})
	}
}

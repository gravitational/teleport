...
var (
	cachedKeys   = map[piv.Slot]*YubiKeyPrivateKey{}
...
func getOrGenerateYubiKeyPrivateKey(ctx context.Context, requiredKeyPolicy PrivateKeyPolicy, slot hardwarekey.PIVSlotKeyString, prompt hardwarekey.Prompt) (*YubiKeyPrivateKey, error) {
	if prompt == nil {
		prompt = hardwarekey.NewStdCLIPrompt()
	}

	promptPolicy := requiredKeyPolicy.GetPromptPolicy()

	// Get the requested or default PIV slot.
	var slotKey hardwarekey.PIVSlotKey
	var err error
	if slot != "" {
		slotKey, err = slot.Parse()
	} else {
		slotKey, err = hardwarekey.GetDefaultSlotKey(promptPolicy)
	}
	if err != nil {
	...
	priv, err := y.newYubiKeyPrivateKey(pivSlot)
	switch {
...
func (y *YubiKeyPrivateKey) sign(ctx context.Context, rand io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	...
	promptPIN := func() (string, error) {
		...
		pass, err := y.prompt.AskPIN(ctx, hardwarekey.PINRequired)
...
func (y *YubiKey) generatePrivateKeyAndCert(slot piv.Slot, requiredKeyPolicy PrivateKeyPolicy) (*YubiKeyPrivateKey, error) {
	...
	return y.newYubiKeyPrivateKey(slot)
}
...
func (y *YubiKey) getPrivateKey(slot piv.Slot) (*YubiKeyPrivateKey, error) {
	...
	if key, ok := cachedKeys[slot]; ok {
		return key, nil
	}

	priv, err := y.newYubiKeyPrivateKey(slot)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	cachedKeys[slot] = priv
	return priv, nil
}

// newYubiKeyPrivateKey prepares a [YubiKeyPrivateKey] from an existing private key in the given PIV slot.
// This method must be called under [cachedKeysMu] lock.
func (y *YubiKey) newYubiKeyPrivateKey(slot piv.Slot) (*YubiKeyPrivateKey, error) {
...
func (c *sharedPIVConnection) reset() error {
	...
	cachedKeysMu.Lock()
	defer cachedKeysMu.Unlock()
	cachedKeys = make(map[piv.Slot]*YubiKeyPrivateKey)

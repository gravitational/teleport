...
func getOrGenerateYubiKeyPrivateKey(ctx context.Context, requiredKeyPolicy PrivateKeyPolicy, slot string) (*PrivateKey, error) {
	...
	// First, check if there is already a private key on the slot. If a specific slot was
	// not provided, check for a teleport-client certificate in the slot as well.
	...
	case trace.IsNotFound(err):
		...
		if err = y.generatePrivateKey(ctx, pivSlot, requiredKeyPolicy); err != nil {
			return nil, trace.Wrap(err)
		}
		priv, err = y.getPrivateKey(pivSlot, requiredKeyPolicy, checkCert)
	}
	...
	if priv.attestation.PINPolicy != piv.PINPolicyNever {
		if slot != "" {
			return nil, trace.NotImplemented(`PIN policy is not currently supported. Please generate a key with PIN policy "never"`)
		} else {
			return nil, trace.BadParameter(`PIN policy is not currently supported. Please reset your PIV slot with "ykman piv reset" and re-run this command to generate a supported PIV key.`)
		}
	}
...
func (y *yubiKey) generatePrivateKey(ctx context.Context, slot piv.Slot, requiredKeyPolicy PrivateKeyPolicy) error {
	yk, err := y.open()
	if err != nil {

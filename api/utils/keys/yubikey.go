...
var (
	cachedKeys   = map[piv.Slot]*hardwarekey.Signer{}
	cachedKeysMu sync.Mutex
)

func newPrivateKey(ctx context.Context, config hardwarekey.PrivateKeyConfig, prompt hardwarekey.Prompt, s hardwarekey.Service) (*hardwarekey.Signer, error) {
	...
	if config.CustomSlot == "" {
		...
		case len(cert.Subject.Organization) == 0 || cert.Subject.Organization[0] != certOrgName:
			if err := promptOverwriteSlot(ctx, prompt, nonTeleportCertificateMessage(pivSlot, cert)); err != nil {
				return nil, trace.Wrap(err)
			}
			return generatePrivateKey()
		}
	...
	case config.Policy.TouchRequired && att.TouchPolicy == piv.TouchPolicyNever:
		...
		if err := promptOverwriteSlot(ctx, prompt, msg); err != nil {
			return nil, trace.Wrap(err)
		}
		return generatePrivateKey()
...
func promptOverwriteSlot(ctx context.Context, prompt hardwarekey.Prompt, msg string) error {
...
func (c *sharedPIVConnection) reset() error {
	...
	cachedKeysMu.Lock()
	defer cachedKeysMu.Unlock()
	cachedKeys = make(map[piv.Slot]*hardwarekey.Signer)

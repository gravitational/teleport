...
func (c *AgentClient) NewPrivateKey(ctx context.Context, config hardwarekey.PrivateKeyConfig) (*hardwarekey.PrivateKey, error) {
	return c.directService.NewPrivateKey(ctx, config)
}

// Sign performs a cryptographic signature using the specified hardware
// private key and provided signature parameters.
func (c *AgentClient) Sign(ctx context.Context, ref *hardwarekey.PrivateKeyRef, rand io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	signature, err := c.agentSign(ctx, ref, rand, digest, opts)
	if err != nil {
		...
		signature, err = c.directService.Sign(ctx, ref, rand, digest, opts)
	}

	return signature, trace.Wrap(err)
}

func (c *AgentClient) agentSign(ctx context.Context, ref *hardwarekey.PrivateKeyRef, _ io.Reader, digest []byte, opts crypto.SignerOpts) ([]byte, error) {
	...
	req := &hardwarekeyagentv1.SignRequest{
		...
		KeyInfo: &hardwarekeyagentv1.KeyInfo{
			...
			ProxyHost:     ref.ContextualKeyInfo.ProxyHost,
		},
	}
...
// SetPrompt sets the hardware key prompt used by the direct service.
func (c *AgentClient) SetPrompt(p hardwarekey.Prompt) {
	c.directService.SetPrompt(p)
}

...
func (s *service) Sign(ctx context.Context, req *hardwarekeyagentv1.SignRequest) (*hardwarekeyagentv1.Signature, error) {
	keyRef, err := getPrivateKeyRef(req.KeyRef, req.KeyInfo)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	...
	signature, err := s.s.Sign(ctx, keyRef, rand.Reader, req.Digest, signerOpts)
	if err != nil {
		return nil, trace.Wrap(err)
	}
...
func getPrivateKeyRef(keyRef *hardwarekeyagentv1.KeyRef, keyInfo *hardwarekeyagentv1.KeyInfo) (*hardwarekey.PrivateKeyRef, error) {
	slotKey, err := pivSlotKeyFromProto(keyRef.SlotKey)
	if err != nil {
		return nil, trace.Wrap(err)
	}

	pub, err := x509.ParsePKIXPublicKey(keyRef.PublicKeyDer)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	...
	return &hardwarekey.PrivateKeyRef{
		...
		Policy: hardwarekey.PromptPolicy{
			TouchRequired: keyInfo.TouchRequired,
			PINRequired:   keyInfo.PinRequired,
		},

...
func (s *fakeYubiKeyPIVService) NewPrivateKey(ctx context.Context, config hardwarekey.PrivateKeyConfig) (*hardwarekey.PrivateKey, error) {
	s.hardwarePrivateKeysMux.Lock()
	defer s.hardwarePrivateKeysMux.Unlock()
	...
	if priv, ok := s.hardwarePrivateKeys[keySlot]; ok {
	...
	s.hardwarePrivateKeys[keySlot] = &fakeHardwarePrivateKey{
		Signer: priv,
		ref:    ref,
	}
...
func (s *fakeYubiKeyPIVService) Sign(ctx context.Context, ref *hardwarekey.PrivateKeyRef, _ hardwarekey.ContextualKeyInfo, rand io.Reader, digest []byte, opts crypto.SignerOpts) (signature []byte, err error) {
	...
	priv, ok := s.hardwarePrivateKeys[hardwareKeySlot{
		serialNumber: serialNumber,
		slot:         ref.SlotKey,
	}]
	if !ok {
...
func (s *fakeYubiKeyPIVService) SetPINCacheTimeout(timeout time.Duration) {}

// TODO(Joerger): DELETE IN v19.0.0
func (s *fakeYubiKeyPIVService) GetMissingKeyRefDetails(ref *hardwarekey.PrivateKeyRef) error {
	s.hardwarePrivateKeysMux.Lock()
	defer s.hardwarePrivateKeysMux.Unlock()

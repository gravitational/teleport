...
func ParsePrivateKey(keyPEM []byte) (*PrivateKey, error) {
	//TODO(gzdunek): ParsePrivateKey has 60 usages, so I created ParsePrivateKeyWithPrompt
	return ParsePrivateKeyWithPrompt(keyPEM, nil)
}

// ParsePrivateKeyWithPrompt returns the PrivateKey for the given key PEM block.
func ParsePrivateKeyWithPrompt(keyPEM []byte, prompt HardwareKeyPrompt) (*PrivateKey, error) {
	...
	case pivYubiKeyPrivateKeyType:
		priv, err := parseYubiKeyPrivateKeyData(block.Bytes, prompt)
		if err != nil {
...
func LoadKeyPair(privFile, sshPubFile string, prompt HardwareKeyPrompt) (*PrivateKey, error) {
	...
	priv, err := ParseKeyPair(privPEM, marshaledSSHPub, prompt)
	if err != nil {
		return nil, trace.Wrap(err)
	}
	return priv, nil
...
func ParseKeyPair(privPEM, marshaledSSHPub []byte, prompt HardwareKeyPrompt) (*PrivateKey, error) {
	priv, err := ParsePrivateKeyWithPrompt(privPEM, prompt)
	if err != nil {
		return nil, trace.Wrap(err)
	}

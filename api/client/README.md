...
These TLS certificates must be associated with a Teleport user.

Once authenticated, the client will be authorized to call client methods that the underlying user has
permissions for.

For example, the client user must have permission to perform `read` action on the `role` resource in order to call:
```go
...
Teleport clients use Credentials in order to perform TLS authentication, connect to proxy servers over SSH, and perform some other actions, depending on the Credentials used. These Credentials are created by using Credential loaders, which gather certificates and data generated by [Teleport CLIs](https://goteleport.com/docs/cli-docs/).
...
For supplementary documentation about using Teleport, visit the main Teleport [docs](https://goteleport.com/docs/).

...
func (c *Client) CreateAccessPolicy(ctx context.Context, policy types.AccessPolicy) error {
	_, err := c.grpc.CreateAccessPolicy(ctx, policy.(*types.AccessPolicyV1), c.callOpts...)
	return trail.FromGRPC(err)
}
...
func (c *Client) GetAccessPolicy(ctx context.Context, name string) (types.AccessPolicy, error) {
	resp, err := c.grpc.GetAccessPolicy(ctx, &proto.GetPolicyRequest{Name: name}, c.callOpts...)
	if err != nil {
		return nil, trail.FromGRPC(err)
	}

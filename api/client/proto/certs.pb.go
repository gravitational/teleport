...
type Certs struct {
	...
	// SSHUserCACerts is a list of SSH user certificate authorities.
	SSHUserCACerts       [][]byte `protobuf:"bytes,5,rep,name=SSHUserCACerts,proto3" json:"ssh_user_ca_certs,omitempty"`
...
var fileDescriptor_9c4dbdba9a1559ee = []byte{
	...
	0x2a, 0x4d, 0x62, 0xe2, 0x62, 0x75, 0x06, 0x29, 0x16, 0x52, 0xe6, 0x62, 0x0e, 0x0e, 0xf6, 0x90,
	...
	0x2f, 0x42, 0xb5, 0x90, 0x3b, 0x17, 0x5f, 0x70, 0xb0, 0x47, 0x68, 0x71, 0x6a, 0x11, 0x4c, 0x3f,
	0x2b, 0x58, 0xbf, 0xfc, 0xab, 0x7b, 0xf2, 0xd2, 0x20, 0xfd, 0xa5, 0xc5, 0xa9, 0x45, 0xd8, 0x0c,
	0x41, 0xd3, 0xe6, 0xe4, 0x72, 0xe2, 0xa1, 0x1c, 0xc3, 0x89, 0x47, 0x72, 0x8c, 0x17, 0x1e, 0xc9,
	0x31, 0x3e, 0x78, 0x24, 0xc7, 0x18, 0x65, 0x94, 0x9e, 0x59, 0x92, 0x51, 0x9a, 0xa4, 0x97, 0x9c,
	0x9f, 0xab, 0x9f, 0x5e, 0x94, 0x58, 0x96, 0x59, 0x92, 0x58, 0x92, 0x99, 0x9f, 0x97, 0x98, 0xa3,
	0x0f, 0x8f, 0x86, 0xc4, 0x82, 0x4c, 0x94, 0x38, 0x48, 0x62, 0x03, 0x53, 0xc6, 0x80, 0x00, 0x00,
	0x00, 0xff, 0xff, 0x1f, 0xc2, 0x5e, 0xf5, 0xaa, 0x01, 0x00, 0x00,
}
...
func (m *Certs) MarshalToSizedBuffer(dAtA []byte) (int, error) {
	...
	if len(m.SSHUserCACerts) > 0 {
		for iNdEx := len(m.SSHUserCACerts) - 1; iNdEx >= 0; iNdEx-- {
			i -= len(m.SSHUserCACerts[iNdEx])
			copy(dAtA[i:], m.SSHUserCACerts[iNdEx])
			i = encodeVarintCerts(dAtA, i, uint64(len(m.SSHUserCACerts[iNdEx])))
			i--
			dAtA[i] = 0x2a
		}
	}
...
func (m *Certs) Size() (n int) {
	...
	if len(m.SSHUserCACerts) > 0 {
		for _, b := range m.SSHUserCACerts {
			l = len(b)
			n += 1 + l + sovCerts(uint64(l))
		}
	}
...
func (m *Certs) Unmarshal(dAtA []byte) error {
	...
	for iNdEx < l {
		...
		case 5:
			if wireType != 2 {
				return fmt.Errorf("proto: wrong wireType = %d for field SSHUserCACerts", wireType)
			}
			var byteLen int
			for shift := uint(0); ; shift += 7 {
				if shift >= 64 {
			...
			m.SSHUserCACerts = append(m.SSHUserCACerts, make([]byte, postIndex-iNdEx))
			copy(m.SSHUserCACerts[len(m.SSHUserCACerts)-1], dAtA[iNdEx:postIndex])
			iNdEx = postIndex
		default:
			iNdEx = preIndex

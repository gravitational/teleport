...
const (
	...
	// DirectiveStatusOk indicates that the directive is safe to promote and enforce.
	DirectiveStatusOk VersionDirectiveStatus = "ok"
...
func (s VersionDirectiveStatus) IsKnownStatus() bool {
	switch s {
	case DirectiveStatusNone, DirectiveStatusOk, DirectiveStatusLocked, DirectiveStatusPoisoned:
		return true
	default:
		return false
	}
...
// Iter iterates directives in "display order" (active -> pending -> draft).
func (s *VersionDirectiveSet) Iter(fn func(d VersionDirective)) {
	if s.Active != nil {
		fn(s.Active)
	}
...
type VersionDirective interface {
	...
	// GetOrigin gets the origin string of the directive.
	GetOrigin() string
...
func (d *VersionDirectiveV1) CheckAndSetDefaults() error {
	...
	if d.Spec.Status == DirectiveStatusNone {
		d.Spec.Status = DirectiveStatusOk
	}
	...
	if !d.Spec.Schedule.NotBefore.IsZero() {
		d.Spec.Schedule.NotBefore = d.Spec.Schedule.NotBefore.UTC()
	}

	if !d.Spec.Schedule.NotAfter.IsZero() {
...
func (d *VersionDirectiveV1) WithNonce(nonce uint64) any {
	shallowCopy := *d
	shallowCopy.Spec.Nonce = nonce
	return &shallowCopy
}

func (d *VersionDirectiveV1) GetOrigin() string {
	return d.Spec.Origin
}

...
// CheckAndSetDefaults checks and sets default valu
// and rules) are equal and false if they are not.
func (i ImpersonateConditions) CheckAndSetDefaults() error {
	if len(i.Users) != 0 && len(i.Roles) == 0 {
		return trace.BadParameter("impersonate.users is set without any impersonate.roles, this role will never match")
	}
	if len(i.Users) == 0 && len(i.Roles) != 0 {
		return trace.BadParameter("impersonate.roles is set without any impersonate.users, this role will never match")
	}
	return nil
}

...
// meaning.
var IsLabelLiteral = regexp.MustCompile(`^[a-zA-Z0-9\.\-_\/:]+$`).MatchString
...
// Convert an installer set to a map of the form kind -> intallers.  Useful for iteration purposes.
func (s *VersionControlInstallerSet) AsMap() map[VersionControlInstallerKind][]VersionControlInstaller {
	m := make(map[VersionControlInstallerKind][]VersionControlInstaller)
	for _, installer := range s.LocalScript {
		m[InstallerKindLocalScript] = append(m[InstallerKindLocalScript], installer)
	}
	return m
}
...
const (
	...
	// InstallerStatusEnabled indicates that the installer can be used.
	InstallerStatusEnabled VersionControlInstallerStatus = "enabled"

	// InstallerStatusDisabled indicates that the installer should not be used.
	InstallerStatusDisabled VersionControlInstallerStatus = "disabled"
)
...
func (s VersionControlInstallerStatus) IsKnownStatus() bool {
	switch s {
	case InstallerStatusNone:
	case InstallerStatusEnabled:
	case InstallerStatusDisabled:
	default:
		return false
	}
...
func (i *LocalScriptInstallerV1) WithNonce(nonce uint64) interface{} {
	shallowCopy := *i
...
func (i *LocalScriptInstallerV1) CheckAndSetDefaults() error {
	...
	if i.Spec.Status == InstallerStatusNone {
		i.Spec.Status = InstallerStatusEnabled
	}

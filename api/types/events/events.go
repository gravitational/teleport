...
func trimBytes(s []byte, n int) []byte {
	return []byte(trimStr(string(s), n))
}
...
// nonEmptyBytes returns the number of non-empty []byte.
func nonEmptyBytes(s ...[]byte) int {
	nonEmptyStrs := 0
	for _, byteSlice := range s {
		if len(byteSlice) != 0 {
			nonEmptyStrs++
		}
	}
...
func calculateEventMaxFieldSize(out AuditEvent, maxSize int, nonEmptyStrsCounts ...int) int {
	maxSize = adjustedMaxSize(out, maxSize)
	var customFieldsCount int
	for _, count := range nonEmptyStrsCounts {
		customFieldsCount += count
	}
	return maxSizePerField(maxSize, customFieldsCount)
}
...
func (m *UserLogin) TrimToMaxSize(maxSize int) AuditEvent {
	...
	out.Status = m.Status.trimToMaxFieldSize(maxFieldSize)
...
func (m *MCPSessionEnd) TrimToMaxSize(maxSize int) AuditEvent {
	...
	maxFieldSize := calculateEventMaxFieldSize(out, maxSize,
		m.Status.nonEmptyStrs(),
		nonEmptyTraits(m.Headers),
	)
	out.Status = m.Status.trimToMaxFieldSize(maxFieldSize)
	out.Headers = trimTraits(m.Headers, maxFieldSize)
	return out
}
...
func (m *MCPSessionRequest) TrimToMaxSize(maxSize int) AuditEvent {
	...
	maxFieldSize := calculateEventMaxFieldSize(out, maxSize,
		m.Message.nonEmptyStrs(),
		m.Status.nonEmptyStrs(),
		nonEmptyTraits(m.Headers),
	)
	out.Message = m.Message.trimToMaxFieldSize(maxFieldSize)
	out.Status = m.Status.trimToMaxFieldSize(maxFieldSize)
	out.Headers = trimTraits(m.Headers, maxFieldSize)
	return out
}
...
func (m *MCPSessionInvalidHTTPRequest) TrimToMaxSize(maxSize int) AuditEvent {
	...
	maxFieldSize := calculateEventMaxFieldSize(out, maxSize,
		nonEmptyStrs(m.Path, m.RawQuery),
		nonEmptyTraits(m.Headers),
		nonEmptyBytes(m.Body),
	)

	out.Path = trimStr(m.Path, maxFieldSize)
	out.RawQuery = trimStr(m.RawQuery, maxFieldSize)
	out.Headers = trimTraits(m.Headers, maxFieldSize)
	out.Body = trimBytes(m.Body, maxFieldSize)
	return out
}

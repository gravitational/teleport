...
// TestDatabaseIsElastiCache verifies if the MemoryDB URI is correctly recognized.
func TestDatabaseIsElastiCache(t *testing.T) {
	database, err := NewDatabaseV3(Metadata{
		Name: "test",
	}, DatabaseSpecV3{
		Protocol: "redis",
		URI:      "redis://clustercfg.test-instance.dwudvg.use1.cache.amazonaws.com:6379?mode=cluster",
	})
	require.NoError(t, err)

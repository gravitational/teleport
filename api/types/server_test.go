...
func TestServerAzureLabels(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.name, func(t *testing.T) {
			t.Run("prefers visible labels", func(t *testing.T) {
				...
				require.Equal(t, test.visibleValue, test.getter(server))
			})

			t.Run("falls back to internal labels", func(t *testing.T) {
				server := &ServerV2{Metadata: Metadata{Labels: map[string]string{
					test.internalKey: test.internalValue,
				}}}
			...
			t.Run("skips empty visible labels", func(t *testing.T) {
				server := &ServerV2{Metadata: Metadata{Labels: map[string]string{
					test.visibleKey:  "",
					test.internalKey: test.internalValue,
				}}}

...
// IsSemanticallyEqual returns whether the Access List is equal to another Access List, ignoring
// ephemeral fields such as `Status`, `Metadata.Revision`, and `IneligibleStatus` on Owners.
func (a *AccessList) IsSemanticallyEqual(b *AccessList) bool {
	if a == nil || b == nil {
		return a == b
	}
...
func clearEphemeralFields(a *AccessList) *AccessList {
	...
	for i, owner := range aCopy.Spec.Owners {
		owner.IneligibleStatus = ""
		aCopy.Spec.Owners[i] = owner
	}
	return aCopy
}

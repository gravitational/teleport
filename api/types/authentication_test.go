...
func TestEncodeMFAFlowMode(t *testing.T) {
	for _, tt := range []struct {
		name        string
		...
		// Negative test cases
		{
			...
			mfaFlowMode: MFAFlowMode(-1),
			wantErr:     true,
		},
		{
			name:        "InvalidLarge",
			mfaFlowMode: MFAFlowMode(999999999),
			wantErr:     true,
		},
	} {
		t.Run(tt.name, func(t *testing.T) {
...
func TestDecodeMFAFlowMode(t *testing.T) {
	for _, tt := range []struct {
		name        string
		mfaFlowMode MFAFlowMode
		encoded     any

...
// MapListResourcesResultToLeafResource maps a listed resource back to the
// requested leaf kind, unwrapping server wrappers (AppServer->App,
// DatabaseServer->Database, KubeServer->KubeCluster). hint is the desired
// kind, used to disambiguate SSH nodes from kubernetes services, which are
